# products/dogan-ai — Dogan-AI (Tuwaiq AI)
...
- ✅ `productCode: "dogan-ai"` is registered in the platform's product layer.
...
The Tuwaiq AI surface (planned domain: `https://tuwaiq-ai.com/`). Detailed
scope, tenant model, module enrollment, and agent topology are deferred to
the product owner's first commit (the alpha promotion).
...
|---|----------|------|--------|
| 1 | App scaffold | `products/dogan-ai/app/` | ❌ |
| 2 | Website scaffold (optional) | `products/dogan-ai/website/` | ❌ |
...
| 6 | Dynamic-UI enrollment file | `products/dogan-ai/dynamic-ui/enrollment.json` | ❌ |
| 7 | Agent enrollment file | `products/dogan-ai/agents/enrollment.json` | ❌ |
| 8 | `package.json` (when `app/` lands) | `products/dogan-ai/app/package.json` | ❌ |
...
`productCode: "dogan-ai"` — kebab-case per
`platform/contracts/product/product.manifest.schema.json`
(`pattern: "^[a-z][a-z0-9-]{1,63}$"`). The Tuwaiq AI brand surfaces in
`displayName: "Dogan-AI (Tuwaiq AI)"`, never in `productCode`.
