...
## Suggested API ↔ page mapping (representative)
...
| workspace-home / dashboard | `GET /workspace-home`, `GET /dashboard` (if exists) |
...
For the full list of routed components, see `src/app/app.routes.ts`. Apply the same pattern: inject `GrcService` (or `HttpClient` if no shared base), call the appropriate backend path in `ngOnInit`, and bind the response in the template.
