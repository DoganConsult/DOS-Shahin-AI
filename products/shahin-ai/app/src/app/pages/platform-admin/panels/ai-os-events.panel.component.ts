...
@Component({
  selector: 'app-platform-admin-ai-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="AI model registry — Events"
      subtitle="Source: dos.ai_event (append-only ledger, 2026-05-04)."
      testid="panel-ai-os-events">
      <cds-table [model]="model" data-testid="ai-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminAiOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/ai-os/events');
