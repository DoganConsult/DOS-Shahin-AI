...
@Component({
  selector: 'app-platform-admin-marketplace-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Marketplace listing — Records"
      subtitle="Source: dos.marketplace_record (controlled by trg_dos_master_only_marketplace_record)."
      testid="panel-marketplace-os-records">
      <cds-table [model]="model" data-testid="marketplace-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminMarketplaceOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/marketplace-os/records');
