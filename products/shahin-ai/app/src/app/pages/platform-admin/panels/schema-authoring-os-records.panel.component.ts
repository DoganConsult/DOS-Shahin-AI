...
@Component({
  selector: 'app-platform-admin-schema-authoring-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Schema artefact — Records"
      subtitle="Source: dos.schema_authoring_record (controlled by trg_dos_master_only_schema_authoring_record)."
      testid="panel-schema-authoring-os-records">
      <cds-table [model]="model" data-testid="schema-authoring-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminSchemaAuthoringOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/schema-authoring-os/records');
