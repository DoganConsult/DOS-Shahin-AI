...
@Component({
  selector: 'app-platform-admin-marketplace-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Marketplace listing — Events"
      subtitle="Source: dos.marketplace_event (append-only ledger, 2026-05-04)."
      testid="panel-marketplace-os-events">
      <cds-table [model]="model" data-testid="marketplace-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminMarketplaceOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/marketplace-os/events');
