...
@Component({
  selector: 'app-platform-admin-feature-flag-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Feature flag — Events"
      subtitle="Source: dos.feature_flag_event (append-only ledger, 2026-05-04)."
      testid="panel-feature-flag-os-events">
      <cds-table [model]="model" data-testid="feature-flag-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminFeatureFlagOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/feature-flag-os/events');
