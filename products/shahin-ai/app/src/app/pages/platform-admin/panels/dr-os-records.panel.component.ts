...
@Component({
  selector: 'app-platform-admin-dr-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Disaster recovery plan — Records"
      subtitle="Source: dos.dr_record (controlled by trg_dos_master_only_dr_record)."
      testid="panel-dr-os-records">
      <cds-table [model]="model" data-testid="dr-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminDrOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/dr-os/records');
