...
@Component({
  selector: 'app-platform-admin-notification-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Notification template — Events"
      subtitle="Source: dos.notification_event (append-only ledger, 2026-05-04)."
      testid="panel-notification-os-events">
      <cds-table [model]="model" data-testid="notification-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminNotificationOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/notification-os/events');
