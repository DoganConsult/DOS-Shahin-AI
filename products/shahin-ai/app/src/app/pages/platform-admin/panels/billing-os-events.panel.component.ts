...
@Component({
  selector: 'app-platform-admin-billing-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Plan + price book — Events"
      subtitle="Source: dos.billing_event (append-only ledger, 2026-05-04)."
      testid="panel-billing-os-events">
      <cds-table [model]="model" data-testid="billing-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminBillingOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/billing-os/events');
