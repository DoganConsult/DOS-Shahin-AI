...
@Component({
  selector: 'app-platform-admin-security-secrets-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Secret metadata + rotation — Records"
      subtitle="Source: dos.security_secret_record (controlled by trg_dos_master_only_security_secret_record)."
      testid="panel-security-secrets-os-records">
      <cds-table [model]="model" data-testid="security-secrets-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminSecuritySecretsOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/security-secrets-os/records');
