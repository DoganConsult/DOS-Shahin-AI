...
@Component({
  selector: 'app-platform-admin-integration-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Connector definition — Events"
      subtitle="Source: dos.integration_event (append-only ledger, 2026-05-04)."
      testid="panel-integration-os-events">
      <cds-table [model]="model" data-testid="integration-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminIntegrationOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/integration-os/events');
