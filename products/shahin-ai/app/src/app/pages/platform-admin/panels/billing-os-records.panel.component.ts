...
@Component({
  selector: 'app-platform-admin-billing-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Plan + price book — Records"
      subtitle="Source: dos.billing_record (controlled by trg_dos_master_only_billing_record)."
      testid="panel-billing-os-records">
      <cds-table [model]="model" data-testid="billing-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminBillingOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/billing-os/records');
