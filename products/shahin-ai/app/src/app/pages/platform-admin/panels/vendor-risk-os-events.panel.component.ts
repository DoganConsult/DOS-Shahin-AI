...
@Component({
  selector: 'app-platform-admin-vendor-risk-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Vendor risk record — Events"
      subtitle="Source: dos.vendor_risk_event (append-only ledger, 2026-05-04)."
      testid="panel-vendor-risk-os-events">
      <cds-table [model]="model" data-testid="vendor-risk-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminVendorRiskOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/vendor-risk-os/events');
