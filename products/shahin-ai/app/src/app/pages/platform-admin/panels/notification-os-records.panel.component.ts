...
@Component({
  selector: 'app-platform-admin-notification-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Notification template — Records"
      subtitle="Source: dos.notification_record (controlled by trg_dos_master_only_notification_record)."
      testid="panel-notification-os-records">
      <cds-table [model]="model" data-testid="notification-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminNotificationOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/notification-os/records');
