...
@Component({
  selector: 'app-platform-admin-vendor-risk-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Vendor risk record — Records"
      subtitle="Source: dos.vendor_risk_record (controlled by trg_dos_master_only_vendor_risk_record)."
      testid="panel-vendor-risk-os-records">
      <cds-table [model]="model" data-testid="vendor-risk-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminVendorRiskOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/vendor-risk-os/records');
