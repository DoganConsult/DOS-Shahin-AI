...
@Component({
  selector: 'app-platform-admin-integration-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Connector definition — Records"
      subtitle="Source: dos.integration_record (controlled by trg_dos_master_only_integration_record)."
      testid="panel-integration-os-records">
      <cds-table [model]="model" data-testid="integration-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminIntegrationOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/integration-os/records');
