...
@Component({
  selector: 'app-platform-admin-dr-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Disaster recovery plan — Events"
      subtitle="Source: dos.dr_event (append-only ledger, 2026-05-04)."
      testid="panel-dr-os-events">
      <cds-table [model]="model" data-testid="dr-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminDrOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/dr-os/events');
