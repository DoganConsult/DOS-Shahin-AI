...
@Component({
  selector: 'app-platform-admin-schema-authoring-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Schema artefact — Events"
      subtitle="Source: dos.schema_authoring_event (append-only ledger, 2026-05-04)."
      testid="panel-schema-authoring-os-events">
      <cds-table [model]="model" data-testid="schema-authoring-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminSchemaAuthoringOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/schema-authoring-os/events');
