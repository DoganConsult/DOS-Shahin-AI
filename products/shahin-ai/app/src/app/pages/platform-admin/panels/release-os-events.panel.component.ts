...
@Component({
  selector: 'app-platform-admin-release-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Release train + gate — Events"
      subtitle="Source: dos.release_event (append-only ledger, 2026-05-04)."
      testid="panel-release-os-events">
      <cds-table [model]="model" data-testid="release-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminReleaseOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/release-os/events');
