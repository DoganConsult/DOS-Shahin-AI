...
@Component({
  selector: 'app-platform-admin-data-governance-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Classification policy — Records"
      subtitle="Source: dos.data_governance_record (controlled by trg_dos_master_only_data_governance_record)."
      testid="panel-data-governance-os-records">
      <cds-table [model]="model" data-testid="data-governance-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminDataGovernanceOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/data-governance-os/records');
