...
@Component({
  selector: 'app-platform-admin-feature-flag-os-records',
  standalone: true,
  ...
  template: `
    <app-admin-panel-frame #frame
      ...
      subtitle="Source: dos.feature_flag_record (controlled by trg_dos_master_only_feature_flag_record)."
      testid="panel-feature-flag-os-records">
      <cds-table [model]="model" data-testid="feature-flag-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminFeatureFlagOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/feature-flag-os/records');
