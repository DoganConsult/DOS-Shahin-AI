...
@Component({
  selector: 'app-platform-admin-release-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Release train + gate — Records"
      subtitle="Source: dos.release_record (controlled by trg_dos_master_only_release_record)."
      testid="panel-release-os-records">
      <cds-table [model]="model" data-testid="release-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminReleaseOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/release-os/records');
