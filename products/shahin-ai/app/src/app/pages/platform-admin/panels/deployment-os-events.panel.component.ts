...
@Component({
  selector: 'app-platform-admin-deployment-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Deployment artefact — Events"
      subtitle="Source: dos.deployment_event (append-only ledger, 2026-05-04)."
      testid="panel-deployment-os-events">
      <cds-table [model]="model" data-testid="deployment-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminDeploymentOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/deployment-os/events');
