...
@Component({
  selector: 'app-platform-admin-data-governance-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Classification policy — Events"
      subtitle="Source: dos.data_governance_event (append-only ledger, 2026-05-04)."
      testid="panel-data-governance-os-events">
      <cds-table [model]="model" data-testid="data-governance-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminDataGovernanceOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/data-governance-os/events');
