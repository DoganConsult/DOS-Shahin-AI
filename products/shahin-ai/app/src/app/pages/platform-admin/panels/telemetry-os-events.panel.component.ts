...
@Component({
  selector: 'app-platform-admin-telemetry-os-events',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Telemetry sink + alert rule — Events"
      subtitle="Source: dos.telemetry_event (append-only ledger, 2026-05-04)."
      testid="panel-telemetry-os-events">
      <cds-table [model]="model" data-testid="telemetry-os-events-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminTelemetryOsEventsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/telemetry-os/events');
