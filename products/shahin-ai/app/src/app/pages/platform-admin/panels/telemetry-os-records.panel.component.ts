...
@Component({
  selector: 'app-platform-admin-telemetry-os-records',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, AdminPanelFrameComponent, TableModule],
  template: `
    <app-admin-panel-frame #frame
      title="Telemetry sink + alert rule — Records"
      subtitle="Source: dos.telemetry_record (controlled by trg_dos_master_only_telemetry_record)."
      testid="panel-telemetry-os-records">
      <cds-table [model]="model" data-testid="telemetry-os-records-table"></cds-table>
    </app-admin-panel-frame>
  `,
})
export class PlatformAdminTelemetryOsRecordsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.frame.bind(async () => {
      const r = await this.api.get<Payload>('/telemetry-os/records');
