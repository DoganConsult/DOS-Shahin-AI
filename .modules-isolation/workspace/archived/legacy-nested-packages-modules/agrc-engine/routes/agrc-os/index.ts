...
import { tenantGuard, subscriptionStatusGuard } from '@dos/platform-core/http';
...
routes.get('/info', (_req: Request, res: Response) => {
  res.json({ service: 'gateway', version: '0.1.0' });
});

routes.get('/metrics', async (_req: Request, res: Response) => {
  try {
    const obs = await import('@dos/platform-core/observability' as string);
...
try {
  const publicContent = require('../domain/public/public-content.routes');
...
routes.get('/admin', (_req: Request, res: Response) => {
