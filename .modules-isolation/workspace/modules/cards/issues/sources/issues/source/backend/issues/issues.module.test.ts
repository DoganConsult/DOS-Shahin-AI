...
beforeAll(async () => {
  const mod = await import('./issues.module');
  manifest = mod.ISSUES_MANIFEST ?? mod.default?.manifest;
});

describe('Issues Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('issues'); });
