import type { Issue, IssueCreateInput } from '../types/issues.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Issue {
  return {
...
export function toEntityList(rows: GenericRow[]): Issue[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): IssueCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Issue): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Issue[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
