/**
 * F-004 — issues subscriber for foundation.position.holder.unassigned.
 */
...
describe('F-004 issues ← foundation.position.holder.unassigned', () => {
  beforeEach(() => {
  ...
  it('nulls assigned_to and creates reassignment task per orphaned issue', async () => {
    safeQueryMock.mockResolvedValueOnce({ rows: [{ issue_id: 'i_1' }, { issue_id: 'i_2' }] });
    ...
    expect(sql).toContain('UPDATE "tenant_t_1".issues');
