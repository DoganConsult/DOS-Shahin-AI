...
beforeAll(async () => {
  const mod = await import('./remediation.module');
  manifest = mod.REMEDIATION_MANIFEST ?? mod.default?.manifest;
});

describe('Remediation Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('remediation'); });
