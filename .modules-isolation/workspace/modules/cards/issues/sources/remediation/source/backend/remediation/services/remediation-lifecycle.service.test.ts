...
import {
  ...
  getOverdueTasks,
  getBlockedPlans,
} from './remediation-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getOverdueTasks should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverdueTasks('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverdueTasks should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverdueTasks('t1');
      expect(result).toEqual([]);
    });

    it('getBlockedPlans should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getBlockedPlans('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getBlockedPlans should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getBlockedPlans('t1');
      expect(result).toEqual([]);
    });
