...
beforeAll(async () => {
  const mod = await import('./action.module.js');
  manifest = mod.ACTION_MANIFEST;
});

describe('Action Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('action'); });
