// Shahin GRC — Audit Hub  |  GRC Lifecycle: ASSURE
...
import { AuditPackageComponent } from '../audit-package/audit-package.component';
import { AuditWorkpapersComponent } from '../audit-workpapers/audit-workpapers.component';
...
import { HubHelpPanelComponent } from '../../../../../../../modules/shared/guided-experience/hub-help-panel.component';
...
const TABS: HubTab[] = [
  { key: 'audit',      labelEn: 'Audit',         labelAr: 'المراجعة',      icon: 'pi-verified' },
  { key: 'trail',      labelEn: 'Audit Trail',   labelAr: 'مسار التدقيق', icon: 'pi-history' },
  { key: 'package',    labelEn: 'Audit Package', labelAr: 'حزمة المراجعة', icon: 'pi-box' },
  { key: 'workpapers', labelEn: 'Workpapers',    labelAr: 'أوراق العمل',  icon: 'pi-file-edit' },
];
...
@Component({
  ...
  selector: 'app-audit-hub', standalone: true,
  imports: [CommonModule, RouterModule, AuditComponent, AuditTrailComponent, AuditPackageComponent, AuditWorkpapersComponent, AgentBadgeComponent, HubConnectionsStripComponent, HubHelpPanelComponent, RaciPanelComponent],
  styles: [HUB_STYLES],
  template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#dcfce7"><i class="pi pi-verified hub-icon" style="color:#15803d"></i></div>
          <div>
            ...
            <h1>{{ i18n.currentLang()==='ar' ? 'مركز المراجعة الموحد' : 'Audit Hub' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'المراجعة، مسار التدقيق، الحزم وأوراق العمل — مكان واحد.' : 'Audit, trails, packages and workpapers — one place.' }}</p>
          </div>
        </div>
        <a class="agrc-link" routerLink="/agrc-os" routerLinkActive="active">{{ i18n.currentLang()==='ar' ? 'AGRC-OS' : 'Open in AGRC-OS' }}</a>
      ...
      <app-raci-panel entityType="audit" entityId="" [canEdit]="true" />
      <app-hub-connections-strip [hubKey]="'audit'" />
      <app-hub-help-panel [hubRoute]="'/audit-hub'" />
      <div class="hub-tab-content">
        @if (activeTab()==='audit')      { <app-audit /> }
        @if (activeTab()==='trail')      { <app-audit-trail /> }
        @if (activeTab()==='package')    { <app-audit-package /> }
        @if (activeTab()==='workpapers') { <app-audit-workpapers /> }
      </div>
    </div>`
})
export class AuditHubComponent implements OnInit {
