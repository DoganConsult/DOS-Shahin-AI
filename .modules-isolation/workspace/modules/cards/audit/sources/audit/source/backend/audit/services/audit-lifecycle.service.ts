...
export async function transitionStatus(
  ...
  if (current.rows.length === 0) { const e: unknown = new Error(`audit ${entityType} not found`); e.statusCode = 404; throw e; }
  ...
  if (evaluateLifecycleTransition) {
    const lifecycleResult = await evaluateLifecycleTransition(tenantId, userId, {
      moduleCode: 'audit', entityType: 'audit', entityId: entityId,
      fromState: fromStatus, toState: targetStatus, permissionCode: 'audit.record.approve'
    });
...
await safeQuery(`UPDATE "${schema}"."${table}" SET status = $1, updated_at = NOW() WHERE id = $2`, [targetStatus, entityId]);
  await safeQuery(
    ...
    if (emitEvent) await emitEvent({ tenantId, userId: userId, module: 'audit', event: 'status_changed', entityType: 'audit', entityId: entityId, data: { fromStatus, targetStatus } });
  } catch {}
