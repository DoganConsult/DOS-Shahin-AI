...
export async function recordAudit(entry) {
    ...
    await withClient(async (client) => {
        ...
        try {
            ...
            let previousHash = '';
            try {
                const lastRow = await client.query(`SELECT entry_hash FROM "${schema}".audit_trail WHERE entry_hash IS NOT NULL ORDER BY timestamp DESC LIMIT 1`);
            ...
            await client.query(`INSERT INTO "${schema}".audit_trail
...
export async function verifyAuditChain(tenantId, startDate, endDate) {
    ...
    const result = await safeQuery(`SELECT entry_id, timestamp, user_id, module, action, entity_type, entity_id, entry_hash, previous_hash
...
export async function queryAuditTrail(tenantId, filters) {
    const schema = tenantSchema(tenantId);
    const { where, params, nextIdx } = buildAuditFilters(filters);
    let idx = nextIdx;
    const limit = filters.limit || 100;
    const offset = filters.offset || 0;
    const result = await safeQuery(`SELECT at.*, u.email AS user_email
...
export async function getAuditTrailTotal(tenantId, filters) {
    const schema = tenantSchema(tenantId);
    const { where, params } = buildAuditFilters(filters);
    const result = await safeQuery(`SELECT COUNT(*)::int AS total FROM "${schema}".audit_trail at ${where}`, params);
    return getFirstRow(result)?.total ?? 0;
}
/** Return distinct module names from audit trail (for dynamic filter dropdown) */
export async function getDistinctModules(tenantId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT DISTINCT module FROM "${schema}".audit_trail ORDER BY module`, []);
...
export async function exportAuditLog(tenantId, filters, format) {
    ...
    const rows = entries.map((e) => fields.map(f => csvEscape(e[f])).join(",")).join("\r\n");
