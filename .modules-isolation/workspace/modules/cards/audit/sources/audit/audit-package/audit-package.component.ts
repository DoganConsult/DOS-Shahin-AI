...
@Component({
  ...
  styles: [`
    ...
    .hash-text { font-size: 0.75rem; word-break: break-all; background: var(--surface-ground); padding: 2px 6px; border-radius: var(--radius-xs); }
    .manifest-box { padding: 1rem; background: var(--surface-ground); border-radius: var(--radius-sm); border: 1px solid var(--surface-border); }
    .manifest-box code { font-size: 0.85rem; word-break: break-all; }
  `]
})
export class AuditPackageComponent implements OnInit, OnDestroy {
  ...
  selectedPackage: Record<string, unknown> | null = null;
  ...
  newPackage: Record<string, unknown> = {};
  ...
  ngOnInit(): void {
    this.subs.push(this.live.debounced(800).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.loadPackages()));
    this.loadPackages();
  }
  ...
  private updateStats(): void {
    this.draftCount = this.packages.filter((p: any) => p.status === 'draft').length;
    this.finalizedCount = this.packages.filter((p: any) => p.status === 'finalized').length;
    this.totalControls = this.packages.reduce((sum: number, p: Record<string, unknown>) => sum + (p.control_count || (p.controls || []).length), 0);
  }

  viewPackage(pkg: Record<string, unknown>): void {
    this.loading = true;
    this.subs.push(
  ...
  finalizePackage(pkg: Record<string, unknown>): void {
  ...
  deletePackage(pkg: Record<string, unknown>): void {
