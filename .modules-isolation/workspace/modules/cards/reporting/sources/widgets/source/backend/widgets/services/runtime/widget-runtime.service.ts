import { WidgetRuntimeRepository } from '../../repositories/widget-runtime.repo';
import { emptyResult, query } from '../../ports/database.port';
import { swallowDefault, EC } from '@dos/platform-core/resilience';
import type { WidgetRenderContext, WidgetResponseDto } from '../../types/widget.types';
import * as insight from '../insight/insight-widgets.service';
...
export class WidgetRuntimeService {
  private runtimeRepo: WidgetRuntimeRepository;

  constructor(private tenantId: string) {
    this.runtimeRepo = new WidgetRuntimeRepository(tenantId);
  }

  async renderWidget(ctx: WidgetRenderContext): Promise<WidgetResponseDto> {
    ...
    try {
      ...
      const schema = await this.runtimeRepo.resolveTenantSchema(ctx.tenantId);
      const result = await this.renderStructuralWidget(ctx.widgetKey, schema);
      return result;
    } catch (err) {
      success = false;
      throw err;
      ...
      this.runtimeRepo.logWidgetRender(ctx.widgetKey, ctx.userId, duration, success).catch(catchHandler(EC.EVENT_BUS));
    }
  }

  async getPublishedCatalog(): Promise<GenericRow[]> {
    return this.runtimeRepo.getPublishedWidgets();
  }

  async getPublishedBundles(): Promise<GenericRow[]> {
    return this.runtimeRepo.getPublishedBundles();
  }

  async getRenderStats(widgetKey?: string): Promise<GenericRow[]> {
    return this.runtimeRepo.getRenderStats(widgetKey);
  }

  isInsightWidget(widgetKey: string): boolean {
    return widgetKey in INSIGHT_FETCHERS;
  }

  private async renderStructuralWidget(widgetKey: string, schema: string): Promise<WidgetResponseDto> {
    switch (widgetKey) {
      ...
      default:
        throw new Error(`Unsupported widget key: ${widgetKey}`);
    }
  }
  ...
  private async executiveSummary(schema: string): Promise<WidgetResponseDto> {
    const [riskRes, findingRes, actionRes] = await Promise.all([
      ...
      swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), query(`SELECT COUNT(*)::int AS cnt FROM "${schema}".action_items WHERE status IS DISTINCT FROM 'closed'`), { operation: 'query action_items' }),
    ]);
    return {
  ...
  private async riskHeatmap(schema: string): Promise<WidgetResponseDto> {
    const result = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), query(
      `SELECT COALESCE(likelihood,0)::int AS likelihood, COALESCE(impact,0)::int AS impact, COUNT(*)::int AS count
       FROM "${schema}".risks GROUP BY likelihood, impact ORDER BY likelihood, impact`
    ...
    return {
      widgetKey: 'risk-heatmap', title: 'Risk Heatmap',
      payload: { cells: result.rows, totalRisks: totalRes.rows[0]?.cnt ?? 0 },
      fetchedAt: new Date().toISOString(),
    };
  }
  ...
  private async overdueActions(schema: string): Promise<WidgetResponseDto> {
    const result = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), query(
      `SELECT item_id::text AS id, title, deadline AS "dueDate", status, assigned_to AS owner, source_type AS "sourceType"
       FROM "${schema}".action_items
       WHERE deadline IS NOT NULL AND deadline < now() AND COALESCE(status,'') NOT IN ('closed','completed')
       ORDER BY deadline ASC LIMIT 10`
    ), { operation: 'query action_items' });
    return {
      widgetKey: 'overdue-actions', title: 'Overdue Actions',
      payload: result.rows,
      fetchedAt: new Date().toISOString(),
    };
  }
  ...
  private async auditExposure(schema: string): Promise<WidgetResponseDto> {
    const [openFindings, highSeverity, overdueRemediations] = await Promise.all([
      ...
      swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), query(`SELECT COUNT(*)::int AS cnt FROM "${schema}".remediation_tasks WHERE due_date < now() AND COALESCE(status,'') NOT IN ('closed','completed')`), { operation: 'query remediation_tasks' }),
    ]);
    return {
      widgetKey: 'audit-exposure', title: 'Audit Exposure',
      payload: {
  ...
  private async privacyIncidents(schema: string): Promise<WidgetResponseDto> {
    const [openIncidents, underReview, closedLast30] = await Promise.all([
      ...
      swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), query(`SELECT COUNT(*)::int AS cnt FROM "${schema}".privacy_reviews WHERE COALESCE(status,'') IN ('open','in_review')`), { operation: 'query privacy_reviews' }),
    ...
    return {
      widgetKey: 'privacy-incidents', title: 'Privacy Incidents',
      payload: {
  ...
  private async maturityScore(schema: string): Promise<WidgetResponseDto> {
    const current = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ avg_score: 0, domain_count: 0 }]), query(
      `SELECT AVG(score)::numeric(10,2) AS avg_score, COUNT(*)::int AS domain_count FROM "${schema}".maturity_scores`
    ), { operation: 'query maturity_scores' });
    return {
      widgetKey: 'maturity-score', title: 'Maturity Score',
      payload: {
  ...
  private async assessmentProgress(schema: string): Promise<WidgetResponseDto> {
    ...
    return {
      widgetKey: 'assessment-progress', title: 'Assessment Progress',
      payload: {
  ...
  private async recommendations(schema: string): Promise<WidgetResponseDto> {
    ...
    return {
      widgetKey: 'recommendations', title: 'Recommendations',
      payload: {
  ...
  private async evidenceCoverage(schema: string): Promise<WidgetResponseDto> {
    const [controlsRes, coveredRes] = await Promise.all([
      ...
      swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), query(
        `SELECT COUNT(DISTINCT control_id)::int AS cnt FROM "${schema}".evidence WHERE control_id IS NOT NULL`
      ), { operation: 'query evidence' }),
    ]);
    const totalControls = controlsRes.rows[0]?.cnt ?? 0;
    const controlsWithEvidence = coveredRes.rows[0]?.cnt ?? 0;
    const coveragePercent = totalControls > 0 ? Math.round((controlsWithEvidence / totalControls) * 100) : 0;
    return {
      widgetKey: 'evidence-coverage', title: 'Evidence Coverage',
      payload: { totalControls, controlsWithEvidence, coveragePercent },
      fetchedAt: new Date().toISOString(),
    };
  }
  ...
  private async kriStatus(schema: string): Promise<WidgetResponseDto> {
    ...
    return {
      widgetKey: 'kri-status', title: 'KRI Status',
      payload: {
