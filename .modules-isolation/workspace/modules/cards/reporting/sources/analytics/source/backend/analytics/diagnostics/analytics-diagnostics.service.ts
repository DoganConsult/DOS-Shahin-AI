...
export async function runDiagnostics(tenantId: string): Promise<DiagnosticsResult> {
  ...
  const { rows: snapshotRows } = await safeQuery(
    `SELECT COUNT(*)::int AS recent_count
     FROM "${schema}".analytics_kpi_snapshots
     WHERE deleted_at IS NULL AND computed_at > NOW() - INTERVAL '48 hours'`,
  ...
  const { rows: kpiDefRows } = await safeQuery(
  ...
  const { rows: staleDashRows } = await safeQuery(
  ...
  checks.push({
    ...
    detail: staleDashboards === 0
      ...
      : `${staleDashboards} dashboards stale (>${ANALYTICS_BUSINESS_THRESHOLDS.STALE_REPORT_DAYS} days)`,
  });

  // 6. Widget refresh health -- check for failed refresh flags
  const { rows: failedWidgetRows } = await safeQuery(
    `SELECT COUNT(*)::int AS failed_count
     FROM "${schema}".analytics_widgets
     WHERE deleted_at IS NULL AND refresh_failed = true`,
  ...
  const { rows: crossRows } = await safeQuery(
    `SELECT table_name FROM information_schema.tables
     WHERE table_schema = $1 AND table_name = ANY($2)`,
    [schema, crossModuleSources],
  ).catch(() => ({ rows: [] }));
  const availableSources = crossRows.map(( r: Record<string, unknown>) => r.table_name);
  ...
  const { rows: cacheRows } = await safeQuery(
    `SELECT COUNT(*)::int AS total,
            COUNT(*) FILTER (WHERE updated_at < NOW() - INTERVAL '${ANALYTICS_TIMEOUTS.CACHE_TTL_MINUTES} minutes')::int AS stale
     FROM "${schema}".analytics_cache
     WHERE deleted_at IS NULL`,
  ).catch(() => ({ rows: [{ total: 0, stale: 0 }] }));
  ...
  checks.push({
    ...
    detail: cacheTotal === 0
      ...
      : `${cacheStale}/${cacheTotal} cache entries stale (>${ANALYTICS_TIMEOUTS.CACHE_TTL_MINUTES}m)`,
  });

  // 9. KPI threshold configuration -- at least one threshold set
  const { rows: thresholdRows } = await safeQuery(
