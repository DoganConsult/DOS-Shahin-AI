...
export async function publish(
  ...
  if (!ANALYTICS_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${ANALYTICS_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: ANALYTICS_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${ANALYTICS_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(ANALYTICS_EVENT_CONTRACT.published);
}
