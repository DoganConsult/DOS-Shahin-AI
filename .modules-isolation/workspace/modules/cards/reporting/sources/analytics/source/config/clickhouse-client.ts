...
interface ClickHouseClientLike {
  query(args: { query: string; query_params?: Record<string, unknown>; format: string }): Promise<ClickHouseResultSetLike>;
  insert(args: { table: string; values: Record<string, unknown>[]; format: string }): Promise<unknown>;
  command(args: { query: string }): Promise<unknown>;
}

let _client: ClickHouseClientLike | null = null;
...
export function getClickHouseClient(): ClickHouseClientLike | null {
  ...
  if (!_client) {
    try {
      ...
      const { createClient } = require('@clickhouse/client') as {
        createClient: (config: Record<string, unknown>) => ClickHouseClientLike;
      };
