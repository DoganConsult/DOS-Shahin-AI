...
export function toEntity(row: GenericRow): KpiSnapshot {
  return {
...
export function toEntityList(rows: GenericRow[]): KpiSnapshot[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): KpiSnapshotCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as KpiSnapshotCreateInput;
}

export function toApiResponse(entity: KpiSnapshot): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: KpiSnapshot[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
