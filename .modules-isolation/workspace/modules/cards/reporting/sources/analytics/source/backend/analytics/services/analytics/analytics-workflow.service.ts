...
export async function executeTransition(
  ...
  emitAnalyticsEvent({ tenantId, entityType: 'analytics' as string, entityId, action: 'status_changed', triggeredBy: userId, previousState: fromStatus as string, newState: toStatus as string, correlationId });
...
export async function onWorkflowTriggered(ctx: AnalyticsWorkflowContext): Promise<void> {
  ...
  emitAnalyticsEvent({ tenantId: ctx.tenantId, entityType: ctx.entityType as string, entityId: ctx.entityId, action: 'status_changed', triggeredBy: ctx.triggeredBy, correlationId: ctx.correlationId, data: { trigger: 'workflow_start' } });
}
export async function onTaskCreated(ctx: AnalyticsWorkflowContext, taskId: string): Promise<void> {

  emitAnalyticsEvent({ tenantId: ctx.tenantId, entityType: ctx.entityType as string, entityId: ctx.entityId, action: 'created', triggeredBy: 'workflow', correlationId: ctx.correlationId, data: { taskId, trigger: 'task_creation' } });
}
export async function onApprovalRequired(ctx: AnalyticsWorkflowContext, approverRole: string): Promise<void> {

  emitAnalyticsEvent({ tenantId: ctx.tenantId, entityType: ctx.entityType as string, entityId: ctx.entityId, action: 'escalated', triggeredBy: 'workflow', correlationId: ctx.correlationId, data: { approverRole, trigger: 'approval_hook' } });
}
export async function onEscalation(ctx: AnalyticsWorkflowContext, reason: string, escalateTo: string): Promise<void> {

  emitAnalyticsEvent({ tenantId: ctx.tenantId, entityType: ctx.entityType as string, entityId: ctx.entityId, action: 'escalated', triggeredBy: 'workflow', correlationId: ctx.correlationId, data: { reason, escalateTo, trigger: 'escalation_hook' } });
}
export async function onClosure(ctx: AnalyticsWorkflowContext, closureReason: string): Promise<void> {

  emitAnalyticsEvent({ tenantId: ctx.tenantId, entityType: ctx.entityType as string, entityId: ctx.entityId, action: 'status_changed', triggeredBy: ctx.triggeredBy, newState: 'closed' as string, correlationId: ctx.correlationId, data: { closureReason, trigger: 'closure_hook' } });
}
export async function onFailure(ctx: AnalyticsWorkflowContext, error: string): Promise<void> {

  emitAnalyticsEvent({ tenantId: ctx.tenantId, entityType: ctx.entityType as string, entityId: ctx.entityId, action: 'status_changed', triggeredBy: 'workflow', correlationId: ctx.correlationId, data: { error, trigger: 'failure_compensation' } });
}
