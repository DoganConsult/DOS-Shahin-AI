"use strict";
// ============================================================================
// Shahin GRC — Executive Narrative Generator (F60 Enhancement)
//
...
exports.generateBoardPack = generateBoardPack;
exports.generateComplianceNarrative = generateComplianceNarrative;
exports.generateRiskNarrative = generateRiskNarrative;
exports.formatForArabicExecutive = formatForArabicExecutive;
exports.formatForEnglishExecutive = formatForEnglishExecutive;
...
function renderExecutiveSummaryEn(compliancePercent, totalControls, compliant, gapCount, criticalRisks, openActions, periodLabel) {
    const lines = [];
    lines.push(`Executive Summary — ${periodLabel}`);
    lines.push("");
    lines.push(`The comprehensive compliance assessment indicates a ${compliancePercent}% compliance level ` +
        `across adopted regulatory frameworks. Of ${totalControls} controls evaluated, ` +
        `${compliant} are fully compliant.`);
    if (gapCount > 0) {
        lines.push("");
        lines.push(`${gapCount} compliance gap${gapCount > 1 ? "s have" : " has"} been identified ` +
            `requiring management attention and remediation planning.`);
    }
    if (criticalRisks > 0) {
        lines.push("");
        lines.push(`${criticalRisks} critical risk${criticalRisks > 1 ? "s have" : " has"} been identified ` +
            `requiring immediate governance committee attention.`);
    }
    if (openActions > 0) {
        lines.push("");
        lines.push(`There are ${openActions} open action item${openActions > 1 ? "s" : ""} pending resolution across the governance program.`);
    }
    return lines.join("\n");
}
/** Generate executive summary section in Arabic */
function renderExecutiveSummaryAr(compliancePercent, totalControls, compliant, gapCount, criticalRisks, openActions, periodLabel) {
    const lines = [];
    lines.push(`الملخص التنفيذي — ${periodLabel}`);
    lines.push("");
    lines.push(`يُظهر تقييم الامتثال الشامل أن المنظمة تحقق مستوى امتثال بنسبة ${toArabicDigits(compliancePercent)}٪ ` +
    ...
    if (gapCount > 0) {
        lines.push("");
        lines.push(`تم تحديد ${toArabicDigits(gapCount)} ثغر${gapCount > 1 ? "ات" : "ة"} ` +
            `${gapCount > 2 ? "حرجة تتطلب" : "تتطلب"} اهتمامًا فوريًا من لجنة الحوكمة.`);
    }
    if (criticalRisks > 0) {
        lines.push("");
        lines.push(`تم تحديد ${toArabicDigits(criticalRisks)} مخاطر حرجة تتطلب اهتمامًا فوريًا من لجنة الحوكمة.`);
    }
    if (openActions > 0) {
        lines.push("");
        lines.push(`يوجد ${toArabicDigits(openActions)} بند إجراء مفتوح بحاجة إلى معالجة ضمن برنامج الحوكمة.`);
    }
    return lines.join("\n");
}
// ── Compliance posture section ─────────────────────────────────────────────
function renderCompliancePostureEn(frameworks) {
    const lines = [];
    lines.push("Compliance Posture");
    lines.push("");
    for (const fw of frameworks) {
        lines.push(`• ${fw.name}: ${fw.compliancePercent}% compliance across ${fw.controlCount} controls ` +
...
function renderCompliancePostureAr(frameworks) {
    const lines = [];
    lines.push("وضع الامتثال");
    lines.push("");
    for (const fw of frameworks) {
        lines.push(`• ${fw.name}: نسبة امتثال ${toArabicDigits(fw.compliancePercent)}٪ عبر ${toArabicDigits(fw.controlCount)} ضابط ` +
...
function renderRiskLandscapeEn(total, critical, high, medium, low, topRisks) {
    const lines = [];
...
function renderRiskLandscapeAr(total, critical, high, medium, low, topRisks) {
    const riskLevelAr = {
...
// ── Decisions section ──────────────────────────────────────────────────────
function renderDecisionsEn(decisions) {
    const lines = [];
...
function renderDecisionsAr(decisions) {
    const statusAr = {
...
function renderOpenActionsEn(actions) {
    const lines = [];
...
function renderOpenActionsAr(actions) {
    const priorityAr = {
        critical: "حرج", high: "عالٍ", medium: "متوسط", low: "منخفض",
    };
    const lines = [];
...
function renderRecommendationsEn(gapCount, criticalRisks, overdueActions) {
    const lines = [];
...
function renderRecommendationsAr(gapCount, criticalRisks, overdueActions) {
    const lines = [];
    lines.push("التوصيات");
    lines.push("");
    let idx = 1;
 ...
 */
async function generateBoardPack(tenantId, options) {
    ...
    const assertionRes = await (0, database_port_1.safeQuery)(`SELECT
     ...
     ${options?.frameworkId ? "WHERE framework_id = $1" : ""}`, options?.frameworkId ? [options.frameworkId] : []);
    ...
    const frameworkRes = await (0, database_port_1.safeQuery)(`SELECT f.id, f.name_en,
     ...
     GROUP BY f.id, f.name_en`, options?.frameworkId ? [options.frameworkId] : []);
    const frameworkBreakdown = frameworkRes.rows.map((r) => ({
    ...
    // ── 3. Fetch risk data ──
    const riskRes = await (0, database_port_1.safeQuery)(`SELECT
    ...
    const topRiskRes = await (0, database_port_1.safeQuery)(`SELECT title_en, title_ar, risk_level
     FROM "${schema}".risks
     ...
     LIMIT 5`, []);
    const topRisks = topRiskRes.rows.map((r) => ({
        titleEn: r.title_en || "",
        titleAr: r.title_ar || "",
        riskLevel: r.risk_level,
    }));
    // ── 4. Fetch decisions ──
    const decisionRes = await (0, database_port_1.safeQuery)(`SELECT title_en, title_ar, decision_type, status, decision_date
     FROM "${schema}".board_decisions
     WHERE decision_date >= $1 AND decision_date <= $2
     ORDER BY decision_date DESC
     LIMIT 10`, [periodStart, periodEnd]);
    const decisions = decisionRes.rows.map((r) => ({
        titleEn: r.title_en || "",
        titleAr: r.title_ar || "",
    ...
    const actionsRes = await (0, database_port_1.safeQuery)(`SELECT title_en, title_ar, priority, due_date, status
     FROM "${schema}".capa_records
     WHERE status IN ('open', 'in_progress', 'overdue')
     ORDER BY CASE priority WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END
     LIMIT 20`, []);
    const openActions = actionsRes.rows.map((r) => ({
        titleEn: r.title_en || "",
        titleAr: r.title_ar || "",
    ...
    events_port_1.eventBus.publish("narrative.board_pack_generated", {
        tenantId,
 ...
 */
async function generateComplianceNarrative(tenantId, frameworkId) {
    await (0, database_port_1.safeQuery)("UPDATE __TENANT_SCHEMA__.analytics_items SET updated_at = NOW()" + (tenantId ? " WHERE tenant_id = $1" : ""), tenantId ? [tenantId] : []);
    return {};
}
// ── Risk Narrative ─────────────────────────────────────────────────────────
/**
 ...
 */
async function generateRiskNarrative(tenantId) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    ...
    // Top risks by severity
    const topRes = await (0, database_port_1.safeQuery)(`SELECT title_en, title_ar, risk_level
     FROM "${schema}".risks
     WHERE deleted_at IS NULL
     ORDER BY CASE risk_level WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END
     LIMIT 5`, []);
    const topRisks = topRes.rows.map((r) => ({
        titleEn: r.title_en || "",
        titleAr: r.title_ar || "",
        riskLevel: r.risk_level,
    }));
 ...
 */
function formatForArabicExecutive(content) {
 ...
 */
function formatForEnglishExecutive(content) {
    let formatted = content;
...
//# sourceMappingURL=executive-narrative.service.js.map