...
exports.computeResponseTimeScore = computeResponseTimeScore;
exports.computeCompletionRateScore = computeCompletionRateScore;
exports.computeEvidenceTimelinessScore = computeEvidenceTimelinessScore;
exports.computeRemediationRateScore = computeRemediationRateScore;
exports.getScoreHistory = getScoreHistory;
exports.serializeScore = serializeScore;
exports.deserializeScore = deserializeScore;
 ...
 */
async function computeEngagementScore(tenantId, vendorId) {
    ...
    const responseTimeResult = await (0, database_port_1.safeQuery)(`SELECT AVG(
     ...
     WHERE vendor_id = $1
       ...
       AND distributed_at >= $2`, [vendorId, cutoffISO]);
    const avgResponseDays = (0, db_1.getFirstRow)(responseTimeResult)?.avg_days ?? null;
    ...
    const completionResult = await (0, database_port_1.safeQuery)(`SELECT
     ...
     WHERE vendor_id = $1
       AND distributed_at IS NOT NULL
       AND distributed_at >= $2`, [vendorId, cutoffISO]);
    const completed = Number((0, db_1.getFirstRow)(completionResult)?.completed ?? 0);
    const totalQ = Number((0, db_1.getFirstRow)(completionResult)?.total ?? 0);
    ...
    try {
       ...
       WHERE vendor_id = $1
         AND created_at >= $2`, [vendorId, cutoffISO]);
        onTimeEvidence = Number((0, db_1.getFirstRow)(evidenceResult)?.on_time ?? 0);
        totalEvidence = Number((0, db_1.getFirstRow)(evidenceResult)?.total ?? 0);
    }
    ...
    try {
       ...
       WHERE vendor_id = $1
         AND created_at >= $2`, [vendorId, cutoffISO]);
        resolvedItems = Number((0, db_1.getFirstRow)(remediationResult)?.resolved ?? 0);
        totalItems = Number((0, db_1.getFirstRow)(remediationResult)?.total ?? 0);
    }
    ...
    // ── Total ──
    const totalScore = responseTimeScore + completionRateScore + evidenceTimelinessScore + remediationRateScore;
    ...
    await (0, database_port_1.safeQuery)(`INSERT INTO ${schema}.vendor_engagement_scores
    ...
    if (totalScore < LOW_SCORE_THRESHOLD) {
        await events_port_1.eventBus.publish({
            eventType: 'vendor.engagement_score_low',
            tenantId,
 ...
 */
function computeResponseTimeScore(avgDays) {
    if (avgDays === null || avgDays === undefined)
        return Math.round(MAX_RESPONSE_TIME / 2);
 ...
 */
function computeCompletionRateScore(completed, total) {
    if (total === 0)
        return 0;
 ...
 */
function computeEvidenceTimelinessScore(onTime, total) {
    if (total === 0)
        return 0;
 ...
 */
function computeRemediationRateScore(resolved, total) {
    if (total === 0)
        return 0;
 ...
 */
async function getScoreHistory(tenantId, vendorId, limit = 50) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    const result = await (0, database_port_1.safeQuery)(`SELECT vendor_id, total_score, response_time_score, completion_rate_score,
            evidence_timeliness_score, remediation_rate_score, computed_at
 ...
 */
function deserializeScore(json) {
...
// ── Internal Helpers ───────────────────────────────────────────────────────
function mapRowToBreakdown(row) {
    return {
