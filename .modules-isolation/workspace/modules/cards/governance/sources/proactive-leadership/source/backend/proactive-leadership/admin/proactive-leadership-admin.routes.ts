...
const router = Router();
router.use(moduleStack('proactive-leadership'));
router.use(auditMiddleware('proactive_leadership'));

router.get(
  '/settings',
  authenticate,
  requirePermission('proactive_leadership.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
