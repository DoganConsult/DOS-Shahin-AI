...
export async function runDiagnostics(tenantId: string): Promise<DiagnosticsResult> {
  ...
  const ownedTables = ['proactive_leadership_configs', 'proactive_leadership_insights', 'proactive_leadership_alerts'];
  ...
  checks.push({
    ...
    detail: `${tableRows.length}/${ownedTables.length} core proactive-leadership tables found`,
  });

  const { rows: staleRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".proactive_leadership_configs WHERE updated_at < NOW() - INTERVAL '90 days'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
  ...
  const { rows: auditRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".audit_trail WHERE module = 'proactive-leadership' AND created_at > NOW() - INTERVAL '24 hours'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
