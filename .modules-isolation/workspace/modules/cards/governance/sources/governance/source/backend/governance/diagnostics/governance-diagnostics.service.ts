/**
 * GovernanceDiagnosticsService — production health checks (Rule 6.1)
 */
...
export async function runGovernanceDiagnostics(tenantId: string): Promise<DiagnosticsReport> {
  ...
  try {
    const tableCheck = await safeQuery(
      `SELECT COUNT(*)::int AS cnt
       ...
       WHERE table_schema = current_schema()
         AND table_name LIKE 'governance%'`,
      [],
    );
    ...
    checks.push({
      ...
      details: `Found ${cnt} governance tables`,
    });
  } catch {
...
export { runGovernanceDiagnostics as runDiagnostics };
