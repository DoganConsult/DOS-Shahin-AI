...
@Component({
  ...
  selector: 'app-governance-obligations',
  standalone: true,
  ...
  template: `
    <div class="gov-page" [attr.dir]="dir()">
      <app-page-header titleEn="Obligations" titleAr="الالتزامات" subtitleEn="Track regulatory and contractual obligations" subtitleAr="تتبع الالتزامات التنظيمية والتعاقدية" icon="balance-scale"
        [breadcrumbs]="[i18n.translate('Dashboard'), i18n.translate('Governance'), i18n.translate('Obligations')]" [actions]="headerActions" [isAr]="i18n.currentLang() === 'ar'" [dir]="dir()" (actionClick)="onHeaderAction($event)" />
      ...
      <div class="gov-body">
        ...
        <div class="health-strip">
          ...
          <div class="health-card"><div class="health-value" style="color:var(--primary)">{{ activeCount }}</div><div class="health-label">{{ i18n.translate('Active') }}</div></div>
        ...
        <div class="page-toolbar">
          <div class="toolbar-primary">
            <p-button [label]="i18n.translate('Add Obligation')" icon="pi pi-plus" (onClick)="openCreate()" />
          ...
          <div class="toolbar-secondary"><app-export-button module="governance-obligations" [data]="filteredItems" /></div>
        </div>
        <div class="table-shell" *ngIf="filteredItems.length > 0">
          <p-table aria-label="Filtered Items table" [value]="filteredItems" [paginator]="filteredItems.length > 10" [rows]="10" styleClass="p-datatable-striped p-datatable-sm">
            ...
            <ng-template pTemplate="body" let-item><tr>
              ...
              <td><p-tag [value]="item.obligation_type || 'regulatory'" severity="info" /></td>
        ...
        <div class="empty-state" *ngIf="!loading && filteredItems.length === 0"><i class="pi pi-inbox empty-icon"></i><p>{{ i18n.translate('No obligations found') }}</p></div>
        <!-- Cross-Module Links -->
        <div class="cross-links">
          <button class="cross-link-btn" (click)="navigateTo('/governance/mandates')"><i class="pi pi-hammer"></i> Mandates</button>
          <button class="cross-link-btn" (click)="navigateTo('/governance/policies')"><i class="pi pi-file"></i> Policies</button>
          <button class="cross-link-btn" (click)="navigateTo('/compliance/controls')"><i class="pi pi-verified"></i> Controls</button>
    ...
    <p-dialog [header]="editMode ? (i18n.translate('Edit')) : (i18n.translate('Add Obligation'))" [(visible)]="showDialog" [modal]="true" [style]="{width:'520px'}" [dismissableMask]="true">
      <div class="dialog-form">
        ...
        <div class="field-row">
          <div class="field"><label>Type</label><p-dropdown [options]="typeOptions" [(ngModel)]="form.obligation_type" optionLabel="label" optionValue="value" class="w-full" /></div>
...
export class GovernanceObligationsComponent implements OnInit {
 ...
 private msg = inject(MessageService);
  ...
  readonly headerActions: PageHeaderAction[] = [{ id: 'add', labelEn: 'Add Obligation', labelAr: 'إضافة التزام', icon: 'plus', primary: true }];
  onHeaderAction(id: string): void { if (id === 'add') this.openCreate(); }
  typeOptions = [{ label: 'Regulatory', value: 'regulatory' }, { label: 'Contractual', value: 'contractual' }, { label: 'Internal', value: 'internal' }];
  statusOptions = [{ label: 'Draft', value: 'draft' }, { label: 'Active', value: 'active' }, { label: 'Completed', value: 'completed' }, { label: 'Overdue', value: 'overdue' }, { label: 'Exempted', value: 'exempted' }];
  ...
  completedCount = 0; activeCount = 0; overdueCount = 0;

  ngOnInit(): void { this.load(); }
  load(): void {
    this.loading = true;
    this.apiclientSvc.get('/governance/obligations').subscribe({ next: res => { this.items = res.obligations || []; this.completedCount = this.items.filter(i => i.status === 'completed').length; this.activeCount = this.items.filter(i => i.status === 'active').length; this.overdueCount = this.items.filter(i => i.status === 'overdue').length; this.filterList(); this.loading = false; }, error: () => { this.loading = false; } });
  }
  ...
  openCreate(): void { this.editMode = false; this.form = { title_en: '', title_ar: '', description: '', obligation_type: 'regulatory', status: 'draft' }; this.showDialog = true; }
  openEdit(item: Record<string, any>): void { this.editMode = true; this.editId = item.obligation_id; this.form = { ...item }; this.showDialog = true; }
  save(): void {
    const obs = this.editMode ? this.apiclientSvc.put(`/governance/obligations/${this.editId}`, this.form) : this.apiclientSvc.post('/governance/obligations', this.form);
  ...
  deleteItem(item: Record<string, any>): void { this.apiclientSvc.del(`/governance/obligations/${item.obligation_id}`).subscribe({ next: () => this.load() }); }
