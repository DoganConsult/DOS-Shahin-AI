...
beforeAll(async () => {
  const mod = await import('./governance.module');
  manifest = mod.GOVERNANCE_MANIFEST ?? mod.default?.manifest;
});

describe('Governance Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('governance'); });
