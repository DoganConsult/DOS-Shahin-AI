...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getDraftPolicies should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getDraftPolicies('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getDraftPolicies should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getDraftPolicies('t1');
      expect(result).toEqual([]);
    });

    it('getExpiredPolicies should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getExpiredPolicies('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getExpiredPolicies should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getExpiredPolicies('t1');
      expect(result).toEqual([]);
    });
