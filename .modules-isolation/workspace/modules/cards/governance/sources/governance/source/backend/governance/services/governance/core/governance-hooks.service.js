...
exports.checkRiskAcceptanceAuthority = checkRiskAcceptanceAuthority;
exports.createGovernanceActionFromControlFailure = createGovernanceActionFromControlFailure;
exports.escalateAuditFindingToGovernance = escalateAuditFindingToGovernance;
exports.escalateIncidentToGovernanceBody = escalateIncidentToGovernanceBody;
exports.escalateEthicsReportToGovernance = escalateEthicsReportToGovernance;
exports.escalateSecurityEventToGovernance = escalateSecurityEventToGovernance;
exports.getBoardAttentionItems = getBoardAttentionItems;
exports.getGovernanceQiyasDimensions = getGovernanceQiyasDimensions;
exports.getStaleEvidenceGovernanceImpact = getStaleEvidenceGovernanceImpact;
exports.createGovernanceActionFromStaleEvidence = createGovernanceActionFromStaleEvidence;
exports.registerGovernanceEventSubscriptions = registerGovernanceEventSubscriptions;
 ...
 */
async function checkRiskAcceptanceAuthority(tenantId, riskId, userId) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        // Check authority matrix for risk acceptance rules
        const matrix = await (0, database_port_1.safeQuery)(`
    ...
    `);
        ...
        const delegation = await (0, database_port_1.safeQuery)(`
 ...
 */
async function createGovernanceActionFromControlFailure(tenantId, controlId, severity) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
    ...
    `, [
        ...
        if (actionId) {
            await (0, events_port_1.emitEvent)({
                tenantId, userId: platform_port_1.SYSTEM_JOB_ACTOR, module: 'action_items', event: 'created',
        ...
        logger_port_1.logger.error(`[GovernanceHooks] Failed to create action from control failure: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
/**
 * Escalate a critical/high audit finding to governance by creating an action item.
 */
async function escalateAuditFindingToGovernance(tenantId, findingId, severity) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        // Get finding details
        const finding = await (0, database_port_1.safeQuery)(`
    ...
    `, [findingId]);
        ...
        const auditResult = await (0, database_port_1.safeQuery)(`
      INSERT INTO "${schema}".governance_action_items
    ...
    `, [
        ...
        logger_port_1.logger.error(`[GovernanceHooks] Failed to escalate audit finding: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
/**
 * Escalate a critical incident to the governance body by creating an action item + agenda item.
 */
async function escalateIncidentToGovernanceBody(tenantId, incidentId, severity) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        // Get incident details
        const incident = await (0, database_port_1.safeQuery)(`
    ...
    `, [incidentId]);
        ...
        const incResult = await (0, database_port_1.safeQuery)(`
      INSERT INTO "${schema}".governance_action_items
    ...
    `, [
        ...
        logger_port_1.logger.error(`[GovernanceHooks] Failed to escalate incident: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
/**
 * Escalate ethics report to governance body.
 */
async function escalateEthicsReportToGovernance(tenantId, reportId, severity) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        const report = await (0, database_port_1.safeQuery)(`
    ...
    `, [reportId]);
        ...
        const ethicsResult = await (0, database_port_1.safeQuery)(`
      INSERT INTO "${schema}".governance_action_items
        (title_en, description, priority, status, source_type, source_id, board_attention, created_at, updated_at)
    ...
    `, [
        ...
        await (0, database_port_1.safeQuery)(`UPDATE "${schema}".ethics_reports SET escalated_to_governance = TRUE, updated_at = NOW() WHERE report_id = $1`, [reportId]).catch((0, resilience_1.catchHandler)(resilience_1.EC.EVENT_BUS, {}));
    }
    catch (err) {
        logger_port_1.logger.error(`[GovernanceHooks] Failed to escalate ethics report: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
/**
 * Escalate security vulnerability to governance when severity >= HIGH.
 */
async function escalateSecurityEventToGovernance(tenantId, entityId, eventType, severity, title) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        const secPriority = severity === 'critical' ? 'critical' : 'high';
        const secResult = await (0, database_port_1.safeQuery)(`
      INSERT INTO "${schema}".governance_action_items
        (title_en, description, priority, status, source_type, source_id, board_attention, created_at, updated_at)
    ...
    `, [
        ...
        logger_port_1.logger.error(`[GovernanceHooks] Failed to escalate security event: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
 ...
 */
async function getBoardAttentionItems(tenantId) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    const results = {
        governance_actions: [],
        critical_risks: [],
    ...
    try {
        ...
        const actions = await (0, database_port_1.safeQuery)(`
    ...
    try {
        // Critical/high risks
        const risks = await (0, database_port_1.safeQuery)(`
    ...
    try {
        ...
        const exceptions = await (0, database_port_1.safeQuery)(`
    ...
    try {
        ...
        const violations = await (0, database_port_1.safeQuery)(`
    ...
    try {
        ...
        const overdue = await (0, database_port_1.safeQuery)(`
 ...
 */
async function getGovernanceQiyasDimensions(tenantId) {
    try {
        ...
        if (!health || !health.dimensions || health.dimensions.length === 0)
            return [];
        const dimensions = [];
...
async function getStaleEvidenceGovernanceImpact(tenantId) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        const result = await (0, database_port_1.safeQuery)(`
...
async function createGovernanceActionFromStaleEvidence(tenantId, evidenceId, title) {
    const schema = (0, database_port_1.tenantSchema)(tenantId);
    try {
        const staleResult = await (0, database_port_1.safeQuery)(`
      INSERT INTO "${schema}".governance_action_items
        (title_en, title_ar, description, priority, status, source_type, source_id, board_attention, created_at, updated_at)
      VALUES
    ...
    `, [
        ...
        logger_port_1.logger.error(`[GovernanceHooks] Failed to create action from stale evidence: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
 ...
 */
async function registerGovernanceEventSubscriptions() {
    try {
        ...
        eventBus.subscribe('risk.exceeded_appetite', 'governance-hook-risk', async (event) => {
            if (!event.tenantId)
                return;
            const riskResult = await (0, resilience_1.swallowDefault)(resilience_1.EC.FALLBACK_QUERY, (0, database_port_1.emptyResult)(), (0, database_port_1.safeQuery)(`
        INSERT INTO "${(0, database_port_1.tenantSchema)(event.tenantId)}".governance_action_items
          (title_en, description, priority, status, source_type, source_id, board_attention, created_at, updated_at)
        VALUES
      ...
      `, [
            ...
            const riskActionId = riskResult?.rows?.[0]?.action_id;
            if (riskActionId) {
                await (0, events_port_1.emitEvent)({
                    tenantId: event.tenantId, userId: platform_port_1.SYSTEM_JOB_ACTOR, module: 'action_items', event: 'created',
        ...
        eventBus.subscribe('audit.finding_created', 'governance-hook-audit', async (event) => {
            ...
            if (sev === 'critical' || sev === 'high') {
                await escalateAuditFindingToGovernance(event.tenantId, String(event.payload.finding_id || event.entityId || ''), String(sev));
            }
        });
        ...
        eventBus.subscribe('incident.escalated', 'governance-hook-incident', async (event) => {
            if (!event.tenantId)
                return;
            await escalateIncidentToGovernanceBody(event.tenantId, String(event.payload?.incident_id || event.entityId || ''), String(event.payload?.severity || 'high'));
        });
        // Control failure → governance action
        eventBus.subscribe('control.failed', 'governance-hook-control', async (event) => {
            if (!event.tenantId)
                return;
            await createGovernanceActionFromControlFailure(event.tenantId, String(event.payload?.control_id || event.entityId || ''), String(event.payload?.severity || 'medium'));
        });
        // Evidence expired → governance action
        eventBus.subscribe('evidence.expired', 'governance-hook-evidence', async (event) => {
            if (!event.tenantId)
                return;
            await createGovernanceActionFromStaleEvidence(event.tenantId, String(event.payload?.evidence_id || event.entityId || ''), String(event.payload?.title || 'Unknown evidence'));
        });
        // Governance health recalculated → feed to Qiyas
        eventBus.subscribe('governance.health_recalculated', 'governance-hook-qiyas', async (event) => {
            if (!event.tenantId)
                return;
            try {
                ...
                if (dimensions.length > 0) {
                    eventBus.publish({
                        eventType: 'qiyas.governance_dimensions_updated',
        ...
        eventBus.subscribe('security.vulnerability_detected', 'governance-hook-security', async (event) => {
            ...
            if (sev === 'critical' || sev === 'high') {
                await escalateSecurityEventToGovernance(event.tenantId, String(event.payload?.vulnerability_id || event.entityId || ''), 'vulnerability_detected', String(sev), String(event.payload?.title || 'Security vulnerability'));
            }
        });
        eventBus.subscribe('ethics.report_created', 'governance-hook-ethics', async (event) => {
            ...
            const sev = event.payload?.severity || 'medium';
            if (sev === 'critical' || sev === 'high') {
                await escalateEthicsReportToGovernance(event.tenantId, String(event.payload?.report_id || event.entityId || ''), String(sev));
            }
        });
        logger_port_1.logger.info('[GovernanceHooks] 8 cross-module event subscriptions registered');
    }
    catch (err) {
        logger_port_1.logger.error(`[GovernanceHooks] Failed to register event subscriptions: ${(0, module_sdk_1.toErrorMessage)(err)}`);
    }
}
//# sourceMappingURL=governance-hooks.service.js.map