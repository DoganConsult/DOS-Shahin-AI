import { describe, it, expect, vi, beforeEach } from 'vitest';

vi.mock('../../../platform/services/event/event-bus.service', () => ({
  eventBus: { publish: vi.fn() },
}));

import { emitGovernanceEvent, emitGovernanceStatusChange, emitFrameworkAdopted, emitFrameworkActivated, emitFrameworkDeprecated, emitControlImplemented } from './governance-event.service';

vi.mock('../../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
import { eventBus } from '../../../ports/events.port';


vi.mock('../../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
describe('Governance Event Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('emitGovernanceEvent', () => {
    it('should publish event to event bus', () => {
      emitGovernanceEvent({
        tenantId: 't1',
        entityType: 'framework' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
    });

    it('should include correlationId in payload', () => {
      emitGovernanceEvent({
        tenantId: 't1',
        entityType: 'framework' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
        correlationId: 'corr-123',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBe('corr-123');
    });

    it('should auto-generate correlationId if not provided', () => {
      emitGovernanceEvent({
        tenantId: 't1',
        entityType: 'framework' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBeDefined();
    });

    it('should include timestamp and eventVersion', () => {
      emitGovernanceEvent({
        tenantId: 't1',
        entityType: 'framework' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.timestamp).toBeDefined();
      expect(call.payload.eventVersion).toBe(1);
    });

    it('should not throw on publish failure', () => {
      (eventBus.publish as any).mockImplementationOnce(() => { throw new Error('fail'); });
      expect(() => emitGovernanceEvent({
        tenantId: 't1',
        entityType: 'framework' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      })).not.toThrow();
    });
  });

  describe('emitGovernanceStatusChange', () => {
    it('should emit status_changed event', () => {
      emitGovernanceStatusChange('t1', 'framework' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.action).toBe('status_changed');
    });
  });

  describe('helper functions', () => {
    it('emitFrameworkAdopted should not throw', () => {
      expect(() => emitFrameworkAdopted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitFrameworkActivated should not throw', () => {
      expect(() => emitFrameworkActivated('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitFrameworkDeprecated should not throw', () => {
      expect(() => emitFrameworkDeprecated('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitControlImplemented should not throw', () => {
      expect(() => emitControlImplemented('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

  });
});
