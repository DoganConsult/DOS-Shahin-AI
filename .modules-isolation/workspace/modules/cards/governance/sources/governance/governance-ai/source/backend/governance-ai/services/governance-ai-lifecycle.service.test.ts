...
} from './governance-ai-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getStaleSignals should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getStaleSignals('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getStaleSignals should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getStaleSignals('t1');
      expect(result).toEqual([]);
    });

    it('getPendingEscalations should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getPendingEscalations('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getPendingEscalations should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getPendingEscalations('t1');
      expect(result).toEqual([]);
    });
