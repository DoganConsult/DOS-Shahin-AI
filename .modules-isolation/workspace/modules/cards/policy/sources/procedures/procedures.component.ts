...
export class ProceduresComponent implements OnInit {
  ...
  editMode = false;
  editId = '';
  form: Record<string, unknown> = { title: '', content: '', description: '', category: 'general', sop_type: 'operational', linked_policy_id: '' };
  ...
  ngOnInit(): void {
    ...
    this.governanceSvc.getGovernancePolicies().subscribe({
      next: (res: Record<string, unknown>) => {
        ...
        this.policyOptions = policies.map((p: any) => ({ label: p.title, value: p.policy_id }));
      },
  ...
  openEditDialog(p: Record<string, unknown>): void {
    this.editMode = true;
  ...
  save(): void {
    ...
    const obs = this.editMode
      ? this.governanceSvc.updateProcedure(this.editId, payload)
  ...
  approve(p: Record<string, unknown>): void {
  ...
  confirmDelete(p: Record<string, unknown>): void {
