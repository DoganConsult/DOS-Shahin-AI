import { describe, it, expect, vi, beforeEach } from 'vitest';

vi.mock('../../../platform/services/event/event-bus.service', () => ({
  eventBus: { publish: vi.fn() },
}));

import { emitPolicyEvent, emitPolicyStatusChange, emitPolicyDrafted, emitPolicyReviewed, emitPolicyApproved, emitPolicyRejected } from './policy-event.service';

vi.mock('../../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
import { eventBus } from '../../../ports/events.port';


vi.mock('../../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
describe('Policy Event Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('emitPolicyEvent', () => {
    it('should publish event to event bus', () => {
      emitPolicyEvent({
        tenantId: 't1',
        entityType: 'policy' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
    });

    it('should include correlationId in payload', () => {
      emitPolicyEvent({
        tenantId: 't1',
        entityType: 'policy' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
        correlationId: 'corr-123',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBe('corr-123');
    });

    it('should auto-generate correlationId if not provided', () => {
      emitPolicyEvent({
        tenantId: 't1',
        entityType: 'policy' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBeDefined();
    });

    it('should include timestamp and eventVersion', () => {
      emitPolicyEvent({
        tenantId: 't1',
        entityType: 'policy' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.timestamp).toBeDefined();
      expect(call.payload.eventVersion).toBe(1);
    });

    it('should not throw on publish failure', () => {
      (eventBus.publish as any).mockImplementationOnce(() => { throw new Error('fail'); });
      expect(() => emitPolicyEvent({
        tenantId: 't1',
        entityType: 'policy' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      })).not.toThrow();
    });
  });

  describe('emitPolicyStatusChange', () => {
    it('should emit status_changed event', () => {
      emitPolicyStatusChange('t1', 'policy' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.action).toBe('status_changed');
    });
  });

  describe('helper functions', () => {
    it('emitPolicyDrafted should not throw', () => {
      expect(() => emitPolicyDrafted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitPolicyReviewed should not throw', () => {
      expect(() => emitPolicyReviewed('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitPolicyApproved should not throw', () => {
      expect(() => emitPolicyApproved('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitPolicyRejected should not throw', () => {
      expect(() => emitPolicyRejected('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

  });
});
