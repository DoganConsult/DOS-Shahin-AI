...
export async function publish(
  ...
  if (!POLICY_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${POLICY_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: POLICY_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${POLICY_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(POLICY_EVENT_CONTRACT.published);
}
