import type { ExceptionRequest, ExceptionRequestCreateInput } from '../types/exception.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): ExceptionRequest {
  return {
    exception_id: row.exception_id ?? row.id,
...
export function toEntityList(rows: GenericRow[]): ExceptionRequest[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): ExceptionRequestCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as ExceptionRequestCreateInput;
}

export function toApiResponse(entity: ExceptionRequest): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: ExceptionRequest[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
