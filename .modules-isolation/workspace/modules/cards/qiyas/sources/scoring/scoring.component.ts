  ...
  template: `
    <app-page-shell icon="chart-bar" [title]="'Scoring Engine'"
      ...
      <div class="scoring-grid">
        <p-card *ngFor="let p of policies" styleClass="scoring-card">
          <div class="scoring-header">
            <h3>{{ p.name }}</h3>
            <p-tag [value]="p.is_default ? 'Default' : 'Custom'" [severity]="p.is_default ? 'success' : 'info'" />
          </div>
...
export class ScoringComponent implements OnInit {
  ...
  loading = false; error = ''; policies: Record<string, unknown>[] = [];
  ...
  ngOnInit() {
    ...
    this.complianceSvc.getScoringPolicies().subscribe({
      next: (d: Record<string, unknown>) => { this.policies = Array.isArray(d) ? d : d.policies || []; this.loading = false; this.cdr.markForCheck(); },
      error: () => { this.error = 'Failed to load data'; this.loading = false; this.cdr.markForCheck(); }
    });
  }
  getWeights(w: Record<string, unknown>): { key: string; value: number }[] {
