...
const SERVICE_CODE = 'qiyas-journey-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    port: config.port,
    routes: [
      { path: '/api/qiyas', router: routes },
    ],
    // Wave 2D: Module lifecycle registration
    modules: await loadModuleRegistrations('qiyas'),
    healthChecks: {
      database: async () => {
        try {
    ...
    onReady: () => {
      ...
      invokeModuleLifecycleHooks(SERVICE_CODE, 'qiyas').catch(() => {});
    },
  });
