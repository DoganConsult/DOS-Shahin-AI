import type { MaturityAssessment, MaturityAssessmentCreateInput } from '../types/qiyas.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): MaturityAssessment {
  return {
...
export function toEntityList(rows: GenericRow[]): MaturityAssessment[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): MaturityAssessmentCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as MaturityAssessmentCreateInput;
}

export function toApiResponse(entity: MaturityAssessment): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: MaturityAssessment[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
