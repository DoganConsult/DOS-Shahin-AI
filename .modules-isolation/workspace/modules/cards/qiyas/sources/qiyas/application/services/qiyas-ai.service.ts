export const QIYAS_AI_CONFIG = {
  moduleCode: 'qiyas' as const,
  enabled: true,

  allowedActions: [
    'qiyas.assessment.draft',
    'qiyas.assessment.recommend',
    'qiyas.assessment.summarize',
    'qiyas.assessment.classify',
    'qiyas.assessment.score',
    'qiyas.assessment.analyze',
    'qiyas.assessment.generate_report',
  ],

  blockedActions: [
    'qiyas.assessment.delete',
    'qiyas.assessment.approve',
    'qiyas.assessment.reject',
    'qiyas.assessment.override',
    'qiyas.assessment.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['qiyas.assessment.approve', 'qiyas.assessment.reject', 'qiyas.assessment.delete'],
    requiresHumanReview: ['qiyas.assessment.classify', 'qiyas.assessment.score'],
    autoExecutable: ['qiyas.assessment.draft', 'qiyas.assessment.summarize', 'qiyas.assessment.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'qiyas_system_prompt_v1',
  },
...
export function isQiyasAiActionAllowed(action: string): boolean {
  return (QIYAS_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isQiyasAiActionBlocked(action: string): boolean {
  return (QIYAS_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresQiyasHumanApproval(action: string): boolean {
  return (QIYAS_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".qiyas_assessments WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Qiyas entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Assessment"} (\${r.assessment_type ?? "maturity"}). Score: \${r.maturity_score ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".qiyas_assessments ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".qiyas_assessments ${where} GROUP BY status`, params);
