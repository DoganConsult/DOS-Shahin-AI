...
describe('qiyas-journey-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["compliance.assessed","control.tested","risk.mitigated"];
  ...
  it('handles control.tested without throwing', async () => {
    const handler = handlers.get('control.tested');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
      eventType: 'control.tested',
      tenantId: 't1',
      userId: 'u1',
      payload: { title: 'Test', category: 'auto', severity: 'medium', type: 'test' },
      timestamp: new Date().toISOString(),
