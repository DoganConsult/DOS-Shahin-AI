...
vi.mock('../events/publisher', () => ({
  ...
  publishQiyasAssessmentCreated: vi.fn().mockResolvedValue(undefined),
  publishQiyasLevelChanged: vi.fn().mockResolvedValue(undefined),
  publishQiyasRoadmapUpdated: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock('../domain/maturity-assessment.service', () => ({
...
import router from '../routes/maturity-assessment.routes';
import * as domainSvc from '../../domain/maturity-assessment.service';
...
function createApp() {
  ...
  app.use('/api/maturity-assessment', router);
  return app;
}

describe('maturity-assessment routes', () => {
  ...
  describe('GET /api/maturity-assessment/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/maturity-assessment/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/maturity-assessment/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/maturity-assessment/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/maturity-assessment/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/maturity-assessment/', () => {
    it('creates new record', async () => {
      const res = await request(app)
        .post('/api/maturity-assessment/')
        .send({ title: 'New Item', category: 'test', type: 'test', severity: 'medium', name: 'test', framework_name: 'test', control_ref: 'CR-1' });
      expect([200, 201, 400]).toContain(res.status);
    });
  });

  describe('PUT /api/maturity-assessment/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/maturity-assessment/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/maturity-assessment/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/maturity-assessment/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
