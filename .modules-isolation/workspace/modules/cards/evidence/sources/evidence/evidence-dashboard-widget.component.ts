...
export class EvidenceDashboardWidgetComponent implements OnInit {
  ...
  frameworkSummary: Record<string, unknown>[] = [];
  private maxReq = 1;
  ...
  ngOnInit(): void {
    this.complianceSvc.getEvidenceRequirementsSummary().subscribe({
      next: (res: Record<string, unknown>) => {
        ...
        this.maxReq = Math.max(...this.frameworkSummary.map((f: Record<string, unknown>) => f.requirement_count || 1), 1);
      },
      error: () => { this.frameworkSummary = []; }
    });
  }

  getPercent(fw: Record<string, unknown>): number {
