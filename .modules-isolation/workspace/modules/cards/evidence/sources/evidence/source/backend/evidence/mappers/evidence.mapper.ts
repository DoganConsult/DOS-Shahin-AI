import type { Evidence, EvidenceCreateInput } from '../types/evidence.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Evidence {
  return {
...
export function toEntityList(rows: GenericRow[]): Evidence[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): EvidenceCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as EvidenceCreateInput;
}

export function toApiResponse(entity: Evidence): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Evidence[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
