...
async function scoreReviewerSignOff(
  ...
  const reviewResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT outcome, reviewed_at
     ...
     LIMIT 1`,
    [evidenceId]
  ), { tenantId: tenantId, operation: 'query evidence_reviews' });

  if (reviewResult.rows.length === 0) {
    return { score: 30, reason: 'No review record found' };
  }

  const review = getFirstRow(reviewResult)!;

  const outcome = (review!.outcome || '').toLowerCase();
  const reviewedAt = review!.reviewed_at ? new Date((review as any).reviewed_at) : null;
...
async function scoreFormatMatch(
  ...
  const reqResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
  ...
  const expectedType = getFirstRow(reqResult)?.evidence_type_code;
  if (!expectedType) {
...
export async function loadQualityWeightsFromDb(tenantId: string): Promise<QualityWeights> {
  ...
  try {
    const res = await safeQuery(
      `SELECT dimension, weight FROM "${schema}".evidence_quality_rules WHERE active = true`,
      [],
    );

    if (res.rows.length === 0) return _dbWeightsCache ?? getDefaultWeights();
    ...
    for (const row of res.rows) {
      const key = dimensionMap[row.dimension as string];
      if (key) raw[key] += parseFloat(row.weight) || 0;
    }
...
export async function scoreEvidenceQuality(
  ...
  evidenceId: string
      ...
      return (result?.rows || []) as unknown as EvidenceQualityScore;
}
...
export async function scoreControlEvidence(
  ...
  const evidenceIds = evidenceResult.rows.map((r: GenericRow) => r.evidence_id);
