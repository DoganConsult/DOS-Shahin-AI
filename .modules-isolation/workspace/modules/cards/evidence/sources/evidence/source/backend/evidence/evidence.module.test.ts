...
beforeAll(async () => {
  const mod = await import('./evidence.module');
  manifest = mod.EVIDENCE_MANIFEST ?? mod.default?.manifest;
});

describe('Evidence Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('evidence'); });
