// Shahin GRC — KnowledgeHub  |  GRC Lifecycle: Assure
...
import { EntityGraphPageComponent } from '../../../../../../../modules/entity-graph/entity-graph-page.component';
import { KSAHubComponent } from '../ksa-hub/ksa-hub.component';
import { LocalKnowledgeHubComponent } from '../local-knowledge-hub/local-knowledge-hub.component';
...
const TABS: HubTab[] = [
  { key: 'knowledge', labelEn: 'Knowledge Base', labelAr: 'قاعدة المعرفة', icon: 'pi-book' },
  { key: 'graph',     labelEn: 'Entity Graph',   labelAr: 'رسم الكيانات',  icon: 'pi-sitemap' },
  { key: 'ksa',       labelEn: 'KSA Hub',        labelAr: 'مركز KSA',      icon: 'pi-star' },
  { key: 'local',     labelEn: 'Local Knowledge', labelAr: 'المعرفة المحلية', icon: 'pi-folder-open' },
];
...
@Component({
  ...
  selector: 'app-knowledge-hub', standalone: true,
  imports: [CommonModule, RouterModule, KnowledgeBaseComponent, EntityGraphPageComponent, KSAHubComponent, LocalKnowledgeHubComponent, AgentBadgeComponent, HubConnectionsStripComponent],
  styles: [HUB_STYLES],
  template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#dcfce7"><i class="pi pi-book hub-icon" style="color:#15803d"></i></div>
          <div>
            <div class="hub-phase" style="background:rgba(var(--module-accent-green-rgb), .12);color:#15803d">{{ i18n.currentLang()==='ar' ? 'التأكيد' : 'Assure' }}</div>
            <h1>{{ i18n.currentLang()==='ar' ? 'مركز المعرفة' : 'Knowledge Hub' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'قاعدة المعرفة، رسم الكيانات، مركز KSA — معلومات استخباراتية في مكان واحد.' : 'Knowledge base, entity graph, KSA hub — intelligence in one place.' }}</p>
          </div>
        </div>
      ...
      <app-hub-connections-strip [hubKey]="'knowledge'" />
      <div class="hub-tab-content">
        @if (activeTab()==='knowledge') { <app-knowledge-base /> }
        @if (activeTab()==='graph')     { <app-entity-graph-page /> }
        @if (activeTab()==='ksa')       { <app-ksa-hub /> }
        @if (activeTab()==='local')      { <app-local-knowledge-hub /> }
      </div>
    </div>`
})
export class KnowledgeHubComponent implements OnInit {
  ...
  activeTab = signal<string>('knowledge');
  ngOnInit(): void {
