...
import { safeQuery as _safeQuery, safeQueryWithClient, tenantSchema } from '../../action/ports/database.port';
...
export async function getCustodyChain(
  ...
  try {
    const res = await LocalKnowledgeAutoRepo.query51(tenantSchema(tenantId), [tenantId, documentId]);
...
export async function verifyCustodyChainIntegrity(
  ...
  for (const event of chain) {
    ...
    const docRes = await LocalKnowledgeAutoRepo.query50(tenantSchema(tenantId), [documentId, tenantId]);
