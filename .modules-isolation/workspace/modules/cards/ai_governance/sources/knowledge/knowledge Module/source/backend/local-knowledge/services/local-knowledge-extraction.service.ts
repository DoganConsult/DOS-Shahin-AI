...
import { claudeJSON, claudeComplete, CLAUDE_MODEL as _CLAUDE_MODEL } from '../../ai/ports/ai.port';
...
async function getIngestion(tenantId: string, ingestionId: string): Promise<Record<string, unknown> | null> {
  ...
  const { rows } = await LocalKnowledgeAutoRepo.query82(tenantSchema(tenantId), [ingestionId]);
  return rows[0] || null;
}
 ...
 */
function flattenJSON(obj: unknown, prefix: string, depth: number): string {
