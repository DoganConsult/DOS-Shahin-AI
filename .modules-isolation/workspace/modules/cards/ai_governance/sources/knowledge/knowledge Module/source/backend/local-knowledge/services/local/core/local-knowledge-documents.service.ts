...
import { emptyResult, safeQuery, tenantSchema, withTransaction } from '../../../ports/database.port';
...
async function listDocumentsCanonical(
  ...
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT * FROM "${schema}".local_knowledge_documents WHERE ${where} ORDER BY updated_at DESC LIMIT $${idx++} OFFSET $${idx++}`,
    [...params, pageSize, offset],
  ), { tenantId, operation: 'list documents' });
  const countRes = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total FROM "${schema}".local_knowledge_documents WHERE ${where}`,
    params,
  ), { tenantId, operation: 'count documents' });
  const total = countRes.rows[0]?.total ?? 0;
  return { data: res.rows.map(mapDocRow), total, page, pageSize, totalPages: Math.ceil(total / pageSize) };
...
async function upsertDocument(
  ...
  const res = await safeQuery(
    `INSERT INTO "${schema}".local_knowledge_documents
     ...
     RETURNING *`,
    [tenantId, title, documentType, ingestionId, JSON.stringify(canonicalData),
...
export async function getDocument(
  ...
  const [chunkRes, accessRes] = await Promise.all([
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ chunk_count: 0 }]), safeQuery(
      `SELECT COUNT(*)::int AS chunk_count
       FROM "${schema}".local_knowledge_chunks
       WHERE document_id = $1 AND tenant_id = $2`,
      [documentId, tenantId],
    ), { tenantId: tenantId, operation: 'query local_knowledge_chunks' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ last_accessed_at: null }]), safeQuery(
      `SELECT MAX(accessed_at) AS last_accessed_at
...
export async function updateDocument(
  ...
  const whereClause = `WHERE document_id = $${paramIndex++} AND tenant_id = $${paramIndex++}`;

  try {
    await safeQuery(
      `UPDATE "${schema}".local_knowledge_documents SET ${sets.join(', ')} ${whereClause}`,
      params,
    );
...
export async function getDocumentStats(tenantId: string): Promise<DocumentStats> {
  ...
  try {
    const [typesRes, statusRes, laneRes, storageRes, embeddingRes, chunksRes] = await Promise.all([
      ...
      safeQuery(
        `SELECT knowledge_lane, COUNT(*)::int AS count
         FROM "${schema}".local_knowledge_documents
         WHERE tenant_id = $1 AND deleted_at IS NULL
 ...
 */
async function ensureManualSource(tenantId: string, schema: string): Promise<string> {
  const existing = await safeQuery(
    `SELECT source_id FROM "${schema}".local_knowledge_sources
