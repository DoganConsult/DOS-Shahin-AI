...
import {
  ...
  getFailedIndexes,
  getStaleDocuments,
} from './local-knowledge-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getFailedIndexes should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getFailedIndexes('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getFailedIndexes should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getFailedIndexes('t1');
      expect(result).toEqual([]);
    });

    it('getStaleDocuments should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getStaleDocuments('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getStaleDocuments should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getStaleDocuments('t1');
      expect(result).toEqual([]);
    });
