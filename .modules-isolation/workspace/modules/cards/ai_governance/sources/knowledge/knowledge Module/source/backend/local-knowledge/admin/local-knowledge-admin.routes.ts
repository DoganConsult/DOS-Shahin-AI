...
const router = Router();
router.use(moduleStack('local-knowledge'));
router.use(auditMiddleware('local_knowledge'));

router.get(
  '/settings',
  authenticate,
  requirePermission('local_knowledge.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
