...
export async function runDiagnostics(tenantId: string): Promise<DiagnosticsResult> {
  ...
  const ownedTables = ['local_knowledge_documents', 'local_knowledge_chunks', 'local_knowledge_embeddings', 'local_knowledge_sources', 'local_knowledge_access_log'];
  ...
  checks.push({
    ...
    detail: `${tableRows.length}/${ownedTables.length} core local-knowledge tables found`,
  });

  const { rows: staleRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".local_knowledge_documents WHERE updated_at < NOW() - INTERVAL '90 days'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
  ...
  const { rows: auditRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".audit_trail WHERE module = 'local-knowledge' AND created_at > NOW() - INTERVAL '24 hours'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
