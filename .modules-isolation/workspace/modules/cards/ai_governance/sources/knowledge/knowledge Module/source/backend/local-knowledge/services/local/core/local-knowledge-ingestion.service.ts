...
import { safeQuery, tenantSchema, withTransaction as _withTransaction } from '../../../ports/database.port';
import { claudeJSON, CLAUDE_MODEL as _CLAUDE_MODEL } from '../../../ports/ai.port';
...
export async function reIngestDocument(
  ...
  await Promise.all([
    safeQuery(
      `DELETE FROM "${schema}".local_knowledge_chunks WHERE document_id = $1 AND tenant_id = $2`,
      [documentId, tenantId],
    ).catch(catchHandler(EC.EVENT_BUS, {})),
    safeQuery(
      `DELETE FROM "${schema}".local_knowledge_index WHERE knowledge_item_id = $1 AND tenant_id = $2`,
      [documentId, tenantId],
    ).catch(catchHandler(EC.EVENT_BUS, {})),
  ]);
...
export async function getIngestionStatus(
  ...
  try {
    ...
    const chunkRes = await safeQuery(
      `SELECT COUNT(*)::int AS count FROM "${schema}".local_knowledge_chunks
...
async function updateKnowledgeIndex(
  ...
  try {
    ...
    // Insert index entry
    await safeQuery(
      `INSERT INTO "${schema}".local_knowledge_index
...
async function updateIngestionStatus(
  ...
  const schema = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${schema}".local_knowledge_ingestions
     SET metadata = COALESCE(metadata, '{}'::jsonb) || $1::jsonb, updated_at = NOW()
     WHERE id = $2`,
    [JSON.stringify(data), ingestionId],
  );
}
...
export async function logIngestion(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".local_knowledge_ingestions (document_id, metadata)
...
export async function listIngestions(
  ...
  const countRes = await safeQuery(
    `SELECT COUNT(*)::int AS total FROM "${schema}".local_knowledge_ingestions`,
  );
  const total = countRes.rows[0]?.total ?? 0;

  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".local_knowledge_ingestions ORDER BY created_at DESC LIMIT $1 OFFSET $2`,
    [pageSize, offset],
  );
