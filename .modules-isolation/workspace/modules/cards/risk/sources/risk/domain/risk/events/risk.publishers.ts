...
export async function publish(
  ...
  if (!RISK_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${RISK_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: RISK_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${RISK_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(RISK_EVENT_CONTRACT.published);
}
