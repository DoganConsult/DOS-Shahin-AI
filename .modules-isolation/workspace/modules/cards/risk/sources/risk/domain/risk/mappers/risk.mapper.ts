...
export function toEntityList(rows: GenericRow[]): Risk[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): RiskCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Risk): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Risk[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
...
const SENSITIVE_FIELDS: readonly string[] = ['financial_impact', 'risk_owner_email'];
