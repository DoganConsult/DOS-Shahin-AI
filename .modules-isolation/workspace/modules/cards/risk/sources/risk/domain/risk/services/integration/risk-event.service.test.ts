import { describe, it, expect, vi, beforeEach } from 'vitest';

vi.mock('../../../platform/services/event/event-bus.service', () => ({
  eventBus: { publish: vi.fn() },
}));

import { emitRiskEvent, emitRiskStatusChange, emitRiskIdentified, emitRiskAssessed, emitRiskAccepted, emitRiskTransferred } from './risk-event.service';

vi.mock('../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
import { eventBus } from '../../ports/events.port';


vi.mock('../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
describe('Risk Event Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('emitRiskEvent', () => {
    it('should publish event to event bus', () => {
      emitRiskEvent({
        tenantId: 't1',
        entityType: 'risk' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
    });

    it('should include correlationId in payload', () => {
      emitRiskEvent({
        tenantId: 't1',
        entityType: 'risk' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
        correlationId: 'corr-123',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBe('corr-123');
    });

    it('should auto-generate correlationId if not provided', () => {
      emitRiskEvent({
        tenantId: 't1',
        entityType: 'risk' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBeDefined();
    });

    it('should include timestamp and eventVersion', () => {
      emitRiskEvent({
        tenantId: 't1',
        entityType: 'risk' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.timestamp).toBeDefined();
      expect(call.payload.eventVersion).toBe(1);
    });

    it('should not throw on publish failure', () => {
      (eventBus.publish as any).mockImplementationOnce(() => { throw new Error('fail'); });
      expect(() => emitRiskEvent({
        tenantId: 't1',
        entityType: 'risk' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      })).not.toThrow();
    });
  });

  describe('emitRiskStatusChange', () => {
    it('should emit status_changed event', () => {
      emitRiskStatusChange('t1', 'risk' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.action).toBe('status_changed');
    });
  });

  describe('helper functions', () => {
    it('emitRiskIdentified should not throw', () => {
      expect(() => emitRiskIdentified('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitRiskAssessed should not throw', () => {
      expect(() => emitRiskAssessed('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitRiskAccepted should not throw', () => {
      expect(() => emitRiskAccepted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitRiskTransferred should not throw', () => {
      expect(() => emitRiskTransferred('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

  });
});
