...
export class VulnerabilitiesComponent implements OnInit {
  ...
  form: Record<string, any> = { title: '', description: '', cve_id: '', source: 'manual', severity: 'medium', cvss_score: null, assigned_to: '', remediation_plan: '', remediation_due: '' };

  ngOnInit(): void {
    this.loadVulns();
    this.live.debounced(800).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.loadVulns());
  }
  ...
  save(): void {
    ...
    obs.subscribe({ next: () => { this.resetForm(); this.loadVulns(); }, error: (e: any) => devError(e) });
  }
