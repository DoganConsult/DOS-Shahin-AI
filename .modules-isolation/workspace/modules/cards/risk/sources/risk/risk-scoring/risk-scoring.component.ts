  ...
  loadPosture(): void {
    ...
    this.apiclientSvc.get('/risk-scoring/posture').subscribe({
      next: (res) => {
        ...
        const data = asRecord(payload['data'] ?? res);
        this.risks = asRecordArray(data['risks']).length ? asRecordArray(data['risks']) : asRecordArray(data['topRisks']);
        const byZone = asRecord(data['byZone']);
        this.zones = [
        ...
        this.recommendations = Array.isArray(data['recommendations'])
          ? data['recommendations'].map((item) => asString(item)).filter(Boolean)
          : [];
        this.riskOptions = this.risks
          .map((r) => ({
            label: asString(r['title']) || asString(r['riskId']) || asString(r['risk_id']) || '—',
            value: asString(r['riskId']) || asString(r['risk_id']),
          }))
