import type { Incident, IncidentCreateInput } from '@dos/types/incident';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Incident {
...
export function toEntityList(rows: GenericRow[]): Incident[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): IncidentCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as IncidentCreateInput;
}

export function toApiResponse(entity: Incident): Record<string, unknown> {

  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Incident[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
