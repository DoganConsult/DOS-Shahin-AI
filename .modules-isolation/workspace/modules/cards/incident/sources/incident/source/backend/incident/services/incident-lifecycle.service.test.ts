...
} from './incident-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getOpenIncidents should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOpenIncidents('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOpenIncidents should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOpenIncidents('t1');
      expect(result).toEqual([]);
    });

    it('getEscalatedIncidents should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getEscalatedIncidents('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getEscalatedIncidents should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getEscalatedIncidents('t1');
      expect(result).toEqual([]);
    });
