...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, PageShellComponent, AiPanelComponent, CardModule, ButtonModule, TagModule, DropdownModule,
    ToolbarModule, InputTextModule, InputTextarea, DialogModule, ProgressBarModule, TooltipModule, BadgeModule, ChipModule, TabViewModule, ToastModule],
    ...
    <app-page-shell icon="clipboard" [title]="i18n.translate('grcOs.assessmentTemplates')"
      ...
      <div class="filter-bar mb-3">
        <div class="flex flex-wrap align-items-center gap-2">
          ...
          <p-dropdown [options]="categoryOptions" [(ngModel)]="selectedCategory" (onChange)="applyFilters()"
            [placeholder]="i18n.translate('assessmentTemplates.allCategories')" [showClear]="true" styleClass="filter-dropdown" />
          <p-dropdown [options]="difficultyOptions" [(ngModel)]="selectedDifficulty" (onChange)="applyFilters()"
            [placeholder]="i18n.translate('assessmentTemplates.allLevels')" [showClear]="true" styleClass="filter-dropdown" />
          <p-dropdown [options]="industryOptions" [(ngModel)]="selectedIndustry" (onChange)="applyFilters()"
            [placeholder]="i18n.translate('assessmentTemplates.allIndustries')" [showClear]="true" styleClass="filter-dropdown" />
          <p-button [label]="i18n.translate('assessmentTemplates.createTemplate')" icon="pi pi-plus" (onClick)="openCreateDialog()" styleClass="mr-2" />
      ...
      <div class="flex flex-wrap gap-2 mb-3">
        <p-chip [label]="'All (' + allTemplates.length + ')'" [styleClass]="!selectedCategory ? 'chip-active' : 'chip-inactive'"
          (click)="selectedCategory = ''; applyFilters()" />
        @for (cat of categories; track cat.id) {
          <p-chip [label]="cat.nameEn + ' (' + cat.count + ')'" [styleClass]="selectedCategory === cat.id ? 'chip-active' : 'chip-inactive'"
            ...
            <div tabindex="0" role="button" (keyup.enter)="openDetail(tpl)" class="tpl-card" (click)="openDetail(tpl)">
              <div class="tpl-header">
                <div class="flex justify-content-between align-items-start">
                  ...
                  <p-tag [value]="tpl.difficulty || 'intermediate'" [severity]="difficultyColor(tpl.difficulty)" [rounded]="true" />
                </div>
                <div class="tpl-desc mt-2">{{ tpl.description_en || tpl.description }}</div>
              </div>
              <div class="tpl-meta">
                <div class="flex flex-wrap gap-2 mb-2">
                  <span class="meta-chip"><i class="pi pi-clock"></i> {{ tpl.estimated_minutes || 60 }} min</span>
                  <span class="meta-chip"><i class="pi pi-list"></i> {{ tpl.question_count || 0 }} questions</span>
                  <span class="meta-chip"><i class="pi pi-chart-bar"></i> {{ scoringLabel(tpl.scoring_methodology) }}</span>
                </div>
              ...
              <div class="tpl-actions">
                <p-button label="Start" icon="pi pi-play" size="small" (onClick)="startAssessment(tpl); $event.stopPropagation()" />
                <p-button label="Preview" icon="pi pi-eye" size="small" severity="secondary" [outlined]="true"
                  (onClick)="openDetail(tpl); $event.stopPropagation()" />
                <p-button label="Edit" icon="pi pi-pencil" size="small" severity="secondary" [text]="true"
                  (onClick)="openEditDialog(tpl); $event.stopPropagation()" />
                <p-button icon="pi pi-trash" size="small" severity="danger" [text]="true"
                  (onClick)="confirmDelete(tpl); $event.stopPropagation()" pTooltip="Delete" />
              </div>
            </div>
          </div>
        }
      ...
      <!-- Detail Dialog -->
      <p-dialog [(visible)]="showDetail" [modal]="true" [style]="{width:'700px','max-height':'85vh'}" [header]="detailTemplate?.name_en || ''">
        @if (detailTemplate) {
          <p-tabView>
            <p-tabPanel header="Overview">
              ...
              <div class="grid">
                ...
                <div class="col-6"><strong>Difficulty:</strong> <p-tag [value]="detailTemplate.difficulty" [severity]="difficultyColor(detailTemplate.difficulty)" /></div>
            ...
            <p-tabPanel header="Questions & AI Guide" [disabled]="!detailQuestions.length">
                  ...
                  <div class="flex align-items-start gap-2">
                    ...
                    <div class="flex-grow-1">
                      ...
                      <div class="q-meta mt-1">
                        <span class="meta-chip"><i class="pi pi-tag"></i> {{ q.domain }}</span>
                        <span class="meta-chip"><i class="pi pi-star"></i> Max {{ q.maxScore }}</span>
                        <span class="meta-chip"><i class="pi pi-file"></i> {{ q.evidenceHint }}</span>
                      </div>
                      @if (q.aiGuide) {
                        <div class="ai-guide mt-2">
                          <div class="ai-label"><i class="pi pi-bolt"></i> AI Guide</div>
          ...
          <div class="flex justify-content-end gap-2 mt-3">
            <p-button label="Close" severity="secondary" [outlined]="true" (onClick)="showDetail = false" />
            <p-button label="Start Assessment" icon="pi pi-play" (onClick)="startAssessment(detailTemplate); showDetail = false" />
          </div>
        }
      </p-dialog>

      <!-- Create/Edit Template Dialog -->
      <p-dialog [header]="editMode ? i18n.translate('assessmentTemplates.editTemplate') : i18n.translate('assessmentTemplates.createTemplate')"
        ...
        <div class="dialog-form">
          ...
          <div class="field-row">
            ...
            <div class="field"><label>Difficulty</label>
              <p-dropdown [(ngModel)]="form.difficulty" [options]="difficultyOptions" optionLabel="label" optionValue="value" styleClass="w-full" />
            </div>
          </div>
          <div class="field-row">
            <div class="field"><label>Scoring</label>
              <p-dropdown [(ngModel)]="form.scoring_methodology" [options]="scoringOptions" optionLabel="label" optionValue="value" styleClass="w-full" />
            </div>
        ...
        <ng-template pTemplate="footer">
          <p-button [label]="i18n.translate('common.cancel')" icon="pi pi-times" severity="secondary" [text]="true" (onClick)="showFormDialog = false" />
          <p-button [label]="i18n.translate('common.save')" icon="pi pi-check" (onClick)="saveTemplate()" [disabled]="!form.name_en" />
        </ng-template>
      </p-dialog>

      <!-- Delete Confirmation Dialog -->
      <p-dialog [header]="i18n.translate('assessmentTemplates.confirmDelete')"
        ...
        <ng-template pTemplate="footer">
          ...
          <p-button [label]="i18n.translate('common.delete')" icon="pi pi-trash" severity="danger" (onClick)="deleteTemplate()" />
        </ng-template>
      </p-dialog>
    </app-page-shell>
...
export class AssessmentTemplatesComponent implements OnInit {
  ...
  openDetail(tpl: GrcRecord) {
    ...
    this.complianceSvc.getAssessmentTemplateQuestions(id).subscribe({
      next: (data: any) => { this.detailQuestions = data.questions || []; },
      error: (e) => devError("[API]", e)
    });
  }
