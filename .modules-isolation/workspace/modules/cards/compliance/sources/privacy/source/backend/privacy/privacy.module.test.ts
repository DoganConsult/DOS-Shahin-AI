...
beforeAll(async () => {
  const mod = await import('./privacy.module');
  manifest = mod.PRIVACY_MANIFEST ?? mod.default?.manifest;
});

describe('Privacy Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('privacy'); });
