export const PRIVACY_AI_CONFIG = {
  moduleCode: 'privacy' as const,
  enabled: true,

  allowedActions: [
    'privacy.record.draft',
    'privacy.record.recommend',
    'privacy.record.summarize',
    'privacy.record.classify',
    'privacy.record.score',
    'privacy.record.analyze',
    'privacy.record.generate_report',
  ],

  blockedActions: [
    'privacy.record.delete',
    'privacy.record.approve',
    'privacy.record.reject',
    'privacy.record.override',
    'privacy.record.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['privacy.record.approve', 'privacy.record.reject', 'privacy.record.delete'],
    requiresHumanReview: ['privacy.record.classify', 'privacy.record.score'],
    autoExecutable: ['privacy.record.draft', 'privacy.record.summarize', 'privacy.record.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'privacy_system_prompt_v1',
  },
...
export function isPrivacyAiActionAllowed(action: string): boolean {
  return (PRIVACY_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isPrivacyAiActionBlocked(action: string): boolean {
  return (PRIVACY_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresPrivacyHumanApproval(action: string): boolean {
  return (PRIVACY_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".privacy_requests WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Privacy entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Privacy Request"} (\${r.request_type ?? "access"}). Deadline: \${r.response_deadline ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".privacy_requests ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".privacy_requests ${where} GROUP BY status`, params);
