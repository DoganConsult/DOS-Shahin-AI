...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getOpenAssessments should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOpenAssessments('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOpenAssessments should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOpenAssessments('t1');
      expect(result).toEqual([]);
    });

    it('getExpiredConsents should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getExpiredConsents('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getExpiredConsents should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getExpiredConsents('t1');
      expect(result).toEqual([]);
    });
