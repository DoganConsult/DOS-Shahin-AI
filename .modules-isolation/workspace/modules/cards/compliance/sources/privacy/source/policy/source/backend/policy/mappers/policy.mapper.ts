import type { Policy, PolicyCreateInput } from '../types/policy.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Policy {
  return {
...
export function toEntityList(rows: GenericRow[]): Policy[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): PolicyCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as PolicyCreateInput;
}

export function toApiResponse(entity: Policy): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Policy[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
