...
import * as svc from '../../domain/control.service';
...
describe('control.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { control_ref: 'test-control_ref', title: 'test-title' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { control_ref: 'x', title: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
