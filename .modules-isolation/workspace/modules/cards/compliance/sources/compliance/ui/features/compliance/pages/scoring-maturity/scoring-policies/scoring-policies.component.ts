    ...
    <app-page-shell
      ...
      <section cdsToolbar styleClass="mb-3">
        ...
        <ng-template pTemplate="end">
          <button cdsButton [label]="i18n.translate('scoringPolicies.export')" icon="" severity="secondary"
      ...
      <div *ngIf="loaded && filteredItems.length === 0" class="empty-state">
        ...
        <button cdsButton [label]="i18n.translate('scoringPolicies.addPolicy')" icon=""
      ...
      <cds-modal
        ...
        <div class="dialog-form">
          ...
          <div class="field-row">
            <div class="field">
              ...
              <cds-dropdown [(ngModel)]="form.model_type" [options]="modelTypeOptions"
        ...
        <ng-template pTemplate="footer">
          <button cdsButton [label]="i18n.translate('scoringPolicies.cancel')" icon=""
                    severity="secondary" [text]="true" (onClick)="showDialog = false" />
          <button cdsButton [label]="i18n.translate('scoringPolicies.save')" icon=""
                    (onClick)="saveItem()" [disabled]="!form.name" />
        </ng-template>
      </cds-modal>

      <cds-modal [header]="i18n.translate('scoringPolicies.confirmDelete')"
                [(visible)]="showDeleteDialog" [modal]="true" [style]="{width:'400px'}">
        <p>{{ i18n.translate('scoringPolicies.confirmDeleteMsg') }}</p>
        <ng-template pTemplate="footer">
          <button cdsButton [label]="i18n.translate('scoringPolicies.cancel')" severity="secondary" [text]="true"
                    (onClick)="showDeleteDialog = false" />
          <button cdsButton [label]="i18n.translate('scoringPolicies.delete')" icon=""
...
export class ScoringPoliciesComponent implements OnInit {
  ...
  form: Record<string, any> = { name: '', description: '', model_type: 'weighted_average', status: 'draft' };
  statusOptions = [
  ...
  summarizeWeights(config: Record<string, any>): string {
    if (!config) return '—';
