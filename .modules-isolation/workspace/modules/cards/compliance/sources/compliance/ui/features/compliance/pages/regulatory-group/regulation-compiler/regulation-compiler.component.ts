...
import { ButtonModule, InputModule, NotificationModule, TableModule, TagModule, TilesModule } from 'carbon-components-angular';
import { MessageService } from '@app/services/toast.service';

@Component({
    changeDetection: ChangeDetectionStrategy.OnPush,
    selector: 'app-regulation-compiler',
    imports: [CommonModule, FormsModule, PageShellComponent, TilesModule, InputModule, ButtonModule, TagModule, TableModule, NotificationModule],
    providers: [],
    template: `
    <app-page-shell icon="gavel" title="Regulation Compiler"
      ...
      <div class="search-row" *ngIf="!error">
        ...
        <button cdsButton label="Compile" icon="" (onClick)="compile()" />
      </div>
      <div *ngIf="result && !error" class="result-section">
        ...
        <table cdsTable aria-label="Data table" [value]="result.controls || []" [paginator]="true" [rows]="20" styleClass="p-datatable-sm">
          ...
          <ng-template pTemplate="body" let-c>
            <tr>
              ...
              <td>{{ c.domain }}</td>
              <td><cds-tag [value]="c.level || 'standard'" /></td>
            </tr>
          </ng-template>
        </table>
      </div>
