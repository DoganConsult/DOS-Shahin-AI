...
@Component({
    ...
    styleUrl: './control-posture.component.css'
})
export class ControlPostureComponent implements OnInit {
  ...
  loadAll(): void {
    this.http.get<any>(`${this.api}/controls`).pipe(catchError(() => of({ controls: [] })), takeUntilDestroyed(this.destroyRef)).subscribe(d => {
  ...
  computeStats(): void {
    ...
    this.activeControls.set(c.filter((x: Record<string, any>) => x.status === 'active' || x.stage === 'active').length);
    const stale = c.filter((x: Record<string, any>) => x.status === 'stale' || x.is_stale).length;
    this.stalePercent.set(c.length > 0 ? Math.round((stale / c.length) * 100) : 0);
    const tested = c.filter((x: Record<string, any>) => x.last_tested || x.tested).length;
  ...
  buildEffectivenessGrid(): void {
    const c = this.controls();
    if (!c.length) return;
    const domains = [...new Set(c.map((x: Record<string, any>) => x.domain || x.category || 'General'))].slice(0, 8);
    const levels = [
    ...
    domains.forEach((dom, di) => {
      const domControls = c.filter((x: Record<string, any>) => (x.domain || x.category || 'General') === dom);
      const eff = domControls.filter((x: Record<string, any>) => x.effectiveness === 'effective' || x.score >= 80).length;
      const part = domControls.filter((x: Record<string, any>) => x.effectiveness === 'partial' || (x.score >= 40 && x.score < 80)).length;
      const ineff = domControls.filter((x: Record<string, any>) => x.effectiveness === 'ineffective' || (x.score > 0 && x.score < 40)).length;
  ...
  buildControlSankey(frameworks: Record<string, any>[]): void {
    if (!frameworks.length) return;
    const c = this.controls();
    const fwNames = frameworks.slice(0, 6).map((f: Record<string, any>) => f.name || f.framework_name || 'FW');
    const domains = [...new Set(c.map((x: Record<string, any>) => x.domain || x.category || 'General'))].slice(0, 6);
    const nodes = [...fwNames.map(n => ({ name: n })), ...domains.map(n => ({ name: n }))];
    const links: Record<string, any>[] = [];
    fwNames.forEach(fw => {
      domains.forEach(dom => {
        const count = c.filter((x: Record<string, any>) => (x.domain || x.category || 'General') === dom).length;
