...
@Component({
  ...
  template: `
    <app-page-shell
      icon="check-square"
      [title]="i18n.translate('soxCompliance.title')"
      [subtitle]="i18n.translate('soxCompliance.subtitle')"
      [breadcrumbs]="['Dashboard', 'SOX Compliance']"
      [loading]="!loaded">
      ...
      <div class="kpi-row" *ngIf="loaded">
        <div class="kpi-card">
          ...
          <span class="kpi-label">{{ i18n.translate('soxCompliance.totalControls') }}</span>
        </div>
        <div class="kpi-card">
          <span class="kpi-value">{{ controlsEffective }}</span>
          <span class="kpi-label">{{ i18n.translate('soxCompliance.effectiveControls') }}</span>
        </div>
        <div class="kpi-card">
          <span class="kpi-value">{{ policiesApproved }}/{{ policiesTotal }}</span>
          <span class="kpi-label">{{ i18n.translate('soxCompliance.policiesApproved') }}</span>
        </div>
        <div class="kpi-card">
          <span class="kpi-value">{{ frameworksList.length }}</span>
          <span class="kpi-label">{{ i18n.translate('soxCompliance.frameworks') }}</span>
        </div>
      </div>

      <!-- Frameworks Table (read-only) -->
      <h3 class="section-title" *ngIf="loaded && frameworksList.length > 0">{{ i18n.translate('soxCompliance.frameworks') }}</h3>

      <table cdsTable aria-label="Frameworks List table" [value]="frameworksList" [paginator]="frameworksList.length > 10"
               [rows]="10" styleClass="p-datatable-striped p-datatable-gridlines mb-3"
               *ngIf="frameworksList.length > 0">
        <ng-template pTemplate="header">
          <tr>
            <th>{{ i18n.translate('soxCompliance.framework') }}</th>
            <th>{{ i18n.translate('soxCompliance.code') }}</th>
            <th>{{ i18n.translate('soxCompliance.controls') }}</th>
            <th>{{ i18n.translate('common.status') }}</th>
          </tr>
        </ng-template>
        <ng-template pTemplate="body" let-f>
          <tr>
            <td><strong>{{ f.name ?? f.framework_name ?? f.title }}</strong></td>
            <td><code class="code-badge">{{ f.code ?? f.framework_code ?? '—' }}</code></td>
            <td>{{ f.control_count ?? f.controls ?? 0 }}</td>
            <td><app-status-badge [status]="f.status ?? 'active'" /></td>
          </tr>
        </ng-template>
        <ng-template pTemplate="emptymessage">
          <tr><td colspan="99" class="empty-msg">{{ i18n.translate('soxCompliance.noFrameworks') }}</td></tr>
        </ng-template>
      </table>

      <!-- Remediations Section -->
      <h3 class="section-title" *ngIf="loaded">{{ i18n.translate('soxCompliance.openRemediations') }}</h3>

      <section cdsToolbar styleClass="mb-3" *ngIf="loaded">
        <ng-template pTemplate="start">
          <button cdsButton
            [label]="i18n.translate('soxCompliance.addRemediation')"
            icon="" (onClick)="openCreateDialog()" />
          <span class="p-input-icon-left ms-3">
            <i class=""></i>
            <input type="text" pInputText [(ngModel)]="searchTerm"
                   [placeholder]="i18n.translate('soxCompliance.search')" [attr.aria-label]="i18n.translate('soxCompliance.search')"
                   (input)="filterItems()" class="search-input" />
          </span>
        </ng-template>
        <ng-template pTemplate="end">
          <button cdsButton [label]="i18n.translate('common.export')" icon="" severity="secondary"
      ...
      <table cdsTable aria-label="Filtered Items table" [value]="filteredItems" [paginator]="filteredItems.length > 10"
        ...
        <ng-template pTemplate="header">
          <tr>
            <th>{{ i18n.translate('soxCompliance.issue') }}</th>
      ...
      <div *ngIf="loaded && filteredItems.length === 0" class="empty-state">
        ...
        <p>{{ i18n.translate('soxCompliance.noOpenRemediations') }}</p>
        <button cdsButton [label]="i18n.translate('soxCompliance.addRemediation')" icon=""
                  (onClick)="openCreateDialog()" [outlined]="true" />
      </div>

      <cds-modal
        [header]="editMode ? i18n.translate('soxCompliance.editRemediation') : i18n.translate('soxCompliance.newRemediation')"
        [(visible)]="showDialog" [modal]="true" [style]="{width:'560px'}">
        <div class="dialog-form">
          <div class="field">
            <label>{{ i18n.translate('soxCompliance.titleField') }}</label>
          ...
          <div class="field-row">
            <div class="field">
              <label>{{ i18n.translate('common.priority') }}</label>
        ...
        <ng-template pTemplate="footer">
          <button cdsButton [label]="i18n.translate('common.cancel')" icon=""
                    severity="secondary" [text]="true" (onClick)="showDialog = false" />
          <button cdsButton [label]="i18n.translate('common.save')" icon=""
                    (onClick)="saveItem()" [disabled]="!form.title" />
        </ng-template>
      </cds-modal>

      <cds-modal [header]="i18n.translate('soxCompliance.confirmDelete')"
                [(visible)]="showDeleteDialog" [modal]="true" [style]="{width:'400px'}">
        <p>{{ i18n.translate('soxCompliance.confirmDeleteMessage') }}</p>
        <ng-template pTemplate="footer">
          <button cdsButton [label]="i18n.translate('common.cancel')" severity="secondary" [text]="true"
                    (onClick)="showDeleteDialog = false" />
          <button cdsButton [label]="i18n.translate('common.delete')" icon=""
                    severity="danger" (onClick)="deleteItem()" />
        </ng-template>
      </cds-modal>

    </app-page-shell>
    <app-ai-panel module="sox-compliance" />
  `,
  styles: [`
    ...
    .code-badge { background: var(--surface-hover, var(--surface-ice)); padding: 2px 8px; border-radius: var(--radius-xs); font-size: var(--font-size-sm); font-family: monospace; }
  `]
})
export class SOXComplianceComponent implements OnInit {
  private msg = inject(MessageService);

    private apiclientSvc = inject(ApiClientService);
  items: Record<string, any>[] = [];
  filteredItems: Record<string, any>[] = [];
  frameworksList: Record<string, any>[] = [];
  loaded = false;
  searchTerm = '';
  showDialog = false;
  ...
  load(): void {
    forkJoin({
      ...
      frameworks: this.complianceSvc.getFrameworks().pipe(catchError(() => of([]))),
      remediations: this.complianceSvc.getRemediations().pipe(catchError(() => of([]))),
    }).subscribe({
      next: ({ compliance, frameworks: fw, remediations }) => {
        const cc = compliance as any; const rr = remediations as any;
        ...
        const fwList = (fw as any)?.frameworks ?? (Array.isArray(fw) ? fw : []);
        this.frameworksList = fwList;
  ...
  saveItem(): void {
    ...
    obs.subscribe({
      next: () => {
        ...
        this.load();
        this.msg.add({ severity: 'success', summary: this.i18n.translate('common.success'),
          detail: this.editMode ? this.i18n.translate('soxCompliance.updated') : this.i18n.translate('soxCompliance.created'), life: 3000 });
      },
      error: () => { this.msg.add({ severity: 'error', summary: this.i18n.translate('common.error'), detail: this.i18n.translate('soxCompliance.operationFailed'), life: 4000 }); }
    });
  }
  ...
  deleteItem(): void {
    ...
    this.apiclientSvc.del('/compliance/remediations/' + id).subscribe({
      next: () => {
        ...
        this.load();
        this.msg.add({ severity: 'success', summary: this.i18n.translate('soxCompliance.deleted'), detail: this.i18n.translate('soxCompliance.recordRemoved'), life: 3000 });
      },
      error: () => { this.msg.add({ severity: 'error', summary: this.i18n.translate('common.error'), detail: this.i18n.translate('soxCompliance.deleteFailed'), life: 4000 }); }
    });
  }
  ...
  exportCSV(): void {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'sox-remediations.csv'; a.click();
  }
}
