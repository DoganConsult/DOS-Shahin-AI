...
interface ComplianceDashboardData {
  ...
  kpis: { totalObligations: number; compliant: number; nonCompliant: number; partiallyCompliant: number; overdue: number; complianceRate: number; avgRemediationDays: number; frameworksCovered: number };
  frameworkBreakdown: Array<{ framework: string; obligationCount: number; complianceRate: number }>;
  severityBreakdown: Array<{ severity: string; count: number; overdueCount: number }>;
}
...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('compliance.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalObligations }}</div><div class="kpi-label">Total Obligations</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.partiallyCompliant }}</div><div class="kpi-label">Partial</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.overdue }}</div><div class="kpi-label">Overdue</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.complianceRate }}%</div><div class="kpi-label">Compliance Rate</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgRemediationDays }}d</div><div class="kpi-label">Avg Remediation</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.frameworksCovered }}</div><div class="kpi-label">Frameworks</div></div>
        </div>
        ...
        <div class="grid-2">
            ...
            <div class="breakdown-card">
              ...
              @for (item of data()!.frameworkBreakdown; track item.framework) {
                <div class="breakdown-row">
                  ...
                  <span>{{ item.obligationCount }} ({{ item.complianceRate }}%)</span>
                </div>
              }
            </div>
          </div>
          <div class="breakdown-section">
            <div class="breakdown-card">
              ...
              @for (item of data()!.severityBreakdown; track item.severity) {
                <div class="breakdown-row">
...
export class ComplianceDashboardComponent implements OnInit {
  ...
  loading = signal(true);
  data = signal<ComplianceDashboardData | null>(null);

  ngOnInit(): void {
    this.http.get<{ data: ComplianceDashboardData }>('/api/compliance/dashboard')
