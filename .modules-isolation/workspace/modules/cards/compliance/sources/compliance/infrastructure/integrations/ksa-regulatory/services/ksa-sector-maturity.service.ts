...
Respond with JSON: { "narrative": "...", "roadmap": [{ "phase": 1, "title": "...", "description": "...", "dimension": "governance|risk|compliance|technology|people", "impact": "high|medium|low", "effort": "high|medium|low", "estimatedWeeks": N, "dependencies": [] }] }`,
    ...
    narrative = typeof aiResult.narrative === 'string' ? aiResult.narrative : '';
    const roadmapItems = Array.isArray(aiResult.roadmap) ? aiResult.roadmap : [];
    ...
    if (!narrative && roadmap.length === 0) throw new Error('AI response did not contain usable content');
  } catch (err) {
