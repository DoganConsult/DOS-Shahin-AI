...
// Shahin FE (controls-api.service.ts:162) calls `GET /api/lifecycle/summary`
// for the lifecycle dashboard card. Aggregate counts by status + staleness.
router.get('/summary', authenticate, requirePermission('compliance.control.read'), validate({ query: genericPayloadSchema }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId!);
  const stale = await checkStaleness(req.tenantId!, 90).catch(() => [] as any[]);
  const byStatus = await safeQuery(
    `SELECT status, COUNT(*)::int AS count FROM "${schema}".controls WHERE deleted_at IS NULL GROUP BY status`,
  ).catch(() => ({ rows: [] as Array<{ status: string; count: number }> }));
  const total = (byStatus.rows as Array<{ count: number }>).reduce((s, r) => s + Number(r.count || 0), 0);
  res.json({
    success: true,
    data: {
      total,
      byStatus: Object.fromEntries((byStatus.rows as Array<{ status: string; count: number }>).map((r) => [r.status, Number(r.count)])),
...
async function transitionControlHandler(req: Request, res: Response): Promise<void> {
...
router.post('/:controlId/transition', authenticate, requirePermission('compliance.control.approve'), validate({ body: createTransitionBody }), asyncHandler(transitionControlHandler));

// Shahin FE uses `/controls/:controlId/*` subpaths (controls-api.service.ts
// lines 166-176). Alias them onto the bare `/:controlId/*` handlers
// already defined above so the FE contract matches without changing
// frontend code.
router.get('/controls/:controlId/staleness', authenticate, requirePermission('compliance.control.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const results = await checkStaleness(req.tenantId!, req.params.controlId);
  res.json({ success: true, data: results[0] || null });
}));

router.get('/controls/:controlId/history', authenticate, requirePermission('compliance.control.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId!);
  const result = await safeQuery(
    `SELECT lh.id, lh.from_state, lh.to_state, lh.changed_by, lh.changed_at, lh.reason
     FROM "${schema}".lifecycle_history lh
...
router.post('/controls/:controlId/transition', authenticate, requirePermission('compliance.control.approve'), validate({ body: createTransitionBody }), asyncHandler(transitionControlHandler));
