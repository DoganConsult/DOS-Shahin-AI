...
exports.resolveRegulatoryProfile = resolveRegulatoryProfile;
exports.resolveRegulatoryProfileBySectorId = resolveRegulatoryProfileBySectorId;
exports.resolveMultiSectorProfile = resolveMultiSectorProfile;
exports.resolveRegulatoryProfileForTenant = resolveRegulatoryProfileForTenant;
exports.searchControls = searchControls;
const database_port_1 = require("../../ports/database.port");
const db_1 = require("@dos/db");
// ─── Main Resolution Function ──────────────────────────────────────────────
async function resolveRegulatoryProfile(sectorCode, tenantId) {
    // 0. Sector name lookup
    const sectorNameResult = await (0, database_port_1.safeQuery)(`SELECT sector_name_en, COALESCE(sector_name_ar, sector_name_en) AS sector_name_ar
     FROM public.lookup_isic4_sectors WHERE section_code = $1 LIMIT 1`, [sectorCode]);
    const sectorNameEn = (0, db_1.getFirstRow)(sectorNameResult)?.sector_name_en || sectorCode;
    const sectorNameAr = (0, db_1.getFirstRow)(sectorNameResult)?.sector_name_ar || sectorCode;
    // 1. Authorities for this sector
    const authResult = await (0, database_port_1.safeQuery)(`SELECT
     ...
     WHERE m.sector_code = $1
       AND m.is_active
       AND a.is_active
     ORDER BY m.priority, m.authority_code`, [sectorCode]);
    const authorities = authResult.rows;
    ...
    const fwResult = await (0, database_port_1.safeQuery)(`SELECT DISTINCT
     ...
     WHERE f.authority_code = ANY($1)
       AND f.is_active
     ORDER BY f.framework_code`, [authorityCodes]);
    const frameworks = fwResult.rows;
    ...
    const controlResult = await (0, database_port_1.safeQuery)(`SELECT count(*)::int AS cnt
     FROM public.regulatory_controls rc
     JOIN public.control_domains cd ON cd.id = rc.domain_id
     WHERE cd.framework_code = ANY($1)`, [frameworkCodes]);
    const controlCount = (0, db_1.getFirstRow)(controlResult)?.cnt ?? 0;
    ...
    const schema = tenantId ? (0, database_port_1.tenantSchema)(tenantId) : null;
    ...
    const evidenceResult = await (0, database_port_1.safeQuery)(`SELECT count(*)::int AS cnt
     FROM ${evidenceTable} cer
     JOIN public.regulatory_controls rc ON ${joinCondition}
     JOIN public.control_domains cd ON cd.id = rc.domain_id
     WHERE cd.framework_code = ANY($1)`, [frameworkCodes]);
    const evidenceTaskCount = (0, db_1.getFirstRow)(evidenceResult)?.cnt ?? 0;
    // 5. Modules from framework triggers
    const moduleResult = await (0, database_port_1.safeQuery)(`SELECT DISTINCT module_code
     FROM public.lookup_framework_module_triggers
     WHERE framework_code = ANY($1)
       AND is_active
     ORDER BY module_code`, [frameworkCodes]);
    const modules = moduleResult.rows.map((r) => r.module_code);
    // 6. Sector risks
    const riskResult = await (0, database_port_1.safeQuery)(`SELECT
       r.risk_title_en,
     ...
     ORDER BY sr.priority_rank NULLS LAST, r.risk_title_en`, [sectorCode]);
 ...
 */
async function resolveRegulatoryProfileBySectorId(sectorId) {
    const isicResult = await (0, database_port_1.safeQuery)(`SELECT isic_code FROM public.sector_isic_map
     WHERE sector_id = $1 ORDER BY weight DESC LIMIT 1`, [sectorId]);
    if (isicResult.rows.length === 0) {
        // Sector not mapped to ISIC — return sector name but empty resolution
        const sectorRow = await (0, database_port_1.safeQuery)(`SELECT name_en, COALESCE(name_ar, name_en) AS name_ar FROM public.sectors WHERE sector_id = $1`, [sectorId]);
        return {
            sectorCode: sectorId,
            sectorNameEn: (0, db_1.getFirstRow)(sectorRow)?.name_en || sectorId,
            sectorNameAr: (0, db_1.getFirstRow)(sectorRow)?.name_ar || sectorId,
            authorities: [], frameworks: [], controlCount: 0,
            evidenceTaskCount: 0, modules: [], risks: [],
        };
    }
    return resolveRegulatoryProfile((0, db_1.getFirstRow)(isicResult)?.isic_code);
}
 ...
 * while deduplicating.
 */
async function resolveMultiSectorProfile(sectorCodes, tenantId) {
    ...
    const resolutions = await Promise.all(sectorCodes.map((code) => resolveRegulatoryProfile(code)));
    ...
    return {
        ...
        controlCount: (0, db_1.getFirstRow)(controlResult)?.cnt ?? 0,
        evidenceTaskCount: (0, db_1.getFirstRow)(evidenceResult)?.cnt ?? 0,
 ...
 */
async function resolveRegulatoryProfileForTenant(tenantId) {
    const sectorRes = await (0, database_port_1.safeQuery)(`SELECT sector_code FROM public.tenant_sectors
     WHERE tenant_id = $1 ORDER BY is_primary DESC, added_at ASC`, [tenantId]);
    if (sectorRes.rows.length === 0) {
        // Fallback: try to get sector from workspace profile
        const wpRes = await (0, database_port_1.safeQuery)(`SELECT sector_code FROM public.tenants WHERE tenant_id = $1`, [tenantId]);
        const sectorCode = (0, db_1.getFirstRow)(wpRes)?.sector_code;
        if (!sectorCode) {
            return {
                sectorCode: "",
    ...
    return resolveMultiSectorProfile(sectorRes.rows.map((r) => r.sector_code), tenantId);
}
/**
 * Full-text search across all regulatory controls.
 * Uses PostgreSQL tsvector for fast, ranked search results.
 */
async function searchControls(searchQuery, limit = 50, frameworkFilter) {
    ...
    const res = await (0, database_port_1.safeQuery)(`SELECT
...
//# sourceMappingURL=regulatory-resolution.service.js.map