...
Respond with JSON: {
  ...
  "remediation_steps": [{"step": 1, "action_en": "...", "action_ar": "...", "control_id": null, "estimated_days": 5, "priority": "high"}]
      ...
      aiReportEn = aiResult.report_en || '';
      aiReportAr = aiResult.report_ar || '';
      remediationPlan = (aiResult.remediation_steps || []).map((s: any) => ({
        step: s.step,
        actionEn: s.action_en,
        actionAr: s.action_ar,
        controlId: s.control_id,
        owner: null,
        estimatedDays: s.estimated_days || 5,
...
function buildImpactSummary(changes: DetectedChange[]): ImpactSummary {
  ...
  const totalAffectedControls = changes.reduce((sum: number, c: any) => sum + c.affectedControlsCount, 0);
  const totalGapDelta = changes.reduce((sum: number, c: any) => sum + c.complianceGapDelta, 0);
...
Respond with JSON: {
  ...
  return {
    summaryEn: aiResult.summary_en || impact.aiSummaryEn,
    summaryAr: aiResult.summary_ar || impact.aiSummaryAr,
    actions: (aiResult.actions || []).map((a: any) => ({
      priority: a.priority,
      ...
      relatedChangeId: a.related_change_id || changes[0]?.changeId || '',
      estimatedEffortDays: a.estimated_effort_days || 5,
