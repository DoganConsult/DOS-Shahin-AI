...
import {
  ...
  getOverdueTests,
} from './controls-lifecycle.service';
...
describe('Controls Lifecycle Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('domain queries', () => {
    it('getIneffectiveControls should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getIneffectiveControls('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getIneffectiveControls should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getIneffectiveControls('t1');
      expect(result).toEqual([]);
    });

    it('getOverdueTests should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverdueTests('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverdueTests should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverdueTests('t1');
      expect(result).toEqual([]);
    });
