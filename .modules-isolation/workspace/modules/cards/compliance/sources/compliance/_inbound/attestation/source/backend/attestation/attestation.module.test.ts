...
describe('attestation module manifest', () => {
  it('exports a valid manifest', async () => {
    const mod = await import('./attestation.module');
    const manifest = mod.ATTESTATION_MANIFEST ?? mod.default;
    expect(manifest).toBeTruthy();
    expect(manifest.code).toBe('attestation');
