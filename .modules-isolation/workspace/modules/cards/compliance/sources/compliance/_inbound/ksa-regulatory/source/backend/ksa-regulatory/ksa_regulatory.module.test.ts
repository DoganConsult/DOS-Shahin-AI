...
beforeAll(async () => {
  const mod = await import('./ksa-regulatory.module');
  manifest = mod.KSA_REGULATORY_MANIFEST ?? mod.default?.manifest;
});

describe('Ksa Regulatory Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('ksa-regulatory'); });
