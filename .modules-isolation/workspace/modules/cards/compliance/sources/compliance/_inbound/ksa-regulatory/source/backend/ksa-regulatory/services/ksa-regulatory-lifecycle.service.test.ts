...
import {
  ...
  getOpenGaps,
  getDraftObligations,
} from './ksa-regulatory-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getOpenGaps should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOpenGaps('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOpenGaps should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOpenGaps('t1');
      expect(result).toEqual([]);
    });

    it('getDraftObligations should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getDraftObligations('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getDraftObligations should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getDraftObligations('t1');
      expect(result).toEqual([]);
    });
