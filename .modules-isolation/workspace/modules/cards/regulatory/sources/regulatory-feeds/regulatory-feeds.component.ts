...
@Component({
  ...
  imports: [
    ...
    MultiSelectModule, SkeletonModule, InputTextModule,
    ...
    <section class="page-shell">
      ...
      <ng-container *ngIf="selectedUpdate">
        <div class="detail-panel">
          <button aria-label="Back" pButton class="p-button-text p-button-sm" icon="pi pi-arrow-left"
            [label]="i18n.translate('Back to feed')" (click)="closeDetail()"></button>

          <div class="detail-header">
            <h3>{{ selectedUpdate.instrumentName }}</h3>
            <p-tag [value]="selectedUpdate.impactLevel || 'low'"
      ...
      <ng-container *ngIf="!selectedUpdate">
        <div class="toolbar">
          <p-multiSelect [options]="regulatorOptions" [(ngModel)]="selectedRegulators"
        ...
        <p-skeleton *ngIf="loading" width="100%" height="300px" />

        <div *ngIf="!loading && errorMsg" class="error-state">
          <i class="pi pi-exclamation-triangle"></i>
          <p>{{ errorMsg }}</p>
          <button pButton class="p-button-outlined p-button-sm" [label]="i18n.translate('Retry')" (click)="loadData()"></button>
        </div>
        ...
        <p-table [paginator]="true" [rows]="20" [rowsPerPageOptions]="[10,20,50]" aria-label="Data table" *ngIf="!loading && !errorMsg && filtered.length > 0"
          ...
          <ng-template pTemplate="header">
            <tr>
              <th pSortableColumn="instrumentName">{{ i18n.translate('Title') }} <p-sortIcon field="instrumentName" /></th>
              <th>{{ i18n.translate('Regulator') }}</th>
              <th pSortableColumn="detectedAt">{{ i18n.translate('Publication Date') }} <p-sortIcon field="detectedAt" /></th>
              <th pSortableColumn="impactLevel">{{ i18n.translate('Impact Severity') }} <p-sortIcon field="impactLevel" /></th>
          ...
          <ng-template pTemplate="body" let-u>
            <tr [pSelectableRow]="u" class="feed-row">
              ...
              <td><p-tag [value]="u.impactLevel || 'low'" [severity]="severityColor(u.impactLevel)" size="small" /></td>
              <td>
...
export class RegulatoryFeedsComponent implements OnInit {
  ...
  loadData(): void {
    ...
    forkJoin({
      ...
      next: (res) => {
        const deltas: Record<string, unknown>[] = res.deltas || [];
        const impacts: Record<string, unknown>[] = res.impacts || [];
        const instruments: Record<string, unknown>[] = (res.instruments as GrcRecord)?.instruments || res.instruments || [];
        ...
        this.allUpdates = deltas.map((d: Record<string, unknown>) => {
  ...
  private inferSeverity(delta: Record<string, unknown>): 'low' | 'medium' | 'high' | 'critical' {
