...
export function toEntity(row: GenericRow): Asset {
...
export function toEntityList(rows: GenericRow[]): Asset[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): AssetCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Asset): Record<string, unknown> {

  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Asset[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
