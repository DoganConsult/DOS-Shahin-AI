...
export class AssetRepository {
  ...
  async findAll(filters: ListAssetsFilter = {}): Promise<{ rows: GenericRow[]; total: number }> {
    ...
    if (filters.type) { conditions.push(`a.type = $${idx++}`); params.push(filters.type); }
    if (filters.category) { conditions.push(`a.asset_category = $${idx++}`); params.push(filters.category); }
    if (filters.criticality) { conditions.push(`a.criticality = $${idx++}`); params.push(filters.criticality); }
    if (filters.classification) { conditions.push(`a.classification = $${idx++}`); params.push(filters.classification); }
    if (filters.status) { conditions.push(`a.status = $${idx++}`); params.push(filters.status); }
    if (filters.lifecycleStage) { conditions.push(`a.lifecycle_stage = $${idx++}`); params.push(filters.lifecycleStage); }
    if (filters.ownerId) { conditions.push(`a.owner = $${idx++}`); params.push(filters.ownerId); }
    if (filters.serviceId) { conditions.push(`a.business_service_id = $${idx++}`); params.push(filters.serviceId); }
    ...
    const countResult = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${this.schema}".assets a WHERE ${where}`, params,
    );
  ...
  async create(userId: string, data: CreateAssetInput): Promise<GenericRow | null> {
    const result = await safeQuery(`
  ...
  async update(assetId: string, updates: Partial<CreateAssetInput>): Promise<GenericRow | null> {
    const allowed = [
      'name', 'name_en', 'name_ar', 'type', 'asset_category', 'description', 'criticality',
      'owner', 'custodian_id', 'department', 'location', 'ip_address', 'mac_address', 'os',
      'classification', 'status', 'lifecycle_stage', 'business_service_id',
    ...
    const result = await safeQuery(
      `UPDATE "${this.schema}".assets SET ${sets.join(', ')}, updated_at = NOW()
       WHERE asset_id = $1 AND deleted_at IS NULL RETURNING *`,
      [assetId, ...vals],
    );
    return getFirstRow(result);
  }
  ...
  async softDelete(assetId: string): Promise<boolean> {
    const result = await safeQuery(
      `UPDATE "${this.schema}".assets SET deleted_at = NOW()
       WHERE asset_id = $1 AND deleted_at IS NULL RETURNING asset_id`,
      [assetId],
    );
    return (result.rows?.length ?? 0) > 0;
  }

  async bulkSoftDelete(assetIds: string[]): Promise<number> {
    const result = await safeQuery(
      `UPDATE "${this.schema}".assets SET deleted_at = NOW()
  ...
  async getStats(): Promise<GenericRow | null> {
    const result = await safeQuery(`
      SELECT
        COUNT(*)::int AS total,
  ...
  async count(filters: { status?: string; criticality?: string } = {}): Promise<number> {
    ...
    const result = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${this.schema}".assets WHERE ${conditions.join(' AND ')}`,
      params,
    );
...
export class AssetDependencyRepository {
  ...
  async findAll(filters: {
    ...
    if (filters.target_id) { conditions.push(`target_id = $${idx++}`); params.push(filters.target_id); }
    if (filters.dependency_type) { conditions.push(`dependency_type = $${idx++}`); params.push(filters.dependency_type); }
    ...
    const countR = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${this.schema}".asset_dependencies WHERE ${where}`, params,
    );
    const total = getFirstRow(countR)?.total ?? 0;
  ...
  async create(userId: string, data: {
    ...
    dependency_type?: string; criticality?: string; direction?: string;
  ...
  async softDelete(dependencyId: string): Promise<boolean> {
    const result = await safeQuery(
      `UPDATE "${this.schema}".asset_dependencies SET deleted_at = NOW()
       WHERE dependency_id = $1 AND deleted_at IS NULL RETURNING dependency_id`,
      [dependencyId],
    );
    return (result.rows?.length ?? 0) > 0;
  }

  async getUpstreamChain(entityType: string, entityId: string, maxDepth: number = 10): Promise<GenericRow[]> {
    const result = await safeQuery(`
  ...
  async getDownstreamChain(entityType: string, entityId: string, maxDepth: number = 10): Promise<GenericRow[]> {
    const result = await safeQuery(`
      WITH RECURSIVE chain AS (
        SELECT dependency_id, source_type, source_id, target_type, target_id,
               dependency_type, criticality, 1 AS depth
