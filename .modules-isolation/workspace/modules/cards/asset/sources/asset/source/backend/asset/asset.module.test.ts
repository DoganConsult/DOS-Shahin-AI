...
beforeAll(async () => {
  const mod = await import('./asset.module');
  manifest = mod.ASSET_MANIFEST ?? mod.default?.manifest;
});

describe('Asset Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('asset'); });
