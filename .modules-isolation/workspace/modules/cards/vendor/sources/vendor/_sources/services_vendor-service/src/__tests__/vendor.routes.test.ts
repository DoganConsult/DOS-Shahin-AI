...
vi.mock('../events/publisher', () => ({
  ...
  publishVendorCreated: vi.fn().mockResolvedValue(undefined),
  publishVendorAssessmentCompleted: vi.fn().mockResolvedValue(undefined),
  publishVendorRiskElevated: vi.fn().mockResolvedValue(undefined),
  publishVendorContractExpiring: vi.fn().mockResolvedValue(undefined),
}));
...
import * as domainSvc from '../domain/vendor.service';
...
describe('vendor routes', () => {
  ...
  describe('GET /api/vendor/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/vendor/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/vendor/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/vendor/', () => {
  ...
  describe('DELETE /api/vendor/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/vendor/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
