import type { Vendor, VendorCreateInput } from '@dos/types/vendor';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Vendor {
...
export function toEntityList(rows: GenericRow[]): Vendor[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): VendorCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Vendor): Record<string, unknown> {

  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Vendor[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
