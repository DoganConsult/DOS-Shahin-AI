export const VENDOR_AI_CONFIG = {
  moduleCode: 'vendor' as const,
  enabled: true,

  allowedActions: [
    'vendor.record.draft',
    'vendor.record.recommend',
    'vendor.record.summarize',
    'vendor.record.classify',
    'vendor.record.score',
    'vendor.record.analyze',
    'vendor.record.generate_report',
  ],

  blockedActions: [
    'vendor.record.delete',
    'vendor.record.approve',
    'vendor.record.reject',
    'vendor.record.override',
    'vendor.record.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['vendor.record.approve', 'vendor.record.reject', 'vendor.record.delete'],
    requiresHumanReview: ['vendor.record.classify', 'vendor.record.score'],
    autoExecutable: ['vendor.record.draft', 'vendor.record.summarize', 'vendor.record.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'vendor_system_prompt_v1',
  },
...
export function isVendorAiActionAllowed(action: string): boolean {
  return (VENDOR_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isVendorAiActionBlocked(action: string): boolean {
  return (VENDOR_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresVendorHumanApproval(action: string): boolean {
  return (VENDOR_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  if (!r) return { summary: 'Vendor entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? r.name ?? "Vendor"} record. Status: \${r.status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".vendors ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".vendors ${where} GROUP BY status`, params);
