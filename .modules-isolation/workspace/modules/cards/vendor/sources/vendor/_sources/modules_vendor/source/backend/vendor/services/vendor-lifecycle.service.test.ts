...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getHighRiskVendors should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getHighRiskVendors('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getHighRiskVendors should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getHighRiskVendors('t1');
      expect(result).toEqual([]);
    });

    it('getExpiredContracts should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getExpiredContracts('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getExpiredContracts should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getExpiredContracts('t1');
      expect(result).toEqual([]);
    });
