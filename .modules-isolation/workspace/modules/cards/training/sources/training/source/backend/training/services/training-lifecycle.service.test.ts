...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getOverdueEnrollments should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverdueEnrollments('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverdueEnrollments should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverdueEnrollments('t1');
      expect(result).toEqual([]);
    });

    it('getDraftPrograms should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getDraftPrograms('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getDraftPrograms should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getDraftPrograms('t1');
      expect(result).toEqual([]);
    });
