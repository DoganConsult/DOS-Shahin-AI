import type { TrainingCampaign, TrainingCampaignCreateInput } from '@dos/types/training';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): TrainingCampaign {
  return {
...
export function toEntityList(rows: GenericRow[]): TrainingCampaign[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): TrainingCampaignCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as TrainingCampaignCreateInput;
}

export function toApiResponse(entity: TrainingCampaign): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: TrainingCampaign[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
