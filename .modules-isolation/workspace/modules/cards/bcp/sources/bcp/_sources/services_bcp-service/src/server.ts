...
import bcpPlanRouter from './routes/bcp-plan.routes';
...
const SERVICE_CODE = 'bcp-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    port: config.port,
    routes: [
      { path: '/api/bcp', router: bcpPlanRouter },
      { path: '/api/bcp-service', router: routes },
    ],
    modules: await loadModuleRegistrations('bcp'),
    healthChecks: {
      database: async () => {
        try {
    ...
    onReady: () => {
      invokeModuleLifecycleHooks(SERVICE_CODE, 'bcp').catch(() => {});
    },
  });
