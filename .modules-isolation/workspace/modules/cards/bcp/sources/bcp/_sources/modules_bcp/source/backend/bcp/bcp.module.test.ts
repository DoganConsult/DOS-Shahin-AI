...
beforeAll(async () => {
  const mod = await import('./bcp.module');
  manifest = mod.BCP_MANIFEST ?? mod.default?.manifest;
});

describe('Bcp Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('bcp'); });
