// Shahin GRC — Connector Hub  |  GRC Lifecycle: OPERATE
import { Component, OnInit, signal, inject, ChangeDetectionStrategy, DestroyRef } from '@angular/core';
import { CommonModule } from '@angular/common';
import { RouterModule, ActivatedRoute } from '@angular/router';
import { I18nService } from '@app/core/services/ui-infra/i18n.service';
import { ConnectorManagerComponent } from '../../../../../../../modules/connector-manager/connector-manager.component';
import { ConnectorHealthComponent } from '../../../../../../../modules/connector-health/connector-health.component';
import { IntegrationMarketplaceComponent } from '../../config-center/integration-marketplace/integration-marketplace.component';
...
const TABS: HubTab[] = [
  { key: 'manager',     labelEn: 'Connector Mgr',    labelAr: 'إدارة الموصلات', icon: 'pi-link' },
  { key: 'health',      labelEn: 'Connector Health', labelAr: 'صحة الموصلات',   icon: 'pi-heart' },
  { key: 'marketplace', labelEn: 'Marketplace',      labelAr: 'سوق التكاملات',  icon: 'pi-shopping-bag' },
];
...
@Component({
  ...
  selector: 'app-connector-hub', standalone: true,
  imports: [CommonModule, RouterModule, ConnectorManagerComponent, ConnectorHealthComponent, IntegrationMarketplaceComponent, AgentBadgeComponent, HubConnectionsStripComponent],
  styles: [HUB_STYLES],
  template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#f3e8ff"><i class="pi pi-link hub-icon" style="color:#7e22ce"></i></div>
          <div>
            ...
            <h1>{{ i18n.currentLang()==='ar' ? 'مركز الاتصالات' : 'Connector Hub' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'التكاملات والموصلات والصحة والسوق — كل شيء متصل.' : 'Integrations, connectors, health and marketplace — connected.' }}</p>
          </div>
        </div>
      ...
      <app-hub-connections-strip [hubKey]="'connector'" />
      <div class="hub-tab-content">
        @if (activeTab()==='manager')     { <app-connector-manager /> }
        @if (activeTab()==='health')      { <app-connector-health /> }
        @if (activeTab()==='marketplace') { <app-integration-marketplace /> }
      </div>
    </div>`
})
export class ConnectorHubComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  i18n = inject(I18nService);
  private route = inject(ActivatedRoute);
  tabs = TABS;
  readonly agentId = 'A01';
  activeTab = signal<string>('manager');
  ngOnInit(): void {
