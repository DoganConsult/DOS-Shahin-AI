...
import type { AgentDiscovery, AgentHandoff, SharedFinding } from './types';
import { getCycleContext } from './cycle-context';
...
export const COOPERATION_CHAINS: Record<string, string[]> = {
 ...
 */
export async function shareFindings(
  tenantId: string,
  fromAgentId: string,
  toAgentId: string,
  findings: Record<string, unknown>[],
): Promise<SharedFinding[]> {
  if (!tenantId || !fromAgentId || !toAgentId) {
    throw new Error('tenantId, fromAgentId, and toAgentId are required');
  }
  ...
  for (const finding of findings) {
    const id = `sf-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
    const sharedAt = new Date().toISOString();

    const entry: SharedFinding = {
      id,
      fromAgentId,
      toAgentId,
      tenantId,
    ...
    // Persist to agrc_event_log for audit trail
    await safeQuery(
      ...
      [
        ...
        'info',
        JSON.stringify({
          cooperationType: 'share_findings',
          findingId: id,
          fromAgentId,
          toAgentId,
          finding,
  ...
  // Also register in the cycle context if active
  const ctx = getCycleContext(tenantId);
  if (ctx) {
    for (const s of shared) {
      createHandoff(tenantId, fromAgentId, toAgentId, 'finding', 'medium', {
        entityType: 'shared_finding',
        entityId: s.id,
        finding: JSON.stringify(s.finding),
        context: { cooperationType: 'share_findings' },
      });
    }
  }
  ...
  swallow(EC.EVENT_BUS, eventBus.publish({
    ...
    tenantId,
    sourceService: `agent-${fromAgentId}`,
    severity: 'info',
    payload: {
      cooperationType: 'share_findings',
      fromAgentId,
      toAgentId,
      findingCount: shared.length,
    },
  }), { tenantId, operation: 'eventBus:agent.handoff_created' });
...
export async function requestEvidence(
  ...
  if (!tenantId || !fromAgentId || !controlId) {
    throw new Error('tenantId, fromAgentId, and controlId are required');
  }

  const toAgentId = 'A03'; // Evidence Collector is always the target for evidence requests
  const schema = tenantSchema(tenantId);
  ...
  const handoff = createHandoff(
    ...
    'high',
    {
      entityType: 'control',
      entityId: controlId,
      requestedAction: `Collect and validate evidence for control ${controlId}`,
      context: { requestType: 'evidence_request', sourceAgent: fromAgentId },
    },
  );

  // Log to agrc_event_log
  await safeQuery(
    `INSERT INTO "${schema}".agrc_event_log
       (event_type, source_service, tenant_id, severity, payload, created_at)
     VALUES ($1, $2, $3, $4, $5, NOW())`,
    [
      ...
      'info',
      JSON.stringify({
        cooperationType: 'request_evidence',
 ...
 */
export async function getSharedFindings(
  tenantId: string,
  agentId: string,
): Promise<SharedFinding[]> {
  if (!tenantId || !agentId) {
 ...
 */
export async function getCooperationLog(
  tenantId: string,
  limit = 100,
): Promise<Array<{
  id: string;
 ...
 */
export function aggregateFindings(discoveries: AgentDiscovery[]): AgentDiscovery[] {
  ...
  const aggregated: AgentDiscovery[] = [];
  for (const [_key, group] of byEntity) {
    ...
    const reverseSeverity: Record<number, string> = { 1: 'low', 2: 'medium', 3: 'high', 4: 'critical' };
    ...
    const merged: AgentDiscovery = {
      ...group[0],
      severity: (reverseSeverity[boostedSeverity] || 'medium') as AgentDiscovery['severity'],
 ...
 */
export async function persistSharedFindings(tenantId: string, findings: AgentDiscovery[]): Promise<number> {
  ...
  for (const f of findings) {
    try {
      await safeQuery(
        ...
        [f.entityType || 'any', f.entityId || f.id, JSON.stringify(f)]
      );
      persisted++;
    } catch { /* non-critical */ }
  }
