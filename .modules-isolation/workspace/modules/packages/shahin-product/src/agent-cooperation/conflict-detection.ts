...
import type { AgentDiscovery, AgentConflictCandidate } from './types';
 ...
 */
export function detectConflicts(tenantId: string): AgentConflictCandidate[] {
  const ctx = activeCycleContexts.get(tenantId);
  if (!ctx || ctx.discoveries.length < 2) return [];

  const entityGroups = new Map<string, AgentDiscovery[]>();
  ...
  const conflicts: AgentConflictCandidate[] = [];
 ...
 */
export async function recordAgentConflicts(
  tenantId: string,
  cycleId: string | undefined,
  candidates: AgentConflictCandidate[],
): Promise<void> {
  if (candidates.length === 0) return;
  const schema = tenantSchema(tenantId);
  const { queuePendingAction } = await import('../../../modules/platform/services/platform-mode-gate/pending-actions.service');
  ...
  for (const c of candidates) {
    try {
      ...
      const r = await safeQuery(
        ...
        [tenantId, cycleId || null, c.entityType, c.entityId || null, JSON.stringify(proposalsJson), c.conflictType],
      );
 ...
 */
export async function hasOpenConflict(tenantId: string, entityType: string, entityId: string | undefined): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  const r = await safeQuery(
    `SELECT 1 FROM "${schema}".agent_conflicts
     WHERE tenant_id = $1 AND entity_type = $2 AND (entity_id IS NOT DISTINCT FROM $3) AND status = 'open'
     LIMIT 1`,
    [tenantId, entityType, entityId ?? null],
  );
  return (r.rows?.length ?? 0) > 0;
}

// ── Enhanced Conflict Detection (Pillar 2: Agent Cooperation) ────────────────

export interface ConflictRecord {
  conflictId: string;
  discoveryA: AgentDiscovery;
  discoveryB: AgentDiscovery;
  conflictType: 'contradictory_action' | 'duplicate_finding' | 'scope_overlap' | 'priority_clash';
  resolution: 'pending' | 'auto_resolved' | 'escalated' | 'manual';
  resolvedBy?: string;
  resolvedAt?: string;
}
 ...
 */
export function detectDiscoveryConflicts(discoveries: AgentDiscovery[]): ConflictRecord[] {
  ...
  for (const [_key, group] of byEntity) {
    ...
    for (let i = 0; i < group.length; i++) {
      for (let j = i + 1; j < group.length; j++) {
        ...
        if (a.agentId === b.agentId) continue; // same agent, no conflict
 ...
 */
function classifyConflict(a: AgentDiscovery, b: AgentDiscovery): ConflictRecord['conflictType'] | null {
 ...
 */
export function resolveConflicts(conflicts: ConflictRecord[]): ConflictRecord[] {
  return conflicts.map(c => {
    if (c.conflictType === 'duplicate_finding') {
      ...
      return { ...c, resolution: 'auto_resolved' as const, resolvedBy: keepA ? c.discoveryA.agentId : c.discoveryB.agentId };
    }
    if (c.conflictType === 'contradictory_action') {
      return { ...c, resolution: 'escalated' as const };
    }
    return { ...c, resolution: 'auto_resolved' as const };
  });
}
 ...
 */
export async function persistConflictRecords(tenantId: string, conflicts: ConflictRecord[]): Promise<void> {
  ...
  for (const c of conflicts) {
    swallow(EC.AGENT_ACTION, safeQuery(
      ...
      [c.conflictId, JSON.stringify(c)]
    ), { tenantId, operation: 'persistConflictRecord' });
  }
}
