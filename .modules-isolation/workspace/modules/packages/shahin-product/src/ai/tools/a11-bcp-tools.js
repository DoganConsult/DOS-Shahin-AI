...
export function buildA11Tools() {
    return [
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = tenantSchema(tenantId);
                const exercises = await safeRows(`SELECT exercise_id, title, exercise_type, status, next_exercise_date, last_exercise_date
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = tenantSchema(tenantId);
                const drifts = await safeRows(`SELECT p.plan_id, p.title, p.rto_hours AS target_rto, p.rpo_hours AS target_rpo,
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = tenantSchema(tenantId);
                const rows = await safeRows(`INSERT INTO "${schema}".risks (title, description, risk_category, likelihood, impact, status, created_at)
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = tenantSchema(tenantId);
                const rows = await safeRows(`INSERT INTO "${schema}".tasks (title, description, category, priority, status, created_at)
