...
export function buildA10Tools(): AgentToolDefinition[] {
  return [
    {
      ...
      handler: async (tenantId) => {
        ...
        const findings = await safeRows(
          `SELECT finding_id, title, severity, status, source_type, source_id, created_at
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        ...
        const rows = await safeRows(
          `INSERT INTO "${schema}".findings (title, description, severity, status, source_type, created_at)
           VALUES ($1, $2, $3, 'open', $4, NOW()) RETURNING finding_id`,
          [`[A10] ${input.title}`, input.description, input.severity, input.sourceType || 'agent_A10']
        );
