    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const programs = await safeRows(`SELECT program_id, title, category, status, created_at
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = tenantSchema(tenantId);
                const rows = await safeRows(`INSERT INTO "${schema}".training_assignments (program_id, user_id, status, due_date, assigned_at)
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = tenantSchema(tenantId);
                const stats = await safeRows(`SELECT COUNT(*)::int AS total,
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = tenantSchema(tenantId);
                const rows = await safeRows(`INSERT INTO "${schema}".training_programs (title, description, category, status, created_at)
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = tenantSchema(tenantId);
                const gaps = await safeRows(`SELECT u.user_id, u.email, u.full_name,
