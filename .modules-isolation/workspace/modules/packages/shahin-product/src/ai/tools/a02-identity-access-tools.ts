...
export function buildA02Tools(): AgentToolDefinition[] {
  return [
    {
      ...
      handler: async (tenantId) => {
        ...
        const users = await safeRows(
          `SELECT user_id, email, display_name AS name, role, platform_role,
                  status, created_at, last_login,
                  (status = 'active') AS is_active
             FROM dos.users
            WHERE tenant_id = $1
            ORDER BY role NULLS LAST, display_name`,
          [tenantId],
        );
        ...
        const admins = users.filter(u =>
          (u.platform_role && /admin/i.test(String(u.platform_role))) ||
    ...
    },
    {
      ...
      handler: async (tenantId) => {
        const roles = await safeRows(
          `SELECT role, COUNT(*)::int AS cnt, COUNT(*) FILTER (WHERE is_active)::int AS active
           FROM users WHERE tenant_id = $1 GROUP BY role ORDER BY cnt DESC`, [tenantId]
        );
        const total = roles.reduce((s: number, r: Record<string, unknown>) => s + r.cnt, 0);
        const adminRatio = roles.find((r: GenericRow) => ['admin'].includes(r.role))?.cnt || 0;
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        const { safeQuery, tenantSchema } = await import('../../../../config/database'); const schema = tenantSchema(tenantId); const listPositions = async (_t: string, _f: any) => (await safeQuery(`SELECT * FROM "${schema}".positions WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT 100`)).rows; const createPosition = async (_t: string, p: any) => (await safeQuery(`INSERT INTO "${schema}".positions (title_en, dept_id, grade, created_by) VALUES ($1,$2,$3,$4) RETURNING *`, [p.title_en, p.dept_id, p.grade, p.created_by])).rows[0]; const updatePosition = async (_t: string, id: string, p: any) => (await safeQuery(`UPDATE "${schema}".positions SET title_en=COALESCE($2,title_en), grade=COALESCE($3,grade) WHERE position_id=$1 RETURNING *`, [id, p.title_en, p.grade])).rows[0];
        if (input.action === 'list') return { positions: await listPositions(tenantId, {}) };
        if (input.action === 'create') return await createPosition(tenantId, { title_en: input.title_en, dept_id: input.dept_id, grade: input.grade, created_by: 'agent-A02' });
        if (input.action === 'update' && input.position_id) return await updatePosition(tenantId, input.position_id, { title_en: input.title_en, grade: input.grade, updated_by: 'agent-A02' });
    ...
    },
    {
      ...
      input_schema: { type: 'object', properties: {}, required: [] },
      handler: async (tenantId) => {
        const { safeQuery, tenantSchema } = await import('../../../../config/database'); const schema = tenantSchema(tenantId); const getReportingTree = async (_t: string) => (await safeQuery(`SELECT p.position_id, p.title_en, p.reports_to_position_id FROM "${schema}".positions p WHERE p.deleted_at IS NULL ORDER BY p.title_en`)).rows;
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        ...
        if (input.action === 'list') return { delegations: await listDelegations(tenantId) };
        if (input.action === 'create') return await createDelegation(tenantId, { delegator_user_id: input.delegator_user_id!, delegate_user_id: input.delegate_user_id!, authority_type: input.authority_type!, valid_from: input.valid_from!, valid_to: input.valid_to!, created_by: 'agent-A02' });
        if (input.action === 'revoke' && input.delegation_id) return await revokeDelegation(tenantId, input.delegation_id);
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        const { safeQuery, tenantSchema } = await import('../../../../config/database'); const schema = tenantSchema(tenantId); const getCommittees = async (_t: string) => (await safeQuery(`SELECT * FROM "${schema}".governance_committees WHERE deleted_at IS NULL ORDER BY name_en`)).rows; const getCommitteeDetail = async (_t: string, id: string) => (await safeQuery(`SELECT * FROM "${schema}".governance_committees WHERE committee_id=$1`, [id])).rows[0];
        if (input.action === 'list') return { committees: await getCommittees(tenantId) };
        if (input.action === 'detail' && input.committee_id) return await getCommitteeDetail(tenantId, input.committee_id);
    ...
    },
    {
      ...
      input_schema: { type: 'object', properties: {}, required: [] },
      handler: async (tenantId) => {
        const { safeQuery, tenantSchema } = await import('../../../../config/database'); const schema = tenantSchema(tenantId); const getOwnershipMap = async (_t: string) => (await safeQuery(`SELECT entity_type, entity_id, owner_user_id, ownership_type FROM "${schema}".responsibilities WHERE deleted_at IS NULL LIMIT 500`)).rows;
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        const { safeQuery, tenantSchema } = await import('../../../../config/database'); const schema = tenantSchema(tenantId); const listOwnership = async (_t: string, domain: string) => (await safeQuery(`SELECT * FROM "${schema}".responsibilities WHERE entity_type=$1 AND deleted_at IS NULL`, [domain])).rows; const assignOwnership = async (_t: string, domain: string, p: any) => (await safeQuery(`INSERT INTO "${schema}".responsibilities (entity_type, entity_id, owner_user_id, ownership_type, created_by) VALUES ($1,$2,$3,$4,$5) RETURNING *`, [domain, p.entityId, p.userId, p.ownershipType, p.assignedBy])).rows[0]; const removeOwnership = async (_t: string, _d: string, id: string) => { await safeQuery(`UPDATE "${schema}".responsibilities SET deleted_at=NOW() WHERE id=$1`, [id]); };
        const domain = input.domain as 'control' | 'risk' | 'evidence';
        if (input.action === 'list') return { owners: await listOwnership(tenantId, domain) };
        if (input.action === 'assign') return await assignOwnership(tenantId, domain, { entityId: input.entityId!, userId: input.userId!, ownershipType: input.ownershipType || 'primary', assignedBy: 'agent-A02' });
        if (input.action === 'remove' && input.ownerId) { await removeOwnership(tenantId, domain, input.ownerId); return { removed: true }; }
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        ...
        if (filter === 'inactive') return { users: users.filter(u => !u.is_active) };
        if (filter === 'no_mfa') return { users: users.filter(u => !u.mfa_enabled) };
        if (filter === 'admin') return { users: users.filter(u => ['admin'].includes(u.role)) };
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        ...
        if (scope === 'over_privileged') return { overPrivileged, count: overPrivileged.length };
        if (scope === 'inactive') return { inactiveWithRoles: inactive, count: inactive.length };
        if (scope === 'excessive_roles') return { excessiveRoles, count: excessiveRoles.length };
    ...
    },
    {
      ...
      handler: async (tenantId, input) => {
        ...
        for (const a of allAssignments) { userRoleCounts.set(a.user_id, (userRoleCounts.get(a.user_id) || 0) + 1); }
