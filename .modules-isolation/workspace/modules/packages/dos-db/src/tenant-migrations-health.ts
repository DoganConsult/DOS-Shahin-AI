...
export interface TenantMigrationsHealth {
  ...
  topFailedMigrations: Array<{ migrationId: string; failedTenants: number; sampleError: string | null }>;
  oldestFailureAt: string | null;
  status: 'ok' | 'degraded' | 'critical';
}

export async function summarizeTenantMigrationsHealth(): Promise<TenantMigrationsHealth> {
  // 1. tenants tracked (any row at all)
  const tenants = await safeQuery<{ n: number }>(`SELECT count(DISTINCT tenant_id)::int AS n FROM dos.tenant_migrations`);
  const tenantsTracked = tenants.rows[0]?.n ?? 0;

  // 2. tenants with at least one failed migration in latest state
  const failed = await safeQuery<{ n: number; total: number; oldest: string | null }>(`
  ...
  const top = await safeQuery<{ migration_id: string; failed_tenants: number; sample_error: string | null }>(`
    SELECT migration_id,
