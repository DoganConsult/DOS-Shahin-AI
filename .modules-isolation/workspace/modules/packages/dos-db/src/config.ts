...
export function getPlatformConnectionConfig(): PlatformDbConfig {
  ...
  const config: PlatformDbConfig = {
    ...
    pool: {
      // Default kept conservative because the platform fans out across ~42
      // PM2 services against a single PostgreSQL with max_connections=200.
      // 42 * 20 (old default) = 840 → guaranteed pool exhaustion cascade
      // under any sustained load. 42 * 8 = 336 worst-case but in practice
      // services hold ~1–3 simultaneous backends, so this leaves headroom
      // for the migrator and DBA sessions. High-traffic services should
      // override via PG_POOL_MAX in their per-service env file.
      max: parseInt(process.env.PG_POOL_MAX || '8', 10),
      // Reap idle backends faster so a brief spike doesn't pin connections
      // for 30s after the work is done.
