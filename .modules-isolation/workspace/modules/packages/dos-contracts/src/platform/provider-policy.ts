...
export const PROVIDER_POLICY_MATRIX: ProviderPolicyEntry[] = [
...
export function isDeterministicTask(task: string): boolean {
  const entry = PROVIDER_POLICY_MATRIX.find(e => e.task === task);
  return entry?.determinism === 'deterministic';
}

export function getTaskFallbackBehavior(task: string): FallbackBehavior {
  const entry = PROVIDER_POLICY_MATRIX.find(e => e.task === task);
  return entry?.fallbackBehavior ?? 'never';
}

export function getDeterministicTasks(): ProviderPolicyEntry[] {
  return PROVIDER_POLICY_MATRIX.filter(e => e.determinism === 'deterministic');
}

export function getAiEnhancedTasks(): ProviderPolicyEntry[] {
  return PROVIDER_POLICY_MATRIX.filter(e => e.determinism === 'ai_enhanced');
}

export function validateProviderPolicyIntegrity(): string[] {
  const errors: string[] = [];
  for (const entry of PROVIDER_POLICY_MATRIX) {
