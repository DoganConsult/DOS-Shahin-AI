...
export interface AgentGovernance {
  ...
  escalationPolicy: { escalateTo: string; afterMinutes: number };
...
export interface ProductDefinition {
  ...
  rbac: { roleLandings: RoleLandingConfig };
  nav: NavItemDef[];
  kpis: KpiDef[];
...
const _productRegistry = new Map<string, ProductDefinition>();

export function registerProduct(definition: ProductDefinition): void {
  _productRegistry.set(definition.code, definition);
}

export function getProduct(code: string): ProductDefinition | undefined {
