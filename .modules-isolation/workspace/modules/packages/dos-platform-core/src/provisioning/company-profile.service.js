...
export function classifyCompanySize(employeeCountOrBand) {
...
export async function createCompanyProfile(tenantId, profile) {
    await ensureCompanyProfileTable();
    const updatedAt = new Date().toISOString();
    await safeQuery(`INSERT INTO public.company_profiles (tenant_id, profile, updated_at)
...
export async function getCompanyProfile(tenantId) {
    await ensureCompanyProfileTable();
...
export async function detectApplicableFrameworks(industrySector) {
...
export function recommendRolesForSize(employeeCount, frameworks = []) {
