...
export function toXML(data: any[], moduleCode: string): string {
...
export function toXLSX(data: any[], columns?: string[], sheetName: string = 'Export'): Buffer {
...
export function buildZipBuffer(files: Record<string, string>): Buffer {
...
function utf16BEEncode(str: string): Buffer {
...
export function toPDF(data: any[], title: string, columns?: string[], watermark?: string): Buffer {
  const headers = columns && columns.length > 0 ? columns : (data.length > 0 ? Object.keys(data[0]) : []);
  const headerLabels = headers.map(h => humanizeHeader(h));
...
function buildUnicodePDFBuffer(lines: string[], title: string): Buffer {
  const fontSize = 10;
  const margin = 50;
  const pageWidth = 595;
  const pageHeight = 842;
