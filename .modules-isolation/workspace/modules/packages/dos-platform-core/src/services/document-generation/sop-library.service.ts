...
function normalizeSteps(raw: unknown): Array<{ step: number; description: string }> {
...
export async function getSOPs(tenantId: string, filters: SopFilters = {}): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  const params: unknown[] = [];
  const conditions: string[] = [];
  let idx = 1;
  if (filters.processType) { conditions.push(`process_type = $${idx++}`); params.push(filters.processType); }
  if (filters.stageId) { conditions.push(`stage_id = $${idx++}`); params.push(filters.stageId); }
  if (filters.roleId) { conditions.push(`role_id = $${idx++}`); params.push(filters.roleId); }
  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
  const result = await safeQuery(
    `SELECT sop_id, process_type, stage_id, role_id, title_en, title_ar, steps_en, steps_ar, prerequisites, expected_output, sla_hours, version, status, updated_at
     FROM "${schema}".sop_procedures
     ${where}
     ORDER BY process_type, stage_id, updated_at DESC`,
    params,
  ).catch(() => ({ rows: [] as any[] }));
...
export async function upsertSOP(tenantId: string, sop: Record<string, any>): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);
  const sopId = String(sop.sopId ?? sop.sop_id ?? randomUUID());
...
export async function deleteSOP(tenantId: string, sopId: string): Promise<void> {
  const schema = tenantSchema(tenantId);
  await safeQuery(`DELETE FROM "${schema}".sop_procedures WHERE sop_id = $1`, [sopId]);
}
...
export async function seedDefaultSOPs(tenantId: string): Promise<number> {
  ...
  for (const sop of BASELINE_SOPS) {
    try {
      ...
      logger.warn('[SOPLibrary] seed skip', { tenantId, processType: sop.processType, stageId: sop.stageId, error: toErrorMessage(error) });
    }
  }
  return seeded;
}

export async function startSOPCompletion(tenantId: string, sopId: string, userId: string, totalSteps: number): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);
  await ensureCompletionTable(schema);
  const completionId = randomUUID();
  await safeQuery(
    `INSERT INTO "${schema}".sop_completions (completion_id, sop_id, user_id, total_steps, completed_steps, status)
     VALUES ($1, $2, $3, $4, '[]'::jsonb, 'in_progress')`,
    [completionId, sopId, userId, totalSteps],
  );
  return { completionId, sopId, userId, totalSteps, completedSteps: [], status: 'in_progress' };
}

export async function updateSOPProgress(tenantId: string, completionId: string, completedSteps: unknown[], notes?: string): Promise<Record<string, unknown> | null> {
  const schema = tenantSchema(tenantId);
  await ensureCompletionTable(schema);
  const normalizedSteps = normalizeSteps(completedSteps);
  const existing = await safeQuery(
    `SELECT sop_id, user_id, total_steps FROM "${schema}".sop_completions WHERE completion_id = $1 LIMIT 1`,
    [completionId],
  );
  const row = existing.rows[0] as Record<string, any> | undefined;
  if (!row) return null;
  const isCompleted = normalizedSteps.length >= Number(row.total_steps ?? 0) && Number(row.total_steps ?? 0) > 0;
  await safeQuery(
    `UPDATE "${schema}".sop_completions
    ...
    [completionId, JSON.stringify(normalizedSteps), notes ?? null, isCompleted ? 'completed' : 'in_progress'],
  );
...
export async function getSOPCompletions(tenantId: string, filters: SopCompletionFilters = {}): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  await ensureCompletionTable(schema);
  const params: unknown[] = [];
  const conditions: string[] = [];
  let idx = 1;
  if (filters.sopId) { conditions.push(`sop_id = $${idx++}`); params.push(filters.sopId); }
  if (filters.userId) { conditions.push(`user_id = $${idx++}`); params.push(filters.userId); }
  if (filters.status) { conditions.push(`status = $${idx++}`); params.push(filters.status); }
  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
  const limit = Math.min(filters.limit ?? 100, 500);
  const result = await safeQuery(
    `SELECT completion_id, sop_id, user_id, total_steps, completed_steps, notes, status, started_at, updated_at, completed_at
...
export async function getSOPComplianceStats(tenantId: string): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);
  await ensureCompletionTable(schema);
  const [sopResult, completionResult] = await Promise.all([
    safeQuery(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'active')::int AS active FROM "${schema}".sop_procedures`).catch(() => ({ rows: [{ total: 0, active: 0 }] })),
    safeQuery(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'completed')::int AS completed FROM "${schema}".sop_completions`).catch(() => ({ rows: [{ total: 0, completed: 0 }] })),
  ]);
