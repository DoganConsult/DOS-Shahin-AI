...
export async function getSOPs(tenantId, filters = {}) {
    ...
    const result = await safeQuery(`SELECT sop_id, process_type, stage_id, role_id, title_en, title_ar, steps_en, steps_ar, prerequisites, expected_output, sla_hours, version, status, updated_at
...
export async function upsertSOP(tenantId, sop) {
    const schema = tenantSchema(tenantId);
    ...
    await safeQuery(`INSERT INTO "${schema}".sop_procedures
...
export async function deleteSOP(tenantId, sopId) {
    const schema = tenantSchema(tenantId);
    await safeQuery(`DELETE FROM "${schema}".sop_procedures WHERE sop_id = $1`, [sopId]);
}
export async function seedDefaultSOPs(tenantId) {
    let seeded = 0;
...
export async function startSOPCompletion(tenantId, sopId, userId, totalSteps) {
    const schema = tenantSchema(tenantId);
    await ensureCompletionTable(schema);
    const completionId = randomUUID();
    await safeQuery(`INSERT INTO "${schema}".sop_completions (completion_id, sop_id, user_id, total_steps, completed_steps, status)
...
export async function updateSOPProgress(tenantId, completionId, completedSteps, notes) {
    const schema = tenantSchema(tenantId);
    await ensureCompletionTable(schema);
    const normalizedSteps = normalizeSteps(completedSteps);
    const existing = await safeQuery(`SELECT sop_id, user_id, total_steps FROM "${schema}".sop_completions WHERE completion_id = $1 LIMIT 1`, [completionId]);
    const row = existing.rows[0];
    if (!row)
        return null;
    const isCompleted = normalizedSteps.length >= Number(row.total_steps ?? 0) && Number(row.total_steps ?? 0) > 0;
    await safeQuery(`UPDATE "${schema}".sop_completions
...
export async function getSOPCompletions(tenantId, filters = {}) {
    const schema = tenantSchema(tenantId);
    await ensureCompletionTable(schema);
    const params = [];
    const conditions = [];
    ...
    const result = await safeQuery(`SELECT completion_id, sop_id, user_id, total_steps, completed_steps, notes, status, started_at, updated_at, completed_at
...
export async function getSOPComplianceStats(tenantId) {
    const schema = tenantSchema(tenantId);
    await ensureCompletionTable(schema);
