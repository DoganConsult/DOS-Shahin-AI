...
export async function advanceRollbackStatus(
  ...
  if (newStatus === 'completed') {
    await safeQuery(
      `UPDATE dos_rollbacks SET status = 'completed', completed_at = $1 WHERE rollback_id = $2`,
      [now, rollbackId],
    );
    ...
    await safeQuery(
      `UPDATE dos_rollbacks SET status = 'failed', failed_at = $1, failure_reason = $2 WHERE rollback_id = $3`,
      [now, failureReason ?? null, rollbackId],
    );
    await publish('delivery.rollback.failed', tenantId, { rollbackId, reason: failureReason }, {});
  } else {
    await safeQuery(
      `UPDATE dos_rollbacks SET status = $1 WHERE rollback_id = $2`,
...
export async function getRollback(tenantId: string, rollbackId: string): Promise<RollbackRecord | null> {
  const result = await safeQuery(
    `SELECT * FROM dos_rollbacks WHERE rollback_id = $1 LIMIT 1`,
    [rollbackId],
  );
...
export async function getRollbackByDeployment(tenantId: string, deploymentId: string): Promise<RollbackRecord | null> {
  const result = await safeQuery(
    `SELECT * FROM dos_rollbacks WHERE deployment_id = $1 ORDER BY initiated_at DESC LIMIT 1`,
    [deploymentId],
  );
  if (!result.rows[0]) return null;
  return mapRollbackRow(result.rows[0]);
...
export async function listRollbacks(tenantId: string): Promise<RollbackRecord[]> {
  const result = await safeQuery(
    `SELECT * FROM dos_rollbacks ORDER BY initiated_at DESC`,
    [],
  );
