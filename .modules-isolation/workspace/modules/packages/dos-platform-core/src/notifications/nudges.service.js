...
export async function getActiveNudges(tenantId, userId, currentRoute) {
    await ensureNudgesTable();
    const result = await safeQuery(`SELECT nudge_id, title, body, route, created_at
...
export async function dismissNudge(tenantId, nudgeId) {
    await ensureNudgesTable();
