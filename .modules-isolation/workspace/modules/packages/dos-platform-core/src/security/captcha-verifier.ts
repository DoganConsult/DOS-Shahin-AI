...
import { verifyCaptchaSvg } from './captcha-svg.service';
...
const EXTERNAL_PROVIDER_URLS: Record<Exclude<CaptchaProvider, 'svg-self-hosted'>, string> = {
...
export async function verifyCaptcha(input: CaptchaVerifyInput): Promise<CaptchaVerifyResult> {
  ...
  if (provider === 'svg-self-hosted') {
    ...
    const r = await verifyCaptchaSvg(input.redis ?? null, input.captchaId, input.token);
    return {
      success: r.success,
      provider,
      durationMs: Date.now() - startedAt,
  ...
  try {
    ...
    const errors = Array.isArray(json['error-codes'])
      ? (json['error-codes'] as string[])
      : undefined;
    const score = typeof json.score === 'number' ? (json.score as number) : undefined;
