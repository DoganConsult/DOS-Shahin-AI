...
let _logger: { warn: (obj: Record<string, unknown>, msg: string) => void } | null = null;

function getLogger(): { warn: (obj: Record<string, unknown>, msg: string) => void } {
  if (!_logger) {
    try {
 ...
 */
export function logFieldDenial(entry: FieldDenialAuditEntry): void {
...
async function persistFieldDenialAudit(entry: FieldDenialAuditEntry): Promise<void> {
  try {
 ...
 */
export function logSuperAdminFieldAccess(entry: Omit<FieldDenialAuditEntry, 'fieldsDenied' | 'fieldsRedacted'> & {
