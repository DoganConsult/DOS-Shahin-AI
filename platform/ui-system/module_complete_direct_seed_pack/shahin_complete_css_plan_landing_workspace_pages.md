...
# 10. Actual-state audit (appended 2026-05-04)

Audit of this CSS plan vs current repo state.

## 10.1 File presence

| Spec path | Actual state | Gap |
|---|---|---|
| `src/marketing/marketing-home.page.scss` | EXISTS | none |
| `src/marketing/marketing-home.page.ts` (`styleUrl` link) | EXISTS | verify wiring |
| `src/shell/workspace-host-kit/workspace-host-kit.scss` | NOT PRESENT — kit shipped as `src/shell/workspace-host-kit.ts` (flat barrel, no sidecar SCSS) | GAP-CSS-1 |
| `src/shell/workspace-header/workspace-header.component.scss` | NOT PRESENT — `workspace-header.component.ts` uses inline `styles: [\`…\`]` | GAP-CSS-2 |
| `src/shell/workspace-sidebar/workspace-sidebar.component.scss` | NOT PRESENT — inline styles in `workspace-sidebar.component.ts` | GAP-CSS-3 |
| `src/shell/mobile-drawer/mobile-drawer.component.scss` | NOT PRESENT — inline styles in `mobile-drawer.component.ts` | GAP-CSS-4 |
| `src/shell/mobile-bottom-nav/mobile-bottom-nav.component.scss` | NOT PRESENT — inline styles in `mobile-bottom-nav.component.ts` | GAP-CSS-5 |
| `src/shell/command-search/command-search.component.scss` | NOT PRESENT — inline styles in `command-search.component.ts` | GAP-CSS-6 |
| `src/shell/inbox-center/inbox-center.component.scss` | NOT PRESENT — inline styles in `inbox-center.component.ts` | GAP-CSS-7 |
| `src/page/page-layout.scss` | DIRECTORY ABSENT — `src/page/` does not exist | GAP-CSS-8 |
| `src/page/page-archetypes.scss` | ABSENT | GAP-CSS-8 |
| `src/page/page-responsive.scss` | ABSENT | GAP-CSS-8 |
| `src/page/page-states.scss` | ABSENT | GAP-CSS-8 |

Spec §1.2 explicitly says *"if current wrappers use different exact paths, do not rename files. Add styles beside the existing components."* All 7 shell wrappers ship inline-`styles` instead of sidecar `.component.scss`. **This is compliant with the §1.2 escape clause** but means the CSS layered architecture (token bridge + logical-property normalization) currently lives inline; auditing/diffing tokens across surfaces requires reading 7 inline blocks instead of dedicated SCSS files.

## 10.2 Fix path

- **GAP-CSS-1..7 (shell wrapper SCSS extraction)** — non-blocking. Two acceptable resolutions:
  1. Promote the inline `styles: [\`…\`]` blocks to sidecar `*.component.scss` files and switch the components to `styleUrl`. Keeps spec exactly. Patch order: header → sidebar → mobile-drawer → mobile-bottom-nav → command-search → inbox-center.
  2. Update this spec §1.2 to reflect the inline-styles convention and add a token-coverage CI guard (`grep -RE '#[0-9a-fA-F]{3,8}|margin-left|padding-right' platform/ui-system/dos-ui-system/src/shell/*.component.ts`) instead of file-presence checks.
  
  Recommended: (1) — sidecar SCSS makes the §2 token bridge auditable per surface and aligns with the marketing wrapper.

- **GAP-CSS-8 (page primitive SCSS)** — non-blocking. Create `src/page/` with the four files exactly as §1.3 specifies. None of the 60+ business pages currently consume this layer; introduction is additive. Wire only when a module page opts in via class names like `dos-page`, `dos-page__masthead`, etc.

- **Patch sequence per spec §6:** Patch 1 (marketing) DONE. Patch 2 (workspace shell) = GAP-CSS-1..7. Patch 3 (page primitives) = GAP-CSS-8. Patch 4 (contract refactor) untouched.

## 10.3 Validation already passing

- `marketing-home.page.scss` exists with the §3 scaffold (`dos-mh-*` selectors covered).
- Shahin SPA build green (per Task-2 status header in companion `shahin_workspace_step2_fix_plan.md`).
- No raw vendor leakage outside ui-system; ShellHost imports only `@dos/ui-system` chrome.

## 10.4 Verdict

`COMPLETE WITH NON-BLOCKING FOLLOW-UP` — Patch 1 satisfied; Patch 2 + 3 pending and additive only. No P0 blocker. Approval gate: confirm whether to convert inline styles to sidecar SCSS files (recommended) or amend the spec convention.
