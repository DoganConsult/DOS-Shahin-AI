...
export interface DecisionDiffField {
  field:    string;             // pre-resolved label
...
export interface DecisionAffected {
  kind:     string;             // 'user' | 'role' | 'policy' | 'control' | …
  id:       string;
  name?:    string;
}
...
export interface DecisionPreview {
  ...
  beforeRaw?:          unknown;          // free-form fallback
  afterRaw?:           unknown;
  ...
  whyBlocked?:         string;            // when riskLevel === 'blocked'
...
export class DosDecisionPreviewPanelComponent {
  @Input() preview?: DecisionPreview;

  // Pre-resolved labels (i18n done upstream).
  @Input() eyebrowLabel        = 'decision preview';
  @Input() riskLabel           = 'risk';
  @Input() blockedTitle        = 'Action blocked';
  @Input() diffFieldLabel      = 'Field';
  @Input() diffBeforeLabel     = 'Before';
  @Input() diffAfterLabel      = 'After';
  @Input() diffAriaLabel       = 'Field-level diff';
  @Input() permissionImpactLabel = 'Permission impact';
  @Input() sodImpactLabel      = 'SoD impact';
  @Input() workflowImpactLabel = 'Workflow impact';
  @Input() auditEventLabel     = 'Audit event';
  @Input() affectedLabel       = 'Affected';
  @Input() rollbackLabel       = 'Rollback';
  @Input() rollbackNoneLabel   = 'No rollback available';
  @Input() rollbackManualLabel = 'Manual rollback';
  @Input() rollbackAutoLabel   = 'Automatic rollback supported';
  @Input() approversLabel      = 'Approvers';
  @Input() reasonLabel         = 'Reason';
  @Input() reasonPlaceholder   = 'Why are you making this change?';
  @Input() cancelLabel         = 'Cancel';
  @Input() requestChangeLabel  = 'Request change';
  ...
  rollbackOptionLabel(opt: 'none' | 'manual' | 'automatic'): string {
    if (opt === 'manual')    return this.rollbackManualLabel;
    if (opt === 'automatic') return this.rollbackAutoLabel;
