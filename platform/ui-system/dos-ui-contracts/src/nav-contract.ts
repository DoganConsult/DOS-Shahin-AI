...
export type DosNavDisabledReason =
  | 'not-entitled'
  | 'missing-permission'
  | 'backend-offline'
  | 'route-not-wired'
  | 'coming-soon'
  // Phase G T5 — trial-aware reason codes consumed by Dynamic UI.
  // 'trial-expired'       → tenant trial ended (grace also passed) and the
  //                         module's trial entitlement was deactivated.
  // 'trial-limit-reached' → module is entitled but a per-tenant cap in
  //                         tenant_module_entitlements.limits_json was hit
  //                         (e.g. maxAiRuns / maxRecords). UI surfaces an
  //                         upgrade CTA rather than hiding the item.
  | 'trial-expired'
  | 'trial-limit-reached';
