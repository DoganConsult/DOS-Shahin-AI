...
export async function recordObservation(input: ObservationInput): Promise<Observation | null> {
  const schema = tenantSchema(input.tenantId);
  try {
    const result = await safeQuery(
      `INSERT INTO "${schema}".ai_observations
...
export async function listObservations(
  ...
  const [dataRes, countRes] = await Promise.all([
    safeQuery(
      `SELECT * FROM "${schema}".ai_observations WHERE ${where} ORDER BY created_at DESC LIMIT $${idx} OFFSET $${idx + 1}`,
      [...params, limit, offset],
    ),
...
export async function getObservationsForEntity(
  ...
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_observations
...
export async function acknowledgeObservation(
  ...
  try {
    const result = await safeQuery(
      `UPDATE "${schema}".ai_observations SET status = 'acknowledged'
...
export async function getObservationStats(tenantId: string): Promise<{
  active: number; acknowledged: number; resolved: number; dismissed: number; total: number;
  ...
  try {
    ...
    const r: any = getFirstRow(result) || {};
    return {
      active: r.active || 0,
