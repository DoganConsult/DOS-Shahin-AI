...
export async function createAlert(input: AlertInput): Promise<Alert | null> {
  ...
  try {
    ...
    const row = getFirstRow(result) as Alert | undefined;
    if (row) {
      eventBus.publish({
...
export async function listAlerts(
  ...
  const countRow = getFirstRow(countRes) as { total: number } | undefined;
  return { items: dataRes.rows as Alert[], total: countRow?.total || 0 };
}
  ...
  try {
    ...
    if (result.rows.length > 0) {
      eventBus.publish(({
              ...
              payload: {
                alertId,
                escalationLevel: (getFirstRow(result) as { escalation_level: number } | undefined)
                  ?.escalation_level,
              },
            } as any));
      return true;
...
export async function getAlertStats(tenantId: string): Promise<{
  ...
  try {
    ...
    const r = (getFirstRow(statusRes) as StatusAgg | undefined) ?? ({} as StatusAgg);
