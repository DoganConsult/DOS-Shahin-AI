...
import { aiRoutes, agentRoutes, agentRegistryRoutes, infoRoutes } from './routes/index';
import runtimeHealthRoutes from './routes/runtime-health.routes';
import { setServiceBus } from './events/publisher';
import { registerConsumers } from './events/consumer';

const SERVICE_CODE = 'ai-gateway-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    routes: [
      ...
      { path: '/api/ai/agents', router: agentRoutes },
      { path: '/api/ai/registry', router: agentRegistryRoutes },
      { path: '/api/ai-gateway', router: infoRoutes },
      { path: '/api/runtime-health', router: runtimeHealthRoutes },
    ],
    healthChecks: {
      database: async () => {
        try {
