...
import { LANGGRAPH_CONFIG as _LANGGRAPH_CONFIG } from '../config/langgraph.config';
...
function calculatePercentiles(values: number[], percentiles: number[]): Record<string, number> {
  if (values.length === 0) return Object.fromEntries(percentiles.map(p => [`p${p}`, 0]));
...
function calculateThroughput(totalDurationMs: number, totalOperations: number): number {
  if (totalDurationMs <= 0) return 0;
  return (totalOperations / totalDurationMs) * 1000;
}

function getFirstRow(result: { rows: Record<string, any>[] }): Record<string, any> {
  return result.rows[0] || {};
}

async function streamAgentMetrics(_tenantId: string, _metrics: AgentMetrics): Promise<void> {
...
export interface AgentMetrics {
  ...
  // Percentile metrics (calculated from historical data)
  p50LatencyMs?: number;
  p95LatencyMs?: number;
  p99LatencyMs?: number;
 ...
 */
export async function recordAgentMetrics(metrics: AgentMetrics): Promise<void> {
  const schema = tenantSchema(metrics.tenantId);

  try {
    // Calculate percentiles from historical data (last 100 runs for same agent)
    const historicalData = await safeQuery(
      `SELECT duration_ms, tool_calls
       FROM "${schema}".langgraph_agent_metrics
       WHERE agent_id = $1 AND tenant_id = $2 AND status = 'success'
       ORDER BY start_time DESC
       LIMIT 100`,
      [metrics.agentId, metrics.tenantId],
    );

    const latencyValues = historicalData.rows.map((r: GenericRow) => r.duration_ms).filter((v: number) => v > 0);
    const percentiles = calculatePercentiles(latencyValues, [50, 95, 99]);

    // Calculate throughput: operations per second
    const totalOperations = historicalData.rows.reduce((sum: number, r: GenericRow) => sum + (r.tool_calls || 0), 0);
    const totalDuration = historicalData.rows.reduce((sum: number, r: GenericRow) => sum + (r.duration_ms || 0), 0);
    ...
    await safeQuery(
      `INSERT INTO "${schema}".langgraph_agent_metrics
    ...
    const metricsWithPercentiles: AgentMetrics = {
      ...metrics,
      p50LatencyMs: p50,
      p95LatencyMs: p95,
      p99LatencyMs: p99,
  ...
  try {
    const result = await safeQuery(
      `SELECT
       ...
       GROUP BY agent_id, graph_type, template_type`,
      [tenantId, cutoff],
    );
...
export async function getErrorAnalytics(
  ...
  try {
    ...
    // Get errors by agent
    const agentErrorsResult = await safeQuery(
      `SELECT
         agent_id,
         error,
         COUNT(*)::int AS count,
    ...
    const errorTypeResult = await safeQuery(
      `SELECT
         CASE
    ...
    // Get recent errors
    const recentErrorsResult = await safeQuery(
      `SELECT
         agent_id,
         error,
         graph_type,
  ...
  try {
    const result = await safeQuery(
      `SELECT
       ...
       FROM "${schema}".langgraph_agent_metrics
       WHERE tenant_id = $1 AND agent_id = $2 AND start_time >= $3`,
 ...
 */
export async function getMetricsByTraceId(
  tenantId: string,
  langsmithTraceId: string,
  ...
  try {
    const result = await safeQuery(
      `SELECT * FROM "${schema}".langgraph_agent_metrics
       WHERE tenant_id = $1 AND langsmith_trace_id = $2
       LIMIT 1`,
      [tenantId, langsmithTraceId],
    );
