 ...
 */
export async function addToDeadLetterQueue(input: CreateDLQEntryInput): Promise<string> {
  const schema = tenantSchema(input.tenantId);

  try {
    const result = await safeQuery(
      `INSERT INTO "${schema}".agent_dead_letter_queue
...
export async function resolveDLQEntry(
  ...
  try {
    await safeQuery(
      `UPDATE "${schema}".agent_dead_letter_queue
       SET resolved_at = NOW(),
           resolution_action = $1,
           resolved_by = $2,
           resolution_notes = $3,
           updated_at = NOW()
       WHERE dlq_id = $4 AND tenant_id = $5`,
      [resolutionAction, resolvedBy, resolutionNotes || null, dlqId, tenantId],
    );
  } catch (err: unknown) {
