 ...
 */
export async function recordAndScore(rec: SmokeRunRecord): Promise<{
  score: number; total: number; samples: number; mean: number; eligibleForProduction: boolean;
}> {
  if (!rec.agentId) return { score: 0, total: 0, samples: 0, mean: 0, eligibleForProduction: false };

  let evalResult: { score: number; matched: number; total: number };
  try {
    ...
    logger.warn(`[smoke-evaluator] eval failed: ${(err as Error).message}`);
  ...
  try {
    ...
    await safeQuery(
      `INSERT INTO public.ai_smoke_scores
         (agent_code, score, matched, total, trace_ref, prompt_version)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [rec.agentId, evalResult.score, evalResult.matched, evalResult.total, rec.traceRef ?? null, rec.promptVersion ?? null],
    );

    // Rolling readiness over the most recent 20 evaluations.
    const r = await safeQuery(
      `SELECT count(*)::int AS n, avg(score)::numeric(5,4) AS mean
         FROM (
    ...
    logger.warn(`[smoke-evaluator] persist failed: ${(err as Error).message}`);
 ...
 */
export async function getPromotionReadiness(): Promise<Array<{
  agentCode: string; samples: number; mean: number; eligibleForProduction: boolean; lastEvaluatedAt: string | null;
}>> {
  try {
    const { safeQuery } = await import('@dos/db');
    const r = await safeQuery(
    ...
    logger.warn(`[smoke-evaluator] readiness query failed: ${(err as Error).message}`);
    return [];
  }
}
