...
interface LangSmithRunPost {
  id: string;
  trace_id: string;
  dotted_order: string;
  name: string;
  run_type: 'llm' | 'tool' | 'chain' | 'retriever' | 'parser' | 'embedding';
...
function dottedOrderFor(startedAt: number, runId: string): string {
...
function isEnabled(): boolean {
  return (
    (process.env.LANGSMITH_TRACING === 'true' || process.env.LANGSMITH_TRACING === '1') &&
    !!process.env.LANGSMITH_API_KEY
  );
}

async function postBatch(payload: { post?: LangSmithRunPost[]; patch?: Array<Record<string, unknown>> }): Promise<void> {
  if (!isEnabled()) return;
  ...
  try {
    ...
    logger.warn(`[langsmith-bridge] ingest failed: ${(err as Error).message}`);
  }
}
 ...
 */
export async function recordLlmRun(rec: LlmRunRecord): Promise<void> {
  if (!isEnabled()) return;
  const session = process.env.LANGSMITH_PROJECT || 'default';
  const id = randomUUID();
  const post: LangSmithRunPost = {
    id,
    trace_id: id,
