...
function offlineStub(opts: ClaudeRequestOptions, latencyMs: number, reason: string): ClaudeResponse {
  ...
  return {
    ...
    stopReason: 'end_turn',
    toolCalls: [],
    rawBlocks: [{ type: 'text', text: summary } as any],
    latencyMs,
  };
}

async function callOpenRouter(opts: ClaudeRequestOptions, apiKey: string): Promise<ClaudeResponse> {
  ...
  return {
    ...
    toolCalls: [],
    rawBlocks: [{ type: 'text', text: content } as any],
    latencyMs: Date.now() - start,
  };
}

async function callOllama(opts: ClaudeRequestOptions, baseUrl: string): Promise<ClaudeResponse> {
  const start = Date.now();
...
// ---------------------------------------------------------------------------
// Wave 5.3 — LLM provider circuit breaker.
// Opens after 5 provider failures within 60s; cooldown 30s before
// allowing a half-open trial. While open, callClaude returns the offline
// stub so the agent runtime stays alive instead of cascading 5xx errors.
// Per-process state; multi-instance deployments recover independently.
// "no provider configured" is a config issue and does NOT trip the
// breaker — operator wants the warning, not the silence of an open
// circuit.
// ---------------------------------------------------------------------------
type _BreakerState = 'closed' | 'open' | 'half_open';
const _llmBreaker = {
  state: 'closed' as _BreakerState,
  failures: [] as number[],
  openedAt: 0,
  windowMs: 60_000,
  threshold: 5,
...
function _llmBreakerCheck(): { allow: boolean; reason?: string } {
  const now = Date.now();
...
function _llmBreakerSuccess(): void {
  if (_llmBreaker.state !== 'closed') logger.info(`[ClaudeClient] circuit closed (recovered from ${_llmBreaker.state})`);
  _llmBreaker.state = 'closed';
  _llmBreaker.failures = [];
  _llmBreaker.openedAt = 0;
}

function _llmBreakerFailure(): void {
  const now = Date.now();
...
export function getLlmCircuitBreakerState(): { state: _BreakerState; recentFailures: number; openedAt: number | null } {
  const now = Date.now();
  return {
...
async function _callClaudeInner(opts: ClaudeRequestOptions): Promise<ClaudeResponse> {
  const start = Date.now();
  ...
  if (providerError) _llmBreakerFailure();
