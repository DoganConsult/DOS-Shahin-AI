...
interface ProducerEntity {
  entityType: string;          // 'risk', 'compliance_gap', 'evidence', 'audit_plan', 'control'
  table: string;               // tenant table to scan
  eventType: string;           // canonical event published to bus + outbox
  aggregateType: string;       // for outbox row
  idColumn: string;            // PK column on the table
  titleColumn?: string;        // shown in event payload
  severityColumn?: string;     // optional severity field
  createdAtColumn: string;     // cursor column
}

const ENTITIES: ProducerEntity[] = [
...
async function listTenantSchemas(getPool: any): Promise<string[]> {
...
async function ensureCursor(client: any, entityType: string): Promise<Date> {
  const r = await client.query(
    `INSERT INTO ai_event_producer_cursors (entity_type)
     VALUES ($1)
...
async function emitForEntity(
  ...
  if (tableExists.rows.length === 0) return 0;
  ...
  if (ent.titleColumn) cols.push(`${ent.titleColumn} AS title`);
  if (ent.severityColumn) cols.push(`${ent.severityColumn} AS severity`);
  ...
  for (const row of r.rows) {
    ...
    if ((row as any).title) payload.title = (row as any).title;
    if ((row as any).severity) payload.severity = (row as any).severity;
    ...
    try {
      ...
      logger.warn(`[catch-up-producer] eventBus.publish failed: ${(err as Error).message}`);
    }
    ...
    try {
      await client.query(
        ...
        [ent.eventType, ent.aggregateType, String(row.aggregate_id), JSON.stringify(payload)],
      );
  ...
  await client.query(
    `UPDATE ai_event_producer_cursors
        SET last_seen_at = GREATEST(last_seen_at, $2),
            last_run_at  = NOW(),
            rows_emitted = rows_emitted + $3
      WHERE entity_type = $1`,
    [ent.entityType, maxSeen.toISOString(), emitted],
  );

  return emitted;
}

export async function runCatchUpProducerOnce(): Promise<{ tenants: number; emitted: number }> {
  ...
  for (const schema of schemas) {
    ...
    try {
      ...
      logger.warn(`[catch-up-producer] tenant ${tenantId} failed: ${(err as Error).message}`);
    }
  }
...
export function startCatchUpProducer(intervalMs: number = 60_000): void {
  if (_timer) return;
...
export function stopCatchUpProducer(): void {
