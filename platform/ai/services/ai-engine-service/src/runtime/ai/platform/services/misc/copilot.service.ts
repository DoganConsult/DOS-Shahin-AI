...
async function ensureCopilotTables(schema: string): Promise<void> {
...
export async function handleQuery(
  ...
  sessionId: string,
  query: string,
  opts: { pageContext?: string; activeAgentId?: string }
): Promise<any> {
  ...
  // Upsert session
  await safeQuery(
    `INSERT INTO "${schema}".copilot_sessions (session_id, tenant_id, user_id, agent_id, page_context, message_count)
     VALUES ($1, $2, $3, $4, $5, 1)
     ON CONFLICT (session_id) DO UPDATE SET message_count = copilot_sessions.message_count + 1, agent_id = $4`,
    [sessionId, tenantId, userId, agentId, opts.pageContext || null]
  ).catch(() => null);

  // Persist user message
  await safeQuery(
    `INSERT INTO "${schema}".copilot_messages (session_id, tenant_id, role, content, agent_id) VALUES ($1, $2, 'user', $3, $4)`,
    [sessionId, tenantId, query, agentId]
  ).catch(() => null);
  ...
  await safeQuery(
    `INSERT INTO "${schema}".copilot_messages (session_id, tenant_id, role, content, agent_id, tool_calls, tokens_in, tokens_out, duration_ms)
     VALUES ($1, $2, 'assistant', $3, $4, $5, $6, $7, $8)`,
    [sessionId, tenantId, responseMessage, agentId, JSON.stringify(proposedActions),
     runResult.totalInputTokens, runResult.totalOutputTokens, runResult.durationMs]
  ).catch(() => null);
...
// ── Session history ───────────────────────────────────────────────
export async function getSessionHistory(sessionId: string, tenantId: string): Promise<any[]> {
  const schema = tenantSchema(tenantId);
  await ensureCopilotTables(schema);
  const result = await safeQuery(
    `SELECT message_id, session_id, role, content, agent_id, tool_calls, tokens_in, tokens_out, duration_ms, created_at
     FROM "${schema}".copilot_messages WHERE session_id = $1 ORDER BY created_at ASC LIMIT 200`,
    [sessionId]
  );
  return result.rows;
}
...
// ── Agent performance ─────────────────────────────────────────────
export async function getAgentPerformance(agentId?: string): Promise<any[]> {
...
// ── Suggestions ───────────────────────────────────────────────────
export async function buildSuggestions(tenantId: string, agentId: string): Promise<{ suggestions: any[] }> {
  const schema = tenantSchema(tenantId);
  ...
  try {
    ...
    const recentErrors = await safeQuery(
      `SELECT COUNT(*) as cnt FROM "${schema}".agent_performance_log
       WHERE agent_id = $1 AND is_error = TRUE AND created_at > NOW() - INTERVAL '24 hours'`,
      [agentId]
    );
    ...
    const pendingDelegations = await safeQuery(
      `SELECT COUNT(*) as cnt FROM "${schema}".agent_delegations
...
/** Canonical landing/public agent row — aligned with packages/shahin-product AGRC_AGENTS (A01–A13). */
export type CanonicalPublicAgentLanding = {
  id: string;
  name: string;
  nameAr: string;
  summary: string;
...
export function getCanonicalPublicAgentsForLanding(): CanonicalPublicAgentLanding[] {
  return [
    {
      id: 'A01',
      ...
      nameAr: 'وكيل الإعداد',
      summary:
        'Guides new tenants through onboarding: collects industry, size, licenses, IT landscape, and recommends applicable KSA regulatory frameworks (NCA-ECC, SAMA-CSF, PDPL, etc.). Generates workspace seed configuration.',
      summaryAr:
        'يوجه المستأجرين الجدد عبر الإعداد: يجمع بيانات القطاع والحجم والتراخيص ويوصي بالأطر التنظيمية السعودية المناسبة.',
    ...
    },
    {
      ...
      summary:
        'User provisioning and RBAC: detects access anomalies, enforces least-privilege and MFA posture, supports SSO patterns (Azure AD / OIDC), and surfaces role distribution and over-privileged accounts for review.',
      summaryAr:
        'توفير المستخدمين وRBAC: يكشف حالات الوصول الشاذة ويفرض أقل الصلاحيات ووضع المصادقة الثنائية، ويدعم أنماط SSO، ويعرض توزيع الأدوار والحسابات ذات الصلاحيات المفرطة للمراجعة.',
    ...
    },
    {
      ...
      summary:
        'Maps controls across KSA regulatory frameworks (NCA-ECC, SAMA-CSF, PDPL, ISO 27001, PCI-DSS). Identifies overlapping requirements, reduces duplicate effort, and produces unified control matrices with confidence scores.',
    ...
    },
    {
      ...
      summary:
        'Drafts compliance controls, security policies, and implementation procedures aligned with KSA regulatory requirements. Generates bilingual (EN/AR) content with proper regulatory citations, maturity level targets, and evidence requirements.',
    ...
    },
    {
      ...
      summary:
        'Automates evidence gathering for compliance assessments. Analyzes uploaded documents (policies, screenshots, logs, certificates) to determine which controls they satisfy. Tracks evidence freshness, identifies gaps, and suggests required artifacts.',
    ...
    },
    {
      ...
      summary:
        'Analyzes assessment results to identify compliance gaps, calculates risk-weighted priority scores, generates phased remediation roadmaps with timelines, resource estimates, and projected compliance score improvements.',
    ...
    },
    {
      ...
      summary:
        'Manages the enterprise risk register; identifies cyber, compliance, and operational risks; scores using likelihood x impact matrices (5x5); recommends treatment strategies (Mitigate, Transfer, Accept, Avoid); tracks Key Risk Indicators (KRIs) and risk appetite thresholds.',
    ...
    },
    {
      ...
      summary:
        'Manages the complete policy lifecycle: drafting, review, approval, publication, distribution, acknowledgment tracking, periodic review, and retirement. Monitors regulatory changes that require policy updates and triggers review workflows automatically.',
    ...
    },
    {
      ...
      summary:
        'Assesses and monitors third-party/vendor risks: conducts due diligence questionnaires, evaluates vendor security posture, tracks SLA compliance, monitors for vendor breaches, and ensures supply chain alignment with NCA-ECC Third-Party Cybersecurity and SAMA-CSF requirements.',
    ...
    },
    {
      ...
      summary:
        'Generates comprehensive audit reports for regulators (NCA, SAMA, SDAIA), produces executive compliance dashboards, tracks certification status, and automates regulatory submission packages with proper formatting, Arabic translations, and evidence attachments.',
    ...
    },
    {
      ...
      summary:
        'Monitors business continuity readiness: BIA alignment, exercise cadence, RTO/RPO targets, dependency maps, and crisis communications discipline aligned with resilience expectations under national cybersecurity baselines.',
      summaryAr:
        'يراقب جاهزية استمرارية الأعمال: محاذاة تحليل الأثر التجاري، وتيرة التمارين، وأهداف RTO/RPO، وخرائط التبعيات.',
    ...
    },
    {
      ...
      summary:
        'Tracks security awareness and training programs: completion rates, overdue assignments, skill-gap themes, and campaign effectiveness — supporting human-risk reduction alongside technical controls.',
      summaryAr:
        'يتتبع برامج التوعية والتدريب الأمني: معدلات الإنجاز والتعيينات المتأخرة ومواضيع فجوات المهارات.',
    ...
    },
    {
      ...
      summary:
        'Public-facing assistant for anonymous visitors: explains Shahin-AI GRC capabilities, KSA regulatory context, and safe next steps — without tenant data or write actions.',
      summaryAr:
        'مساعد عام للزوار المجهولين: يشرح قدرات شاهين والسياق التنظيمي السعودي دون بيانات مستأجر أو إجراءات كتابة.',
...
function publicStr(v: unknown): string {
  return typeof v === 'string' ? v : v != null ? String(v) : '';
}

/** Deterministic, structured tool outputs for public chat (no tenant DB). */
async function executePublicBuiltinTool(
  name: string,
  input: Record<string, unknown>,
): Promise<string> {
  const clip = (s: string, n = 7500) => (s.length > n ? `${s.slice(0, n)}\n…(truncated)` : s);
  ...
  if (name === 'suggest_controls') {
    ...
    const extra: string[] = [];
    if (/(pdpl|privacy|pii|personal data)/i.test(req)) extra.push('Data inventory, RoPA-style processing records, DPIA where applicable, breach notification workflow');
    if (/(pci|payment|card)/i.test(req)) extra.push('Cardholder data environment scoping, segmentation evidence, ASV scanning where applicable');
    if (/(sama|bank|finance)/i.test(req)) extra.push('SAMA-CSF-aligned control themes: governance, defense, resilience, third-party');
    if (/(nca|ecc|cyber)/i.test(req)) extra.push('NCA-ECC subdomains: cybersecurity governance, defense, resilience, third-party/industrial as applicable');
...
async function buildOpenClawToolsForPublic(): Promise<{
  tools: Array<Record<string, unknown>>;
  runOpenClaw: (syntheticName: string, input: Record<string, unknown>) => Promise<string>;
}> {
  ...
  try {
    ...
    for (const row of listed) {
      ...
      if (!rawName || !allowed.has(rawName)) continue;
      const synthetic = `oc_${crypto.createHash('sha256').update(rawName).digest('hex')}`;
      if (syntheticToReal.has(synthetic)) continue;
    ...
    const bearer =
      process.env.OPENCLAW_PUBLIC_BEARER_TOKEN?.trim() ||
      process.env.OPENCLAW_HTTP_BEARER_TOKEN?.trim();

    const runOpenClaw = async (syntheticName: string, input: Record<string, unknown>) => {
...
const AGENT_PERSONA: Record<string, string> = {
  ...
  A13:
    'You are the Landing Copilot Agent: concise public explanations of Shahin-AI GRC, KSA frameworks (NCA-ECC, SAMA-CSF, PDPL), and demo/next-step CTAs — never claim access to tenant data.',
};

// ── Public query (no auth, rate-limited) ──────────────────────────
export async function handlePublicQuery(query: string, agentId?: string): Promise<any> {
  const publicTools = [
    {
      ...
      input_schema: { type: 'object' as const, properties: { concept: { type: 'string', description: 'The GRC concept to explain' } }, required: ['concept'] },
    },
    {
      name: 'compare_frameworks',
      description: 'Compare two compliance frameworks (e.g., ISO 27001 vs SOC2, NCA-ECC vs SAMA-CSF)',
      input_schema: { type: 'object' as const, properties: { framework_a: { type: 'string' }, framework_b: { type: 'string' } }, required: ['framework_a', 'framework_b'] },
    },
    {
      name: 'suggest_controls',
      description: 'Suggest relevant controls for a given risk or compliance requirement',
      input_schema: { type: 'object' as const, properties: { requirement: { type: 'string', description: 'The risk or compliance requirement' } }, required: ['requirement'] },
    },
  ];
  ...
  const allTools = [...publicTools, ...ocTools] as any[];
...
- If a tool errors, explain briefly and continue with safe guidance.`;
  ...
  try {
    ...
    const messages: Anthropic.MessageParam[] = [{ role: 'user', content: query.slice(0, 8000) }];
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      messages.push({
        role: 'assistant',
        content: response.rawBlocks as Anthropic.MessageParam['content'],
      });
...
// ── Audit export ──────────────────────────────────────────────────
export async function exportCopilotAudit(tenantId: string, opts: { from?: string; to?: string; topic?: string; format?: string }): Promise<any> {
  const schema = tenantSchema(tenantId);
  await ensureCopilotTables(schema);
  ...
  const result = await safeQuery(
    `SELECT m.message_id, m.session_id, m.role, m.content, m.agent_id, m.tokens_in, m.tokens_out, m.duration_ms, m.created_at
  ...
  if (opts.format === 'csv') {
    ...
    const rows = result.rows.map((r: any) =>
      `${r.message_id},${r.session_id},${r.role},${r.agent_id || ''},${r.tokens_in},${r.tokens_out},${r.duration_ms},${r.created_at},"${(r.content || '').replace(/"/g, '""').slice(0, 500)}"`
    );
...
export async function recordAgentPerformance(
  ...
  agentId: string,
  durationMs: number,
  tokens: { inputTokens?: number; outputTokens?: number; toolCalls?: number; stopReason?: string; isError?: boolean; sessionId?: string }
): Promise<void> {
  const schema = tenantSchema(tenantId);
  await ensureCopilotTables(schema);
  await safeQuery(
    `INSERT INTO "${schema}".agent_performance_log (tenant_id, agent_id, session_id, duration_ms, tokens_in, tokens_out, tool_calls, stop_reason, is_error)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
    [tenantId, agentId, tokens.sessionId || `auto-${crypto.randomUUID()}`, durationMs,
     tokens.inputTokens || 0, tokens.outputTokens || 0, tokens.toolCalls || 0,
     tokens.stopReason || 'unknown', tokens.isError || false]
  ).catch(() => null);
}
