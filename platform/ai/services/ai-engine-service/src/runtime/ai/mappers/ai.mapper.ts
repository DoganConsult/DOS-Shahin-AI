// @ts-nocheck
import type { AgentExecution, AgentExecutionCreateInput } from '@dos/types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): AgentExecution {
  return {
...
export function toEntityList(rows: GenericRow[]): AgentExecution[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): AgentExecutionCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as AgentExecutionCreateInput;
}

export function toApiResponse(entity: AgentExecution): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: AgentExecution[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
