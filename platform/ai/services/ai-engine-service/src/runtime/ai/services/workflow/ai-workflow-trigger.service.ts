...
export interface AITriggerConfig {
  ...
  module_triggers: Record<string, ModuleTriggerRule>;
  updated_at: string;
}

export interface ModuleTriggerRule {
  enabled: boolean;
...
export type TriggerModuleType =
  | 'risk' | 'compliance_gap' | 'incident'
  | 'vendor' | 'audit' | 'policy' | 'evidence'
  | 'governance' | 'bcp' | 'asset' | 'training'
  | 'qiyas' | 'ai-governance' | 'exception'
  | 'remediation' | 'action' | 'foundation'
  | 'reporting' | 'team' | 'notification';

interface TriggerContext {
  type: TriggerModuleType;
  entityId: string;
  score?: number;
  gapPercent?: number;
  severity?: string;
...
const TRIGGER_WORKFLOW_MAP: Record<string, string> = {
  risk: 'risk_treatment',
...
const DEFAULT_MODULE_TRIGGERS: Record<string, ModuleTriggerRule> = {
...
export async function getConfig(tenantId: string): Promise<AITriggerConfig | null> {
  const { safeQuery } = await import('@dos/db');
  const { getFirstRow } = await import('../../../../utils/db-utils');
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_trigger_config WHERE tenant_id = $1`,
    [tenantId]
  );
...
export async function upsertConfig(tenantId: string, config: Partial<AITriggerConfig>): Promise<AITriggerConfig> {
  const { safeQuery } = await import('@dos/db');
  const { getFirstRow } = await import('../../../../utils/db-utils');
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_trigger_config (tenant_id, enabled, risk_threshold, compliance_gap_threshold, incident_severity_threshold, module_triggers, updated_at)
...
export async function getModuleTriggerConfig(tenantId: string, moduleCode: string): Promise<ModuleTriggerRule | null> {
  const config = await getConfig(tenantId);
  if (!config) return DEFAULT_MODULE_TRIGGERS[moduleCode] ?? null;
  return config.module_triggers?.[moduleCode] ?? DEFAULT_MODULE_TRIGGERS[moduleCode] ?? null;
}

export async function upsertModuleTrigger(tenantId: string, moduleCode: string, rule: Partial<ModuleTriggerRule>): Promise<ModuleTriggerRule> {
  const config = await getConfig(tenantId);
  ...
  const merged: ModuleTriggerRule = { ...defaultRule, ...existing[moduleCode], ...rule } as ModuleTriggerRule;
...
): Promise<{ triggered: boolean; workflowInstanceId?: string; reason?: string }> {
  const config = await getConfig(tenantId);
...
function evaluateModuleTrigger(
  ...
  switch (moduleRule.threshold_operator) {
    case '>=': shouldTrigger = numericValue >= thresholdNumeric; break;
    case '>':  shouldTrigger = numericValue > thresholdNumeric; break;
    case '==': shouldTrigger = numericValue === thresholdNumeric; break;
    case '<=': shouldTrigger = numericValue <= thresholdNumeric; break;
    case '<':  shouldTrigger = numericValue < thresholdNumeric; break;
...
export function getDefaultModuleTriggers(): Record<string, ModuleTriggerRule> {
  return { ...DEFAULT_MODULE_TRIGGERS };
}

export function getSupportedTriggerModules(): string[] {
