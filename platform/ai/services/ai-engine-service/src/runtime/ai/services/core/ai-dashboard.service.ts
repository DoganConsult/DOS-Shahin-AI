...
export async function getAiDashboard(tenantId: string): Promise<AiDashboardSummary> {
  ...
  try {
    const [agents, runs, proposals, recommendations, tools, tokens, topAgents, failures] = await Promise.all([
      ...
      ).catch(() => ({ rows: [{ total: 0, active: 0, disabled: 0 }] })),
      safeQuery(
        `SELECT
           COUNT(*)::int AS total,
      ...
      ).catch(() => ({ rows: [{ total: 0, completed: 0, failed: 0, avg_ms: null }] })),
      safeQuery(
...
async function getTopAgentUsage(schema: string): Promise<AgentUsageItem[]> {
  const result = await safeQuery(
    `SELECT ar.agent_id, aa.agent_code,
...
async function getRecentFailures(schema: string): Promise<RecentFailureItem[]> {
  const result = await safeQuery(
    `SELECT ar.run_id, aa.agent_code, ar.module_code, ar.error_message, ar.started_at
     FROM "${schema}".ai_agent_runs ar
     LEFT JOIN "${schema}".ai_agents aa ON aa.agent_id = ar.agent_id
...
export async function getAiCostUsage(tenantId: string, days: number = 30): Promise<AiCostUsageResult> {
  const schema = tenantSchema(tenantId);
  try {
    const [totals, byAgent, byModel] = await Promise.all([
      safeQuery(
        ...
        [days],
      ).catch(() => ({ rows: [{ total_runs: 0, total_tokens: 0, total_cost: 0 }] })),
      safeQuery(
        `SELECT aa.agent_code, COUNT(*)::int AS runs, COALESCE(SUM(ar.tokens_used), 0)::int AS tokens
         FROM "${schema}".ai_agent_runs ar
         LEFT JOIN "${schema}".ai_agents aa ON aa.agent_id = ar.agent_id
      ...
      safeQuery(
        `SELECT model_id, COUNT(*)::int AS runs, COALESCE(SUM(tokens_used), 0)::int AS tokens
         FROM "${schema}".ai_agent_runs
    ...
    return {
      ...
      byAgent: byAgent.rows.map((r: GenericRow) => ({ agentCode: r.agent_code ?? '', runs: r.runs ?? 0, tokens: r.tokens ?? 0 })),
      byModel: byModel.rows.map((r: GenericRow) => ({ modelId: r.model_id ?? '', runs: r.runs ?? 0, tokens: r.tokens ?? 0 })),
    };
  } catch (err) {
...
export interface AiCostUsageResult {
  periodDays: number;
  totalRuns: number;
  totalTokens: number;
  totalCostUsd: number | null;
  byAgent: { agentCode: string; runs: number; tokens: number }[];
  byModel: { modelId: string; runs: number; tokens: number }[];
}
