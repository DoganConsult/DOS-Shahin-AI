 ...
 */
export const DEFAULT_EVENT_AGENT_BINDINGS: DefaultBinding[] = [
  {
 ...
 */
export async function seedDefaultEventAgentBindings(tenantId: string): Promise<number> {
  ...
  for (const b of DEFAULT_EVENT_AGENT_BINDINGS) {
    try {
      const exists = await safeQuery(
        `SELECT 1 FROM "${schema}".event_trigger_binding
         WHERE tenant_id = $1 AND event_type = $2 AND target_agent_id = $3
         LIMIT 1`,
        [tenantId, b.event_type, b.target_agent_id],
      );
      ...
      if (exists.rows.length === 0) {
        await safeQuery(
          ...
          [tenantId, b.event_type, b.target_agent_id, b.action_type, b.cooldown_seconds],
        );
        seeded++;
      }
    } catch {
