...
export async function evaluatePolicies(
  tenantId: string,
  context: { scope: string; targetId?: string; agentId?: string; actionType?: string; metadata?: Record<string, unknown> },
...
export async function createPolicyRule(
  tenantId: string,
  input: { ruleName: string; ruleType: string; targetScope?: string; targetId?: string; conditionJson?: Record<string, unknown>; actionOnMatch?: string; severity?: string; createdBy?: string },
...
export async function updatePolicyRule(
  tenantId: string,
  ruleId: string,
  updates: { ruleName?: string; ruleType?: string; targetScope?: string; targetId?: string; conditionJson?: Record<string, unknown>; actionOnMatch?: string; severity?: string },
): Promise<PolicyRule | null> {
  const schema = tenantSchema(tenantId);
...
export function evaluateRateLimitPolicy(rule: PolicyRule, _context: Record<string, unknown>): { allowed: boolean; reason: string } {
  const windowMs = (rule.condition_json as Record<string, unknown>)?.windowMs ?? 60000;
  const maxCount = (rule.condition_json as Record<string, unknown>)?.maxCount ?? 10;

  const allowed = enforceThrottle(`${rule.tenant_id}:${rule.rule_id}`, (windowMs as any), maxCount);
...
export async function getBlockedActionLog(tenantId: string, hours: number = 24): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
