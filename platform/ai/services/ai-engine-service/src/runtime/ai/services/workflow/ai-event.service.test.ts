...
import { emitAiEvent, emitAiStatusChange } from './ai-event.service';
...
describe('Ai Event Service', () => {
  ...
  describe('emitAiStatusChange', () => {
    it('should emit status_changed event', () => {
      emitAiStatusChange('t1', 'agent' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
  ...
  describe('helper functions', () => {
    it('should have convenience helpers defined', () => {
      expect(typeof emitAiEvent).toBe('function');
      expect(typeof emitAiStatusChange).toBe('function');
    });
  });
});
