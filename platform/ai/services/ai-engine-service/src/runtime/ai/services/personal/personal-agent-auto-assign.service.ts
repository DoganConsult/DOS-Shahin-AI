...
import type { PlatformMode } from '../../ports/platform.port';
import type { ActivationMode } from './personal-agent.types';
import {
  assignPersonalAgent,
 ...
 */
export async function autoAssignAgentsForTenantMode(
  tenantId: string,
  targetMode: PlatformMode,
  activatedBy: string
): Promise<number> {
  // Map platform mode to activation mode
  const activationModeMap: Record<PlatformMode, ActivationMode> = {
    'human': 'human',
    'hybrid': 'hyper',
    'shadow_agent': 'hyper',
  ...
  for (const user of users.rows) {
    try {
      ...
      if (existing) {
        ...
        await updatePersonalAgentAssignment(
          tenantId,
          user.user_id,
          existing.agentId,
          { activationMode },
