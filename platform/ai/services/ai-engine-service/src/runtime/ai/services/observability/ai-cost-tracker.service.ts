...
export async function recordUsage(
  ...
  try {
    await safeQuery(
      ...
      [tenantId, agentId, model, inputTokens, outputTokens, totalTokens, costUsd],
    );
...
export async function getTenantUsage(tenantId: string, daysBack = 30): Promise<UsageStats> {
  const schema = tenantSchema(tenantId);
  try {
...
export async function getAgentUsage(agentId: string, daysBack = 30): Promise<UsageStats> {
  try {
    const schemas = await safeQuery(
...
export async function checkBudget(
  tenantId: string,
): Promise<{ allowed: boolean; remainingTokens: number; remainingCostUsd: number; usagePct: number }> {
  const schema = tenantSchema(tenantId);
  try {
    const r = await safeQuery(
