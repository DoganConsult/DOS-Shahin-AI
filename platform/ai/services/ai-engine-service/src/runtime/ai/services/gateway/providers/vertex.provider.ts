...
export async function callVertexApi(
  ...
  try {
    ...
    const googleAuthModuleName: string = 'google-auth-library';
    const authMod: any = await import(googleAuthModuleName);
    ...
    if (parsed.error) throw new Error(parsed.error.message || 'Vertex API error');
    ...
    throw new Error(
      `[vertex] Provider unavailable (install google-auth-library and set GOOGLE_APPLICATION_CREDENTIALS): ${(err as Error).message}`,
    );
  }
}
