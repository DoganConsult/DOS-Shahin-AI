...
async function loadEnabledRules(): Promise<AlertRule[]> {
  const r = await safeQuery(
    `SELECT id, tenant_id, rule_name, condition_type, condition_config, severity, message_template, is_active
...
async function recentlyFired(ruleId: string, throttleMinutes: number): Promise<boolean> {
  const r = await safeQuery(
    `SELECT 1
      ...
      LIMIT 1`,
    [ruleId, throttleMinutes],
  ).catch(() => ({ rows: [] as any[] }));
  return r.rows.length > 0;
}

async function recordAlert(rule: AlertRule, breach: Breach): Promise<void> {
  const firedAt = new Date().toISOString();
  await safeQuery(
    `INSERT INTO public.ai_activity_alerts
  ...
  ).catch((err: any) => logger.warn('[alerts] insert failed', { error: err?.message }));
...
async function evalCostCapPctAbove(rule: AlertRule): Promise<Breach[]> {
...
async function evalHitlBacklogAbove(rule: AlertRule): Promise<Breach[]> {
...
async function evalAgentFailureRateAbove(rule: AlertRule): Promise<Breach[]> {
...
async function evalSoDViolationsAbove(rule: AlertRule): Promise<Breach[]> {
  ...
  if (rule.tenant_id) params.push(rule.tenant_id);
  const r = await safeQuery(
    `SELECT tenant_id, COUNT(*)::int AS n
      ...
      WHERE module = 'ai'
        ...
        ${tenantClause}
      GROUP BY tenant_id
     HAVING COUNT(*) >= $${rule.tenant_id ? 3 : 2}`,
...
async function evalDenyRateAbove(rule: AlertRule): Promise<Breach[]> {
  ...
  if (total < minDecisions) return [];
  const denyPct = (stats.denied / total) * 100;
  if (denyPct < thresholdPct) return [];
  return [{
...
async function evalShiftsStalled(rule: AlertRule): Promise<Breach[]> {
  ...
  const r = await safeQuery(
    `SELECT count(*)::int AS n FROM public.ai_employee_reports
      WHERE filed_at >= NOW() - ($1 || ' minutes')::interval`,
    [windowMinutes],
  ).catch(() => ({ rows: [{ n: 0 }] }));
  const n = Number(r.rows[0]?.n ?? 0);
  if (n >= minExpected) return [];
  return [{
    tenantId: null,
...
async function evaluateRule(rule: AlertRule): Promise<Breach[]> {
...
async function tick(): Promise<void> {
  ...
  try {
    ...
    for (const rule of rules) {
      ...
      if (await recentlyFired(rule.id, throttleMin)) continue;
...
export function startAlertRulesRunner(): void {
...
export function stopAlertRulesRunner(): void {
