...
import { lookupShiftHandler, listRegisteredShiftHandlers, type ShiftKpiSnapshot } from './shift-handlers';
...
const SHAHIN_AI_EMPLOYEES = RAW_EMPLOYEES as unknown as Record<string, AgentEmployeeRecord>;
// Lazy-required at use site to avoid cron parser as a hard boot dep.
...
async function listActiveTenants(): Promise<string[]> {
  const r = await safeQuery(
    `SELECT tenant_id FROM dos.tenants WHERE status = 'active' ORDER BY tenant_id`,
    [],
  ).catch(() => ({ rows: [] as TenantRow[] }));
  return (r.rows as TenantRow[]).map(t => t.tenant_id);
}
...
let _cronParser: { parseExpression: (s: string, opts?: any) => { next: () => { toDate: () => Date } } } | null = null;
try { _cronParser = requireCjs('cron-parser'); }
catch (err) {
...
function nextRunFromCron(cronExpr: string, fromMs = Date.now(), tz = 'UTC'): Date {
  if (!_cronParser) {
...
async function tenantTimezone(tenantId: string | null): Promise<string> {
  if (!tenantId) return 'UTC';
  const r = await safeQuery(
    `SELECT settings -> 'timezone' AS tz FROM dos.tenants WHERE tenant_id = $1`,
    [tenantId],
  ).catch(() => ({ rows: [] as any[] }));
...
async function nextRunForShift(shift: { cron_expr: string; tenant_id: string | null; local_to_tenant: boolean }, fromMs = Date.now()): Promise<Date> {
 ...
 */
export async function seedShiftsForTenant(tenantId: string): Promise<{ inserted: number; updated: number; skipped?: string }> {
  if (!tenantId || tenantId.length < 3) {
    return { inserted: 0, updated: 0, skipped: 'invalid_tenant_id' };
  }
  const exists = await safeQuery(
    `SELECT 1 FROM dos.tenants WHERE tenant_id = $1 AND status = 'active' LIMIT 1`,
    [tenantId],
  ).catch(() => ({ rows: [] as any[] }));
  ...
  for (const { agentId, shift } of allShifts) {
    if (!shift.perTenant) continue;     // global shifts already covered by seedShiftsForAllTenants
    const tz = shift.localToTenant ? await tenantTimezone(tenantId) : 'UTC';
    const next = nextRunFromCron(shift.cron, Date.now(), tz);
    const r = await safeQuery(
      `INSERT INTO public.ai_employee_shifts
      ...
      [tenantId, agentId, shift.code, shift.cron, shift.localToTenant, shift.produces, next],
    ).catch((err: any) => { logger.warn('[ai-hr] tenant shift upsert failed', { tenantId, agentId, shiftCode: shift.code, error: err?.message }); return { rows: [] as any[] }; });
    if (r.rows[0]?.was_inserted) inserted++; else if (r.rows.length > 0) updated++;
  }
  logger.info('[ai-hr] tenant shifts seeded', { tenantId, inserted, updated });
  return { inserted, updated };
}

export async function seedShiftsForAllTenants(): Promise<{ inserted: number; updated: number; tenants: number }> {
    ...
    const r = await safeQuery(
      `INSERT INTO public.ai_employee_shifts
       ...
       ON CONFLICT (tenant_id, agent_id, shift_code) DO UPDATE SET
         ...
         updated_at  = NOW()
       RETURNING (xmax = 0) AS was_inserted`,
      [agentId, shift.code, shift.cron, shift.localToTenant, shift.produces, next],
    ).catch((err: any) => { logger.warn('[ai-hr] global shift upsert failed', { agentId, shiftCode: shift.code, error: err?.message }); return { rows: [] as any[] }; });
    if (r.rows[0]?.was_inserted) inserted++; else if (r.rows.length > 0) updated++;
  }
  ...
  for (const tenantId of tenants) {
    for (const { agentId, shift } of allShifts) {
      if (!shift.perTenant) continue;  // already seeded above
      ...
      const r = await safeQuery(
        `INSERT INTO public.ai_employee_shifts
         ...
         ON CONFLICT (tenant_id, agent_id, shift_code) DO UPDATE SET
           ...
           updated_at      = NOW()
         RETURNING (xmax = 0) AS was_inserted`,
        [tenantParam, agentId, shift.code, shift.cron, shift.localToTenant, shift.produces, shift.perTenant, next],
      ).catch((err: any) => { logger.warn('[ai-hr] shift upsert failed', { agentId, shiftCode: shift.code, error: err?.message }); return { rows: [] as any[] }; });
      if (r.rows[0]?.was_inserted) inserted++; else if (r.rows.length > 0) updated++;
    }
  }
...
// ── Read-side composer for the org-chart page ─────────────────────────
...
export interface EmployeeView {
  ...
  kpisTotal: number;
  // SB-1: surface "is this agent doing real domain work" so the org-chart
  // page can show an honest "Active / Phase 1 standby" badge per card.
  hasRealHandler: boolean;
  shiftsActive: number;
  // Nested for detail view
  deliverables?: AgentEmployeeRecord['deliverables'];
  kpis?: AgentEmployeeRecord['kpis'];
  schedule?: AgentEmployeeRecord['schedule'];
  recentReports?: Array<{ filedAt: string; title: string; summary: string | null; status: string; deliverableCode: string }>;
}

interface AgentDefSlim { id: string; name: string; nameAr: string; domain: string; domainAr: string; color: string; icon: string }

async function loadAgentDefs(): Promise<Record<string, AgentDefSlim>> {
  const mod = await import('@shahin-ai/product/agrc-agents');
  const out: Record<string, AgentDefSlim> = {};
  for (const a of mod.AGRC_AGENTS) out[a.id] = a as AgentDefSlim;
  return out;
}

async function loadShiftSummary(tenantId: string | null, agentId?: string): Promise<Record<string, { count: number; lastRunAt: string | null }>> {
  ...
  const r = await safeQuery(
    `SELECT agent_id, COUNT(*)::int AS n, MAX(last_run_at) AS last_run
       FROM public.ai_employee_shifts
      WHERE ${tenantClause} AND enabled = TRUE ${agentClause}
      GROUP BY agent_id`,
    params,
  ).catch(() => ({ rows: [] as any[] }));
  const out: Record<string, { count: number; lastRunAt: string | null }> = {};
  for (const row of r.rows as Array<{ agent_id: string; n: number; last_run: string | null }>) {
    out[row.agent_id] = { count: row.n, lastRunAt: row.last_run };
  }
  return out;
}

async function loadLatestKpis(tenantId: string | null): Promise<Record<string, { onTarget: number; offTarget: number; unevaluated: number; tracked: number }>> {
  ...
  const out: Record<string, { onTarget: number; offTarget: number; unevaluated: number; tracked: number }> = {};
  for (const row of r.rows as Array<{ agent_id: string; on_target: number; off_target: number; unknown: number }>) {
...
async function loadReportSummary(tenantId: string | null): Promise<Record<string, { last7d: number; pending: number }>> {
  const tenantClause = tenantId ? '(tenant_id = $1 OR tenant_id IS NULL)' : 'TRUE';
  const params: unknown[] = tenantId ? [tenantId] : [];
  const r = await safeQuery(
    `SELECT agent_id,
  ...
  const out: Record<string, { last7d: number; pending: number }> = {};
  for (const row of r.rows as Array<{ agent_id: string; last7d: number; pending: number }>) {
    out[row.agent_id] = { last7d: row.last7d, pending: row.pending };
  }
  return out;
}

async function loadRecentReports(tenantId: string | null, agentId: string, limit = 10) {
  const tenantClause = tenantId ? '(tenant_id = $1 OR tenant_id IS NULL)' : 'TRUE';
  const params: unknown[] = tenantId ? [tenantId, agentId, limit] : [agentId, limit];
  const placeholderAgent = tenantId ? '$2' : '$1';
  const placeholderLimit = tenantId ? '$3' : '$2';
  const r = await safeQuery(
    `SELECT filed_at, title, summary, status, deliverable_code
       FROM public.ai_employee_reports
  ...
  return (r.rows as Array<any>).map((row) => ({
...
/**
 * Org-chart aggregated by manager role — returns the count of direct
 * reports + open pending reports per manager. Powers the manager filter
 * pill on the HR page so a Compliance Manager can see "I have 6 staff,
 * 4 reports filed this week, 0 pending".
 */
export interface ManagerSummary {
  managerRoleCode: string;
  managerLabel: string;
  managerLabelAr: string;
  directReports: string[];     // agent IDs that report to this manager
  reportsLast7d: number;
  pendingReports: number;
}

export async function listManagers(tenantId: string | null): Promise<ManagerSummary[]> {
...
export async function listEmployees(tenantId: string | null, managerRoleFilter?: string): Promise<EmployeeView[]> {
  ...
  for (const [agentId, emp] of Object.entries(SHAHIN_AI_EMPLOYEES)) {
    if (managerRoleFilter && emp.managerRoleCode !== managerRoleFilter) continue;
...
export async function getEmployee(tenantId: string | null, agentId: string): Promise<EmployeeView | null> {
...
// ── Agent profile: timeline + KPI trend ──────────────────────────────

export interface AgentTimelineItem {
  ts: string;
  kind: 'shift_completed' | 'report_filed' | 'manager_action' | 'kpi_snapshot';
  title: string;
 ...
 */
export async function getAgentTimeline(tenantId: string | null, agentId: string, limit = 50): Promise<AgentTimelineItem[]> {
  const tenantClause = tenantId ? '(tenant_id = $1 OR tenant_id IS NULL)' : 'TRUE';
  const params: unknown[] = tenantId ? [tenantId, agentId, limit] : [agentId, limit];
  const placeholderAgent = tenantId ? '$2' : '$1';
  const placeholderLimit = tenantId ? '$3' : '$2';

  const reportsR = await safeQuery(
    `SELECT filed_at AS ts, 'report_filed'::text AS kind, title, summary, status, deliverable_code, shift_code
       FROM public.ai_employee_reports
      WHERE ${tenantClause} AND agent_id = ${placeholderAgent}
      ORDER BY filed_at DESC LIMIT ${placeholderLimit}`,
    params,
  ).catch(() => ({ rows: [] as any[] }));

  const auditR = await safeQuery(
    `SELECT created_at AS ts,
      ...
      ORDER BY created_at DESC LIMIT ${placeholderAgent === '$2' ? '$5' : '$4'}`,
    [...params, `agent:${agentId}`, limit],
  ).catch(() => ({ rows: [] as any[] }));

  const kpiR = await safeQuery(
    `SELECT computed_at AS ts, 'kpi_snapshot'::text AS kind, kpi_code, value_numeric, value_text, target_met
       FROM public.ai_employee_kpi_snapshots
      WHERE ${tenantClause} AND agent_id = ${placeholderAgent}
      ORDER BY computed_at DESC LIMIT ${placeholderLimit}`,
    params,
  ).catch(() => ({ rows: [] as any[] }));

  const items: AgentTimelineItem[] = [];
  for (const r of reportsR.rows as any[]) items.push({
  ...
  for (const r of auditR.rows as any[]) items.push({
    ts: r.ts,
    kind: r.kind as AgentTimelineItem['kind'],
    title: String(r.action ?? '').replace(/^agent\.|^manager\./, ''),
    detail: { action: r.action, actor: r.actor_id, payload: r.payload },
  });
  for (const r of kpiR.rows as any[]) items.push({
...
export async function getAgentKpiTrend(tenantId: string | null, agentId: string, days = 30): Promise<Record<string, KpiTrendPoint[]>> {
  const tenantClause = tenantId ? '(tenant_id = $1 OR tenant_id IS NULL)' : 'TRUE';
  const params: unknown[] = tenantId ? [tenantId, agentId, days] : [agentId, days];
  const placeholderAgent = tenantId ? '$2' : '$1';
  const placeholderDays  = tenantId ? '$3' : '$2';
  const r = await safeQuery(
    `SELECT kpi_code, computed_for_date::text AS d, value_numeric, value_text, target_met
       FROM public.ai_employee_kpi_snapshots
      WHERE ${tenantClause} AND agent_id = ${placeholderAgent}
        AND computed_for_date >= CURRENT_DATE - (${placeholderDays} || ' days')::interval
      ORDER BY kpi_code, computed_for_date`,
    params,
  ).catch(() => ({ rows: [] as any[] }));

  const out: Record<string, KpiTrendPoint[]> = {};
  for (const row of r.rows as any[]) {
    if (!out[row.kpi_code]) out[row.kpi_code] = [];
...
export interface ManagerInboxRow {
  reportId: string;
...
export async function listManagerInbox(
  ...
  filters: { status?: 'filed' | 'acknowledged' | 'actioned' | 'all'; limit?: number; offset?: number; isSuperAdmin?: boolean } = {},
): Promise<ManagerInboxRow[]> {
  ...
  if (isSuper) {
    ...
    for (const [agentId, emp] of Object.entries(SHAHIN_AI_EMPLOYEES)) {
      if (callerRoleCodes.includes(emp.managerRoleCode)) managedAgents.push(agentId);
    }
  }
  ...
  const r = await safeQuery(
    `SELECT report_id, tenant_id, agent_id, shift_code, deliverable_code, title, summary,
...
async function writeInboxAudit(reportId: string, action: string, actorId: string, prevStatus: string, newStatus: string, tenantId: string | null, agentId: string): Promise<void> {
  await safeQuery(
  ...
  ).catch((err: any) => logger.warn('[ai-hr] inbox audit failed', { reportId, action, error: err?.message }));
}

export async function acknowledgeReport(reportId: string, ackBy: string): Promise<boolean> {
  const r = await safeQuery(
    `UPDATE public.ai_employee_reports
        SET status = 'acknowledged', acknowledged_by = $2, acknowledged_at = NOW()
      WHERE report_id = $1 AND status = 'filed'
      RETURNING report_id, tenant_id, agent_id`,
    [reportId, ackBy],
  ).catch(() => ({ rows: [] as any[] }));
...
export async function actionReport(reportId: string, actionBy: string): Promise<boolean> {
  const r = await safeQuery(
    `UPDATE public.ai_employee_reports
      ...
      RETURNING report_id, tenant_id, agent_id, status`,
    [reportId, actionBy],
  ).catch(() => ({ rows: [] as any[] }));
  if ((r.rows?.length ?? 0) === 0) return false;
  const row = r.rows[0];
 ...
 * shifts.
 */
async function executeShift(shift: ShiftRow): Promise<{ title: string; summary: string; body: Record<string, unknown>; kpiSnapshots?: ShiftKpiSnapshot[] }> {
...
async function writeKpiSnapshots(tenantId: string | null, agentId: string, snapshots: ShiftKpiSnapshot[] | undefined): Promise<void> {
  if (!snapshots || snapshots.length === 0) return;
  for (const s of snapshots) {
    try {
      await safeQuery(
        `INSERT INTO public.ai_employee_kpi_snapshots
         ...
         ON CONFLICT (tenant_id, agent_id, kpi_code, computed_for_date) DO UPDATE SET
           ...
           computed_at   = NOW()`,
        [tenantId, agentId, s.kpiCode, s.valueNumeric ?? null, s.valueText ?? null, s.targetMet ?? null],
      );
    } catch (err: any) {
...
export async function runDueShifts(now = new Date()): Promise<{ executed: number; failed: number }> {
  const r = await safeQuery(
    `SELECT shift_id, tenant_id, agent_id, shift_code, cron_expr, local_to_tenant, produces, per_tenant
       FROM public.ai_employee_shifts
      WHERE enabled = TRUE
  ...
  let traceSurfaceCall: typeof import('../../../../domain/agrc-engine/observability/langfuse-bridge')['traceSurfaceCall'] | null = null;
  try {
  ...
  const runShift = async (shift: ShiftRow): Promise<{ title: string; summary: string; body: Record<string, unknown>; kpiSnapshots?: ShiftKpiSnapshot[] }> => {
    const result = await executeShift(shift);
    return result;
  };

  for (const shift of r.rows as ShiftRow[]) {
    try {
      ...
      await safeQuery(
        `INSERT INTO public.ai_employee_reports
           (tenant_id, agent_id, shift_code, deliverable_code, title, summary, body, destination)
         VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb, $8)`,
        [shift.tenant_id, shift.agent_id, shift.shift_code, shift.produces, result.title, result.summary, JSON.stringify(result.body), destination],
      );
      ...
      ).catch((err: any) => logger.warn('[ai-hr] audit trail insert failed', { error: err?.message }));
      // Advance next_run_at — honor tenant timezone if local_to_tenant.
      const next = await nextRunForShift(shift, now.getTime() + 1000);
      await safeQuery(
        `UPDATE public.ai_employee_shifts SET last_run_at = $1, next_run_at = $2, updated_at = NOW() WHERE shift_id = $3`,
        [now, next, shift.shift_id],
      );
      executed++;
    } catch (err: any) {
