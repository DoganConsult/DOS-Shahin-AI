...
export async function getCockpitSnapshot(tenantId: string): Promise<CockpitSnapshot> {
  const schema = tenantSchema(tenantId);
  const mode = await getTenantPlatformMode(tenantId);

  // Agent health
  const agentHealth: AgentHealthSignal[] = [];
  ...
  for (const agent of agentCatalog) {
    try {
      ...
      const errors = runs.rows.filter((r: GenericRow) => r.status === 'failed').length;
      const avgMs = runs.rows.length > 0
        ? Math.round(runs.rows.reduce((s: number, r: Record<string, unknown>) => (s as any) + (r.duration_ms || 0), 0) / runs.rows.length)
        : 0;
