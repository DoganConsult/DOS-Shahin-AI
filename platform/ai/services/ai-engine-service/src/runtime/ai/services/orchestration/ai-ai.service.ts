...
export function isAiAiActionAllowed(action: string): boolean {
  return (AI_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isAiAiActionBlocked(action: string): boolean {
  return (AI_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresAiHumanApproval(action: string): boolean {
  return (AI_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_invocations WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'AI entity not found.', keyPoints: [], confidence: 0 };
  return {
