...
export async function expireStaleMemories(tenantId: string): Promise<number> {
...
export async function mergeDuplicateMemories(tenantId: string): Promise<number> {
  const schema = tenantSchema(tenantId);
  let merged = 0;

  try {
    const dupes = await safeQuery(
      `SELECT namespace, content, COUNT(*)::int AS cnt, ARRAY_AGG(memory_id) AS ids
    ...
    for (const dupe of dupes.rows) {
      ...
      if (idsToDelete.length > 0) {
        await safeQuery(
          `UPDATE "${schema}".agent_memories SET is_deleted = TRUE WHERE memory_id = ANY($1)`,
          [idsToDelete],
        );
...
export async function compactOldMemories(
  tenantId: string,
  ...
  try {
    const namespaces = await safeQuery(
      `SELECT DISTINCT namespace FROM "${schema}".agent_memories
       WHERE tenant_id = $1 AND is_deleted = FALSE
    ...
    for (const ns of namespaces.rows) {
      const memories = await safeQuery(
        `SELECT memory_id, content, importance_score FROM "${schema}".agent_memories
         WHERE tenant_id = $1 AND namespace = $2 AND is_deleted = FALSE AND summary IS NULL
         ORDER BY created_at ASC LIMIT $3`,
        [tenantId, ns.namespace, batchSize],
      );

      if (memories.rows.length < 3) continue;

      const contents = memories.rows.map((m: GenericRow) => m.content).join('\n---\n');
      ...
      try {
        ...
        const ids = memories.rows.map((m: GenericRow) => m.memory_id);
        const keepId = ids[0];
        ...
        await safeQuery(
          `UPDATE "${schema}".agent_memories
           ...
           WHERE memory_id = $3`,
          [summaryText, 'Compacted from ' + ids.length + ' entries', keepId],
        );
...
export async function runCompaction(tenantId: string): Promise<CompactionResult> {
...
export async function getMemoryHealth(tenantId: string): Promise<{
  totalMemories: number;
  activeMemories: number;
  expiringSoon: number;
  oldUncompacted: number;
  duplicates: number;
}> {
  const schema = tenantSchema(tenantId);
  ...
  try {
    ...
    const dupeResult = await safeQuery(
