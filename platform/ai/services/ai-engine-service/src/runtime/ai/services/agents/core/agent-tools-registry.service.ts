...
import { registerAgentTools, getToolsForAgent as getToolsForAgentSync, AgentToolDefinition } from './agent-tool-executor.service';
...
const BUILTIN_BUILDERS: Record<string, () => AgentToolDefinition[]> = {
 ...
 */
export function initAgentToolsRegistry(): void {
  ...
  for (const agentId of catalogIds) {
    ...
    tools.push({
        ...
        handler: async (tenantId: string, input: Record<string, unknown>, opts?: any) => {
      ...
      tools.push({
        ...
        handler: async (tenantId: string, input: Record<string, unknown>) => {
          ...
          try {
            ...
            const matchedTool = tools.find((t: any) => t.name === serverName || t.name === queryType);
            if (matchedTool) {
          ...
          if (serverName === 'nist-cve') {
            ...
            try {
              ...
              if (res.ok) {
                ...
                const vulns = (data.vulnerabilities || []).slice(0, 5).map((v: any) => ({
...
export async function getToolsForAgent(_tenantId: string, agentId: string): Promise<any[]> {
