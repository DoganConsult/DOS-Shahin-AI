...
import type { AgentRunResult, AgentRunOpts } from './agent-runner.types';
...
export type { AgentAction, AgentRunResult, AgentRunOpts } from './agent-runner.types';
...
const _ksCache = new Map<string, { at: number; active: boolean; reason: string }>();
const _ksTtlMs = 5_000;
async function _isKillSwitchActive(tenantId: string, agentId: string, schema: string): Promise<{ active: boolean; reason: string }> {
  const key = `${tenantId}:${agentId}`;
  const cached = _ksCache.get(key);
  if (cached && Date.now() - cached.at < _ksTtlMs) return { active: cached.active, reason: cached.reason };
  try {
...
} from '../../orchestration/agent-action-executor.service';
...
export async function runAgent(
  ...
  const executionRes = await safeQuery(
    `INSERT INTO "${schema}".ai_agent_executions (agent_code, user_id, status, input_payload)
     VALUES ($1, $2, 'running', $3::jsonb) RETURNING execution_id`,
    [agentId, null, JSON.stringify({ query: opts.query ?? null, callerAgentId: opts.callerAgentId ?? null })],
  ).catch(() => ({ rows: [] as any[] }));
  const executionId = (executionRes.rows?.[0]?.execution_id as string | undefined) ?? null;
  ...
  try {
    ...
    if (killed.active) {
      ...
      if (executionId) {
        await safeQuery(
          `UPDATE "${schema}".ai_agent_executions
           SET status = 'completed', completed_at = NOW(), output_result = $1::jsonb, error_message = $2
           WHERE execution_id = $3`,
          [JSON.stringify(result), `kill_switch:${killed.reason}`, executionId],
        ).catch(() => undefined);
      }
      return result;
    }
    ...
    const agentCtxBuilder = (CONTEXT_BUILDERS as any)[agentId];
    ...
    let agentDef: any = loadAgentDef(agentId) || { id: agentId, name: agentId };
    try {
      ...
      if (dbDef) {
        agentDef = {
          ...
          id: dbDef.agentCode,
          name: dbDef.name,
          model: dbDef.model ?? (agentDef as any).model,
          temperature: dbDef.temperature ?? (agentDef as any).temperature,
          promptName: dbDef.promptName,
          datasetName: dbDef.datasetName,
          capabilities: dbDef.capabilities ?? (agentDef as any).capabilities,
    ...
    const [priorMemories, ragContext] = await Promise.all([
      ...
      ragEnabled && opts.query
        ? import('../../gateway/rag-pipeline.service')
            .then((m) => m.buildRAGContext(tenantId, agentId, opts.query as string, 3))
    ...
    const memoryBlock = (priorMemories || []).length
      ? `\n=== PRIOR MEMORY (top ${priorMemories.length}) ===\n${
          priorMemories
            .map((m: any, i: number) => `[${i + 1}] (${m.memoryType ?? 'memory'}) ${m.content ?? m.summary ?? ''}`)
            .join('\n')
        }\n=== END MEMORY ===\n`
      : '';

    const ragBlock =
      ragContext && (ragContext as any).documents?.length
        ? await import('../../gateway/rag-pipeline.service').then((m) => m.formatRAGForPrompt(ragContext as any))
        : '';

    const enrichedContext = {
      agent: { id: agentId, name: (agentDef as any).name || agentId },
      ...
      retrievalStats: {
        ...
        ragDocuments: (ragContext as any)?.documents?.length || 0,
        ragTokenEstimate: (ragContext as any)?.tokenEstimate || 0,
      },
    };
    ...
    if (memoryEnabled && (toolResult as any).finalText) {
      import('../../memory/memory-store.service')
        .then((m) =>
          m.commitMemories({
            tenantId,
            ...
            summary: String((toolResult as any).finalText).slice(0, 2048),
            facts: [
              {
                kind: 'task',
                text: String((toolResult as any).finalText).slice(0, 4096),
                importance: 0.5,
                metadata: {
                  toolCalls: (toolResult as any).totalToolCalls,
                  stopReason: (toolResult as any).stopReason,
                },
              },
            ],
          }),
    ...
    if (process.env.AI_AUTO_EVAL_ENABLED === 'true' && (toolResult as any).finalText) {
      import('../lifecycle/agent-eval.service')
        .then((m) =>
          m.evaluateOutput(
            tenantId,
            agentId,
            String(opts.query || `agent ${agentId} autonomous run`).slice(0, 1000),
            String((toolResult as any).finalText).slice(0, 2000),
            'quality',
    ...
    const actionsExecuted = (toolResult.toolResults || []).filter((t: any) => !t.isError).length;
    ...
    const result: AgentRunResult = {
      ...
      costUsd: typeof (toolResult as any)?.costUsd === 'number' ? (toolResult as any).costUsd : 0,
    ...
    if (executionId) {
      await safeQuery(
        `UPDATE "${schema}".ai_agent_executions
         SET status = 'completed', completed_at = NOW(), output_result = $1::jsonb
         WHERE execution_id = $2`,
        [JSON.stringify({ ...toolResult, summary }), executionId],
      ).catch(() => undefined);
    }
    ...
    } as any)), { tenantId, operation: 'eventBus:ai.agent.completed' });
    ...
    } as any)), { tenantId, operation: 'eventBus:ai.agent.failed' });
