...
import {
  ...
  ClaudeToolDef,
  ToolMessage,
  ToolUseCall as _ToolUseCall,
  ClaudeToolResponse,
} from '../../../ports/ai.port';
import { gatewayWithTools } from "../../gateway/ai-gateway.service";
...
import { recordAgentPerformance } from '../../../ports/platform.port';
...
// ----------------------------------------------------------------
// Tool Definition & Registry Types
// ----------------------------------------------------------------

/** Full tool definition with executable handler */
export interface AgentToolDefinition {
  ...
  /** Handler receives (tenantId, validated input, opts) → returns any result */
  handler: (tenantId: string, input: Record<string, unknown>, opts?: Record<string, any>) => Promise<unknown>;
}

/** Result from a single tool execution step */
...
export interface AgentRunResult {
  ...
  toolResults: ToolStepResult[];
  finalText: string;
  totalInputTokens: number;
  totalOutputTokens: number;
  durationMs: number;
  stopReason: string;
  discoveries: Array<{ type: string; payload: Record<string, unknown> }>;
}
...
export function registerAgentTools(agentId: string, tools: AgentToolDefinition[]): void {
  toolRegistry.set(agentId, tools);
}

/** Get registered tools for an agent */
export function getToolsForAgent(agentId: string): AgentToolDefinition[] {
  return toolRegistry.get(agentId) || [];
}

/** Get Anthropic-formatted tool definitions (no handler) for API call */
function getClaudeToolDefs(agentId: string): ClaudeToolDef[] {
...
function findHandler(agentId: string, toolName: string): AgentToolDefinition["handler"] | null {
  const tools = getToolsForAgent(agentId);
...
export async function runAgentWithTools(
  ...
  opts?: {
    ...
    signal?: AbortSignal;
    /** Wave 2: forwarded to Langfuse trace.userId for attribution. */
    userId?: string;
    /** Wave 2: multi-turn session grouping in Langfuse. */
    sessionId?: string;
  },
): Promise<AgentRunResult> {
  const startMs = Date.now();
  const maxSteps = opts?.maxSteps || 5;
  const toolResultsRecord: ToolStepResult[] = [];
  ...
  const agentDef: any = loadAgentDef(agentId) || { name: agentId };
  ...
  try {
    ...
    if (resolved?.body) {
      ...
      if (typeof context.priorMemoryBlock === 'string' && context.priorMemoryBlock) ctxParts.push(context.priorMemoryBlock);
      if (typeof context.ragContextBlock === 'string' && context.ragContextBlock) ctxParts.push(context.ragContextBlock);
    ...
    if (resolved?.model && !agentDef.model) agentDef.model = resolved.model;
    if (resolved?.temperature != null && agentDef.temperature == null) agentDef.temperature = resolved.temperature;
  ...
  try {
    ...
    if (lfTrace && opts?.sessionId) {
      try { lfTrace.update({ sessionId: opts.sessionId }); } catch { /* ignore */ }
    }
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (!costCheck.allowed) {
      ...
      try {
        const m: any = await import('../../../../audit/services/audit/core/audit-trail.service.js');
    ...
    if (lfTrace) {
      try {
        ...
        await lf.logGeneration({
          ...
          model: (response as any).model || process.env.CLAUDE_MODEL || 'claude-sonnet-4',
    ...
    if (response.stopReason === 'tool_use' && response.toolCalls && response.toolCalls.length > 0) {
      ...
      for (const tCall of response.toolCalls) {
        ...
        const gate = await enforceToolGate({
          ...
          approvalBoundary: agentMeta?.approvalBoundary as any,
          toolRiskTier: (tCall as any)?.riskTier || agentMeta?.approvalBoundary as any,
          maxActionsPerCycle: agentMeta?.maxActionsPerCycle as any,
        ...
        if (!gate.allow) {
          ...
          try {
            ...
            if (dType) discoveries.push({ type: dType, payload: resultData });
  ...
  try {
    await safeQuery(
      `INSERT INTO public.agent_performance_log
...
function buildAgentSystemPrompt(
  ...
  if (agentDef.knowledge?.length) {
    parts.push(
      `\n## Knowledge Domains\n${agentDef.knowledge.map((k: string) => `- ${k}`).join("\n")}`
    );
  }
  ...
  if (context && typeof context === "object") {
    for (const [key, value] of Object.entries(context)) {
      if (key === '_agentMemory' || key === '_pendingHandoffs') continue;
      if (key === 'priorMemoryBlock' || key === 'ragContextBlock' || key === 'retrievalStats') continue;
...
function buildAutonomousTrigger(agentId: string, context: Record<string, unknown>): string {
  ...
  if ((context as any).pendingHandoffs > 0) urgencyCues.push(`${context.pendingHandoffs} inter-agent handoffs pending.`);
  if ((context as any).highRisks > 0) urgencyCues.push(`${context.highRisks} high-severity items need attention.`);
  if ((context as any).overdueItems > 0) urgencyCues.push(`${context.overdueItems} items are overdue.`);
