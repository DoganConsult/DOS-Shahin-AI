 ...
 */
export async function appendDraft(input: AppendDraftInput): Promise<{ draftId: string | null; turnIdx: number }> {
  if (!input.tenantId || !input.sessionId || !input.agentCode) return { draftId: null, turnIdx: 0 };
  try {
    const schema = tenantSchema(input.tenantId);
    // Fetch the next turn index for this session, atomically.
    const r = await safeQuery(
      `SELECT COALESCE(MAX(turn_idx) + 1, 0) AS next_idx
    ...
    // Demote prior latest.
    await safeQuery(
      `UPDATE "${schema}".ai_drafts
          SET is_latest = FALSE, updated_at = NOW()
        WHERE tenant_id = $1 AND session_id = $2 AND is_latest = TRUE`,
      [input.tenantId, input.sessionId],
    );
    ...
    logger.warn(`[conversation-memory] appendDraft failed: ${(err as Error).message}`);
...
export async function loadRecentTurns(
  ...
  try {
    ...
    logger.warn(`[conversation-memory] loadRecentTurns failed: ${(err as Error).message}`);
    return [];
  }
}
