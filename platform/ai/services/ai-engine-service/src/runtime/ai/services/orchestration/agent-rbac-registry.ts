...
export async function getRegisteredAgentIds(tenantId: string): Promise<string[]> {
...
export async function isAgentAuthorized(tenantId: string, agentId: string, action: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT 1 FROM "${schema}".ai_agent_permissions
