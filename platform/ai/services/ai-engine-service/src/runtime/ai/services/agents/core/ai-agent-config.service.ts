...
export type AgentOperatingState = 'on' | 'off' | 'trial' | 'maintenance';
...
const _configCache = new Map<string, { config: AgentConfig; ts: number }>();
 ...
 */
export async function getAgentConfig(tenantId: string, agentId: string): Promise<AgentConfig | null> {
  ...
  try {
    ...
    const result = await safeQuery(
      `SELECT * FROM "${schema}".ai_agent_configs WHERE tenant_id = $1 AND agent_id = $2`,
      [tenantId, agentId],
    );
    const row = getFirstRow(result) as GenericRow | undefined;
    if (!row) return null;
 ...
 */
export async function getEnabledAgents(tenantId: string): Promise<AgentConfig[]> {
  try {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(
      `SELECT * FROM "${schema}".ai_agent_configs
 ...
 */
export async function canAgentExecute(tenantId: string, agentId: string): Promise<{ allowed: boolean; reason?: string }> {
  const config = await getAgentConfig(tenantId, agentId);
  if (!config) return { allowed: true }; // No config = use defaults (backwards compat)

  if (!config.enabled) return { allowed: false, reason: 'Agent disabled by admin' };
  if (config.operatingState === 'off') return { allowed: false, reason: 'Agent state: off' };
  if (config.operatingState === 'maintenance') return { allowed: false, reason: 'Agent in maintenance mode' };
 ...
 */
export async function getCircuitBreakerConfig(tenantId: string, agentId: string): Promise<{
...
export async function upsertAgentConfig(
  tenantId: string,
  agentId: string,
  updates: Partial<Omit<AgentConfig, 'configId' | 'agentId'>>,
  createdBy?: string,
): Promise<string | null> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `INSERT INTO "${schema}".ai_agent_configs
...
export async function setAgentOperatingState(
  tenantId: string,
  agentId: string,
...
export function invalidateAgentConfigCache(tenantId?: string, agentId?: string): void {
  if (tenantId && agentId) {
    _configCache.delete(`${tenantId}:${agentId}`);
  } else if (tenantId) {
...
function mapRowToConfig(row: Record<string, unknown>): AgentConfig {
  return {
    ...
    systemPromptKey: row.system_prompt_key as string | null,
    toolsEnabled: (row.tools_enabled as string[]) ?? [],
    maxActionsPerRun: row.max_actions_per_run as number,
    enabled: row.enabled as boolean,
    operatingState: row.operating_state as AgentConfig['operatingState'],
    trialExpiresAt: row.trial_expires_at ? new Date(row.trial_expires_at as string) : null,
    priority: row.priority as number,
    cbFailureThreshold: row.cb_failure_threshold as number,
    cbSuccessThreshold: row.cb_success_threshold as number,
    cbTimeoutMs: row.cb_timeout_ms as number,
    cbWindowMs: row.cb_window_ms as number,
