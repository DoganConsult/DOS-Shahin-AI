 ...
 */
export function computeExecutionWaves(): ExecutionWave[] {
  ...
  while (completed.size < allAgents.length) {
    waveNum++;
    const ready = allAgents.filter(a =>
      !completed.has(a) &&
...
export async function persistCycleSummary(tenantId: string): Promise<void> {
  ...
  try {
    ...
    await safeQuery(
      `INSERT INTO "${schema}".agent_cycle_summaries
    ...
    for (const h of ctx.handoffs) {
      await safeQuery(
        `INSERT INTO "${schema}".agent_handoffs (id, from_agent, to_agent, handoff_type, priority, status, payload, created_at, completed_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
         ON CONFLICT (id) DO UPDATE SET status = EXCLUDED.status, completed_at = EXCLUDED.completed_at`,
        [h.id, h.fromAgent, h.toAgent, h.handoffType, h.priority, h.status,
    ...
    for (const d of ctx.discoveries) {
      await safeQuery(
        `INSERT INTO "${schema}".agent_discoveries (id, agent_id, discovery_type, title, severity, entity_type, entity_id, details, created_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
        ...
        [d.id, d.agentId, d.type, d.title, d.severity, d.entityType, d.entityId, JSON.stringify(d.details), d.timestamp]
      );
    }
    ...
    for (const c of ctx.correlations) {
      await safeQuery(
        ...
        [c.id, c.agents, c.sharedEntity, c.severity, JSON.stringify(c.findings)]
      );
    }
  } catch (err: unknown) {
