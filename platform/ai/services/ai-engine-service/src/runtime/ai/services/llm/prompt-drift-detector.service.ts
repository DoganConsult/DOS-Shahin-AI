...
async function getAgentIdsForPrompt(schema: string, promptVersionId: string): Promise<string[]> {
  const result = await safeQuery(
    `SELECT inv.asset_key
  ...
  return (result.rows as { asset_key: string }[]).map((r) => r.asset_key);
}

// ── Compute drift metrics from recent decision_record rows ──────────────────

export async function computeDriftMetrics(
  tenantId: string,
  promptVersionId: string,
  ...
  const result = await safeQuery(
    `SELECT confidence, outcome, decision_type, created_at
    ...
    [tenantId, ...agentIds, days],
  );
  const rows = result.rows as { confidence: number | null; outcome: Record<string, unknown>; decision_type: string }[];
  ...
  for (const r of rows) {
    ...
    if (r.outcome && (r.outcome as { status?: string }).status === 'rejected') rejectCount++;
  ...
  for (const [k, v] of Object.entries(actionCounts)) actionDistribution[k] = v / total;
...
export async function getBaseline(tenantId: string, promptVersionId: string): Promise<BaselineRow | null> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT baseline_id, prompt_version_id, avg_confidence, rejection_rate,
  ...
  return (getFirstRow(result) as BaselineRow) || null;
}

// ── Flag prompt for review in ai_prompt_registry ────────────────────────────

export async function flagPromptForReview(
  tenantId: string,
  promptVersionId: string,
  reason: string,
): Promise<void> {
  const schema = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${schema}".ai_prompt_registry
     SET flagged_for_review = TRUE, flagged_at = NOW(), flag_reason = $1, updated_at = NOW()
     WHERE prompt_version_id = $2`,
    [reason, promptVersionId],
  );
}

// ── Clear flag (e.g. after human review) ────────────────────────────────────

export async function clearPromptFlag(tenantId: string, promptVersionId: string): Promise<void> {
  const schema = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${schema}".ai_prompt_registry
...
export async function detectDrift(
  ...
  promptVersionId: string,
  options?: { recentDays?: number },
): Promise<{ drifted: boolean; reason?: string; current?: DriftMetrics; baseline?: BaselineRow }> {
  ...
  if (!baseline) return { drifted: false, current }; // no baseline to compare
...
export async function runDriftDetectionForPrompt(
  tenantId: string,
  promptVersionId: string,
): Promise<boolean> {
  const result = await detectDrift(tenantId, promptVersionId);
  if (!result.drifted || !result.reason) return false;
...
export async function listActivePromptVersions(tenantId: string): Promise<{ prompt_version_id: string }[]> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT DISTINCT prompt_version_id FROM "${schema}".prompt_drift_baselines`,
    [],
  );
  return result.rows as { prompt_version_id: string }[];
}
