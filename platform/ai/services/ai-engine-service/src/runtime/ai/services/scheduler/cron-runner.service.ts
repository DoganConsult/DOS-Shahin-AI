...
function fieldMatches(field: string | undefined, current: number, min: number, max: number): boolean {
...
function ruleMatches(rule: any, now: Date, lastFiredAt: Date | null): boolean {
  ...
  if (Number.isFinite(rule.atUtcHour) && Number.isFinite(rule.atUtcMinute)) {
    const sameMinute =
      now.getUTCHours() === Number(rule.atUtcHour) &&
...
async function loadEnabledTriggers(): Promise<TriggerRow[]> {
  const res = await safeQuery(
    `SELECT trigger_id, tenant_id, module, signal, rule, workflow_code, enabled
       FROM dos.ai_workflow_triggers
      WHERE enabled = TRUE
        AND module = 'ai'
        AND (rule ? 'cron' OR rule ? 'everyMinutes' OR rule ? 'atUtcHour')`,
    [],
  ).catch(() => ({ rows: [] as any[] }));
  return (res.rows || []) as TriggerRow[];
}

async function loadLastFiredAt(triggerId: string): Promise<Date | null> {
  const res = await safeQuery(
    `SELECT MAX(emitted_at) AS last_fired_at FROM dos.ai_workflow_trigger_log WHERE trigger_id = $1`,
    [triggerId],
  ).catch(() => ({ rows: [] as any[] }));
...
async function recordFire(
  ...
  await safeQuery(
    `INSERT INTO dos.ai_workflow_trigger_log (trigger_id, tenant_id, module, signal, payload, emitted_at)
     VALUES ($1, $2, $3, $4, $5::jsonb, NOW())`,
    [triggerId, tenantId, module, signal, JSON.stringify({ status, ...details })],
  ).catch(() => undefined);
}

async function fireTrigger(trigger: TriggerRow): Promise<void> {
  const agentId =
    (trigger.rule?.agentId as string) ||
    (trigger.workflow_code as string) ||
    (trigger.signal as string) ||
    'A01';
  const query = (trigger.rule?.query as string) || `Scheduled run: ${trigger.signal || trigger.workflow_code || 'autonomous'}`;
  ...
  try {
    ...
    await recordFire(trigger.trigger_id, trigger.tenant_id, trigger.module, trigger.signal, 'fired', {
      agentId,
      actionsProposed: (result as any).actionsProposed,
...
async function tick(): Promise<void> {
  ...
  try {
    ...
    for (const trigger of triggers) {
      ...
      if (!ruleMatches(trigger.rule, now, lastFiredAt)) continue;

      // de-dup: don't fire the same minute twice
      if (lastFiredAt && now.getTime() - lastFiredAt.getTime() < 55_000) continue;
