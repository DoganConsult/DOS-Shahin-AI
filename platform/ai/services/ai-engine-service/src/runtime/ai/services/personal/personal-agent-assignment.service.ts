...
import type { PersonalAgentAssignment, ActivationMode } from './personal-agent.types';
 ...
 */
export async function assignPersonalAgent(
  tenantId: string,
  userId: string,
  agentId: string,
  config: {
...
): Promise<PersonalAgentAssignment> {
  ...
  await safeQuery(
    `INSERT INTO "${schema}".personal_agent_assignments
 ...
 */
export async function getPersonalAgentAssignment(
  tenantId: string,
  userId: string,
  agentId?: string
): Promise<PersonalAgentAssignment | null> {
  const schema = tenantSchema(tenantId);
 ...
 */
export async function updatePersonalAgentAssignment(
  tenantId: string,
  userId: string,
  agentId: string,
  updates: Partial<PersonalAgentAssignment>,
  updatedBy: string
): Promise<PersonalAgentAssignment> {
  const schema = tenantSchema(tenantId);
...
export async function getUserRoles(tenantId: string, userId: string): Promise<string[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT role FROM "${schema}".users WHERE user_id = $1`,
      [userId]
    );
...
export async function getUserPermissions(tenantId: string, userId: string): Promise<string[]> {
  ...
  try {
    ...
    for (const r of rows) permissions.add(r.permission_code);
    if (permissions.size > 0) return Array.from(permissions);
  ...
  try {
    ...
    if (roles.length > 0) {
      ...
      for (const r of rows) {
        const allowedRoles: string[] = r.allowed_roles || [];
...
export function mapAssignmentRow(row: Record<string, unknown>): PersonalAgentAssignment {
  return {
    assignmentId: row.assignment_id as string,
    tenantId: row.tenant_id as string,
    userId: row.user_id as string,
    agentId: row.agent_id as string,
    agentNameEn: row.agent_name_en as string | undefined,
    agentNameAr: row.agent_name_ar as string | undefined,
    inheritedRoles: typeof row.inherited_roles === 'string' ? JSON.parse(row.inherited_roles) : row.inherited_roles as string[],
    inheritedPermissions: typeof row.inherited_permissions === 'string' ? JSON.parse(row.inherited_permissions) : row.inherited_permissions as string[],
    activationMode: row.activation_mode as PersonalAgentAssignment['activationMode'],
    slaBasedActivation: row.sla_based_activation as boolean,
    slaThresholdHours: row.sla_threshold_hours as number | undefined,
    slaPriorityFilter: row.sla_priority_filter ? (typeof row.sla_priority_filter === 'string' ? JSON.parse(row.sla_priority_filter) : row.sla_priority_filter as string[]) : undefined,
    companyPolicyRules: typeof row.company_policy_rules === 'string' ? JSON.parse(row.company_policy_rules) : row.company_policy_rules as Record<string, unknown>,
    processGovernanceRules: typeof row.process_governance_rules === 'string' ? JSON.parse(row.process_governance_rules) : row.process_governance_rules as Record<string, unknown>,
    allowedActionTypes: typeof row.allowed_action_types === 'string' ? JSON.parse(row.allowed_action_types) : row.allowed_action_types as string[],
    blockedActionTypes: typeof row.blocked_action_types === 'string' ? JSON.parse(row.blocked_action_types) : row.blocked_action_types as string[],
    requiresApprovalFor: typeof row.requires_approval_for === 'string' ? JSON.parse(row.requires_approval_for) : row.requires_approval_for as string[],
    autoApproveBelowRisk: row.auto_approve_below_risk as string,
    isActive: row.is_active as boolean,
    isEnabled: row.is_enabled as boolean,
    lastActivityAt: row.last_activity_at as string | undefined,
    totalActionsExecuted: (row.total_actions_executed as number) || 0,
    totalActionsApproved: (row.total_actions_approved as number) || 0,
    totalActionsRejected: (row.total_actions_rejected as number) || 0,
    userConsentGranted: row.user_consent_granted as boolean,
    consentGrantedAt: row.consent_granted_at as string | undefined,
    consentPurpose: row.consent_purpose as string | undefined,
