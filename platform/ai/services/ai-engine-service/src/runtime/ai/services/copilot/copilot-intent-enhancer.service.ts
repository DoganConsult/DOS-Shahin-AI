...
export interface EnhancedIntent {
  ...
  contextualActions: Array<{ action: string; description: string; endpoint: string }>;
}

// GRC domain intent taxonomy
const INTENT_TAXONOMY: Record<string, { module: string; keywords: string[]; actions: string[] }> = {
 ...
 */
export async function classifyIntent(userInput: string, tenantId?: string): Promise<string> {
  ...
  if (tenantId) {
    try {
      ...
      return (result as Record<string, unknown>).intent || bestMatch || 'dashboard_query';
    } catch { /* fallback */ }
  }
 ...
 */
export async function enhanceIntent(tenantId: string, userInput: string): Promise<EnhancedIntent> {
...
Respond with JSON: {
  ...
  const e = enhanced as Record<string, unknown>;
  return {
