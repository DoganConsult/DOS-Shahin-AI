...
import { AGENT_MONITORING_CONFIGS, type ProactiveTrigger } from '../../../../governance-os/services/misc/proactive-signal-evaluators.service';
...
const monitoringState = new Map<string, {
  lastPoll: number;
  lastTrigger: number;
 ...
 */
export async function monitorTenant(tenantId: string): Promise<{
  triggers: ProactiveTrigger[];
  agentsTriggered: string[];
}> {
  const triggers: ProactiveTrigger[] = [];
  const agentsTriggered = new Set<string>();
    ...
    try {
      ...
      for (const trigger of agentTriggers) {
        ...
        await swallow(EC.EVENT_BUS, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:agent.proactive_trigger' });
 ...
 */
export async function startProactiveMonitoring(): Promise<void> {
  logger.info('[ProactiveMonitor] Starting proactive monitoring service');

  // Get all active tenants
  const getActiveTenants = async (): Promise<Array<{ tenant_id: string }>> => {
    try {
      const result = await safeQuery(
        `SELECT tenant_id FROM tenants WHERE deleted_at IS NULL AND status = 'active'`
      );
      return result.rows;
    } catch {
      return [];
    ...
    try {
      ...
      for (const tenant of tenants) {
        try {
          ...
          if (triggers.length > 0) {
            logger.info(
              `[ProactiveMonitor] Tenant ${tenant.tenant_id}: ${triggers.length} triggers, ` +
 ...
 */
export async function triggerMonitoringForTenant(tenantId: string): Promise<ProactiveTrigger[]> {
