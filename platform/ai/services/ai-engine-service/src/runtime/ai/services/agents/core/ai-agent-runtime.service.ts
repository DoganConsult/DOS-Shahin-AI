...
const agentCircuits = new Map<string, AgentCircuitState>();
const AGENT_CB_FAILURE_THRESHOLD = 3;
const AGENT_CB_RECOVERY_MS = 120_000;

function getCircuit(key: string): AgentCircuitState {
...
export function getAgentCircuitState(tenantId: string, agentId: string): AgentCircuitState {
  return { ...getCircuit(`${tenantId}:${agentId}`) };
}

export function checkAgentCircuit(tenantId: string, agentId: string): { allowed: boolean; reason: string; waitMs?: number } {
  ...
  if (cb.state === 'half_open') cb.probeCount++;
  return { allowed: true, reason: 'OK' };
}

export function recordAgentCircuitSuccess(tenantId: string, agentId: string): void {
...
export function recordAgentCircuitFailure(tenantId: string, agentId: string): void {
  const cb = getCircuit(`${tenantId}:${agentId}`);
  cb.failures++;
...
export function resetAgentCircuit(tenantId: string, agentId: string): void {
  agentCircuits.delete(`${tenantId}:${agentId}`);
}

export async function getAgentRuntimeConfig(tenantId: string, agentId: string): Promise<AgentRuntimeConfig | null> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT * FROM "${schema}".agent_runtime_config WHERE tenant_id = $1 AND agent_id = $2`,
      [tenantId, agentId],
    );
...
export async function isAgentEnabled(tenantId: string, agentId: string): Promise<boolean> {
  ...
  if ((config as any)?.paused_at) return false;
  return true;
}

export async function isAgentInCooldown(tenantId: string, agentId: string): Promise<{ inCooldown: boolean; remainingMs: number }> {
  const config = await getAgentRuntimeConfig(tenantId, agentId);
  const cooldownSec = config?.cooldown_seconds ?? 0;
  if (cooldownSec <= 0) return { inCooldown: false, remainingMs: 0 };
  ...
  try {
    ...
    if (!lastCompleted) return { inCooldown: false, remainingMs: 0 };
...
export async function setAgentEnabled(tenantId: string, agentId: string, enabled: boolean, updatedBy?: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    await safeQuery(
      `INSERT INTO "${schema}".agent_runtime_config (tenant_id, agent_id, enabled, updated_by, updated_at)
       VALUES ($1, $2, $3, $4, NOW())
       ON CONFLICT (tenant_id, agent_id) DO UPDATE SET enabled = $3, updated_by = $4, updated_at = NOW()`,
      [tenantId, agentId, enabled, updatedBy || SYSTEM_JOB_ACTOR],
    );
    eventBus.publish(({ eventType: enabled ? 'ai.agent.enabled' : 'ai.agent.disabled', tenantId, sourceService: 'ai-agent-runtime', severity: 'info', payload: { agentId, updatedBy } } as any));
    return true;
  } catch {
    return false;
  }
...
export async function updateRuntimeConfig(
  tenantId: string,
  agentId: string,
  updates: { enabled?: boolean; maxRetries?: number; retryDelayMs?: number; cooldownSeconds?: number; escalationOnFailure?: string; stuckThresholdMs?: number },
  updatedBy?: string,
): Promise<AgentRuntimeConfig | null> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `INSERT INTO "${schema}".agent_runtime_config
...
export async function getRetryPolicy(tenantId: string, agentId: string): Promise<{ maxRetries: number; retryDelayMs: number }> {
  const config = await getAgentRuntimeConfig(tenantId, agentId);
...
export async function retryWithBackoff<T>(
  fn: () => Promise<T>,
  opts: { maxRetries: number; baseDelayMs: number; tenantId: string; agentId: string },
): Promise<T> {
  let attempt = 0;
  while (true) {
    try {
...
export async function detectStuckRuns(tenantId: string): Promise<Array<{ run_id: string; agent_id: string; started_at: string; duration_ms: number }>> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT r.run_id, r.agent_id, r.created_at AS started_at,
...
export async function cancelStuckRun(tenantId: string, runId: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `UPDATE "${schema}".agent_runs SET status = 'cancelled', completed_at = NOW()
...
export async function escalateFailedRun(tenantId: string, runId: string, agentId: string): Promise<void> {
  const config = await getAgentRuntimeConfig(tenantId, agentId);
  const escalation = config?.escalation_on_failure || 'notify';

  eventBus.publish(({ eventType: 'ai.run.escalated', tenantId, sourceService: 'ai-agent-runtime', severity: 'warning', payload: { runId, agentId, escalationType: escalation } } as any));
...
export async function getAgentRunStats(tenantId: string, agentId: string, hours: number = 24): Promise<{
  totalRuns: number; succeeded: number; failed: number; avgDurationMs: number; lastRunAt: string | null;
}> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
...
export async function listAllRuntimeConfigs(tenantId: string): Promise<AgentRuntimeConfig[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT * FROM "${schema}".agent_runtime_config WHERE tenant_id = $1 ORDER BY agent_id`,
      [tenantId],
    );
    return result.rows;
  } catch {
...
export async function setAgentDisableReason(tenantId: string, agentId: string, reason: string, disabledBy: string): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  try {
    await safeQuery(
      `INSERT INTO "${schema}".agent_runtime_config (tenant_id, agent_id, enabled, updated_by, updated_at)
      ...
      [tenantId, agentId, disabledBy, `disabled:${reason}`],
    );
    eventBus.publish(({ eventType: 'ai.agent.disabled', tenantId, sourceService: 'ai-agent-runtime', severity: 'warning', payload: { agentId, reason, disabledBy } } as any));
    return true;
  } catch {
    return false;
  }
...
export async function getRuntimeHealthSummary(tenantId: string): Promise<{
  totalAgents: number; enabledAgents: number; disabledAgents: number;
  circuitOpenCount: number; stuckRunCount: number;
  failedRunsLast24h: number; avgDurationMsLast24h: number;
}> {
  const schema = tenantSchema(tenantId);
  try {
    ...
    for (const [, state] of agentCircuits) {
      if (state.state === 'open') circuitOpenCount++;
    }
...
export async function validateRuntimeConfigIntegrity(tenantId: string): Promise<{ valid: boolean; issues: string[] }> {
  const configs = await listAllRuntimeConfigs(tenantId);
  const issues: string[] = [];
  for (const c of configs) {
    if (c.max_retries < 0 || c.max_retries > 20) issues.push(`${c.agent_id}: max_retries out of range (0-20)`);
    if (c.retry_delay_ms < 100 || c.retry_delay_ms > 60000) issues.push(`${c.agent_id}: retry_delay_ms out of range (100-60000)`);
    if (c.cooldown_seconds < 0 || c.cooldown_seconds > 86400) issues.push(`${c.agent_id}: cooldown_seconds out of range (0-86400)`);
    if (c.stuck_threshold_ms < 10000 || c.stuck_threshold_ms > 3600000) issues.push(`${c.agent_id}: stuck_threshold_ms out of range (10s-1h)`);
