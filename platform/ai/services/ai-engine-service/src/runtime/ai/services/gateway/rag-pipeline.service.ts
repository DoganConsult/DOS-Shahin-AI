...
async function searchContextSources(
  tenantId: string,
  agentId: string,
  query: string,
  topK: number = 5,
  ...
  try {
    const res = await safeQuery(
      `SELECT source_id, source_type, source_ref, title, content
        ...
        LIMIT $3`,
      [agentId, query.slice(0, 200), topK],
    );
    return (res.rows || []).map((row: GenericRow, idx: number) => ({
