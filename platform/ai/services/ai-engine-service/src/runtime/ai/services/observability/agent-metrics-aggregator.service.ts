...
export interface CooperationMetrics {
  ...
  topHandoffPairs: { from: string; to: string; count: number }[];
...
export interface FleetHealthSnapshot {
  ...
  fleetScore: number;
  fleetStatus: 'optimal' | 'degraded' | 'critical' | 'idle';
}

// ── Per-Agent Performance ───────────────────────────────────────────────────

export async function getAgentPerformanceSummaries(tenantId: string, hours: number = 24): Promise<AgentPerformanceSummary[]> {
  ...
  try {
    ...
    for (const row of res.rows) {
      let topActions: { type: string; count: number }[] = [];
      try {
        const actionRes = await safeQuery(
          `SELECT tool_name AS type, COUNT(*)::int AS cnt
           FROM agent_performance
           WHERE tenant_id = $1 AND agent_id = $2 AND executed_at >= $3
           GROUP BY tool_name ORDER BY cnt DESC LIMIT 5`,
          [tenantId, row.agent_id, cutoff]
        );
        topActions = actionRes.rows.map((a: GenericRow) => ({ type: a.type, count: a.cnt }));
      } catch { /* non-critical */ }
...
export async function getCooperationMetrics(tenantId: string, hours: number = 24): Promise<CooperationMetrics> {
  const schema = tenantSchema(tenantId);
  const cutoff = new Date(Date.now() - hours * 3600_000).toISOString();
  const defaults: CooperationMetrics = {
  ...
  try {
    ...
    const pairRes = await safeQuery(
      `SELECT from_agent, to_agent, COUNT(*)::int AS cnt
       FROM "${schema}".agent_handoffs
       WHERE created_at >= $1
    ...
    const discoveryRes = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${schema}".agent_discoveries WHERE created_at >= $1`,
      [cutoff]
    );

    const correlationRes = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${schema}".agent_correlations WHERE created_at >= $1`,
      [cutoff]
    );
    ...
    return {
      ...
      topHandoffPairs: pairRes.rows.map((p: GenericRow) => ({ from: p.from_agent, to: p.to_agent, count: p.cnt })),
...
export async function getFleetHealth(tenantId: string, hours: number = 24): Promise<FleetHealthSnapshot> {
  ...
  let fleetStatus: FleetHealthSnapshot['fleetStatus'] = 'optimal';
  if (agents.length === 0) fleetStatus = 'idle';
  else if (fleetScore < 40 || cbStats.state === 'OPEN') fleetStatus = 'critical';
  else if (fleetScore < 70) fleetStatus = 'degraded';
...
export async function saveFleetSnapshot(tenantId: string): Promise<void> {
  const schema = tenantSchema(tenantId);
  ...
  try {
    ...
    await safeQuery(
      `INSERT INTO "${schema}".agent_fleet_snapshots
...
export async function getFleetSnapshotHistory(tenantId: string, limit: number = 30): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  try {
    const res = await safeQuery(
      `SELECT * FROM "${schema}".agent_fleet_snapshots ORDER BY created_at DESC LIMIT $1`,
      [limit]
    );
    return res.rows;
  } catch {
...
export async function getAgentTrends(tenantId: string, days: number = 7): Promise<unknown> {
  ...
  try {
    ...
    const trends: Record<string, { day: string; runs: number; successes: number; avgMs: number }[]> = {};
    for (const row of res.rows) {
      if (!trends[row.agent_id]) trends[row.agent_id] = [];
