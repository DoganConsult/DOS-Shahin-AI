...
export interface DocumentAnalysisResult {
  ...
  source: 'db_content' | 'db_metadata' | 'not_found';
}

export async function analyzeDocumentWithRAG(tenantId: string, documentId: string, query: string): Promise<DocumentAnalysisResult> {
  ...
  const evidenceResult = await safeQuery(
    `SELECT title, description, file_name, file_type, content_text, status, expires_at, created_at
     FROM "${schema}".evidence WHERE evidence_id = $1 LIMIT 1`,
    [documentId]
  ).catch(() => ({ rows: [] }));
  ...
  if (!docContent) {
    const policyResult = await safeQuery(
      `SELECT title, content, description, status, review_date, version, created_at
       FROM "${schema}".policies WHERE policy_id = $1 LIMIT 1`,
      [documentId]
    ).catch(() => ({ rows: [] }));
  ...
  if (!docContent) {
    const findingResult = await safeQuery(
      `SELECT title, description, details, severity, status, created_at
       FROM "${schema}".findings WHERE finding_id = $1 LIMIT 1`,
      [documentId]
    ).catch(() => ({ rows: [] }));
  ...
  try {
    ...
    const lines = docContent.split('\n').filter(line =>
      lowerQuery.split(/\s+/).some(word => word.length > 3 && line.toLowerCase().includes(word))
    );
