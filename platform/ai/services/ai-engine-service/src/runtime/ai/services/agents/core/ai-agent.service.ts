...
export async function assessRisk(tenantId: string, riskId: string): Promise<Record<string, unknown>> {
  ...
  const risk = (getFirstRow(riskResult) as GenericRow) || {
    risk_id: riskId,
    title: `Risk ${riskId}`,
    category: 'general',
    likelihood: 2,
  ...
  try {
    const ai = await gatewayJSON<Record<string, unknown>>({
      tenantId,
...
function assessRiskFallback(risk: GenericRow): Record<string, unknown> {
...
export async function analyzeComplianceGap(tenantId: string, frameworkId: string): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);

  // Get all registry controls for framework
  const registryControls = await safeQuery(
    `SELECT node_id, code, title_en, title_ar, priority, evidence_types
  ...
  const implementedNodes = new Set<string>();
  ...
  const gaps = registryControls.rows.filter((rc: any) => !implementedNodes.has(rc.node_id));
  ...
  try {
    const topGapSummary = prioritizedGaps.slice(0, 10).map((g: Record<string, unknown>) => `${g.code}: ${g.titleEn} (${g.priority})`).join('\n');
    const aiResult = await gatewayJSON<{ recommendations: string[]; quickWinStrategy: string }>({
...
Respond with JSON: { "recommendations": ["..."], "quickWinStrategy": "..." }`,
  ...
  return {
    ...
    quickWins: prioritizedGaps.filter((g: Record<string, unknown>) => g.priority === 'low').slice(0, 5),
    criticalGaps: prioritizedGaps.filter((g: Record<string, unknown>) => g.priority === 'high').slice(0, 10),
    aiRecommendations,
...
export async function generatePolicy(tenantId: string, params: {
  frameworkId: string;
  policyType: string;
  ...
  const tenantResult = await safeQuery(
    `SELECT org_name, industry FROM tenants WHERE tenant_id = $1`, [tenantId]
  );
  const tenant = getFirstRow(tenantResult) || { org_name: 'Organization', industry: 'general' };

  const fwResult = await safeQuery(
    `SELECT name_en, name_ar FROM instruments WHERE instrument_id = $1`, [params.frameworkId]
  );
  const framework = getFirstRow(fwResult) || { name_en: params.frameworkId, name_ar: params.frameworkId };

  const agentDef = loadAgentDef("A04") as Record<string, unknown> | null;
  ...
  try {
    const aiResult = await gatewayJSON<{
      ...
      systemPrompt: (agentDef?.systemPrompt as string) || "You are a KSA GRC policy drafting expert.",
...
function generatePolicyFallback(tenant: GenericRow, framework: GenericRow, params: Record<string, unknown>): Record<string, unknown> {
  const templates: Record<string, { title: string; sections: string[] }> = {
  ...
  const template = templates[params.policyType as any] || templates['information_security'];
  const content = template.sections.map((s: string) => `## ${s}\n\n[Content to be developed based on ${framework.name_en} requirements]\n`).join('\n');
...
export async function prepareAudit(tenantId: string, frameworkId: string): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);

  // Get controls for framework
  const controls = await safeQuery(
    `SELECT control_id, title, status, test_status, last_tested_at, evidence_ids, mapped_registry_nodes
     FROM "${schema}".controls
  ...
  const checklist = controls.rows.map((c: GenericRow) => {
    const issues: string[] = [];
  ...
  const readyCount = checklist.filter((c: GenericRow) => c.auditReady).length;
  const totalCount = checklist.length;

  // AI-enhanced audit readiness assessment
  let aiInsights: { overallReadiness: string; criticalFindings: string[]; actionPlan: string[] } | null = null;
  try {
    const summaryData = {
      readyPercent: totalCount > 0 ? Math.round((readyCount / totalCount) * 100) : 0,
      totalControls: totalCount,
      readyControls: readyCount,
      missingEvidence: checklist.filter((c: GenericRow) => c.issues.includes('Missing evidence')).length,
      failedTests: checklist.filter((c: GenericRow) => c.issues.includes('Control not tested or failed')).length,
      overdueTests: checklist.filter((c: GenericRow) => c.issues.includes('Test overdue (>30 days)')).length,
      neverTested: checklist.filter((c: GenericRow) => c.issues.includes('Never tested')).length,
    };

    aiInsights = await gatewayJSON<{ overallReadiness: string; criticalFindings: string[]; actionPlan: string[] }>({
...
Respond with JSON: { "overallReadiness": "ready|needs_work|not_ready", "criticalFindings": ["..."], "actionPlan": ["step1", "step2", ...] }`,
  ...
  return {
    ...
    controlsNeedingAttention: checklist.filter((c: GenericRow) => !c.auditReady),
    checklist,
    aiInsights,
    recommendations: [
      readyCount < totalCount ? `${totalCount - readyCount} controls need attention before audit` : 'All controls are audit-ready',
      checklist.some((c: GenericRow) => c.issues.includes('Missing evidence'))
        ? 'Collect missing evidence for flagged controls' : null,
      checklist.some((c: GenericRow) => c.issues.includes('Never tested'))
        ? 'Schedule testing for untested controls' : null,
      checklist.some((c: GenericRow) => c.issues.includes('Test overdue (>30 days)'))
...
export async function triageIncident(tenantId: string, incidentId: string): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);
  let incident: GenericRow | null = null;

  const tryTables = ['incidents', 'risk_incidents', 'security_incidents'];
  for (const table of tryTables) {
    const res = await safeQuery(
      `SELECT incident_id, title, category, severity, affected_controls
       FROM "${schema}".${table}
       WHERE incident_id = $1
       LIMIT 1`,
      [incidentId],
    );
    incident = (getFirstRow(res) as GenericRow) || null;
    if (incident) break;
  }
...
function triageIncidentFallback(incident: GenericRow): Record<string, unknown> {
...
export async function analyzeRegulatoryChange(tenantId: string, instrumentId: string): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);

  const nodesRes = await safeQuery(
    `SELECT node_id, code, title_en
     FROM instrument_structure
     WHERE instrument_id = $1 AND level >= 4
     ORDER BY sort_order, code`,
    [instrumentId],
  );
  const nodeIds = nodesRes.rows.map((r: any) => r.node_id).filter(Boolean);

  const controlsRes = await safeQuery(
    `SELECT control_id, title, status, test_status, mapped_registry_nodes
     FROM "${schema}".controls
     WHERE mapped_registry_nodes && $1::uuid[]`,
    [nodeIds],
  );

  const mapped = new Set<string>();
  const implemented = new Set<string>();
  for (const c of controlsRes.rows as any[]) {
    for (const n of (c.mapped_registry_nodes || [])) mapped.add(n);
    if (c.status === 'implemented' || c.test_status === 'passed') {
      for (const n of (c.mapped_registry_nodes || [])) implemented.add(n);
    }
  }
...
export async function getProactiveInsights(tenantId: string): Promise<Record<string, unknown>[]> {
  ...
  try {
    const expiring = await safeQuery(
      `SELECT evidence_id, title, expiry_date FROM "${schema}".evidence
       WHERE expiry_date IS NOT NULL AND expiry_date <= $1 AND expiry_date >= $2
       ORDER BY expiry_date ASC LIMIT 10`,
      [in7Days.toISOString(), now.toISOString()]
    );
  ...
  } catch (e) { logger.debug('[AgentInsights] query skipped:', e); }

  // Overdue remediation tasks
  try {
    const overdue = await safeQuery(
      `SELECT task_id, title, due_date FROM "${schema}".remediation_tasks
  ...
  // High-risk items
  try {
    const highRisks = await safeQuery(
      `SELECT risk_id, title, risk_score FROM "${schema}".risks
  ...
  try {
    const upcoming = await safeQuery(
      `SELECT assessment_id, title, updated_at FROM "${schema}".assessments
  ...
  try {
    const unhealthyConnectors = await safeQuery(
      `SELECT connector_id, source_system_type, failure_count, last_success_at
  ...
  try {
    ...
    if (snapshots.rows.length >= 3) {
      const scores = snapshots.rows.map((r: GenericRow) => parseFloat(r.compliance_score)).filter((v: number) => !isNaN(v));
      if (scores.length >= 3) {
        const recent = scores.slice(0, 3).reduce((s: number, v: number) => s + v, 0) / 3;
        const older = scores.slice(-3).reduce((s: number, v: number) => s + v, 0) / Math.min(3, scores.length);
        if (recent < older - 5) {
          insights.push({
  ...
  try {
    const overdueApprovals = await safeQuery(
      `SELECT approval_id, approver_id, step_id, sla_deadline
...
export async function autoClassifyRisk(riskData: { title?: string; description?: string; category?: string }, tenantId: string): Promise<{
  suggestedCategory: string; suggestedLikelihood: number; suggestedImpact: number;
}> {
  try {
    return await gatewayJSON<{ suggestedCategory: string; suggestedLikelihood: number; suggestedImpact: number }>({
...
function autoClassifyRiskFallback(riskData: { title?: string; description?: string; category?: string }): {
  suggestedCategory: string; suggestedLikelihood: number; suggestedImpact: number;
} {
  ...
  if (text.includes('cyber') || text.includes('security') || text.includes('hack') || text.includes('breach')) {
    category = 'cybersecurity'; impact = 4;
  } else if (text.includes('data') || text.includes('privacy') || text.includes('personal')) {
    category = 'data_privacy'; impact = 4;
  } else if (text.includes('financial') || text.includes('fraud') || text.includes('money')) {
    category = 'financial'; impact = 4; likelihood = 2;
  } else if (text.includes('compliance') || text.includes('regulatory') || text.includes('legal')) {
    category = 'compliance'; impact = 4;
  } else if (text.includes('vendor') || text.includes('third') || text.includes('supplier')) {
    category = 'third_party'; likelihood = 3;
  }
  if (text.includes('critical') || text.includes('severe') || text.includes('major')) { impact = 5; }
  if (text.includes('likely') || text.includes('frequent') || text.includes('common')) { likelihood = 4; }

  return { suggestedCategory: category, suggestedLikelihood: likelihood, suggestedImpact: impact };
}

export async function autoClassifyIncident(incidentData: { title?: string; description?: string; category?: string }, tenantId: string): Promise<{
  suggestedSeverity: string; suggestedResponseActions: string[];
}> {
  try {
    return await gatewayJSON<{ suggestedSeverity: string; suggestedResponseActions: string[] }>({
...
function autoClassifyIncidentFallback(incidentData: { title?: string; description?: string; category?: string }): {
  suggestedSeverity: string; suggestedResponseActions: string[];
} {
...
export async function recommendNextStep(workflowState: { currentStep: string; completedSteps: string[] }, history: string[], tenantId: string): Promise<string[]> {
  try {
    const result = await gatewayJSON<{ recommendations: string[] }>({
...
export function generateComplianceGapAlert(frameworkUpdate: { frameworkId: string; newControls: number; updatedControls: number }): {
  alertLevel: string; message: string; actions: string[];
} {
