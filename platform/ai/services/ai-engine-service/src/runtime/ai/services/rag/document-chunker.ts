...
export interface ChunkOptions {
  /** Max characters per chunk (default: 1500 ~= 375 tokens) */
  maxChunkSize?: number;
  /** Overlap characters between adjacent chunks (default: 200) */
  overlapSize?: number;
  /** Split on paragraph boundaries when possible (default: true) */
 ...
 */
export function chunkDocument(text: string, options?: ChunkOptions): ChunkInput[] {
 ...
 */
function characterChunk(text: string, maxSize: number, overlap: number): ChunkInput[] {
  const chunks: ChunkInput[] = [];
  let start = 0;
