...
import type { AgentActivity, AgentDashboardSummary } from './personal-agent.types';
...
export async function getAgentDashboardSummary(
  ...
  const dashboardQuery = await safeQuery(
    `WITH
    -- Assignment stats
    assignment_stats AS (
      SELECT
  ...
  const activitiesByType: Record<string, number> = stats.activities_by_type || {};
  const activitiesByStatus: Record<string, number> = stats.activities_by_status || {};
...
export async function getAgentAuditTrail(
  ...
  const countResult = await safeQuery(
    `SELECT COUNT(*) as total FROM "${schema}".agent_activity_log WHERE ${conditions.join(' AND ')}`,
    params
  );
...
export async function getAgentActivityTimeline(
  ...
  const params: unknown[] = userId ? [tenantId, startDate, userId] : [tenantId, startDate];
