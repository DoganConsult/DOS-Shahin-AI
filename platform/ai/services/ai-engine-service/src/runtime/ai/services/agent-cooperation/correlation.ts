...
import type { AgentDiscovery, CrossAgentCorrelation } from './types';
import { activeCycleContexts } from './cycle-context';
import type { GenericRow } from '@dos/types';

// ── Severity Helper ──────────────────────────────────────────────────────────

export function _maxSeverity(severities: string[]): 'critical' | 'high' | 'medium' | 'low' {
  const rank: Record<string, number> = { critical: 4, high: 3, medium: 2, low: 1 };
  let max = 0;
  for (const s of severities) max = Math.max(max, rank[s] || 0);
 ...
 */
export async function correlateDiscoveries(
  tenantId: string,
  currentAgentId?: string,
  newDiscoveries?: AgentDiscovery[],
): Promise<CrossAgentCorrelation[]> {
  const schema = tenantSchema(tenantId);
  const correlations: CrossAgentCorrelation[] = [];

  try {
    // Get new discoveries from current agent run if not provided
    let currentDiscoveries: AgentDiscovery[] = newDiscoveries || [];
    ...
    if (currentDiscoveries.length === 0 && currentAgentId) {
      ...
      const obsResult = await safeQuery(
        `SELECT observation_id as id, agent_id as "agentId", observation_type as type,
         ...
         LIMIT 50`,
        [tenantId, currentAgentId, last24h],
      );
      (currentDiscoveries as any) = obsResult.rows.map((row: GenericRow) => ({
        id: row.id,
        agentId: row.agentId,
    ...
    const dbDiscoveriesResult = await safeQuery(
      `SELECT id, agent_id as "agentId", discovery_type as type, title, severity,
    ...
    const obsDiscoveriesResult = await safeQuery(
      `SELECT observation_id as id, agent_id as "agentId", observation_type as type,
              title, severity, entity_type as "entityType", entity_id as "entityId",
              description as details, created_at as timestamp
       FROM "${schema}".ai_observations
    ...
    const otherDiscoveries: AgentDiscovery[] = ([
      ...dbDiscoveriesResult.rows.map((row: GenericRow) => ({
        id: row.id,
        agentId: row.agentId,
        type: (row.type || 'anomaly') as string,
        severity: (row.severity || 'medium') as string,
        entityType: row.entityType || '',
        entityId: row.entityId || undefined,
        title: row.title || '',
      ...
      ...obsDiscoveriesResult.rows.map((row: GenericRow) => ({
        id: row.id,
        agentId: row.agentId,
        type: (row.type || 'anomaly') as string,
        severity: (row.severity || 'medium') as string,
    ...
    const uniqueOtherDiscoveries = Array.from(
      new Map(otherDiscoveries.map(d => [d.id, d])).values()
    );
    ...
    const entityGroups = new Map<string, { current: AgentDiscovery[]; other: AgentDiscovery[] }>();
    ...
    for (const [key, groups] of entityGroups) {
      if (groups.current.length > 0 && groups.other.length > 0) {
        ...
        if (agents.length >= 2) {
          ...
          const correlation: CrossAgentCorrelation = {
            id: corrId,
          ...
          // Persist to database
          try {
            const corrResult = await safeQuery(
              `INSERT INTO "${schema}".agent_correlations (id, agents, shared_entity, severity, findings, created_at)
               VALUES ($1, $2, $3, $4, $5, NOW())
               ON CONFLICT (id) DO UPDATE SET
                 agents = EXCLUDED.agents, shared_entity = EXCLUDED.shared_entity, severity = EXCLUDED.severity,
                 findings = EXCLUDED.findings
               RETURNING id`,
              [corrId, agents, key, sev, JSON.stringify(correlation.findings)],
            );
            if (corrResult.rows.length > 0) logger.info(`[AgentCooperation] Stored correlation ${corrId} for ${key} (${agents.length} agents)`);
          } catch (err: unknown) {
    ...
    if (criticals.length >= 2 || (criticals.length >= 1 && highs.length >= 3)) {
      ...
      if (involvedAgents.length >= 2) {
        ...
        // Persist to database
        try {
          const cascadeResult = await safeQuery(
            `INSERT INTO "${schema}".agent_correlations (id, agents, shared_entity, severity, findings, created_at)
             VALUES ($1, $2, $3, $4, $5, NOW())
             ON CONFLICT (id) DO UPDATE SET
               agents = EXCLUDED.agents, shared_entity = EXCLUDED.shared_entity, severity = EXCLUDED.severity,
               findings = EXCLUDED.findings
             RETURNING id`,
            [cascadeId, involvedAgents, 'multi-domain', 'critical', JSON.stringify(correlation.findings)],
          );
          if (cascadeResult.rows.length > 0) logger.info(`[AgentCooperation] Stored cascade correlation ${cascadeId} (${involvedAgents.length} agents)`);
        } catch (err: unknown) {
    ...
    try {
      ...
      for (const pattern of detectedPatterns) {
        ...
        const alreadyCovered = correlations.some(c =>
          c.agents.length === pattern.agents.length &&
        ...
        if (!alreadyCovered && pattern.confidence >= 0.6) {
          ...
          // Persist to database
          try {
            const patternResult = await safeQuery(
              `INSERT INTO "${schema}".agent_correlations (id, agents, shared_entity, severity, findings, created_at)
               VALUES ($1, $2, $3, $4, $5, NOW())
               ON CONFLICT (id) DO UPDATE SET
                 agents = EXCLUDED.agents, shared_entity = EXCLUDED.shared_entity, severity = EXCLUDED.severity,
                 findings = EXCLUDED.findings
               RETURNING id`,
              [patternCorrId, pattern.agents, correlation.sharedEntity, pattern.severity, JSON.stringify(correlation.findings)],
            );
            if (patternResult.rows.length > 0) logger.info(`[AgentCooperation] Stored pattern correlation ${patternCorrId} (${pattern.patternName}, ${pattern.agents.length} agents)`);
          } catch (err: unknown) {
...
// ── Enhanced Correlation Patterns (Pillar 2: Agent Cooperation) ──────────────

export interface CorrelationPattern {
  patternId: string;
  patternType: 'temporal' | 'entity_cluster' | 'cascade' | 'recurring';
 ...
 */
export function detectTemporalCorrelations(discoveries: AgentDiscovery[], windowMs: number = 300000): CorrelationPattern[] {
  ...
  for (let i = 0; i < sorted.length; i++) {
    const window: AgentDiscovery[] = [sorted[i]];
 ...
 */
export function detectCascades(discoveries: AgentDiscovery[]): CorrelationPattern[] {
  ...
  const patterns: CorrelationPattern[] = [];
  const byEntity = new Map<string, AgentDiscovery[]>();
  ...
  for (const [_key, group] of byEntity) {
    ...
    for (const [trigger, effects] of Object.entries(cascadeMap)) {
      if (types.includes(trigger as AgentDiscovery['type']) && types.some(t => effects.includes(t))) {
        const matchedEffects = effects.filter(e => types.includes(e as AgentDiscovery['type']));
        patterns.push({
 ...
 */
export function runFullCorrelation(discoveries: AgentDiscovery[]): CorrelationPattern[] {
  return [
