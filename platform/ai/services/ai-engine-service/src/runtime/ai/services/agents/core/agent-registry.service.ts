...
import type { ApprovalStatus, DeploymentStatus } from '../../../../../domain/ai-governance/services/ai-governance-lifecycle.service';
...
function must(condition: unknown, status: number, code: string, message: string, details?: unknown): void {
  if (condition) return;
...
async function getVersionRow(schema: string, versionId: string): Promise<AgentVersion | null> {
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_agent_registry WHERE agent_version_id = $1 LIMIT 1`,
    [versionId],
  );
  return (result.rows[0] as AgentVersion) || null;
}

async function getPreviousVersion(schema: string, assetId: string, nextVersionNumber: number): Promise<AgentVersion | null> {
  const prevVer = Math.max(0, nextVersionNumber - 1);
  if (prevVer <= 0) return null;
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_agent_registry WHERE asset_id = $1 AND version_number = $2 LIMIT 1`,
    [assetId, prevVer],
  );
  return (result.rows[0] as AgentVersion) || null;
}

function computeAgentDiffSummary(
  prev: AgentVersion | null,
  agentConfig: Record<string, unknown>,
  capabilities: unknown[],
  linkedPromptAssetId: string | null,
  ...
  if (prevCfg !== nextCfg) diffs.push('config_changed');

  const prevCaps = JSON.stringify(prev.capabilities || []);
  const nextCaps = JSON.stringify(capabilities || []);
  if (prevCaps !== nextCaps) diffs.push('capabilities_changed');

  if ((prev.linked_prompt_asset_id || null) !== (linkedPromptAssetId || null)) diffs.push('linked_prompt_changed');
  if ((prev.linked_model_asset_id || null) !== (linkedModelAssetId || null)) diffs.push('linked_model_changed');

  return diffs.length > 0 ? diffs.join(',') : 'no_change';
}

async function assertParentAsset(tenantId: string, assetId: string): Promise<any> {
  const asset = await getAssetById(tenantId, assetId);
  if (!asset) throw new NotFoundError('AI asset', assetId);
...
async function assertLinkedAsset(tenantId: string, assetId: string, expectedType: string, label: string): Promise<void> {
...
export async function createDraftAgentVersion(
  tenantId: string,
  input: Record<string, unknown>,
): Promise<AgentVersion> {
  const schema = tenantSchema(tenantId);
  const typed = input as CreateAgentDraftInput;
  ...
  if (typed.linked_prompt_asset_id) await assertLinkedAsset(tenantId, typed.linked_prompt_asset_id, 'prompt', 'prompt');
  if (typed.linked_model_asset_id) await assertLinkedAsset(tenantId, typed.linked_model_asset_id, 'model', 'model');
  ...
  const diffSummary = computeAgentDiffSummary(
    prev,
    typed.agent_config || {},
    typed.capabilities || [],
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_agent_registry
  ...
  const row = result.rows[0] as AgentVersion;
...
export async function updateDraftAgentVersion(
  tenantId: string,
  versionId: string,
  ...
  if (!existing) throw new NotFoundError('Agent version', versionId);
  ...
  if (input.linked_prompt_asset_id) await assertLinkedAsset(tenantId, input.linked_prompt_asset_id, 'prompt', 'prompt');
  if (input.linked_model_asset_id) await assertLinkedAsset(tenantId, input.linked_model_asset_id, 'model', 'model');
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".ai_agent_registry
        SET agent_config = $1,
...
export async function submitAgentVersionForApproval(
  tenantId: string,
  versionId: string,
  submittedBy: string,
): Promise<AgentVersion> {
...
export async function approveAgentVersion(
  tenantId: string,
  versionId: string,
  approvedBy: string,
): Promise<AgentVersion> {
...
export async function rejectAgentVersion(
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".ai_agent_registry
      ...
      RETURNING *`,
    [notes || null, rejectedBy, versionId],
  );
  const row = result.rows[0] as AgentVersion;
...
export async function activateAgentVersion(
  ...
  versionId: string,
  activatedBy: string,
): Promise<{ activated: AgentVersion; deactivated: AgentVersion | null }> {
  const schema = tenantSchema(tenantId);
  const existing = await getVersionRow(schema, versionId);
  if (!existing) throw new NotFoundError('Agent version', versionId);
  ...
  if (existing.linked_prompt_asset_id) await assertLinkedAsset(tenantId, existing.linked_prompt_asset_id, 'prompt', 'prompt');
  if (existing.linked_model_asset_id) await assertLinkedAsset(tenantId, existing.linked_model_asset_id, 'model', 'model');

  return withTransaction(async (client) => {
    const activeRes = await safeQueryWithClient(
      `SELECT * FROM "${schema}".ai_agent_registry WHERE asset_id = $1 AND is_active = TRUE LIMIT 1`,
      [existing.asset_id],
      client,
    );
    const currentActive = (activeRes.rows[0] as AgentVersion) || null;

    let deactivated: AgentVersion | null = null;
    if (currentActive && currentActive.agent_version_id !== versionId) {
      const deactRes = await safeQueryWithClient(
        `UPDATE "${schema}".ai_agent_registry
            SET is_active = FALSE,
          ...
          WHERE agent_version_id = $2
          RETURNING *`,
        [activatedBy, currentActive.agent_version_id],
        client,
      );
      deactivated = (deactRes.rows[0] as AgentVersion) || null;
    }

    const actRes = await safeQueryWithClient(
      `UPDATE "${schema}".ai_agent_registry
          SET is_active = TRUE,
        ...
        WHERE agent_version_id = $2
        RETURNING *`,
      [activatedBy, versionId],
      client,
    );
    const activated = actRes.rows[0] as AgentVersion;
    ...
    await emitAudit(
      ...
      'update',
      versionId,
      { was_active: existing.is_active, prev_active_version: currentActive?.agent_version_id || null },
      { is_active: activated.is_active, deactivated_version: deactivated?.agent_version_id || null },
    );
...
export async function suspendAgentVersion(
  tenantId: string,
  versionId: string,
  suspendedBy: string,
  notes?: string,
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".ai_agent_registry
      ...
      WHERE agent_version_id = $3
      RETURNING *`,
    [notes || null, suspendedBy, versionId],
  );
  const row = result.rows[0] as AgentVersion;
...
export async function retireAgentVersion(
  tenantId: string,
  versionId: string,
  retiredBy: string,
  notes?: string,
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".ai_agent_registry
      ...
      WHERE agent_version_id = $3
      RETURNING *`,
    [notes || null, retiredBy, versionId],
  );
  const row = result.rows[0] as AgentVersion;
...
export async function rollbackAgentVersion(
  ...
  rolledBackBy: string,
  notes?: string,
): Promise<{ activated: AgentVersion; deactivated: AgentVersion | null; rollbackVersion: AgentVersion }> {
  const schema = tenantSchema(tenantId);
  ...
  const rollbackVersion = insertResult.rows[0] as AgentVersion;

  const { activated, deactivated } = await activateAgentVersion(tenantId, rollbackVersion.agent_version_id, rolledBackBy);

  await emitAudit(tenantId, rolledBackBy, 'update', rollbackVersion.agent_version_id,
...
export async function listAgentVersions(
  ...
  if (q.linked_prompt_asset_id) { wheres.push(`linked_prompt_asset_id = $${idx}`); params.push(q.linked_prompt_asset_id); idx++; }
  if (q.linked_model_asset_id) { wheres.push(`linked_model_asset_id = $${idx}`); params.push(q.linked_model_asset_id); idx++; }
  ...
  const [data, count] = await Promise.all([
    safeQuery(
      `SELECT * FROM "${schema}".ai_agent_registry WHERE ${where} ORDER BY asset_id, version_number DESC LIMIT $${idx} OFFSET $${idx + 1}`,
      [...params, limit, offset],
    ),
    safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${schema}".ai_agent_registry WHERE ${where}`,
      params,
    ),
  ]);

  return { versions: data.rows as AgentVersion[], total: (count.rows[0]?.total as number) || 0 };
}
...
export async function getActiveAgentVersionForAsset(
  tenantId: string,
  assetId: string,
): Promise<AgentVersion | null> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_agent_registry
...
export async function resolveActiveAgentForKey(
  tenantId: string,
  agentKey: string,
): Promise<AgentVersion | null> {
...
export async function deleteAgentVersion(
  ...
  const result = await safeQuery(
    `DELETE FROM "${schema}".ai_agent_registry WHERE agent_version_id = $1`,
    [versionId],
  );
  ...
  if (deleted) {
    await emitAudit(tenantId, deletedBy || SYSTEM_JOB_ACTOR, 'delete', versionId,
      { version_number: existing.version_number, approval_status: existing.approval_status },
      null,
    );
  }
