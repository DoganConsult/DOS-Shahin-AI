...
import { trackUsage, UsageRecord, estimateCost } from '../gateway/llm-usage-tracker.service';
...
export interface AttributedUsageRecord extends UsageRecord {
  costCenterId?: string;
  projectId?: string;
 ...
 */
export async function trackUsageWithAttribution(record: AttributedUsageRecord): Promise<void> {
  // Track base usage
  ...
  if (record.costCenterId || record.projectId) {
    ...
    await safeQuery(
      `INSERT INTO "${schema}".llm_cost_attribution
 ...
 */
export async function upsertCostCenter(center: CostCenter): Promise<void> {
  const schema = tenantSchema(center.tenantId);
  await safeQuery(
    `INSERT INTO "${schema}".cost_centers
 ...
 */
export async function getCostAttributionSummary(
  tenantId: string,
  daysBack: number = 30
): Promise<{
  byCostCenter: Array<{ costCenterId: string; costCenterCode: string; cost: number; tokens: number }>;
  byProject: Array<{ projectId: string; projectCode: string; cost: number; tokens: number }>;
  unattributed: { cost: number; tokens: number };
}> {
  const schema = tenantSchema(tenantId);
  const summary = {
    byCostCenter: [] as Array<{ costCenterId: string; costCenterCode: string; cost: number; tokens: number }>,
    byProject: [] as Array<{ projectId: string; projectCode: string; cost: number; tokens: number }>,
    unattributed: { cost: 0, tokens: 0 },
  };

  try {
    // By cost center
    const centerResult = await safeQuery(
      `SELECT cost_center_id, cost_center_code,
    ...
    summary.byCostCenter = centerResult.rows.map((r: GenericRow) => ({
    ...
    summary.byProject = projectResult.rows.map((r: GenericRow) => ({
    ...
    // Unattributed
    const unattributedResult = await safeQuery(
      `SELECT SUM(cost_usd)::real AS cost, SUM(tokens)::int AS tokens
