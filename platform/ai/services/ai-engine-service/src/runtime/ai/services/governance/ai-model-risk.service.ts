...
export async function calculateModelRiskScore(
  ...
  if (modelData.training_data_summary) {
    ...
    if (dataSummary.data_volume && (dataSummary as any).data_volume < 1000) dataRisk += 0.2;
  }
  ...
  if (modelData.performance_metrics) {
    ...
    if (perf.accuracy && (perf as any).accuracy < 0.8) modelRisk += 0.3;
    if (perf.precision && (perf as any).precision < 0.75) modelRisk += 0.2;
    if (perf.recall && (perf as any).recall < 0.75) modelRisk += 0.2;
...
export async function syncModelRiskToEnterpriseRegister(
  ...
  scoreRow?: { score_id?: string; risk_factors?: Record<string, unknown> }
): Promise<void> {
  const schema = tenantSchema(tenantId);
 ...
 */
export async function getModelsRequiringAssessment(tenantId: string): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
