...
/** Context snapshot returned by each module provider */
...
/** Single recent-activity entry surfaced to the copilot */
...
/** An open / pending item requiring attention */
...
/** Alert generated from module analysis */
...
/** Proactive suggestion generated from cross-module analysis */
...
/** Aggregated context across all modules */
export interface FullContext {
  tenantId: string;
  generatedAt: string;
  modules: ModuleContext[];
  crossModuleAlerts: ContextAlert[];
  healthScore?: number;
}

// ── Module metadata (A01–A10) ────────────────────────────────────────────────

const MODULE_META: Record<string, { name: string; agentId: string }> = {
...
// ── Individual module context providers ──────────────────────────────────────

type ProviderFn = (schema: string) => Promise<Partial<ModuleContext>>;

/** A01 – Onboarding & workspace setup */
async function onboardingCtx(schema: string): Promise<Partial<ModuleContext>> {
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, completed: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
            COUNT(*) FILTER (WHERE status = 'completed')::int AS completed
     FROM "${schema}".onboarding_steps`
  ), { operation: 'query onboarding_steps' });
  const r = res.rows[0];
  const pct = r.total > 0 ? Math.round((r.completed / r.total) * 100) : 0;
  const alerts: ContextAlert[] = [];
  if (pct < 100) alerts.push({ severity: 'info', message: `Onboarding ${pct}% complete — ${r.total - r.completed} steps remaining`, route: '/onboarding' });
  return { metrics: { totalSteps: r.total, completedSteps: r.completed, completionPct: pct }, alerts };
}

/** A02 – Identity & access governance */
async function identityCtx(schema: string): Promise<Partial<ModuleContext>> {
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, active: 0, dormant: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
  ...
  const r = res.rows[0];
  const alerts: ContextAlert[] = [];
  if (r.dormant > 0) alerts.push({ severity: 'warning', message: `${r.dormant} user(s) dormant for 90+ days — review access`, route: '/identity/users' });
  return { metrics: { totalUsers: r.total, activeUsers: r.active, dormantUsers: r.dormant }, alerts };
}

/** A03 – Frameworks & cross-mapping */
async function frameworksCtx(schema: string): Promise<Partial<ModuleContext>> {
...
async function controlsCtx(schema: string): Promise<Partial<ModuleContext>> {
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, implemented: 0, ineffective: 0, draft: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
  ...
  const alerts: ContextAlert[] = [];
  if (r.ineffective > 0) alerts.push({ severity: 'warning', message: `${r.ineffective} control(s) marked ineffective — remediation needed`, route: '/controls' });
...
async function evidenceCtx(schema: string): Promise<Partial<ModuleContext>> {
  ...
  const alerts: ContextAlert[] = [];
  if (req.rows[0].overdue > 0) alerts.push({ severity: 'warning', message: `${req.rows[0].overdue} overdue evidence request(s)`, route: '/evidence/requests' });
  if (ev.rows[0].expiring > 0) alerts.push({ severity: 'info', message: `${ev.rows[0].expiring} evidence item(s) expiring within 30 days`, route: '/evidence' });
...
async function complianceCtx(schema: string): Promise<Partial<ModuleContext>> {
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, open: 0, critical: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
            COUNT(*) FILTER (WHERE status = 'open')::int AS open,
            COUNT(*) FILTER (WHERE severity = 'critical')::int AS critical
     FROM "${schema}".compliance_gaps`
  ), { operation: 'query compliance_gaps' });
  const r = res.rows[0];
  const alerts: ContextAlert[] = [];
  if (r.critical > 0) alerts.push({ severity: 'critical', message: `${r.critical} critical compliance gap(s) require immediate attention`, route: '/compliance/gaps' });
  return { metrics: { totalGaps: r.total, openGaps: r.open, criticalGaps: r.critical }, alerts };
}

/** A07 – Risk management */
async function riskCtx(schema: string): Promise<Partial<ModuleContext>> {
  ...
  const r = risks.rows[0]; const k = kris.rows[0];
  const alerts: ContextAlert[] = [];
  if (r.critical > 0) alerts.push({ severity: 'critical', message: `${r.critical} risk(s) in critical zone`, route: '/risk/register' });
  if (k.breached > 0) alerts.push({ severity: 'warning', message: `${k.breached} KRI(s) breached thresholds`, route: '/risk/kris' });
...
async function policiesCtx(schema: string): Promise<Partial<ModuleContext>> {
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, expired: 0, pending: 0, overdue_review: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
  ...
  const r = res.rows[0];
  const alerts: ContextAlert[] = [];
  if (r.expired > 0) alerts.push({ severity: 'warning', message: `${r.expired} policy/policies expired — renewal required`, route: '/policies' });
  if (r.overdue_review > 0) alerts.push({ severity: 'info', message: `${r.overdue_review} policy/policies overdue for periodic review`, route: '/policies' });
...
async function vendorsCtx(schema: string): Promise<Partial<ModuleContext>> {
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, high_risk: 0, overdue: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
  ...
  ), { operation: 'query vendor_profiles' });
  const r = res.rows[0];
  const alerts: ContextAlert[] = [];
  if (r.high_risk > 0) alerts.push({ severity: 'warning', message: `${r.high_risk} vendor(s) in high/critical risk tier`, route: '/vendors' });
  if (r.overdue > 0) alerts.push({ severity: 'info', message: `${r.overdue} vendor reassessment(s) overdue`, route: '/vendors' });
  return { metrics: { totalVendors: r.total, highRiskVendors: r.high_risk, overdueReassessments: r.overdue }, alerts };
}

/** A10 – Audit & reporting */
async function auditCtx(schema: string): Promise<Partial<ModuleContext>> {
  ...
  const alerts: ContextAlert[] = [];
  if (findings.rows[0].critical > 0) alerts.push({ severity: 'critical', message: `${findings.rows[0].critical} critical audit finding(s) open`, route: '/audit/findings' });
 ...
 */
export async function getModuleContext(tenantId: string, moduleKey: string): Promise<ModuleContext | null> {
 ...
 */
export async function getFullContext(tenantId: string): Promise<FullContext> {
  const schema = tenantSchema(tenantId);
  const moduleResults = await Promise.all(
    Object.keys(PROVIDERS).map(async (key) => {
      try { return await getModuleContext(tenantId, key); }
      catch { return null; }
    }),
  );
  const modules = moduleResults.filter(Boolean) as ModuleContext[];
  const crossModuleAlerts: ContextAlert[] = [];
  ...
  const healthRes = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT overall_score FROM "${schema}".governance_health_scores ORDER BY created_at DESC LIMIT 1`
  ), { operation: 'query governance_health_scores' });
 ...
 */
export async function getProactiveSuggestions(tenantId: string): Promise<ProactiveSuggestion[]> {
