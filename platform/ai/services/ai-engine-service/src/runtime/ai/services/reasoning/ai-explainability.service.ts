...
export interface ExplainabilityRecord {
  ...
  compliance_status: 'compliant' | 'partial' | 'non_compliant' | 'not_required';
}

/**
 * Create explainability record
 */
export async function createExplainabilityRecord(
  tenantId: string,
  record: {
    session_id?: string;
    agent_id: string;
  ...
  if (record.explanation_content.feature_importance) qualityScore += 0.1;
  if (record.explanation_content.reasoning_chain) qualityScore += 0.1;
  if (record.explanation_content.confidence_breakdown) qualityScore += 0.1;
  ...
  // Check compliance
  let complianceStatus: 'compliant' | 'partial' | 'non_compliant' | 'not_required' = 'not_required';
  if (record.explainability_required) {
    if (meetsThreshold && record.explanation_content) complianceStatus = 'compliant';
    else if (record.explanation_content && Object.keys(record.explanation_content).length > 0) complianceStatus = 'partial';
    else complianceStatus = 'non_compliant';
  }

  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_explainability_records 
 ...
 */
export async function reviewExplainabilityRecord(
  tenantId: string,
  recordId: string,
  reviewedBy: string,
  reviewFeedback?: string,
  explanationApproved?: boolean
): Promise<boolean> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `UPDATE "${schema}".ai_explainability_records 
     SET human_reviewed = TRUE, reviewed_by = $1, reviewed_at = NOW(),
         review_feedback = $2, explanation_approved = $3
     WHERE record_id = $4
     RETURNING record_id`,
    [reviewedBy, reviewFeedback || null, explanationApproved || null, recordId]
  );
 ...
 */
export async function createCounterfactualAnalysis(
  tenantId: string,
  explainabilityRecordId: string,
  analysis: {
    scenario_description: string;
    input_changes: Record<string, unknown>;
    expected_output_changes?: Record<string, unknown> & { magnitude?: number };
  ...
  let robustness: 'robust' | 'moderate' | 'sensitive' | 'fragile' = 'moderate';
  if (sensitivityScore < 0.2) robustness = 'robust';
  else if (sensitivityScore < 0.5) robustness = 'moderate';
  else if (sensitivityScore < 0.8) robustness = 'sensitive';
  else robustness = 'fragile';

  const result = await safeQuery(
    `INSERT INTO "${schema}".ai_counterfactual_analysis 
 ...
 */
export async function getExplainabilityRequirements(
  tenantId: string,
  agentId?: string,
  decisionType?: string
 ...
 */
export async function enrichWithArabicExplanation(
  tenantId: string,
  recordId: string,
  explanationContent: Record<string, unknown>
): Promise<void> {
  try {
...
Respond with JSON: {explanation_ar: string, key_factors_ar: string[], recommendation_ar: string}`,
    ...
    await safeQuery(
      `UPDATE "${schema}".ai_explainability_records
       ...
       WHERE record_id = $2`,
      [JSON.stringify({ explanation_ar: (arabic as Record<string, unknown>).explanation_ar, key_factors_ar: (arabic as Record<string, unknown>).key_factors_ar }), recordId]
    );
  } catch {
 ...
 */
export async function calculateTransparencyMetrics(
  tenantId: string,
  agentId: string,
  periodStart: string,
  periodEnd: string
