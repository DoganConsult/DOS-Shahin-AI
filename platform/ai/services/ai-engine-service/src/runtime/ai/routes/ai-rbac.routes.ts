...
router.get('/analysis/role-usage', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId);
  const result = await safeQuery(
    `SELECT fr.code AS role_code, fr.name AS role_name, fr.is_active,
...
router.get('/analysis/permission-coverage', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId);
  ...
  const assigned = await safeQuery(
    `SELECT COUNT(DISTINCT p.id) AS cnt FROM "${schema}".permissions p
     JOIN "${schema}".role_permissions rp ON rp.permission_id = p.id
     WHERE p.is_active = TRUE`,
  );
  const unassigned = await safeQuery(
    `SELECT p.code, p.description FROM "${schema}".permissions p
...
router.get('/analysis/sod-risk', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId);
  const conflicts = await safeQuery(
    `SELECT sr.role_code_a, sr.role_code_b, sr.conflict_level, sr.reason,
...
router.get('/analysis/least-privilege', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId);
  const overProvisioned = await safeQuery(
    `SELECT ura.user_id, COUNT(DISTINCT rp.permission_id) AS total_permissions,
  ...
  const recommendations = overProvisioned.rows
    .filter(( r: Record<string, unknown>) => {
      const total = parseInt((r as any).total_permissions, 10);
      const used = parseInt((r as any).used_permissions, 10);
      return total > 0 && (used / total) < 0.5;
    })
    .map(( r: Record<string, unknown>) => ({
      userId: r.user_id,
      totalPermissions: parseInt((r as any).total_permissions, 10),
      usedPermissions: parseInt((r as any).used_permissions, 10),
      utilizationPercent: Math.round((parseInt((r as any).used_permissions, 10) / parseInt((r as any).total_permissions, 10)) * 100),
...
router.get('/analysis/access-heatmap', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const schema = tenantSchema(req.tenantId);
  const days = parseInt(req.query.days as string, 10) || 30;
  const heatmap = await safeQuery(
    `SELECT module_code, permission_code, decision, COUNT(*) AS cnt
