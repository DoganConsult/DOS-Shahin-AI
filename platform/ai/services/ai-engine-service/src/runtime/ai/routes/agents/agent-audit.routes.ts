...
import { assertTenantId as _assertTenantId, safeQuery, tenantSchema } from '../../ports/database.port';
import { GenericRow } from '../../ports/platform.port';
 ...
 */
router.get('/history', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    ...
    const conditions: string[] = [`module = 'agent_runner'`, `action LIKE 'agent.%'`];
    ...
    const entries = result.rows.map((row: GenericRow) => ({
 ...
 */
router.get('/:agentId/actions', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const { agentId } = req.params;
    const actionType = req.query.actionType as 'proposed' | 'executed' | 'verified' | undefined;
    const startDate = req.query.startDate as string | undefined;
    const endDate = req.query.endDate as string | undefined;
    const limit = parseInt(req.query.limit as string) || 100;
    const offset = parseInt(req.query.offset as string) || 0;
    ...
    const params: unknown[] = [`agent-${agentId}`];
    let idx = 2;
    ...
    const actions = result.rows.map((row: GenericRow) => {
 ...
 */
router.get('/:agentId/predictions', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const { agentId } = req.params;
    ...
    const predictions = result.rows.map((row: GenericRow) => {
      const afterState = row.after_state ? (typeof row.after_state === 'string' ? JSON.parse(row.after_state) : row.after_state) : {};
      return {
        entryId: row.entry_id,
        timestamp: row.timestamp?.toISOString?.() || row.timestamp,
 ...
 */
router.get('/:agentId/tool-calls', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const { agentId } = req.params;
    ...
    const toolCalls = result.rows.map((row: GenericRow) => {
      const afterState = row.after_state ? (typeof row.after_state === 'string' ? JSON.parse(row.after_state) : row.after_state) : {};
      return {
        entryId: row.entry_id,
        timestamp: row.timestamp?.toISOString?.() || row.timestamp,
 ...
 */
router.get('/:agentId/decisions', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const { agentId } = req.params;
    ...
    const decisions = result.rows.map((row: GenericRow) => {
      const afterState = row.after_state ? (typeof row.after_state === 'string' ? JSON.parse(row.after_state) : row.after_state) : {};
      return {
        entryId: row.entry_id,
        timestamp: row.timestamp?.toISOString?.() || row.timestamp,
 ...
 */
router.get('/:agentId/handoffs', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const { agentId } = req.params;
    ...
    const handoffs = result.rows.map((row: GenericRow) => {
      const afterState = row.after_state ? (typeof row.after_state === 'string' ? JSON.parse(row.after_state) : row.after_state) : {};
      return {
        entryId: row.entry_id,
        timestamp: row.timestamp?.toISOString?.() || row.timestamp,
 ...
 */
router.get('/:agentId/memory', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const { agentId } = req.params;
    const memoryType = req.query.memoryType as 'fact' | 'pattern' | 'lesson' | 'preference' | undefined;
    const startDate = req.query.startDate as string | undefined;
    const endDate = req.query.endDate as string | undefined;
    const limit = parseInt(req.query.limit as string) || 100;
    const offset = parseInt(req.query.offset as string) || 0;
    ...
    const memories = result.rows.map((row: GenericRow) => {
      const afterState = row.after_state ? (typeof row.after_state === 'string' ? JSON.parse(row.after_state) : row.after_state) : {};
      return {
        entryId: row.entry_id,
        timestamp: row.timestamp?.toISOString?.() || row.timestamp,
 ...
 */
router.get('/run/:runId', validate({ query: z.record(z.unknown()) }), requirePermission('ai.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
