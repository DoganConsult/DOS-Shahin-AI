...
router.put('/budget', authenticate, requirePermission('admin.tenant.manage'), validate({ body: budgetPutBody }), asyncHandler(async (req, res) => {
  ...
  const ok = await updateBudgetLimits(
  req.tenantId,
  req.body.monthlyTokenLimit || 10000000,
  req.body.monthlyCostLimit || 100,
  req.body.softLimitPct || 80,
  req.body.hardLimitAction || 'throttle',
  );
  swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user!.userId, module: 'governance', event: 'updated', entityType: 'ai_enhanced', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.ai_enhanced.updated' });
  res.json({ success: ok });
}));
...
router.post('/prompts/:assetId', authenticate, requirePermission('admin.tenant.manage'), validate({ body: promptsAssetIdPostBody }), asyncHandler(async (req, res) => {
  ...
  const version = await createDraftPromptVersion(
  req.tenantId, {
...
router.get('/cache/stats', validate({ query: z.record(z.unknown()) }), authenticate, async (_req: Request, res: Response) => {
  try {
...
router.post('/stream/chat', authenticate, validate({ body: streamChatPostBody }), asyncHandler(async (req, res) => {
  ...
  if (!guard.allowed) {
  res.status(400).json({ error: guard.warning }); return;
  }
  const { streamClaudeResponse } = await import('../../services/llm/llm-stream.service');
  const messages = [
  ...(req.body.systemPrompt ? [{ role: 'system' as const, content: req.body.systemPrompt }] : []),
  { role: 'user' as const, content: guard.sanitizedInput },
  ];
  await streamClaudeResponse(messages, res, { agentId: req.body.agentId, tenantId: req.tenantId });
  } catch (err: unknown) {
  if (!res.headersSent) res.status(500).json({ error: toErrorMessage(err) });
  }
}));
...
router.get('/evals', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const days = parseInt(req.query.days as string) || 30;
  const summary = await getEvalSummary(req.tenantId, req.query.agentId as string, days);
  res.json(summary);
}));
...
router.get('/cycle-memory', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const cycles = await getCycleIds(req.tenantId, parseInt(req.query.limit as string) || 20);
  res.json({ cycles });
}));
...
router.get('/cycle-memory/:cycleId', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const entries = await getCycleMemory(req.tenantId, req.params.cycleId, req.query.agentId as string);
  res.json({ entries });
}));
...
router.get('/shared-memory', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const entries = await readAllSharedScopes(
  req.tenantId, req.query.agentId as string,
...
router.get('/bilingual-prompts', validate({ query: z.record(z.unknown()) }), authenticate, async (_req: Request, res: Response) => {
  try {
...
router.get('/gateway/health', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('ai.agent.read'), async (_req: Request, res: Response) => {
  try {
...
router.get('/proposed-actions', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  `SELECT * FROM "${schema}".proposed_actions
...
router.put('/proposed-actions/:id', authenticate, requirePermission('admin.tenant.manage'), validate({ body: proposedActionsIdPutBody }), asyncHandler(async (req, res) => {
  ...
  `UPDATE "${schema}".proposed_actions
  SET status = $1, reviewed_by = $2, reviewed_at = NOW(), review_notes = $3
  WHERE id = $4 AND tenant_id = $5 RETURNING *`,
  [status, req.user?.userId || 'admin', reviewNotes || null, req.params.id, req.tenantId]);
  } else {
  result = await safeQuery(
  `SELECT * FROM "${schema}".proposed_actions WHERE id = $1 AND tenant_id = $2`,
  ...
  res.json({ success: true, item: getFirstRow((result as any)) });
}));
...
router.get('/cost-forecast', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const historicalCost = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT DATE(created_at) AS day, SUM(cost_usd) AS daily_cost, SUM(input_tokens + output_tokens) AS daily_tokens, COUNT(*) AS daily_calls
     FROM "${schema}".llm_usage_log WHERE created_at > NOW() - INTERVAL '30 days'
     GROUP BY DATE(created_at) ORDER BY day`
  ), { tenantId: req.tenantId, operation: 'query llm_usage_log' });
...
Respond with JSON: {
  ...
  res.json({ historical: historicalCost.rows, ...((forecast || {}) as unknown) });
}));
...
router.get('/health-dashboard', authenticate, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const healthyCount = agentHealth.rows.filter((a: Record<string, unknown>) => a.health_status === 'healthy').length;
