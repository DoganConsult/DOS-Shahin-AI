...
import { NotFoundError, ValidationError as _ValidationError } from '../../../../errors';
...
} from '../../services/code-search/code-search.service';
...
router.get('/ai-os/code-search/engines/:engineCode', authenticate, aiReadLimiter, requirePermission('ai.code_search.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  if (!engine) throw new NotFoundError('CodeSearchEngine', req.params.engineCode);
  setCacheHeaders(res, 30);
  res.ok(engine);
}));
