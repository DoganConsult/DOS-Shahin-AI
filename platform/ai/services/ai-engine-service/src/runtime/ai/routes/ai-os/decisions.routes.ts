...
import {
  aiOsRecommendationsDecisionIdAcceptPostBody,
  aiOsRecommendationsDecisionIdRejectPostBody,
  aiOsRecommendationsBatchAcceptPostBody,
  aiOsRecommendationsBatchRejectPostBody,
} from './ai-os-schemas';
import {
  listDecisions,
  getDecisionById,
  ...
  getDecisionStats,
  getDecisionTrend,
  getExplainabilityChain,
  getDecisionsByEntityForApi,
} from '../../services/reasoning/ai-decision-engine.service';
import {
  listRecommendations,
...
} from '../../services/reasoning/ai-recommendation-engine.service';
...
router.get(
  ...
  aiReadLimiter,
  validate({ query: paginationQuery.extend(agentFilterQuery.shape).extend(entityFilterQuery.shape) }),
  asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const { agentId, entityType } = req.query as Record<string, string | undefined>;
    const { page, pageSize, offset, limit } = extractPagination(req.query as Record<string, string | undefined>);
    const result = await listDecisions(tenantId, { agentId, entityType, limit, offset });
    setCacheHeaders(res, 30);
    const items = (result as Record<string, unknown>).items || (result as Record<string, unknown>).decisions || result;
    const total = (result as Record<string, unknown>).total ?? (Array.isArray(items) ? items.length : 0);

    res.paginated((items as any), total, page, pageSize);
  }),
);
...
router.get(
  ...
  authenticate,
  requirePermission('ai.agent.read'),
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
...
router.get(
  ...
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    if (!record) throw new NotFoundError('Decision', req.params.decisionId);
    setCacheHeaders(res, 30);
    res.ok(record);
  }),
);
...
router.get(
  ...
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    if (!chain.decision) throw new NotFoundError('Decision', req.params.decisionId);
    setCacheHeaders(res, 30);
    res.ok(chain);
  }),
);
...
router.get(
  ...
  requirePermission('ai.agent.read'),
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const { runIds, initialLimit = '10' } = req.query as Record<string, string>;
    ...
    const runIdList = runIds.split(',').filter((id: string) => id.trim());
    ...
    const chains = await Promise.all(runIdList.map(async (runId: string) => {
      try {
...
router.get(
  ...
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    const { offset, limit } = req.query as Record<string, string | undefined>;
    const trace = await getRunTrace(req.tenantId, runId);
    // Include reasoning chain if available
    let reasoningChain: unknown[] = [];
    try {
...
router.get(
  ...
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    const summary = await getReasoningChainSummary(req.tenantId, runId);
    if (!summary) throw new NotFoundError('ReasoningChainSummary', runId);
    setCacheHeaders(res, 30);
    res.ok(summary);
  }),
);
...
router.get(
  ...
  aiReadLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    const obsResult = await safeQuery(
    ...
    const decResult = await safeQuery(
      `SELECT DISTINCT entity_type, entity_id, outcome->>'title' as title
       FROM "${schema}".decision_record
       WHERE run_id = $1 AND tenant_id = $2 AND outcome->>'title' IS NOT NULL`,
      [runId, req.tenantId],
    );

    const entities = [
      ...obsResult.rows.map(( r: Record<string, unknown>) => ({
        entityType: r.entity_type,
        entityId: r.entity_id,
        title: r.title || `${r.entity_type}:${r.entity_id}`,
      })),
      ...decResult.rows.map(( r: Record<string, unknown>) => ({
        entityType: r.entity_type,
        entityId: r.entity_id,
        title: r.title || `${r.entity_type}:${r.entity_id}`,
      })),
    ...
    const uniqueEntities = Array.from(
      new Map(entities.map(e => [`${e.entityType}:${e.entityId}`, e])).values(),
    );
...
router.get(
  ...
  authenticate,
  requirePermission('ai.agent.read'),
  aiExportLimiter, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const runId = req.params.runId;
    const { format = 'json' } = req.query as Record<string, string | undefined>;
    const { getReasoningChain, getReasoningChainSummary } = await import('../../services/reasoning/reasoning-chain.service');
    ...
    if (format === 'json') {
      ...
      if (steps && Array.isArray(steps) && steps.length > 0) {
        ...
        for (let i = 0; i < steps.length; i++) {
          ...
          if (step.input) doc.fontSize(9).fillColor('#555').text(`Input: ${typeof step.input === 'string' ? step.input.substring(0, 500) : JSON.stringify(step.input).substring(0, 500)}`);
          if (step.output) doc.fontSize(9).fillColor('#555').text(`Output: ${typeof step.output === 'string' ? step.output.substring(0, 500) : JSON.stringify(step.output).substring(0, 500)}`);
          if (step.duration_ms) doc.fontSize(9).fillColor('#888').text(`Duration: ${step.duration_ms}ms`);
          doc.moveDown(0.5);
        }
      } else {
...
router.get(
  ...
  requirePermission('ai.agent.read'),
  aiReadLimiter,
  validate({ query: paginationQuery.extend(agentFilterQuery.shape).extend({ status: z.string().optional() }) }),
  asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
...
router.post(
  '/ai-os/recommendations/:decisionId/accept',
  authenticate,
  requirePermission('ai.agent.approve'),
  aiApproveLimiter,
  validate({ body: aiOsRecommendationsDecisionIdAcceptPostBody }),
  asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
...
router.post(
  ...
  requirePermission('ai.agent.approve'),
  aiApproveLimiter,
  validate({ body: aiOsRecommendationsDecisionIdRejectPostBody }),
  asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
...
router.post(
  ...
  requirePermission('ai.agent.approve'),
  aiApproveLimiter,
  validate({ body: aiOsRecommendationsBatchAcceptPostBody }),
  asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
...
router.post(
  ...
  requirePermission('ai.agent.approve'),
  aiApproveLimiter,
  validate({ body: aiOsRecommendationsBatchRejectPostBody }),
  asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
