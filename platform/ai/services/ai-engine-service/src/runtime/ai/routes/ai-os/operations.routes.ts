...
import {
  aiOsMaintenancePruneSignalsPostBody, aiOsMaintenancePruneDecisionsPostBody,
  aiOsRegressionsDetectPostBody, aiOsRegressionsAlertIdAcknowledgePostBody,
  aiOsAbTestsPostBody, aiOsAbTestsTestIdAssignPostBody, aiOsAbTestsTestIdResultPostBody,
  aiOsToolsPostBody, aiOsToolsToolIdVersionsVersionDeprecatePostBody,
  aiOsCostAttributionCentersPostBody, aiOsCostAttributionProjectsPostBody,
  aiOsWebhookTestPostBody,
} from './ai-os-schemas';
...
router.get('/ai-os/ab-tests', authenticate, aiReadLimiter, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  if (!testId) throw new ValidationError([{ path: 'testId', message: 'testId query parameter is required' }]);
