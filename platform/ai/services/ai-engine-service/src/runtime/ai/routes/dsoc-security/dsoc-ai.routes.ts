...
router.get('/gate-decisions', requirePermission('platform.dsoc.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = (req.query.tenant as string) || undefined;
    const agentId = (req.query.agent as string) || undefined;
    const decision = (req.query.decision as 'allowed' | 'denied' | undefined);
...
router.get('/gate-stats', requirePermission('platform.dsoc.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = (req.query.tenant as string) || undefined;
    const windowHours = req.query.window ? Number(req.query.window) : undefined;
...
router.get('/hitl-queue', requirePermission('platform.dsoc.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = (req.query.tenant as string) || undefined;
    const limit = req.query.limit ? Number(req.query.limit) : undefined;
...
router.get('/hitl-backlog', requirePermission('platform.dsoc.read'), async (req: Request, res: Response) => {
  try {
