...
import {
  listAssets,
  type AssetType,
} from '../../../ai-governance/services/ai/registry/ai-asset-inventory.service';
...
router.get(
  "/assets",
  authenticate,
  requirePermission("ai.agent.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
...
router.get(
  ...
  requirePermission("ai.agent.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    if (!version) { res.status(404).json({ error: "Agent version not found" }); return; }
    res.json(version);
  }),
);
...
router.post(
  ...
  validate({ body: createVersionBody, strict: true }),
  asyncHandler(async (req, res) => {
    ...
    res.status(400).json({ error: "asset_id and agent_config are required" }); return;
    }
    ...
    setAuditData(res as any, { action: "create", entityType: "agent_version", entityId: version.agent_version_id, afterState: version });
...
router.patch(
  ...
  authenticate,
  requirePermission("ai.agent.write"),
  validate({ body: updateVersionBody, strict: true }),
  asyncHandler(async (req, res) => {
    try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    ...
    setAuditData(res as any, { action: "update", entityType: "agent_version", entityId: req.params.versionId, afterState: version });
    res.json(version);
    } catch (err: unknown) {
...
router.post(
  ...
  authenticate,
  requirePermission("ai.agent.write"),
  validate({ body: emptyBody, strict: true }),
  asyncHandler(async (req, res) => {
    try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
...
router.post(
  ...
  authenticate,
  requireAnyPermission("ai.agent.approve", "ai.agent.approve"),
  validate({ body: emptyBody, strict: true }),
  asyncHandler(async (req, res) => {
    try {
    const tenantId = req.tenantId;
...
router.post(
  ...
  authenticate,
  requireAnyPermission("ai.agent.approve", "ai.agent.approve"),
  validate({ body: notesBody, strict: true }),
  asyncHandler(async (req, res) => {
    try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
...
router.post(
  ...
  authenticate,
  requirePermission("ai.agent.manage"),
  validate({ body: rollbackBody, strict: true }),
  asyncHandler(async (req, res) => {
    try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
...
router.delete(
  "/versions/:versionId",
  authenticate,
  requirePermission("ai.agent.write"), validate({ body: emptyBody, strict: true }), asyncHandler(async (req, res) => {
    try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;

    const deleted = await deleteAgentVersion(tenantId, req.params.versionId, userId);
    if (!deleted) { res.status(404).json({ error: "Agent version not found" }); return; }

    setAuditData(res as any, { action: "delete", entityType: "agent_version", entityId: req.params.versionId });
