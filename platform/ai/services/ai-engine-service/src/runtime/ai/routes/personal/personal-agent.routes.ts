...
// ============================================================
// Personal Agent Routes — API endpoints for dedicated user agents
// ============================================================
...
} from '../../services/personal/personal-agent.service';
import { toErrorMessage } from '@dos/module-sdk';
import { getFirstRow } from '@dos/db';
import { auditMiddleware, asyncHandler as _asyncHandler, validate, requireTenant, moduleStack, mutationEventHook } from '../../ports/middleware.port';
...
router.post(
  '/assign',
  authenticate,
  requireTenant, validate({ body: assignPostBody }),
 ...
 */
router.get(
  '/my-agent', validate({ query: z.record(z.unknown()) }), authenticate,
  requireTenant,
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
...
router.patch(
  '/my-agent/:agentId',
  authenticate,
  requireTenant, validate({ body: myAgentAgentIdPatchBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const userId = req.user!.userId;
      ...
      const assignment = await updatePersonalAgentAssignment(
        tenantId,
        userId,
        agentId,
        req.body,
...
router.post(
  '/consent',
  authenticate,
  requireTenant, validate({ body: consentPostBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const userId = req.user!.userId;
...
router.post(
  '/execute',
  authenticate,
  requireTenant, validate({ body: executePostBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const userId = req.user!.userId;
 ...
 */
router.get(
  '/activities', validate({ query: z.record(z.unknown()) }), authenticate,
  requireTenant,
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      ...
      const result = await safeQuery(
        `SELECT * FROM "${schema}".agent_activity_log
 ...
 */
router.get(
  '/activities/:activityId', validate({ query: z.record(z.unknown()) }), authenticate,
  requireTenant,
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
...
router.post(
  ...
  authenticate,
  requireTenant,
  requirePermission('ai.personal-agent.approve'), validate({ body: activitiesActivityIdApprovePostBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const userId = req.user!.userId;
...
router.post(
  ...
  authenticate,
  requireTenant,
  requirePermission('ai.personal-agent.approve'), validate({ body: activitiesActivityIdRejectPostBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const userId = req.user!.userId;
...
router.post(
  '/check-sla',
  authenticate,
  requireTenant, validate({ body: checkSlaPostBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const userId = req.user!.userId;
 ...
 */
router.get(
  '/tenant-mode', validate({ query: z.record(z.unknown()) }), authenticate,
  requireTenant,
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      ...
      const tenantConfig = await safeQuery(
        `SELECT operation_mode, ai_autonomy_level
      ...
      let effectiveMode: 'human' | 'hyper' | 'autonomous' = 'human';
 ...
 */
router.get(
  '/dashboard', validate({ query: z.record(z.unknown()) }), authenticate,
  requireTenant,
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
 ...
 */
router.get(
  '/timeline', validate({ query: z.record(z.unknown()) }), authenticate,
  requireTenant,
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
...
router.post(
  ...
  authenticate,
  requireTenant, validate({ body: activitiesActivityIdConfirmPostBody }),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
...
router.get(
  ...
  requireTenant,
  requirePermission('ai.personal-agent.write'),
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.tenantId!;
