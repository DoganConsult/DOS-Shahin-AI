...
router.get("/queue", authenticate, requirePermission("ai.agent.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  let idx = 1;
  if (req.query.state) { conditions.push(`hitl_state = $${idx++}`); params.push(req.query.state); }
  if (req.query.entityType) { conditions.push(`entity_type = $${idx++}`); params.push(req.query.entityType); }
  const where = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';
  const result = await safeQuery(
  `SELECT state_id, entity_type, entity_id, hitl_state, ai_agent_id, confidence,
...
router.get("/dashboard", authenticate, requirePermission("ai.agent.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const [stateResult, gateResult] = await Promise.all([
    ...
    ).catch(() => ({ rows: [{ totalPendingGates: 0, totalApprovedGates: 0, totalRejectedGates: 0, totalExpiredGates: 0 }] })),
  ]);
...
router.post("/:entityType/:entityId/review", authenticate, requirePermission("ai.governance.review"), validate({ body: entityTypeEntityIdReviewPostBody }), asyncHandler(async (req, res) => {
  ...
  const result = await safeQuery(
  `UPDATE "${schema}".hitl_states
  SET hitl_state = $3, review_decision = $4, last_actor_type = 'human', updated_at = NOW()
  WHERE entity_type = $1 AND entity_id = $2
  RETURNING *`,
  [entityType, entityId, newState, decision]
  );
  if (!getFirstRow(result)) { res.status(404).json({ error: "HITL state not found" }); return; }
  setAuditData(res as any, { action: "update", entityType: "hitl", entityId: getFirstRow(result)?.state_id, afterState: getFirstRow(result) });
  ...
  eventBus.publish(({ eventType: 'ai.hitl.completed', tenantId, sourceService: 'hitl', severity: 'info', payload: { entityType, entityId, decision: newState, reviewedBy: req.user?.userId } } as any));
...
router.post("/batch/review", authenticate, requirePermission("ai.governance.review"), validate({ body: batchReviewPostBody }), asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(items) || !decision) { res.status(400).json({ error: "items array and decision required" }); return; }
  ...
  WHERE entity_type = $1 AND entity_id = $2
  RETURNING state_id`,
  [item.entityType, item.entityId, newState, decision]
  );
  if (getFirstRow(result)) {
  updated++;
  eventBus.publish(({ eventType: 'ai.hitl.completed', tenantId, sourceService: 'hitl', severity: 'info', payload: { entityType: item.entityType, entityId: item.entityId, decision: newState, reviewedBy: req.user?.userId } } as any));
  }
  }
...
router.get("/sla-status", authenticate, requirePermission("ai.agent.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const result = await safeQuery(
  `SELECT state_id, entity_type, entity_id, hitl_state, ai_agent_id, confidence, updated_at,
  ...
  [req.tenantId, slaHours]
  );
  const items = result.rows.map(( r: Record<string, unknown>) => ({
  ...r,
  hours_pending: Math.round(parseFloat((r as any).hours_pending) * 10) / 10,
  hours_remaining: Math.round(parseFloat((r as any).hours_remaining) * 10) / 10,
  sla_breached: parseFloat((r as any).hours_remaining) <= 0,
  }));
  const breached = items.filter((i: Record<string, unknown>) => i.sla_breached).length;
...
router.post("/auto-escalate", authenticate, requirePermission("ai.governance.review"), validate({ body: autoEscalatePostBody }), asyncHandler(async (req, res) => {
  ...
  RETURNING state_id, entity_type, entity_id`,
  [slaHours]
  );
  for (const row of result.rows) {
  eventBus.publish(({ eventType: 'ai.hitl.completed', tenantId, sourceService: 'hitl', severity: 'warning', payload: { entityType: row.entity_type, entityId: row.entity_id, decision: 'escalated', reviewedBy: 'system:auto-escalation' } } as any));
  }
