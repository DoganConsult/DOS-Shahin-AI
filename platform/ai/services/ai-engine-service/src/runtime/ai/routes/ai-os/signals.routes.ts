...
import {
  aiOsObservationsIdAcknowledgePostBody, aiOsObservationsIdResolvePostBody, aiOsObservationsIdDismissPostBody,
  aiOsAlertsIdAcknowledgePostBody, aiOsAlertsIdResolvePostBody, aiOsAlertsIdEscalatePostBody, aiOsAlertsIdDismissPostBody,
  aiOsActivityAlertsEvaluatePostBody, aiOsActivityAlertsRulesPostBody, aiOsActivityAlertsAlertIdAcknowledgePostBody,
} from './ai-os-schemas';
...
router.get('/ai-os/observations', authenticate, aiReadLimiter, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  const { agentId, entityType, status, severity, limit, offset } = req.query as Record<string, string | undefined>;
...
router.post('/ai-os/observations/:id/acknowledge', authenticate, aiWriteLimiter, requirePermission('ai.agent.write'), validate({ body: aiOsObservationsIdAcknowledgePostBody }), asyncHandler(async (req, res) => {
  ...
  if (!ok) throw new NotFoundError('Observation', req.params.id);
...
router.get('/ai-os/alerts/system', authenticate, aiReadLimiter, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  ...
  const alerts: Array<{ level: string; source: string; message: string }> = [];
  ...
  if (healthSummary.circuitOpenCount > 0) alerts.push({ level: 'critical', source: 'runtime', message: `${healthSummary.circuitOpenCount} agent circuit(s) OPEN` });
  if (healthSummary.stuckRunCount > 0) alerts.push({ level: 'warning', source: 'runtime', message: `${healthSummary.stuckRunCount} stuck run(s) detected` });
  if (healthSummary.disabledAgents > 0) alerts.push({ level: 'info', source: 'runtime', message: `${healthSummary.disabledAgents} agent(s) disabled` });
  if (healthSummary.failedRunsLast24h > 5) alerts.push({ level: 'warning', source: 'runtime', message: `${healthSummary.failedRunsLast24h} failed runs in last 24h` });
  for (const issue of configIntegrity.issues) alerts.push({ level: 'warning', source: 'config', message: issue });
  for (const c of conflicts) alerts.push({ level: 'warning', source: 'policy', message: `${c.ruleA} \u2194 ${c.ruleB}: ${c.conflict}` });
  setCacheHeaders(res, 15);
...
router.get('/ai-os/alerts', authenticate, aiReadLimiter, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
  const { alertType, severity, status, entityType, limit, offset } = req.query as Record<string, string | undefined>;
...
router.post('/ai-os/alerts/:id/acknowledge', authenticate, aiWriteLimiter, requirePermission('ai.agent.write'), validate({ body: aiOsAlertsIdAcknowledgePostBody }), asyncHandler(async (req, res) => {
  ...
  if (!ok) throw new NotFoundError('Alert', req.params.id);
  setNoCacheHeaders(res);
  res.ok({ success: true });
}));
