...
function getFirstRow<T = any>(result: { rows: T[] }): T | null {
...
export async function getDelegationRules(tenantId: string): Promise<any[]> {
  const schema = tenantSchema(tenantId);
  try {
    const res = await safeQuery(
      `SELECT * FROM "${schema}".delegation_rules WHERE tenant_id = $1`,
      [tenantId],
    );
    return res.rows || [];
  } catch {
...
export async function evaluateDelegationRule(rule: any, context: any): Promise<boolean> {
  if (!rule || rule.allowed === false) return false;
  ...
  if (rule.time_window_start && rule.time_window_end) {
    ...
    if (hhmm < rule.time_window_start || hhmm > rule.time_window_end) return false;
  }
  return true;
}
...
export async function evaluateDelegation(
  ...
  if (userId && userId !== 'system') {
    try {
      ...
      if (shadowResult.rows.length > 0) {
        const cfg: any = getFirstRow(shadowResult);
        if (!cfg.enabled) {
  ...
  try {
    const ruleResult = await safeQuery(
      ...
      [tenantId, userId || 'system', agentId, actionType],
    );

    if (ruleResult.rows.length > 0) {
      const rule: any = getFirstRow(ruleResult);
