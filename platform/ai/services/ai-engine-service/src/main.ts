...
import { setEventBus, toErrorMessage, type PlatformEvent } from '@dos/module-sdk';
...
function initSdkEventBus(serviceCode: string): void {
  ...
  setEventBus({
    publish: async (event: PlatformEvent) => {
      const eventId = await backbone.publish(event.eventType, event.payload, {
        ...
        userId: event.userId,
        idempotencyKey: (event as PlatformEvent & { idempotencyKey?: string; correlationId?: string }).idempotencyKey
          || (event as PlatformEvent & { correlationId?: string }).correlationId,
      });
    ...
    subscribe: (eventType: string, subscriberId: string, handler: (event: PlatformEvent) => Promise<void>) => {
      void subscriberId;
...
async function bootstrap() {
  try {
    ...
    try {
      ...
      logger.warn('Copilot secondary mount skipped', { error: (err as Error).message });
    }
    ...
    try {
      ...
      logger.warn('AI Enhanced secondary mount skipped', { error: (err as Error).message });
    }
    ...
    try {
      ...
      logger.warn('AGRC OS secondary mount skipped', { error: (err as Error).message });
    }
    ...
    try {
      ...
      logger.warn('Quantum Security secondary mount skipped', { error: (err as Error).message });
    }
    ...
    const aiTopLevelMounts: Array<[string, string]> = [
    ...
    for (const [mountPath, modPath] of aiTopLevelMounts) {
      try {
        ...
        logger.warn(`AI top-level mount skipped ${mountPath}`, { error: (err as Error).message });
      }
    }
    ...
    aiOsAggregator.use((req, res, next) => {
      ...
      if (typeof r.ok !== 'function') {
        r.ok = (data: unknown) => res.status(200).json(data);
      }
      if (typeof r.created !== 'function') {
        r.created = (data: unknown) => res.status(201).json(data);
      }
      if (typeof r.deleted !== 'function') {
        r.deleted = (message: string) => res.status(200).json({ message });
      }
      if (typeof r.paginated !== 'function') {
        r.paginated = (items: unknown[], total: number, page: number, pageSize: number) =>
          res.status(200).json({ items, total, page, pageSize });
      }
      next();
    });
    const aiOsMounts: Array<[string, string]> = [
    ...
    for (const [label, mod] of aiOsMounts) {
      try {
        ...
        logger.warn(`AI OS aggregator skipped ${label}`, { error: (err as Error).message });
      }
    }
    ...
    try {
      ...
      setRecordAudit(async (record: any) => {
        await localAudit.recordAudit(
          record.tenantId,
          record.userId || 'system',
          record.action,
          record.entityType,
          record.entityId,
          { module: record.module, beforeState: record.beforeState, afterState: record.afterState },
        );
      });
    } catch (err: unknown) {
    ...
    try {
      ...
      seedShiftsForAllTenants().then((r) =>
        logger.info('[ai-hr] shifts seeded', r),
      ).catch((err) => logger.warn('[ai-hr] seed failed', { error: toErrorMessage(err) }));
      ...
      const onTenantOnboarded = async (event: PlatformEvent) => {
        const tenantId = event.tenantId || (event.payload as Record<string, unknown> | null)?.tenantId as string | undefined;
        if (!tenantId) return;
        try {
    ...
    try {
      ...
      const tbls = await pool.query(
        `SELECT count(*)::int AS n FROM information_schema.tables
          WHERE table_schema LIKE 'tenant_%' AND table_name = 'ai_context_sources'`,
      ).catch(() => null);
      const rows = await pool.query(
        `SELECT COALESCE(SUM(GREATEST(c.reltuples, 0))::bigint, 0) AS n
           FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
          WHERE n.nspname LIKE 'tenant_%' AND c.relname = 'ai_context_sources'`,
      ).catch(() => null);
