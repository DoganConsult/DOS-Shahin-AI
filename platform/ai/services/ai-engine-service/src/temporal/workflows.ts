...
const { checkGovernance } = proxyActivities<typeof activities>({
...
const { recordExecution } = proxyActivities<typeof activities>({
...
export interface AgentInvocationResult {
  ...
  costUsd?: number;
  usedTools?: string[];
  governance: { decision: string; reason: string };
}
 ...
 */
export async function agentInvocationWorkflow(input: AgentInvocationInput): Promise<AgentInvocationResult> {
...
export interface AgentSquadInput {
  tenantId: string;
  agentCodes: string[];
  input: Record<string, unknown>;
 ...
 */
export async function agentSquadWorkflow(input: AgentSquadInput): Promise<AgentInvocationResult[]> {
  let cancelled = false;
  setHandler(cancelSquadSignal, () => { cancelled = true; });

  const inflight = input.agentCodes.map(code =>
    agentInvocationWorkflow({ tenantId: input.tenantId, agentCode: code, input: input.input }),
  );

  const settled = await Promise.race([
    Promise.all(inflight),
    condition(() => cancelled).then(() => [] as AgentInvocationResult[]),
  ]);
