// @ts-nocheck
import type { AiSystem, AiSystemCreateInput } from '@dos/types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): AiSystem {
  return {
...
export function toEntityList(rows: GenericRow[]): AiSystem[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): AiSystemCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as AiSystemCreateInput;
}

export function toApiResponse(entity: AiSystem): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: AiSystem[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
