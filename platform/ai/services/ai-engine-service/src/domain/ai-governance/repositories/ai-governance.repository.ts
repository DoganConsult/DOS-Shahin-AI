...
export class AiGovernanceRepository {
  ...
  async findById(id: string): Promise<GenericRow | null> {
    const result = await safeQuery(
      `SELECT * FROM "${this.schema}".ai_governance_systems WHERE system_id = $1 AND deleted_at IS NULL`, [id]);
  ...
  async findAll(filters: { status?: string; search?: string; page?: number; pageSize?: number; sortBy?: string; sortDir?: string } = {}): Promise<{ rows: GenericRow[]; total: number }> {
    ...
    const dataResult = await safeQuery(
      `SELECT * FROM "${this.schema}".ai_governance_systems ${where} ORDER BY ${sortCol} ${sortDir} LIMIT $${idx++} OFFSET $${idx++}`,
  ...
  async create(data: Record<string, unknown>): Promise<GenericRow | null> {
    ...
    for (const k of keys) { if (k !== 'system_id') { cols.push(k); vals.push(data[k]); } }
    const placeholders = vals.map((_, i) => `$${i + 1}`).join(',');
    const result = await safeQuery(
      `INSERT INTO "${this.schema}".ai_governance_systems (${cols.join(',')}) VALUES (${placeholders}) RETURNING *`, vals);
    return getFirstRow(result);
  }
  ...
  async update(id: string, data: Record<string, unknown>): Promise<GenericRow | null> {
    ...
    params.push(id);
    const result = await safeQuery(
      `UPDATE "${this.schema}".ai_governance_systems SET ${sets.join(', ')} WHERE system_id = $${idx} AND deleted_at IS NULL RETURNING *`, params);
    return getFirstRow(result);
  }

  async softDelete(id: string, deletedBy?: string): Promise<boolean> {
    const result = await safeQuery(
      `UPDATE "${this.schema}".ai_governance_systems SET deleted_at = NOW(), deleted_by = $2, updated_at = NOW() WHERE system_id = $1 AND deleted_at IS NULL RETURNING system_id`,
  ...
  async count(filters: { status?: string } = {}): Promise<number> {
    ...
    if (filters.status) { conditions.push(`status = $${idx++}`); params.push(filters.status); }
    const result = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${this.schema}".ai_governance_systems WHERE ${conditions.join(' AND ')}`, params);
