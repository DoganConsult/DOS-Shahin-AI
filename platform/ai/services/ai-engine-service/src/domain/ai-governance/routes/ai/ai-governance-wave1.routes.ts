...
/**
 * AI Governance Wave 1 Routes
 * - Alert Rules & History (W1.1)
 * - Kill Switches (W1.2)
 * - Agent Runtime Stats (W1.3)
 * - Model Metrics / Drift (W1.4)
 * - Maturity Scorecard (W1.5)
 * - Board Summary (W1.6)
 */
...
// ── Zod Schemas ──────────────────────────────────────────────────────────
import { validate, requireOwnership as _requireOwnership, auditMiddleware, setAuditData, automationMiddleware, moduleStack, mutationEventHook } from '../../ports/middleware.port';
import { alertRulesPostBody, alertRulesIdPutBody, alertHistoryIdAcknowledgePostBody, killSwitchesPostBody, killSwitchesIdTestPostBody, killSwitchesIdActivatePostBody, modelMetricsPostBody, driftThresholdsPutBody } from "../../schemas/ai-governance.schemas";
import { z } from "zod";
...
async function tq(tenantId: string, sql: string, params?: unknown[]) {
...
router.get("/alert-rules", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { enabled, limit = 100, offset = 0 } = req.query;
    let sql = `SELECT * FROM ai_alert_rules WHERE tenant_id = $1`;
    const params: unknown[] = [tenantId];
    if (enabled !== undefined) { params.push(enabled === 'true'); sql += ` AND enabled = $${params.length}`; }
...
router.post("/alert-rules", authenticate, requirePermission("ai_governance.manage"), validate({ body: alertRulesPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const { rows } = await tq(tenantId,
      `INSERT INTO ai_alert_rules (tenant_id, rule_name, description, trigger_condition, channels, escalation_chain, enabled, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [tenantId, rule_name, description || null, JSON.stringify(trigger_condition || {}), JSON.stringify(channels || []), JSON.stringify(escalation_chain || []), enabled !== false, userId]
    );
    setAuditData(res as any, { action: "create", entityType: "ai_alert_rule", entityId: rows[0].rule_id, afterState: rows[0] });
...
router.put("/alert-rules/:id", authenticate, requirePermission("ai_governance.manage"), validate({ body: alertRulesIdPutBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
    const { rule_name, description, trigger_condition, channels, escalation_chain, enabled, snooze_until } = req.body;
    const { rows } = await tq(tenantId,
      `UPDATE ai_alert_rules SET rule_name=COALESCE($2,rule_name), description=COALESCE($3,description),
       ...
       snooze_until=$8, updated_at=NOW() WHERE rule_id=$1 AND tenant_id=$9 RETURNING *`,
      [id, rule_name, description, trigger_condition ? JSON.stringify(trigger_condition) : null, channels ? JSON.stringify(channels) : null, escalation_chain ? JSON.stringify(escalation_chain) : null, enabled, snooze_until || null, tenantId]
    );
    if (!rows.length) return res.status(404).json({ error: "Rule not found" });
    setAuditData(res as any, { action: "update", entityType: "ai_alert_rule", entityId: id, afterState: rows[0] });
...
router.delete("/alert-rules/:id", validate({ body: genericPayloadSchema }), authenticate, requirePermission("ai_governance.manage"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
    await tq(tenantId, `DELETE FROM ai_alert_rules WHERE rule_id=$1 AND tenant_id=$2`, [id, tenantId]);
    setAuditData(res as any, { action: "delete", entityType: "ai_alert_rule", entityId: id });
...
// Alert History
router.get("/alert-history", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { limit = 50, offset = 0, rule_id } = req.query;
    let sql = `SELECT * FROM ai_alert_history WHERE tenant_id = $1`;
    const params: unknown[] = [tenantId];
    if (rule_id) { params.push(rule_id); sql += ` AND rule_id = $${params.length}`; }
...
router.post("/alert-history/:id/acknowledge", authenticate, requirePermission("ai_governance.manage"), validate({ body: alertHistoryIdAcknowledgePostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { id } = req.params;
    const { rows } = await tq(tenantId,
      `UPDATE ai_alert_history SET acknowledged=TRUE, acknowledged_at=NOW(), acknowledged_by=$2 WHERE alert_id=$1 AND tenant_id=$3 RETURNING *`,
      [id, userId, tenantId]
    );
...
router.get("/kill-switches", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { rows } = await tq(tenantId,
      `SELECT * FROM ai_kill_switches WHERE tenant_id=$1 ORDER BY created_at DESC`, [tenantId]);
...
router.post("/kill-switches", authenticate, requirePermission("ai_governance.manage"), validate({ body: killSwitchesPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { asset_id, asset_name, asset_type, kill_switch_type, trigger_method, fallback_procedure } = req.body;
    if (!asset_name || !asset_type) return res.status(400).json({ error: "asset_name and asset_type required" });
    const { rows } = await tq(tenantId,
      `INSERT INTO ai_kill_switches (tenant_id, asset_id, asset_name, asset_type, kill_switch_type, trigger_method, fallback_procedure, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [tenantId, asset_id || null, asset_name, asset_type, kill_switch_type || 'api_disable', trigger_method || null, fallback_procedure || null, userId]
    );
    setAuditData(res as any, { action: "create", entityType: "ai_kill_switch", entityId: rows[0].kill_switch_id, afterState: rows[0] });
    return res.status(201).json(rows[0]);
  } catch (_err: unknown) {
...
router.post("/kill-switches/:id/test", authenticate, requirePermission("ai_governance.manage"), validate({ body: killSwitchesIdTestPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
    // Dry-run: update last_tested_at without actually activating
    const { rows } = await tq(tenantId,
      `UPDATE ai_kill_switches SET last_tested_at=NOW(), updated_at=NOW() WHERE kill_switch_id=$1 AND tenant_id=$2 RETURNING *`,
      [id, tenantId]
    );
    if (!rows.length) return res.status(404).json({ error: "Kill switch not found" });
    setAuditData(res as any, { action: "update", entityType: "ai_kill_switch", entityId: id, afterState: { tested: true } });
...
router.post("/kill-switches/:id/activate", authenticate, requirePermission("ai_governance.manage"), validate({ body: killSwitchesIdActivatePostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { id } = req.params;
    ...
    if (!enforcement.success) {
      const r = await tq(tenantId,
        `UPDATE ai_kill_switches SET status='activated', last_activated_at=NOW(), activated_by=$2, updated_at=NOW()
    ...
    const ks = rows[0] as Record<string, unknown>;

    if (ks.asset_id) {
      try {
        // Attempt to suspend active versions for this asset
        await tq(tenantId,
          `UPDATE ai_model_registry SET deployment_status='suspended', updated_at=NOW()

           WHERE asset_id=$1 AND tenant_id=$2 AND deployment_status='active'`, [ks.asset_id, tenantId]);
        await tq(tenantId,
          `UPDATE ai_agent_registry SET deployment_status='suspended', updated_at=NOW()

           WHERE asset_id=$1 AND tenant_id=$2 AND deployment_status='active'`, [ks.asset_id, tenantId]);
        await tq(tenantId,
          `UPDATE ai_prompt_registry SET deployment_status='suspended', updated_at=NOW()

           WHERE asset_id=$1 AND tenant_id=$2 AND deployment_status='active'`, [ks.asset_id, tenantId]);
      } catch { /* tables may not all exist for this asset type */ }
    }
    setAuditData(res as any, { action: "activate", entityType: "ai_kill_switch", entityId: id, afterState: { status: "activated", activated_by: userId } });
...
router.delete("/kill-switches/:id", validate({ body: genericPayloadSchema }), authenticate, requirePermission("ai_governance.manage"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
...
router.get("/agent-runtime-stats/:agentAssetId", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const statsResult = await tq(tenantId,
      `SELECT
    ...
    // Recent action log
    const logResult = await tq(tenantId,
    ...
    const actionLog = logResult.rows.map(( r: Record<string, unknown>) => ({
...
router.get("/model-metrics/:versionId", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { versionId } = req.params;
    const { metric_type, limit = 100 } = req.query;
    let sql = `SELECT * FROM ai_model_metrics WHERE tenant_id=$1 AND version_id=$2`;
    const params: unknown[] = [tenantId, versionId];
    if (metric_type) { params.push(metric_type); sql += ` AND metric_type=$${params.length}`; }
...
router.post("/model-metrics", authenticate, requirePermission("ai.governance.write"), validate({ body: modelMetricsPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { version_id, asset_id, metric_type, value, metadata } = req.body;
    if (!version_id || !metric_type || value === undefined) return res.status(400).json({ error: "version_id, metric_type, value required" });
    const { rows } = await tq(tenantId,
      `INSERT INTO ai_model_metrics (tenant_id, version_id, asset_id, metric_type, value, metadata)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [tenantId, version_id, asset_id || null, metric_type, value, JSON.stringify(metadata || {})]
    );

    // Check drift thresholds
    if (asset_id) {
      const thresholds = await tq(tenantId,
        `SELECT * FROM ai_drift_thresholds WHERE tenant_id=$1 AND asset_id=$2 AND metric_type=$3 AND enabled=TRUE`,
        [tenantId, asset_id, metric_type]
      );
        ...
        if (t.baseline_value !== null) {
          ...
          if (delta >= t.critical_delta) {
            ...
            try {
              await tq(tenantId,
                `INSERT INTO agrc_event_log (tenant_id, event_type, severity, details, created_at)
                 VALUES ($1, 'model.drift_detected', 'critical', $2, NOW())`,
                [tenantId, JSON.stringify({ asset_id, version_id, metric_type, value, baseline: t.baseline_value, delta, level: 'critical' })]
              );
            ...
            try {
              await tq(tenantId,
                `INSERT INTO agrc_event_log (tenant_id, event_type, severity, details, created_at)
                 VALUES ($1, 'model.drift_detected', 'warning', $2, NOW())`,
                [tenantId, JSON.stringify({ asset_id, version_id, metric_type, value, baseline: t.baseline_value, delta, level: 'warning' })]
              );
            } catch { /* event log may not exist */ }
          }
        }
...
// Drift thresholds
router.get("/drift-thresholds/:assetId", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { assetId } = req.params;
    const { rows } = await tq(tenantId,
      `SELECT * FROM ai_drift_thresholds WHERE tenant_id=$1 AND asset_id=$2 ORDER BY metric_type`, [tenantId, assetId]);
...
router.put("/drift-thresholds", authenticate, requirePermission("ai_governance.manage"), validate({ body: driftThresholdsPutBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { asset_id, metric_type, warning_delta, critical_delta, baseline_value, enabled } = req.body;
    if (!asset_id || !metric_type) return res.status(400).json({ error: "asset_id and metric_type required" });
    const { rows } = await tq(tenantId,
      `INSERT INTO ai_drift_thresholds (tenant_id, asset_id, metric_type, warning_delta, critical_delta, baseline_value, enabled)
       ...
       ON CONFLICT (tenant_id, asset_id, metric_type) DO UPDATE SET
         ...
         updated_at=NOW()
       RETURNING *`,
      [tenantId, asset_id, metric_type, warning_delta ?? 0.05, critical_delta ?? 0.10, baseline_value ?? null, enabled ?? true]
    );
    return res.json(rows[0]);
  } catch (_err: unknown) {
...
router.get("/maturity-scorecard", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const score = (result: PromiseSettledResult<any>, max: number = 5): number => {
      if (result.status !== 'fulfilled') return 1;
      const r = result.value?.rows?.[0];
      if (!r || !r.total || parseInt(r.total) === 0) return 1;
...
router.get("/board-summary", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const safe = (r: PromiseSettledResult<any>) => r.status === 'fulfilled' ? r.value.rows : [];
