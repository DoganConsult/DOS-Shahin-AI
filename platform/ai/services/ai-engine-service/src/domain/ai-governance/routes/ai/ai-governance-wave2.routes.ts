...
/**
 * AI Governance Wave 2 Routes
 * - Bias & Fairness Scanner (W2.1)
 * - EU AI Act Classification (W2.2)
 * - Red Team Scheduling (W2.3)
 * - Ethics Review Board (W2.4)
 * - AI Impact Assessment (W2.5)
 * - Regulatory Change Intelligence (W2.6)
 */
...
function sq(tenantId: string, sql: string, params?: unknown[]) {
...
router.get("/fairness/metrics", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { model_asset_id, limit = 100 } = req.query;
    let sql = `SELECT * FROM ai_fairness_metrics WHERE tenant_id = $1`;
    const params: unknown[] = [tenantId];
    if (model_asset_id) { params.push(model_asset_id); sql += ` AND model_asset_id = $${params.length}`; }
...
router.get("/fairness/scans", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { rows } = await sq(tenantId,
      `SELECT * FROM ai_fairness_scans WHERE tenant_id=$1 ORDER BY started_at DESC LIMIT 20`, [tenantId]);
...
router.post("/fairness/scan", authenticate, requirePermission("ai_governance.manage"), validate({ body: fairnessScanPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { model_asset_id, model_name } = req.body;
    if (!model_asset_id) return res.status(400).json({ error: "model_asset_id required" });

    // Create scan record
    const { rows: scanRows } = await sq(tenantId,
      `INSERT INTO ai_fairness_scans (tenant_id, model_asset_id, model_name, status, triggered_by)
       VALUES ($1,$2,$3,'completed',$4) RETURNING *`,
      [tenantId, model_asset_id, model_name || 'Unknown', userId]);
    ...
    for (const def of metricDefs) {
      ...
      const existing = await sq(tenantId,
        `SELECT value FROM ai_model_metrics WHERE tenant_id=$1 AND asset_id=$2 AND metric_type=$3 ORDER BY recorded_at DESC LIMIT 1`,
        [tenantId, model_asset_id, def.name]);
      ...
      const { rows } = await sq(tenantId,
        `INSERT INTO ai_fairness_metrics (tenant_id, model_asset_id, model_name, metric_name, value, threshold, status, scan_id)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
        [tenantId, model_asset_id, model_name || 'Unknown', def.name, value ?? 0, def.threshold, status, scan.scan_id]);
    ...
    await sq(tenantId,
      `UPDATE ai_fairness_scans SET completed_at=NOW(), summary=$2 WHERE scan_id=$1 AND tenant_id=$3`,
      [scan.scan_id, JSON.stringify({ total: metrics.length, pass: passCount, fail: failCount }), tenantId]);

    // Emit event if any metric failed
    if (failCount > 0) {
      try {
        await sq(tenantId,
          `INSERT INTO agrc_event_log (tenant_id, event_type, severity, details, created_at)
           VALUES ($1, 'bias.threshold_breached', 'warning', $2, NOW())`,
          [tenantId, JSON.stringify({ model_asset_id, model_name, failed_metrics: failCount })]);
      } catch { /* event log may not exist */ }
    }

    setAuditData(res as any, { action: "create", entityType: "ai_fairness_scan", entityId: scan.scan_id, afterState: { metrics_count: metrics.length, fail_count: failCount } });
...
router.get("/eu-ai-act/questions", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (_req: Request, res: Response) => {
  return res.json({ questions: EU_AI_ACT_QUESTIONS });
});

router.get("/eu-ai-act/classifications", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { model_id } = req.query;
    let sql = `SELECT * FROM ai_eu_classifications WHERE tenant_id=$1`;
    const params: unknown[] = [tenantId];
    if (model_id) { params.push(model_id); sql += ` AND model_id=$${params.length}`; }
...
router.post("/eu-ai-act/classify", authenticate, requirePermission("ai_governance.manage"), validate({ body: euAiActClassifyPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { model_id, model_name, answers } = req.body;
    if (!model_id || !answers) return res.status(400).json({ error: "model_id and answers required" });

    // Determine risk category
    let risk_category = 'minimal';
    const requirements: string[] = [];
    const answerObj = answers as Record<string, boolean>;

    // Check unacceptable
    for (const q of EU_AI_ACT_QUESTIONS) {
      if ((q as Record<string, unknown>).unacceptable && answerObj[q.id] === true) {
    ...
    if (risk_category !== 'unacceptable') {
      for (const q of EU_AI_ACT_QUESTIONS) {
        if ((q as Record<string, unknown>).high_risk && answerObj[q.id] === true) {
    ...
    const { rows } = await sq(tenantId,
      `INSERT INTO ai_eu_classifications (tenant_id, model_id, model_name, risk_category, answers, requirements, classified_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [tenantId, model_id, model_name || 'Unknown', risk_category, JSON.stringify(answers), JSON.stringify(requirements), userId]);

    const classification = getFirstRowOrThrow({ rows }, 'Classification insert failed');
    setAuditData(res as any, { action: "create", entityType: "ai_eu_classification", entityId: classification.classification_id, afterState: { risk_category } });
...
router.get("/red-team/schedules", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { rows } = await sq(tenantId,
      `SELECT * FROM ai_red_team_schedules WHERE tenant_id=$1 ORDER BY created_at DESC`, [tenantId]);
...
router.post("/red-team/schedules", authenticate, requirePermission("ai_governance.manage"), validate({ body: redTeamSchedulesPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    ...
    const freqHours: Record<string, number> = { daily: 24, weekly: 168, monthly: 720 };
    const nextRun = new Date(Date.now() + (freqHours[frequency || 'weekly'] || 168) * 3600000);

    const { rows } = await sq(tenantId,
      `INSERT INTO ai_red_team_schedules (tenant_id, name, model_id, prompt_template, frequency, enabled, next_run_at, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [tenantId, name, model_id || null, prompt_template || 'all', frequency || 'weekly', enabled !== false, nextRun, userId]);

    const schedule = getFirstRowOrThrow({ rows }, 'Schedule creation failed');
    setAuditData(res as any, { action: "create", entityType: "ai_red_team_schedule", entityId: schedule.schedule_id });
...
router.put("/red-team/schedules/:id", authenticate, requirePermission("ai_governance.manage"), validate({ body: redTeamSchedulesIdPutBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
    const { name, model_id, prompt_template, frequency, enabled } = req.body;
    const { rows } = await sq(tenantId,
      `UPDATE ai_red_team_schedules SET name=COALESCE($2,name), model_id=COALESCE($3,model_id),
       prompt_template=COALESCE($4,prompt_template), frequency=COALESCE($5,frequency), enabled=COALESCE($6,enabled)
       WHERE schedule_id=$1 AND tenant_id=$7 RETURNING *`,
      [id, name, model_id, prompt_template, frequency, enabled, tenantId]);
...
router.delete("/red-team/schedules/:id", validate({ body: genericPayloadSchema }), authenticate, requirePermission("ai_governance.manage"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
...
router.get("/ethics-reviews", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const params: unknown[] = [tenantId];
    if (status) { params.push(status); sql += ` AND r.decision=$${params.length}`; }
...
router.post("/ethics-reviews", authenticate, requirePermission("ai_governance.manage"), validate({ body: ethicsReviewsPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { system_name, system_type, description, risk_category, assessment_data } = req.body;
    if (!system_name) return res.status(400).json({ error: "system_name required" });
    const { rows } = await sq(tenantId,
      `INSERT INTO ai_ethics_reviews (tenant_id, system_name, system_type, description, risk_category, assessment_data, submitted_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [tenantId, system_name, system_type || null, description || null, risk_category || null, JSON.stringify(assessment_data || {}), userId]);
    const review = getFirstRowOrThrow({ rows }, 'Ethics review creation failed');
    setAuditData(res as any, { action: "create", entityType: "ai_ethics_review", entityId: review.review_id });
...
router.post("/ethics-reviews/:id/vote", authenticate, requirePermission("ai_governance.manage"), validate({ body: ethicsReviewsIdVotePostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { id } = req.params;
    ...
    const existing = await sq(tenantId,
      `SELECT vote_id FROM ai_ethics_votes WHERE review_id=$1 AND voter_id=$2 AND tenant_id=$3`, [id, userId, tenantId]);
    if (existing.rows.length > 0) return res.status(409).json({ error: "Already voted on this review" });

    const { rows } = await sq(tenantId,
      `INSERT INTO ai_ethics_votes (tenant_id, review_id, voter_id, vote, notes) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [tenantId, id, userId, vote, notes || null]);

    const voteRow = getFirstRowOrThrow({ rows }, 'Vote creation failed');
    setAuditData(res as any, { action: "create", entityType: "ai_ethics_vote", entityId: voteRow.vote_id, afterState: { review_id: id, vote } });
...
router.post("/ethics-reviews/:id/decide", authenticate, requirePermission("ai_governance.manage"), validate({ body: ethicsReviewsIdDecidePostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
    ...
    const { rows } = await sq(tenantId,
      `UPDATE ai_ethics_reviews SET decision=$2, conditions=$3, decided_at=NOW() WHERE review_id=$1 AND tenant_id=$4 RETURNING *`,
      [id, decision, JSON.stringify(conditions || []), tenantId]);
    if (!rows.length) return res.status(404).json({ error: "Review not found" });
    setAuditData(res as any, { action: "update", entityType: "ai_ethics_review", entityId: id, afterState: { decision } });
...
router.get("/impact-assessments", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { status, limit = 50 } = req.query;
...
router.post("/impact-assessments", authenticate, requirePermission("ai_governance.manage"), validate({ body: impactAssessmentsPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    ...
    if (!system_name) return res.status(400).json({ error: "system_name required" });
    const { rows } = await sq(tenantId,
      `INSERT INTO ai_impact_assessments (tenant_id, system_name, model_asset_id, assessor_id, steps_data, status)
       VALUES ($1,$2,$3,$4,$5,'draft') RETURNING *`,
      [tenantId, system_name, model_asset_id || null, userId, JSON.stringify(steps_data || {})]);
    const assessment = getFirstRowOrThrow({ rows }, 'Impact assessment creation failed');
    setAuditData(res as any, { action: "create", entityType: "ai_impact_assessment", entityId: assessment.assessment_id });
...
router.put("/impact-assessments/:id", authenticate, requirePermission("ai_governance.manage"), validate({ body: impactAssessmentsIdPutBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { id } = req.params;
    ...
    const { rows } = await sq(tenantId,
      `UPDATE ai_impact_assessments SET
       ...
       WHERE assessment_id=$1 AND tenant_id=$6 RETURNING *`,
      [id, steps_data ? JSON.stringify(steps_data) : null, status, impact_score, recommendation, tenantId]);
...
router.delete("/impact-assessments/:id", validate({ body: genericPayloadSchema }), authenticate, requirePermission("ai_governance.manage"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
...
router.get("/regulatory-changes", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
...
router.post("/regulatory-changes", authenticate, requirePermission("ai_governance.manage"), validate({ body: regulatoryChangesPostBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const { rows } = await sq(tenantId,
      `INSERT INTO ai_regulatory_changes (tenant_id, source, title, description, framework_code, severity, affected_controls, recommended_action)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [tenantId, source, title, description || null, framework_code || null, severity || 'medium',
    ...
    setAuditData(res as any, { action: "create", entityType: "ai_regulatory_change", entityId: change.change_id });
...
router.put("/regulatory-changes/:id/review", authenticate, requirePermission("ai_governance.manage"), validate({ body: regulatoryChangesIdReviewPutBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const userId = req.user!.userId;
    const { id } = req.params;
    ...
    if (enforcement.blocked) return res.status(403).json({ error: 'Transition denied', reason: enforcement.reason });
    if (enforcement.pendingApproval) return res.status(202).json({ pendingApproval: true, approvalId: enforcement.approvalId, reason: enforcement.reason });
    let rows: unknown[];
    if (!enforcement.success) {
      const r = await sq(tenantId,
        `UPDATE ai_regulatory_changes SET status=$2, reviewed_by=$3, reviewed_at=NOW()
    ...
    setAuditData(res as any, { action: "update", entityType: "ai_regulatory_change", entityId: id, afterState: { status } });
