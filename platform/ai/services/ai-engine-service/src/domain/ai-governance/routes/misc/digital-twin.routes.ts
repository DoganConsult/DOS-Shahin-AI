...
router.post("/", authenticate, requirePermission("ai.governance.write"), validate({ body: rootPostBody }), asyncHandler(async (req, res) => {
  ...
  setAuditData(res as any, { action: "create", entityType: "simulation", entityId: sim.simulation_id, afterState: sim });

  swallow(EC.EVENT_BUS, emitEvent(({ tenantId: req.tenantId, userId: req.user!.userId, module: 'ai-governance', event: 'digital_twin.created', entityType: 'digital_twin', entityId: sim.simulation_id || '' } as any)), { tenantId: req.tenantId, operation: 'grcEvent:risks.digital_twin.created' });
...
router.post("/scenario-templates/:code/execute", authenticate, requirePermission("ai.governance.write"), validate({ body: createExecuteBody }), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    setAuditData(res as any, { action: "create", entityType: "scenario_execution", entityId: result.simulation_id, afterState: result });
    swallow(EC.EVENT_BUS, emitEvent(({ tenantId: req.tenantId, userId: req.user!.userId, module: 'ai-governance', event: 'digital_twin.created', entityType: 'scenario_execution', entityId: result.simulation_id } as any)), { tenantId: req.tenantId, operation: 'grcEvent:risks.scenario_execution.created' });
...
router.post("/:id/change", authenticate, requirePermission("ai.governance.write"), validate({ body: idChangePostBody }), asyncHandler(async (req, res) => {
  ...
  if (!type || !entityId || !changes) { res.status(400).json({ error: "type, entityId, changes required" }); return; }
  const sim = await applyChange(req.tenantId, id, { type, entityId, changes, cascade: cascade === true });
  swallow(EC.EVENT_BUS, emitEvent(({ tenantId: req.tenantId, userId: req.user!.userId, module: 'ai-governance', event: 'digital_twin.updated', entityType: 'digital_twin', entityId: id } as any)), { tenantId: req.tenantId, operation: 'grcEvent:risks.digital_twin.updated' });
  res.json(sim);
}));
...
router.delete("/:id", authenticate, requirePermission("ai_governance.manage"), validate({ body: genericPayloadSchema }), asyncHandler(async (req, res) => {
  ...
  setAuditData(res as any, { action: "delete", entityType: "simulation", entityId: id });
  swallow(EC.EVENT_BUS, emitEvent(({ tenantId: req.tenantId, userId: req.user!.userId, module: 'ai-governance', event: 'digital_twin.deleted', entityType: 'digital_twin', entityId: req.params.id || '' } as any)), { tenantId: req.tenantId, operation: 'grcEvent:risks.digital_twin.deleted' });
...
router.post("/:id/org-impact", authenticate, requirePermission("ai.governance.write"), validate({ body: createOrgImpactBody }), asyncHandler(async (req, res) => {
  ...
  setAuditData(res as any, { action: "analyze", entityType: "org_structure_impact", entityId: id, afterState: analysis });
  swallow(EC.EVENT_BUS, emitEvent(({ tenantId: req.tenantId, userId: req.user!.userId, module: 'ai-governance', event: 'digital_twin.updated', entityType: 'digital_twin', entityId: id } as any)), { tenantId: req.tenantId, operation: 'grcEvent:risks.digital_twin.updated' });
  res.json(analysis);
}));
