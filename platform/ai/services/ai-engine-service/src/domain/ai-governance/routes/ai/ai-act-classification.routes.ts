...
import {
  classifyAiSystem,
  createAiImpactAssessment,
  getAiImpactAssessment,
  scoreAiImpactAssessment,
  ensureAiImpactAssessmentsTable,
} from '../../services/ai/compliance/ai-act-classification.service';
import { z } from "zod";
...
router.post(
  '/models/:modelId/classify',
  authenticate,
  requirePermission('ai.governance.write'),
  validate({ body: createClassifyBody }),
...
router.post(
  '/models/:modelId/impact-assessment',
  authenticate,
  requirePermission('ai.governance.write'),
  validate({ body: createImpactAssessmentBody }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
...
router.get(
  '/impact-assessments/:assessmentId',
  authenticate,
  requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
...
router.post(
  '/impact-assessments/:assessmentId/score',
  authenticate,
  requirePermission('ai.governance.write'),
