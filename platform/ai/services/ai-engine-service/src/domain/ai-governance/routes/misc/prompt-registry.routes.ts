...
import {
  ...
  updateDraftPromptVersion,
  submitPromptVersionForApproval,
  approvePromptVersion,
  rejectPromptVersion,
  activatePromptVersion,
  suspendPromptVersion,
  retirePromptVersion,
...
} from '../../services/misc/prompt-registry.service';
...
router.get(
  ...
  requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    asset_type: 'prompt' as AssetType,
    limit,
    offset,
    });
...
router.get(
  ...
  requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    if (!version) { res.status(404).json({ error: "Prompt version not found" }); return; }
    res.json(version);
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(400).json({ error: "asset_id and template_text are required" }); return;
    }
    ...
    setAuditData(res as any, { action: "create", entityType: "prompt_version", entityId: version.prompt_version_id, afterState: version });
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    setAuditData(res as any, { action: "update", entityType: "prompt_version", entityId: req.params.versionId, afterState: version });
    res.json(version);
    } catch (err: unknown) {
...
router.post(
  ...
  authenticate,
  requireAnyPermission("ai.prompt.approve", "ai.agent.approve"),
...
router.delete(
  ...
  requirePermission("ai_governance.manage"), validate({ body: genericPayloadSchema }), asyncHandler(async (req, res) => {
    ...
    if (!deleted) { res.status(404).json({ error: "Prompt version not found" }); return; }

    setAuditData(res as any, { action: "delete", entityType: "prompt_version", entityId: req.params.versionId });
    res.json({ deleted: true, prompt_version_id: req.params.versionId });
    } catch (err: unknown) {
    if (toErrorMessage(err).includes('active version')) { res.status(422).json({ error: toErrorMessage(err) }); return; }
