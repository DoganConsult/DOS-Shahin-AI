...
export interface AiImpactAssessment {
  ...
  sections: AiiaSection[];
  overall_risk_score: number;
  recommendations: string[];
}

export interface AiiaSection {
  section_code: string;
  title_en: string;
  title_ar: string;
  questions: AiiaQuestion[];
}

export interface AiiaQuestion {
  code: string;
  question_en: string;
  question_ar: string;
  answer: string | null;
  risk_impact: 'high' | 'medium' | 'low' | null;
}
...
const ARTICLE_DEFINITIONS: Record<AiActRiskLevel, { articles: string[]; description: string }> = {
  unacceptable: {
    articles: ['Article 5'],
...
const HIGH_RISK_REQUIRED_ACTIONS: RequiredAction[] = [
  {
    article: 'Article 9',
...
const LIMITED_RISK_REQUIRED_ACTIONS: RequiredAction[] = [
  {
...
const AIIA_SECTIONS: AiiaSection[] = [
  {
...
export async function classifyAiSystem(
  ...
  const modelResult = await safeQuery(
    `SELECT asset_id, asset_key, display_name, description, metadata, tags
  ...
  let riskLevel: AiActRiskLevel = 'minimal';
 ...
 */
export async function createAiImpactAssessment(
  tenantId: string,
  modelId: string,
  userId: string,
  ...
  // Verify model exists
  const modelResult = await safeQuery(
    `SELECT asset_id FROM "${schema}".ai_asset_inventory WHERE asset_id = $1`,
    [modelId],
  );
  ...
  const sections: AiiaSection[] = JSON.parse(JSON.stringify(AIIA_SECTIONS));

  // Classify the model to attach classification metadata
  let classificationJson: any = null;
  try {
  ...
  await safeQuery(
    `INSERT INTO "${schema}".ai_impact_assessments
 ...
 */
export async function getAiImpactAssessment(
  tenantId: string,
  assessmentId: string,
): Promise<AiImpactAssessment | null> {
  const schema = tenantSchema(tenantId);

  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_impact_assessments WHERE assessment_id = $1`,
    [assessmentId],
  );
  ...
  const sections: AiiaSection[] = typeof row.sections_json === 'string'
    ? JSON.parse(row.sections_json)
    : row.sections_json || [];
  const recommendations: string[] = typeof row.recommendations_json === 'string'
 ...
 */
export async function scoreAiImpactAssessment(
  tenantId: string,
  assessmentId: string,
): Promise<{ risk_score: number; recommendations: string[] }> {
  const schema = tenantSchema(tenantId);
  ...
  const sectionRisks: { code: string; title: string; risk: number }[] = [];
  ...
  for (const section of sections) {
    ...
    if (sectionScore >= 70) {
      recommendations.push(
        `High risk identified in "${section.title_en}" (${section.title_ar}). ` +
        `Immediate mitigation actions are recommended.`,
      );
    } else if (sectionScore >= 50) {
      recommendations.push(
        `Moderate risk in "${section.title_en}". Review and strengthen controls.`,
      );
    }
  }
  ...
  if (answeredCount < totalQuestions * 0.5) {
    recommendations.unshift(
      `Only ${answeredCount} of ${totalQuestions} questions answered. ` +
  ...
  else if (riskScore >= 50) riskLevel = 'limited';
  else riskLevel = 'minimal';

  // Update the assessment record with the computed score
  await safeQuery(
    `UPDATE "${schema}".ai_impact_assessments
     SET overall_risk_score = $1,
         risk_level = $2,
         recommendations_json = $3,
         updated_at = NOW()
     WHERE assessment_id = $4`,
    [riskScore, riskLevel, JSON.stringify(recommendations), assessmentId],
  );
 ...
 */
async function checkExistingCompliance(
  schema: string,
  modelId: string,
  _requiredActions: RequiredAction[],
): Promise<string[]> {
  const gaps: string[] = [];

  // Check if an impact assessment already exists for this model
  const existingResult = await safeQuery(
    `SELECT assessment_id, status, overall_risk_score, sections_json
  ...
  const sections: AiiaSection[] = typeof existing.sections_json === 'string'
 ...
 */
export async function ensureAiImpactAssessmentsTable(tenantId: string): Promise<void> {
  const schema = tenantSchema(tenantId);
  await safeQuery(`
