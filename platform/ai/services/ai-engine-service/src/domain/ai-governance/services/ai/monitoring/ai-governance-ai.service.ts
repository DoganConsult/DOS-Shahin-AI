export const AI_GOVERNANCE_AI_CONFIG = {
  moduleCode: 'ai-governance' as const,
  enabled: true,
...
export function isAiGovernanceAiActionAllowed(action: string): boolean {
  return (AI_GOVERNANCE_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isAiGovernanceAiActionBlocked(action: string): boolean {
  return (AI_GOVERNANCE_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresAiGovernanceHumanApproval(action: string): boolean {
  return (AI_GOVERNANCE_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_systems WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'AI Governance entity not found.', keyPoints: [], confidence: 0 };
  return {
