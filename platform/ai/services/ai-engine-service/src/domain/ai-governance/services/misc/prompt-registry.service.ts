...
import { getAssetById, getAssetByKey } from '../../runtime/ai/registry/ai-asset-inventory.service';
import {
  type ApprovalStatus,
...
async function loadParentPromptAsset(tenantId: string, assetId: string): Promise<unknown> {
  return assertParentAssetValid(tenantId, assetId, ASSET_TYPE_LABEL);
}

async function assertLinkedModelValid(tenantId: string, linkedId: string): Promise<void> {
  return assertLinkedAssetValid(tenantId, linkedId, 'model', 'model');
}

async function getVersionRow(schema: string, versionId: string): Promise<PromptVersion | null> {
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_prompt_registry WHERE prompt_version_id = $1`,
    [versionId],
  );
  return (getFirstRow(result) as PromptVersion) || null;
}

async function getPreviousVersion(schema: string, assetId: string, currentVersionNum: number): Promise<PromptVersion | null> {
  const result = await safeQuery(
    `SELECT * FROM "${schema}".ai_prompt_registry
...
export function computeDiffSummary(
  ...
  if (prevText === null) {
    ...
    for (let i = 0; i < maxLen; i++) {
      if (i >= prevLines.length) { added++; continue; }
      if (i >= newLines.length) { removed++; continue; }
      if (prevLines[i] !== newLines[i]) { added++; removed++; }
    }
  ...
  const prevVarNames = (prevVars || []).map((v: GenericRow) => (typeof v === 'string' ? v : v.name)).sort();
  const newVarNames = newVars.map((v: GenericRow) => (typeof v === 'string' ? v : v.name)).sort();

  const addedVars = newVarNames.filter((n: string) => !prevVarNames.includes(n));
  const removedVars = prevVarNames.filter((n: string) => !newVarNames.includes(n));

  if (addedVars.length > 0) parts.push(`Variables added: ${addedVars.join(', ')}`);
  if (removedVars.length > 0) parts.push(`Variables removed: ${removedVars.join(', ')}`);
...
export async function updateDraftPromptVersion(
  tenantId: string,
  versionId: string,
...
export async function submitPromptVersionForApproval(
  tenantId: string,
  versionId: string,
  submittedBy: string,
): Promise<PromptVersion> {
...
export async function activatePromptVersion(
  ...
  versionId: string,
  activatedBy: string,
): Promise<{ activated: PromptVersion; deactivated: PromptVersion | null }> {
      const result = await safeQuery("SELECT * FROM __TENANT_SCHEMA__.ai_governance_items" + (tenantId ? " WHERE tenant_id = $1" : ""), tenantId ? [tenantId] : []);
      return result?.rows || [];
}

export async function suspendPromptVersion(
  tenantId: string,
  versionId: string,
  suspendedBy: string,
  notes?: string,
...
export async function rollbackPromptVersion(
  ...
  rolledBackBy: string,
  notes?: string,
): Promise<{ activated: PromptVersion; deactivated: PromptVersion | null; rollbackVersion: PromptVersion }> {
  ...
  const rollbackVersion = getFirstRow(insertResult) as PromptVersion;

  const { activated, deactivated } = await activatePromptVersion(tenantId, rollbackVersion.prompt_version_id, rolledBackBy);

  await emitAudit(tenantId, rolledBackBy, 'update', rollbackVersion.prompt_version_id,
...
export async function getActivePromptVersionForAsset(
  tenantId: string,
  assetId: string,
): Promise<PromptVersion | null> {
  const schema = tenantSchema(tenantId);
...
export async function resolveActivePromptForAgent(
  tenantId: string,
  agentKey: string,
): Promise<PromptVersion | null> {
...
export async function deletePromptVersion(
  ...
  const result = await safeQuery(
    `DELETE FROM "${schema}".ai_prompt_registry WHERE prompt_version_id = $1`,
    [versionId],
  );
  ...
  if (deleted) {
    await emitAudit(tenantId, deletedBy || SYSTEM_JOB_ACTOR, 'delete', versionId,
      { version_number: existing.version_number, template_text_length: existing.template_text.length, approval_status: existing.approval_status },
      null,
    );
  }
