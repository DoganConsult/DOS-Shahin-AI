...
async function deriveUserPermissions(tenantId: string, userId: string): Promise<{ module_codes: string[]; access_profile_code: string }> {
  try {
    const schema = tenantSchema(tenantId);
    const { rows: roleRows } = await safeQuery(
      `SELECT DISTINCT rp.permission_code FROM "${schema}".actor_role_assignments ara
    ...
    const moduleCodes = [...new Set(roleRows.map((r: GenericRow) => {
    ...
    const { rows: profileRows } = await safeQuery(
      `SELECT ap.profile_code FROM "${schema}".actor_access_assignments aaa
  ...
  } catch { return { module_codes: [], access_profile_code: 'standard_user' }; }
}
...
export async function canAiAct(
  tenantId: string,
  userId: string,
  moduleCode: string,
  ...
  if (policy.autonomy_level !== 'block' && !isRiskAllowed(riskLevel, policy.max_risk_level)) {
    ...
    if (policy.approval_role) {
      ...
      decision.escalate_to_user_ids = await resolveRoleToUsers(
        tenantId, moduleCode, policy.approval_role
      );
    }
  ...
  if (policy.autonomy_level === 'block') {
    const decision = blocked(
      `AI policy blocks ${actionClass} for ${moduleCode} in archetype ${blueprint.archetype_code}`
    );
    await logAiDecision(tenantId, userId, moduleCode, actionClass, decision, context);
    return decision;
  }
  ...
  const decision: AiActionDecision = {
    allowed: true,
    autonomy_level: policy.autonomy_level as AiAutonomyLevel,
...
export async function resolveModuleAiAutonomy(
  tenantId: string,
  moduleCode: string,
): Promise<Record<AiActionClass, AiAutonomyLevel>> {
  const schema = tenantSchema(tenantId);
  const blueprint = await getTenantBlueprint(tenantId);

  const defaults: Record<string, AiAutonomyLevel> = {
    observe: 'observe',
    recommend: 'recommend',
    draft: 'block',
    auto_assign: 'block',
  ...
  if (!blueprint) return defaults as Record<AiActionClass, AiAutonomyLevel>;

  const { rows } = await safeQuery(
    `SELECT action_class, autonomy_level
     FROM "${schema}".ai_action_policies
  ...
  return defaults as Record<AiActionClass, AiAutonomyLevel>;
}
...
export async function resolveRoleToUsers(
  tenantId: string,
  moduleCode: string,
  ...
  return rows.map((r: GenericRow) => r.user_id as string);
}
...
): Promise<{ user_id: string; role_code: string } | null> {
  const schema = tenantSchema(tenantId);

  // Find someone with higher authority in the same module, excluding current user
  const { rows } = await safeQuery(
    `SELECT eura.user_id, eura.functional_role_code AS role_code, eura.authority_level
     FROM "${schema}".enterprise_user_role_assignments eura
     JOIN public.users u ON u.user_id = eura.user_id AND u.status = 'active'
...
function isRiskAllowed(actual: string, max: string): boolean {
  return (RISK_RANK[actual] ?? 2) <= (RISK_RANK[max] ?? 3);
}

function blocked(reason: string): AiActionDecision {
