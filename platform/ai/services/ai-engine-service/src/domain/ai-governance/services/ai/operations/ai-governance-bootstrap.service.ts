...
import {
  discoverAndSeedAssets,
  getCanonicalAgents,
  getCanonicalModels,
  buildCanonicalPromptAssets,
  getRuntimeToolDefinitions,
  getAllowlistModels,
  getDefaultModelKey,
} from '../../runtime/ai/services/governance/compliance/ai-asset-discovery.service';
import type { DiscoveryResult } from '../../runtime/ai/services/governance/compliance/ai-asset-discovery.service';
import { backfillTenantAllowlistAssetRefs, type BackfillResult } from '../compliance/ai-binding-governance.service';
...
export interface GovernanceHealthReport {
  ...
  healthy: boolean;
  checks: {
    agentAssets: { expected: number; found: number; missing: string[] };
    modelAssets: { expected: number; found: number; missing: string[] };
    promptAssets: { expected: number; found: number; missing: string[] };
    modelVersions: { expected: number; found: number; missingActiveFor: string[] };
    promptVersions: { expected: number; found: number; missingActiveFor: string[] };
    agentVersions: { expected: number; found: number; missingActiveFor: string[] };
    toolBindings: { expected: number; found: number };
    allowlistEntries: { expected: number; found: number };
    allowlistLegacyRows: { total: number; withoutAssetId: number };
    configKeys: { expected: string[]; found: string[]; missing: string[] };
  };
}
...
async function resolveAssetId(schema: string, assetType: string, assetKey: string): Promise<string | null> {
...
async function versionExists(schema: string, table: string, assetId: string): Promise<boolean> {
  const r = await safeQuery(
    `SELECT 1 FROM "${schema}".${table} WHERE asset_id = $1 LIMIT 1`,
    [assetId],
  );
  return r.rows.length > 0;
}

async function seedModelVersions(schema: string, _tenantId: string): Promise<number> {
  ...
  for (const m of models) {
    ...
    if (!assetId) continue;
    if (await versionExists(schema, 'ai_model_registry', assetId)) continue;
    ...
    await safeQuery(
      ...
      [assetId, provider, providerModelId, JSON.stringify(m.metadata || {})],
    );
    seeded++;
  }
  return seeded;
...
async function seedPromptVersions(schema: string, _tenantId: string): Promise<number> {
  ...
  for (const p of prompts) {
    ...
    if (!assetId) continue;
    if (await versionExists(schema, 'ai_prompt_registry', assetId)) continue;

    const agentId = p.metadata?.linked_agent as string | undefined;
    const templateText = agentId
      ? `Default system prompt for agent ${agentId}. Configure via AI Governance.`
      : '';

    await safeQuery(
      `INSERT INTO "${schema}".ai_prompt_registry
       ...
       ON CONFLICT (asset_id, version_number) DO NOTHING`,
      [assetId, templateText, defaultModelAssetId],
    );
    seeded++;
  }
  return seeded;
...
async function seedAgentVersions(schema: string, _tenantId: string): Promise<number> {
  ...
  for (const a of agents) {
    ...
    if (!assetId) continue;
    if (await versionExists(schema, 'ai_agent_registry', assetId)) continue;
  ...
  for (const t of tools) {
    ...
    try {
      await safeQuery(
        ...
        [agentAssetId, toolAssetId, tenantId],
      );
      seeded++;
    } catch {}
  }
...
async function seedAllowlist(schema: string, tenantId: string): Promise<number> {
  ...
  for (const entry of allowlistModels) {
    ...
    try {
      await safeQuery(
        `INSERT INTO "${schema}".tenant_ai_allowlist
        ...
        [tenantId, assetId, entry.provider, entry.model_id],
      );
      seeded++;
    } catch {}
  }
...
async function seedGovernanceConfig(schema: string): Promise<string[]> {
  const seeded: string[] = [];
...
export async function checkGovernanceHealth(tenantId: string): Promise<GovernanceHealthReport> {
  ...
  const bindingResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS cnt FROM "${schema}".ai_agent_tool_bindings WHERE tenant_id = $1`,
    [tenantId],
  ), { operation: 'query ai_agent_tool_bindings' });

  const allowlistResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ cnt: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS cnt FROM "${schema}".tenant_ai_allowlist WHERE tenant_id = $1 AND is_enabled = TRUE`,
    [tenantId],
  ), { operation: 'query ai_agent_tool_bindings' });

  const allowlistLegacyResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, without_asset_id: 0 }]), safeQuery(
    `SELECT COUNT(*)::int AS total,
            COUNT(*) FILTER (WHERE asset_id IS NULL)::int AS without_asset_id
     FROM "${schema}".tenant_ai_allowlist WHERE tenant_id = $1`,
    [tenantId],
  ), { operation: 'query tenant_ai_allowlist' });
  ...
  const configResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT config_key FROM "${schema}".platform_operation_config WHERE config_key = ANY($1) AND owner_module IN ('ai_governance', 'platform')`,
    [expectedConfigKeys],
...
async function checkAssetPresence(
  schema: string,
  assetType: string,
  expectedKeys: string[],
  ...
  const result = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT asset_key FROM "${schema}".ai_asset_inventory
...
): Promise<{ expected: number; found: number; missingActiveFor: string[] }> {
  const result = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
    `SELECT DISTINCT ai.asset_key
     FROM "${schema}".${table} r
...
export async function repairAiGovernance(tenantId: string): Promise<{
