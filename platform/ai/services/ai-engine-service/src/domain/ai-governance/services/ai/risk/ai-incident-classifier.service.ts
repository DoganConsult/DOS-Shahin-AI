...
// NCA ECC compliant incident types
export type AiIncidentType =
  | 'safety_hazard'          // Physical or digital safety risk
  | 'discrimination'         // Bias-driven discriminatory outcome
  | 'transparency_failure'   // Unexplainable AI decision
  | 'data_breach'           // PII exposure via AI
  | 'operational_failure'    // AI system outage or malfunction
  | 'hallucination'         // AI generated false/misleading content
  | 'prompt_injection'      // Security attack on AI system
  | 'regulatory_violation'  // AI output violates regulatory requirement
  | 'consent_violation'     // AI processed data without consent
  | 'model_drift';          // AI model performance degradation

export interface AiIncidentClassification {
  incident_type: AiIncidentType;
  severity: 'low' | 'medium' | 'high' | 'critical';
  nca_reportable: boolean;
  sama_reportable: boolean;
  sdaia_reportable: boolean;
  eu_ai_act_reportable: boolean;
  mandatory_notification_hours: number;
...
const NOTIFICATION_HOURS: Record<string, number> = {
  safety_hazard: 24,
  discrimination: 72,
 ...
 */
export async function classifyAiIncident(
  tenantId: string,
  incidentData: {
    description: string;
    agent_id?: string;
...
): Promise<AiIncidentClassification> {
...
Classify the incident and assess its severity. Respond with JSON:
{
  ...
  const incidentType = aiClassification.incident_type as AiIncidentType;
  const severity = aiClassification.severity;

  const classification: AiIncidentClassification = {
  ...
  await safeQuery(
    `INSERT INTO "${schema}".ai_incident_classifications
    ...
    [tenantId, incidentType, severity, JSON.stringify(classification), incidentData.agent_id || 'any',
     classification.nca_reportable, classification.sama_reportable, classification.sdaia_reportable,
     classification.mandatory_notification_hours]
  ).catch(catchHandler(EC.EVENT_BUS, {}));

  // Emit ai_incident.reported when an AI incident is classified
  emitEvent(({ tenantId, userId: SYSTEM_JOB_ACTOR, module: 'ai-governance', event: 'ai_incident.reported', entityType: 'ai_incident_classification', entityId: incidentData.agent_id || 'any', data: { incidentType, severity, ncaReportable: classification.nca_reportable, harmType: classification.harm_type } } as any)).catch(catchHandler(EC.EVENT_BUS));
  ...
  if (severity === 'critical' || severity === 'high') {
    for (const escalationTarget of classification.escalation_path.slice(0, 3)) {
      await safeQuery(
        ...
        [tenantId, escalationTarget,
         `AI Incident [${severity.toUpperCase()}]: ${incidentType.replace(/_/g, ' ')}`,
         `${incidentData.description}\n\nImmediate actions: ${classification.immediate_actions.join('; ')}`]
      ).catch(catchHandler(EC.EVENT_BUS, {}));
    }
  }
