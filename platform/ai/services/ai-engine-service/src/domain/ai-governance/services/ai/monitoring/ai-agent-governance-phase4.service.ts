...
export async function createAgentScope(
  ...
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_agent_authority_scopes
...
export async function startAgentSession(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_agent_session_governance
...
export async function recordAgentAction(
  ...
  const schema = tenantSchema(tenantId);
  await safeQuery(
    `UPDATE "${schema}".ai_agent_session_governance
...
export async function checkAgentBudget(
  tenantId: string,
  sessionId: string,
): Promise<{ allowed: boolean; remaining_actions: number; remaining_cost: number; reason: string }> {
...
export async function detectPromptInjection(
  ...
  sessionId: string,
  input: string,
): Promise<{ detected: boolean; patterns_matched: string[] }> {
  ...
  if (matched.length > 0) {
    ...
    await swallow(EC.EVENT_BUS, eventBus.publish({
      ...
      eventType: 'agent.prompt_injection.detected' as any, tenantId, sourceService: 'ai-agent-governance', severity: 'critical',
...
// 7. scanOutputForPII
export async function scanOutputForPII(
  tenantId: string,
  sessionId: string,
  output: string,
): Promise<{ pii_found: boolean; types_detected: string[] }> {
  const types: string[] = [];
...
export async function approveSubAgentSpawn(
  tenantId: string,
  parentSessionId: string,
  childConfig: Record<string, unknown>,
): Promise<{ approved: boolean; child_session_id?: string; reason: string }> {
  const schema = tenantSchema(tenantId);

  // Check current delegation depth
  const { rows: chain } = await safeQuery(
  ...
  const { rows: scope } = await safeQuery(
    `SELECT sc.max_sub_agent_depth
  ...
  const { rows: childSession } = await safeQuery(
    `INSERT INTO "${schema}".ai_agent_session_governance
       (scope_id, agent_id, session_isolation, status)
     SELECT scope_id, $1, $2, 'active'
     FROM "${schema}".ai_agent_session_governance WHERE id = $3
     RETURNING id`,
    [childConfig.agent_id, childConfig.isolation ?? 'isolated', parentSessionId],
  );

  // Record chain of custody
  await safeQuery(
    `INSERT INTO "${schema}".ai_agent_chain_of_custody
...
export async function getAgentAuditTrail(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT s.*, sc.module_code, sc.scope_type, sc.max_actions_per_session
     FROM "${schema}".ai_agent_session_governance s
     JOIN "${schema}".ai_agent_authority_scopes sc ON s.scope_id = sc.id
     WHERE s.agent_id = $1
...
// 11. listAgentScopes
export async function listAgentScopes(
  tenantId: string,
  agentId?: string,
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  ...
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".ai_agent_authority_scopes ${where} ORDER BY created_at DESC LIMIT 100`,
    params,
  );
  return rows;
}
...
export async function getAgentScopeById(
  tenantId: string,
  scopeId: string,
): Promise<unknown> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".ai_agent_authority_scopes WHERE id = $1`,
    [scopeId],
  );
  return rows[0] ?? null;
}
...
export async function listAgentSessions(
  ...
  let idx = 1;
  if (filters?.agent_id) { conditions.push(`s.agent_id = $${idx++}`); params.push(filters.agent_id); }
  if (filters?.status) { conditions.push(`s.status = $${idx++}`); params.push(filters.status); }
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const { rows } = await safeQuery(
    `SELECT s.*, sc.module_code, sc.scope_type
     FROM "${schema}".ai_agent_session_governance s
...
export async function getAgentSessionById(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT s.*, sc.module_code, sc.scope_type, sc.max_actions_per_session, sc.max_cost_per_session
     FROM "${schema}".ai_agent_session_governance s
     LEFT JOIN "${schema}".ai_agent_authority_scopes sc ON s.scope_id = sc.id
     WHERE s.id = $1`,
    [sessionId],
...
export async function getDelegationChain(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT c.*, ps.agent_id as parent_agent, cs.agent_id as child_agent
     FROM "${schema}".ai_agent_chain_of_custody c
...
export async function escalateToHuman(
  ...
  await swallow(EC.EVENT_BUS, eventBus.publish({
    ...
    eventType: 'agent.escalated_to_human' as any, tenantId, sourceService: 'ai-agent-governance', severity: 'warning',
