...
function computeHndlRisk(dataSensitivity: string | null): string {
...
export async function inventoryCryptoAsset(
  tenantId: string,
  ...
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".cryptographic_inventory
...
export async function assessVulnerability(
  tenantId: string,
  assetId: string,
): Promise<{ is_quantum_vulnerable: boolean; hndl_risk_level: string; recommendation: string }> {
...
export async function createMigrationPlan(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".quantum_migration_plans
...
export async function recordPqcTestResult(
  tenantId: string,
  result: Record<string, unknown>,
): Promise<{ id: string }> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".pqc_test_results
...
export async function listCryptoAssets(
  tenantId: string,
  filters?: { is_quantum_vulnerable?: boolean; hndl_risk_level?: string; migration_status?: string },
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  const conditions: string[] = [];
  const params: unknown[] = [];
  let idx = 1;
  if (filters?.is_quantum_vulnerable !== undefined) { conditions.push(`is_quantum_vulnerable = $${idx++}`); params.push(filters.is_quantum_vulnerable); }
  if (filters?.hndl_risk_level) { conditions.push(`hndl_risk_level = $${idx++}`); params.push(filters.hndl_risk_level); }
  if (filters?.migration_status) { conditions.push(`migration_status = $${idx++}`); params.push(filters.migration_status); }
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".cryptographic_inventory ${where} ORDER BY created_at DESC LIMIT 200`,
    params,
  );
  return rows;
}
...
export async function getCryptoAssetById(
  tenantId: string,
  assetId: string,
): Promise<unknown> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".cryptographic_inventory WHERE id = $1`,
    [assetId],
  );
  return rows[0] ?? null;
}
...
export async function listMigrationPlans(
  ...
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".quantum_migration_plans ${where} ORDER BY created_at DESC LIMIT 100`,
    params,
  );
  return rows;
}
...
export async function getMigrationPlanById(
  tenantId: string,
  planId: string,
): Promise<unknown> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".quantum_migration_plans WHERE id = $1`,
    [planId],
  );
  return rows[0] ?? null;
}
...
export async function listPqcTestResults(
  tenantId: string,
  filters?: { plan_id?: string; asset_id?: string; result?: string },
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  const conditions: string[] = [];
  const params: unknown[] = [];
  let idx = 1;
  if (filters?.plan_id) { conditions.push(`plan_id = $${idx++}`); params.push(filters.plan_id); }
  ...
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".pqc_test_results ${where} ORDER BY test_date DESC LIMIT 100`,
    params,
  );
  return rows;
}
...
export async function getQuantumReadinessDashboard(
  tenantId: string,
): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);
  ...
  const [assets, vulnerable, migrationProgress, plans] = await Promise.all([
    ...
    safeQuery(
      `SELECT count(*) as total, hndl_risk_level
