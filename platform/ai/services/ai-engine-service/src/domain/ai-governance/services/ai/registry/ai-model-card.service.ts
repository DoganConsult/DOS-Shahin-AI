 ...
 */
export async function generateModelCard(tenantId: string, agentId: string): Promise<ModelCard> {
  ...
  for (const row of performance.rows) {
    perfMetrics[(row as any).metric_type] = { average: parseFloat((row as any).avg_val), samples: parseInt((row as any).sample_count) };
  }
...
Respond with JSON: {
  ...
  await safeQuery(
    `INSERT INTO "${schema}".ai_model_cards (tenant_id, agent_id, card_json, generated_at)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT (tenant_id, agent_id) DO UPDATE SET card_json = $3, generated_at = NOW()`,
    [tenantId, agentId, JSON.stringify(card)]
 ...
 */
export async function getModelCard(tenantId: string, agentId: string): Promise<ModelCard | null> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT card_json FROM "${schema}".ai_model_cards WHERE tenant_id = $1 AND agent_id = $2`,
    [tenantId, agentId]
  );
 ...
 */
export async function listModelCards(tenantId: string): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT agent_id, card_json->>'model_name' AS model_name, card_json->>'version' AS version,
