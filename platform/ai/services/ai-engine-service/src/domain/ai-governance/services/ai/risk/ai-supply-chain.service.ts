...
export async function registerModelProvenance(
  tenantId: string,
  provenance: Record<string, unknown>,
): Promise<{ id: string }> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_model_provenance
  ...
  swallow(EC.EVENT_BUS, eventBus.publish({
    ...
    eventType: 'ai_supply_chain.provenance_registered' as any, tenantId, sourceService: 'ai-supply-chain', severity: 'info',
...
export async function trackDataLineage(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_data_lineage
  ...
  swallow(EC.EVENT_BUS, eventBus.publish({
    ...
    eventType: 'ai_supply_chain.lineage_tracked' as any, tenantId, sourceService: 'ai-supply-chain', severity: 'info',
...
export async function getProvenanceById(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".ai_model_provenance WHERE id = $1`,
    [provenanceId],
  );
  return rows[0] ?? null;
}
...
export async function getLineageById(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".ai_data_lineage WHERE id = $1`,
    [lineageId],
  );
  return rows[0] ?? null;
}
...
export async function assessSupplyChainRisk(
  tenantId: string,
  systemId: string,
): Promise<{ overall_score: number; risk_factors: string[]; recommendations: string[] }> {
  const schema = tenantSchema(tenantId);
...
export async function checkDataSovereignty(
  tenantId: string,
  systemId: string,
): Promise<{ compliant: boolean; issues: string[] }> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".ai_data_lineage WHERE system_id = $1 AND cross_border = TRUE`,
    [systemId],
  );
  ...
  for (const d of rows) {
    for (const country of (d.destination_countries ?? [])) {
      if (!adequateCountries.includes(country) && !d.adequacy_decision_ref) {
        issues.push(
          `Dataset ${d.dataset_name}: Transfer to ${country} lacks adequacy decision or appropriate safeguards`,
        );
      }
    }
  }
...
export async function registerProvider(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_provider_registry
  ...
  swallow(EC.EVENT_BUS, eventBus.publish({
    ...
    eventType: 'ai_supply_chain.provider_registered' as any, tenantId, sourceService: 'ai-supply-chain', severity: 'info',
...
export async function listProviders(
  tenantId: string,
  filters?: { risk_tier?: string; compliance_status?: string },
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
...
export async function getProviderById(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT * FROM "${schema}".ai_provider_registry WHERE id = $1`,
    [providerId],
  );
  return rows[0] ?? null;
}

export async function updateProviderCompliance(
  tenantId: string,
  providerId: string,
  update: { compliance_status: string; risk_tier?: string },
): Promise<{ updated: boolean }> {
  const schema = tenantSchema(tenantId);
  const { rowCount } = await safeQuery(
    `UPDATE "${schema}".ai_provider_registry
     SET compliance_status = $2,
         risk_tier = COALESCE($3, risk_tier),
         last_assessed_at = now(),
         updated_at = now()
     WHERE id = $1`,
    [providerId, update.compliance_status, update.risk_tier ?? null],
  );
  if ((rowCount ?? 0) > 0) {
    swallow(EC.EVENT_BUS, eventBus.publish({

      eventType: 'ai_supply_chain.provider_compliance_updated' as any, tenantId, sourceService: 'ai-supply-chain', severity: 'info',
...
export async function createSupplierAgreement(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_supplier_agreements
  ...
  swallow(EC.EVENT_BUS, eventBus.publish({
    ...
    eventType: 'ai_supply_chain.agreement_created' as any, tenantId, sourceService: 'ai-supply-chain', severity: 'info',
...
export async function getAgreementById(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT a.*, p.name_en AS provider_name, p.provider_code
     FROM "${schema}".ai_supplier_agreements a
     JOIN "${schema}".ai_provider_registry p ON p.id = a.provider_id
     WHERE a.id = $1`,
    [agreementId],
...
export async function updateAgreementStatus(
  ...
  const schema = tenantSchema(tenantId);
  const { rowCount } = await safeQuery(
    `UPDATE "${schema}".ai_supplier_agreements
...
export async function checkExpiringAgreements(
  tenantId: string,
  daysAhead: number = 30,
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
...
export async function recordModelModification(
  tenantId: string,
  modification: Record<string, unknown>,
): Promise<{ id: string }> {
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `INSERT INTO "${schema}".ai_model_modifications
  ...
  swallow(EC.EVENT_BUS, eventBus.publish(({
      eventType: eventType as string, tenantId, sourceService: 'ai-supply-chain',
...
export async function listModelModifications(
  tenantId: string,
  filters?: { provenance_id?: string; system_id?: string; is_substantial?: boolean },
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
...
export async function getModificationById(
  ...
  const schema = tenantSchema(tenantId);
  const { rows } = await safeQuery(
    `SELECT m.*, p.model_name, p.model_version
     FROM "${schema}".ai_model_modifications m
     JOIN "${schema}".ai_model_provenance p ON p.id = m.provenance_id
     WHERE m.id = $1`,
    [modificationId],
...
export async function getSubstantialModifications(
  tenantId: string,
  systemId: string,
): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
