...
import {
  createSimulation,
  applyScenario,
  getSimulationWithImpact,
  ScenarioDefinition,
} from './digital-twin.service';
...
export interface ScenarioTemplate {
  ...
  category: 'vendor' | 'regulatory' | 'personnel' | 'cyber' | 'operational';
  severity: 'critical' | 'high' | 'medium';
  changes: ScenarioChange[];
}

export interface ScenarioChange {
  type: 'control_change' | 'risk_change' | 'policy_change' | 'org_structure_change';
  description_en: string;
  description_ar: string;
 ...
 */
function resolveTemplateChanges(
  templateChanges: ScenarioChange[],
  snapshot: Record<string, unknown>,
): ScenarioDefinition['changes'] {

  const controls: unknown[] = snapshot?.controls || [];

  const risks: unknown[] = snapshot?.risks || [];

  const policies: unknown[] = snapshot?.policies || [];

  return templateChanges.map((change) => {
    const { entityIndex, cascade, ...restParams } = change.change_params as Record<string, unknown>;
 ...
 */
function extractAffectedDepartments(
  snapshot: Record<string, unknown>,
  orgMetrics: Record<string, unknown>,
): string[] {

  const orgStructure: unknown[] = snapshot?.orgStructure || [];
  const departments = orgStructure

    .filter((u: Record<string, unknown>) => u.unit_type === 'department')

    .map((u: Record<string, unknown>) => u.name || u.unit_name || 'Unknown Department');

  // If we have org-wide metrics showing departments affected, return those
  const count = orgMetrics.departmentsAffected || 0;
  if ((count as any) > 0 && departments.length > 0) {

    return departments.slice(0, (count as any));
  }

  // Fallback: return all departments if org-wide scope
  return departments.length > 0 ? (departments as any) : ['All Departments'];
}
