...
import { getAssetByKey, type AIAsset } from '../../runtime/ai/registry/ai-asset-inventory.service';
import { isAssetAllowlistedForTenant } from '../../runtime/ai/compliance/ai-binding-governance.service';
import { getActiveModelVersionForAsset, type ModelVersion } from '../../services/misc/model-registry.service';
import {
  getAgentModelConfig,
  resolveModelForAgent,
  type AgentModelConfig,
} from '../../runtime/ai/services/gateway/llm-router.service';
import { recordAudit } from '../../../audit/services/audit/core/audit-trail.service';
import {
  getTenantEnforcementMode,
  setTenantEnforcementMode as _setTenantEnforcementMode,
  getGlobalFallbackMode,
  setGlobalFallbackMode,
  type EnforcementMode,
} from '../../runtime/ai/operations/ai-governance-config.service';
...
export type ResolutionStatus =
  | 'resolved_governed'
  | 'resolved_runtime_fallback'
  | 'mismatch_pass'
  | 'mismatch_warn'
  | 'blocked_no_asset'
  | 'blocked_no_active_version'
  | 'blocked_mismatch_enforced';
...
export interface GovernanceResolution {
  ...
  governance_version_id: string | null;
  governance_version_number: number | null;
  governance_provider: string | null;
  governance_model_id: string | null;
  governance_config: Record<string, unknown> | null;
  mismatch: boolean;
  ...
  allowlisted: boolean;
  allowlist_advisory: string | null;
}

export interface MismatchReport {
  agent_id: string;
  tenant_id: string;
  runtime: { provider: string; model: string; max_tokens: number; temperature: number };
  registry: { provider: string | null; model_id: string | null; config: Record<string, unknown> | null; version_number: number | null; deployment_status: string | null } | null;
  mismatches: string[];
  timestamp: string;
}
...
function buildAssetKey(provider: string, modelId: string): string {
...
export async function resolveGovernedModel(
  ...
  const base: GovernanceResolution = {
    agent_id: agentId,
  ...
  let activeVersion: ModelVersion | null = null;
  try {
  ...
  if (mode === 'enforce') {
    ...
    (base as any).resolved_max_tokens = activeVersion.config?.max_tokens ?? resolved.maxTokens;
    (base as any).resolved_temperature = activeVersion.config?.temperature ?? resolved.temperature;
...
export async function detectAllMismatches(tenantId: string): Promise<MismatchReport[]> {
  const schema = tenantSchema(tenantId);
  const reports: MismatchReport[] = [];

  let configs: AgentModelConfig[] = [];
  try {
    const result = await safeQuery(`SELECT * FROM "${schema}".agent_model_config ORDER BY agent_id`);
    configs = result.rows as AgentModelConfig[];
  } catch {
    return reports;
  }
  ...
  for (const cfg of configs) {
    ...
    if (resolved.provider === 'auto' || resolved.model === 'auto') continue;
