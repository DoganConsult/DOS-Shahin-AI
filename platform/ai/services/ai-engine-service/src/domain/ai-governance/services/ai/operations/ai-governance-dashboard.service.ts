...
export interface AiGovernanceDashboardSummary {
  totalSystems: number;
  ...
  pendingDpias: number;
  expiredDpias: number;
  supplyChainHighRisk: number;
  overdueReviews: number;
  systemsByRiskLevel: RiskBreakdown[];
...
export async function getAiGovernanceDashboard(tenantId: string): Promise<AiGovernanceDashboardSummary> {
  ...
  try {
    const [systems, modelCards, policies, dpias, supplyChain, reviews, riskBreakdown, activity] = await Promise.all([
      ...
      ).catch(() => ({ rows: [{ total: 0, deployed: 0, high_risk: 0, suspended: 0 }] })),
      safeQuery(
        `SELECT
           COUNT(*)::int AS total,
      ...
      ).catch(() => ({ rows: [{ total: 0, active: 0, deprecated: 0 }] })),
      safeQuery(
        `SELECT
           COUNT(*)::int AS total,
           COUNT(*) FILTER (WHERE status = 'active')::int AS active
    ...
    return {
      ...
      systemsByRiskLevel: riskBreakdown.rows.map((r: GenericRow) => ({
...
export async function getAiDpiaStatusSummary(tenantId: string): Promise<DpiaSummaryItem[]> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT ad.dpia_id, ad.system_id, asr.code AS system_code, asr.name_en AS system_name,
...
async function getRecentAiGovActivity(schema: string): Promise<DashboardActivityItem[]> {
  const result = await safeQuery(
...
export interface DpiaSummaryItem {
  dpiaId: string;
  systemId: string;
  systemCode: string;
  systemName: string;
