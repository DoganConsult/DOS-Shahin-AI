...
import type { EngineRun, EngineResult } from '@dos/types';

export function toRun(row: GenericRow): EngineRun {
...
export function toRunList(rows: GenericRow[]): EngineRun[] { return rows.map(toRun); }

export function toResult(row: GenericRow): EngineResult {
