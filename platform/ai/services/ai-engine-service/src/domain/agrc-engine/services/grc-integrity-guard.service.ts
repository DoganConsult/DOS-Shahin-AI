...
export async function runIntegrityGuard(tenantId: string): Promise<IntegrityResult> {
  ...
  try {
    ...
    if (orphanControls.rows.length > 0) {
      ...
      const fallbackPolicy = getFirstRow(defaultPolicy) || getFirstRow((await safeQuery(
      ...
      if (fallbackPolicy) {
        for (const ctrl of orphanControls.rows) {
          ...
          swallow(EC.AGENT_ACTION, eventBus.publish(({
                    ...
                    } as any)), { tenantId, operation: 'eventBus:control.state_changed' });
      ...
      if (fwIds.length > 0) {
        for (const pol of orphanPolicies.rows) {
          ...
          swallow(EC.AGENT_ACTION, eventBus.publish(({
                    ...
                    } as any)), { tenantId, operation: 'eventBus:policy.approved' });
      ...
      if (getFirstRow(generalControl)) {
        for (const ev of orphanEvidence.rows) {
          await safeQuery(
            ...
            [getFirstRow(generalControl)?.control_id, ev.evidence_id]
          );
          evidenceFixed++;
        }
        ...
        swallow(EC.AGENT_ACTION, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:evidence.uploaded' });
      } else {
