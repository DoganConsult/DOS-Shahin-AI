...
export async function transitionStatus(
  ...
  entityType: 'run' | 'result' | 'config' = 'run', _reason?: string,
  ...
  if (current.rows.length === 0) { const e: unknown = new Error(`agrc-engine ${entityType} not found`); e.statusCode = 404; throw e; }
...
await safeQuery(`UPDATE "${schema}"."${table}" SET status = $1, updated_at = NOW() WHERE id = $2`, [targetStatus, entityId]);
  await safeQuery(
    `INSERT INTO "${schema}".audit_trail (tenant_id, actor_id, module, action, entity_type, entity_id, before_state, after_state) VALUES ($1,$2,'agrc-engine','transition',$3,$4,$5,$6)`,
    ...
    if (emitEvent) await emitEvent({ tenantId, userId: userId, module: 'agrc-engine', event: 'status_changed', entityType: 'agrc-engine', entityId: entityId, data: { fromStatus, targetStatus } });
  } catch {}
...
export async function bulkTransitionStatus(
  ...
  entityType: 'run' | 'result' | 'config' = 'run',
...
export async function getStatusHistory(tenantId: string, entityId: string): Promise<Array<{ fromStatus: string; toStatus: string; changedBy: string; changedAt: string }>> {
  ...
  try {
    const result = await safeQuery(
      `SELECT before_state AS "fromStatus", after_state AS "toStatus", actor_id AS "changedBy", created_at AS "changedAt" FROM "${schema}".audit_trail WHERE entity_id = $1 AND module = 'agrc-engine' AND action = 'transition' ORDER BY created_at ASC`,
      [entityId],
    );
    return result.rows;
  } catch { return []; }
...
export async function getFailedRuns(tenantId: string): Promise<Array<{ id: string; status: string; updatedAt: string }>> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT id, status, updated_at AS "updatedAt" FROM "${schema}".agrc_engine_runs WHERE status = 'failed' ORDER BY updated_at ASC LIMIT 100`,
    );
    return result.rows;
  } catch { return []; }
}

export async function getStuckRuns(tenantId: string): Promise<Array<{ id: string; status: string; updatedAt: string }>> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
      `SELECT id, status, updated_at AS "updatedAt" FROM "${schema}".agrc_engine_runs WHERE status = 'running' ORDER BY updated_at ASC LIMIT 100`,
    );
    return result.rows;
  } catch { return []; }
}
