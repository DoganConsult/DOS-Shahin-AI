...
import { eventBus, PlatformEvent } from '../ports/events.port';
...
interface _AgentShadow {
  shadow_id: string;
  user_id: string;
  team_id: string;
  agent_name: string;
  activation_mode: string;
  raci_mirror: Record<string, unknown>;
  capabilities: string[];
  auto_actions: string[];
  enabled: boolean;
}
...
const TRIGGER_EVALUATORS: Record<TriggerType, (schema: string, config: Record<string, unknown>) => Promise<{ triggered: boolean; context: Record<string, unknown> }>> = {
  ...
  compliance_drop: async (schema, config) => {
    ...
    return { triggered: (avg as any) < minScore, context: { avgComplianceScore: avg, minScore } };
  },
...
async function executeActivationAction(
  ...
  shadow: any, rule: ActivationRule, triggerContext: Record<string, unknown>,
): Promise<string> {
  ...
  switch (actionType) {
    case 'run_agent': {
      ...
      if (agentId) {
        ...
        const result = await runAgent(tenantId, (agentId as any));
    ...
    case 'delegate_to_agent': {
      const agentId = actionConfig.agentId || mapShadowToAgent(shadow);
      if (agentId) {
        await autoDelegateToAgent(tenantId, shadow.user_id, (agentId as any), shadow.team_id, triggerContext);
...
export function initEventDrivenAgentTriggers(): void {
  ...
  for (const [eventType, agentIds] of Object.entries(EVENT_AGENT_MAP)) {
    eventBus.subscribe(eventType as string, `agrc-os-reactive-${eventType}`, async (event: PlatformEvent) => {
...
export async function getPersistentPendingHandoffs(tenantId: string, agentId: string): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  const res = await safeQuery(
...
export async function completePersistentHandoff(tenantId: string, handoffId: string, result: Record<string, unknown>): Promise<void> {
  const schema = tenantSchema(tenantId);
  await safeQuery(
...
async function autoDelegateToAgent(
  ...
  teamId: string, context: Record<string, unknown>,
): Promise<void> {
  try {
...
export async function seedAgentTargetsFromProfile(tenantId: string): Promise<{ seeded: number; targets: string[] }> {
  ...
  const upsertTarget = async (agentId: string, targetType: string, config: Record<string, unknown>, priority: string) => {
    await safeQuery(
