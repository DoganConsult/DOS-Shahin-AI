...
} from './agrc-engine-workflow.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('validateTransition', () => {
    ...
    it('should reject transitions from unknown states', () => {
      expect(validateTransition('nonexistent', 'running')).toBe(false);
    });
  });
  ...
  describe('isProtectedTransition', () => {
    ...
    it('should return false for non-protected transitions', () => {
      expect(isProtectedTransition('pending', 'running')).toBe(false);
    });
  });

  describe('getAvailableTransitions', () => {
    it('should return available transitions for a state', () => {
      const result = getAvailableTransitions('pending');
  ...
  describe('executeTransition', () => {
    it('should reject invalid transitions', async () => {
      const result = await executeTransition('t1', 'e1', 'pending', 'archived', 'u1');
      expect(result.success).toBe(false);
      expect(result.error).toBeDefined();
    });

    it('should execute valid transitions', async () => {
      const result = await executeTransition('t1', 'e1', 'pending', 'running', 'u1');
  ...
  describe('handleApprovalOutcome', () => {
    ...
    it('should handle rejected outcome', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ status: 'running' }] });
  ...
  describe('workflow hooks', () => {
    const ctx = { tenantId: 't1', entityId: 'e1', entityType: 'agrc-engine', triggeredBy: 'u1' };
