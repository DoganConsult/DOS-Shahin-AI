...
export async function runAutonomousEngine(tenantId: string): Promise<AutonomousEngineResult> {
...
async function _runInternal(tenantId: string): Promise<AutonomousEngineResult> {
  const start = Date.now();
  const schema = tenantSchema(tenantId);
  const warnings: string[] = [];
  let evidenceAlerts = 0, policyAlerts = 0, vendorAlerts = 0,
      riskDrifts = 0, complianceGaps = 0, controlIssues = 0,
      slaWarnings = 0, frameworkGaps = 0, privacyAlerts = 0,
      remediationsCreated = 0;

  // ── 1. Evidence Staleness Scanner ──────────────────────────────────────────
  try {
    const staleEvidence = await safeQuery(
      `SELECT e.evidence_id, e.title, e.control_id, e.uploaded_at, e.expiry_date
    ...
    for (const ev of staleEvidence.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:evidence.expired' });
      evidenceAlerts++;
    }
  } catch (err: unknown) {
  ...
  // ── 2. Policy Expiry Predictor ─────────────────────────────────────────────
  try {
    const expiringPolicies = await safeQuery(
      `SELECT policy_id, title, review_date, status
    ...
    for (const pol of expiringPolicies.rows) {
      ...
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:policy.expired_or_review_due' });
      policyAlerts++;
    }
  } catch (err: unknown) {
  ...
  // ── 3. Vendor Contract Expiry Scanner ──────────────────────────────────────
  try {
    const expiringContracts = await safeQuery(
      `SELECT vendor_id, name, contract_end_date, risk_tier
       FROM "${schema}".vendors
    ...
    for (const v of expiringContracts.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:vendor.contract_expiring' });
      vendorAlerts++;
    }

    // Also check vendor assessments that are overdue
    const overdueAssessments = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
      `SELECT vendor_id, name, last_assessment_date
       FROM "${schema}".vendors
    ...
    for (const v of overdueAssessments.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:vendor.assessment_due' });
      vendorAlerts++;
    }
  } catch (err: unknown) {
  ...
  try {
    ...
    const risks = await safeQuery(
      `SELECT risk_id, title, category, risk_score, likelihood, impact, treatment_status, owner
       FROM "${schema}".risks
    ...
    for (const risk of risks.rows) {
      ...
      const check = await swallowDefault(EC.FALLBACK_QUERY, { withinAppetite: true, maxAllowed: 999, requiredRole: '' }, checkRiskAgainstAppetite(
        tenantId, risk.category || 'operational', risk.risk_score
      ...
      if (!check.withinAppetite) {
        swallow(EC.AGENT_ACTION, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:risk.exceeded_appetite' });
        riskDrifts++;
      }
      ...
      if (risk.risk_score >= 15 && (!risk.treatment_status || risk.treatment_status === 'open')) {
        swallow(EC.AGENT_ACTION, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:risk.mitigation_required' });
        riskDrifts++;
      }
    }
  } catch (err: unknown) {
  ...
  try {
    ...
    if (posturePercent < 70) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:compliance.posture_changed' });
      complianceGaps++;
    }
    ...
    if (failing > 0) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:control.effectiveness_low' });
  ...
  try {
    ...
    const noEvidence = await safeQuery(
    ...
    for (const ctrl of noEvidence.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:evidence.coverage_low' });
      controlIssues++;
    }
  } catch (err: unknown) {
    ...
    for (const fw of frameworks.rows) {
      ...
      if (coverage < 80) {
        swallow(EC.AGENT_ACTION, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:framework.gap_identified' });
        frameworkGaps++;
      }
    }
  } catch (err: unknown) {
  ...
  // ── 9. Privacy Impact Scanner ──────────────────────────────────────────────
  try {
    const highImpact = await safeQuery(
      `SELECT processing_id, activity_name, data_categories, impact_level
    ...
    for (const pa of highImpact.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:privacy.impact_high' });
      privacyAlerts++;
    }
  } catch (err: unknown) {
  ...
  try {
    ...
    const staleIncidents = await safeQuery(
      `SELECT i.incident_id, i.title, i.severity, i.created_at
    ...
    const unlinkedFindings = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
      `SELECT ai.item_id, ai.assessment_id, ai.control_node_id, ai.status
       FROM "${schema}".assessment_items ai
       WHERE ai.status IN ('non_compliant', 'partially_compliant')
       LIMIT 20`
    ), { tenantId: tenantId, operation: 'query assessment_items' });
    ...
    for (const finding of unlinkedFindings.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:audit.remediation_due' });
      remediationsCreated++;
    }
  } catch (err: unknown) {
  ...
  let raciGaps = 0;
  try {
    const gapView = await safeQuery(
      `SELECT entity_type, entity_id, has_responsible, has_accountable, has_user_owner, has_team_owner
    ...
    for (const gap of gapView.rows) {
      ...
      if (!gap.has_responsible) missing.push('responsible');
      if (!gap.has_accountable) missing.push('accountable');
      if (!gap.has_user_owner) missing.push('user_owner');
      if (!gap.has_team_owner) missing.push('team_owner');
      ...
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:raci.gap_detected' });
  ...
  try {
    const expiring = await safeQuery(
      `SELECT assignment_id, entity_type, entity_id, raci_role, effective_to, user_id, team_id
    ...
    for (const row of expiring.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:raci.renewal_due' });
      raciExpiryAlerts++;
    }
    ...
    if (expired.rows.length > 0) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:raci.expired' });
  ...
  try {
    ...
    if (incidentSlaAlerts > 0) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:incident.sla_breached' });
    }
  } catch (err: unknown) {
  ...
  try {
    const stale = await safeQuery(
      `SELECT bcp_id, title FROM "${schema}".bcp_plans
    ...
    for (const plan of stale.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:bcp.plan_stale' });
    }
  } catch (err: unknown) {
  ...
  try {
    ...
    if (trainingGaps > 0) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:training.compliance_gap' });
    }
  } catch (err: unknown) {
  ...
  try {
    const overdue = await safeQuery(
      `SELECT e.exercise_id, e.title, e.bcp_plan_id, e.scheduled_date
    ...
    for (const ex of overdue.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:bcp.exercise_overdue' });
    }
  } catch (err: unknown) {
  ...
  try {
    const drifts = await safeQuery(
      `SELECT er.result_id, er.exercise_id, er.rto_actual_hours, er.rpo_actual_hours,
    ...
    for (const d of drifts.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish({
        eventType: 'bcp.rto_rpo_drift' as any, tenantId,
  ...
  try {
    const staleNodes = await safeQuery(
    ...
    for (const node of staleNodes.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
            ...
            } as any)), { tenantId, operation: 'eventBus:bcp.dependency_critical' });
    }
  } catch (err: unknown) {
  ...
  try {
    const staleComms = await safeQuery(
      `SELECT plan_id, title, last_reviewed_at, status
    ...
    for (const cc of staleComms.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish({
        eventType: 'bcp.crisis_readiness_low' as any, tenantId,
  ...
  let bcpBiaAlerts = 0;
  try {
    const staleBia = await safeQuery(
      `SELECT bia_id, title, created_at, criticality_rating
    ...
    for (const bia of staleBia.rows) {
      swallow(EC.AGENT_ACTION, eventBus.publish({
        eventType: 'bcp.bia_stale' as any, tenantId,
  ...
  try {
    const maturity = await safeQuery(
      `SELECT assessment_id, overall_score, assessment_date
    ...
    if (maturity.rows.length >= 2) {
      ...
      if (current < previous) {
        ...
        swallow(EC.AGENT_ACTION, eventBus.publish({
          eventType: 'bcp.maturity_regression' as any, tenantId,
  ...
  try {
    ...
    bcpSpofAlerts = spofs.filter((s: { risk: string }) => s.risk === 'critical' || s.risk === 'high').length;
    for (const spof of spofs.filter((s: { risk: string }) => s.risk === 'critical')) {
      swallow(EC.AGENT_ACTION, eventBus.publish(({
              eventType: 'bcp.dependency_critical', tenantId,
              sourceService: 'autonomous-grc-engine',
...
async function logCycleResult(schema: string, data: Record<string, unknown>): Promise<void> {
  try {
    await safeQuery(`
    ...
    await safeQuery(
      `INSERT INTO "${schema}".autonomous_engine_log
...
export async function getAutonomousEngineStatus(tenantId: string): Promise<unknown> {
  const schema = tenantSchema(tenantId);
  try {
    const latest = await safeQuery(
      `SELECT * FROM "${schema}".autonomous_engine_log ORDER BY created_at DESC LIMIT 1`
    );
    const last24h = await safeQuery(
      `SELECT SUM(total_actions)::int AS total_actions_24h,
...
export async function getAutonomousEngineHistory(tenantId: string, limit: number = 50): Promise<Record<string, unknown>[]> {
  const schema = tenantSchema(tenantId);
  try {
