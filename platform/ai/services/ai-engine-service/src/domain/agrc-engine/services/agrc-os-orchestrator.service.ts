...
async function _runAGRCOSCycleInternal(tenantId: string): Promise<AGRCOSCycleResult> {
  ...
  try {
    swallow(EC.AGENT_ACTION, eventBus.publish(({
        ...
        } as any)), { tenantId, operation: 'eventBus:cycle.started' });
    ...
    try {
      ...
      for (const policy of policiesResult.rows) {
        ...
        for (const rule of rules) {
          const result = (executeRule as any)(rule, context);
          if (result.triggered) {
            policyDecisions++;
    ...
    try {
      ...
      if (esc.escalated > 0) {
        swallow(EC.AGENT_ACTION, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:report.overdue' });
      }
    } catch (err: unknown) {
    ...
    try {
      ...
      if (agentActionsTotal > 0) {
        swallow(EC.AGENT_ACTION, eventBus.publish(({
                ...
                } as any)), { tenantId, operation: 'eventBus:agents.cycle_completed' });
      }
    ...
    swallow(EC.AGENT_ACTION, eventBus.publish(({
        ...
        } as any)), { tenantId, operation: 'eventBus:cycle.completed' });
    ...
    swallow(EC.AGENT_ACTION, eventBus.publish(({
        ...
        } as any)), { tenantId, operation: 'eventBus:cycle.failed' });
...
async function buildTenantContext(tenantId: string): Promise<Record<string, unknown>> {
  const schema = tenantSchema(tenantId);
  const context: Record<string, unknown> = { tenantId };
