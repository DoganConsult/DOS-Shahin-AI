...
export class AgrcEngineScheduler {
  ...
  private service = new AgrcEngineService();
  ...
  start(intervalMs = 5 * 60 * 1000) {
    ...
    this.evalTimer = setInterval(async () => {
      ...
      try {
        const tenants = await this.service.listActiveTenants();
        for (const tenantId of tenants) {
          if (notReadyTenants.has(tenantId)) continue;
          try {
      ...
      try {
        ...
        for (const tenantId of tenants) {
          ...
          try {
            ...
            const consolidatedCount = typeof consolidated === 'number' ? consolidated : (consolidated as any)?.consolidated ?? 0;
