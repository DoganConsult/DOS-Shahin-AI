...
router.get('/integration/status', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('tenant.config.manage'), async (req: Request, res: Response) => {
  try {
...
router.post('/integration/run', authenticate, requirePermission('tenant.config.manage'), heavyOpLimiter, validate({ body: createRunBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    setAuditData(res as any, { action: 'create', entityType: 'integration_cycle', entityId: 'manual' });
...
router.post('/integration/seed-targets', authenticate, requirePermission('tenant.config.manage'), writeLimiter, validate({ body: createSeedTargetsBody }), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    setAuditData(res as any, { action: 'create', entityType: 'agent_monitoring_targets', entityId: 'seed' });
    emitEvent(({ tenantId: req.tenantId, userId: req.user!.userId, module: 'governance', event: 'created', entityType: 'agrc_os', entityId: req.params.id || '' } as any)).catch(catchHandler(EC.EVENT_BUS, {}));
    res.json(result);
  } catch (_err: unknown) { res.status(500).json({ error: errMsg('INTERNAL_ERROR', req) }); }
});
...
router.get('/integration/monitoring-targets', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('platform.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { tenantSchema, safeQuery } = await import('@dos/db');
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(
      `SELECT * FROM "${schema}".agent_monitoring_targets WHERE enabled = TRUE ORDER BY agent_id, priority`
    );
...
router.get('/integration/monitored-controls', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('platform.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { tenantSchema, safeQuery } = await import('@dos/db');
    ...
    const targetsRes = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
      `SELECT agent_id, target_type, target_config FROM "${schema}".agent_monitoring_targets WHERE enabled = TRUE AND target_type = 'framework_focus'`
    ), { tenantId: tenantId, operation: 'query agent_monitoring_targets' });
    ...
    for (const row of targetsRes.rows) {
      ...
      if (Array.isArray(fws)) fws.forEach((f: string) => f && !frameworkIds.includes(f) && frameworkIds.push(f));
      else if (typeof fws === 'string' && !frameworkIds.includes(fws)) frameworkIds.push(fws);
    }
    const controlsByFramework: Record<string, Record<string, unknown>[]> = {};
    let allControls: Record<string, unknown>[] = [];
    for (const fwId of frameworkIds.slice(0, 5)) {
      try {
        const list = await getControls(tenantId, { framework: fwId });
        (controlsByFramework as any)[fwId] = list;
        allControls = allControls.concat((list as any));
      } catch (_) { controlsByFramework[fwId] = []; }
    }
    const unique = Array.from(new Map(allControls.map((c: Record<string, unknown>) => [c.controlId || c.control_id || c.id || c.code, c]).entries()).values()).slice(0, limit);
...
router.get('/integration/feedback', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('platform.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { tenantSchema, safeQuery } = await import('@dos/db');
    const schema = tenantSchema(tenantId);
...
router.get('/integration/handoffs', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('platform.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    const { tenantSchema, safeQuery } = await import('@dos/db');
    const schema = tenantSchema(tenantId);
    ...
    const result = await safeQuery(
      `SELECT * FROM "${schema}".agent_handoffs WHERE status = $1 ORDER BY created_at DESC LIMIT 100`,
      [status]
    );
...
router.get('/integration/mesh', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('platform.agent.read'), async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
    ...
    const [participants, handoffsRows, targetsRows, pendingActions] = await Promise.all([
      ...
      swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
        `SELECT * FROM "${schema}".agent_handoffs WHERE status = 'pending' ORDER BY created_at DESC LIMIT 50`
      ), { tenantId: tenantId, operation: 'query agent_handoffs' }),
      swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(
        `SELECT * FROM "${schema}".agent_monitoring_targets WHERE enabled = TRUE ORDER BY agent_id, priority`
      ), { tenantId: tenantId, operation: 'query agent_handoffs' }),
    ...
    for (const t of targetsRows.rows) {
      ...
      byAgent[(id as any)] = (byAgent[(id as any)] || 0) + 1;
    }
