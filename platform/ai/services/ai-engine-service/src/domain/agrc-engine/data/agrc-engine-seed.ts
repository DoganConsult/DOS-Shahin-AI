...
export async function seedAgrcEngineModule(tenantId: string, schema: string): Promise<void> {
  ...
  for (const [key, value] of Object.entries(data.defaultConfigs)) {
    await safeQuery(
      ...
      ['agrc-engine', key, JSON.stringify(value), tenantId]).catch(catchHandler(EC.EVENT_BUS));
  }
}
