  ...
  async run(ctx: EngineRunContext): Promise<EngineWorkerResult> {
    ...
    for (const row of breached.rows) {
      ...
      const canCreate = await canRunDedupAction(
        ctx.schema, dedupKey, 'kri', row.kri_id, 'kri_breach', 12
      );
      ...
      if (row.owner && (await isFeatureEnabled(ctx.schema, 'agrc_auto_notification_enabled'))) {
        await createNotification(
          ctx.schema, String(row.owner), 'kri_breach',
          'KRI threshold breached',
          `KRI "${row.name}" is in ${row.status.toUpperCase()} state.`,
          `/risk/kris/${row.kri_id}`
        );
