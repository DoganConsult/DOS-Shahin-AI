  ...
  async run(ctx: EngineRunContext): Promise<EngineWorkerResult> {
    ...
    for (const row of overdue.rows) {
      ...
      const canCreate = await canRunDedupAction(
        ctx.schema, dedupKey, 'remediation_task', row.task_id, 'overdue_escalation', 24
      );
      ...
      if (row.assigned_to) {
        await createNotification(
          ctx.schema, String(row.assigned_to), 'remediation_overdue',
          'Remediation overdue',
          `Remediation "${row.title}" is overdue and requires attention.`,
          `/remediation/${row.task_id}`
        );
