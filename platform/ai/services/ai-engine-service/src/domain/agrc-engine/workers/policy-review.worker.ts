  ...
  async run(ctx: EngineRunContext): Promise<EngineWorkerResult> {
    ...
    for (const row of due.rows) {
      ...
      const canCreate = await canRunDedupAction(
        ctx.schema, dedupKey, 'policy', row.policy_id, 'policy_review_due', 168
      );
      ...
      if (await isFeatureEnabled(ctx.schema, 'agrc_auto_task_creation_enabled')) {
        await swallowNull(EC.FALLBACK_QUERY, query(
          ...
          `,
          [`Policy review due: ${row.title}`, row.policy_id, row.owner ?? null]
        ), { operation: 'insert action_items' });
        tasksCreated++;
      }

      if (row.owner && (await isFeatureEnabled(ctx.schema, 'agrc_auto_notification_enabled'))) {
        await createNotification(
          ctx.schema, String(row.owner), 'policy_review_due',
          'Policy review due',
          `Policy "${row.title}" requires review.`,
          `/governance/policies/${row.policy_id}`
        );
