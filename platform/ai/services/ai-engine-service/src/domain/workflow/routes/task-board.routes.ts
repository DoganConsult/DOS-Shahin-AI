...
import { createTasksBody, updateTasksidStatusBody } from '../schemas/task-board.schemas';
...
router.post(
  ...
  validate({ body: createTasksBody }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.user!.tenantId!;
    const userId = req.user!.userId!;
    const { title, description, assignedTo, dueDate, entityType, entityId } = req.body as {
      title: string;
      description?: string;
      assignedTo?: string;
      dueDate?: string;
    ...
    void publishEvent({
      ...
      entityId: task.taskId,
    } as Parameters<typeof publishEvent>[0]).catch(() => {
...
router.put(
  ...
  authenticate,
  requirePermission('workflow.instance.write'),
  validate({ body: updateTasksidStatusBody }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.user!.tenantId!;
    const userId = req.user!.userId!;
