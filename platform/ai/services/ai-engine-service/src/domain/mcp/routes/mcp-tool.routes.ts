...
router.get(
  '/',
  authenticate,
  requirePermission('mcp.tool.read'),
  validate({ query: listToolsQuery }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
    const { page, pageSize, status, agentId, moduleCode, domainCode, category, riskLevel, search, sortBy, sortDir } = req.query as Record<string, string>;
...
router.post(
  '/execute',
  authenticate,
  requirePermission('mcp.tool.execute'),
  validate({ body: executeToolBody }),
  asyncHandler(async (req: Request, res: Response) => {
    const { toolName, arguments: args, agentId, autonomyLevel, traceId } = req.body;
    const tenantId = req.body.tenantId || req.tenantId;
    const userId = req.user?.userId;

    setAuditData(res as any, { action: 'execute', entityType: 'mcp_tool', entityId: toolName });
...
router.patch(
  '/:toolName/status',
  authenticate,
  requirePermission('mcp.tool.manage'),
  validate({ body: toolStatusBody }),
  asyncHandler(async (req: Request, res: Response) => {
    setAuditData(res as any, { action: 'update_status', entityType: 'mcp_tool', entityId: req.params.toolName });
...
router.patch(
  ...
  authenticate,
  requirePermission('mcp.tool.manage'),
  validate({ body: toolEnableBody }),
  asyncHandler(async (req: Request, res: Response) => {
    setAuditData(res as any, { action: req.body.enabled ? 'enable' : 'disable', entityType: 'mcp_tool', entityId: req.params.toolName });
    if (req.body.enabled) {
...
router.get(
  '/stats/execution',
  authenticate,
  requirePermission('mcp.stats.read'),
  validate({ query: executionStatsQuery }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
    const { toolName, limit: _limit } = req.query as Record<string, string>;
...
router.put(
  '/overrides/:toolName',
  authenticate,
  requirePermission('mcp.override.manage'),
  validate({ body: toolOverrideBody }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
    setAuditData(res as any, { action: 'upsert_override', entityType: 'mcp_tool_override', entityId: req.params.toolName });
...
router.delete(
  '/overrides/:toolName',
  authenticate,
  requirePermission('mcp.override.manage'), validate({ body: genericRouteSchema }), asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
    setAuditData(res as any, { action: 'delete_override', entityType: 'mcp_tool_override', entityId: req.params.toolName });
...
router.post(
  '/logs/purge',
  authenticate,
  requirePermission('mcp.admin.manage'),
  validate({ body: purgeLogsBody }),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId;
    setAuditData(res as any, { action: 'purge_logs', entityType: 'mcp_execution_log', entityId: tenantId });
