...
import type { McpDiagnosticsContract } from '../contracts/mcp.contract';
import type { McpHealthReport, ToolExecutionStatsRow } from '@dos/types';
...
export async function getExecutionStats(tenantId: string, toolName?: string): Promise<ToolExecutionStatsRow[]> {
  const rows = await executionRepo.getExecutionStats(tenantId, toolName);
  return rows.map(r => ({
    toolName: r.tool_name as string,
    callCount: (r.call_count as number) || 0,
    avgMs: (r.avg_ms as number) || 0,
    errorCount: (r.error_count as number) || 0,
    lastCalled: (r.last_called as string) || '',
  }));
}

export async function getHealthReport(tenantId: string): Promise<McpHealthReport> {
  const warnings: string[] = [];
  const errors: string[] = [];

  let tools: Awaited<ReturnType<typeof toolRepo.listTools>> = { rows: [], total: 0 };
  let agents: Awaited<ReturnType<typeof agentRepo.listAllAgents>> = [];
  try {
  ...
  if (errors.length > 0) serverStatus = 'down';
  else if (warnings.length > 0) serverStatus = 'degraded';

  const overallHealth = errors.length > 0 ? 'critical' as const : warnings.length > 0 ? 'degraded' as const : 'healthy' as const;
...
export async function getDiagnostics(tenantId: string): Promise<McpDiagnosticsContract> {
...
export async function listApprovalRequests(tenantId: string, status?: string) {
  return executionRepo.listApprovalRequests(tenantId, status);
}

export async function reviewApproval(tenantId: string, requestId: string, decision: 'approved' | 'rejected', reviewedBy: string, notes?: string): Promise<void> {
...
export async function expireOldApprovals(tenantId: string): Promise<number> {
  ...
  if (count > 0) logger.info(`[MCP-Execution] Expired ${count} old approval requests for tenant ${tenantId}`);
  return count;
}
