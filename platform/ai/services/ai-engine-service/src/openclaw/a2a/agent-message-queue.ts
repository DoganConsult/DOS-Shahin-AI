 ...
 */
export async function enqueueA2AMessage(
  tenantId: string,
  fromAgentId: string,
  toAgentId: string,
  messageType: string,
  ...
  const result = await safeQuery(
    `INSERT INTO "${schema}".a2a_messages
       (from_agent_id, to_agent_id, message_type, payload, priority, status)
     VALUES ($1, $2, $3, $4, $5, 'pending')
     RETURNING message_id`,
    [fromAgentId, toAgentId, messageType, JSON.stringify(payload), priority],
  );
 ...
 */
export async function dequeueA2AMessages(
  tenantId: string,
  agentId: string,
  limit: number = 10,
): Promise<A2AMessage[]> {
  ...
  const result = await safeQuery(
    `UPDATE "${schema}".a2a_messages
     ...
     RETURNING message_id, from_agent_id, to_agent_id, message_type, payload, priority, status, created_at`,
    [agentId, limit],
  );
 ...
 */
export async function ackA2AMessage(
  tenantId: string,
  messageId: string,
): Promise<void> {
  const schema = tenantSchema(tenantId);
 ...
 */
export async function nackA2AMessage(
  tenantId: string,
  messageId: string,
  errorMessage: string,
): Promise<void> {
 ...
 */
export async function getA2AQueueStats(
  tenantId: string,
