 ...
 */
export async function startA2AConsumer(tenantId, agentId, handler) {
    let running = true;
    let pollTimer = null;
    ...
    const subscribed = await subscribeToAgent(tenantId, agentId, () => {
        processMessages().catch((err) => logger.warn(`[A2A-Consumer] Process on notification failed: ${toErrorMessage(err)}`));
    });
    ...
    pollTimer = setInterval(() => {
        processMessages().catch((err) => logger.warn(`[A2A-Consumer] Poll cycle failed: ${toErrorMessage(err)}`));
    }, POLL_INTERVAL_MS);
    ...
    return {
        ...
        stop: () => {
            ...
            unsubscribeFromAgent(tenantId, agentId).catch((err) => logger.warn(`[A2A-Consumer] Unsubscribe on stop failed: ${toErrorMessage(err)}`));
            logger.info(`[A2A-Consumer] Stopped consumer for agent ${agentId}`);
        },
    };
}
//# sourceMappingURL=a2a-consumer.js.map