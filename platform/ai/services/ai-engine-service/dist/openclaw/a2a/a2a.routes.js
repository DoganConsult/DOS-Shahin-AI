...
router.post('/enqueue', requirePermission('platform.agent.write'), async (req, res) => {
    try {
...
router.post('/dequeue', requirePermission('platform.agent.read'), async (req, res) => {
    try {
        const tenantId = req.tenantId || req.headers['x-tenant-id'];
        if (!tenantId) {
            res.status(400).json({ error: 'tenantId required' });
        ...
        res.status(500).json({ error: 'Dequeue failed', details: err.message });
    }
});
// POST /:messageId/ack — Acknowledge a processed message
router.post('/:messageId/ack', requirePermission('platform.agent.write'), async (req, res) => {
    try {
        const tenantId = req.tenantId || req.headers['x-tenant-id'];
        if (!tenantId) {
            res.status(400).json({ error: 'tenantId required' });
        ...
        res.status(500).json({ error: 'Ack failed', details: err.message });
    }
});
// POST /:messageId/nack — Negative acknowledge (mark failed)
router.post('/:messageId/nack', requirePermission('platform.agent.write'), async (req, res) => {
    try {
        const tenantId = req.tenantId || req.headers['x-tenant-id'];
        if (!tenantId) {
            res.status(400).json({ error: 'tenantId required' });
        ...
        res.status(500).json({ error: 'Nack failed', details: err.message });
    }
});
...
//# sourceMappingURL=a2a.routes.js.map