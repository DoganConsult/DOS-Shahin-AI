...
aiEngineRouter.get('/events', async (req, res) => {
...
aiEngineRouter.get('/llm-status', async (_req, res) => {
    try {
...
aiEngineRouter.get('/smoke-readiness', async (_req, res) => {
    try {
...
const mountAgrcOsRoutes = async () => {
    try {
        ...
        console.warn('[agrc-os] Core routes failed to mount:', e.message);
    }
...
const lazyMountRoutes = async () => {
    ...
    for (const [label, mod] of mounts) {
        try {
            ...
            console.warn(`[ai-engine] /${label} mount skipped:`, e.message);
        }
    }
};
lazyMountRoutes();
...
const mountA2ARoutes = async () => {
    try {
        ...
        console.warn('[ai-engine] /openclaw/a2a mount skipped:', e.message);
    }
};
mountA2ARoutes();
...
const mountRagRoutes = async () => {
    try {
        ...
        console.warn('[ai-engine] /rag mount skipped:', e.message);
    }
};
mountRagRoutes();
...
const mountQuantumSecurityRoutes = async () => {
    try {
        ...
        console.warn('[ai-engine] /security/quantum mount skipped:', e.message);
    }
};
mountQuantumSecurityRoutes();
aiEngineRouter.get('/health', (req, res) => {
...
async function dashboardHandler(req, res) {
    ...
    if (!tenantId) {
        res.json({ data: empty, tenantId: null });
        return;
    }
    try {
        ...
        const [promotionRows, breaker] = await Promise.all([
          ...
          WHERE rn <= 20
          GROUP BY agent_code
          ORDER BY agent_code`).then((r) => r.rows).catch(() => []),
        ...
        const data = await withTenantClient(tenantId, async (client) => {
            ...
            const [agentExec, hitl, killSw, gov, biasFails, ragSources, domain, outbox,] = await Promise.all([
                safe(`SELECT
             ...
             COALESCE((SELECT SUM(cost_usd) FROM llm_usage_log WHERE created_at > NOW() - INTERVAL '24 hours'), 0)::numeric AS cost`, [], { total: 0, ex24h: 0, ok24h: 0, fail24h: 0, cost: 0 }),
                safe(`SELECT
             ...
             (SELECT count(*) FROM hitl_gates WHERE status = 'approved' AND updated_at > NOW() - INTERVAL '24 hours')::int AS approved24h`, [], { pending: 0, expired: 0, approved24h: 0 }),
                safe(`SELECT
             (SELECT count(*) FROM ai_kill_switches WHERE status = 'armed' AND active = FALSE)::int AS armed,
             (SELECT count(*) FROM ai_kill_switches WHERE active = TRUE)::int AS activated`, [], { armed: 0, activated: 0 }),
                safe(`SELECT
             (SELECT count(*) FROM ai_governance_policies WHERE status = 'active')::int AS policies,
             (SELECT count(*) FROM ai_gov_bias_reports)::int AS bias`, [], { policies: 0, bias: 0 }),
                safe(`SELECT count(*)::int AS fails FROM ai_gov_bias_reports WHERE outcome = 'fail'`, [], { fails: 0 }),
                safe(`SELECT count(*)::int AS sources FROM ai_context_sources WHERE active = TRUE`, [], { sources: 0 }),
                safe(`SELECT
             ...
             (SELECT count(*) FROM evidence_requests)::int AS evidence`, [], { frameworks: 0, controls: 0, risks: 0, vendors: 0, policies: 0, bcp: 0, evidence: 0 }),
                safe(`SELECT
             ...
             (SELECT count(*) FROM event_outbox WHERE status = 'dispatched' AND dispatched_at > NOW() - INTERVAL '24 hours')::int AS dispatched`, [], { pending: 0, failed: 0, dispatched: 0 }),
            ]);
            return {
                agents: {
...
nudgesRouter.get('/active', async (req, res) => {
    try {
        const tenantId = req.tenantId || req.headers['x-tenant-id'];
        if (!tenantId) {
        ...
        try {
            ...
            const rows = await withTenantClient(tenantId, async (client) => {
                const result = await client.query(`SELECT nudge_id AS id, message, message_ar AS "messageAr", priority, module, action_label AS "actionLabel",
        ...
        res.status(500).json({ error: 'Failed to fetch nudges', details: err.message });
    }
});
nudgesRouter.patch('/:nudgeId/dismiss', async (req, res) => {
    try {
        const tenantId = req.tenantId || req.headers['x-tenant-id'];
        if (!tenantId) {
        ...
        try {
            ...
            await withTenantClient(tenantId, async (client) => {
                await client.query(`UPDATE tenant_nudges SET dismissed_at = NOW() WHERE nudge_id = $1`, [req.params.nudgeId]);
            });
        }
        ...
        res.status(500).json({ error: 'Failed to dismiss nudge', details: err.message });
    }
});
