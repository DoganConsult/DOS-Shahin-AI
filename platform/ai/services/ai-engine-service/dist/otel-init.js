 ...
 */
const ENABLED = process.env.OTEL_TRACING_ENABLED === 'true' || process.env.OTEL_ENABLED === 'true';
if (ENABLED) {
    ...
    try {
        ...
        console.log(`[otel] tracing enabled → ${process.env.OTEL_EXPORTER_OTLP_ENDPOINT}/v1/traces ` +
            `(service=${process.env.OTEL_SERVICE_NAME || 'ai-engine-service'})`);
    }
    catch (err) {
        console.warn('[otel] failed to initialise SDK — tracing disabled:', err.message);
    }
}
export {};
