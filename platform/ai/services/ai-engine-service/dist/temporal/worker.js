...
let _bootstrapErrors = [];
let _temporalConnected = false;
let _bootstrapAt = Date.now();
function initWorkerEventBus(serviceCode) {
    ...
    setEventBus({
        ...
        subscribe: (eventType, _subscriberId, handler) => {
...
async function initWorkerDb() {
    ...
    try {
        ...
        logger.warn('[ai-os-temporal-worker] @dos/db preflight failed: ' + err.message);
    }
}
    ...
    const server = http.createServer((req, res) => {
        if (req.url === '/health' || req.url === '/ready') {
            const workerStates = _runningWorkers.map(h => ({
                instance: h.instance,
                state: typeof h.worker.getState === 'function' ? h.worker.getState() : 'unknown',
            }));
...
async function run() {
    ...
    try {
        ...
        _bootstrapErrors.push(`dsoc-audit-bridge: ${err.message}`);
        logger.warn('[ai-os-temporal-worker] DSOC audit bridge install skipped: ' + err.message);
    }
    ...
    for (let i = 0; i < WORKER_INSTANCES; i++) {
        ...
        created.push(worker.run().catch(err => {
            logger.error(`[ai-os-temporal-worker:${i}] poller exited: ${err.message}`);
        }));
    }
...
run().catch(err => {
    ...
    _bootstrapErrors.push(`fatal: ${err.message}`);
    logger.error('[ai-os-temporal-worker] fatal: ' + err.message);
    process.exit(1);
});
