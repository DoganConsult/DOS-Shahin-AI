...
function classifyAndRethrow(err, executionId) {
    const e = err;
...
export async function invokeAgent(input) {
    ...
    try {
        ...
        const runner = mod.runAgent || mod.default;
        if (typeof runner === 'function') {
            const query = typeof input.input?.query === 'string'
                ? input.input.query
 ...
 */
export async function checkGovernance(input) {
...
export async function recordExecution(input) {
    ...
    try {
        ...
        if (typeof dbMod.withTenantClient === 'function') {
            await dbMod.withTenantClient(input.tenantId, async (client) => {
                await client.query(`INSERT INTO ai_agent_executions (agent_code, status, output_result, completed_at)
           VALUES ($1, $2, $3::jsonb, NOW())
           ON CONFLICT DO NOTHING`, [input.agentCode, input.result.status, JSON.stringify(input.result)]).catch(() => undefined);
            });
        }
    }
    catch (err) {
