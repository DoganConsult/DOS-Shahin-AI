...
Respond with JSON: { discoveries: [{type, detail, confidence}], proposedActions: [{type, target, detail}], summary: string, done: boolean }`;
    ...
    // Load context
    const contextRows = await safeQuery(`SELECT signal_type, value, metadata, created_at
    ...
    // Finalize metrics
    if (metricsCallback && typeof metricsCallback.finalize === 'function') {
        await metricsCallback.finalize('success');
    }
    return {
 ...
 */
async function runWithDirectLoop(tenantId, agentId, runId, opts) {
    const maxIter = opts.maxIterations || LANGGRAPH_CONFIG.maxToolIterations || 5;
    const result = {
        discoveries: [],
        proposedActions: [],
...
Analyze and respond with JSON.`,
            ...
            try {
                ...
                if (parsed.discoveries)
                    result.discoveries.push(...(Array.isArray(parsed.discoveries) ? parsed.discoveries : []));
                if (parsed.proposedActions)
                    result.proposedActions.push(...(Array.isArray(parsed.proposedActions) ? parsed.proposedActions : []));
                if (parsed.summary)
                    result.summary = parsed.summary;
                if (parsed.done || (i > 0 && !parsed.discoveries?.length && !parsed.proposedActions?.length))
                    break;
            }
            catch {
...
//# sourceMappingURL=single-agent.graph.js.map