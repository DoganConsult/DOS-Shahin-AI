 ...
 */
export async function getHITLQueue(query, opts = {}) {
    ...
    for (const schema of targetSchemas) {
      ...
      LIMIT $2`, [schema, limit]).catch(() => ({ rows: [] }));
...
export async function getHITLBacklogSummary(query) {
    const out = [];
    for (const schema of KNOWN_TENANT_SCHEMAS) {
       ...
       FROM "${schema}".copilot_proposed_actions
      WHERE status IN ('pending','escalated')`, []).catch(() => ({ rows: [{ pending: 0, escalated: 0, oldest_age: 0 }] }));
...
//# sourceMappingURL=hitl-queue.service.js.map