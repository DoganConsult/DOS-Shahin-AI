 ...
 */
async function ensureVectorTable(schema) {
 ...
 */
export async function indexDocument(tenantId, content, sourceType, sourceId, metadata = {}) {
    const schema = tenantSchema(tenantId);
    await ensureVectorTable(schema);
    const hash = contentHash(content);
    const embedding = await generateEmbedding(content);
    const embeddingStr = `[${embedding.join(',')}]`;
    const result = await safeQuery(`INSERT INTO "${schema}".rag_embeddings
       (content, content_hash, source_type, source_id, embedding, metadata, tenant_id)
     VALUES ($1, $2, $3, $4, $5::vector, $6, $7)
     ON CONFLICT (document_id) DO UPDATE SET
       content = EXCLUDED.content, embedding = EXCLUDED.embedding,
       metadata = EXCLUDED.metadata, updated_at = NOW()
     RETURNING document_id`, [content, hash, sourceType, sourceId, embeddingStr, JSON.stringify(metadata), tenantId]);
 ...
 */
export async function indexDocumentWithChunking(tenantId, content, sourceType, sourceId, metadata = {}, chunkOptions) {
    const schema = tenantSchema(tenantId);
    await ensureVectorTable(schema);
    ...
    const parentResult = await safeQuery(`INSERT INTO "${schema}".rag_embeddings
       (content, content_hash, source_type, source_id, metadata, tenant_id, chunk_index)
     VALUES ($1, $2, $3, $4, $5, $6, -1)
     RETURNING document_id`, [content.slice(0, 500) + '...', contentHash(content), sourceType, sourceId, JSON.stringify({ ...metadata, isParent: true, chunkCount: chunks.length }), tenantId]);
 ...
 */
export async function batchIndexDocuments(tenantId, documents, chunkOptions) {
    const results = [];
    for (const doc of documents) {
        try {
            const result = await indexDocumentWithChunking(tenantId, doc.content, doc.sourceType, doc.sourceId || '', doc.metadata || {}, chunkOptions);
            results.push(result);
        }
        catch (err) {
...
export async function semanticSearch(opts) {
    ...
    WHERE 1 - (embedding <=> $1::vector) > $2
      AND chunk_index >= 0
  `;
    const params = [embeddingStr, minSim];
...
export async function hybridSearch(opts) {
    ...
    const embeddingStr = `[${queryEmbedding.join(',')}]`;
    const limit = opts.limit || 10;
    const result = await safeQuery(`SELECT document_id, content, metadata, source_type, source_id,
     ...
     LIMIT ${limit}`, [embeddingStr, opts.query]);
    return result.rows.map((row) => ({
        documentId: row.document_id,
        content: row.content,
        metadata: typeof row.metadata === 'string' ? JSON.parse(row.metadata) : row.metadata || {},
...
export async function deleteDocument(tenantId, documentId) {
    ...
    const childResult = await safeQuery(`DELETE FROM "${schema}".rag_embeddings WHERE parent_document_id = $1 AND tenant_id = $2`, [documentId, tenantId]);
    // Delete the document itself
    const parentResult = await safeQuery(`DELETE FROM "${schema}".rag_embeddings WHERE document_id = $1 AND tenant_id = $2`, [documentId, tenantId]);
    return (childResult.rowCount ?? 0) + (parentResult.rowCount ?? 0);
}
/**
 * Get index statistics for a tenant.
 */
export async function getIndexStats(tenantId) {
    const schema = tenantSchema(tenantId);
    await ensureVectorTable(schema);
    const countResult = await safeQuery(`SELECT
    ...
    const byTypeResult = await safeQuery(`SELECT source_type, COUNT(*)::int AS count
     FROM "${schema}".rag_embeddings
     WHERE tenant_id = $1 AND chunk_index >= 0
     GROUP BY source_type`, [tenantId]);
    const row = countResult.rows[0] || {};
    const bySourceType = {};
    for (const r of byTypeResult.rows) {
        bySourceType[r.source_type] = r.count;
    }
    return {
        ...
        newestDocument: row.newest || null,
    };
}
