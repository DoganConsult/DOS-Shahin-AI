 ...
 */
export async function registerTool(tenantId, tool) {
    const schema = tenantSchema(tenantId);
    await safeQuery(`INSERT INTO "${schema}".tool_registry
    ...
    // Record version
    await safeQuery(`INSERT INTO "${schema}".tool_versions
 ...
 */
export async function getTool(tenantId, toolId, version) {
    const schema = tenantSchema(tenantId);
    let query;
    let params;
    if (version) {
 ...
 */
export async function listTools(tenantId, category, includeDeprecated = false) {
    const schema = tenantSchema(tenantId);
    ...
    try {
        const result = await safeQuery(`SELECT DISTINCT ON (id) *
       FROM "${schema}".tool_registry
       WHERE ${conditions.join(' AND ')}
       ORDER BY id, version DESC`, params);
 ...
 * Get tool versions
 */
export async function getToolVersions(tenantId, toolId) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`SELECT * FROM "${schema}".tool_versions
       WHERE tool_id = $1
       ORDER BY released_at DESC`, [toolId]);
        return result.rows.map((row) => ({
            toolId: row.tool_id,
            version: row.version,
 ...
 */
export async function deprecateToolVersion(tenantId, toolId, version, replacedBy) {
    const schema = tenantSchema(tenantId);
    await safeQuery(`UPDATE "${schema}".tool_registry
     SET deprecated_at = NOW(), replaced_by = $1, updated_at = NOW()
     WHERE id = $2 AND version = $3`, [replacedBy || null, toolId, version]);
    await safeQuery(`UPDATE "${schema}".tool_versions
     SET is_deprecated = TRUE, is_default = FALSE
     WHERE tool_id = $1 AND version = $2`, [toolId, version]).catch(catchHandler(EC.EVENT_BUS, {}));
}
