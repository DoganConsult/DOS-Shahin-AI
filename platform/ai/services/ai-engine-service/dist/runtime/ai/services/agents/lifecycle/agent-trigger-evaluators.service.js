 ...
 */
async function evaluateA01Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Check for incomplete onboarding
        const incomplete = await safeQuery(`SELECT COUNT(*)::int AS n FROM tenants
       WHERE tenant_id = $1
         AND (org_type IS NULL OR primary_sector_id IS NULL OR grc_maturity_level IS NULL)`, [tenantId]);
        ...
        const missingFrameworks = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".frameworks WHERE deleted_at IS NULL`);
 ...
 */
async function evaluateA02Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Check MFA enforcement
        const mfaNotEnforced = await safeQuery(`SELECT COUNT(*)::int AS n FROM users
       WHERE tenant_id = $1
         AND (mfa_enabled = FALSE OR mfa_enabled IS NULL)
         AND role != 'viewer'`, [tenantId]);
        ...
        try {
            const staleReviews = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".access_reviews
 ...
 */
async function evaluateA03Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        const unmapped = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".ucf_controls
 ...
 */
async function evaluateA04Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        const missingEvidence = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".ucf_controls c
       WHERE NOT EXISTS (
 ...
 */
async function evaluateA05Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        ...
        // Coverage gaps
        const coverageGaps = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".ucf_controls c
       WHERE NOT EXISTS (
         SELECT 1 FROM "${schema}".evidence e
 ...
 */
async function evaluateA06Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Overdue gaps
        const overdue = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".compliance_gaps
 ...
 */
async function evaluateA07Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        ...
        // Unscored risks
        const unscored = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".risks
 ...
 */
async function evaluateA08Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Policies expiring
        const expiring = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".policies
       WHERE review_date IS NOT NULL
        ...
        // Approval delays
        const delayed = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".policies
 ...
 */
async function evaluateA09Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Contract expiry
        const expiring = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".vendors
       WHERE contract_end_date IS NOT NULL
        ...
        // SLA violations
        try {
            const slaBreaches = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".vendor_sla_measurements
 ...
 */
async function evaluateA10Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Report schedules due
        const reportsDue = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".report_schedules
       WHERE enabled = TRUE
 ...
 */
async function evaluateA11Triggers(tenantId) {
    const triggers = [];
    try {
 ...
 */
async function evaluateA12Triggers(tenantId) {
    const triggers = [];
    const schema = tenantSchema(tenantId);
    try {
        // Check for integration errors (if integration_logs table exists)
        try {
            const errors = await safeQuery(`SELECT COUNT(*)::int AS n FROM "${schema}".integration_logs
         WHERE status = 'error'
 ...
 */
export const AGENT_TRIGGER_EVALUATORS = {
...
//# sourceMappingURL=agent-trigger-evaluators.service.js.map