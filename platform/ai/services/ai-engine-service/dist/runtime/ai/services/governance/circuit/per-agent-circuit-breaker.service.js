 ...
 */
async function getCircuitState(tenantId, agentId) {
    ...
    try {
        const result = await safeQuery(`SELECT agent_id, state, failure_count, success_count, last_failure_at, last_success_at, opened_at, total_calls, total_failures
 ...
 */
async function saveCircuitState(state) {
    const schema = tenantSchema(state.tenantId);
    await safeQuery(`INSERT INTO "${schema}".agent_circuit_breaker
 ...
 */
export async function canCallAgent(tenantId, agentId, config = {}) {
 ...
 */
export async function recordSuccess(tenantId, agentId, config = {}) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    const state = await getCircuitState(tenantId, agentId);
    const key = `${tenantId}:${agentId}`;
    state.totalCalls++;
    ...
    if (state.state === 'HALF_OPEN') {
        if (state.successCount >= cfg.successThreshold) {
            ...
            await swallow(EC.EVENT_BUS, eventBus.publish({
                eventType: 'ai.circuit_breaker.closed',
                tenantId,
 ...
 */
export async function recordFailure(tenantId, agentId, config = {}) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    const state = await getCircuitState(tenantId, agentId);
    const key = `${tenantId}:${agentId}`;
    state.totalCalls++;
    ...
    if (state.state === 'HALF_OPEN') {
        ...
        state.successCount = 0;
        await swallow(EC.EVENT_BUS, eventBus.publish({
            eventType: 'ai.circuit_breaker.opened',
            tenantId,
            sourceService: 'per-agent-circuit-breaker',
            severity: 'warning',
 ...
 */
export async function getAgentCircuitState(tenantId, agentId) {
    return await getCircuitState(tenantId, agentId);
}
/**
 * Get all circuit breaker states for a tenant
 */
export async function getAllCircuitStates(tenantId) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`SELECT agent_id, state, failure_count, success_count, last_failure_at, last_success_at, opened_at, total_calls, total_failures
       FROM "${schema}".agent_circuit_breaker
 ...
 */
export async function resetCircuitBreaker(tenantId, agentId) {
    const key = `${tenantId}:${agentId}`;
    const state = await getCircuitState(tenantId, agentId);
    state.state = 'CLOSED';
    state.failureCount = 0;
    ...
    await saveCircuitState(state);
    await swallow(EC.EVENT_BUS, eventBus.publish({
        eventType: 'ai.circuit_breaker.reset',
        tenantId,
        sourceService: 'per-agent-circuit-breaker',
        severity: 'info',
        payload: { agentId },
...
//# sourceMappingURL=per-agent-circuit-breaker.service.js.map