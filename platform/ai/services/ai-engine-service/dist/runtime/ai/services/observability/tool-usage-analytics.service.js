 ...
 * Record tool usage
 */
export async function recordToolUsage(record) {
    const schema = tenantSchema(record.tenantId);
    await safeQuery(`INSERT INTO "${schema}".tool_usage_log
 ...
 */
export async function getToolUsageStats(tenantId, toolId, daysBack = 30) {
    ...
    try {
       ...
       ORDER BY total_calls DESC`, [tenantId, ...params]);
        ...
        // Get per-agent stats
        if (toolId) {
            const agentResult = await safeQuery(`SELECT agent_id, tool_id, tool_version,
                COUNT(*)::int AS calls,
         ...
         GROUP BY agent_id, tool_id, tool_version`, [tenantId, toolId, daysBack]);
            ...
            // Get error types
            const errorResult = await safeQuery(`SELECT error_message, COUNT(*)::int AS count
         FROM "${schema}".tool_usage_log
         WHERE tenant_id = $1 AND tool_id = $2 AND NOT success
           AND created_at > NOW() - make_interval(days => $3)
           AND error_message IS NOT NULL
         GROUP BY error_message`, [tenantId, toolId, daysBack]);
 ...
 * Get most used tools
 */
export async function getMostUsedTools(tenantId, limit = 10, daysBack = 30) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`SELECT tool_id,
              COUNT(*)::int AS calls,
       ...
       GROUP BY tool_id
       ORDER BY calls DESC
       LIMIT $3`, [tenantId, daysBack, limit]);
...
//# sourceMappingURL=tool-usage-analytics.service.js.map