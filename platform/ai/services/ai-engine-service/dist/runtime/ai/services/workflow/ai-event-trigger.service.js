...
async function ensureEventTriggerBindingTable(schema) {
    await safeQuery(`CREATE TABLE IF NOT EXISTS "${schema}".event_trigger_binding (
    ...
    await safeQuery(`CREATE UNIQUE INDEX IF NOT EXISTS uq_event_trigger_binding_tenant_event_agent
      ON "${schema}".event_trigger_binding (tenant_id, event_type, target_agent_id)`, []).catch(() => undefined);
    await safeQuery(`CREATE INDEX IF NOT EXISTS idx_event_trigger_binding_tenant_event
      ON "${schema}".event_trigger_binding (tenant_id, event_type)`, []).catch(() => undefined);
    await safeQuery(`CREATE INDEX IF NOT EXISTS idx_event_trigger_binding_tenant_enabled
      ON "${schema}".event_trigger_binding (tenant_id, enabled)`, []).catch(() => undefined);
}
async function seedDefaultBindings(tenantId, schema) {
    const defaults = [
    ...
    for (const d of defaults) {
       ...
       ON CONFLICT (tenant_id, event_type, target_agent_id) DO NOTHING`, [tenantId, d.eventType, d.agentId, JSON.stringify(d.conditionJson || {}), d.cooldownSeconds, SYSTEM_JOB_ACTOR]).catch(() => undefined);
    }
}
export async function processEventTriggers(tenantId, eventType, payload) {
    ...
    try {
        ...
        const result = await safeQuery(`SELECT * FROM "${schema}".event_trigger_binding
       WHERE tenant_id = $1 AND event_type = $2 AND enabled = TRUE`, [tenantId, eventType]);
        for (const binding of result.rows) {
            ...
            const now = Date.now();
            if (now - lastTriggered < binding.cooldown_seconds * 1000)
                continue;
            if (binding.condition_json && Object.keys(binding.condition_json).length > 0) {
                if (!matchesCondition(binding.condition_json, payload))
                    continue;
            }
            _cooldownMap.set(cooldownKey, now);
            await safeQuery(`UPDATE "${schema}".event_trigger_binding SET last_triggered_at = NOW() WHERE binding_id = $1`, [binding.binding_id]);
        ...
        logger.warn(`[AI EventTrigger] processEventTriggers failed for ${eventType}: ${err.message}`);
    }
    return triggered;
}
async function executeBindingAction(tenantId, binding, payload) {
    try {
        ...
        logger.warn(`[AI EventTrigger] executeBindingAction failed for ${binding.binding_id}/${binding.action_type}: ${err.message}`);
    }
}
    ...
    for (const [key, expected] of Object.entries(condition)) {
        ...
        if (expected && typeof expected === 'object' && !Array.isArray(expected)) {
            ...
            if (Array.isArray(e.$in)) {
                if (!e.$in.includes(actual))
                    return false;
                continue;
            }
            if (e.$prefix !== undefined) {
                if (typeof actual !== 'string' || !actual.startsWith(String(e.$prefix)))
                    return false;
                continue;
            }
...
export async function listEventTriggerBindings(tenantId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT * FROM "${schema}".event_trigger_binding WHERE tenant_id = $1 ORDER BY event_type`, [tenantId]);
    return result.rows;
}
export async function createEventTriggerBinding(tenantId, input) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`INSERT INTO "${schema}".event_trigger_binding
       (tenant_id, event_type, target_agent_id, action_type, condition_json, cooldown_seconds, created_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7)
     RETURNING *`, [tenantId, input.eventType, input.targetAgentId, input.actionType || 'run_agent', JSON.stringify(input.conditionJson || {}), input.cooldownSeconds || 60, input.createdBy || SYSTEM_JOB_ACTOR]);
    return getFirstRow(result) || null;
}
export async function deleteEventTriggerBinding(tenantId, bindingId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`DELETE FROM "${schema}".event_trigger_binding WHERE binding_id = $1 AND tenant_id = $2 RETURNING binding_id`, [bindingId, tenantId]);
    return (result.rows?.length || 0) > 0;
}
export async function toggleEventTriggerBinding(tenantId, bindingId, enabled) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`UPDATE "${schema}".event_trigger_binding SET enabled = $3 WHERE binding_id = $1 AND tenant_id = $2 RETURNING binding_id`, [bindingId, tenantId, enabled]);
    return (result.rows?.length || 0) > 0;
}
export async function testFireBinding(tenantId, bindingId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT * FROM "${schema}".event_trigger_binding WHERE binding_id = $1 AND tenant_id = $2`, [bindingId, tenantId]);
    const binding = getFirstRow(result);
    if (!binding)
        return { fired: false, actionType: '' };
    await executeBindingAction(tenantId, binding, { _testFire: true });
    return { fired: true, actionType: binding.action_type };
}
export async function updateEventTriggerBinding(tenantId, bindingId, updates) {
    const schema = tenantSchema(tenantId);
    const setClauses = [];
    const params = [];
    let idx = 1;
    ...
    const result = await safeQuery(`UPDATE "${schema}".event_trigger_binding SET ${setClauses.join(', ')} WHERE binding_id = $${idx++} AND tenant_id = $${idx} RETURNING *`, params);
    return getFirstRow(result) || null;
}
export async function getTriggerFireLog(tenantId, hours = 24) {
    const schema = tenantSchema(tenantId);
    try {
...
export function initEventTriggerListener() {
    eventBus.onAfterPublish('ai-event-trigger-dispatcher', async (event) => {
        if (!event.tenantId || event.tenantId === SYSTEM_TENANT)
            return;
        try {
            await processEventTriggers(event.tenantId, event.eventType, event.payload);
        }
        catch (err) {
            logger.warn(`[AI EventTrigger] afterPublish dispatch failed: ${err.message}`);
        }
    });
...
//# sourceMappingURL=ai-event-trigger.service.js.map