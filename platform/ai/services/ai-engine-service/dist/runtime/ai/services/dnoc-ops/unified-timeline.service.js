 ...
 */
export async function getUnifiedTimeline(query, langfuseQuery, opts = {}) {
    ...
    const surfaceFilter = (e) => !opts.surface || e.surface === opts.surface;
    const agentFilter = (e) => !opts.agentId || e.agentId === opts.agentId;
    const kindFilter = (e) => !opts.kindPrefix || (typeof e.kind === 'string' && e.kind.startsWith(opts.kindPrefix));
    const tenantParam = opts.tenantId ? [opts.tenantId] : [];
    // 1. dos.audit_trail (every tool-gate decision + run completion)
    const auditPromise = query(`SELECT entry_id, tenant_id, actor_id, action, entity_type, entity_id, module, payload, created_at
       FROM dos.audit_trail
      WHERE created_at >= $1 AND created_at <= $2
        ${tenantClause.replace('$TENANT$', '$3')}
        AND module IN ('ai', 'ai-governance', 'ai-soc', 'ai-noc')
      ORDER BY created_at DESC
      LIMIT $${tenantClause ? 4 : 3}`, [since, until, ...tenantParam, limit]).catch(() => ({ rows: [] }));
    // 2. dos.ai_workflow_trigger_log (cron + event triggers)
    const triggerPromise = query(`SELECT log_id, tenant_id, trigger_id, signal, payload, emitted_at
    ...
    const perfPromise = query(`SELECT id, tenant_id, agent_id, session_id, duration_ms, tokens_in, tokens_out,
    ...
    const langfusePromise = langfuseQuery
        ? langfuseQuery(`SELECT id, name, tags, user_id, session_id, metadata, created_at
           FROM traces
          WHERE created_at >= $1 AND created_at <= $2
          ORDER BY created_at DESC
          LIMIT $3`, [since, until, limit]).catch(() => ({ rows: [] }))
    ...
    for (const r of auditRes.rows) {
        ...
        events.push({
            ...
            actorId: r.actor_id,
            payload,
            status: r.action.endsWith('.denied') || r.action.endsWith('.failed')
                ? 'denied'
...
//# sourceMappingURL=unified-timeline.service.js.map