...
export async function getActivityTimeline(tenantId, entityType, entityId, daysBack = 7) {
    ...
    try {
       ...
       LIMIT 200`, [entityType, entityId, safeDays]);
        return rows;
    }
    catch (err) {
...
export async function findCorrelatedActivities(tenantId, entityType, entityId, timeWindowMs = 300_000) {
    const schema = tenantSchema(tenantId);
    const safeWindowMinutes = Math.max(1, Math.min(Math.round(timeWindowMs / 60_000), 1440));
    try {
        const { rows: anchor } = await safeQuery(`SELECT created_at FROM "${schema}".activity_feed
       WHERE entity_type = $1 AND entity_id = $2
       ...
       ORDER BY created_at DESC
       LIMIT 50`, [anchorTime, safeWindowMinutes, entityType, entityId]);
        return rows;
    }
    catch (err) {
...
//# sourceMappingURL=activity-correlation.service.js.map