...
function mapCriticalityToPriority(criticality, hasEvidence) {
    if (criticality === 'critical' || criticality === 'mandatory')
        return 'critical';
    if (criticality === 'high')
        return hasEvidence ? 'medium' : 'high';
    if (criticality === 'medium')
        return 'medium';
    return 'low';
}
/** Estimate effort in days based on control type and evidence gaps */
function estimateEffort(criticality, missingEvidenceCount, automationPossible) {
    let baseDays = 3;
    if (criticality === 'critical' || criticality === 'mandatory')
        baseDays = 7;
...
function suggestOwnerRole(domainCode, criticality) {
    ...
    if (domainRoleMap[upper])
        return domainRoleMap[upper];
...
function generateSteps(controlTitle, currentStatus, missingEvidenceCount, automationPossible) {
    const steps = [];
    // Step 1: Assessment
 ...
 */
export async function generateRemediationPlan(tenantId, frameworkCode) {
    throw new Error("Not implemented: Stubbed during microservice extraction");
}
//# sourceMappingURL=ai-remediation.service.js.map