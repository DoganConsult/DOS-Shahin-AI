...
function agentName(agentId, lang = 'en') {
...
function extractAgentId(row) {
    ...
    if (payload.agent_id && /^A\d{2}$/.test(payload.agent_id))
        return payload.agent_id;
    if (payload.agentId && /^A\d{2}$/.test(payload.agentId))
 ...
 */
function extractActionType(eventType) {
    if (eventType.includes('task') || eventType.includes('create_task'))
        return 'create_task';
    if (eventType.includes('notification') || eventType.includes('send_notification'))
        return 'send_notification';
    if (eventType.includes('risk') || eventType.includes('flag_risk'))
        return 'flag_risk';
    if (eventType.includes('evidence') || eventType.includes('request_evidence'))
 ...
 */
export async function getActivityFeed(tenantId, opts) {
    ...
    const result = await safeQuery(`SELECT event_id, event_type, source_service, entity_type, entity_id, severity, payload, created_at
 ...
 */
export async function getAgentPerformanceStats(tenantId) {
    const schema = tenantSchema(tenantId);
    // Fetch all agent-related events in one query
    const eventsResult = await safeQuery(`SELECT event_type, source_service, payload, created_at
     FROM "${schema}".agrc_event_log
    ...
    for (const row of eventsResult.rows) {
        ...
        if (createdAt >= sevenDaysAgo)
            stats.last7d++;
        if (payload.was_overridden === true || payload.overridden === true)
            stats.overridden++;
    ...
    const taskStatsResult = await safeQuery(`SELECT
 ...
 */
export async function getAgentCooperationTimeline(tenantId, limit = 30) {
    const schema = tenantSchema(tenantId);
...
//# sourceMappingURL=agent-activity-feed.service.js.map