...
async function getMonthlyBudget(schema) {
    try {
        const result = await safeQuery(`SELECT monthly_token_budget, monthly_cost_budget
...
function classifyTrend(slope, dailyAvg) {
    if (dailyAvg === 0)
        return 'stable';
...
function generateRecommendations(trend, alertLevel, topAgent, daysUntilExhausted) {
    const recs = [];
 ...
 */
export async function getUsageForecast(tenantId) {
    ...
    const dailyResult = await safeQuery(`SELECT
     ...
     ORDER BY day ASC`);
    const dailyPoints = dailyResult.rows.map((r, i) => ({
        x: i,
        y: Number(r.tokens || 0),
    }));
    const costPoints = dailyResult.rows.map((r, i) => ({
        x: i,
        y: Number(r.cost || 0),
    }));
    ...
    const projectedRemainingTokens = Math.max(0, remainingDays * (tokenRegression.slope * (dailyPoints.length + remainingDays / 2) + tokenRegression.intercept));
    const projectedRemainingCost = Math.max(0, remainingDays * (costRegression.slope * (costPoints.length + remainingDays / 2) + costRegression.intercept));
    ...
    // Per-agent breakdown
    const agentResult = await safeQuery(`SELECT
       agent_id,
    ...
    const perAgentBreakdown = agentResult.rows.map((r) => ({
 ...
 */
export async function getUsageTrend(tenantId, daysBack = 30) {
    const schema = tenantSchema(tenantId);
 ...
 */
export async function checkBudgetAlerts(tenantId) {
    const schema = tenantSchema(tenantId);
    // Current month spend
    const monthResult = await safeQuery(`SELECT
       COALESCE(SUM(cost_usd), 0)::real AS total_cost,
