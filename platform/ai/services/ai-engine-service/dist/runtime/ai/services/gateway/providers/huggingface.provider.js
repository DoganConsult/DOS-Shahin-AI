...
export function callHuggingFaceApi(config, messages, maxTokens, overrideModel) {
        ...
        const req = https.request({
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.error) {
                        reject(new Error(`[huggingface] ${toErrorMessage(parsed.error) || JSON.stringify(parsed.error)}`));
                        return;
                    }
                    resolve({
