...
export async function persistProposedActions(tenantId, sessionId, userId, agentId, actions) {
    ...
    await withTransaction(tenantId, async (client) => {
        for (const action of actions) {
            ...
            const autoExecAt = timerSeconds
                ...
                : null;
            await safeQueryWithClient(`INSERT INTO "${schema}".copilot_proposed_actions
         ...
         WHERE (proposed_actions.title, proposed_actions.priority) IS DISTINCT FROM (EXCLUDED.title, EXCLUDED.priority)`, [actionId, sessionId, userId, agentId, action.type, action.title,
    ...
    }), { tenantId, operation: 'eventBus:copilot.actions_proposed' });
    return results;
}
// ── Quality Gate: Pre-Execution ───────────────────────────────────────────
async function validatePreExecution(tenantId, action) {
    const schema = tenantSchema(tenantId);
    const checks = [];
    // 1. Entity exists
    if (action.entity_id && action.entity_type) {
        ...
        if (mapping) {
            try {
                const exists = await safeQuery(`SELECT 1 FROM "${schema}"."${mapping.table}" WHERE "${mapping.pk}" = $1 LIMIT 1`, [action.entity_id]);
    ...
    if (action.entity_id && action.entity_type) {
        const dup = await safeQuery(`SELECT 1 FROM "${schema}".copilot_proposed_actions
       WHERE action_type = $1 AND entity_type = $2 AND entity_id = $3
         AND status IN ('completed', 'auto_completed')
         AND executed_at > NOW() - INTERVAL '24 hours'
         AND action_id != $4 LIMIT 1`, [action.action_type, action.entity_type, action.entity_id, action.action_id]);
    ...
    if (action.entity_id && action.entity_type) {
        const conflict = await safeQuery(`SELECT action_id, action_type, agent_id FROM "${schema}".copilot_proposed_actions
       WHERE entity_type = $1 AND entity_id = $2
         AND status IN ('executing', 'auto_executing')
         AND action_id != $3 LIMIT 1`, [action.entity_type, action.entity_id, action.action_id]);
        checks.push({
            name: 'no_conflict',
    ...
    if (opposites.length > 0 && action.entity_id) {
        const contradiction = await safeQuery(`SELECT action_type, agent_id FROM "${schema}".copilot_proposed_actions
       WHERE entity_type = $1 AND entity_id = $2
         AND action_type = ANY($3)
         AND status IN ('completed', 'auto_completed')
         AND executed_at > NOW() - INTERVAL '1 hour'
       LIMIT 1`, [action.entity_type, action.entity_id, opposites]);
        checks.push({
...
async function validatePostExecution(tenantId, action, delegationResult) {
    const checks = [];
    ...
    if (['create_task', 'flag_risk', 'request_evidence', 'create_remediation'].includes(action.action_type)) {
        try {
            const schema = tenantSchema(tenantId);
            const recent = await safeQuery(`SELECT task_id, team_id, assigned_user_id, sla_hours
...
// ── Build Action Trail ────────────────────────────────────────────────────
async function buildActionTrail(tenantId, delegationResult, action) {
    const schema = tenantSchema(tenantId);
    const trail = {
    ...
    // Get grant scopes
    try {
        const grant = await safeQuery(`SELECT scopes FROM "${schema}".delegation_grants WHERE grant_id = $1`, [delegationResult.grantId]);
        if (grant.rows.length) {
            ...
            trail.scope = scopes.join(', ');
        }
    }
    catch { /* non-critical */ }
    // Get process task details
    try {
        const task = await safeQuery(`SELECT pt.task_id, pt.team_id, pt.assigned_user_id, pt.sla_hours, t.team_name
    ...
    try {
        const { safeQuery: sq, tenantSchema: ts } = await import('@dos/db');
        const getRACIMatrix = async (tid, scopeType, scopeId) => { const s = ts(tid); const r = await sq(`SELECT raci_type, team_code as "teamName", platform_role as "platformRole" FROM "${s}".raci_assignments WHERE scope_type=$1 AND scope_id=$2`, [scopeType, scopeId]); const m = { responsible: [], accountable: [], consulted: [], informed: [] }; for (const row of r.rows) {
            if (m[row.raci_type])
                m[row.raci_type].push(row);
        } return m; };
        const domainScopes = {
        ...
        if (ds) {
            ...
            trail.raci = {
                responsible: matrix.responsible?.map((r) => r.teamName || r.platformRole).filter(Boolean),
                accountable: matrix.accountable?.map((r) => r.teamName || r.platformRole).filter(Boolean),
                consulted: matrix.consulted?.map((r) => r.teamName || r.platformRole).filter(Boolean),
                informed: matrix.informed?.map((r) => r.teamName || r.platformRole).filter(Boolean),
            };
        }
    }
    catch { /* non-critical */ }
...
async function executeProposedAction(tenantId, action, userId, method) {
    const schema = tenantSchema(tenantId);
    // Pre-execution quality gate
    const preGate = await validatePreExecution(tenantId, action);
    await safeQuery(`UPDATE "${schema}".copilot_proposed_actions SET pre_validation = $1, updated_at = NOW() WHERE action_id = $2`, [JSON.stringify(preGate), action.action_id]);
    if (!preGate.passed) {
        await safeQuery(`UPDATE "${schema}".copilot_proposed_actions
       SET status = 'failed', failure_reason = $1, updated_at = NOW() WHERE action_id = $2`, [preGate.failReason, action.action_id]);
        return { success: false, error: preGate.failReason };
    }
    // Mark as executing
    const execStatus = method === 'auto_execute' ? 'auto_executing' : 'executing';
    await safeQuery(`UPDATE "${schema}".copilot_proposed_actions SET status = $1, updated_at = NOW() WHERE action_id = $2`, [execStatus, action.action_id]);
    ...
    try {
        ...
        await safeQuery(`UPDATE "${schema}".copilot_proposed_actions
       SET status = 'failed', failure_reason = $1, updated_at = NOW() WHERE action_id = $2`, [toErrorMessage(err), action.action_id]);
    ...
    await safeQuery(`UPDATE "${schema}".copilot_proposed_actions
     ...
     WHERE action_id = $7`, [finalStatus, method === 'auto_execute' ? 'system' : userId, method,
...
// ── Approve Action ────────────────────────────────────────────────────────
export async function approveAction(tenantId, actionId, userId) {
    const schema = tenantSchema(tenantId);
    await ensureTable(schema);
    // Atomic claim — prevents race with auto-executor
    const claimed = await safeQuery(`UPDATE "${schema}".copilot_proposed_actions
...
// ── Reject Action ─────────────────────────────────────────────────────────
export async function rejectAction(tenantId, actionId, userId, reason) {
    const schema = tenantSchema(tenantId);
    const _responseTimeMs = Date.now(); // Will be adjusted below
    const result = await safeQuery(`UPDATE "${schema}".copilot_proposed_actions
     ...
     RETURNING action_id`, [userId, reason || 'User rejected', actionId]);
    if (result.rows.length) {
        ...
        }), { tenantId, operation: 'eventBus:copilot.action_rejected' });
    }
    return { success: result.rows.length > 0 };
}
// ── Cancel Auto-Execution ─────────────────────────────────────────────────
export async function cancelAutoExecute(tenantId, actionId, _userId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`UPDATE "${schema}".copilot_proposed_actions
...
export async function autoExecutePendingActions(tenantId) {
    const schema = tenantSchema(tenantId);
    ...
    const pending = await safeQuery(`SELECT * FROM "${schema}".copilot_proposed_actions
...
export async function escalateStaleActions(tenantId) {
    const schema = tenantSchema(tenantId);
    let escalated = 0;
    try {
        await ensureTable(schema);
    ...
    const stale = await safeQuery(`SELECT * FROM "${schema}".copilot_proposed_actions
     WHERE status = 'pending'
    ...
    for (const action of stale.rows) {
        ...
        try {
            const team = await safeQuery(`SELECT team_id, team_name FROM "${schema}".teams WHERE team_code = $1 LIMIT 1`, [teamCode]);
            if (team.rows.length) {
                const teamId = getFirstRow(team)?.team_id;
                // Check escalation path
                const escPath = await safeQuery(`SELECT escalate_to_team_id FROM "${schema}".team_escalation_paths
           WHERE from_team_id = $1 AND escalation_level = 1 LIMIT 1`, [teamId]);
       ...
       WHERE action_id = $2 AND status = 'pending'`, [escalationTarget, action.action_id]);
        ...
        }), { tenantId, operation: 'eventBus:copilot.action_escalated' });
        escalated++;
    }
    return { escalated };
}
// ── Get Pending Count ─────────────────────────────────────────────────────
export async function getPendingCount(tenantId, userId) {
    const schema = tenantSchema(tenantId);
    try {
        await ensureTable(schema);
        const result = await safeQuery(`SELECT COUNT(*)::int AS count FROM "${schema}".copilot_proposed_actions
...
//# sourceMappingURL=proposed-action.service.js.map