...
export function lookupShiftHandler(agentId, shiftCode) {
    return REGISTRY.get(key(agentId, shiftCode));
}
export function listRegisteredShiftHandlers() {
...
async function callAgentTool(agentId, toolName, tenantId, input = {}) {
    const importer = TOOL_BUILDERS[agentId];
    if (!importer)
        throw new Error(`no tool builder for ${agentId}`);
    ...
    if (typeof builder !== 'function')
        throw new Error(`builder ${builderName} not found in ${agentId} module`);
    const tools = builder();
    const tool = tools.find((t) => t.name === toolName);
    if (!tool)
        throw new Error(`tool ${toolName} not found on ${agentId}`);
    return tool.handler(tenantId, input);
}
/** Set RLS tenant context for the duration of `fn` so platform tables (dos.*) filter correctly. */
async function withTenantCtx(tenantId, fn) {
    if (!tenantId)
        return fn();
...
registerShiftHandler('A01', 'morning_health_scan', async ({ tenantId, agentId, shiftCode }) => {
    ...
    return withTenantCtx(tenantId, async () => {
        const profile = await callAgentTool(agentId, 'scan_org_profile', tenantId).catch((e) => ({ error: e.message }));
        const health = await callAgentTool(agentId, 'check_workspace_health', tenantId).catch((e) => ({ error: e.message }));
        const missing = profile?.missingFields ?? [];
        const completeness = profile?.completeness ?? 'unknown';
        const summary = `Workspace health scan — ${missing.length === 0 ? 'profile complete' : `${missing.length} fields missing`}.`;
        const profileCompletePct = profile?.profile
...
const toolFailed = (v) => !!v && typeof v === 'object' && 'error' in v;
// ──────────────────────────────────────────────────────────────────
...
registerShiftHandler('A02', 'mfa_morning_sweep', async ({ tenantId, agentId, shiftCode }) => {
    ...
    return withTenantCtx(tenantId, async () => {
        const accessInfo = await callAgentTool(agentId, 'list_users_with_access_info', tenantId).catch((e) => ({ error: e.message }));
        const dist = await callAgentTool(agentId, 'check_role_distribution', tenantId).catch((e) => ({ error: e.message }));
        if (toolFailed(accessInfo)) {
            logger.warn('[A02 shift] tool error', { error: accessInfo.error });
            return { title: 'Daily MFA drift report', summary: `Tool error: ${accessInfo.error}`, body: { phase: 2, error: true } };
        }
        const totalUsers = Number(accessInfo.totalUsers ?? 0);
        const noMfa = Number(accessInfo.usersWithoutMFA ?? 0);
        const inactive = Number(accessInfo.inactiveUsers ?? 0);
        const admins = Number(accessInfo.adminCount ?? 0);
        const mfaSource = String(accessInfo.mfaSource ?? 'tenant_db');
...
registerShiftHandler('A05', 'daily_evidence_sweep', async ({ tenantId, agentId, shiftCode }) => {
    ...
    return withTenantCtx(tenantId, async () => {
        const gaps = await callAgentTool(agentId, 'detect_evidence_gaps', tenantId).catch((e) => ({ error: e.message }));
        const freshness = await callAgentTool(agentId, 'check_evidence_freshness', tenantId).catch((e) => ({ error: e.message }));
        if (toolFailed(freshness)) {
            logger.warn('[A05 shift] tool error', { error: freshness.error });
            return { title: 'Daily evidence freshness scan', summary: `Tool error: ${freshness.error}`, body: { phase: 2, error: true } };
        }
        const total = Number(freshness.total ?? 0);
        const current = Number(freshness.current ?? 0);
        const expired = Number(freshness.expired ?? 0);
        const expiringSoon = Number(freshness.expiring_soon ?? 0);
        const stale = expired; // canonical "stale" definition
        const freshPct = total > 0 ? Math.round((current / total) * 100) : 100;
        const gapCount = !toolFailed(gaps) ? Number(gaps.gapCount ?? 0) : 0;
...
registerShiftHandler('A07', 'daily_kri_pulse', async ({ tenantId, agentId, shiftCode }) => {
    ...
    return withTenantCtx(tenantId, async () => {
        const risksResult = await callAgentTool(agentId, 'identify_risks', tenantId).catch((e) => ({ error: e.message }));
        const appetite = await callAgentTool(agentId, 'check_risk_appetite', tenantId).catch((e) => ({ error: e.message }));
        if (toolFailed(risksResult)) {
            logger.warn('[A07 shift] tool error', { error: risksResult.error });
            return { title: 'Daily KRI pulse', summary: `Tool error: ${risksResult.error}`, body: { phase: 2, error: true } };
        }
        const totalOpen = Number(risksResult.totalOpen ?? 0);
        const unscored = Number(risksResult.unscoredCount ?? 0);
        const highRiskCount = Number(risksResult.highRiskCount ?? 0);
        const breaches = !toolFailed(appetite) ? Number(appetite.breachCount ?? 0) : 0;
        ...
        return {
            ...
            body: {
                ...
                topRisks: (risksResult.risks ?? []).slice(0, 10),
                appetite,
            },
            kpiSnapshots: [
...
registerShiftHandler('A10', 'monthly_exec_dashboard', async ({ tenantId, agentId, shiftCode }) => {
    ...
    return withTenantCtx(tenantId, async () => {
        const readiness = await callAgentTool(agentId, 'check_audit_readiness', tenantId).catch((e) => ({ error: e.message }));
        const compliance = await callAgentTool(agentId, 'generate_compliance_summary', tenantId).catch((e) => ({ error: e.message }));
        const findings = await callAgentTool(agentId, 'list_audit_findings', tenantId).catch((e) => ({ error: e.message }));
        if (toolFailed(readiness)) {
            logger.warn('[A10 shift] tool error', { error: readiness.error });
            return { title: 'Monthly executive dashboard pack', summary: `Tool error: ${readiness.error}`, body: { phase: 2, error: true } };
        }
        const score = Number(readiness.readinessScore ?? 0);
        const level = String(readiness.readinessLevel ?? 'unknown');
        const openFindings = Number(readiness.openFindings ?? 0);
...
registerShiftHandler('A13', 'daily_lead_summary', async ({ tenantId, agentId, shiftCode }) => {
    ...
    const r = await safeQuery(`SELECT count(*)::int AS total,
      ...
      WHERE captured_at >= NOW() - INTERVAL '7 days'`, []).catch(() => ({ rows: [{ total: 0, last24h: 0, demo_intents: 0 }] }));
...
//# sourceMappingURL=shift-handlers.js.map