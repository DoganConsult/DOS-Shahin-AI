...
export function emitSignals(answers) {
...
export function runInference(answers) {
    ...
    for (const rule of INFERENCE_RULES) {
        ...
        if (!hasAllRequired && rule.requiredSignals.length > 0)
            continue;
        ...
        if (rule.requiredSignals.length === 0 && confidence < 0.5)
            continue;
        const entity = {
            id: rule.targetId,
...
function generateControlsFromFrameworks(frameworkIds) {
    const controls = [
    ...
    if (frameworkIds.includes("INST-KSA-NCA-ECC")) {
        controls.push({ id: uuid().slice(0, 8), title: "Cybersecurity Strategy Document", frameworks: ["INST-KSA-NCA-ECC"], automatable: false, priority: "critical" }, { id: uuid().slice(0, 8), title: "Cybersecurity Committee Establishment", frameworks: ["INST-KSA-NCA-ECC"], automatable: false, priority: "high" });
    }
    if (frameworkIds.includes("INST-KSA-SDAIA-PDPL")) {
        controls.push({ id: uuid().slice(0, 8), title: "Data Subject Rights Process", frameworks: ["INST-KSA-SDAIA-PDPL"], automatable: false, priority: "critical" }, { id: uuid().slice(0, 8), title: "Privacy Impact Assessment", frameworks: ["INST-KSA-SDAIA-PDPL"], automatable: false, priority: "high" });
    }
    if (frameworkIds.includes("INST-KSA-SAMA-CSF")) {
        controls.push({ id: uuid().slice(0, 8), title: "Financial Transaction Monitoring", frameworks: ["INST-KSA-SAMA-CSF"], automatable: true, priority: "critical" });
    }
    return controls;
}
//# sourceMappingURL=signal-inference.engine.js.map