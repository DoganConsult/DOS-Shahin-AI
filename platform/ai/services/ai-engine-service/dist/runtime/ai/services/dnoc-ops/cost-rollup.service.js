...
export async function getCostRollup(query, opts = {}) {
    ...
    const perfRes = await query(`WITH today AS (
     ...
     SELECT t.tenant_id,
      ...
      ${tenantClause}
      ORDER BY tenant_id`, params).catch(() => ({ rows: [] }));
    // Per-tenant caps live in dos.tenants.settings->>'ai_daily_usd_cap'.
    const capsRes = await query(`SELECT tenant_id, COALESCE((settings->>'ai_daily_usd_cap')::float, NULL) AS cap
       FROM dos.tenants`, []).catch(() => ({ rows: [] }));
    const capByTenant = new Map();
    for (const r of capsRes.rows) {
        if (typeof r.cap === 'number' && Number.isFinite(r.cap)) {
            capByTenant.set(r.tenant_id, r.cap);
        }
    }
    return perfRes.rows.map((r) => {
...
//# sourceMappingURL=cost-rollup.service.js.map