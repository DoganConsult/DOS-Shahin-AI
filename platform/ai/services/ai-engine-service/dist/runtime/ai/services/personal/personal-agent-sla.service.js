 ...
 */
export async function checkSlaAndActivateAgent(tenantId, userId) {
    const schema = tenantSchema(tenantId);
    const activatedActivities = [];
    // Get user's personal agent assignments with SLA-based activation
    const assignments = await safeQuery(`SELECT * FROM "${schema}".personal_agent_assignments
     WHERE tenant_id = $1 AND user_id = $2
       AND is_active = true AND is_enabled = true
       AND sla_based_activation = true`, [tenantId, userId]);
    for (const assignmentRow of assignments.rows) {
        ...
        const slaRules = await safeQuery(`SELECT * FROM "${schema}".agent_sla_activation_rules
       WHERE tenant_id = $1 AND assignment_id = $2 AND is_active = true`, [tenantId, assignment.assignmentId]);
        for (const slaRule of slaRules.rows) {
            // Find overdue items
            const overdueItems = await findOverdueItems(tenantId, slaRule.entity_type, slaRule.hours_overdue_threshold, slaRule.priority_filter ? JSON.parse(slaRule.priority_filter) : undefined);
            for (const item of overdueItems) {
                // Create agent activity for this overdue item
                const activity = await executeAgentActivity(tenantId, userId, assignment.agentId, {
                ...
                if (activity.canProceed) {
                    ...
                    await safeQuery(`UPDATE "${schema}".agent_sla_activation_rules
             SET last_triggered_at = NOW(), trigger_count = trigger_count + 1
             WHERE rule_id = $1`, [slaRule.rule_id]);
                }
            }
        }
    }
...
async function findOverdueItems(tenantId, entityType, hoursThreshold, priorityFilter) {
    const schema = tenantSchema(tenantId);
    ...
    if (entityType === 'task' || entityType === 'workflow_instance') {
        const result = await safeQuery(`SELECT workflow_instance_id as entity_id, title, due_date as deadline, priority
       FROM "${schema}".workflow_instances
       WHERE tenant_id = $1
         AND status IN ('Pending', 'InProgress')
         AND due_date < $2
         AND ($3::VARCHAR[] IS NULL OR priority = ANY($3))
       LIMIT 50`, [tenantId, thresholdDate.toISOString(), priorityFilter || null]);
...
//# sourceMappingURL=personal-agent-sla.service.js.map