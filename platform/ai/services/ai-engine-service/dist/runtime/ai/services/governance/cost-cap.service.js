...
export function tokensToUsd(tokensIn, tokensOut) {
    const inUsd = (tokensIn / 1_000_000) * priceInputPerMTok();
    const outUsd = (tokensOut / 1_000_000) * priceOutputPerMTok();
    return inUsd + outUsd;
}
async function tenantDailyCapUsd(tenantId) {
    try {
        const r = await safeQuery(`SELECT (settings->>'ai_daily_usd_cap')::float AS cap
         FROM dos.tenants
        WHERE tenant_id = $1
        LIMIT 1`, [tenantId]);
...
export async function getTenantSpendToday(tenantId) {
    try {
        const r = await safeQuery(`SELECT COALESCE(SUM(tokens_in), 0)::int AS tin,
              COALESCE(SUM(tokens_out), 0)::int AS tout
         FROM public.agent_performance_log
        WHERE tenant_id = $1
          AND created_at >= CURRENT_DATE`, [tenantId]);
 ...
 */
export async function checkCostCap(tenantId, estimatedNextCallUsd = 0) {
    if (!flagOn()) {
        return {
            allowed: true,
...
//# sourceMappingURL=cost-cap.service.js.map