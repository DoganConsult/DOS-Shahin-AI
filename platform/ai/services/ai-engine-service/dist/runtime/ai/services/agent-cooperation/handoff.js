...
// ── Handoff Creation ───────────────────────────────────────────────────────
export function createHandoff(tenantId, fromAgent, toAgent, handoffType, priority, payload) {
    const handoff = {
    ...
    import('../../../../openclaw/a2a/agent-message-queue.js').then(({ sendAgentMessage }) => sendAgentMessage(tenantId, fromAgent, toAgent, `[Handoff:${handoffType}] ${payload?.requestedAction || payload?.finding || 'Agent handoff'}`, { handoffId: handoff.id, handoffType, priority, ...payload })).catch(catchHandler(EC.EVENT_BUS));
    return handoff;
}
/**
 ...
 */
export async function getPendingHandoffs(tenantId, agentId, useBatching = false) {
 ...
 */
export function completeHandoff(tenantId, handoffId, result) {
    const ctx = activeCycleContexts.get(tenantId);
    if (!ctx)
        return;
 ...
 */
export function _evaluateCooperationRules(tenantId, discovery, _ctx) {
    const severityRank = { critical: 4, high: 3, medium: 2, low: 1 };
    const discoveryRank = severityRank[discovery.severity] || 0;
    for (const rule of COOPERATION_RULES) {
        if (rule.trigger.agentId !== discovery.agentId)
            continue;
        if (rule.trigger.discoveryType !== discovery.type)
            continue;
        const minRank = severityRank[rule.trigger.minSeverity] || 0;
        if (discoveryRank < minRank)
            continue;
        const handoff = createHandoff(tenantId, discovery.agentId, rule.handoffTo, rule.handoffType, discovery.severity, {
        ...
        import('../../../agrc-engine/services/agrc-os-integration.service.js').then(({ persistHandoff }) => persistHandoff(tenantId, discovery.agentId, rule.handoffTo, rule.handoffType, discovery.severity, { finding: discovery.title, requestedAction: rule.action,
            entityType: discovery.entityType, entityId: discovery.entityId })).catch(catchHandler(EC.AGENT_ACTION, {}));
        swallow(EC.EVENT_BUS, eventBus.publish({
            eventType: 'agent.handoff_created',
