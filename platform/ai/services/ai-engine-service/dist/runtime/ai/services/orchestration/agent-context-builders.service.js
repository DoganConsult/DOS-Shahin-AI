...
function settledCount(result, field = 'n') {
...
export async function loadAgentOrgContext(tenantId) {
    try {
        const result = await safeQuery(`SELECT org_type, legal_form, listing_status, org_size, employee_count,
...
export async function loadGovernanceContext(tenantId) {
    try {
        const result = await safeQuery(`SELECT business_profile, regulatory_profile, framework_profile,
...
export async function isAgentModuleActive(tenantId, agentId) {
    const requiredModules = AGENT_MODULE_MAP[agentId];
    if (!requiredModules || requiredModules.length === 0)
        return { active: true, reason: 'no module requirement' };
    try {
        const result = await safeQuery(`SELECT module_code, state, trial_expiry_at
       FROM public.module_operating_states
       WHERE tenant_id = $1 AND is_active = true AND module_code = ANY($2)`, [tenantId, requiredModules]);
        if (result.rows.length === 0)
            return { active: true, reason: 'no operating state rows — default active' };
        for (const row of result.rows) {
            if (row.state === 'on')
                return { active: true, reason: `module ${row.module_code} is on` };
            if (row.state === 'trial') {
        ...
        return { active: true, reason: 'module check failed — default active' };
    }
}
// ── Playbook loader for agent context enrichment ───────────────────────────
export async function loadAgentPlaybooks(agentCode) {
    try {
        const result = await safeQuery(`SELECT trigger_event, playbook_action, description_en, output_entity_type, priority
       FROM public.agent_playbook_assignments
       WHERE agent_code = $1 AND is_active = true ORDER BY priority`, [agentCode]);
        return result.rows;
    }
    catch {
        return [];
...
async function buildContextA01(tenantId, schema) {
    ...
    const profile = settledValue(orgCtx, {});
    return {
...
async function buildContextA02(tenantId, _schema) {
...
async function buildContextA03(tenantId, schema) {
...
async function buildContextA04(tenantId, schema) {
...
async function buildContextA05(tenantId, schema) {
...
async function buildContextA06(tenantId, schema) {
...
async function buildContextA07(tenantId, schema) {
...
async function buildContextA08(tenantId, schema) {
...
async function buildContextA09(tenantId, schema) {
...
async function buildContextA10(tenantId, schema) {
...
async function buildContextA11(tenantId, schema) {
    ...
    const [orgCtx, stalePlans, overdueExercises, untestedPlans, staleBIAs, untestedCrisisComms, unlinkedStrategies, exercisePassRate, maturityScores] = await Promise.allSettled([
        loadAgentOrgContext(tenantId),
...
async function buildContextA12(tenantId, schema) {
...
async function buildA13PolicyReviewContext(tenantId, schema) {
    const [orgCtx, overdueReviews, approachingReviews, stalePolicies, ownerlessPolicies, totalPublished] = await Promise.allSettled([
       ...
       WHERE status = 'published' AND (owner IS NULL OR owner = '')`),
        safeQuery(`SELECT COUNT(*) AS n FROM "${schema}".policies WHERE status = 'published'`),
    ]);
    return {
...
async function buildA14PolicyGapContext(tenantId, schema) {
    const [orgCtx, policiesWithoutControls, policiesWithoutRisks, orphanControls, openGaps] = await Promise.allSettled([
       ...
       WHERE p.status = 'published'
        ...
        safeQuery(`SELECT COUNT(*) AS n FROM "${schema}".ucf_controls c
       WHERE NOT EXISTS (
         SELECT 1 FROM "${schema}".policy_control_links pcl WHERE pcl.control_id = c.id
       )`),
        safeQuery(`SELECT COUNT(*) AS n FROM "${schema}".policy_gaps
...
async function buildA15PolicyAttestationContext(tenantId, schema) {
    const [orgCtx, activeCampaigns, overdueAttestations, uncoveredPolicies, overallAckRate] = await Promise.allSettled([
       ...
       GROUP BY ac.campaign_id, ac.title, ac.status`),
        safeQuery(`SELECT COUNT(*) AS n FROM "${schema}".attestation_records
       ...
       WHERE p.status = 'published'
        ...
        safeQuery(`SELECT CASE WHEN COUNT(*) = 0 THEN 0
    ...
    const campaigns = activeCampaigns.status === 'fulfilled'
        ? activeCampaigns.value.rows.map((r) => ({
...
//# sourceMappingURL=agent-context-builders.service.js.map