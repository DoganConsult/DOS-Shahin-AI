...
export function invalidateContextCache(tenantId) {
    contextCache.delete(tenantId);
    moduleStateCache.delete(tenantId);
}
export async function readGovernanceContext(tenantId) {
    const cached = contextCache.get(tenantId);
    if (cached && Date.now() - cached.fetchedAt < CTX_TTL_MS)
        return cached.ctx;
    try {
        const result = await safeQuery(`SELECT tenant_id, context_version, complexity,
...
export async function readModuleOperatingStates(tenantId) {
    const cached = moduleStateCache.get(tenantId);
    if (cached && Date.now() - cached.fetchedAt < CTX_TTL_MS)
        return cached.states;
    try {
        const result = await safeQuery(`SELECT module_code, state, activation_source, trial_expiry_at, is_mandatory
...
export function isModuleOn(states, moduleCode) {
