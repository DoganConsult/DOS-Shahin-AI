 ...
 */
export async function getAgentHealthStatus(tenantId, agentId, agentName) {
    try {
        ...
        // Get recent errors
        const errorsResult = await safeQuery(`SELECT error, started_at FROM "${schema}".agent_tasks
       ...
       ORDER BY started_at DESC LIMIT 1`, [agentId]);
        ...
        // Get last activity
        const lastActivityResult = await safeQuery(`SELECT MAX(started_at) as last_activity FROM "${schema}".agent_tasks WHERE agent_id = $1`, [agentId]);
 ...
 */
export async function getAgentHealthDashboard(tenantId, days = 30) {
    try {
        const schema = tenantSchema(tenantId);
        // Get all agent IDs
 ...
 */
async function getHealthTrends(tenantId, days) {
    const schema = tenantSchema(tenantId);
    try {
        // Health over time (daily averages)
        const healthResult = await safeQuery(`SELECT 
       ...
       ORDER BY date ASC`, []);
        ...
        const learningVelocityOverTime = healthOverTime.map((point, index) => {
            if (index === 0)
                return { date: point.date, velocity: 0 };
...
//# sourceMappingURL=agent-health-visualization.service.js.map