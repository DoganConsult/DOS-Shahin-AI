...
export function callAzureOpenAI(config, messages, maxTokens) {
...
export function callGemini(apiKey, model, messages, maxTokens) {
    return new Promise((resolve, reject) => {
        ...
        const req = https.request({
            ...
            res.on('end', () => {
                try {
                    ...
                    const tokens = (parsed.usageMetadata?.promptTokenCount || 0) +
...
export async function callFreeProvider(name, cfg, messages, maxTokens) {
    if (name === 'gemini') {
