...
export function callLmStudioApi(config, messages, maxTokens, overrideModel) {
        ...
        const req = requestModule.request({
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.error) {
                        reject(new Error(`[lmstudio] ${toErrorMessage(parsed.error) || JSON.stringify(parsed.error)}`));
                        return;
                    }
                    resolve({
