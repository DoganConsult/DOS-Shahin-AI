...
export async function analyzeRbacUsage(tenantId) {
    ...
    const rolesResult = await safeQuery(`SELECT fr.code, fr.is_active, COUNT(DISTINCT ura.user_id) AS user_count
     ...
     GROUP BY fr.id, fr.code, fr.is_active`).catch(() => ({ rows: [] }));
    ...
    const usageResult = await safeQuery(`SELECT ura.user_id, COUNT(DISTINCT rp.permission_id) AS total_perms,
     ...
     GROUP BY ura.user_id HAVING COUNT(DISTINCT rp.permission_id) > 0`).catch(() => ({ rows: [] }));
    const overProvisionedUsers = usageResult.rows
        .filter((r) => {
        const total = parseInt(r.total_perms, 10);
        const used = parseInt(r.used_perms, 10);
    ...
    const sodResult = await safeQuery(`SELECT COUNT(*) AS cnt FROM "${schema}".sod_rules WHERE is_active = TRUE`).catch(() => ({ rows: [{ cnt: 0 }] }));
    const sodConflicts = parseInt(sodResult.rows[0]?.cnt || '0', 10);
    const recommendations = [];
    if (unusedRoles.length > 0)
        recommendations.push(`${unusedRoles.length} roles have zero assignments — consider deactivating`);
    if (overProvisionedUsers.length > 0)
        recommendations.push(`${overProvisionedUsers.length} users use <50% of granted permissions — review for least-privilege`);
    if (sodConflicts === 0)
        recommendations.push('No SoD rules defined — consider adding separation of duties policies');
    return { totalRoles, activeRoles, unusedRoles, overProvisionedUsers, sodConflicts, recommendations };
}
//# sourceMappingURL=ai-rbac-optimizer.service.js.map