 ...
 */
export async function recordLearningEvent(tenantId, agentId, eventType, data) {
    const schema = tenantSchema(tenantId);
    try {
        await safeQuery(`INSERT INTO "${schema}".agent_learning_events (
 ...
 */
export async function calculateLearningCurve(tenantId, agentId, period = 'week', metric = 'success_rate') {
    const schema = tenantSchema(tenantId);
    try {
       ...
       ORDER BY period_start ASC`, [period === 'day' ? 'day' : period === 'week' ? 'week' : 'month', agentId]);
        const points = [];
...
function determineSkillLevel(metrics) {
    ...
    if (score >= 0.95 && metrics.totalTasks >= 100)
        return 'expert';
    if (score >= 0.85 && metrics.totalTasks >= 50)
        return 'advanced';
    if (score >= 0.70 && metrics.totalTasks >= 20)
        return 'intermediate';
    return 'beginner';
}
/**
 ...
 */
function calculateConfidence(dataPoints, totalTasks) {
 ...
 */
export async function getAgentLearningProfile(tenantId, agentId) {
    const schema = tenantSchema(tenantId);
    try {
        // Get skills from learning events
        const skillsResult = await safeQuery(`SELECT skill_name, MAX(skill_level) as max_level, COUNT(*) as experience_points,
              MAX(created_at) as last_improved_at
       FROM "${schema}".agent_learning_events
       WHERE agent_id = $1 AND skill_name IS NOT NULL
       GROUP BY skill_name`, [agentId]);
        // Calculate improvement rate for each skill from historical data
        const skills = await Promise.all(skillsResult.rows.map(async (row) => {
            ...
            const historyResult = await safeQuery(`SELECT skill_level, created_at
           FROM "${schema}".agent_learning_events
           WHERE agent_id = $1 AND skill_name = $2 AND skill_level IS NOT NULL
           AND created_at >= NOW() - INTERVAL '30 days'
           ORDER BY created_at ASC`, [agentId, skillName]);
            let improvementRate = 0;
        ...
        const patternsResult = await safeQuery(`SELECT pattern, event_type, COUNT(*) as count
       FROM "${schema}".agent_learning_events
       WHERE agent_id = $1 AND pattern IS NOT NULL
       GROUP BY pattern, event_type`, [agentId]);
        ...
        const overallSkillLevel = Math.round(metrics.successRate * 50 + metrics.verificationRate * 50);
        ...
        const lessonsResult = await safeQuery(`SELECT COUNT(*) as count FROM "${schema}".agent_lessons_learned WHERE agent_id = $1`, [agentId]);
        const lessonsLearned = parseInt(lessonsResult.rows[0]?.count || '0', 10);
        // Get last learning event
        const lastEventResult = await safeQuery(`SELECT MAX(created_at) as last_event FROM "${schema}".agent_learning_events WHERE agent_id = $1`, [agentId]);
 ...
 */
export async function getAllAgentLearningCurves(tenantId, period = 'week') {
    const schema = tenantSchema(tenantId);
    try {
        const agentsResult = await safeQuery(`SELECT DISTINCT agent_id FROM "${schema}".agent_tasks WHERE tenant_id = $1`, [tenantId]);
        const curves = {};
...
//# sourceMappingURL=agent-learning-curve.service.js.map