 ...
 */
export function classifyTrend(slope, threshold = 0.01) {
    if (slope > threshold)
        return "improving";
    if (slope < -threshold)
        return "declining";
 ...
 */
export function computeRollingStats(values) {
    if (values.length === 0)
 ...
 */
export function isAnomaly(value, mean, stdDev, sigmaThreshold = 2) {
    if (stdDev === 0)
        return false;
 ...
 */
export function serializeTrendResult(trend) {
 ...
 */
export function deserializeTrendResult(json) {
 ...
 */
export function serializeAnomalyAlert(alert) {
    return JSON.stringify({
 ...
 */
export function deserializeAnomalyAlert(json) {
    const obj = JSON.parse(json);
    return {
        kpiName: obj.kpiName,
 ...
 */
export function forecastLinear(values, horizon = 7) {
    if (values.length < 2)
 ...
 */
export async function computeTrends(tenantId, days = 30) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT snapshot_date, compliance_score, risk_score, evidence_coverage, remediation_closure_rate
    ...
    return KPI_COLUMNS.map((kpi) => {
        ...
        return {
            kpiName: kpi.name,
            direction: values.length < 2 ? "insufficient_data" : classifyTrend(slope),
 ...
 */
export async function detectAnomalies(tenantId, windowSize = 20) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT compliance_score, risk_score, evidence_coverage, remediation_closure_rate
     FROM "${schema}".kpi_snapshots
 ...
 */
export function buildSummaryText(report, language, anomalies = []) {
...
${metricsJson}`,
        ...
        const metrics = report.data || {};
        const metricEntries = Object.entries(metrics).filter(([, v]) => typeof v === "number");
        const metricText = metricEntries.length > 0
