...
function getMode() {
    ...
    if (raw === 'enforce' || raw === 'warn' || raw === 'audit')
        return raw;
    return 'warn';
}
...
async function recordToolAudit(tenantId, agentId, toolName, decision, toolInput) {
...
async function checkSoD(input, mode) {
    if (!flag('AI_SOD_ENABLED', true))
        return null;
    ...
    if (!input.callerType || !input.proposerType)
        return null;
    if (!input.proposerId || !input.approverId)
        return null;
    if (input.proposerId === input.approverId)
        return null;
...
function checkApprovalBoundary(input, mode) {
    if (!input.approvalBoundary || !input.toolRiskTier)
        return null;
...
function checkCycleCap(input, mode) {
    if (input.maxActionsPerCycle === undefined)
        return null;
...
async function checkPermission(input, mode) {
 ...
 */
export async function enforceToolGate(input) {
    const mode = getMode();
    ...
    if (sod) {
        ...
        if (mode === 'enforce')
            return sod;
        if (mode === 'warn')
            console.warn(`[tool-gate] WARN sod: ${sod.reason}`);
    ...
    if (ab) {
        ...
        if (mode === 'enforce')
            return ab;
        if (mode === 'warn')
            console.warn(`[tool-gate] WARN approval_boundary: ${ab.reason}`);
    ...
    if (mode === 'enforce')
        return perm;
    if (mode === 'warn')
        console.warn(`[tool-gate] WARN permission: ${perm.reason}`);
    return { ...perm, allow: true, reason: `${perm.reason} (mode=${mode}, allowed)` };
}
//# sourceMappingURL=tool-gate.service.js.map