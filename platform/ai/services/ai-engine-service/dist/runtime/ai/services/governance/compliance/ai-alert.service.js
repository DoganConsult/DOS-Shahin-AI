...
export async function createAlert(input) {
    ...
    try {
        const result = await safeQuery(`INSERT INTO "${schema}".ai_alerts
...
export async function listAlerts(tenantId, filters) {
    ...
    const [dataRes, countRes] = await Promise.all([
        safeQuery(`SELECT * FROM "${schema}".ai_alerts WHERE ${where} ORDER BY created_at DESC LIMIT $${idx} OFFSET $${idx + 1}`, [...params, limit, offset]),
...
export async function getAlertsForEntity(tenantId, entityType, entityId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT * FROM "${schema}".ai_alerts
...
export async function acknowledgeAlert(tenantId, alertId, userId) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`UPDATE "${schema}".ai_alerts
...
export async function resolveAlert(tenantId, alertId, userId, autoResolved = false) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`UPDATE "${schema}".ai_alerts
...
export async function escalateAlert(tenantId, alertId) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`UPDATE "${schema}".ai_alerts
...
export async function getAlertStats(tenantId) {
    const schema = tenantSchema(tenantId);
    try {
        const [statusRes, typeRes] = await Promise.all([
         ...
         FROM "${schema}".ai_alerts WHERE tenant_id = $1`, [tenantId]),
            safeQuery(`SELECT alert_type, COUNT(*)::int AS cnt
