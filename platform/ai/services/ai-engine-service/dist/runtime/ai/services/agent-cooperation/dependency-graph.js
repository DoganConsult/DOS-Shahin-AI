...
export const AGENT_DEPENDENCY_GRAPH = {
 ...
 */
export async function getAgentDependencyConfig(tenantId, agentId) {
    const schema = tenantSchema(tenantId);
    const now = new Date();
    try {
        // Get most recent active config for this agent
        const result = await safeQuery(`SELECT depends_on_agents, feeds_into_agents, priority, can_parallel_with
       ...
       LIMIT 1`, [tenantId, agentId, now]);
        if (result.rows.length > 0) {
            const row = getFirstRow(result);
            return {
                dependsOn: (row.depends_on_agents || []),
                feedsInto: (row.feeds_into_agents || []),
                priority: row.priority || 1,
                canParallelWith: (row.can_parallel_with || []),
            };
        }
    }
    catch (err) {
        logger.warn(`[Agent Cooperation] Failed to get runtime dependency config: ${toErrorMessage(err)}`);
    }
 ...
 */
export async function setAgentDependencyConfig(tenantId, agentId, config) {
    const schema = tenantSchema(tenantId);
    try {
        await safeQuery(`INSERT INTO "${schema}".agent_dependency_config
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
        ...
        logger.warn(`[Agent Cooperation] Failed to set dependency config: ${toErrorMessage(err)}`);
        return false;
    }
}
/**
 ...
 */
export async function getAllAgentDependencies(tenantId) {
    const merged = {};
    // Start with defaults
    ...
    const now = new Date();
    try {
        const result = await safeQuery(`SELECT DISTINCT ON (agent_id)
       ...
       ORDER BY agent_id, effective_from DESC`, [tenantId, now]);
        ...
        logger.warn(`[Agent Cooperation] Failed to get all dependencies: ${toErrorMessage(err)}`);
    }
    return merged;
}
