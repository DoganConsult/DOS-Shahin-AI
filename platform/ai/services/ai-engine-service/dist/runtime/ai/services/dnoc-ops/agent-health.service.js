...
export async function getAgentHealth(query, langfuseQuery, opts = {}) {
    ...
    const perfRes = await query(`SELECT
       tenant_id, agent_id,
       COUNT(*)::int                                                    AS runs,
    ...
    ORDER BY tenant_id, agent_id`, [windowHours, ...tenantParam]).catch(() => ({ rows: [] }));
    ...
    if (langfuseQuery) {
        const lfRes = await langfuseQuery(`SELECT DISTINCT ON (metadata->>'agentId')
        ...
        ORDER BY metadata->>'agentId', created_at DESC`, []).catch(() => ({ rows: [] }));
