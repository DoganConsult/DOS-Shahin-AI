...
export async function getAiDiagnosticsSnapshot(tenantId) {
    const schema = tenantSchema(tenantId);
    const [runRes, failedRes, stuckRes, approvalRes, handoffRes, memRes, circuitRes, costRes] = await Promise.all([
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ active: 0, failed24h: 0, avg_latency: 0, error_rate: 0 }]), safeQuery(`
          SELECT
        ...
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ stuck: 0 }]), safeQuery(`
        ...
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, blocked: 0, failed: 0 }]), safeQuery(`
          SELECT
            COUNT(*)::int AS total,
        ...
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ pending: 0 }]), safeQuery(`SELECT COUNT(*)::int AS pending FROM "${schema}".agent_approvals WHERE status = 'pending'`), { tenantId, operation: 'diagnostics approvals' }),
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ pending: 0 }]), safeQuery(`SELECT COUNT(*)::int AS pending FROM "${schema}".agent_handoffs WHERE status = 'pending'`), { tenantId, operation: 'diagnostics handoffs' }),
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total: 0, stale: 0, tokens: 0 }]), safeQuery(`
          SELECT
            COUNT(*)::int AS total,
        ...
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ circuit_open: 0 }]), safeQuery(`SELECT COUNT(*)::int AS circuit_open FROM "${schema}".agent_circuit_breaker WHERE state = 'open'`), { tenantId, operation: 'diagnostics circuit breaker' }),
        swallowDefault(EC.FALLBACK_QUERY, emptyResult([{ total_tokens: 0, budget: 1000000 }]), safeQuery(`
          SELECT
    ...
    return {
        ...
        modelDiagnostics: {
            ...
            providerErrors24h: 0,
            fallbackCount24h: 0,
            avgTokensPerRun: Number(run.active) > 0
...
export async function getFailedRunAnalysis(tenantId, limit = 50) {
    const schema = tenantSchema(tenantId);
    try {
       ...
       LIMIT $1`, [limit]);
        return result.rows.map((r) => ({
            ...
            failedAt: r.updated_at ? new Date(r.updated_at).toISOString() : new Date().toISOString(),
            retriable: true,
        }));
    }
    catch (_err) {
...
export async function getBlockedToolInvocations(tenantId, limit = 50) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`SELECT
...
export async function getQueueThroughputDiagnostics(tenantId) {
    const schema = tenantSchema(tenantId);
