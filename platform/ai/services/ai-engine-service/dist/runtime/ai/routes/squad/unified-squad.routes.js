...
router.get('/participants', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    const filters = {
        deploymentMode: req.query.deploymentMode,
        role: req.query.role,
...
router.post('/participants', authenticate, validate({ body: participantsPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "unified-squad", entityId: result?.id || userId, afterState: result });
...
router.put('/participants/:id/status', authenticate, validate({ body: participantsIdStatusPutBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "update", entityType: "unified-squad", entityId: req.params.id, afterState: { status: req.body.status } });
...
router.post('/sync', authenticate, validate({ body: syncPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "update", entityType: "unified-squad", entityId: instanceId, afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
    res.json(result);
}));
...
router.post('/assign', authenticate, validate({ body: assignPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "update", entityType: "unified-squad", entityId: taskId, afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
    res.json(result);
}));
// ── Intervention ───────────────────────────────────────────────────────────
...
router.post('/intervene', authenticate, validate({ body: intervenePostBody }), asyncHandler(async (req, res) => {
    try {
        ...
        setAuditData(res, { action: "create", entityType: "unified-squad", entityId: result?.interventionId || "intervention", afterState: result });
        swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
        res.json(result);
    }
    catch (err) {
        const status = (err?.statusCode) || 500;
        res.status(status).json({ error: toErrorMessage(err), code: 'INTERVENTION_FAILED' });
    }
}));
...
router.get('/interventions', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const filters = {
        workflowStepId: req.query.workflowStepId,
...
router.post('/handoff', authenticate, validate({ body: handoffPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "unified-squad", entityId: result?.id || "handoff", afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
    res.status(201).json(result);
}));
// ── ERP Connector ──────────────────────────────────────────────────────────
...
router.post('/erp/connections', authenticate, validate({ body: erpConnectionsPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "unified-squad", entityId: result?.id || "erp-connection", afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
    res.status(201).json(result);
}));
...
router.put('/erp/connections/:id', authenticate, validate({ body: erpConnectionsIdPutBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "update", entityType: "unified-squad", entityId: req.params.id, afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'updated', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.updated' });
    res.json(result);
}));
...
router.post('/erp/connections/:id/mappings', authenticate, validate({ body: erpConnectionsIdMappingsPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "unified-squad", entityId: req.params.id, afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
    res.status(201).json(result);
}));
...
router.post('/agents/seed', authenticate, validate({ body: agentsSeedPostBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "unified-squad", entityId: tenantId, afterState: { seeded: result.length } });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'unified_squad', entityId: req.params.id || '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.unified_squad.created' });
...
router.put('/agents/:id/status', authenticate, validate({ body: agentsIdStatusPutBody }), asyncHandler(async (req, res) => {
    try {
        ...
        const status = (err?.statusCode) || 500;
        res.status(status).json({ error: toErrorMessage(err), code: 'AGENT_STATUS_FAILED' });
    }
}));
...
router.get('/agents/monitoring', authenticate, requirePermission('ai.squad.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    try {
        ...
        for (const agent of agents) {
            try {
                const metrics = await agentMgr.getAgentMetrics(tenantId, agent.userId);
                if (metrics.length > 0) {
                    agentMetricsMap.set(agent.userId, metrics[0]); // Latest snapshot
                }
            }
            catch (err) {
        ...
        const workflowCounts = await safeQuery(`SELECT assigned_participant_id, status, COUNT(*)::int as count
  ...
  GROUP BY assigned_participant_id, status`);
        ...
        const monitoringData = agents.map(agent => {
            ...
            const metrics = agentMetricsMap.get(agent.userId);
            const workflows = workflowMap.get(agent.userId) || [];
            return {
...
router.get('/workflow-timeline', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    if (workflowType) {
        conds.push(`workflow_type = $${idx++}`);
        params.push(workflowType);
    }
    if (participantId) {
        conds.push(`assigned_participant_id = $${idx++}`);
        params.push(participantId);
    }
    const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
    const result = await safeQuery(`SELECT * FROM "${schema}".workflow_timeline_entries ${where} ORDER BY due_date ASC NULLS LAST LIMIT 200`, params);
...
router.post('/ai-assign', authenticate, validate({ body: createAiAssignBody }), asyncHandler(async (req, res) => {
    ...
    const schema = tenantSchema(tenantId);
    const participants = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(`SELECT p.*,
     ...
     WHERE p.status = 'active'`, []), { tenantId: req.tenantId, operation: 'query process_tasks' });
...
Respond with JSON: {
  ...
  estimated_completion_hours: number
    ...
    if (assignment.recommended_assignee?.match_score >= 80 && taskId) {
        await safeQuery(`UPDATE "${schema}".process_tasks SET assigned_to = $1, updated_at = NOW() WHERE task_id = $2`, [assignment.recommended_assignee.user_id, taskId]).catch(catchHandler(EC.EVENT_BUS, {}));
    }
...
router.get('/capability-matrix', authenticate, validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    const taskMap = new Map(taskHistory.rows.map((r) => [r.assigned_to, r]));
    const matrix = participants.rows.map((p) => ({
        ...p,
...
//# sourceMappingURL=unified-squad.routes.js.map