...
providerRouter.post('/agent/:agentCode/invoke', async (req, res) => {
    try {
        ...
        const schema = "public";
        const execution = await safeQuery(`INSERT INTO ${schema}.ai_agent_executions (agent_code, user_id, status, input_payload) 
             VALUES ($1, $2, 'running', $3) RETURNING execution_id`, [agentCode, req.headers['x-user-id'] || null, JSON.stringify({ prompt, threadId })]);
