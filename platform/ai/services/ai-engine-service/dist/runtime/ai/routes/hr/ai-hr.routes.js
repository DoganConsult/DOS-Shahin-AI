...
router.get('/employees', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
...
router.get('/managers', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user?.tenantId ?? null;
...
router.get('/employees/:id/timeline', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user?.tenantId ?? null;
...
router.get('/employees/:id/kpi-trend', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user?.tenantId ?? null;
...
router.get('/employees/:id', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user?.tenantId ?? null;
 ...
 */
router.get('/inbox', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const user = req.user || {};
        const tenantId = user?.tenantId ?? null;
        const roleCodes = Array.isArray(user?.roles)
            ? user.roles.map((r) => (typeof r === 'string' ? r : r?.code)).filter(Boolean)
            : [];
        const isSuperAdmin = user?.is_super_admin === true || user?.isSuperAdmin === true;
        const status = req.query.status || 'filed';
...
router.post('/inbox/:id/acknowledge', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const user = req.user || {};
        const ok = await acknowledgeReport(req.params.id, user?.sub ?? user?.email ?? 'unknown');
        if (!ok) {
            res.status(404).json({ error: 'report not found or not in filed state' });
            return;
        }
        res.json({ ok: true });
    }
...
router.post('/inbox/:id/action', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const user = req.user || {};
        const ok = await actionReport(req.params.id, user?.sub ?? user?.email ?? 'unknown');
        if (!ok) {
            res.status(404).json({ error: 'report not found or already actioned' });
            return;
        }
        res.json({ ok: true });
    }
...
router.post('/_seed', requirePermission('admin:write'), async (_req, res) => {
    try {
...
router.post('/_run-due', requirePermission('admin:write'), async (_req, res) => {
    try {
...
//# sourceMappingURL=ai-hr.routes.js.map