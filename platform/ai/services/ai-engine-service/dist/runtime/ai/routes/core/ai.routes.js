...
router.post("/generate-policy", authenticate, requirePermission("policy.document.write"), validate({ body: generatePolicyBody }), asyncHandler(async (req, res) => {
    ...
    if (!frameworkId || !policyType) {
        res.status(400).json({ error: "frameworkId, policyType required" });
        return;
    }
    const policy = await generatePolicy(req.tenantId, req.body);
    setAuditData(res, { action: "create", entityType: "ai-analysis", entityId: policy?.id || "generate-policy", afterState: policy });
...
router.post("/classify-risk", authenticate, requirePermission("risk.record.read"), validate({ body: autoEvalBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "ai-analysis", entityId: "classify-risk", afterState: result });
...
router.post("/classify-incident", authenticate, requirePermission("incident.record.read"), validate({ body: autoEvalBody }), asyncHandler(async (req, res) => {
    ...
    setAuditData(res, { action: "create", entityType: "ai-analysis", entityId: "classify-incident", afterState: result });
    swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'ai', entityId: '' }), { tenantId: req.tenantId, operation: 'grcEvent:governance.ai.created' });
    res.json(result);
}));
...
async function getModuleStats(tenantId, moduleCode) {
    ...
    try {
        ...
        for (const r of statusResult.rows)
            statusBreakdown[r.status] = parseInt(r.cnt) || 0;
        return {
...
router.post("/query/interpret", authenticate, requirePermission("ai.query.read"), asyncHandler(async (req, res) => {
    ...
    if (Object.keys(filters).length === 0 && !sort) {
        try {
            const schema = aiTenantSchema(tenantId);
            const tableName = moduleCode.replace(/-/g, '_');
            const colResult = await aiSafeQuery(`SELECT column_name FROM information_schema.columns
         WHERE table_schema = $1 AND table_name = $2
         ORDER BY ordinal_position`, [schema, tableName]);
            const columns = colResult.rows.map((r) => r.column_name);
            const queryWords = lowerQuery.split(/\s+/).filter((w) => w.length > 2);
            for (const col of columns) {
                if (queryWords.some((w) => col.includes(w) || w.includes(col.replace(/_/g, '')))) {
...
router.post("/classify", authenticate, requirePermission("ai.classify.read"), asyncHandler(async (req, res) => {
    ...
    let confidence = 0.60;
    const suggestedTags = [moduleCode];
    ...
    try {
        const schema = aiTenantSchema(tenantId);
        const tableName = moduleCode.replace(/-/g, '_');
        const ownerResult = await aiSafeQuery(`SELECT DISTINCT owner, COUNT(*) as cnt FROM "${schema}"."${tableName}"
       WHERE deleted_at IS NULL AND owner IS NOT NULL
       GROUP BY owner ORDER BY cnt DESC LIMIT 3`).catch(() => ({ rows: [] }));
        suggestedOwners.push(...ownerResult.rows.map((r) => r.owner).filter(Boolean));
    }
    catch { /* ignore */ }
...
router.post("/predict", authenticate, requirePermission("ai.predict.read"), asyncHandler(async (req, res) => {
    ...
    try {
        const schema = aiTenantSchema(tenantId);
        const tableName = moduleCode.replace(/-/g, '_');
        const colCheck = await aiSafeQuery(`SELECT column_name FROM information_schema.columns
       WHERE table_schema = $1 AND table_name = $2 AND column_name = $3`, [schema, tableName, field]).catch(() => ({ rows: [] }));
        if (colCheck.rows.length > 0) {
            const freqResult = await aiSafeQuery(`SELECT "${field}" as val, COUNT(*) as cnt
         FROM "${schema}"."${tableName}"
         WHERE deleted_at IS NULL AND "${field}" IS NOT NULL
         GROUP BY "${field}"
         ORDER BY cnt DESC LIMIT 5`).catch(() => ({ rows: [] }));
            if (freqResult.rows.length > 0) {
                const totalSeen = freqResult.rows.reduce((s, r) => s + parseInt(r.cnt), 0);
...
router.post("/analytics/anomalies", authenticate, requirePermission("ai.analytics.read"), asyncHandler(async (req, res) => {
    ...
    try {
        const schema = aiTenantSchema(tenantId);
        const tableName = moduleCode.replace(/-/g, '_');
        const volumeResult = await aiSafeQuery(`SELECT
       ...
       FROM "${schema}"."${tableName}" WHERE deleted_at IS NULL`).catch(() => ({ rows: [{ today: 0, daily_avg_week: 0, daily_avg_month: 0 }] }));
        ...
        const staleResult = await aiSafeQuery(`SELECT id FROM "${schema}"."${tableName}"
       ...
       LIMIT 20`).catch(() => ({ rows: [] }));
        if (staleResult.rows.length > 0) {
            anomalies.push({
                ...
                confidence: 0.85,
                affectedItemIds: staleResult.rows.map((r) => r.id),
                timestamp: new Date().toISOString(),
            });
        }
    }
