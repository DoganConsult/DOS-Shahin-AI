...
router.post('/chat', authenticate, requirePermission('ai.copilot.write'), validate({ body: createChatBody }), async (req, res) => {
    try {
        ...
        setAuditData(res, { action: 'create', entityType: 'copilot', entityId: activeSessionId, afterState: result });
...
router.post('/intent-to-query', authenticate, requirePermission('ai.copilot.read'), validate({ body: intentToQueryBody }), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        ...
        setAuditData(res, { action: 'query', entityType: 'copilot-intent', entityId: moduleCode, afterState: result });
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ error: toErrorMessage(err) });
...
router.get('/sessions/:id', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get('/export', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.get('/agent-performance', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('ai.copilot.read'), async (req, res) => {
    try {
...
router.get('/suggestions', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const agentId = req.query.agentId;
        if (!agentId) {
...
router.post('/execute-action', authenticate, requirePermission('ai.copilot.write'), validate({ body: createExecuteActionBody }), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
...
router.patch('/actions/:id/approve', authenticate, requirePermission('ai.copilot.write'), validate({ body: updateApproveBody }), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        ...
        setAuditData(res, { action: 'update', entityType: 'copilot-action', entityId: actionId, afterState: { status: 'completed', executedBy: userId } });
...
router.patch('/actions/:id/reject', authenticate, requirePermission('ai.copilot.write'), validate({ body: updateRejectBody }), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const actionId = req.params.id;
        ...
        setAuditData(res, { action: 'update', entityType: 'copilot-action', entityId: actionId, afterState: { status: 'rejected', rejectedBy: userId } });
        swallow(EC.EVENT_BUS, emitModuleEvent({ tenantId: req.user.tenantId, userId: req.user.userId, module: 'governance', event: 'updated', entityType: 'copilot', entityId: req.params.id || '' }), { tenantId: req.user.tenantId, operation: 'grcEvent:governance.copilot.updated' });
        res.json(result);
    }
    catch (err) {
...
router.patch('/actions/:id/cancel-auto', authenticate, requirePermission('ai.copilot.write'), validate({ body: updateCancelAutoBody }), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
        const actionId = req.params.id;
...
router.get('/actions/pending', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const userId = req.user.userId;
...
router.get('/public-agents', publicAgentsLimiter, async (_req, res) => {
    try {
...
router.post('/public-chat', publicChatLimiter, validate({ body: createPublicChatBody }), async (req, res) => {
    try {
        ...
        const userQuery = (typeof req.body?.message === 'string' ? req.body.message : undefined)
        ...
        const contactEmail = rawEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(rawEmail) ? rawEmail : undefined;
        const contactCompany = typeof req.body?.company === 'string' ? req.body.company : undefined;
        const contactRoleTitle = typeof req.body?.roleTitle === 'string' ? req.body.roleTitle : undefined;
        ...
        const sessionId = typeof req.body?.sessionId === 'string' && /^[0-9a-f-]{8,}$/i.test(req.body.sessionId)
            ? req.body.sessionId
            : crypto.randomUUID();
        ...
        const leadId = (intent !== 'general' || contactEmail) ? await captureA13Lead({
            ...
            visitorIp, userAgent: req.headers['user-agent'], sessionId,
        }) : null;
        if (leadId) {
            void writeVisitorAudit({
        ...
        res.json({ ...result, intent, leadId, sessionId });
    }
    catch {
...
router.get('/stream', authenticate, requirePermission('ai.copilot.read'), async (req, res) => {
    ...
    const activeAgentId = req.query.activeAgentId;
    ...
    const unsub = subscribe({
        ...
        handler: async (event) => {
            if (event.tenantId !== tenantId && event.tenant_id !== tenantId)
                return;
            try {
 ...
 */
function classifyA13Intent(message) {
    ...
    if (new RegExp(`(demo|trial|see\\s+it\\s+work|walkthrough|عرض${arabicGap}توضيحي|تجربة)`).test(m))
        return 'demo';
    if (new RegExp(`(price|pricing|cost|quote|سعر|تكلفة|عرض${arabicGap}أسعار)`).test(m))
        return 'pricing';
    if (/(contact|sales|talk\s+to|اتصل|تواصل|مبيعات)/.test(m))
        return 'contact';
    if (/(docs?|documentation|api|spec|توثيق|مستندات)/.test(m))
        return 'docs';
    return 'general';
}
async function captureA13Lead(input) {
    try {
        ...
        if (input.email) {
            const existing = await safeQuery(`SELECT lead_id FROM public.copilot_leads
          WHERE email = $1 AND captured_at >= NOW() - INTERVAL '24 hours'
          ORDER BY captured_at DESC LIMIT 1`, [input.email]);
            const existingLeadId = existing.rows?.[0]?.lead_id;
            if (existingLeadId) {
                await safeQuery(`UPDATE public.copilot_leads
              ...
              intent = CASE
                ...
                ELSE intent
              END
            WHERE lead_id = $1`, [existingLeadId, input.intent, input.message.slice(0, 2000)]).catch(() => undefined);
                return existingLeadId;
            }
        }
        const r = await safeQuery(`INSERT INTO public.copilot_leads
       ...
       RETURNING lead_id`, [input.intent, input.email ?? null, input.company ?? null, input.roleTitle ?? null,
