...
router.get('/ai-os/settings', authenticate, aiReadLimiter, requirePermission('ai.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    res.ok({
        ...
        enabledAgents: configs.filter((c) => c.enabled).length,
        disabledAgents: configs.filter((c) => !c.enabled).length,
        policyStats,
        signalStats,
...
{health_score: number (0-100), status: "healthy"|"degraded"|"critical",
 ...
 autonomy_assessment: string, risk_posture: string}`,
    ...
    res.ok({ ...introspection, _raw: { agents: agents.rows.length, decisions: decisions.rows[0], signals: signals.rows[0], alerts: alerts.rows[0] } });
}));
// ── Capability Map ──────────────────────────────────────────────────────────
...
Respond with JSON: {
  ...
  counterfactual: string (what would have happened differently)
    ...
    res.ok({ entity_type, entity_id, ...explanation });
}));
export default router;
let genericPayloadSchema = z.record(z.unknown());
//# sourceMappingURL=introspection.routes.js.map