...
const ALLOWED_STATUSES = ['new', 'routed', 'contacted', 'converted', 'rejected'];
async function writeSalesAudit(actorId, action, leadId, payload) {
    await safeQuery(`INSERT INTO dos.audit_trail (tenant_id, actor_id, action, entity_type, entity_id, module, payload)
     VALUES ('platform', $1, $2, 'copilot_lead', $3, 'ai', $4::jsonb)`, [actorId, action, leadId, JSON.stringify(payload)]).catch((err) => logger.warn('[sales-leads] audit insert failed', { error: err?.message, leadId }));
}
/**
 ...
 */
router.get('/', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        ...
        else if (status !== 'all' && ALLOWED_STATUSES.includes(status)) {
            where.push(`status = $${p++}`);
            params.push(status);
        }
        if (intent === 'qualified') {
        ...
        params.push(offset);
        const offsetIdx = p++;
        const r = await safeQuery(`SELECT lead_id, intent, email, company, role_title, message, source,
        ...
        ORDER BY
          ...
          captured_at DESC
        LIMIT $${limitIdx} OFFSET $${offsetIdx}`, params).catch(() => ({ rows: [] }));
        const user = req.user || {};
        void writeSalesAudit(user?.sub ?? user?.email ?? 'unknown', 'sales.lead.list.viewed', 'list', { count: r.rows.length, status, intent, search: search.slice(0, 50) });
        res.json({
...
router.get('/:id', requirePermission('ai.copilot.read'), async (req, res) => {
    try {
        const r = await safeQuery(`SELECT lead_id, intent, email, company, role_title, message, source,
              status, routed_to, routed_at, captured_at, visitor_ip, user_agent,
              session_id, metadata
         FROM public.copilot_leads WHERE lead_id = $1`, [req.params.id]);
        if (r.rows.length === 0) {
        ...
        const row = r.rows[0];
        const user = req.user || {};
        void writeSalesAudit(user?.sub ?? user?.email ?? 'unknown', 'sales.lead.viewed', row.lead_id, { intent: row.intent, status: row.status });
        res.json({
            leadId: row.lead_id,
 ...
 */
router.post('/:id/transition', requirePermission('ai.copilot.write'), async (req, res) => {
    try {
        ...
        const r = await safeQuery(`UPDATE public.copilot_leads
          SET status = $2,
        ...
        WHERE lead_id = $1
        RETURNING status`, [req.params.id, to, routedTo, actor, note]);
        if (r.rows.length === 0) {
            res.status(404).json({ error: 'lead not found' });
            return;
        }
        await writeSalesAudit(actor, `sales.lead.${to}`, req.params.id, { routedTo, note });
...
//# sourceMappingURL=copilot-leads.routes.js.map