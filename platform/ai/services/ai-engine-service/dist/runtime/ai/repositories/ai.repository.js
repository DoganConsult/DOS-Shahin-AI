...
export class AIRepository {
    ...
    // ── Agent Registry ───────────────────────────────────
    async findAgentVersionById(versionId) {
        const result = await safeQuery(`SELECT * FROM "${this.schema}".ai_agent_registry WHERE agent_version_id = $1`, [versionId]);
        return getFirstRow(result);
    }
    async findAgentVersionsByAsset(assetId) {
        const result = await safeQuery(`SELECT * FROM "${this.schema}".ai_agent_registry
       WHERE asset_id = $1
       ORDER BY version_number DESC`, [assetId]);
        return result.rows;
    }
    async findAllAgentVersions(filters = {}) {
        const conditions = [];
        const params = [];
        let idx = 1;
        if (filters.asset_id) {
        ...
        if (filters.deployment_status) {
            conditions.push(`deployment_status = $${idx++}`);
            params.push(filters.deployment_status);
        }
        ...
        const countResult = await safeQuery(`SELECT COUNT(*)::int AS total FROM "${this.schema}".ai_agent_registry ${where}`, params);
        const total = getFirstRow(countResult)?.total ?? 0;
        const dataResult = await safeQuery(`SELECT * FROM "${this.schema}".ai_agent_registry ${where}
       ORDER BY created_at DESC
       LIMIT $${idx++} OFFSET $${idx++}`, [...params, pageSize, offset]);
        return { rows: dataResult.rows, total };
    }
    async findActiveAgentVersion(assetId) {
        const result = await safeQuery(`SELECT * FROM "${this.schema}".ai_agent_registry
       WHERE asset_id = $1 AND is_active = TRUE
       LIMIT 1`, [assetId]);
        return getFirstRow(result);
    }
    async findPreviousAgentVersion(assetId, currentVersionNum) {
        const result = await safeQuery(`SELECT * FROM "${this.schema}".ai_agent_registry
    ...
    async findMemoryById(memoryId) {
        const result = await safeQuery(`SELECT * FROM "${this.schema}".agent_memories
       WHERE memory_id = $1 AND is_deleted = FALSE`, [memoryId]);
        return getFirstRow(result);
    }
    async findAllMemories(filters = {}) {
        ...
        if (filters.namespace) {
            conditions.push(`namespace ILIKE $${idx++}`);
        ...
        const pageSize = Math.min(100, Math.max(1, filters.pageSize || 25));
        const offset = (page - 1) * pageSize;
        const countResult = await safeQuery(`SELECT COUNT(*)::int AS total FROM "${this.schema}".agent_memories ${where}`, params);
        const total = getFirstRow(countResult)?.total ?? 0;
        const dataResult = await safeQuery(`SELECT memory_id, tenant_id, user_id, agent_id, memory_type, namespace,
    ...
    async storeMemory(data) {
        ...
        const result = await safeQuery(`INSERT INTO "${this.schema}".agent_memories
       ...
       RETURNING memory_id, tenant_id, user_id, agent_id, memory_type, namespace,
                 content, summary, metadata, importance_score, created_at`, params);
        return getFirstRow(result);
    }
    async softDeleteMemory(memoryId) {
        const result = await safeQuery(`UPDATE "${this.schema}".agent_memories
       SET is_deleted = TRUE, updated_at = NOW()
       WHERE memory_id = $1 AND is_deleted = FALSE
       RETURNING memory_id`, [memoryId]);
        return (result.rows?.length ?? 0) > 0;
    }
    async purgeExpiredMemories(tenantId) {
        const result = await safeQuery(`UPDATE "${this.schema}".agent_memories
       SET is_deleted = TRUE, updated_at = NOW()
    ...
    async recordUsage(data) {
        ...
        await safeQuery(`INSERT INTO "${this.schema}".llm_usage_log
    ...
    async getUsageStats(tenantId, daysBack = 30) {
        const result = await safeQuery(`SELECT COUNT(*)::int AS calls,
    ...
    async getAgentUsageStats(agentId, daysBack = 30) {
        const result = await safeQuery(`SELECT COUNT(*)::int AS calls,
              COALESCE(SUM(input_tokens), 0)::int AS input_tokens,
              COALESCE(SUM(output_tokens), 0)::int AS output_tokens,
              COALESCE(SUM(cost_usd), 0)::real AS cost_usd
    ...
    async getBudget(tenantId) {
        const result = await safeQuery(`SELECT monthly_token_limit, monthly_cost_limit, tokens_used_month, cost_used_month
       FROM "${this.schema}".tenant_llm_budgets WHERE tenant_id = $1`, [tenantId]);
        return getFirstRow(result);
    }
    async incrementBudgetUsage(tenantId, tokens, costUsd) {
        await safeQuery(`UPDATE "${this.schema}".tenant_llm_budgets
       SET tokens_used_month = tokens_used_month + $1,
           cost_used_month = cost_used_month + $2,
           updated_at = NOW()
       WHERE tenant_id = $3`, [tokens, costUsd, tenantId]);
    }
}
