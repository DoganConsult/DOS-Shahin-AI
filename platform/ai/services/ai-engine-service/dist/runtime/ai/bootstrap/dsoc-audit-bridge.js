...
export function installDSOCAuditBridge() {
    let wired = 0;
    for (const [aiEvent, { category, action }] of Object.entries(AUDIT_BRIDGE)) {
        eventBus.subscribe(aiEvent, `ai.dsoc-audit-bridge.${aiEvent}`, async (envelope) => {
            ...
            const actor = payload?.actorId
                ? { type: 'user', id: String(payload.actorId) }
                : { type: 'service', id: SOURCE };
            try {
                eventBus.publish({
                    eventType: `dsoc.audit.${category}`,
                    tenantId,
...
//# sourceMappingURL=dsoc-audit-bridge.js.map