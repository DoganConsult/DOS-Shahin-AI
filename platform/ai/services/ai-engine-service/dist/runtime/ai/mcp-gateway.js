...
export class MCPGateway {
    ...
    async initialize() {
        ...
        try {
            ...
            this.client = new Client({ name: 'ai-engine-service', version: '1.0.0' }, { capabilities: {} });
            await this.client.connect(this.transport);
            logger.info(`[MCP Gateway] connected to ${this.baseUrl}`);
        }
        catch (err) {
            logger.error(`[MCP Gateway] initialize failed — ${err.message}`);
            this.client = null;
            this.transport = null;
        }
    }
    ...
    async callTool(principal, toolName, args = {}) {
        if (!this.isEnabled)
            return null;
        if (!this.client)
            await this.initialize();
        if (!this.client)
            throw new Error('[MCP Gateway] client unavailable');
    ...
    async fetchContext(principal, modelName, query) {
        if (!this.isEnabled)
            return '';
        try {
            ...
            if (!contextTool)
                return '';
            const result = (await this.callTool(principal, contextTool.name, { modelName, query }));
            const text = result?.content?.find((c) => c.type === 'text')?.text;
            return text ?? '';
        }
        catch (err) {
            logger.warn(`[MCP Gateway] fetchContext failed (modelName=${modelName}) — ${err.message}`);
            return '';
        }
    }
    async shutdown() {
        if (this.transport) {
            try {
                ...
                logger.warn(`[MCP Gateway] transport close failed — ${err.message}`);
            }
        }
        this.client = null;
        this.transport = null;
