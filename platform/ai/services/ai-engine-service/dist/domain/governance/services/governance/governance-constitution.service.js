...
export async function getRiskAppetite(tenantId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT id, metadata, status, created_at, updated_at
     FROM "${schema}".risk_appetite_statements
     WHERE status != 'archived'
     ORDER BY created_at DESC`).catch(() => ({ rows: [] }));
...
export async function upsertRiskAppetite(tenantId, appetite) {
    const schema = tenantSchema(tenantId);
    ...
    for (const item of items) {
        ...
        await safeQuery(`INSERT INTO "${schema}".risk_appetite_statements (id, tenant_id, metadata, status, updated_at)
       VALUES ($1, $2, $3::jsonb, 'active', NOW())
       ON CONFLICT (id) DO UPDATE SET metadata = $3::jsonb, status = 'active', updated_at = NOW()`, [id, tenantId, JSON.stringify(metadata)]);
        upserted.push({ id, ...metadata });
    }
    return upserted;
}
export async function getAuthorityMatrix(tenantId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT config_value FROM "${schema}".governance_settings
     WHERE config_key = 'authority_matrix' AND is_active = TRUE
     LIMIT 1`).catch(() => ({ rows: [] }));
    return parseJsonb(result.rows[0]?.config_value, { rules: [] });
}
export async function upsertAuthorityMatrix(tenantId, matrix) {
    const schema = tenantSchema(tenantId);
    await safeQuery(`INSERT INTO "${schema}".governance_settings (tenant_id, config_key, config_value, is_active, updated_at)
     ...
     ON CONFLICT (config_key) DO UPDATE SET config_value = $2::jsonb, is_active = TRUE, updated_at = NOW()`, [tenantId, JSON.stringify(matrix)]).catch(async () => {
        await safeQuery(`INSERT INTO "${schema}".governance_settings (tenant_id, config_key, config_value, is_active, updated_at)
       VALUES ($1, 'authority_matrix', $2::jsonb, TRUE, NOW())`, [tenantId, JSON.stringify(matrix)]);
    });
    return matrix;
}
export async function getEscalationThresholds(tenantId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`SELECT config_value FROM "${schema}".governance_settings
...
export async function upsertEscalationThresholds(tenantId, thresholds) {
    const schema = tenantSchema(tenantId);
    await safeQuery(`INSERT INTO "${schema}".governance_settings (tenant_id, config_key, config_value, is_active, updated_at)
     VALUES ($1, 'escalation_thresholds', $2::jsonb, TRUE, NOW())
     ON CONFLICT (config_key) DO UPDATE SET config_value = $2::jsonb, is_active = TRUE, updated_at = NOW()`, [tenantId, JSON.stringify(thresholds)]).catch(async () => {
        await safeQuery(`INSERT INTO "${schema}".governance_settings (tenant_id, config_key, config_value, is_active, updated_at)
       VALUES ($1, 'escalation_thresholds', $2::jsonb, TRUE, NOW())`, [tenantId, JSON.stringify(thresholds)]);
    });
    return thresholds;
}
export async function getConstitution(tenantId) {
...
export async function resolveApprover(tenantId, decisionType, criticality) {
    const matrix = await getAuthorityMatrix(tenantId);
    const rules = Array.isArray(matrix.rules) ? matrix.rules : [];
    const match = rules.find(r => r.decisionType === decisionType && (r.criticality === criticality || r.criticality === '*'));
    if (!match)
        return null;
    return {
...
export async function checkRiskAgainstAppetite(tenantId, risk) {
    ...
    const matching = appetite.find((a) => a.category === category || a.category === '*');
    if (!matching)
        return true;
    const maxResidual = Number(matching.maxResidualScore ?? matching.maxScore ?? 100);
    return score <= maxResidual;
}
export async function validateConstitution(tenantId) {
...
export async function seedDefaultConstitution(tenantId) {
