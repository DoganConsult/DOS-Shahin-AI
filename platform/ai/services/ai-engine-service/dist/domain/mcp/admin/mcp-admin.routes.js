...
router.get('/settings', authenticate, requirePermission('mcp.admin.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT key, value FROM "${schema}".module_settings WHERE module_code = $1`, ['mcp']).catch(() => ({ rows: [] }));
    res.json({ success: true, data: rows });
}));
router.get('/health', authenticate, requirePermission('mcp.admin.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const schema = tenantSchema(tenantId);
    const { rows: tables } = await safeQuery(`SELECT table_name FROM information_schema.tables
       WHERE table_schema IN ($1, 'public')
         AND table_name LIKE 'mcp_%'`, [schema]).catch(() => ({ rows: [] }));
...
router.post('/expire-approvals', authenticate, requirePermission('mcp.admin.manage'), validate({ body: genericRouteSchema }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
...
router.get('/sla', authenticate, requirePermission('mcp.admin.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const { rows } = await safeQuery(`SELECT config_value FROM "${schema}".module_configs WHERE module_code = 'mcp' AND config_key = 'sla_policy' LIMIT 1`, []).catch(() => ({ rows: [] }));
...
router.get('/escalation-policy', authenticate, requirePermission('mcp.admin.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const { rows } = await safeQuery(`SELECT config_value FROM "${schema}".module_configs WHERE module_code = 'mcp' AND config_key = 'escalation_policy' LIMIT 1`, []).catch(() => ({ rows: [] }));
...
router.get('/runbooks', authenticate, requirePermission('mcp.tool.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (_req, res) => {
    res.json({
        success: true, data: {
            moduleCode: 'mcp',
            runbooks: [
...
//# sourceMappingURL=mcp-admin.routes.js.map