...
router.get('/', authenticate, requirePermission('mcp.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (_req, res) => {
...
router.get('/all', authenticate, requirePermission('mcp.agent.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (_req, res) => {
    const agents = await registryService.listAllAgents();
    res.json({ success: true, data: agents, total: agents.length });
}));
router.get('/:agentId', authenticate, requirePermission('mcp.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
...
router.get('/:agentId/tools', authenticate, requirePermission('mcp.tool.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
...
router.patch('/:agentId/enable', authenticate, requirePermission('mcp.agent.manage'), validate({ body: agentEnableBody }), asyncHandler(async (req, res) => {
    if (req.body.enabled) {
...
//# sourceMappingURL=mcp-agent.routes.js.map