...
export async function getMcpDiagnosticsSnapshot(tenantId) {
    const schema = tenantSchema(tenantId);
    const checks = [];
    const schemaRes = await safeQuery(`SELECT 1 FROM information_schema.schemata WHERE schema_name = $1`, [schema]).catch(() => ({ rows: [] }));
    checks.push({
        check: 'schema_exists',
        status: schemaRes.rows.length > 0 ? 'pass' : 'fail',
        detail: schemaRes.rows.length > 0 ? 'Schema exists' : 'Schema missing',
    });
    const tableRes = await safeQuery(`SELECT table_name FROM information_schema.tables WHERE table_schema = $1 AND table_name LIKE 'mcp_%'`, [schema]).catch(() => ({ rows: [] }));
    ...
    const pendingRes = await safeQuery(`SELECT COUNT(*) as cnt FROM "${schema}".mcp_tool_approval_requests WHERE status = 'pending' AND created_at < NOW() - INTERVAL '48 hours'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
    ...
    const failedRes = await safeQuery(`SELECT COUNT(*) as cnt FROM "${schema}".mcp_tool_execution_log WHERE status = 'error' AND executed_at > NOW() - INTERVAL '24 hours'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
    ...
    const staleRes = await safeQuery(`SELECT COUNT(*) as cnt FROM "${schema}".mcp_tool_registry WHERE status = 'active' AND updated_at < NOW() - INTERVAL '90 days'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
    ...
    const suspendedRes = await safeQuery(`SELECT COUNT(*) as cnt FROM "${schema}".mcp_tool_registry WHERE status = 'suspended'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
    ...
    const auditRes = await safeQuery(`SELECT COUNT(*) as cnt FROM "${schema}".audit_trail WHERE module_code = 'mcp' AND created_at > NOW() - INTERVAL '24 hours'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
...
//# sourceMappingURL=mcp-diagnostics.service.js.map