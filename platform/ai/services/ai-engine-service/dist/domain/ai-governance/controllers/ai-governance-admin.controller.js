...
export async function updateModuleConfig(req, res) {
    setAuditData(res, { action: 'update', entityType: 'ai-governance_config', entityId: 'ai-governance' });
...
export async function reseedModule(req, res) {
    ...
    setAuditData(res, { action: 'reseed', entityType: 'ai-governance', entityId: tenantId });
...
export async function reindexModule(req, res) {
    setAuditData(res, { action: 'reindex', entityType: 'ai-governance', entityId: req.tenantId });
    res.json(action('Reindex initiated', req));
}
export async function backfillModule(req, res) {
    setAuditData(res, { action: 'backfill', entityType: 'ai-governance', entityId: req.tenantId });
    res.json(action('Backfill initiated', req));
}
export async function getSlaConfig(req, res) {
    const schema = tenantSchema(req.tenantId);
    const { rows } = await safeQuery(`SELECT config_value FROM "${schema}".module_configs WHERE module_code = 'ai-governance' AND config_key = 'sla_policy' LIMIT 1`, []).catch(() => ({ rows: [] }));
...
export async function getEscalationPolicy(req, res) {
    const schema = tenantSchema(req.tenantId);
    const { rows } = await safeQuery(`SELECT config_value FROM "${schema}".module_configs WHERE module_code = 'ai-governance' AND config_key = 'escalation_policy' LIMIT 1`, []).catch(() => ({ rows: [] }));
...
//# sourceMappingURL=ai-governance-admin.controller.js.map