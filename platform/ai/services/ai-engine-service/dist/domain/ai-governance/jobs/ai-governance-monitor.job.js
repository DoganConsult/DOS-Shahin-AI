...
export async function getAiGovernanceJobs() {
                ...
                try {
                    ...
                    for (const t of tenants) {
                        try {
                            ...
                            const overdueResult = await safeQuery(`SELECT COUNT(*)::int as count FROM "${schema}".ai_governance_model_registrys
            ...
            handler: async () => {
                ...
                try {
                    ...
                    for (const t of tenants) {
                        try {
                            const schema = tenantSchema(t.tenant_id);
                            const result = await safeQuery(`UPDATE "${schema}".ai_governance_model_registrys
            ...
            handler: async () => {
                ...
                try {
                    ...
                    for (const t of tenants) {
                        try {
                            const schema = tenantSchema(t.tenant_id);
                            await safeQuery(`UPDATE "${schema}".ai_governance_model_registrys
                 SET deleted_at = NOW()
                 WHERE deleted_at IS NULL
                   AND status = 'archived'
...
//# sourceMappingURL=ai-governance-monitor.job.js.map