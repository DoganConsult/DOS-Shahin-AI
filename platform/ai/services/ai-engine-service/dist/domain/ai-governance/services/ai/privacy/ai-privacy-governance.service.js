...
// -----------------------------------------------
export async function registerPrivacyImpact(tenantId, impact) {
...
// -----------------------------------------------
export async function reportPrivacyIncident(tenantId, incident) {
    ...
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_privacy_incidents
    ...
    swallow(EC.EVENT_BUS, eventBus.publish({
        eventType: 'privacy.dpia_required',
        tenantId,
...
// -----------------------------------------------
export async function registerAutomatedDecision(tenantId, decision) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`INSERT INTO "${schema}".automated_decision_register
...
// -----------------------------------------------
export async function registerProfiling(tenantId, profiling) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_profiling_register
...
// -----------------------------------------------
export async function configureHumanOversight(tenantId, config) {
...
// -----------------------------------------------
export async function registerTrainingData(tenantId, data) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_training_data_registry
...
export async function listTrainingData(tenantId, systemId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_training_data_registry
...
// -----------------------------------------------
export async function getPrivacyDashboard(tenantId, systemId) {
    const schema = tenantSchema(tenantId);
    const systemFilter = systemId ? ' AND system_id = $1' : '';
    const params = systemId ? [systemId] : [];
    const { rows: incidentRows } = await safeQuery(`SELECT
       COUNT(*)::int AS total,
       COUNT(*) FILTER (WHERE status = 'open')::int AS open,
       COUNT(*) FILTER (WHERE severity = 'critical')::int AS critical
     FROM "${schema}".ai_privacy_incidents
     WHERE 1=1${systemFilter}`, params);
    const { rows: epsilonRows } = await safeQuery(`SELECT
    ...
    const { rows: decisionRows } = await safeQuery(`SELECT
       COUNT(*)::int AS total,
     ...
     WHERE 1=1${systemFilter}`, params);
    const { rows: oversightRows } = await safeQuery(`SELECT system_id, oversight_level, intervention_triggers, review_frequency_days, updated_at
...
// -----------------------------------------------
export async function getPrivacyImpactById(tenantId, impactId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_privacy_impact_register WHERE id = $1`, [impactId]);
    return rows[0] ?? null;
}
export async function getPrivacyIncidentById(tenantId, incidentId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_privacy_incidents WHERE id = $1`, [incidentId]);
    return rows[0] ?? null;
}
export async function getAutomatedDecisionById(tenantId, decisionId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".automated_decision_register WHERE id = $1`, [decisionId]);
    return rows[0] ?? null;
}
export async function getProfilingById(tenantId, profilingId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_profiling_register WHERE id = $1`, [profilingId]);
    return rows[0] ?? null;
}
// -----------------------------------------------
// 9. List/GET methods for audit trail
// -----------------------------------------------
export async function listPrivacyImpacts(tenantId, filters) {
    const schema = tenantSchema(tenantId);
...
export async function listPrivacyIncidents(tenantId, filters) {
    const schema = tenantSchema(tenantId);
...
export async function listAutomatedDecisions(tenantId, filters) {
    const schema = tenantSchema(tenantId);
...
export async function listProfilingActivities(tenantId, filters) {
    const schema = tenantSchema(tenantId);
...
export async function getHumanOversightConfig(tenantId, systemId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".human_oversight_config
     WHERE system_id = $1 AND is_active = TRUE
     ORDER BY created_at DESC LIMIT 1`, [systemId]);
    return rows[0] ?? null;
}
//# sourceMappingURL=ai-privacy-governance.service.js.map