...
export async function evaluateAIActionApproval(tenantId, actionId) {
...
export async function enforceAIGovernanceWorkflow(tenantId, actionId, requestedBy) {
    ...
    try {
        const approverRes = await safeQuery(`SELECT user_id FROM "${schema}".user_roles WHERE role_code = $1 LIMIT 1`, [firstApprover]);
    ...
    await swallow(EC.EVENT_BUS, eventBus.publish({
        eventType: 'ai-governance.approval_requested',
...
export function registerAIGovernanceApprovalSubscribers() {
    eventBus.subscribe('ai-governance.action_submitted', 'ai-gov-approval:submitted', async (event) => {
        if (!event.tenantId)
            return;
        const actionId = event.payload?.actionId || event.entityId;
        const requestedBy = event.payload?.requestedBy || 'system';
        if (actionId) {
            try {
                await enforceAIGovernanceWorkflow(event.tenantId, actionId, requestedBy);
            }
            catch (err) {
                logger.error(`[AIGovernanceApproval] workflow failed: ${err.message}`);
            }
        }
    });
    eventBus.subscribe('ai-governance.model_deployed', 'ai-gov-approval:model-audit', async (event) => {
        if (!event.tenantId)
            return;
        const modelId = event.payload?.modelId || event.entityId;
        await swallow(EC.EVENT_BUS, recordAudit({
            tenantId: event.tenantId, userId: event.payload?.deployedBy || 'system',
...
//# sourceMappingURL=ai-governance-approval-bridge.service.js.map