...
export function registerAiGovernanceSeedProvider(productCode, provider) {
    seedProviders.set(productCode, provider);
}
export function getAggregatedCatalog() {
    const agents = [];
    const providers = [];
    const models = [];
    const tools = [];
...
export function getRegisteredSeedProviders() { return [...seedProviders.keys()]; }
