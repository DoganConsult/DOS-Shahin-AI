...
export async function createMonitoringPlan(tenantId, plan) {
    ...
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_monitoring_plans
...
export async function recordMetric(tenantId, metric) {
    const schema = tenantSchema(tenantId);
    const isBreach = metric.threshold_value != null && metric.metric_value < metric.threshold_value;
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_performance_metrics
    ...
    if (isBreach) {
        await swallow(EC.EVENT_BUS, eventBus.publish({
            eventType: 'ai.metric.breach', tenantId, sourceService: 'ai-post-market', severity: 'warning',
            payload: {
...
// 3. checkThresholds
export async function checkThresholds(tenantId, systemId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT m.*, p.threshold_config
     ...
     ORDER BY m.created_at DESC LIMIT 50`, [systemId]);
    const { rows: total } = await safeQuery(`SELECT count(*) as cnt FROM "${schema}".ai_performance_metrics WHERE system_id = $1`, [systemId]);
...
export async function reportSeriousIncident(tenantId, incident) {
    const schema = tenantSchema(tenantId);
    const isSafety = incident.incident_type === 'safety';
    const deadlineInterval = isSafety ? "interval '72 hours'" : "interval '15 days'";
    const authorityRequired = ['high', 'critical'].includes(incident.severity);
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_serious_incidents
    ...
    await swallow(EC.EVENT_BUS, eventBus.publish({
        eventType: 'ai.serious_incident.reported', tenantId, sourceService: 'ai-post-market', severity: 'critical',
...
export async function generatePeriodicReport(tenantId, systemId, period = 'monthly') {
    const schema = tenantSchema(tenantId);
    ...
    const [system, metrics, incidents, complaints] = await Promise.all([
        ...
        safeQuery(`SELECT metric_name, avg(metric_value) as avg_val, count(*) as cnt,
       ...
       GROUP BY metric_name`, [systemId]),
        safeQuery(`SELECT count(*) as cnt, severity
       FROM "${schema}".ai_serious_incidents
       WHERE system_id = $1 AND created_at >= now() - interval '${interval}'
       GROUP BY severity`, [systemId]),
        safeQuery(`SELECT count(*) as cnt, status
       FROM "${schema}".ai_user_complaints
       WHERE system_id = $1 AND created_at >= now() - interval '${interval}'
       GROUP BY status`, [systemId]),
    ]);
    return {
...
export async function listMonitoringPlans(tenantId, systemId) {
    const schema = tenantSchema(tenantId);
    const where = systemId ? 'WHERE system_id = $1' : '';
    const params = systemId ? [systemId] : [];
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_monitoring_plans ${where} ORDER BY created_at DESC LIMIT 100`, params);
    return rows;
}
// 7. getMonitoringPlanById
export async function getMonitoringPlanById(tenantId, planId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_monitoring_plans WHERE id = $1`, [planId]);
    return rows[0] ?? null;
}
// 8. listMetrics
export async function listMetrics(tenantId, filters) {
    const schema = tenantSchema(tenantId);
    const conditions = [];
    const params = [];
    let idx = 1;
    ...
    if (filters?.is_breach !== undefined) {
        conditions.push(`is_breach = $${idx++}`);
        params.push(filters.is_breach);
    }
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_performance_metrics ${where} ORDER BY created_at DESC LIMIT 100`, params);
    return rows;
}
// 9. listSeriousIncidents
export async function listSeriousIncidents(tenantId, filters) {
    const schema = tenantSchema(tenantId);
    const conditions = [];
    const params = [];
    let idx = 1;
    ...
    if (filters?.report_status) {
        conditions.push(`report_status = $${idx++}`);
        params.push(filters.report_status);
    }
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_serious_incidents ${where} ORDER BY created_at DESC LIMIT 100`, params);
    return rows;
}
// 10. getSeriousIncidentById
export async function getSeriousIncidentById(tenantId, incidentId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_serious_incidents WHERE id = $1`, [incidentId]);
    return rows[0] ?? null;
}
// 11. listUserComplaints
export async function listUserComplaints(tenantId, filters) {
    const schema = tenantSchema(tenantId);
    const conditions = [];
    const params = [];
    let idx = 1;
    ...
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_user_complaints ${where} ORDER BY created_at DESC LIMIT 100`, params);
    return rows;
}
// 12. handleUserComplaint
export async function handleUserComplaint(tenantId, complaint) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_user_complaints
    ...
    if (complaint.severity === 'critical') {
        await safeQuery(`UPDATE "${schema}".ai_user_complaints
...
//# sourceMappingURL=ai-post-market-monitoring.service.js.map