...
export async function createExperiment(tenantId, experiment) {
    ...
    const { rows } = await safeQuery(`INSERT INTO "${schema}".ai_model_experiments
...
export async function recordVariantMetrics(tenantId, experimentId, variant, metrics) {
    const schema = tenantSchema(tenantId);
    ...
    await safeQuery(`UPDATE "${schema}".ai_model_experiments
     SET ${resultCol} = $1, ${sampleCol} = ${sampleCol} + $2, updated_at = now()
     WHERE id = $3`, [JSON.stringify(metrics), metrics.sample_count ?? 1, experimentId]);
}
// 3. declareWinner
export async function declareWinner(tenantId, experimentId, winner, significance, declaredBy) {
    const schema = tenantSchema(tenantId);
    await safeQuery(`UPDATE "${schema}".ai_model_experiments
     SET winner = $1, statistical_significance = $2,
         declared_by = $3, declared_at = now(),
         status = 'completed', updated_at = now()
     WHERE id = $4`, [winner, significance, declaredBy, experimentId]);
}
// 4. getExperimentById
export async function getExperimentById(tenantId, experimentId) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_model_experiments WHERE id = $1`, [experimentId]);
    return rows[0] ?? null;
}
// 5. listExperiments
export async function listExperiments(tenantId, systemId) {
    const schema = tenantSchema(tenantId);
    if (systemId) {
        const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_model_experiments
       WHERE system_id = $1 ORDER BY created_at DESC`, [systemId]);
        return rows;
    }
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".ai_model_experiments ORDER BY created_at DESC`);
    return rows;
}
//# sourceMappingURL=ai-model-experiments.service.js.map