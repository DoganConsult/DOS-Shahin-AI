 ...
 */
export async function getOrgStructureSnapshot(tenantId) {
 ...
 */
export async function analyzeOrgImpact(tenantId, changeScenario) {
    const schema = tenantSchema(tenantId);
    ...
    const complianceImpactAreas = [];
    const recommendations = [];
    for (const change of changeScenario.changes) {
        switch (change.operation) {
            case 'add_department': {
                unitsAdded++;
                recommendations.push(`New unit "${change.unitName || change.unitId}" requires control assignment and RACI mapping`);
                break;
            }
            case 'remove_department': {
                ...
                if (unit) {
                    ...
                    if (unit.controlCount > 0) {
                        complianceImpactAreas.push(`Controls from ${unit.unitName} must be reassigned (${unit.controlCount} controls)`);
                    }
                }
                recommendations.push(`Removing "${unit?.unitName || change.unitId}" requires control/risk reassignment plan`);
                break;
            }
            case 'move_department': {
                ...
                recommendations.push(`Moving department changes reporting lines — verify delegation chains`);
                break;
            }
            case 'add_role': {
                rolesChanged++;
                ...
                try {
                    const { rows: existingRoles } = await safeQuery(`SELECT role_name, permissions
             FROM "${schema}".functional_roles
             WHERE org_unit_id = $1 AND deleted_at IS NULL`, [change.unitId]);
                    if (existingRoles.length > 3) {
                        sodConflictsIntroduced++;
                        complianceImpactAreas.push(`Adding role "${change.roleName}" to a unit with ${existingRoles.length} existing roles — SoD review required`);
                    }
                }
                catch { /* non-fatal */ }
                break;
            ...
            case 'remove_role': {
                rolesChanged++;
                try {
                    const { rows: affectedAssignments } = await safeQuery(`SELECT COUNT(*)::int AS cnt
             FROM "${schema}".user_role_assignments
             WHERE role_name = $1 AND deleted_at IS NULL`, [change.roleName]);
                    const cnt = affectedAssignments[0]?.cnt || 0;
                    affectedUsers += cnt;
                    raciChanges += cnt;
                }
                catch { /* non-fatal */ }
                recommendations.push(`Removing role "${change.roleName}" requires RACI reassignment`);
                break;
            }
            case 'change_reporting_line': {
                unitsMoved++;
                raciChanges++;
                recommendations.push('Reporting line change may affect delegation chains and approval workflows');
                break;
            }
        }
    }
    ...
    let overallRisk;
    if (totalImpact > 50 || sodConflictsIntroduced > 3)
        overallRisk = 'critical';
    else if (totalImpact > 20 || sodConflictsIntroduced > 1)
        overallRisk = 'high';
 ...
 */
export async function compareScenarios(tenantId, scenarioA, scenarioB) {
    ...
    const riskDelta = riskRank[impactA.riskAssessment.overallRisk] - riskRank[impactB.riskAssessment.overallRisk];
    const controlCoverageDelta = impactA.afterState.controlCoverage - impactB.afterState.controlCoverage;
    const sodConflictDelta = impactA.sodConflictsIntroduced - impactB.sodConflictsIntroduced;
...
//# sourceMappingURL=digital-twin-org-analysis.service.js.map