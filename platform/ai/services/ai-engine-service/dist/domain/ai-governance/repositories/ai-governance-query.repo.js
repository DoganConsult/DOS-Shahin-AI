...
export async function getDashboardStats(tenantId) {
    ...
    FROM "${schema}".ai_governance_model_registrys
...
export async function getKpiMetrics(tenantId) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`
    SELECT
      COUNT(*)::int as total,
...
export async function searchEntities(tenantId, params) {
    const schema = tenantSchema(tenantId);
    ...
    const countResult = await safeQuery(`SELECT COUNT(*)::int as total FROM "${schema}".ai_governance_model_registrys WHERE ${where}`, values);
    const dataResult = await safeQuery(`SELECT * FROM "${schema}".ai_governance_model_registrys WHERE ${where} ORDER BY ${sortBy} ${sortDir} LIMIT $${idx} OFFSET $${idx + 1}`, [...values, pageSize, offset]);
    return { rows: dataResult.rows, total: countResult.rows[0]?.total || 0 };
}
export async function getExportData(tenantId, filters) {
    const schema = tenantSchema(tenantId);
    // Parameterize filters.status instead of interpolating into SQL.
    const conditions = ['deleted_at IS NULL'];
    const values = [];
    if (filters?.status) {
    ...
    const result = await safeQuery(`SELECT * FROM "${schema}".ai_governance_model_registrys WHERE ${conditions.join(' AND ')} ORDER BY created_at DESC`, values);
    return result.rows;
}
export async function getCrossModuleView(tenantId, linkedModule) {
    const schema = tenantSchema(tenantId);
    const result = await safeQuery(`
    SELECT e.*, el.linked_entity_id, el.linked_module, el.link_type
    FROM "${schema}".ai_governance_model_registrys e
    LEFT JOIN "${schema}".entity_links el ON el.source_entity_id = e.id AND el.source_module = 'ai-governance'
    WHERE e.deleted_at IS NULL AND el.linked_module = $1
    ORDER BY e.created_at DESC
  `, [linkedModule]);
    return result.rows;
}
//# sourceMappingURL=ai-governance-query.repo.js.map