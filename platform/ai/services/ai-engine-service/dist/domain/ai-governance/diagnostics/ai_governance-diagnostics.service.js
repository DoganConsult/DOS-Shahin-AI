...
export async function runDiagnostics(tenantId) {
    ...
    const ownedTables = ['ai_gov_assessments', 'ai_gov_audit_log', 'ai_gov_bias_reports', 'ai_gov_data_lineage', 'ai_gov_ethical_reviews'];
    ...
    checks.push({
        ...
        detail: `${tableRows.length}/${ownedTables.length} core ai-governance tables found`,
    });
    const { rows: staleRows } = await safeQuery(`SELECT COUNT(*) AS cnt FROM "${schema}".ai_gov_assessments WHERE updated_at < NOW() - INTERVAL '90 days'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
    ...
    const { rows: configRows } = await safeQuery(`SELECT COUNT(*) AS cnt FROM "${schema}".module_configs WHERE module_code = $1`, ['ai-governance']).catch(() => ({ rows: [{ cnt: 0 }] }));
    ...
    const { rows: auditRows } = await safeQuery(`SELECT COUNT(*) AS cnt FROM "${schema}".audit_trail WHERE module = 'ai-governance' AND created_at > NOW() - INTERVAL '24 hours'`, []).catch(() => ({ rows: [{ cnt: 0 }] }));
...
//# sourceMappingURL=ai_governance-diagnostics.service.js.map