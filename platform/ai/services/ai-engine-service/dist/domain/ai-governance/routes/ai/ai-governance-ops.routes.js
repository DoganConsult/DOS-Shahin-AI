...
router.get("/summary", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
...
router.get("/events", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
...
router.post("/break-glass", authenticate, requirePermission("ai_governance.manage"), validate({ body: breakGlassPostBody }), async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
router.post("/break-glass/:id/revoke", authenticate, requirePermission("ai_governance.manage"), validate({ body: breakGlassIdRevokePostBody }), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const userId = req.user.userId;
        if (!tenantId)
...
router.get("/break-glass", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
...
router.post("/promotions", authenticate, requirePermission("ai_governance.manage"), validate({ body: promotionsPostBody }), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const userId = req.user.userId;
        if (!tenantId)
...
router.get("/promotions", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
...
router.get("/health", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
...
router.post("/repair", authenticate, requirePermission("platform.system.admin"), validate({ body: repairPostBody }), async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
// ── AI Cost Summary ──────────────────────────────────────────────────────
router.get("/cost-summary", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
        ...
        const { costRows, byAgentRows, byModelRows } = await withTenantClient(tenantId, async (client) => {
            const cost = await client.query(`SELECT
            ...
            const agent = await client.query(`SELECT agent_id AS "agentId", agent_id AS "agentName", model,
             COUNT(*)::int AS "totalCalls",
            ...
            const model = await client.query(`SELECT model, COUNT(*)::int AS calls,
             COALESCE(SUM(cost_usd), 0)::float AS "costUsd",
...
// ── AI Budget ────────────────────────────────────────────────────────────
router.get("/budget", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
        ...
        const result = await safeQuery(`SELECT COALESCE((config->>'ai_monthly_budget_usd')::float, 0) AS "monthlyLimit"
...
// ── AI Opt-Out Config ────────────────────────────────────────────────────
router.get("/opt-out-config", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
        const schema = `tenant_${tenantId}`;
        const { safeQuery } = await import('@dos/db');
        const result = await safeQuery(`SELECT config FROM "${schema}".tenant_config WHERE key = 'ai_opt_out' LIMIT 1`, []);
...
router.put("/opt-out-config", authenticate, requirePermission("ai.governance.write"), validate({ body: optOutConfigSchema }), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const userId = req.user.userId;
        if (!tenantId)
        ...
        await safeQuery(`INSERT INTO "${schema}".tenant_config (key, config, updated_at)
         VALUES ('ai_opt_out', $1::jsonb, NOW())
         ON CONFLICT (key) DO UPDATE SET config = $1::jsonb, updated_at = NOW()`, [JSON.stringify(config)]);
        setAuditData(res, {
            action: "update",
...
// ── AI Audit Trail ───────────────────────────────────────────────────────
router.get("/audit-trail", validate({ query: z.record(z.unknown()) }), authenticate, requirePermission("ai.governance.read"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: "Missing tenant context" });
        ...
        const { safeQuery } = await import('@dos/db');
        const result = await safeQuery(`SELECT id, tenant_id AS "tenantId", user_id AS "userId", agent_id AS "agentId",
...
//# sourceMappingURL=ai-governance-ops.routes.js.map