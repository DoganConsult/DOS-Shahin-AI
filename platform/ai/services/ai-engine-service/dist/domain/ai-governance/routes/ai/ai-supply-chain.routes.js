...
router.get('/vendors', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const agreements = await listSupplierAgreements(req.tenantId);
    res.json({ success: true, data: agreements, total: agreements.length });
}));
router.post('/agreements', authenticate, requirePermission('ai.governance.write'), validate({ body: agreementBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await createSupplierAgreement(req.tenantId, { ...req.body, created_by: userId });
    setAuditData(res, { action: 'create', entityType: 'ai_supplier_agreement', entityId: result.id });
...
router.get('/agreements/:agreementId', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
...
router.patch('/agreements/:agreementId/status', authenticate, requirePermission('ai.governance.write'), validate({ body: agreementStatusBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await updateAgreementStatus(req.tenantId, req.params.agreementId, req.body.status);
    setAuditData(res, { action: 'status_change', entityType: 'ai_supplier_agreement', entityId: req.params.agreementId, afterState: { status: req.body.status } });
...
router.get('/agreements/expiring', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
...
router.get('/providers', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const providers = await listProviders(req.tenantId);
    res.json({ success: true, data: providers, total: providers.length });
}));
router.post('/providers', authenticate, requirePermission('ai.governance.write'), validate({ body: providerBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await registerProvider(req.tenantId, { ...req.body, created_by: userId });
    setAuditData(res, { action: 'create', entityType: 'ai_provider', entityId: result.id });
...
router.get('/providers/:providerId', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
...
router.patch('/providers/:providerId/compliance', authenticate, requirePermission('ai.governance.write'), validate({ body: updateComplianceBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await updateProviderCompliance(req.tenantId, req.params.providerId, req.body);
    setAuditData(res, { action: 'update_compliance', entityType: 'ai_provider', entityId: req.params.providerId });
...
router.post('/provenance', authenticate, requirePermission('ai.governance.write'), validate({ body: provenanceBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await registerModelProvenance(req.tenantId, { ...req.body, created_by: userId });
    setAuditData(res, { action: 'create', entityType: 'ai_model_provenance', entityId: result.id });
...
router.get('/provenance/:provenanceId', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const result = await getProvenanceById(req.tenantId, req.params.provenanceId);
    if (!result) {
        res.status(404).json({ success: false, error: 'Provenance record not found' });
        return;
    }
    res.json({ success: true, data: result });
}));
router.post('/lineage', authenticate, requirePermission('ai.governance.write'), validate({ body: lineageBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await trackDataLineage(req.tenantId, req.body);
    setAuditData(res, { action: 'create', entityType: 'ai_data_lineage', entityId: result.id });
...
router.get('/lineage/:lineageId', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const result = await getLineageById(req.tenantId, req.params.lineageId);
    if (!result) {
        res.status(404).json({ success: false, error: 'Lineage record not found' });
        return;
    }
    res.json({ success: true, data: result });
}));
router.get('/:systemId/aibom', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const bom = await generateAiBom(req.tenantId, req.params.systemId);
    res.json({ success: true, data: bom });
}));
router.get('/:systemId/risk-assessment', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const assessment = await assessSupplyChainRisk(req.tenantId, req.params.systemId);
    res.json({ success: true, data: assessment });
}));
router.get('/:systemId/data-sovereignty', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const result = await checkDataSovereignty(req.tenantId, req.params.systemId);
    res.json({ success: true, data: result });
}));
router.post('/modifications', authenticate, requirePermission('ai.governance.write'), validate({ body: modificationBody }), asyncHandler(async (req, res) => {
    const userId = req.user.userId;
    const result = await recordModelModification(req.tenantId, { ...req.body, approved_by: userId });
    setAuditData(res, { action: 'create', entityType: 'ai_model_modification', entityId: result.id, afterState: { modification_type: req.body.modification_type, is_substantial: req.body.is_substantial } });
...
router.get('/modifications', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const filters = {};
    if (req.query.provenance_id)
        filters.provenance_id = req.query.provenance_id;
    if (req.query.system_id)
        filters.system_id = req.query.system_id;
    if (req.query.is_substantial !== undefined)
        filters.is_substantial = req.query.is_substantial === 'true';
...
router.get('/modifications/:modificationId', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const result = await getModificationById(req.tenantId, req.params.modificationId);
    if (!result) {
        res.status(404).json({ success: false, error: 'Modification record not found' });
        return;
    }
    res.json({ success: true, data: result });
}));
router.get('/:systemId/substantial-modifications', authenticate, requirePermission('ai.governance.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const modifications = await getSubstantialModifications(req.tenantId, req.params.systemId);
    res.json({ success: true, data: modifications, total: modifications.length });
}));
export default router;
let genericPayloadSchema = z.record(z.unknown());
//# sourceMappingURL=ai-supply-chain.routes.js.map