...
router.post("/versions", authenticate, requirePermission("ai.governance.write"), validate({ body: createVersionsBody }), asyncHandler(async (req, res) => {
    try {
        ...
        if (!asset_id || !provider || !provider_model_id) {
            res.status(400).json({ error: "asset_id, provider, and provider_model_id are required" });
            return;
        }
        ...
        setAuditData(res, { action: "create", entityType: "model_version", entityId: version.model_version_id, afterState: version });
...
router.patch("/versions/:versionId", authenticate, requirePermission("ai.governance.write"), validate({ body: updateVersionsBody }), asyncHandler(async (req, res) => {
    try {
        ...
        setAuditData(res, { action: "update", entityType: "model_version", entityId: req.params.versionId, afterState: version });
        res.json(version);
    }
    catch (err) {
...
router.post("/versions/:versionId/approve", authenticate, requireAnyPermission("ai.model.approve", "ai.agent.approve"), validate({ body: createApproveBody }), asyncHandler(async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const userId = req.user.userId;
...
router.post("/versions/:versionId/reject", authenticate, requireAnyPermission("ai.model.approve", "ai.agent.approve"), validate({ body: createRejectBody }), asyncHandler(async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const userId = req.user.userId;
...
router.delete("/versions/:versionId", authenticate, requirePermission("ai_governance.manage"), validate({ body: genericPayloadSchema }), asyncHandler(async (req, res) => {
    try {
        ...
        setAuditData(res, { action: "delete", entityType: "model_version", entityId: req.params.versionId });
...
router.get("/governance/resolve/:agentId", authenticate, requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const { task_type, input_token_estimate } = req.query;
    const resolution = await resolveGovernedModel(tenantId, req.params.agentId, task_type, input_token_estimate ? parseInt(input_token_estimate) : undefined);
    res.json(resolution);
}));
router.get("/governance/mismatches", authenticate, requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
...
//# sourceMappingURL=model-registry.routes.js.map