...
function computeAssetStatus(row) {
    if (row.migration_status === 'completed')
        return 'active';
    if (row.expiry_date && new Date(row.expiry_date) < new Date())
        return 'expired';
    if (row.expiry_date && new Date(row.expiry_date) < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000))
        return 'expiring';
    if (row.is_quantum_vulnerable)
        return 'deprecated';
    return 'active';
}
// ── Helper: map DB vulnerability row to frontend shape ──
function computeVulnType(riskLevel) {
    switch (riskLevel) {
...
function computeTimeToQuantum(riskLevel) {
    switch (riskLevel) {
...
function computeVulnStatus(migrationStatus) {
...
function computeTestStatus(result) {
    switch (result) {
...
router.get('/kpis', asyncHandler(async (req, res) => {
    ...
    const vulnerableByRisk = dashboard.vulnerable_by_risk ?? [];
    const migrationProgress = dashboard.migration_progress ?? [];
    const quantumVulnerableAssets = vulnerableByRisk.reduce((sum, r) => sum + Number.parseInt(r.total ?? '0', 10), 0);
    const migratedRow = migrationProgress.find((r) => r.migration_status === 'completed');
    ...
    const criticalVulns = vulnerableByRisk.find((r) => r.hndl_risk_level === 'critical');
    // PQC test stats
    let pqcTestsPassed = 0;
    let pqcTestsFailed = 0;
    try {
        const allTests = await listPqcTestResults(tenantId);
        pqcTestsPassed = allTests.filter((t) => t.result === 'pass').length;
        pqcTestsFailed = allTests.filter((t) => t.result === 'fail').length;
    }
    catch { /* tables may not exist yet */ }
    const readinessScore = totalAssets > 0
        ? Math.min(Math.round(((pqcMigratedAssets * 40 +
            pqcTestsPassed * 30 +
...
router.get('/crypto-inventory', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
    const filters = {};
    ...
    if (req.query.hndl_risk_level)
        filters.hndl_risk_level = req.query.hndl_risk_level;
    if (req.query.migration_status)
        filters.migration_status = req.query.migration_status;
    const assets = await listCryptoAssets(tenantId, filters);
...
router.get('/crypto-inventory/:id', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
...
router.post('/crypto-inventory', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
...
router.post('/vulnerabilities/:assetId/assess', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
...
router.get('/migration-plans', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
    const filters = {};
    ...
    const plans = await listMigrationPlans(tenantId, filters);
...
router.get('/migration-plans/:id', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
...
router.post('/migration-plans', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
...
router.get('/pqc-tests', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
    const filters = {};
    if (req.query.plan_id)
        filters.plan_id = req.query.plan_id;
    if (req.query.asset_id)
        filters.asset_id = req.query.asset_id;
    if (req.query.result)
        filters.result = req.query.result;
    const tests = await listPqcTestResults(tenantId, filters);
...
router.post('/pqc-tests', asyncHandler(async (req, res) => {
    const tenantId = resolveTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: 'tenantId required' });
...
//# sourceMappingURL=quantum-security.routes.js.map