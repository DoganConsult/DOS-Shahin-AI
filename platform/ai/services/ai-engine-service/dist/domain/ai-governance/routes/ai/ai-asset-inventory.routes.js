...
router.get("/", authenticate, requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    if (req.query.asset_type) {
        if (!VALID_ASSET_TYPES.includes(req.query.asset_type)) {
            res.status(400).json({ error: `Invalid asset_type. Allowed: ${VALID_ASSET_TYPES.join(', ')}` });
            return;
        }
        q.asset_type = req.query.asset_type;
    }
    if (req.query.scope_type) {
        if (!VALID_SCOPE_TYPES.includes(req.query.scope_type)) {
            res.status(400).json({ error: `Invalid scope_type. Allowed: ${VALID_SCOPE_TYPES.join(', ')}` });
            return;
        }
        q.scope_type = req.query.scope_type;
    }
    if (req.query.lifecycle_status) {
        if (!VALID_LIFECYCLE.includes(req.query.lifecycle_status)) {
            res.status(400).json({ error: `Invalid lifecycle_status. Allowed: ${VALID_LIFECYCLE.join(', ')}` });
            return;
        }
    ...
    if (req.query.status) {
        if (!VALID_STATUS.includes(req.query.status)) {
            res.status(400).json({ error: `Invalid status. Allowed: ${VALID_STATUS.join(', ')}` });
            return;
        }
        q.status = req.query.status;
    }
    if (req.query.source_type) {
        if (!VALID_SOURCE.includes(req.query.source_type)) {
            res.status(400).json({ error: `Invalid source_type. Allowed: ${VALID_SOURCE.join(', ')}` });
            return;
        }
...
router.get("/lookup", authenticate, requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const { asset_type, asset_key, scope_type } = req.query;
    if (!asset_type || !asset_key) {
        res.status(400).json({ error: "asset_type and asset_key are required query params" });
        return;
    }
    ...
    const asset = await getAssetByKey(tenantId, asset_type, asset_key, scope_type);
    if (!asset) {
...
router.get("/:id", authenticate, requirePermission("ai.governance.read"), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    ...
    const transitions = getLifecycleTransitions(asset.lifecycle_status);
    res.json({ ...asset, allowed_transitions: transitions });
}));
router.post("/", authenticate, requirePermission("ai.governance.write"), validate({ body: createAiGovernanceBody }), asyncHandler(async (req, res) => {
    try {
        const tenantId = req.tenantId;
        ...
        if (!asset_type || !asset_key || !display_name) {
            res.status(400).json({ error: "asset_type, asset_key, and display_name are required" });
            return;
        }
        if (scope_type === 'global') {
            res.status(403).json({ error: "Tenant admins cannot create global-scoped assets. Global assets are platform-seeded only." });
            return;
        }
        ...
        setAuditData(res, { action: "create", entityType: "ai_asset", entityId: asset.asset_id, afterState: asset });
        res.status(201).json(asset);
    }
    catch (err) {
        if (toErrorMessage(err).includes('uq_ai_asset_scope') || toErrorMessage(err).includes('duplicate key')) {
            res.status(409).json({ error: "Asset with this scope+type+key already exists" });
            return;
        }
        res.status(400).json({ error: toErrorMessage(err) });
    }
}));
router.patch("/:id", authenticate, requirePermission("ai.governance.write"), requireOwnership('ai_asset'), validate({ body: updateAiGovernanceBody }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const userId = req.user.userId;
    ...
    if (isSeededGlobal(existing)) {
        res.status(403).json({ error: "Cannot modify seeded global assets. These are platform-managed." });
        return;
    }
    const { lifecycle_status, ...rest } = req.body;
    if (lifecycle_status) {
        res.status(400).json({ error: "Use POST /:id/transition to change lifecycle status. Do not include lifecycle_status in PATCH." });
        return;
    }
    ...
    setAuditData(res, { action: "update", entityType: "ai_asset", entityId: req.params.id, beforeState: existing, afterState: updated });
    res.json(updated);
}));
router.post("/:id/transition", authenticate, requirePermission("ai_governance.manage"), requireOwnership('ai_asset'), validate({ body: createTransitionBody }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const userId = req.user.userId;
    const existing = await getAssetById(tenantId, req.params.id);
    if (!existing) {
    ...
    if (isSeededGlobal(existing)) {
        res.status(403).json({ error: "Cannot transition seeded global assets. These are platform-managed." });
        return;
    }
    const { target_status } = req.body;
    if (!target_status) {
        res.status(400).json({ error: "target_status is required" });
        return;
    }
    if (!VALID_LIFECYCLE.includes(target_status)) {
        res.status(400).json({ error: `Invalid target_status. Allowed: ${VALID_LIFECYCLE.join(', ')}` });
        return;
    }
    const allowed = getLifecycleTransitions(existing.lifecycle_status);
    ...
    res.json({ ...updated, previous_status: existing.lifecycle_status, allowed_transitions: getLifecycleTransitions(target_status) });
}));
router.delete("/:id", authenticate, requirePermission("ai_governance.manage"), requireOwnership('ai_asset'), validate({ body: genericPayloadSchema }), asyncHandler(async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const existing = await getAssetById(tenantId, req.params.id);
        if (!existing) {
        ...
        if (isSeededGlobal(existing)) {
            res.status(403).json({ error: "Cannot delete seeded global assets. These are platform-managed." });
            return;
        }
        ...
        setAuditData(res, { action: "delete", entityType: "ai_asset", entityId: req.params.id, beforeState: existing });
...
router.post("/discover", authenticate, requirePermission("platform.system.admin"), validate({ body: createDiscoverBody }), asyncHandler(async (req, res) => {
    const tenantId = req.tenantId;
    const result = await discoverAndSeedAssets(tenantId);
    setAuditData(res, { action: "create", entityType: "ai_asset_discovery", entityId: tenantId, afterState: result.summary });
    res.json(result);
}));
export default router;
let genericPayloadSchema = z.record(z.unknown());
//# sourceMappingURL=ai-asset-inventory.routes.js.map