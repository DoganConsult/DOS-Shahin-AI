...
export async function getAgrcEngineJobs() {
                ...
                try {
                    ...
                    for (const t of tenants) {
                        try {
                            ...
                            const result = await safeQuery(`SELECT COUNT(*)::int AS stuck FROM "${schema}".agrc_engine_runs WHERE status = 'running' AND started_at < NOW() - INTERVAL '1 hour'`);
                            const stuck = result.rows[0]?.stuck || 0;
                            if (stuck > 0)
                                logger.warn(`[Job] agrc-engine-stuck-run-detector: tenant ${t.tenant_id} — ${stuck} stuck runs`);
                        }
                        catch { }
                    }
                }
                catch (err) {
                    logger.error(`[Job] agrc-engine-stuck-run-detector error: ${toErrorMessage(err)}`);
                }
            },
        },
    ];
}
//# sourceMappingURL=agrc-engine-monitor.job.js.map