...
router.get('/ready', validate({ query: z.record(z.unknown()) }), (_req, res) => {
...
router.get('/agent-status', validate({ query: z.record(z.unknown()) }), authenticate, requirePermission('platform.agent.read'), async (req, res) => {
    ...
    const perfResult = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), dbQuery(`SELECT agent_id,
...
//# sourceMappingURL=reporting-metrics-health.routes.js.map