...
router.get('/role-experience/profile', authenticate, requirePermission('platform.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    ...
    const roles = await safeQuery(`SELECT fr.code, fr.name, fr.description FROM "${schema}".functional_roles fr
     JOIN "${schema}".user_role_assignments ura ON ura.functional_role_id = fr.id
     WHERE ura.user_id = $1 AND ura.is_active = TRUE AND fr.is_active = TRUE`, [user.userId || user.id]);
    const modules = await safeQuery(`SELECT DISTINCT tme.module_code FROM public.tenant_module_entitlements tme
     WHERE tme.tenant_id = $1 AND tme.is_active = TRUE ORDER BY tme.module_code`, [req.tenantId]);
    const recentActivity = await safeQuery(`SELECT permission_code, module_code, decision, created_at
     FROM "${schema}".authz_decision_log
     WHERE user_id = $1 AND decision = 'allow'
     ORDER BY created_at DESC LIMIT 20`, [user.userId || user.id]).catch(() => ({ rows: [] }));
    const frequentModules = new Map();
    ...
    res.json({
        ...
        data: {
            ...
            roles: roles.rows,
            entitledModules: modules.rows.map((r) => r.module_code),
...
router.get('/role-experience/recommendations', authenticate, requirePermission('platform.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const user = req.user;
    const schema = tenantSchema(req.tenantId);
    const roles = await safeQuery(`SELECT fr.code FROM "${schema}".functional_roles fr
     JOIN "${schema}".user_role_assignments ura ON ura.functional_role_id = fr.id
     WHERE ura.user_id = $1 AND ura.is_active = TRUE`, [user.userId || user.id]);
    const roleCodes = roles.rows.map((r) => r.code);
    const pending = await safeQuery(`SELECT entity_type, COUNT(*) AS cnt FROM "${schema}".workflow_steps
     WHERE assignee_id = $1 AND status IN ('pending', 'in_progress')
     GROUP BY entity_type ORDER BY cnt DESC`, [user.userId || user.id]).catch(() => ({ rows: [] }));
...
router.get('/role-experience/shortcuts', authenticate, requirePermission('platform.agent.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const user = req.user;
    const schema = tenantSchema(req.tenantId);
    const roles = await safeQuery(`SELECT fr.code FROM "${schema}".functional_roles fr
     JOIN "${schema}".user_role_assignments ura ON ura.functional_role_id = fr.id
     WHERE ura.user_id = $1 AND ura.is_active = TRUE`, [user.userId || user.id]);
    const roleCodes = new Set(roles.rows.map((r) => r.code));
    const shortcuts = [
...
//# sourceMappingURL=role-experience.routes.js.map