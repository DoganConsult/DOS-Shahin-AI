...
router.get('/gates/registry', authenticate, requirePermission('gate.record.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const result = await safeQuery(`SELECT gate_id, gate_code, gate_type, display_name_en, display_name_ar, description_en,
     ...
     ORDER BY g.gate_type, g.gate_code`, [req.tenantId]);
    res.json({ gates: result.rows, count: result.rows.length });
}));
// GET /gates/registry/:gateCode — Get single gate definition with rules
router.get('/gates/registry/:gateCode', authenticate, requirePermission('gate.record.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const gate = await safeQuery(`SELECT * FROM "${schema}".gate_definitions
     WHERE gate_code = $1 AND tenant_id IN ($2, '00000000-0000-0000-0000-000000000000')
     ORDER BY tenant_id DESC LIMIT 1`, [req.params.gateCode, req.tenantId]);
    if (!gate.rows[0]) {
        res.status(404).json({ error: 'Gate not found' });
        return;
    }
    const rules = await safeQuery(`SELECT * FROM "${schema}".gate_validation_rules
     WHERE gate_id = $1 AND enabled = true ORDER BY execution_order`, [gate.rows[0].gate_id]);
    res.json({ ...gate.rows[0], rules: rules.rows });
}));
// POST /gates/registry — Create custom gate definition
router.post('/gates/registry', authenticate, requirePermission('tenant.config.manage'), validate({ body: createRegistryBody }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const { gate_code, gate_type, display_name_en, display_name_ar, description_en, validation_rules, ai_analysis_enabled, ai_prompt_template, required_permission, severity_on_block, override_allowed, sla_hours } = req.body;
    if (!gate_code || !gate_type || !display_name_en) {
        res.status(400).json({ error: errMsg('MISSING_FIELDS', req) });
        return;
    }
    const result = await safeQuery(`INSERT INTO "${schema}".gate_definitions
     ...
     RETURNING *`, [req.tenantId, gate_code, gate_type || 'custom', display_name_en, display_name_ar || '',
    ...
    emitEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'created', entityType: 'gate_definition', entityId: result.rows[0]?.gate_id || '' }).catch(catchHandler(EC.AGENT_ACTION, {}));
    res.status(201).json(result.rows[0]);
}));
// PUT /gates/registry/:gateCode — Update gate definition
router.put('/gates/registry/:gateCode', authenticate, requirePermission('tenant.config.manage'), validate({ body: updateRegistryBody }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const fields = [];
    const values = [req.params.gateCode, req.tenantId];
    let paramIdx = 3;
    ...
    const result = await safeQuery(`UPDATE "${schema}".gate_definitions SET ${fields.join(', ')}
     WHERE gate_code = $1 AND tenant_id = $2 RETURNING *`, values);
    emitEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: 'updated', entityType: 'gate_definition', entityId: req.params.gateCode }).catch(catchHandler(EC.AGENT_ACTION, {}));
    res.json(result.rows[0] || { error: 'Gate not found' });
}));
// POST /gates/registry/:gateCode/rules — Add validation rule to gate
router.post('/gates/registry/:gateCode/rules', authenticate, requirePermission('tenant.config.manage'), validate({ body: createRulesBody }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const gate = await safeQuery(`SELECT gate_id FROM "${schema}".gate_definitions WHERE gate_code = $1 AND tenant_id = $2`, [req.params.gateCode, req.tenantId]);
    if (!gate.rows[0]) {
        res.status(404).json({ error: 'Gate not found' });
        return;
    }
    ...
    const result = await safeQuery(`INSERT INTO "${schema}".gate_validation_rules
     (gate_id, tenant_id, rule_name, rule_type, rule_config, error_message_en, error_message_ar, severity, execution_order)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`, [gate.rows[0].gate_id, req.tenantId, rule_name, rule_type, JSON.stringify(rule_config),
...
router.post('/gates/override/:overrideId/approve', authenticate, requirePermission('tenant.config.manage'), validate({ body: createApproveBody }), asyncHandler(async (req, res) => {
    ...
    emitEvent({ tenantId: req.tenantId, userId: req.user.userId, module: 'governance', event: approved ? 'approved' : 'rejected', entityType: 'gate_override', entityId: req.params.overrideId }).catch(catchHandler(EC.AGENT_ACTION, {}));
    res.json(result);
}));
// ===============================================================
...
router.get('/gates/log', authenticate, requirePermission('gate.record.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    ...
    let paramIdx = 1;
    if (gateType) {
        conditions.push(`gate_type = $${paramIdx++}`);
        params.push(gateType);
    }
    if (allowed !== undefined) {
        conditions.push(`allowed = $${paramIdx++}`);
    ...
    const result = await safeQuery(`SELECT * FROM "${schema}".enforcement_gate_log ${where}
     ORDER BY created_at DESC LIMIT $${paramIdx} OFFSET $${paramIdx + 1}`, [...params, parseInt(limit) || 50, parseInt(offset) || 0]);
    res.json({ logs: result.rows, count: result.rows.length });
}));
// GET /gates/analytics — AI-powered gate analytics dashboard
router.get('/gates/analytics', authenticate, requirePermission('gate.record.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
...
router.get('/gates/analytics/ai-insights', authenticate, requirePermission('gate.record.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req, res) => {
    const schema = tenantSchema(req.tenantId);
    const days = Math.min(parseInt(req.query.days) || 30, 365);
...
//# sourceMappingURL=gates.routes.js.map