...
// ── Helpers ────────────────────────────────────────────────────────────────
async function notifyAdmins(event, notif) {
    try {
...
async function notifyOwners(event, notif) {
    try {
        if (!shouldSendNotification(event, notif.type))
            return;
        const { createNotification } = await import('../../notification/services/notification.service.js');
...
async function sendOutboundNotifications(tenantId, notif) {
    try {
        ...
        if (channels.email?.enabled && channels.email?.recipients?.length > 0) {
            try {
                ...
                for (const recipient of channels.email.recipients) {
                    await sendEmail(recipient, notif.title, `<h3>${notif.title}</h3><p>${notif.body}</p><p><a href="${notif.link}">View in AGRC-OS</a></p>`).catch(catchHandler(EC.AGENT_ACTION, {}));
                }
            }
...
async function notifyPlatformAdmins(event, notif) {
    try {
        if (!shouldSendNotification(event, notif.type))
            return;
        const { createNotification } = await import('../../notification/services/notification.service.js');
       ...
       LIMIT 10`, [event.tenantId]);
        // Backward-compatible fallback for older schemas lacking is_super_admin
        if (!admins.rows.length) {
            admins = await safeQuery(`SELECT user_id FROM users WHERE tenant_id=$1 AND role IN ('owner', 'admin') LIMIT 10`, [event.tenantId]);
        }
        for (const admin of admins.rows) {
            await createNotification(event.tenantId, { userId: admin.user_id, ...notif }).catch(catchHandler(EC.AGENT_ACTION, {}));
        }
...
function shouldSendNotification(event, type) {
    const now = Date.now();
...
//# sourceMappingURL=agrc-notification-bridge.service.js.map