...
export async function computeMetrics(tenantId, hours = 24) {
    ...
    let cycleCount = 0, avgCycleMs = 0, enforcementRate = 0, lastCycleAt = null;
    let staleControlPct = 0, telemetryIngestionRate = 0;
    let eventCount = 0, criticalEvents = 0;
    // Orchestration cycles
    try {
        const cycleRes = await safeQuery(`SELECT COUNT(*)::int AS cnt, COALESCE(AVG(cycle_ms),0)::int AS avg_ms,
    ...
    // Stale controls
    try {
        const ccmRes = await safeQuery(`SELECT COALESCE(SUM(stale_controls),0)::int AS stale, COALESCE(SUM(controls_evaluated),0)::int AS total
    ...
    // Event bus stats
    try {
        const eventRes = await safeQuery(`SELECT COUNT(*)::int AS total,
    ...
    if (criticalEvents > 5 || staleControlPct > 30)
        healthStatus = 'critical';
    else if (criticalEvents > 0 || staleControlPct > 10 || avgCycleMs > 30000)
...
export async function saveMetricsSnapshot(tenantId) {
    const schema = tenantSchema(tenantId);
    ...
    `);
        await safeQuery(`INSERT INTO "${schema}".agrc_metrics_snapshots
         (cycle_count, avg_cycle_ms, enforcement_rate, stale_control_pct,
          telemetry_ingestion_rate, event_count, critical_events)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`, [metrics.cycleCount, metrics.avgCycleMs, metrics.enforcementRate,
            metrics.staleControlPct, metrics.telemetryIngestionRate,
            metrics.eventCount, metrics.criticalEvents]);
    }
    catch { /* best effort */ }
}
export async function getMetricsHistory(tenantId, limit = 30) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`SELECT * FROM "${schema}".agrc_metrics_snapshots ORDER BY snapshot_at DESC LIMIT $1`, [limit]);
        return result.rows;
    }
    catch {
        return [];
...
export async function purgeOldSnapshots(tenantId, retentionDays = 90) {
    const schema = tenantSchema(tenantId);
    try {
        const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000).toISOString();
        const result = await safeQuery(`DELETE FROM "${schema}".agrc_metrics_snapshots WHERE snapshot_at < $1`, [cutoff]);
...
export async function purgeOldCycleLogs(tenantId, retentionDays = 90) {
    const schema = tenantSchema(tenantId);
    try {
        const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000).toISOString();
        const result = await safeQuery(`DELETE FROM "${schema}".agrc_os_cycle_log WHERE executed_at < $1`, [cutoff]);
        return result.rowCount || 0;
    }
    catch {
        return 0;
...
//# sourceMappingURL=agrc-metrics.service.js.map