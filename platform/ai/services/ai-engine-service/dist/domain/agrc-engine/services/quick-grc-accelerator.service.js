 ...
 */
export async function getAcceleratorProgress(tenantId) {
    const schema = tenantSchema(tenantId);
    try {
        const result = await safeQuery(`SELECT value FROM "${schema}".tenant_config WHERE key = 'quick_accelerator'`, []);
...
function initializeProgress(tenantId) {
    const actions = ACTION_DEFINITIONS.map(def => ({
        ...def,
        status: 'pending',
        completedAt: null,
        error: null,
...
function mergeWithDefinitions(tenantId, stored) {
    ...
    const actions = ACTION_DEFINITIONS.map(def => {
        ...
        if (existing)
            return { ...def, status: existing.status, completedAt: existing.completedAt, error: existing.error };
        return { ...def, status: 'pending', completedAt: null, error: null };
    });
...
async function saveProgress(tenantId, progress) {
    const schema = tenantSchema(tenantId);
    ...
    await safeQuery(`INSERT INTO "${schema}".tenant_config (key, value)
 ...
 */
export async function executeAction(tenantId, userId, actionId) {
    const progress = await getAcceleratorProgress(tenantId);
    if (!progress.startedAt)
        progress.startedAt = new Date().toISOString();
    const action = progress.actions.find(a => a.id === actionId);
    if (!action)
        return { actionId, success: false, message: 'Unknown action ID' };
    if (action.status === 'completed')
        return { actionId, success: true, message: 'Already completed' };
 ...
 */
export async function skipAction(tenantId, actionId) {
    const progress = await getAcceleratorProgress(tenantId);
    const action = progress.actions.find(a => a.id === actionId);
    if (!action)
        return { actionId, success: false, message: 'Unknown action ID' };
 ...
 */
export async function resetProgress(tenantId) {
    const progress = initializeProgress(tenantId);
    await saveProgress(tenantId, progress);
    return progress;
}
// ── Action Logic Dispatcher ────────────────────────────────────────────────
async function runActionLogic(tenantId, userId, actionId) {
    switch (actionId) {
...
async function executeOnboardingWizard(tenantId, _userId) {
...
async function executeFrameworkMapping(tenantId, _userId) {
    ...
    const sectorIds = profileObj.industrySector
        ? sectors.filter((s) => s.name_en?.toLowerCase().includes(String(profileObj.industrySector).toLowerCase())).map((s) => s.sector_id)
    ...
    await safeQuery(`INSERT INTO "${schema}".tenant_config (key, value)
     ...
     ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [JSON.stringify({ sectorIds, regulations, mappedAt: new Date().toISOString() })]);
    return {
        actionId: 'framework-mapping',
        success: true,
        message: `Mapped ${(regulations.mandatory)?.length || 0} mandatory and ${(regulations.recommended)?.length || 0} recommended frameworks`,
        details: { sectorIds, mandatoryCount: (regulations.mandatory)?.length || 0, recommendedCount: (regulations.recommended)?.length || 0 },
    };
}
async function executeSeedControls(tenantId, userId) {
    const schema = tenantSchema(tenantId);
    ...
    const configResult = await safeQuery(`SELECT value FROM "${schema}".tenant_config WHERE key = 'mapped_frameworks'`, []);
    ...
    if (frameworkData?.regulations?.mandatory) {
        for (const fw of frameworkData.regulations.mandatory) {
            const nodes = await safeQuery(`SELECT node_id, code, title_en, title_ar, level FROM instrument_structure
         WHERE instrument_id = $1 AND level = 4 LIMIT 50`, [fw.instrumentId || fw.instrument_id]);
            for (const node of nodes.rows) {
                try {
                    await safeQuery(`INSERT INTO "${schema}".controls (control_id, title, description, framework_id, status, created_by)
             ...
             WHERE (controls.title, controls.description) IS DISTINCT FROM (EXCLUDED.title, EXCLUDED.description)`, [node.title_en || node.code, `Control from ${node.code}`, fw.instrumentId || fw.instrument_id, userId]);
                    seededCount++;
                }
...
async function executeGenerateRaci(tenantId, _userId) {
    ...
    const team = recommendTeamStructure(companyProfile);
    const teamObj = (team ?? {});
    const roles = Array.isArray(teamObj.roles) ? teamObj.roles : [];
    const raci = generateRaciMatrix(companyProfile, roles);
    // Persist RACI to tenant config
    const schema = tenantSchema(tenantId);
    await safeQuery(`INSERT INTO "${schema}".tenant_config (key, value)
     VALUES ('raci_matrix', $1)
     ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`, [JSON.stringify({ matrix: raci, team, generatedAt: new Date().toISOString() })]);
...
async function executeEnableNudges(tenantId, userId) {
    const schema = tenantSchema(tenantId);
    ...
    try {
        ...
        if (roadmap) {
            const nudges = generateNudges(roadmap, { currentModule: 'dashboard', userId, isFirstVisit: true }, new Date());
            if (nudges.length > 0) {
...
async function executeGapAssessment(tenantId, userId) {
...
async function executeJourneyRoadmap(tenantId, userId) {
    ...
    return {
        ...
        details: { phases: phases.length, totalTasks: phases.reduce((sum, p) => sum + (Array.isArray(p.tasks) ? p.tasks.length : 0), 0) },
    };
}
async function executeAutoEval(tenantId, userId) {
    const schema = tenantSchema(tenantId);
...
async function executeContextualAI(tenantId, userId) {
    const schema = tenantSchema(tenantId);
...
//# sourceMappingURL=quick-grc-accelerator.service.js.map