...
export async function list(tenantId, query_) {
    ...
    if (query_?.engineType) {
        conds.push(`engine_type = $${idx++}`);
        params.push(query_.engineType);
    }
    if (query_?.category) {
    ...
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".agrc_engine_runs WHERE ${conds.join(' AND ')} ORDER BY created_at DESC LIMIT ${limit}`, params).catch(() => ({ rows: [] }));
    return rows.map(mapRow);
}
export async function getById(tenantId, id) {
    const schema = tenantSchema(tenantId);
    const { rows } = await safeQuery(`SELECT * FROM "${schema}".agrc_engine_runs WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [id]).catch(() => ({ rows: [] }));
    return rows.length > 0 ? mapRow(rows[0]) : null;
}
export async function create(tenantId, body, userId) {
    const schema = tenantSchema(tenantId);
    const uid = userId ?? SYSTEM_JOB_ACTOR;
    const { rows } = await safeQuery(`INSERT INTO "${schema}".agrc_engine_runs
     ...
     VALUES ($1,$2,$3,$4,'queued',$5,NOW(),NOW()) RETURNING *`, [
        ...
        uid,
    ]).catch((e) => { logger.warn('[agrc-engine] create query failed', { error: e.message }); return { rows: [] }; });
    if (rows.length === 0)
        return { id: 'pending', status: 'failed', tenantId };
    ...
    }).catch((e) => logger.warn('[agrc-engine] audit failed', { error: e.message }));
    emitEvent({
        tenantId, userId: uid, module: 'agrc-engine', event: 'run.created',
        entityType: 'agrc_engine_run', entityId: runId,
        data: { engineType: record.engineType, category: record.category },
    }).catch((e) => logger.warn('[agrc-engine] event emission failed', { error: e.message }));
...
export async function update(tenantId, id, body, userId) {
    const schema = tenantSchema(tenantId);
    const uid = userId ?? SYSTEM_JOB_ACTOR;
    const { rows: beforeRows } = await safeQuery(`SELECT * FROM "${schema}".agrc_engine_runs WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [id]).catch(() => ({ rows: [] }));
    ...
    if (body.completedAt) {
        ...
        params.push(body.completedAt);
    }
    const { rows } = await safeQuery(`UPDATE "${schema}".agrc_engine_runs SET ${sets.join(', ')} WHERE id = $1 AND deleted_at IS NULL RETURNING *`, params).catch(() => ({ rows: [] }));
    if (rows.length === 0)
        return null;
    const record = mapRow(rows[0]);
    recordAudit({
...
async function enrichWithAI(tenantId, runId, _record) {
    try {
        ...
        if (langGraphEnabled) {
            ...
            const schema = tenantSchema(tenantId);
            await safeQuery(`UPDATE "${schema}".agrc_engine_runs
         SET status = 'completed', results = $2, completed_at = NOW(), updated_at = NOW()
         WHERE id = $1`, [runId, JSON.stringify({
        ...
        logger.warn('[agrc-engine] AI enrichment failed — graceful degradation', {
            tenantId, runId, error: err.message,
        });
    }
}
async function publishHealthSnapshot(tenantId, runId, _record) {
    try {
        const schema = tenantSchema(tenantId);
        ...
        emitEvent({
            ...
            data: snapshot,
        }).catch((e) => logger.warn('[agrc-engine] health snapshot event failed', { error: e.message }));
        logger.info('[agrc-engine] health snapshot published', { tenantId, runId, snapshot });
    }
    catch (err) {
        logger.warn('[agrc-engine] health snapshot generation failed', { tenantId, error: err.message });
    }
}
function mapRow(r) {
    return {
