    ...
    async run(ctx) {
      ...
      `);
        ...
        for (const row of stale.rows) {
            ...
            const canCreate = await canRunDedupAction(ctx.schema, dedupKey, 'control', row.control_id, 'create_evidence_task', 24);
            if (!canCreate)
                continue;
          ...
          `, [ctx.tenantId, row.workspace_id ?? null, row.control_id]), { operation: 'insert evidence_tasks' });
                tasksCreated++;
            }
            await swallowNull(EC.FALLBACK_QUERY, query(`
        ...
        `, [row.control_id, JSON.stringify({ title: row.title, latestExpiry: row.latest_expiry, runId: ctx.runId })]), { operation: 'insert agrc_event_log' });
            if (autoNotify && row.owner) {
                await createNotification(ctx.schema, String(row.owner), 'control_evidence_stale', 'Evidence expired or missing', `Control "${row.title}" needs fresh evidence.`, `/controls/${row.control_id}`);
      ...
      `, [ctx.runId, stale.rows.length, stale.rows.length]), { operation: 'insert ccm_cycle_log' });
        return {
...
//# sourceMappingURL=control-monitor.worker.js.map