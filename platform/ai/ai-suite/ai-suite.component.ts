// Shahin GRC — AI Suite  |  GRC Lifecycle: IMPROVE
...
import { AISquadComponent } from '../ai-squad/ai-squad.component';
import { AIQueueComponent } from '../ai-queue/ai-queue.component';
...
const TABS: HubTab[] = [
  { key: 'hub',     labelEn: 'AI Hub',   labelAr: 'مركز الذكاء الاصطناعي', icon: 'pi-microchip-ai' },
  { key: 'squad',   labelEn: 'AI Squad', labelAr: 'فريق الذكاء الاصطناعي', icon: 'pi-users' },
  { key: 'queue',   labelEn: 'AI Queue', labelAr: 'قائمة الانتظار',         icon: 'pi-list' },
  { key: 'copilot', labelEn: 'Copilot',  labelAr: 'المساعد الذكي',          icon: 'pi-comments' },
];
...
@Component({
  ...
  selector: 'app-ai-suite', standalone: true,
  imports: [CommonModule, RouterModule, AIHubComponent, AISquadComponent, AIQueueComponent, CopilotComponent, AgentBadgeComponent],
  styles: [HUB_STYLES],
  template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#e0e7ff"><i class="pi pi-microchip-ai hub-icon" style="color:#4338ca"></i></div>
          <div>
            ...
            <h1>{{ i18n.currentLang()==='ar' ? 'مجموعة الذكاء الاصطناعي' : 'AI Suite' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'مركز الذكاء الاصطناعي، الفرق، قائمة الانتظار، المساعد — أتمتة GRC ذكية.' : 'AI Hub, Squad, Queue, Copilot — intelligent GRC automation.' }}</p>
          </div>
        </div>
      ...
      <div class="hub-tab-content">
        @if (activeTab()==='hub')     { <app-ai-hub /> }
        @if (activeTab()==='squad')   { <app-ai-squad /> }
        @if (activeTab()==='queue')   { <app-ai-queue /> }
        @if (activeTab()==='copilot') { <app-copilot /> }
      </div>
    </div>`
})
export class AISuiteComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  i18n = inject(I18nService);
  private route = inject(ActivatedRoute);
  tabs = TABS;
  readonly agentId = 'A01';
  activeTab = signal<string>('hub');
  ngOnInit(): void {
