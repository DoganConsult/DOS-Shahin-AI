...
**Wave 1 — Slice A (Phase-1 module seed consolidation):** inventory + canonical seed pack + read-only `dos.*` cooperate audit — see **[phase-1-consolidation-audit.md](./phase-1-consolidation-audit.md)**. Publication gates / `pnpm module:publish` ×4 / tenant DNA smoke remain **Slice B** (deferred per that report).

| Milestone | Status | Started | Closed | Notes |
|-----------|--------|---------|--------|-------|
| Wave 1 Slice A | OPEN | 2026-05-04 | — | Phase-1 consolidation audit artifact + seed inventory + canonical `module_complete_direct_seed_pack/` — **[phase-1-consolidation-audit.md](./phase-1-consolidation-audit.md)**; **freeze: pending reviewer sign-off**. Slice B (publish ×4, VERIFY cleanup, gates, MD-only JSON) deferred. |
...
| M7 D2 | OPEN | 2026-05-04 | — | **Provisioning orchestrator + Temporal consumer for `dos_master.provisioning_job`.** Target: `services/provisioning-service`, worker loop (`SELECT … FOR UPDATE SKIP LOCKED` or equivalent), persist **`temporal_workflow_id`**, populate **`provisioning_step`** / compensation rows per DDS Master §2 item 7 + Phase 2 guards (`provisioning-temporal-workflow-present`, `provisioning-compensation-chain-complete`). Today: **`services/provisioning-service` absent**, **no in-repo consumer** updates jobs past `queued`; **`signup-repo.completeAttempt`** inline comment defers Temporal kickoff here. **Gateway gap:** `services/gateway/src/server.ts` does **not** proxy `/api/public/signup` (allocation-only); Shahin **OIDC register/login** uses **`auth-service` → `tenant-service POST /register`** for real **`dos.tenants`** — see `platform/docs/dos-master/self-registration-product-flow.md`. |
