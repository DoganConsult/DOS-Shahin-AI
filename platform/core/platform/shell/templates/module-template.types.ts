...
// ─── Canonical 31 Page Archetype Names ──────────────────────────────────────
...
// Universal types consumed by the 31 archetypes when they expose mission
...
// Sanity guard — every archetype in the registry must have an enhancement profile.
export function assertEnhancementMatrixCoverage(): void {
  const profiled = new Set(ARCHETYPE_ENHANCEMENT_MATRIX.map(p => p.archetype));
  const missing: string[] = [];
  for (const e of ARCHETYPE_REGISTRY) {
    if (!profiled.has(e.archetype)) missing.push(e.archetype);
  }
  if (missing.length) {
    throw new Error(`[archetype-enhancement-matrix] missing profiles: ${missing.join(', ')}`);
  }
