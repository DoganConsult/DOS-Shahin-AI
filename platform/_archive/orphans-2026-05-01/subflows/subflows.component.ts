...
export class SubflowsComponent implements OnInit {
  ...
  form: Record<string, unknown> = { name: '', code: '', description: '', category: 'subflow', status: 'draft' };
  statusOptions = [
    ...
    this.apiclientSvc.get('/workflows/templates').subscribe({
      next: (res: Record<string, unknown>) => {
        ...
        this.items = this.allItems.filter(
          (t: Record<string, unknown>) => t.type === 'subflow' || t.category === 'subflow' || !t.type && !t.category
        );
        this.filterItems();
        this.loaded = true;
      },
