...
beforeAll(async () => {
  const mod = await import('./integrations.module');
  manifest = mod.INTEGRATIONS_MANIFEST ?? mod.default?.manifest;
});

describe('Integrations Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('integrations'); });
