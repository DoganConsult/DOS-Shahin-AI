...
export async function publish(
  ...
  if (!INTEGRATIONS_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${INTEGRATIONS_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: INTEGRATIONS_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${INTEGRATIONS_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(INTEGRATIONS_EVENT_CONTRACT.published);
}
