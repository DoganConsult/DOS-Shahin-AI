export const INTEGRATIONS_AI_CONFIG = {
  moduleCode: 'integrations' as const,
  enabled: true,

  allowedActions: [
    'integrations.connector.draft',
    'integrations.connector.recommend',
    'integrations.connector.summarize',
    'integrations.connector.classify',
    'integrations.connector.score',
    'integrations.connector.analyze',
    'integrations.connector.generate_report',
  ],

  blockedActions: [
    'integrations.connector.delete',
    'integrations.connector.approve',
    'integrations.connector.reject',
    'integrations.connector.override',
    'integrations.connector.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['integrations.connector.approve', 'integrations.connector.reject', 'integrations.connector.delete'],
    requiresHumanReview: ['integrations.connector.classify', 'integrations.connector.score'],
    autoExecutable: ['integrations.connector.draft', 'integrations.connector.summarize', 'integrations.connector.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'integrations_system_prompt_v1',
  },
...
export function isIntegrationsAiActionAllowed(action: string): boolean {
  return (INTEGRATIONS_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isIntegrationsAiActionBlocked(action: string): boolean {
  return (INTEGRATIONS_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresIntegrationsHumanApproval(action: string): boolean {
  return (INTEGRATIONS_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  if (!r) return { summary: 'Integration entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Integration"} (\${r.integration_type ?? "api"}). Health: \${r.health_status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".integrations ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".integrations ${where} GROUP BY status`, params);
