...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getFailedConnections should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getFailedConnections('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getFailedConnections should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getFailedConnections('t1');
      expect(result).toEqual([]);
    });

    it('getStaleIntegrations should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getStaleIntegrations('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getStaleIntegrations should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getStaleIntegrations('t1');
      expect(result).toEqual([]);
    });
