import type { Connector, ConnectorCreateInput } from '../types/integrations.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Connector {
  return {
    connector_id: row.connector_id ?? row.id,
...
export function toEntityList(rows: GenericRow[]): Connector[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): ConnectorCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Connector): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Connector[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
