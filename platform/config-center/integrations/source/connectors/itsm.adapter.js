...
exports.ITSMAdapter = void 0;
const base_adapter_1 = require("./base.adapter");
class ITSMAdapter extends base_adapter_1.BaseAdapter {
    async authenticate(config) {
        ...
        if (platform === 'servicenow' && username && password) {
            ...
            if (!response.ok)
                throw new Error(`ITSM ServiceNow auth failed: ${response.status}`);
            this.token = {
    ...
    async extract(config, query) {
        ...
        if (platform === 'servicenow') {
            const table = query?.table || 'incident';
        ...
        if (!response.ok)
            throw new Error(`ITSM extract failed: ${response.status}`);
...
exports.ITSMAdapter = ITSMAdapter;
//# sourceMappingURL=itsm.adapter.js.map