...
export async function publish(
  ...
  if (!RECORDS_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${RECORDS_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: RECORDS_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${RECORDS_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(RECORDS_EVENT_CONTRACT.published);
}
