...
export async function runDiagnostics(tenantId: string): Promise<DiagnosticsResult> {
  ...
  const { rows: staleRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".records WHERE updated_at < NOW() - INTERVAL '90 days'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
  ...
  const { rows: auditRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".audit_trail WHERE module = 'records' AND created_at > NOW() - INTERVAL '24 hours'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
