export const RECORDS_AI_CONFIG = {
  moduleCode: 'records' as const,
  enabled: true,
...
export function isRecordsAiActionAllowed(action: string): boolean {
  return (RECORDS_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isRecordsAiActionBlocked(action: string): boolean {
  return (RECORDS_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresRecordsHumanApproval(action: string): boolean {
  return (RECORDS_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".records WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Records entity not found.', keyPoints: [], confidence: 0 };
  return {
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".records ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".records ${where} GROUP BY status`, params);
