...
beforeAll(async () => {
  const mod = await import('./records.module');
  manifest = mod.RECORDS_MANIFEST ?? mod.default?.manifest;
});

describe('Records Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('records'); });
