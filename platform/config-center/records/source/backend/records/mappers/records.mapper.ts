...
export function toEntity(row: GenericRow): Record_ {
  return {
...
export function toEntityList(rows: GenericRow[]): Record_[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): RecordCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as RecordCreateInput;
}

export function toApiResponse(entity: Record_): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Record_[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
