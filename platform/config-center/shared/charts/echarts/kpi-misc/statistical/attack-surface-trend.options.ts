...
export function buildAttackSurfaceTrendOptions(points: AttackSurfacePoint[]): EChartsOption {
  ...
  return {
    ...
    series: categories.map((cat, i) => {
      ...
      return {
        ...
        type: 'line' as const,
        data: allDates.map(d => {
          const pt = points.find(p => p.date === d && p.category === cat);
        ...
        itemStyle: { color },
        areaStyle: {
          color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: color + '30' }, { offset: 1, color: color + '05' }] },
        },
      };
    }),
  } as EChartsOption;
