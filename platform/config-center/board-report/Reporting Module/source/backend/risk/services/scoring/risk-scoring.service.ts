...
import type { RiskLevel } from "@shahin/shared-risk-types";
import { query as _query, safeQuery, tenantSchema } from '../../ports/database.port';
import { createNotification } from "../../../notification/services/notification.service";
import { orchestratedAssessRisk as assessRisk } from "../../../ai/services/orchestration/ai-os-orchestrator.service";
...
export function computeCompositeScore(
  ...
  if (model.formula === "multiplicative") {
    ...
    for (const ds of dimensionScores) {
      ...
      const max = dim.scale?.max ?? 100;
      const range = max - min;
      const normalized = range > 0
        ? Math.max(0, Math.min(1, (ds.score - min) / range))
        : 0;
      product *= normalized;
      matched++;
    }
...
export async function updateRiskModel(
  ...
  const existing = await safeQuery(
    `SELECT * FROM "${schema}".risk_scoring_models WHERE model_id = $1 LIMIT 1`,
    [modelId],
  );
  const currentRow = getFirstRow(existing) as any;
  if (!currentRow) {
    throw new Error(`Risk scoring model not found: ${modelId}`);
  }

  const currentModel = rowToModel(currentRow);
  const nameEn = patch.nameEn ?? currentRow.name_en ?? '';
  const nameAr = patch.nameAr ?? currentRow.name_ar ?? '';

  const nextDimensions = patch.dimensions ?? currentModel.dimensions;
  const nextThresholds = patch.thresholds ?? currentModel.thresholds;
  const nextFormula = patch.formula ?? currentModel.formula;
  const nextZoneDefinitions = patch.zoneDefinitions ?? currentRow.zone_definitions ?? {};

  const updated = await safeQuery(
    `UPDATE "${schema}".risk_scoring_models
     SET name_en = $2,
         name_ar = $3,
         dimensions = $4,
         thresholds = $5,
    ...
    [
      ...
      nextFormula,
      typeof nextZoneDefinitions === 'string' ? nextZoneDefinitions : JSON.stringify(nextZoneDefinitions),
    ],
  );

  const row = getFirstRow(updated) as any;
  return { ...rowToModel(row), nameEn: row?.name_en ?? nameEn, nameAr: row?.name_ar ?? nameAr };
}
...
export async function scoreRisk(
  ...
  const modelResult = await safeQuery(
    `SELECT * FROM "${schema}".risk_scoring_models WHERE model_id = $1 LIMIT 1`,
    [modelId],
  );
  const modelRow = getFirstRow(modelResult) as any;
  const model = modelRow ? rowToModel(modelRow) : { modelId, dimensions: [], thresholds: { low: 0, medium: 25, high: 50, critical: 75 }, formula: 'weighted' as RiskFormula };

  const compositeScore = computeCompositeScore(dimensionScores, model);
  const zone = determineZone(compositeScore, model.thresholds);
  ...
  const prevRow = getFirstRow(prev) as any;
  const previousScore = prevRow?.composite_score != null ? parseFloat(String(prevRow.composite_score)) : compositeScore;

  const crossing = checkThresholdCrossing(previousScore, compositeScore, model.thresholds);

  await safeQuery(
    `INSERT INTO "${schema}".risk_score_history (risk_id, composite_score, zone, scored_at)
     VALUES ($1, $2, $3, NOW())`,
    [riskId, compositeScore, zone],
  );

  if (crossing.crossed && (crossing.toZone === 'high' || crossing.toZone === 'critical')) {
    await createNotification(tenantId, {
      type: 'risk.threshold.crossed',
      riskId,
      modelId,
      fromZone: crossing.fromZone,
      toZone: crossing.toZone,
  ...
  return { compositeScore, zone, thresholdCrossing: crossing.crossed ? crossing : null };
}
...
export async function predictRiskTrajectory(
  ...
  const resolvedThresholds = await (async (): Promise<{ critical: number; high: number; medium: number; low: number }> => {
    if (!modelId) return { low: 0, medium: 25, high: 50, critical: 75 };
    const modelResult = await safeQuery(
      `SELECT * FROM "${schema}".risk_scoring_models WHERE model_id = $1 LIMIT 1`,
      [modelId],
    );
    const modelRow = getFirstRow(modelResult) as any;
    if (!modelRow) return { low: 0, medium: 25, high: 50, critical: 75 };
    return rowToModel(modelRow).thresholds as any;
  })();

  const points = history.rows.map((r: any) => ({
    scoredAt: r.scored_at instanceof Date ? r.scored_at : new Date(String(r.scored_at)),
    score: parseFloat(String(r.composite_score ?? 0)),
  }));

  const last = points.length > 0 ? points[points.length - 1] : { scoredAt: new Date(), score: 0 };
  const first = points.length > 0 ? points[0] : last;

  const days = Math.max(1, Math.round((last.scoredAt.getTime() - first.scoredAt.getTime()) / (24 * 60 * 60 * 1000)));
  const slopePerDay = (last.score - first.score) / days;

  const trajectory: Array<{ date: string; score: number; zone: RiskZone }> = [];
  ...
  for (let i = 0; i <= lookaheadDays; i++) {
    ...
    const rawScore = last.score + slopePerDay * i;
    const score = Math.max(0, Math.min(100, Math.round(rawScore * 100) / 100));
    trajectory.push({
      date: d.toISOString().split('T')[0],
      score,
      zone: determineZone(score, resolvedThresholds),
    });
  }

  const currentScore = Math.max(0, Math.min(100, Math.round(last.score * 100) / 100));
  const currentZone = determineZone(currentScore, resolvedThresholds);

  const projectedScore = trajectory.length > 0 ? trajectory[trajectory.length - 1].score : currentScore;
  const projectedZone = determineZone(projectedScore, resolvedThresholds);

  const high = projectedScore >= resolvedThresholds.high
    ? Math.max(0, Math.min(1, (projectedScore - resolvedThresholds.high) / 25))
    : 0;
  const critical = projectedScore >= resolvedThresholds.critical
    ? Math.max(0, Math.min(1, (projectedScore - resolvedThresholds.critical) / 25))
    : 0;
  return {
    currentScore,
    currentZone,
    projectedScore,
    projectedZone,
    breachProbability: { high, critical },
    trajectory,
