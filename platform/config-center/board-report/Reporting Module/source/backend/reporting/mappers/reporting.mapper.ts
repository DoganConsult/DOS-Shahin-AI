import type { Report, ReportCreateInput } from '@dos/types/reporting';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Report {
...
export function toEntityList(rows: GenericRow[]): Report[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): ReportCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Report): Record<string, unknown> {

  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Report[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
