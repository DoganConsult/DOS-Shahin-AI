export const REPORTING_AI_CONFIG = {
  moduleCode: 'reporting' as const,
  enabled: true,

  allowedActions: [
    'reporting.document.draft',
    'reporting.document.recommend',
    'reporting.document.summarize',
    'reporting.document.classify',
    'reporting.document.score',
    'reporting.document.analyze',
    'reporting.document.generate_report',
  ],

  blockedActions: [
    'reporting.document.delete',
    'reporting.document.approve',
    'reporting.document.reject',
    'reporting.document.override',
    'reporting.document.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['reporting.document.approve', 'reporting.document.reject', 'reporting.document.delete'],
    requiresHumanReview: ['reporting.document.classify', 'reporting.document.score'],
    autoExecutable: ['reporting.document.draft', 'reporting.document.summarize', 'reporting.document.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'reporting_system_prompt_v1',
  },
...
export function isReportingAiActionAllowed(action: string): boolean {
  return (REPORTING_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isReportingAiActionBlocked(action: string): boolean {
  return (REPORTING_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresReportingHumanApproval(action: string): boolean {
  return (REPORTING_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".generated_reports WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Reporting entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Report"} (\${r.report_type ?? "custom"}). Status: \${r.status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".generated_reports ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".generated_reports ${where} GROUP BY status`, params);
