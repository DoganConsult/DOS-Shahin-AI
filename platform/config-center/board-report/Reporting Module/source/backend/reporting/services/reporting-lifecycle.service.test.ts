...
import {
  ...
  getFailedReports,
  getStaleReports,
} from './reporting-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getFailedReports should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getFailedReports('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getFailedReports should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getFailedReports('t1');
      expect(result).toEqual([]);
    });

    it('getStaleReports should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getStaleReports('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getStaleReports should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getStaleReports('t1');
      expect(result).toEqual([]);
    });
