...
beforeAll(async () => {
  const mod = await import('./reporting.module');
  manifest = mod.REPORTING_MANIFEST ?? mod.default?.manifest;
});

describe('Reporting Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('reporting'); });
