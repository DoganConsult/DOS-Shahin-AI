...
export class ReportBuilderPageComponent implements OnInit {
  ...
  generate() {
    const body: Record<string, unknown> = { template_id: this.selectedTemplate, format: this.format };
    ...
    this.apiclientSvc.post('/report-ext/generate', body).subscribe({
      next: (r: Record<string, unknown>) => this.reports = [r, ...this.reports]
    });
  }
  ...
  download(r: Record<string, unknown>) {
