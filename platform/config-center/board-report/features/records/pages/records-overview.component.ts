...
@Component({
      ...
      <div class="page-header">
        <div class="title-row">
          ...
          <div>
            ...
            <p class="subtitle">Retention policies, lifecycle tracking, and disposition</p>
          </div>
        </div>
        ...
        <div class="main-content">
          ...
          <div *ngIf="!loading() && items().length > 0" class="data-list">
            <div class="list-item" *ngFor="let item of items()">
              <div class="item-icon"><i class="pi pi-folder-open"></i></div>
  ...
  styles: [`
    ...
    .item-icon { width: 36px; height: 36px; border-radius: var(--radius); background: var(--teal-50, #f0fdfa); display: flex; align-items: center; justify-content: center; }
    .item-icon i { font-size: var(--font-size-md); color: var(--teal-500); }
...
export class RecordsOverviewComponent implements OnInit {
  ...
  moduleKitConfig = computed<ModuleOverviewKitConfig>(() => ({
    ...
    slaHours: 720,
    transitions: this.recordsTransitions,
    currentStatus: 'active',
    agents: this.recordsAgents,
  ...
  refresh(): void {
    ...
    this.http.get<{ data?: unknown[] }>(`${environment.apiUrl}/records`).pipe(takeUntilDestroyed(this.destroyRef), catchError(() => of({ data: [] }))).subscribe(res => { const arr = Array.isArray(res) ? res : (res?.data || []); this.items.set(arr.map((r: Record<string, unknown>) => ({ title: String(r['title'] || r['name'] || 'Record'), status: String(r['status'] || 'active'), date: String(r['created_at'] || r['updatedAt'] || new Date().toISOString()), severity: String(r['severity'] || r['priority'] || 'info'), }))); this.stats.set([{ value: String(arr.length), label: 'Total' }, { value: String(arr.filter((r: Record<string, unknown>) => r['status'] === 'active').length), label: 'Active' }, { value: String(arr.filter((r: Record<string, unknown>) => r['status'] === 'archived').length), label: 'Archived' }, { value: String(arr.filter((r: Record<string, unknown>) => r['status'] === 'on_hold').length), label: 'On Hold' }]); this.loading.set(false); });
  }
}
