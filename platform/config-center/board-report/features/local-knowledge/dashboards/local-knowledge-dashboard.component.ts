...
interface LocalKnowledgeDashboardData {
...
@Component({
  ...
  selector: 'app-local-knowledge-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('localKnowledge.dashboard') }}</h2>
      @if (loading()) {
...
export class LocalKnowledgeModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<LocalKnowledgeDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: LocalKnowledgeDashboardData }>(`/api/local-knowledge/dashboard`)
