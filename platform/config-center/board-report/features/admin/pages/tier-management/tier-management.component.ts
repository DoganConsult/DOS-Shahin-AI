...
export class TierManagementComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiclientSvc.get('/tier').subscribe({
      next: (d: Record<string, any>) => { this.tiers = d.tiers || []; this.currentTier = d.currentTier || 'starter'; this.loading = false; this.cdr.markForCheck(); },
      error: () => { this.loading = false; this.cdr.markForCheck(); }
    });
    this.apiclientSvc.get('/tier/usage').subscribe({ next: (d: Record<string, any>) => { this.usage = d; } });
  }
