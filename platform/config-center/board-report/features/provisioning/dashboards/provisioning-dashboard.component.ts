...
interface ProvisioningDashboardData {
...
@Component({
  ...
  selector: 'app-provisioning-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('provisioning.dashboard') }}</h2>
      @if (loading()) {
...
export class ProvisioningModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<ProvisioningDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: ProvisioningDashboardData }>(`/api/provisioning/dashboard`)
