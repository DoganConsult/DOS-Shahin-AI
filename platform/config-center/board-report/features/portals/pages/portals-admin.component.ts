...
export class PortalsAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Array<{ key: string; value: string }> }>(`/api/portals/admin/settings`).pipe(
    ...
    this.http.get<{ data: { overallHealth?: string } }>(`/api/portals/admin/diagnostics`).pipe(
      takeUntilDestroyed(this.destroyRef),
