  ...
  template: `
    <div class="detail-shell" [dir]="i18n.direction()">
      <header class="detail-header">
        ...
        <h2>{{ entity()?.title || 'Portals Detail' }}</h2>
      </header>
...
export class PortalsDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.http.get<Record<string, unknown>>(`/api/portals/${id}`).pipe(
