...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('asset.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalAssets }}</div><div class="kpi-label">Total Assets</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.active }}</div><div class="kpi-label">Active</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.retired }}</div><div class="kpi-label">Retired</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.unclassified }}</div><div class="kpi-label">Unclassified</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgAge }}</div><div class="kpi-label">Avg Age (days)</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.complianceRate }}</div><div class="kpi-label">Compliance %</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.pendingReview }}</div><div class="kpi-label">Pending Review</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Asset Type</h3>
              @for (item of data()!.breakdownA; track item.assetType) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Classification</h3>
              @for (item of data()!.breakdownB; track item.classification) {
                <div class="breakdown-row">
...
export class AssetDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/asset/dashboard')
