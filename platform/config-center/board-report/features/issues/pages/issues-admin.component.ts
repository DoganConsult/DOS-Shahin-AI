...
export class IssuesAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Array<{ key: string; value: string }> }>(`/api/issues/admin/settings`).pipe(
    ...
    this.http.get<{ data: { overallHealth?: string } }>(`/api/issues/admin/diagnostics`).pipe(
      takeUntilDestroyed(this.destroyRef),
