...
@Component({
  ...
  template: `
    <div class="page-shell">
      <div class="page-header">
        <div class="title-row">
          <div class="icon-wrap" style="background: var(--red-50, #fef2f2)">
            <i class="pi pi-exclamation-circle" style="color: var(--red-500)"></i>
          </div>
          <div>
            <h1>Issues Management</h1>
            <p class="subtitle">Track, triage and resolve operational issues</p>
          </div>
        </div>
        ...
        <div class="main-content">
          ...
          <app-empty-state
            *ngIf="!loading() && items().length === 0"
            ...
            description="Issues will appear here once created."
            actionLabel="Get Started"
            (action)="refresh()" />
          <div *ngIf="!loading() && items().length > 0" class="data-list">
            <div class="list-item" *ngFor="let item of items()">
              <div class="item-icon"><i class="pi pi-exclamation-circle"></i></div>
  ...
  styles: [`
    ...
    .item-icon { width: 36px; height: 36px; border-radius: var(--radius); background: var(--red-50, #fef2f2); display: flex; align-items: center; justify-content: center; }
    .item-icon i { font-size: var(--font-size-md); color: var(--red-500); }
...
export class IssuesOverviewComponent implements OnInit {
  ...
  readonly issuesAgents: AgentInfo[] = [];

  readonly issuesTransitions = [
    { from: 'open', to: 'triaged' },
    { from: 'triaged', to: 'investigating' },
    { from: 'investigating', to: 'in_progress' },
    { from: 'investigating', to: 'triaged' },
    { from: 'in_progress', to: 'pending_verification' },
    { from: 'in_progress', to: 'investigating' },
    { from: 'pending_verification', to: 'resolved', requiresApproval: true },
  ...
  moduleKitConfig = computed<ModuleOverviewKitConfig>(() => ({
    ...
    slaHours: 72,
    transitions: this.issuesTransitions,
    currentStatus: 'open',
    agents: this.issuesAgents,
  ...
  refresh(): void {
    ...
    this.http.get<{ data?: unknown[] }>(`${environment.apiUrl}/issues`).pipe(takeUntilDestroyed(this.destroyRef), catchError(() => of({ data: [] }))).subscribe(res => { const arr = Array.isArray(res) ? res : (res?.data || []); this.items.set(arr.map((r: Record<string, unknown>) => ({ title: String(r['title'] || r['name'] || 'Issue'), status: String(r['status'] || 'open'), date: String(r['created_at'] || r['updatedAt'] || new Date().toISOString()), severity: String(r['severity'] || r['priority'] || 'info'), }))); this.stats.set([{ value: String(arr.length), label: 'Total' }, { value: String(arr.filter((r: Record<string, unknown>) => r['status'] === 'open').length), label: 'Open' }, { value: String(arr.filter((r: Record<string, unknown>) => r['status'] === 'in_progress').length), label: 'In Progress' }, { value: String(arr.filter((r: Record<string, unknown>) => r['status'] === 'resolved').length), label: 'Resolved' }]); this.loading.set(false); });
  }
}
