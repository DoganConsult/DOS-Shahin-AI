...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('issues.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalIssues }}</div><div class="kpi-label">Total Issues</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgResolutionDays }}</div><div class="kpi-label">Avg Resolution (d)</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.reopened }}</div><div class="kpi-label">Reopened</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.closureRate }}</div><div class="kpi-label">Closure Rate %</div></div>
        </div>
...
export class IssuesDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/issues/dashboard')
