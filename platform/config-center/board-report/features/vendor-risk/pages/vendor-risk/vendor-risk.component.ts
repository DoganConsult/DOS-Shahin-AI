      ...
      <div class="page-header">
        <div class="title-row">
          <div class="icon-wrap" style="background: var(--orange-50, #f0f9ff)">
            <i class="pi pi-exclamation-circle" style="color: var(--orange-500)"></i>
          </div>
          <div>
            <h1>Vendor Risk Assessments</h1>
            <p class="subtitle">Evaluate and score third-party vendor risks</p>
          </div>
        </div>
...
export class VendorRiskComponent implements OnInit {
  ...
  refresh(): void {
    ...
    this.http.get<{ data?: unknown[] }>(`${environment.apiUrl}/vendor-risk`).pipe(takeUntilDestroyed(this.destroyRef), catchError(() => of({ data: [] }))).subscribe(res => { const arr = Array.isArray(res) ? res : (res?.data || []); this.items.set(arr.map((r: Record<string, unknown>) => ({ title: String((r as Record<string, unknown>)['title'] || (r as Record<string, unknown>)['name'] || 'Record'), status: String((r as Record<string, unknown>)['status'] || 'active'), date: String((r as Record<string, unknown>)['created_at'] || (r as Record<string, unknown>)['updatedAt'] || new Date().toISOString()), severity: String((r as Record<string, unknown>)['severity'] || (r as Record<string, unknown>)['priority'] || 'info'), }))); this.stats.set([{ value: String(arr.length), label: 'Total' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'active').length), label: 'Active' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'pending').length), label: 'Pending' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'completed').length), label: 'Completed' }]); this.loading.set(false); });
  }
}
