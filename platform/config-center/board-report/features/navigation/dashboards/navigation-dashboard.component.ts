...
interface NavigationDashboardData {
...
@Component({
  ...
  selector: 'app-navigation-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('navigation.dashboard') }}</h2>
      @if (loading()) {
...
export class NavigationModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<NavigationDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: NavigationDashboardData }>(`/api/navigation/dashboard`)
