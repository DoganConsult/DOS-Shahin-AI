...
export class ReportExtComponent implements OnInit {
  ...
  scorecard: Record<string, any> | null = null;
  scorecardItems: Record<string, any>[] = [];
  boardView: Record<string, any> | null = null;
  ...
  ngOnInit() {
    ...
    this.operationsSvc.getReportExtTemplates().subscribe({
      next: (d: Record<string, any>) => { this.templates = asArray(d, 'templates'); this.loading = false; this.cdr.markForCheck(); },
    ...
    this.apiclientSvc.get('/report-ext/board-view').subscribe({ next: (d: Record<string, any>) => { this.boardView = d; } });
  }

  generateReport(t: Record<string, any>) {
    const id = t.id || t.name;
    this.apiclientSvc.post('/report-ext/generate', { templateId: id }).subscribe({
      next: (d: Record<string, any>) => { this.msg.add({ severity: 'success', summary: this.i18n.translate('reportExt.reportGenerated'), detail: d.report_id || 'OK' }); },
      error: (e: any) => { this.msg.add({ severity: 'error', summary: this.i18n.translate('reportExt.error'), detail: e.error?.error }); }
    });
  }
