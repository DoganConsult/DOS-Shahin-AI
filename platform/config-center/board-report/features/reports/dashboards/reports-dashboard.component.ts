...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('reports.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalReports }}</div><div class="kpi-label">Total Reports</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.scheduled }}</div><div class="kpi-label">Scheduled</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.generated }}</div><div class="kpi-label">Generated Today</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.distributed }}</div><div class="kpi-label">Distributed</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgGenerationSec }}</div><div class="kpi-label">Avg Gen (sec)</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.templateCount }}</div><div class="kpi-label">Templates</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Report Type</h3>
              @for (item of data()!.breakdownA; track item.reportType) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Format</h3>
              @for (item of data()!.breakdownB; track item.format) {
                <div class="breakdown-row">
...
export class ReportsDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/reports/dashboard')
