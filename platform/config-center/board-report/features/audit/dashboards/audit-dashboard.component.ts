...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('audit.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalAudits }}</div><div class="kpi-label">Total Audits</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.inProgress }}</div><div class="kpi-label">In Progress</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.planned }}</div><div class="kpi-label">Planned</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.completed }}</div><div class="kpi-label">Completed</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.openFindings }}</div><div class="kpi-label">Open Findings</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.overdueFindings }}</div><div class="kpi-label">Overdue Findings</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgDurationDays }}</div><div class="kpi-label">Avg Duration (d)</div></div>
        ...
        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Audit Type</h3>
              @for (item of data()!.breakdownA; track item.auditType) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Finding Severity</h3>
              @for (item of data()!.breakdownB; track item.severity) {
                <div class="breakdown-row">
...
export class AuditDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/audit/dashboard')
