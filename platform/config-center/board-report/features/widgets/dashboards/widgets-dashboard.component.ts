...
interface WidgetsDashboardData {
...
@Component({
  ...
  selector: 'app-widgets-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('widgets.dashboard') }}</h2>
      @if (loading()) {
...
export class WidgetsModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<WidgetsDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: WidgetsDashboardData }>(`/api/widgets/dashboard`)
