...
export class ContextualAIComponent {
  ...
  entityType = 'risk';
  entityId = '';
  summary: Record<string, any> | null = null;
  recEntityType = 'risk';
  recEntityId = '';
  ...
  getSuggestions() {
    ...
    this.apiclientSvc.get(`/contextual-ai/ai/suggestions?route=${this.route}`).subscribe({
      next: (d: Record<string, any>) => { this.suggestions = asArray(d, 'data'); this.loading = false; this.cdr.markForCheck(); },
  ...
  getSummary() {
    ...
    this.apiclientSvc.get(`/contextual-ai/ai/summary/${this.entityType}/${this.entityId}`).subscribe({
      next: (d: Record<string, any>) => { this.summary = d.data || d; }
    });
  }
  ...
  getRecommendations() {
    ...
    this.apiclientSvc.get(`/contextual-ai/ai/recommendations/${this.recEntityType}/${this.recEntityId}`).subscribe({
      next: (d: Record<string, any>) => { this.recommendations = asArray(d, 'data'); }
    });
  }
