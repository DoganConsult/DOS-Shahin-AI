...
interface Modification {
  id: string;
  systemName: string;
  modType: 'model_update'|'data_change'|'config_change'|'architecture';
  description: string;
  impact: 'substantial'|'minor'|'cosmetic';
...
@Component({
    selector: 'app-ai-modifications',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiModifications') || 'System Modifications' }}</h1>
        <p class="page-subtitle">Track modifications to AI systems that may trigger re-assessment requirements.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Modtype</th><th>Description</th><th>Impact</th><th>Status</th><th>Requestedby</th><th>Approvedat</th></tr></thead>
          <tbody>
...
export class AiModificationsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<Modification[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<Modification[]>('/api/ai-governance/modifications').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
