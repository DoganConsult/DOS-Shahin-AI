...
@Component({
    selector: 'app-ai-model-provenance',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiModelProvenance') || 'Model Provenance' }}</h1>
        <p class="page-subtitle">Track AI model lineage, versioning, and provenance chain for audit compliance.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Modelname</th><th>Version</th><th>Parentmodel</th><th>Trainingdata</th><th>Framework</th><th>Createdat</th><th>Hashsignature</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.modelName }}</td>
                <td>{{ item.version }}</td>
                <td>{{ item.parentModel }}</td>
                <td>{{ item.trainingData }}</td>
                <td>{{ item.framework }}</td>
                <td>{{ item.createdAt | date:'short' }}</td>
                <td>{{ item.hashSignature }}</td>
              </tr>
            } @empty {
...
export class AiModelProvenanceComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<ModelProvenance[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['modelName'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<ModelProvenance[]>('/api/ai-governance/model-provenance').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
