...
@Component({
    selector: 'app-ai-monitoring-plans',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiMonitoringPlans') || 'Monitoring Plans' }}</h1>
        <p class="page-subtitle">Define and track post-market monitoring plans for deployed AI systems.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Monitoring Plans</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Monitoringtype</th><th>Frequency</th><th>Metrics</th><th>Alerts</th><th>Status</th><th>Lastexecution</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.monitoringType }}</td>
                <td>{{ item.frequency }}</td>
                <td>{{ item.metrics }}</td>
                <td>{{ item.alerts }}</td>
                <td><span class="status-badge">{{ item.status }}</span></td>
                <td>{{ item.lastExecution | date:'short' }}</td>
              </tr>
            } @empty {
...
export class AiMonitoringPlansComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<MonitoringPlan[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<MonitoringPlan[]>('/api/ai-governance/monitoring-plans').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
