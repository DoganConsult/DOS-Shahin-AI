// AI Governance — Explainability Dashboard page (ai-explainability-dashboard).
// Phase 2 i18n + API hardening: I18nService bindings (RTL via dir(),
// bilingual labels), ApiClientService fetch against the canonical
// platform endpoint /ai-engine/governance/explainability, explicit loading/error/empty/ready
...
interface Row {
  recordId?: string | number | boolean | null;
  agentId?: string | number | boolean | null;
  decisionType?: string | number | boolean | null;
...
@Component({
  ...
  selector: 'app-ai-explainability-dashboard',
  standalone: true,
  imports: [CommonModule, CardModule, TableModule, TagModule],
  templateUrl: './ai-explainability-dashboard.component.html',
})
export class AiExplainabilityDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.get<unknown>('/ai-engine/governance/explainability').subscribe({
      next: (res) => {
