...
@Component({
    selector: 'app-ai-serious-incidents',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiSeriousIncidents') || 'Serious Incidents' }}</h1>
        <p class="page-subtitle">Report and manage serious incidents per EU AI Act Article 62.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Serious Incidents</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Title</th><th>System</th><th>Incidenttype</th><th>Severity</th><th>Reportedtoauthority</th><th>Status</th><th>Occurredat</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.severity }}</td>
                <td>{{ item.reportedToAuthority ? 'Yes' : 'No' }}</td>
...
export class AiSeriousIncidentsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<SeriousIncident[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<SeriousIncident[]>('/api/ai-governance/serious-incidents').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
