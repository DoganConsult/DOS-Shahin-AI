      ...
      next: (data: Record<string, unknown>) => {
        if (data) {
          ...
          this.config = {
            ...
            slaGraceMultiplier: d.slaGraceMultiplier ?? d.sla_grace_multiplier ?? 1.5,
            aiCanExecuteActions: d.aiCanExecuteActions ?? d.ai_can_execute_actions ?? true,
            aiCanDraftApprovals: d.aiCanDraftApprovals ?? d.ai_can_draft_approvals ?? false,
            requireHumanReview: d.requireHumanReview ?? d.require_human_review ?? true,
            cronIntervalMinutes: d.cronIntervalMinutes ?? d.cron_interval_minutes ?? 5,
          };
        }
        this.loading = false;
        this.cdr.markForCheck();
