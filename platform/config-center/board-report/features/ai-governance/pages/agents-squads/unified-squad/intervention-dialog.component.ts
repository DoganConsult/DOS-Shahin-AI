// AI Governance — Intervention Queue page (intervention-dialog).
// Phase 2 i18n + API hardening: I18nService bindings (RTL via dir(),
// bilingual labels), ApiClientService fetch against the canonical
// platform endpoint /ai-engine/hitl/queue, explicit loading/error/empty/ready
...
interface Row {
  state_id?: string | number | boolean | null;
  entity_type?: string | number | boolean | null;
  hitl_state?: string | number | boolean | null;
  ai_agent_id?: string | number | boolean | null;
  [key: string]: unknown;
}
...
export class InterventionDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.get<unknown>('/ai-engine/hitl/queue').subscribe({
      next: (res) => {
