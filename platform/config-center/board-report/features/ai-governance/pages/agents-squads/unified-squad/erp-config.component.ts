// AI Governance — ERP Integration Config page (erp-config).
// Phase 2 i18n + API hardening: I18nService bindings (RTL via dir(),
// bilingual labels), ApiClientService fetch against the canonical
// platform endpoint /ai-engine/agrc-os/integration, explicit loading/error/empty/ready
...
interface Row {
  integrationKey?: string | number | boolean | null;
  vendor?: string | number | boolean | null;
...
export class ErpConfigComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.get<unknown>('/ai-engine/agrc-os/integration').subscribe({
      next: (res) => {
