// AI Governance — Workflow Versions page (workflow-visualizer).
// Phase 2 i18n + API hardening: I18nService bindings (RTL via dir(),
// bilingual labels), ApiClientService fetch against the canonical
// platform endpoint /ai-engine/agrc-os/workflow-versions, explicit loading/error/empty/ready
...
interface Row {
  versionId?: string | number | boolean | null;
  workflowKey?: string | number | boolean | null;
...
export class WorkflowVisualizerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.get<unknown>('/ai-engine/agrc-os/workflow-versions').subscribe({
      next: (res) => {
