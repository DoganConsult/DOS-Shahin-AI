...
interface OversightConfig {
  id: string;
  systemName: string;
  oversightLevel: 'full'|'partial'|'minimal';
  reviewer: string;
...
@Component({
    selector: 'app-ai-human-oversight',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiHumanOversight') || 'Human Oversight' }}</h1>
        <p class="page-subtitle">Configure and monitor human-in-the-loop requirements per EU AI Act Article 14.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Oversightlevel</th><th>Reviewer</th><th>Reviewfrequency</th><th>Overridecount</th><th>Lastreview</th><th>Compliancescore</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td><span class="status-badge">{{ item.oversightLevel }}</span></td>
                <td>{{ item.reviewer }}</td>
                <td>{{ item.reviewFrequency }}</td>
                <td>{{ item.overrideCount }}</td>
                <td>{{ item.lastReview | date:'short' }}</td>
...
export class AiHumanOversightComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<OversightConfig[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<OversightConfig[]>('/api/ai-governance/human-oversight').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
