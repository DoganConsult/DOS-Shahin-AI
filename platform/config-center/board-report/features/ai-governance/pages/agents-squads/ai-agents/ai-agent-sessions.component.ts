...
@Component({
    selector: 'app-ai-agent-sessions',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiAgentSessions') || 'Agent Sessions' }}</h1>
        <p class="page-subtitle">Monitor active and historical AI agent sessions with performance metrics.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Agentid</th><th>Agentname</th><th>Status</th><th>Startedat</th><th>Duration</th><th>Actions</th><th>Tokensused</th></tr></thead>
          <tbody>
...
export class AiAgentSessionsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<AgentSession[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<AgentSession[]>('/api/ai-governance/agent-sessions').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
