...
@Component({
    selector: 'app-ai-automated-decisions',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiAutomatedDecisions') || 'Automated Decisions' }}</h1>
        <p class="page-subtitle">Track and audit AI-powered automated decisions per EU AI Act Article 14 requirements.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Decisiontype</th><th>Affectedpersons</th><th>Appealrate</th><th>Status</th><th>Lastreview</th><th>Humanoverride</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.decisionType }}</td>
                <td>{{ item.affectedPersons }}</td>
                <td>{{ item.appealRate }}</td>
                <td><span class="status-badge">{{ item.status }}</span></td>
                <td>{{ item.lastReview | date:'short' }}</td>
                <td>{{ item.humanOverride ? 'Yes' : 'No' }}</td>
              </tr>
            } @empty {
...
export class AiAutomatedDecisionsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<AutomatedDecision[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<AutomatedDecision[]>('/api/ai-governance/automated-decisions').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
