...
interface AgentAuthority {
  ...
  agentId: string;
  agentName: string;
  authorityLevel: 'full'|'restricted'|'supervised'|'read_only';
...
@Component({
    selector: 'app-ai-agent-authority',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiAgentAuthority') || 'Agent Authority Matrix' }}</h1>
        <p class="page-subtitle">Define authority levels, delegation boundaries, and escalation rules for AI agents.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Agentid</th><th>Agentname</th><th>Authoritylevel</th><th>Delegationscope</th><th>Escalationtarget</th><th>Maxactions</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.agentId }}</td>
                <td>{{ item.agentName }}</td>
                <td><span class="status-badge">{{ item.authorityLevel }}</span></td>
                <td>{{ item.delegationScope }}</td>
                <td>{{ item.escalationTarget }}</td>
                <td>{{ item.maxActions }}</td>
...
export class AiAgentAuthorityComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<AgentAuthority[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['agentId'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<AgentAuthority[]>('/api/ai-governance/agent-authority').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
