...
@Component({
    ...
    imports: [
        ...
        ProgressRingChartComponent, GaugeChartComponent, SparklineChartComponent, AppNumberPipe,
    ],
    template: `
...
export class AISquadComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadSquad() {
    ...
    this.apiclientSvc.get('/ai-squad').subscribe({
      next: (data: Record<string, unknown>) => {
        const raw: any[] = (data as any)?.agents || [];
        this.agents = raw.map((a: Record<string, any>, i: number) => ({
  ...
  private renderNetwork() {
    ...
    this.simulation = d3.forceSimulation<AgentNode>(nodes)
      ...
      .on('tick', () => {
        linkEls
          .attr('x1', (d: Record<string, any>) => d.source.x).attr('y1', (d: Record<string, any>) => d.source.y)
          .attr('x2', (d: Record<string, any>) => d.target.x).attr('y2', (d: Record<string, any>) => d.target.y);
