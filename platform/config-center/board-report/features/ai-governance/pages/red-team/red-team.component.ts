    ...
    <div class="page-shell">
      <div class="page-header">
        <div class="title-row">
          ...
          <div>
            <h1>AI Red Team</h1>
            <p class="subtitle">Adversarial testing and AI model vulnerability assessment</p>
          </div>
        </div>
        ...
        <div class="main-content">
          ...
          <div *ngIf="!loading() && items().length > 0" class="data-list">
            <div class="list-item" *ngFor="let item of items()">
              <div class="item-icon"><i class="pi pi-shield"></i></div>
...
export class RedTeamComponent implements OnInit {
  ...
  refresh(): void {
    ...
    this.http.get<{ data?: unknown[] }>(`${environment.apiUrl}/ai/red-team/scenarios`).pipe(takeUntilDestroyed(this.destroyRef), catchError(() => of({ data: [] }))).subscribe(res => { const arr = Array.isArray(res) ? res : (res?.data || []); this.items.set(arr.map((r: Record<string, unknown>) => ({ title: String((r as Record<string, unknown>)['title'] || (r as Record<string, unknown>)['name'] || 'Record'), status: String((r as Record<string, unknown>)['status'] || 'active'), date: String((r as Record<string, unknown>)['created_at'] || (r as Record<string, unknown>)['updatedAt'] || new Date().toISOString()), severity: String((r as Record<string, unknown>)['severity'] || (r as Record<string, unknown>)['priority'] || 'info'), }))); this.stats.set([{ value: String(arr.length), label: 'Total' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'active').length), label: 'Active' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'pending').length), label: 'Pending' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'completed').length), label: 'Completed' }]); this.loading.set(false); });
  }
}
