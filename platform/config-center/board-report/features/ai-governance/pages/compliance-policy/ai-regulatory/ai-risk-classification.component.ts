...
interface RiskClassification {
  ...
  assessedBy: string;
  assessedAt: string;
  status: 'classified'|'under_review'|'appealed';
}

@Component({
    selector: 'app-ai-risk-classification',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiRiskClassification') || 'Risk Classification' }}</h1>
        <p class="page-subtitle">Classify AI systems by risk level per EU AI Act Article 6 taxonomy.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Riskclass</th><th>Annex</th><th>Justification</th><th>Assessedby</th><th>Assessedat</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.assessedAt | date:'short' }}</td>
...
export class AiRiskClassificationComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<RiskClassification[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<RiskClassification[]>('/api/ai-governance/risk-classification').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
