...
@Component({
    ...
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiProfiling') || 'AI Profiling' }}</h1>
        <p class="page-subtitle">Monitor and manage AI profiling activities per GDPR Article 22 requirements.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Profilingtype</th><th>Datasubjects</th><th>Legalbasis</th><th>Automated</th><th>Appealable</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.profilingType }}</td>
                <td>{{ item.dataSubjects }}</td>
                <td>{{ item.legalBasis }}</td>
                <td>{{ item.automated ? 'Yes' : 'No' }}</td>
                <td>{{ item.appealable ? 'Yes' : 'No' }}</td>
...
export class AiProfilingComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<ProfilingActivity[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<ProfilingActivity[]>('/api/ai-governance/profiling').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
