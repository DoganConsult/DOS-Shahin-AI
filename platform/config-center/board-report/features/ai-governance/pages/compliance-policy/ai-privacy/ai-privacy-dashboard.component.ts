...
@Component({
    selector: 'app-ai-privacy-dashboard',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiPrivacyDashboard') || 'AI Privacy Dashboard' }}</h1>
        <p class="page-subtitle">Monitor AI system privacy compliance including data minimization and retention.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>AI Privacy Dashboard</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Datacategory</th><th>Retentiondays</th><th>Minimized</th><th>Encrypted</th><th>Dpiaconducted</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.dataCategory }}</td>
                <td>{{ item.retentionDays }}</td>
                <td>{{ item.minimized ? 'Yes' : 'No' }}</td>
                <td>{{ item.encrypted ? 'Yes' : 'No' }}</td>
                <td>{{ item.dpiaConducted ? 'Yes' : 'No' }}</td>
...
export class AiPrivacyDashboardComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<PrivacyMetric[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<PrivacyMetric[]>('/api/ai-governance/privacy-dashboard').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
