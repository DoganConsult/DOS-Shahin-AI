...
@Component({
    selector: 'app-ai-complaints',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiComplaints') || 'AI Complaints' }}</h1>
        <p class="page-subtitle">Manage complaints related to AI system decisions and outputs per regulatory requirements.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Subject</th><th>System</th><th>Category</th><th>Status</th><th>Priority</th><th>Submittedat</th><th>Assignee</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.subject }}</td>
                <td>{{ item.system }}</td>
                <td>{{ item.category }}</td>
...
export class AiComplaintsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<AiComplaint[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['subject'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<AiComplaint[]>('/api/ai-governance/complaints').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
