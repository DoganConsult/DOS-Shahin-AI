...
@Component({
    selector: 'app-ai-privacy-incidents',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiPrivacyIncidents') || 'Privacy Incidents' }}</h1>
        <p class="page-subtitle">Track AI-related privacy incidents and breach notifications.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Privacy Incidents</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Title</th><th>System</th><th>Incidenttype</th><th>Datasubjectsaffected</th><th>Severity</th><th>Status</th><th>Reportedat</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.dataSubjectsAffected }}</td>
                <td>{{ item.severity }}</td>
...
export class AiPrivacyIncidentsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<PrivacyIncident[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<PrivacyIncident[]>('/api/ai-governance/privacy-incidents').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
