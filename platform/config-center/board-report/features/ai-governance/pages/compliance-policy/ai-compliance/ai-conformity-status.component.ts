...
@Component({
    selector: 'app-ai-conformity-status',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiConformityStatus') || 'Conformity Status' }}</h1>
        <p class="page-subtitle">Dashboard view of conformity assessment status across all AI systems.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Conformity Status</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Riskclass</th><th>Cemarking</th><th>Lastassessment</th><th>Compliancescore</th><th>Gaps</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.riskClass }}</td>
                <td>{{ item.ceMarking ? 'Yes' : 'No' }}</td>
...
export class AiConformityStatusComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<ConformityStatus[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<ConformityStatus[]>('/api/ai-governance/conformity-status').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
