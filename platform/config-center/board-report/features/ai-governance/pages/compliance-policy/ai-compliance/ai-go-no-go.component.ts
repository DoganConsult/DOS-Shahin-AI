...
interface GoNoGoDecision {
  ...
  systemName: string;
  reviewType: string;
  decision: 'go'|'no_go'|'conditional'|'deferred';
  reviewDate: string;
  reviewer: string;
  conditions: number;
...
@Component({
    selector: 'app-ai-go-no-go',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiGoNoGo') || 'Go/No-Go Decisions' }}</h1>
        <p class="page-subtitle">AI system deployment gate reviews and go/no-go decision tracking.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Go/No-Go Decisions</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Reviewtype</th><th>Decision</th><th>Reviewdate</th><th>Reviewer</th><th>Conditions</th><th>Riskaccepted</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.reviewType }}</td>
                <td><span class="status-badge">{{ item.decision }}</span></td>
                <td>{{ item.reviewDate | date:'short' }}</td>
                <td>{{ item.reviewer }}</td>
                <td>{{ item.conditions }}</td>
                <td>{{ item.riskAccepted ? 'Yes' : 'No' }}</td>
              </tr>
            } @empty {
...
export class AiGoNoGoComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<GoNoGoDecision[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<GoNoGoDecision[]>('/api/ai-governance/go-no-go').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
