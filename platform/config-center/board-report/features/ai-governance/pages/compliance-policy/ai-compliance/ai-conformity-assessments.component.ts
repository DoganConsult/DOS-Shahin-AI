...
interface ConformityAssessment {
  id: string;
  systemName: string;
...
@Component({
    selector: 'app-ai-conformity-assessments',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiConformityAssessments') || 'Conformity Assessments' }}</h1>
        <p class="page-subtitle">Manage EU AI Act conformity assessment procedures for high-risk AI systems.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Assessmenttype</th><th>Standard</th><th>Status</th><th>Assessor</th><th>Completedat</th><th>Score</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.status }}</td>
                <td>{{ item.assessor }}</td>
                <td>{{ item.completedAt | date:'short' }}</td>
                <td>{{ item.score }}</td>
              </tr>
            } @empty {
...
export class AiConformityAssessmentsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<ConformityAssessment[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<ConformityAssessment[]>('/api/ai-governance/conformity-assessments').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
