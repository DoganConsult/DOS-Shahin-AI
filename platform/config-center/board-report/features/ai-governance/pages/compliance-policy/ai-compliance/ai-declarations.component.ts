...
@Component({
    selector: 'app-ai-declarations',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiDeclarations') || 'EU Declarations' }}</h1>
        <p class="page-subtitle">Manage EU declarations of conformity for AI systems per EU AI Act.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>EU Declarations</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Declarationtype</th><th>Issuedat</th><th>Validuntil</th><th>Notifiedbody</th><th>Status</th><th>Version</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.declarationType }}</td>
                <td>{{ item.issuedAt | date:'short' }}</td>
                <td>{{ item.validUntil }}</td>
                <td>{{ item.notifiedBody }}</td>
...
export class AiDeclarationsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<Declaration[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<Declaration[]>('/api/ai-governance/declarations').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
