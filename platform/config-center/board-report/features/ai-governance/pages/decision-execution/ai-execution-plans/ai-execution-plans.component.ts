// AI Governance — Execution Plans page (ai-execution-plans).
// Phase 2 i18n + API hardening: I18nService bindings (RTL via dir(),
// bilingual labels), ApiClientService fetch against the canonical
// platform endpoint /ai-engine/temporal/workflows, explicit loading/error/empty/ready
...
interface Row {
  workflowId?: string | number | boolean | null;
...
@Component({
  ...
  templateUrl: './ai-execution-plans.component.html',
})
export class AiExecutionPlansComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.get<unknown>('/ai-engine/temporal/workflows').subscribe({
      next: (res) => {
