...
@Component({
    selector: 'app-ai-stakeholders',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiStakeholders') || 'AI Stakeholders' }}</h1>
        <p class="page-subtitle">Map AI governance stakeholders and their interests across the organization.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>AI Stakeholders</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Name</th><th>Role</th><th>Interest</th><th>Influence</th><th>Engagement</th><th>Systems</th><th>Lastengagement</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td><span class="status-badge">{{ item.engagement }}</span></td>
                <td>{{ item.systems }}</td>
                <td>{{ item.lastEngagement | date:'short' }}</td>
              </tr>
            } @empty {
...
export class AiStakeholdersComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<Stakeholder[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<Stakeholder[]>('/api/ai-governance/stakeholders').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
