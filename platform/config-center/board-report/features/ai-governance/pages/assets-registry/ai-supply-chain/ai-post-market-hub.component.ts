...
@Component({
    selector: 'app-ai-post-market-hub',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiPostMarketHub') || 'Post-Market Hub' }}</h1>
        <p class="page-subtitle">Central hub for post-market surveillance of deployed AI systems per EU AI Act.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Post-Market Hub</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Surveillancetype</th><th>Finding</th><th>Severity</th><th>Status</th><th>Reportedat</th><th>Assignee</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.systemName }}</td>
                <td>{{ item.surveillanceType }}</td>
                <td>{{ item.finding }}</td>
                <td>{{ item.severity }}</td>
                <td><span class="status-badge">{{ item.status }}</span></td>
                <td>{{ item.reportedAt | date:'short' }}</td>
                <td>{{ item.assignee }}</td>
              </tr>
            } @empty {
...
export class AiPostMarketHubComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<PostMarketItem[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<PostMarketItem[]>('/api/ai-governance/post-market').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
