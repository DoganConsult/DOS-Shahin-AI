...
@Component({
    selector: 'app-ai-supply-chain-hub',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiSupplyChainHub') || 'Supply Chain Hub' }}</h1>
        <p class="page-subtitle">Central hub for AI supply chain risk management and vendor oversight.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Supply Chain Hub</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Vendorname</th><th>Component</th><th>Risklevel</th><th>Lastassessment</th><th>Certifications</th><th>Status</th><th>Country</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.vendorName }}</td>
                <td>{{ item.component }}</td>
                <td>{{ item.riskLevel }}</td>
                <td>{{ item.lastAssessment | date:'short' }}</td>
...
export class AiSupplyChainHubComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<SupplyChainItem[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['vendorName'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<SupplyChainItem[]>('/api/ai-governance/supply-chain').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
