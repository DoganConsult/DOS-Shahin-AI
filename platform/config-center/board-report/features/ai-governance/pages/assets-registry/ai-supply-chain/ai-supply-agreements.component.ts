...
interface SupplyAgreement {
  ...
  startDate: string;
  endDate: string;
  complianceClauses: number;
  status: 'active'|'expiring'|'expired'|'draft';
}

@Component({
    selector: 'app-ai-supply-agreements',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiSupplyAgreements') || 'Supply Agreements' }}</h1>
        <p class="page-subtitle">Manage contractual agreements with AI supply chain partners.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Supply Agreements</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Partnername</th><th>Agreementtype</th><th>Scope</th><th>Startdate</th><th>Enddate</th><th>Complianceclauses</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.partnerName }}</td>
                <td>{{ item.agreementType }}</td>
                <td>{{ item.scope }}</td>
                <td>{{ item.startDate | date:'short' }}</td>
                <td>{{ item.endDate | date:'short' }}</td>
                <td>{{ item.complianceClauses }}</td>
...
export class AiSupplyAgreementsComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<SupplyAgreement[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['partnerName'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<SupplyAgreement[]>('/api/ai-governance/supply-agreements').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
