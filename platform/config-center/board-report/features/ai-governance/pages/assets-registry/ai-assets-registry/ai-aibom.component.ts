...
@Component({
    selector: 'app-ai-aibom',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiAibom') || 'AI Bill of Materials' }}</h1>
        <p class="page-subtitle">Track AI system components, dependencies, and supply chain metadata (AIBOM).</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>AI Bill of Materials</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Systemname</th><th>Component</th><th>Version</th><th>Supplier</th><th>License</th><th>Riskscore</th><th>Lastaudit</th></tr></thead>
          <tbody>
...
export class AiAibomComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<AibomEntry[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<AibomEntry[]>('/api/ai-governance/aibom').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
