...
interface CustodyRecord {
  id: string;
  assetName: string;
  assetType: 'model'|'dataset'|'pipeline'|'artifact';
  custodian: string;
  action: string;
  timestamp: string;
  verified: boolean;
...
@Component({
    selector: 'app-ai-chain-of-custody',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiChainOfCustody') || 'Chain of Custody' }}</h1>
        <p class="page-subtitle">Track data provenance and custody chain for AI training datasets and model artifacts.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Assetname</th><th>Assettype</th><th>Custodian</th><th>Action</th><th>Timestamp</th><th>Verified</th><th>Integrityhash</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.assetName }}</td>
                <td>{{ item.assetType }}</td>
                <td>{{ item.custodian }}</td>
                <td>{{ item.action }}</td>
                <td>{{ item.timestamp }}</td>
                <td>{{ item.verified ? 'Yes' : 'No' }}</td>
                <td>{{ item.integrityHash }}</td>
              </tr>
            } @empty {
...
export class AiChainOfCustodyComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<CustodyRecord[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['assetName'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<CustodyRecord[]>('/api/ai-governance/chain-of-custody').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
