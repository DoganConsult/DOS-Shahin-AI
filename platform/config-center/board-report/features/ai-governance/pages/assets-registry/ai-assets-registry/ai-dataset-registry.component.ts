...
interface DatasetEntry {
  id: string;
  name: string;
  purpose: 'training'|'validation'|'testing'|'production';
  records: number;
  features: number;
...
@Component({
    selector: 'app-ai-dataset-registry',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiDatasetRegistry') || 'Dataset Registry' }}</h1>
        <p class="page-subtitle">Central registry of datasets used for AI model training, validation, and testing.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Name</th><th>Purpose</th><th>Records</th><th>Features</th><th>Biasassessed</th><th>Lastupdated</th><th>Owner</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.name }}</td>
                <td><span class="status-badge">{{ item.purpose }}</span></td>
                <td>{{ item.records }}</td>
                <td>{{ item.features }}</td>
                <td>{{ item.biasAssessed ? 'Yes' : 'No' }}</td>
...
export class AiDatasetRegistryComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<DatasetEntry[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<DatasetEntry[]>('/api/ai-governance/dataset-registry').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
