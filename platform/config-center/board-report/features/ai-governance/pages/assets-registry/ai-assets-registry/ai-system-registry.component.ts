...
interface AiSystem {
  ...
  status: 'registered'|'operational'|'decommissioned'|'suspended';
  euDbRegistered: boolean;
}

@Component({
    selector: 'app-ai-system-registry',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiSystemRegistry') || 'System Registry' }}</h1>
        <p class="page-subtitle">Central registry of all AI systems per EU AI Act Article 51 requirements.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Name</th><th>Riskclass</th><th>Purpose</th><th>Provider</th><th>Deploymentdate</th><th>Status</th><th>Eudbregistered</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.purpose }}</td>
                <td>{{ item.provider }}</td>
                <td>{{ item.deploymentDate | date:'short' }}</td>
                <td><span class="status-badge">{{ item.status }}</span></td>
                <td>{{ item.euDbRegistered ? 'Yes' : 'No' }}</td>
              </tr>
            } @empty {
...
export class AiSystemRegistryComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<AiSystem[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['name'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<AiSystem[]>('/api/ai-governance/system-registry').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
