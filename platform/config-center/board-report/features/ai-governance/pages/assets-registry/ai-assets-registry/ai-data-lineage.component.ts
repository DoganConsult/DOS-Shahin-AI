...
interface LineageNode {
  id: string;
  name: string;
  nodeType: 'source'|'transform'|'model'|'output';
  upstream: number;
  downstream: number;
  dataVolume: string;
...
@Component({
    selector: 'app-ai-data-lineage',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiDataLineage') || 'Data Lineage' }}</h1>
        <p class="page-subtitle">Visualize and track data lineage from source to AI model training and inference.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Name</th><th>Nodetype</th><th>Upstream</th><th>Downstream</th><th>Datavolume</th><th>Lastprocessed</th><th>Quality</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.lastProcessed | date:'short' }}</td>
                <td>{{ item.quality }}</td>
              </tr>
            } @empty {
  ...
  `,
    ...
    .empty-cell { text-align: center; color: var(--text-color-secondary); padding: 2rem; }
  `]
})
export class AiDataLineageComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<LineageNode[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<LineageNode[]>('/api/ai-governance/data-lineage').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
