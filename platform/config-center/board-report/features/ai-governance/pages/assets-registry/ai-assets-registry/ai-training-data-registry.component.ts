...
@Component({
    selector: 'app-ai-training-data-registry',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiTrainingDataRegistry') || 'Training Data Registry' }}</h1>
        <p class="page-subtitle">Register and govern training datasets with bias assessment and quality metrics.</p>
      </div>
      ...
      <div class="section-card">
        <div class="table-header">
          <h3>Training Data Registry</h3>
          <input type="text" class="search-input" placeholder="Search..." (input)="onSearch($event)" />
        </div>
        <table class="data-table">
          <thead><tr><th>Name</th><th>Purpose</th><th>Records</th><th>Biasscore</th><th>Qualityscore</th><th>Lastvalidated</th><th>Status</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                ...
                <td>{{ item.purpose }}</td>
                <td>{{ item.records }}</td>
                <td>{{ item.biasScore }}</td>
                <td>{{ item.qualityScore }}</td>
                <td>{{ item.lastValidated | date:'short' }}</td>
...
export class AiTrainingDataRegistryComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<TrainingDataset[]>([]);
  ...
  ngOnInit(): void {
    this.http.get<TrainingDataset[]>('/api/ai-governance/training-data').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
