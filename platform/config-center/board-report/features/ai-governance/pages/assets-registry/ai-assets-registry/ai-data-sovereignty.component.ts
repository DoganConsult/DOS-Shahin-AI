...
@Component({
    selector: 'app-ai-data-sovereignty',
    imports: [CommonModule, RouterModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
    <div class="module-page" [dir]="i18n.direction()">
      <div class="page-header">
        <h1 class="page-title">{{ i18n.translate('aiGov.aiDataSovereignty') || 'Data Sovereignty' }}</h1>
        <p class="page-subtitle">Ensure AI data residency and sovereignty requirements across jurisdictions.</p>
      </div>
      ...
      <div class="section-card">
        ...
        <table class="data-table">
          <thead><tr><th>Datasetname</th><th>Jurisdiction</th><th>Storagelocation</th><th>Complianceframework</th><th>Status</th><th>Lastaudit</th><th>Datasubjects</th></tr></thead>
          <tbody>
            @for (item of filteredItems(); track item.id) {
              <tr>
                <td class="cell-title">{{ item.datasetName }}</td>
                <td>{{ item.jurisdiction }}</td>
                <td>{{ item.storageLocation }}</td>
                <td>{{ item.complianceFramework }}</td>
                <td><span class="status-badge">{{ item.status }}</span></td>
                <td>{{ item.lastAudit | date:'short' }}</td>
                <td>{{ item.dataSubjects }}</td>
              </tr>
            } @empty {
...
export class AiDataSovereigntyComponent implements OnInit {
  protected readonly i18n = inject(I18nService);
  private readonly http = inject(HttpClient);
  protected readonly items = signal<SovereigntyRecord[]>([]);
  ...
  protected readonly filteredItems = () => {
    ...
    return this.items().filter(item => {
      const first = (item as any)['datasetName'] as string;
      return first?.toLowerCase().includes(t);
    });
  };

  ngOnInit(): void {
    this.http.get<SovereigntyRecord[]>('/api/ai-governance/data-sovereignty').subscribe({ next: (d) => this.items.set(d), error: () => {} });
  }
