 ...
 */
@Component({
    selector: 'app-ai-agent-asset-list',
    imports: [
        CommonModule,
        FormsModule,
        ButtonModule,
    ...
    <aside class="asset-panel">
      <div class="panel-header">
        <h3 class="panel-title">{{ i18n.translate('ai.agents.assetTitle') }}</h3>
        <button
          *ngIf="canWrite"
          pButton
          icon="pi pi-plus"
          class="p-button-sm p-button-outlined"
          [pTooltip]="i18n.translate('ai.agents.newAsset')"
      ...
      <div *ngIf="!loading && filteredAssets.length === 0" class="empty-state">
        <i class="pi pi-box"></i>
        <span>{{ i18n.translate('ai.agents.noAssets') }}</span>
      </div>
          ...
          *ngFor="let asset of filteredAssets; trackBy: trackAssetById"
          ...
          <div class="asset-item-header">
            ...
            <i *ngIf="isSeededGlobal(asset)"
               class="pi pi-lock"
               [pTooltip]="i18n.translate('ai.agents.seededLocked')"></i>
          </div>
...
export class AiAgentAssetListComponent {
  readonly i18n = inject(I18nService);

  // ── Inputs ────────────────────────────────────────────────────────────────
  @Input() filteredAssets: AgentAsset[] = [];
  @Input() loading = false;
  @Input() searchTerm = '';
  ...
  // ── Outputs ───────────────────────────────────────────────────────────────
  @Output() selectAsset = new EventEmitter<AgentAsset>();
  ...
  trackAssetById(_index: number, asset: AgentAsset): string {
    return asset.id;
  }
