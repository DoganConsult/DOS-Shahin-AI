...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('dora.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalAssets }}</div><div class="kpi-label">ICT Assets</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.incidents }}</div><div class="kpi-label">Major Incidents</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.testsPassed }}</div><div class="kpi-label">Tests Passed</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.testsFailed }}</div><div class="kpi-label">Tests Failed</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.tpspCount }}</div><div class="kpi-label">TPSPs</div></div>
        ...
        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By ICT Category</h3>
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              ...
              @for (item of data()!.breakdownB; track item.riskLevel) {
                <div class="breakdown-row">
...
export class DoraDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/dora/dashboard')
