...
interface DashboardData {
  ...
  kpis: { total: number; active: number; pending: number; expired: number; overdue: number; completionRate: number; avgDurationDays: number; renewalRate: number };
  typeBreakdown: Array<{ exceptionType: string; count: number; activeCount: number; expiredCount: number }>;
  riskBreakdown: Array<{ riskLevel: string; count: number; activeCount: number }>;
}
...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('exception.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.total }}</div><div class="kpi-label">Total</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.completionRate }}%</div><div class="kpi-label">Completion</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgDurationDays }}d</div><div class="kpi-label">Avg Duration</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.renewalRate }}%</div><div class="kpi-label">Renewal Rate</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Exception Type</h3>
              @for (item of data()!.typeBreakdown; track item.exceptionType) {
...
export class ExceptionDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: DashboardData }>('/api/exception/dashboard')
