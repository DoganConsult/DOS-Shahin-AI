...
export class PolicyCodeComponent implements OnInit {
  ...
  error = '';
  ruleJson = '';
  validationResult: Record<string, any> | null = null;
  importJson = '';
  ...
  execPolicyId = '';
  execContext = '';
  execResult: Record<string, any> | null = null;
  ...
  loadPolicies() {
    this.governanceSvc.getGovernancePolicies().subscribe({
      next: (res: Record<string, any>) => {
        ...
        this.policies = list;
        this.policyOptions = list.map((p: Record<string, unknown>) => ({
      ...
      this.apiclientSvc.post('/policies/' + this.execPolicyId + '/evaluate', { context }).pipe(
        catchError(() => {
          ...
          const results = rules.map((rule: Record<string, unknown>) => {
            const contextValue = context[rule.field as string];
            let match = false;
          ...
          return of({ evaluated: true, clientSide: true, results, matchCount: results.filter((r: Record<string, any>) => r.match).length, totalRules: results.length });
        })
      ).subscribe({
        next: (d: Record<string, any>) => {
          this.execResult = d;
