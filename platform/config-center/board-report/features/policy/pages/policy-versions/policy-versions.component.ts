...
@Component({
    ...
    imports: [
        ...
        ButtonModule, SkeletonModule, DialogModule, CheckboxModule, AppDatePipe, DiffViewerComponent,
    ],
    template: `
    <section class="page-shell">
      ...
      <ng-container *ngIf="showDiff">
        ...
        <app-diff-viewer
          ...
          [oldFileName]="'v' + (diffLeft?.version || '')"
          [newFileName]="'v' + (diffRight?.version || '')"
...
export class PolicyVersionsComponent implements OnInit {
  ...
  loadPolicies(): void {
    ...
    this.governanceSvc.getPolicyLifecycleList().subscribe({
      next: (res: Record<string, any>) => {
        ...
        this.policies = raw.map((p: Record<string, unknown>) => this.normalizePolicy(p));
  ...
  loadVersionHistory(policyId: string): void {
    ...
    this.governanceSvc.getPolicyVersions(policyId).subscribe({
      next: (res: Record<string, any>) => {
        ...
        this.versions = (Array.isArray(raw) ? raw : []).map((v: Record<string, any>) => this.normalizeVersion(v))
  ...
  private normalizePolicy(raw: Record<string, any>): PolicySummary {
    return {
  ...
  private normalizeVersion(raw: Record<string, any>): PolicyVersion {
    return {
