              ...
              <tr>
                ...
                <td>
                  <p-tag [value]="r.outcome"
                    [severity]="r.outcome === 'accepted' ? 'success' : r.outcome === 'rejected' ? 'danger' : r.outcome === 'needs_revision' ? 'warning' : 'info'" />
                </td>
                <td>{{ r.reviewer_id }}</td>
...
export class EvidenceReviewsComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    this.apiclientSvc.get(`/evidence/reviews${qs}`).subscribe({
      next: (data: Record<string, unknown>) => { this.reviews = (data.reviews || []) as any; this.applyFilter(); this.loading = false; this.cdr.markForCheck(); },
