// Shahin GRC — Evidence Hub  |  GRC Lifecycle: IMPLEMENT
...
import { EvidenceCatalogComponent } from '../evidence-catalog/evidence-catalog.component';
import { EvidenceTasksComponent } from '../evidence-tasks/evidence-tasks.component';
...
const TABS: HubTab[] = [
  { key: 'evidence', labelEn: 'Evidence',        labelAr: 'الأدلة',       icon: 'pi-folder-open' },
  { key: 'catalog',  labelEn: 'Evidence Catalog', labelAr: 'فهرس الأدلة', icon: 'pi-list' },
  { key: 'tasks',    labelEn: 'Evidence Tasks',   labelAr: 'مهام الأدلة', icon: 'pi-check-square' },
];
...
@Component({
  ...
  selector: 'app-evidence-hub', standalone: true,
  imports: [CommonModule, RouterModule, EvidenceComponent, EvidenceCatalogComponent, EvidenceTasksComponent, AgentBadgeComponent, HubConnectionsStripComponent, HubHelpPanelComponent, RaciPanelComponent],
  styles: [HUB_STYLES],
  template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#dbeafe"><i class="pi pi-folder-open hub-icon" style="color:#1d4ed8"></i></div>
          <div>
            <div class="hub-phase" style="background:rgba(var(--module-accent-blue-rgb), .12);color:#1d4ed8">{{ i18n.currentLang()==='ar' ? 'التطبيق' : 'Implement' }}</div>
            <h1>{{ i18n.currentLang()==='ar' ? 'مركز الأدلة' : 'Evidence Hub' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'جمع الأدلة والفهرس والمهام — متصلة.' : 'Evidence collection, catalog and tasks — connected.' }}</p>
          </div>
        </div>
      ...
      <app-hub-connections-strip [hubKey]="'evidence'" />
      <app-raci-panel entityType="evidence" [entityId]="''" [canEdit]="true" />
      <app-hub-help-panel [hubRoute]="'/evidence-hub'" />
      <div class="hub-tab-content">
        @if (activeTab()==='evidence') { <app-evidence /> }
        @if (activeTab()==='catalog')  { <app-evidence-catalog /> }
        @if (activeTab()==='tasks')    { <app-evidence-tasks /> }
      </div>
    </div>`
})
export class EvidenceHubComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  i18n = inject(I18nService);
  private route = inject(ActivatedRoute);
  tabs = TABS;
  readonly agentId = 'A01';
  activeTab = signal<string>('evidence');
  ngOnInit(): void {
