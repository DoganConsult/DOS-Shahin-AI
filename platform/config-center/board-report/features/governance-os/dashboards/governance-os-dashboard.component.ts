...
interface GovernanceOsDashboardData {
...
@Component({
  ...
  selector: 'app-governance-os-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('governanceOs.dashboard') }}</h2>
      @if (loading()) {
...
export class GovernanceOsModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<GovernanceOsDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: GovernanceOsDashboardData }>(`/api/governance-os/dashboard`)
