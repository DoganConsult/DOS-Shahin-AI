    ...
    <app-widget-shell [title]="title()" [fetchedAt]="fetchedAt()">
      <div class="grid grid-cols-3 gap-3">
        <div class="border rounded-xl p-3">
          ...
          <div class="text-2xl font-semibold">{{ data()?.openRecommendations ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">High Priority</div>
          <div class="text-2xl font-semibold">{{ data()?.highPriorityRecommendations ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Tracked</div>
          <div class="text-2xl font-semibold">{{ data()?.trackedRecommendations ?? 0 }}</div>
        </div>
      </div>
    </app-widget-shell>
  `
...
export class RecommendationsWidgetComponent implements OnInit {
  ...
  readonly title = signal('Recommendations');
  readonly fetchedAt = signal<string | null>(null);
  readonly data = signal<GrcRecord | null>(null);

  async ngOnInit() {
    try {
      const res = await firstValueFrom(this.api.getWidget('recommendations'));
      if (!res) return;
