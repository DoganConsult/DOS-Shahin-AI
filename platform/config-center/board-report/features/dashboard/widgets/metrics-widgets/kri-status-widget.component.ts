...
@Component({
    ...
    selector: 'app-kri-status-widget',
    imports: [CommonModule, WidgetShellComponent],
    template: `
    <app-widget-shell [title]="title()" [fetchedAt]="fetchedAt()">
      <div class="grid grid-cols-3 gap-3">
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Total KRIs</div>
          <div class="text-2xl font-semibold">{{ data()?.totalKris ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Breached</div>
          <div class="text-2xl font-semibold">{{ data()?.breached ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Healthy</div>
          <div class="text-2xl font-semibold">{{ data()?.healthy ?? 0 }}</div>
        </div>
      </div>
    </app-widget-shell>
  `
})
export class KriStatusWidgetComponent implements OnInit {
  @Input() config: Record<string, unknown> = {};
  private api = inject(WidgetsApiService);

  readonly title = signal('KRI Status');
  readonly fetchedAt = signal<string | null>(null);
  readonly data = signal<GrcRecord | null>(null);

  async ngOnInit() {
    try {
      const res = await firstValueFrom(this.api.getWidget('kri-status'));
      if (!res) return;
