...
@Component({
    ...
    selector: 'app-audit-exposure-widget',
    ...
    <app-widget-shell [title]="title()" [fetchedAt]="fetchedAt()">
      <div class="grid grid-cols-3 gap-3">
        ...
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">High Severity</div>
          <div class="text-2xl font-semibold">{{ data()?.highSeverityFindings ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Overdue Remediation</div>
          <div class="text-2xl font-semibold">{{ data()?.overdueRemediations ?? 0 }}</div>
        </div>
      </div>
    </app-widget-shell>
  `
})
export class AuditExposureWidgetComponent implements OnInit {
  @Input() config: Record<string, unknown> = {};
  private api = inject(WidgetsApiService);

  readonly title = signal('Audit Exposure');
  readonly fetchedAt = signal<string | null>(null);
  readonly data = signal<GrcRecord | null>(null);

  async ngOnInit() {
    try {
      const res = await firstValueFrom(this.api.getWidget('audit-exposure'));
      if (!res) return;
