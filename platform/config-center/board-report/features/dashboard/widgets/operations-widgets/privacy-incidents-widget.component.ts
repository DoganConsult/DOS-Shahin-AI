...
@Component({
    ...
    selector: 'app-privacy-incidents-widget',
    ...
    <app-widget-shell [title]="title()" [fetchedAt]="fetchedAt()">
      <div class="grid grid-cols-3 gap-3">
        <div class="border rounded-xl p-3">
          ...
          <div class="text-2xl font-semibold">{{ data()?.openIncidents ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Under Review</div>
          <div class="text-2xl font-semibold">{{ data()?.underReview ?? 0 }}</div>
        </div>
        <div class="border rounded-xl p-3">
          <div class="text-xs text-gray-500">Closed 30d</div>
          <div class="text-2xl font-semibold">{{ data()?.closedLast30Days ?? 0 }}</div>
        </div>
      </div>
    </app-widget-shell>
  `
})
export class PrivacyIncidentsWidgetComponent implements OnInit {
  @Input() config: Record<string, unknown> = {};
  private api = inject(WidgetsApiService);

  readonly title = signal('Privacy Incidents');
  readonly fetchedAt = signal<string | null>(null);
  readonly data = signal<GrcRecord | null>(null);

  async ngOnInit() {
    try {
      const res = await firstValueFrom(this.api.getWidget('privacy-incidents'));
      if (!res) return;
