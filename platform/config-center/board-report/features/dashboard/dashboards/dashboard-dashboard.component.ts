...
interface DashboardDashboardData {
...
@Component({
  ...
  selector: 'app-dashboard-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('dashboard.dashboard') }}</h2>
      @if (loading()) {
...
export class DashboardModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<DashboardDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: DashboardDashboardData }>(`/api/dashboard/dashboard`)
