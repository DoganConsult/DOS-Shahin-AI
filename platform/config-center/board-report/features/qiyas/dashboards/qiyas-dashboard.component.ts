...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('qiyas.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalAssessments }}</div><div class="kpi-label">Total Assessments</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.inProgress }}</div><div class="kpi-label">In Progress</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.overdue }}</div><div class="kpi-label">Overdue</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgScore }}</div><div class="kpi-label">Avg Score</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.maturityLevel }}</div><div class="kpi-label">Maturity Level</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.gapCount }}</div><div class="kpi-label">Gap Count</div></div>
        ...
        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              ...
              @for (item of data()!.breakdownA; track item.framework) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Domain</h3>
              @for (item of data()!.breakdownB; track item.domain) {
                <div class="breakdown-row">
...
export class QiyasDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/qiyas/dashboard')
