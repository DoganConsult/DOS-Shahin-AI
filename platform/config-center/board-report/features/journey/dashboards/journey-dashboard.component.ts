...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('journey.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalJourneys }}</div><div class="kpi-label">Total Journeys</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.stalled }}</div><div class="kpi-label">Stalled</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgMaturity }}</div><div class="kpi-label">Avg Maturity</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.milestonesDue }}</div><div class="kpi-label">Milestones Due</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Stage</h3>
              @for (item of data()!.breakdownA; track item.stage) {
...
export class JourneyDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/journey/dashboard')
