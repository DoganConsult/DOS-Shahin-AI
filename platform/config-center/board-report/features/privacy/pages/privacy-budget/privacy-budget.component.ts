...
export class PrivacyBudgetComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiclientSvc.get('/privacy-budget').subscribe({
      next: (d: Record<string, any>) => {
        this.budgets = Array.isArray(d) ? d : d.budgets || [];
        this.allocations = d.allocations || [];
        this.loading = false;
  ...
  get activeCount(): number { return this.budgets.filter(b => b.status !== 'exhausted').length; }
  get exhaustedCount(): number { return this.budgets.filter(b => b.status === 'exhausted').length; }
  get nearThresholdCount(): number { return this.budgets.filter(b => this.usagePercent(b) >= 80 && b.status !== 'exhausted').length; }

  filteredBudgets(): Record<string, any>[] {
    if (!this.filterStatus) return this.budgets;
    return this.budgets.filter(b => (b.status || 'active') === this.filterStatus);
  }

  applyFilter() {}

  usagePercent(b: Record<string, any>): number { return b.total ? Math.min((b.used / b.total) * 100, 100) : 0; }
  ...
  statusSeverity(status: string): 'success' | 'warning' | 'danger' | 'info' {
    const m: Record<string, any> = { active: 'success', exhausted: 'danger', paused: 'warning' };
