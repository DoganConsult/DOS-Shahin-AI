...
export class PrivacyOpsComponent implements OnInit {
  ...
  loading = false;
  ropa: Record<string, any>[] = []; dsrs: Record<string, any>[] = []; consents: Record<string, any>[] = [];
  breaches: Record<string, any>[] = []; retention: Record<string, any>[] = [];
  constructor(public i18n: I18nService, private apiclientSvc: ApiClientService) {}
  ngOnInit() {
    this.loading = true;
    this.apiclientSvc.get('/privacy-ops/ropa').subscribe({ next: (d: Record<string, any>) => { this.ropa = asArray(d, 'entries'); this.loading = false; this.cdr.markForCheck(); }, error: () => { this.loading = false; this.cdr.markForCheck(); } });
    this.apiclientSvc.get('/privacy-ops/dsr').subscribe({ next: (d: Record<string, any>) => { this.dsrs = asArray(d, 'requests'); } });
    this.apiclientSvc.get('/privacy-ops/consent').subscribe({ next: (d: Record<string, any>) => { this.consents = asArray(d, 'records'); } });
    this.apiclientSvc.get('/privacy-ops/breaches').subscribe({ next: (d: Record<string, any>) => { this.breaches = asArray(d, 'breaches'); } });
    this.apiclientSvc.get('/privacy-ops/retention').subscribe({ next: (d: Record<string, any>) => { this.retention = asArray(d, 'policies'); } });
  }
  createDSR() {
    this.apiclientSvc.post('/privacy-ops/dsr', { subject: 'New Request', request_type: 'access' }).subscribe({
      next: () => { this.apiclientSvc.get('/privacy-ops/dsr').subscribe({ next: (d: Record<string, any>) => { this.dsrs = asArray(d, 'requests'); } }); }
    });
  }
  reportBreach() {
    this.apiclientSvc.post('/privacy-ops/breaches', { description: 'New breach report', severity: 'medium' }).subscribe({
      next: () => { this.apiclientSvc.get('/privacy-ops/breaches').subscribe({ next: (d: Record<string, any>) => { this.breaches = asArray(d, 'breaches'); } }); }
    });
  }
}
