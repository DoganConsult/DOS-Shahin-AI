...
@Component({
    ...
    <div class="page-shell">
      <div class="page-header">
        <div class="title-row">
          <div class="icon-wrap" style="background: var(--green-50, #f0f9ff)">
            <i class="pi pi-replay" style="color: var(--green-500)"></i>
          </div>
          <div>
            <h1>Business Continuity Planning</h1>
            <p class="subtitle">BCP plans, BIA scoring, recovery strategies</p>
          </div>
        </div>
        ...
        <div class="main-content">
          ...
          <div *ngIf="!loading() && items().length > 0" class="data-list">
            <div class="list-item" *ngFor="let item of items()">
              <div class="item-icon"><i class="pi pi-replay"></i></div>
  ...
  `,
    ...
    .item-icon { width: 36px; height: 36px; border-radius: var(--radius); background: var(--green-50, #f0f9ff); display: flex; align-items: center; justify-content: center; }
    .item-icon i { font-size: var(--font-size-md); color: var(--green-500); }
...
export class BCPComponent implements OnInit {
  ...
  readonly bcpAgents: AgentInfo[] = [];

  readonly bcpTransitions = [
    { from: 'draft', to: 'active', requiresApproval: true },
    { from: 'active', to: 'testing' },
    { from: 'testing', to: 'active' },
    { from: 'active', to: 'activated' },
  ...
  moduleKitConfig = computed<ModuleOverviewKitConfig>(() => ({
    ...
    slaHours: 720,
    transitions: this.bcpTransitions,
    currentStatus: 'active',
    agents: this.bcpAgents,
  ...
  refresh(): void {
    ...
    this.http.get<{ data?: unknown[] }>(`${environment.apiUrl}/bcp/plans`).pipe(takeUntilDestroyed(this.destroyRef), catchError(() => of({ data: [] }))).subscribe(res => { const arr = Array.isArray(res) ? res : (res?.data || []); this.items.set(arr.map((r: Record<string, unknown>) => ({ title: String((r as Record<string, unknown>)['title'] || (r as Record<string, unknown>)['name'] || 'Record'), status: String((r as Record<string, unknown>)['status'] || 'active'), date: String((r as Record<string, unknown>)['created_at'] || (r as Record<string, unknown>)['updatedAt'] || new Date().toISOString()), severity: String((r as Record<string, unknown>)['severity'] || (r as Record<string, unknown>)['priority'] || 'info'), }))); this.stats.set([{ value: String(arr.length), label: 'Total' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'active').length), label: 'Active' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'pending').length), label: 'Pending' }, { value: String(arr.filter((r: Record<string, unknown>) => (r as Record<string, unknown>)['status'] === 'completed').length), label: 'Completed' }]); this.loading.set(false); });
  }
}
