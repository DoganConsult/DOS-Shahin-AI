...
@Component({
  ...
  selector: 'app-workpapers-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('workpapers.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalWorkpapers }}</div><div class="kpi-label">Total</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.generated }}</div><div class="kpi-label">Generated</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.templatesActive }}</div><div class="kpi-label">Active Templates</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Template</h3>
              @for (item of data()!.breakdownA; track item.template) {
...
export class WorkpapersDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/workpapers/dashboard')
