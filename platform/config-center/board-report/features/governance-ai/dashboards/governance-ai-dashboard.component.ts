...
@Component({
  ...
  selector: 'app-governance-ai-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('governanceAi.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalModels }}</div><div class="kpi-label">Total Models</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.governed }}</div><div class="kpi-label">Governed</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.ungoverned }}</div><div class="kpi-label">Ungoverned</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.highRisk }}</div><div class="kpi-label">High Risk</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.ethicsReviews }}</div><div class="kpi-label">Ethics Reviews</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.complianceRate }}</div><div class="kpi-label">Compliance %</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.biasAlerts }}</div><div class="kpi-label">Bias Alerts</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Model Risk</h3>
              @for (item of data()!.breakdownA; track item.riskLevel) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Governance Status</h3>
...
export class GovernanceAiDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/governance-ai/dashboard')
