...
@Component({
  ...
  template: `
    <div class="dashboard-container p-6 w-full" [dir]="'ltr'" style="margin-inline-start: auto;">
      <h1 class="text-2xl font-bold mb-4">Benchmarks</h1>
      ...
      <div *ngIf="isLoading" class="flex items-center justify-center py-10 w-full animate-pulse bg-gray-50 rounded">
        ...
        <span class="ms-3 text-indigo-600 font-medium">Loading benchmarks data...</span>
      </div>
...
export class BenchmarksDashboardComponent implements OnInit {
  ...
  items: any[] = [];
  ngOnInit() {
    setTimeout(() => { this.isLoading = false; this.items = []; }, 800);
  }
}
