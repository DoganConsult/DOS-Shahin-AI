...
@Component({
  ...
  selector: 'app-benchmarks-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('benchmarks.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalBenchmarks }}</div><div class="kpi-label">Total</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.active }}</div><div class="kpi-label">Active</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.stale }}</div><div class="kpi-label">Stale</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgScore }}</div><div class="kpi-label">Avg Score</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.datasetsLoaded }}</div><div class="kpi-label">Datasets Loaded</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.comparisonsRun }}</div><div class="kpi-label">Comparisons Run</div></div>
        </div>
        ...
        <div class="grid-2">
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              ...
              @for (item of data()!.breakdownB; track item.source) {
                <div class="breakdown-row">
...
export class BenchmarksDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/benchmarks/dashboard')
