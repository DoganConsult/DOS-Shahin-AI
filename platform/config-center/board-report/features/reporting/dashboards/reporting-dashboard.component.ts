...
interface ReportingDashboardData {
...
@Component({
  ...
  selector: 'app-reporting-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('reporting.dashboard') }}</h2>
      @if (loading()) {
...
export class ReportingModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<ReportingDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: ReportingDashboardData }>(`/api/reporting/dashboard`)
