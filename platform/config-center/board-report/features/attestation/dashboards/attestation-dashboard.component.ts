...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('attestation.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalAttestations }}</div><div class="kpi-label">Total</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgCycleDays }}</div><div class="kpi-label">Avg Cycle (d)</div></div>
        </div>
        ...
        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              ...
              @for (item of data()!.breakdownA; track item.type) {
...
export class AttestationDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/attestation/dashboard')
