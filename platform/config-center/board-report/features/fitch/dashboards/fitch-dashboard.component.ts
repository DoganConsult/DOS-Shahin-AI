...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('fitch.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalProofs }}</div><div class="kpi-label">Total Proofs</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.rulesActive }}</div><div class="kpi-label">Active Rules</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgVerificationMs }}</div><div class="kpi-label">Avg Verify (ms)</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Rule Type</h3>
              @for (item of data()!.breakdownA; track item.ruleType) {
...
export class FitchDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/fitch/dashboard')
