import { Component, OnInit, inject, DestroyRef, ChangeDetectionStrategy} from '@angular/core';
import { CommonModule } from '@angular/common';
import { AppDatePipe } from '@app/shared/pipes/app-date.pipe';
import { FormsModule } from '@angular/forms';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { forkJoin, of } from 'rxjs';
import { catchError } from 'rxjs/operators';
import { GrcLiveService } from '../../core/interceptors/grc-live.service';
import { I18nService } from '@app/infrastructure/i18n/i18n.service';
import { PageShellComponent } from '../../../../../shared/components/layouts/page-shell.component';
import { StatusBadgeComponent } from '../../../../../shared/components/status-indicators/badges/status-badge.component';
import { AiPanelComponent } from '../../../../../shared/ai-panel/ai-panel.component';
import { TableModule } from 'primeng/table';
import { TagModule } from 'primeng/tag';
import { ToolbarModule } from 'primeng/toolbar';
import { ButtonModule } from 'primeng/button';
import { DialogModule } from 'primeng/dialog';
import { InputTextModule } from 'primeng/inputtext';
import { InputTextarea } from 'primeng/textarea';
import { DropdownModule } from 'primeng/select';
import { TooltipModule } from 'primeng/tooltip';
import { ToastModule } from 'primeng/toast';
import { MessageService } from 'primeng/api';
import { GrcOperationsService } from '@app/api';
import { GrcComplianceService } from '@app/grc/services/grc-compliance.service';
import { ApiClientService } from "@app/core/services/api-client.service";

@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
  selector: 'app-business-calendar',
  standalone: true,
  imports: [
    CommonModule, FormsModule,
    PageShellComponent, StatusBadgeComponent, AiPanelComponent,
    TableModule, TagModule, ToolbarModule, ButtonModule, DialogModule,
    InputTextModule, InputTextarea, DropdownModule, TooltipModule, ToastModule, AppDatePipe,],
  providers: [MessageService],
  template: `
    <app-page-shell
      icon="calendar"
      [title]="i18n.translate('businessCalendar.title')"
      [subtitle]="i18n.translate('businessCalendar.subtitle')"
      [breadcrumbs]="['Dashboard', 'Business Calendar']"
      [loading]="!loaded">

      <p-toast />

      <!-- KPI Cards -->
      <div class="kpi-row">
        <div class="kpi-card"><span class="kpi-value">{{ items.length }}</span><span class="kpi-label">{{ i18n.translate('businessCalendar.cadenceTasks') }}</span></div>
        <div class="kpi-card"><span class="kpi-value">{{ schedules.length }}</span><span class="kpi-label">{{ i18n.translate('businessCalendar.evidenceSchedules') }}</span></div>
        <div class="kpi-card"><span class="kpi-value">{{ pendingCount }}</span><span class="kpi-label">{{ i18n.translate('common.pending') }}</span></div>
        <div class="kpi-card"><span class="kpi-value">{{ overdueCount }}</span><span class="kpi-label">{{ i18n.translate('common.overdue') }}</span></div>
      </div>

      <!-- Tasks Section -->
      <h3 class="section-title">{{ i18n.translate('businessCalendar.cadenceTasks') }}</h3>

      <p-toolbar styleClass="mb-3">
        <ng-template pTemplate="start">
          <p-button
            [label]="i18n.translate('businessCalendar.addTask')"
            icon="pi pi-plus" (onClick)="openCreateDialog()" />
          <span class="p-input-icon-left ms-3">
            <i class="pi pi-search"></i>
            <input type="text" pInputText [(ngModel)]="searchTerm"
                   [placeholder]="i18n.translate('businessCalendar.search')" [attr.aria-label]="i18n.translate('businessCalendar.search')"
                   (input)="filterItems()" class="search-input" />
          </span>
        </ng-template>
        <ng-template pTemplate="end">
          <p-button [label]="i18n.translate('common.export')" icon="pi pi-download" severity="secondary"
                    [outlined]="true" (onClick)="exportCSV()" />
        </ng-template>
      </p-toolbar>

      <p-table aria-label="Filtered Items table" [value]="filteredItems" [paginator]="filteredItems.length > 10"
               [rows]="10" styleClass="p-datatable-striped p-datatable-gridlines"
               *ngIf="filteredItems.length > 0">
        <ng-template pTemplate="header">
          <tr>
            <th>{{ i18n.translate('businessCalendar.task') }}</th>
            <th>{{ i18n.translate('common.type') }}</th>
            <th>{{ i18n.translate('common.status') }}</th>
            <th>{{ i18n.translate('common.dueDate') }}</th>
            <th>{{ i18n.translate('businessCalendar.assigned') }}</th>
            <th style="width:120px">{{ i18n.translate('common.actions') }}</th>
          </tr>
        </ng-template>
        <ng-template pTemplate="body" let-item>
          <tr>
            <td><strong>{{ item.control_id ?? item.title ?? '—' }}</strong></td>
            <td>{{ item.task_type ?? item.period_type ?? '—' }}</td>
            <td><app-status-badge [status]="item.status ?? 'pending'" /></td>
            <td>{{ item.due_date | appDate:'medium' }}</td>
            <td>{{ item.assigned_to ?? '—' }}</td>
            <td>
              <div class="action-btns">
                <button aria-label="Edit" class="icon-btn" (click)="openEditDialog(item)" pTooltip="Edit"><i class="pi pi-pencil"></i></button>
                <button aria-label="Delete" class="icon-btn danger" (click)="confirmDelete(item)" pTooltip="Delete"><i class="pi pi-trash"></i></button>
              </div>
            </td>
          </tr>
        </ng-template>
        <ng-template pTemplate="emptymessage">
          <tr><td colspan="99" class="empty-msg">{{ i18n.translate('common.noData') }}</td></tr>
        </ng-template>
      </p-table>

      <div *ngIf="loaded && filteredItems.length === 0" class="empty-state">
        <i class="pi pi-inbox empty-icon"></i>
        <p>{{ i18n.translate('businessCalendar.noCadenceTasks') }}</p>
        <p-button [label]="i18n.translate('businessCalendar.addTask')" icon="pi pi-plus"
                  (onClick)="openCreateDialog()" [outlined]="true" />
      </div>

      <!-- Evidence Schedules Section (read-only) -->
      <h3 class="section-title mt-section">{{ i18n.translate('businessCalendar.evidenceSchedules') }}</h3>

      <p-table aria-label="Schedules table" [value]="schedules" [paginator]="schedules.length > 10"
               [rows]="10" styleClass="p-datatable-striped p-datatable-gridlines"
               *ngIf="schedules.length > 0">
        <ng-template pTemplate="header">
          <tr>
            <th>{{ i18n.translate('businessCalendar.control') }}</th>
            <th>{{ i18n.translate('businessCalendar.frequency') }}</th>
            <th>{{ i18n.translate('common.enabled') }}</th>
            <th>{{ i18n.translate('businessCalendar.lastReminded') }}</th>
          </tr>
        </ng-template>
        <ng-template pTemplate="body" let-s>
          <tr>
            <td>{{ s.control_id ?? '—' }}</td>
            <td>{{ s.cron_expression ?? s.frequency ?? '—' }}</td>
            <td>{{ s.enabled ? i18n.translate('common.yes') : i18n.translate('common.no') }}</td>
            <td>{{ s.last_reminded_at | appDate:'short' }}</td>
          </tr>
        </ng-template>
        <ng-template pTemplate="emptymessage">
          <tr><td colspan="99" class="empty-msg">{{ i18n.translate('businessCalendar.noEvidenceSchedules') }}</td></tr>
        </ng-template>
      </p-table>

      <div *ngIf="loaded && schedules.length === 0" class="empty-state">
        <i class="pi pi-calendar empty-icon"></i>
        <p>{{ i18n.translate('businessCalendar.noEvidenceSchedulesConfigured') }}</p>
      </div>

      <!-- Create/Edit Task Dialog -->
      <p-dialog
        [header]="editMode ? i18n.translate('businessCalendar.editTask') : i18n.translate('businessCalendar.addNewTask')"
        [(visible)]="showDialog" [modal]="true" [style]="{width:'560px'}">
        <div class="dialog-form">
          <div class="field">
            <label>{{ i18n.translate('businessCalendar.titleField') }}</label>
            <input pInputText [(ngModel)]="form.title" class="w-full" />
          </div>
          <div class="field-row">
            <div class="field">
              <label>{{ i18n.translate('businessCalendar.taskType') }}</label>
              <p-dropdown [(ngModel)]="form.task_type" [options]="taskTypeOptions"
                          optionLabel="label" optionValue="value" styleClass="w-full" />
            </div>
            <div class="field">
              <label>{{ i18n.translate('common.status') }}</label>
              <p-dropdown [(ngModel)]="form.status" [options]="statusOptions"
                          optionLabel="label" optionValue="value" styleClass="w-full" />
            </div>
          </div>
          <div class="field-row">
            <div class="field">
              <label>{{ i18n.translate('common.dueDate') }}</label>
              <input type="date" pInputText [(ngModel)]="form.due_date" class="w-full" />
            </div>
            <div class="field">
              <label>{{ i18n.translate('businessCalendar.assignedTo') }}</label>
              <input pInputText [(ngModel)]="form.assigned_to" class="w-full" />
            </div>
          </div>
        </div>
        <ng-template pTemplate="footer">
          <p-button [label]="i18n.translate('common.cancel')" icon="pi pi-times"
                    severity="secondary" [text]="true" (onClick)="showDialog = false" />
          <p-button [label]="i18n.translate('common.save')" icon="pi pi-check"
                    (onClick)="saveItem()" [disabled]="!form.title" />
        </ng-template>
      </p-dialog>

      <!-- Delete Confirmation Dialog -->
      <p-dialog [header]="i18n.translate('businessCalendar.confirmDelete')"
                [(visible)]="showDeleteDialog" [modal]="true" [style]="{width:'400px'}">
        <p>{{ i18n.translate('businessCalendar.confirmDeleteMessage') }}</p>
        <ng-template pTemplate="footer">
          <p-button [label]="i18n.translate('common.cancel')" severity="secondary" [text]="true"
                    (onClick)="showDeleteDialog = false" />
          <p-button [label]="i18n.translate('common.delete')" icon="pi pi-trash"
                    severity="danger" (onClick)="deleteItem()" />
        </ng-template>
      </p-dialog>

    </app-page-shell>
    <app-ai-panel module="business-calendar" />
  `,
  styles: [`
    .mb-3 { margin-bottom: var(--space-md); }
    .ms-3 { margin-inline-start: 12px; }
    .search-input { min-width: 220px; }
    .p-input-icon-left { position: relative; display: inline-flex; align-items: center; }
    .p-input-icon-left > i { position: absolute; inset-inline-start: 12px; color: var(--text-muted); z-index: var(--z-base); }
    .p-input-icon-left > input { padding-inline-start: 36px; }
    .empty-msg { text-align: center; color: var(--text-muted); padding: var(--space-xl); }
    .empty-state { text-align: center; padding: var(--space-2xl); color: var(--text-muted); }
    .empty-icon { font-size: 48px; margin-bottom: var(--space-md); display: block; }
    .action-btns { display: flex; gap: 4px; }
    .icon-btn { background: none; border: none; cursor: pointer; color: var(--text-muted); padding: 6px; border-radius: var(--radius-sm); transition: all 150ms; font-size: var(--font-size-base); }
    .icon-btn:hover { background: #e0f2fe; color: #0369a1; }
    .icon-btn.danger:hover { background: var(--status-danger-bg, #fff1f1); color: var(--error); }
    .dialog-form { display: flex; flex-direction: column; gap: 16px; }
    .field { display: flex; flex-direction: column; gap: 4px; }
    .field label { font-size: var(--font-size-sm); font-weight: 600; color: var(--text-muted); }
    .field-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
    .w-full { width: 100%; }
    .kpi-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; margin-bottom: 20px; }
    .kpi-card { background: var(--surface-card, #fff); border: 1px solid var(--surface-border, var(--border-subtle)); border-radius: var(--radius-lg); padding: 20px; text-align: center; }
    .kpi-value { display: block; font-size: var(--font-size-3xl); font-weight: 600; color: var(--primary-color, var(--primary)); }
    .kpi-label { display: block; font-size: var(--font-size-sm); color: var(--text-muted); margin-top: 4px; }
    .section-title { font-size: var(--font-size-lg); font-weight: 600; margin: 0 0 12px 0; color: var(--text-heading); }
    .mt-section { margin-top: 32px; }
  `]
})
export class BusinessCalendarComponent implements OnInit {
    private apiclientSvc = inject(ApiClientService);
    private complianceSvc = inject(GrcComplianceService);
  items: Record<string, unknown>[] = [];
  filteredItems: Record<string, unknown>[] = [];
  schedules: Record<string, unknown>[] = [];
  loaded = false;
  searchTerm = '';
  showDialog = false;
  showDeleteDialog = false;
  editMode = false;
  editingId: string | null = null;
  deleteTarget: Record<string, unknown> | null = null;
  pendingCount = 0;
  overdueCount = 0;
  form: Record<string, unknown> = { title: '', task_type: 'monthly', due_date: '', assigned_to: '', status: 'pending' };

  statusOptions = [
    { label: 'Pending', value: 'pending' },
    { label: 'In Progress', value: 'in_progress' },
    { label: 'Completed', value: 'completed' },
  ];

  taskTypeOptions = [
    { label: 'Daily', value: 'daily' },
    { label: 'Weekly', value: 'weekly' },
    { label: 'Monthly', value: 'monthly' },
    { label: 'Quarterly', value: 'quarterly' },
    { label: 'Annual', value: 'annual' },
  ];

  private live = inject(GrcLiveService);
  private destroyRef = inject(DestroyRef);

  constructor(
    public i18n: I18nService,
    private msg: MessageService, private operationsSvc: GrcOperationsService
  ) {}

  ngOnInit(): void {
    this.live.debounced(800).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.load());
    this.load();
  }

  load(): void {
    forkJoin({
      tasks: this.operationsSvc.getCadenceTasks().pipe(catchError(() => of({ tasks: [] }))),
      schedules: this.complianceSvc.getEvidenceSchedules().pipe(catchError(() => of({ schedules: [] }))),
    }).subscribe({
      next: ({ tasks: taskRes, schedules: schedRes }) => {
        const taskList = taskRes?.tasks ?? (Array.isArray(taskRes) ? taskRes : []);
        this.items = taskList;
        this.filterItems();
        this.pendingCount = taskList.filter((t: Record<string, unknown>) => t.status === 'pending' || t.status === 'in_progress').length;
        const now = new Date();
        this.overdueCount = taskList.filter((t: Record<string, unknown>) => t.due_date && new Date(t.due_date) < now && t.status !== 'completed').length;
        const schedList = schedRes?.schedules ?? (Array.isArray(schedRes) ? schedRes : []);
        this.schedules = schedList;
        this.loaded = true;
      },
      error: () => { this.loaded = true; }
    });
  }

  filterItems(): void {
    let r = this.items;
    if (this.searchTerm) {
      const t = this.searchTerm.toLowerCase();
      r = r.filter(i => (i.control_id ?? i.title ?? '').toLowerCase().includes(t) || (i.assigned_to ?? '').toLowerCase().includes(t));
    }
    this.filteredItems = r;
  }

  openCreateDialog(): void {
    this.editMode = false; this.editingId = null;
    this.form = { title: '', task_type: 'monthly', due_date: '', assigned_to: '', status: 'pending' };
    this.showDialog = true;
  }

  openEditDialog(item: Record<string, unknown>): void {
    this.editMode = true; this.editingId = item.task_id ?? item.id;
    this.form = {
      title: item.title ?? item.control_id ?? '',
      task_type: item.task_type ?? item.period_type ?? 'monthly',
      due_date: item.due_date ? item.due_date.substring(0, 10) : '',
      assigned_to: item.assigned_to ?? '',
      status: item.status ?? 'pending',
    };
    this.showDialog = true;
  }

  saveItem(): void {
    if (!this.form.title) return;
    const obs = this.editMode && this.editingId
      ? this.operationsSvc.updateCadenceTask(this.editingId, this.form)
      : this.apiclientSvc.post('/cadence/tasks', this.form);
    obs.subscribe({
      next: () => { this.showDialog = false; this.load(); this.msg.add({ severity: 'success', summary: this.i18n.translate('common.success'), detail: this.editMode ? this.i18n.translate('businessCalendar.updated') : this.i18n.translate('businessCalendar.created'), life: 3000 }); },
      error: () => { this.msg.add({ severity: 'error', summary: this.i18n.translate('common.error'), detail: this.i18n.translate('businessCalendar.operationFailed'), life: 4000 }); }
    });
  }

  confirmDelete(item: Record<string, unknown>): void { this.deleteTarget = item; this.showDeleteDialog = true; }

  deleteItem(): void {
    if (!this.deleteTarget) return;
    const id = this.deleteTarget.task_id ?? this.deleteTarget.id;
    this.apiclientSvc.del(`/cadence/tasks/${id}`).subscribe({
      next: () => { this.showDeleteDialog = false; this.deleteTarget = null; this.load(); this.msg.add({ severity: 'success', summary: this.i18n.translate('businessCalendar.deleted'), detail: this.i18n.translate('businessCalendar.recordRemoved'), life: 3000 }); },
      error: () => { this.msg.add({ severity: 'error', summary: this.i18n.translate('common.error'), detail: this.i18n.translate('businessCalendar.deleteFailed'), life: 4000 }); }
    });
  }

  exportCSV(): void {
    if (!this.filteredItems.length) return;
    const headers = Object.keys(this.filteredItems[0]);
    const csv = [headers.join(','), ...this.filteredItems.map(r => headers.map(h => `"${String(r[h] ?? '').replace(/"/g, '""')}"`).join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'business-calendar.csv'; a.click();
  }
}
