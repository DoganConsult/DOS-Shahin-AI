...
@Component({
    ...
    selector: 'app-governance-reviews',
    ...
    <div class="gov-page" [attr.dir]="dir()">
      <app-page-header titleEn="Reviews" titleAr="المراجعات" subtitleEn="Policy review cycles and outcomes" subtitleAr="دورات مراجعة السياسات ونتائجها" icon="search"
        [breadcrumbs]="[i18n.translate('Dashboard'), i18n.translate('Governance'), i18n.translate('Reviews')]" [actions]="headerActions" [isAr]="i18n.currentLang() === 'ar'" [dir]="dir()" (actionClick)="onHeaderAction($event)" />
      ...
      <div class="gov-body">
        ...
        <div class="health-strip">
          ...
          <div class="health-card"><div class="health-value" style="color:var(--success)">{{ approvedCount }}</div><div class="health-label">{{ i18n.translate('Approved') }}</div></div>
          ...
          <div class="health-card"><div class="health-value" style="color:var(--error)">{{ overdueCount }}</div><div class="health-label">{{ i18n.translate('Overdue') }}</div></div>
        </div>
        <div class="page-toolbar">
          <div class="toolbar-primary">
            <p-button [label]="i18n.translate('Add Review')" icon="pi pi-plus" (onClick)="openCreate()" />
          ...
          <div class="toolbar-secondary"><app-export-button module="governance-reviews" [data]="filteredItems" /></div>
        </div>
        <div class="table-shell" *ngIf="filteredItems.length > 0">
          <p-table aria-label="Filtered Items table" [value]="filteredItems" [paginator]="filteredItems.length > 10" [rows]="10" styleClass="p-datatable-striped p-datatable-sm">
            <ng-template pTemplate="header"><tr>
              <th>{{ i18n.translate('Policy ID') }}</th>
            ...
            <ng-template pTemplate="body" let-item><tr>
              ...
              <td><p-tag [value]="item.review_type" severity="info" /></td>
        ...
        <div class="empty-state" *ngIf="!loading && filteredItems.length === 0"><i class="pi pi-inbox empty-icon"></i><p>{{ i18n.translate('No reviews found') }}</p></div>
        <!-- Cross-Module Links -->
        <div class="cross-links">
          <button class="cross-link-btn" (click)="navigateTo('/governance/policies')"><i class="pi pi-file"></i> Policies</button>
          <button class="cross-link-btn" (click)="navigateTo('/governance/policy-reviews')"><i class="pi pi-search"></i> Policy Reviews</button>
    ...
    <p-dialog [header]="editMode ? (i18n.translate('Edit Review')) : (i18n.translate('Add Review'))" [(visible)]="showDialog" [modal]="true" [style]="{width:'520px'}" [dismissableMask]="true">
      <div class="dialog-form">
        <div class="field"><label>Policy ID</label><input pInputText [(ngModel)]="form.policy_id" class="w-full" /></div>
        <div class="field-row">
          <div class="field"><label>Type</label><p-dropdown [options]="typeOptions" [(ngModel)]="form.review_type" optionLabel="label" optionValue="value" class="w-full" /></div>
          <div class="field"><label>Outcome</label><p-dropdown [options]="outcomeOptions" [(ngModel)]="form.outcome" optionLabel="label" optionValue="value" class="w-full" /></div>
        </div>
        <div class="field"><label>Comments</label><textarea pInputTextarea [(ngModel)]="form.comments" rows="3" class="w-full"></textarea></div>
        <div class="field"><label>Next Review Date</label><input pInputText type="date" [(ngModel)]="form.next_review_date" class="w-full" /></div>
      </div>
...
export class GovernanceReviewsComponent implements OnInit {
 ...
 private msg = inject(MessageService);
  ...
  readonly headerActions: PageHeaderAction[] = [{ id: 'add', labelEn: 'Add Review', labelAr: 'إضافة مراجعة', icon: 'plus', primary: true }];
  onHeaderAction(id: string): void { if (id === 'add') this.openCreate(); }
  typeOptions = [{ label: 'Periodic', value: 'periodic' }, { label: 'Triggered', value: 'triggered' }, { label: 'Ad Hoc', value: 'ad_hoc' }, { label: 'Regulatory', value: 'regulatory' }, { label: 'Incident', value: 'incident' }];
  outcomeOptions = [{ label: 'Pending', value: 'pending' }, { label: 'Approved', value: 'approved' }, { label: 'Changes Required', value: 'changes_required' }, { label: 'Rejected', value: 'rejected' }, { label: 'Deferred', value: 'deferred' }];
  loading = true; items: Record<string, any>[] = []; filteredItems: Record<string, any>[] = []; searchTerm = ''; showDialog = false; editMode = false; editId = ''; form: Record<string, any> = {}; overdueCount = 0; approvedCount = 0; pendingCount = 0;

  ngOnInit(): void { this.load(); }
  load(): void {
    this.loading = true;
    this.apiclientSvc.get('/governance/reviews').subscribe({
      next: res => { this.items = res.reviews || []; this.approvedCount = this.items.filter(i => i.outcome === 'approved').length; this.pendingCount = this.items.filter(i => i.outcome === 'pending').length; this.overdueCount = this.items.filter(i => i.next_review_date && new Date(i.next_review_date) < new Date() && i.outcome !== 'approved').length; this.filterList(); this.loading = false; },
      error: () => { this.loading = false; }
    });
  }
  filterList(): void { const s = this.searchTerm.toLowerCase(); this.filteredItems = this.items.filter(i => !s || (i.policy_id || '').toLowerCase().includes(s) || (i.comments || '').toLowerCase().includes(s)); }
  openCreate(): void { this.editMode = false; this.form = { policy_id: '', review_type: 'periodic', outcome: 'pending', comments: '', next_review_date: '' }; this.showDialog = true; }
  openEdit(item: Record<string, any>): void { this.editMode = true; this.editId = item.review_id; this.form = { ...item }; this.showDialog = true; }
  save(): void {
    const obs = this.editMode ? this.apiclientSvc.put(`/governance/reviews/${this.editId}`, this.form) : this.apiclientSvc.post('/governance/reviews', this.form);
  ...
  deleteItem(item: Record<string, any>): void { this.apiclientSvc.del(`/governance/reviews/${item.review_id}`).subscribe({ next: () => this.load() }); }
