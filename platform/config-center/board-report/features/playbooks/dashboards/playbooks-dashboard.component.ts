...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('playbooks.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalPlaybooks }}</div><div class="kpi-label">Total Playbooks</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.executionsToday }}</div><div class="kpi-label">Runs Today</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgSteps }}</div><div class="kpi-label">Avg Steps</div></div>
...
export class PlaybooksDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/playbooks/dashboard')
