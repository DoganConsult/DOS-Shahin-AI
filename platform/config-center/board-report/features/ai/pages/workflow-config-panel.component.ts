// AI Cockpit — Workflow Config Panel page.
...
interface WorkflowVersion {
  versionId: string; workflowKey: string; version: number; status: string; updatedAt?: string;
}
...
@Component({
  ...
  templateUrl: './workflow-config-panel.component.html',
})
export class WorkflowConfigPanelComponent implements OnInit {
  readonly i18n = inject(I18nService);
  private readonly api = inject(ApiClientService);

  readonly rows = signal<WorkflowVersion[]>([]);
  readonly state = signal<'loading' | 'ready' | 'error' | 'empty'>('loading');
  readonly error = signal<string | null>(null);

  ngOnInit(): void {
    this.api.get<{ data?: WorkflowVersion[] }>('/ai-engine/agrc-os/workflow-versions').subscribe({
      next: (res) => {
