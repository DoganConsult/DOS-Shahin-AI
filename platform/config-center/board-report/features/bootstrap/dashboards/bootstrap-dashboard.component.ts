...
interface BootstrapDashboardData {
...
@Component({
  ...
  selector: 'app-bootstrap-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('bootstrap.dashboard') }}</h2>
      @if (loading()) {
...
export class BootstrapModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<BootstrapDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: BootstrapDashboardData }>(`/api/bootstrap/dashboard`)
