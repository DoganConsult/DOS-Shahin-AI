...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('integrations.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalConnectors }}</div><div class="kpi-label">Total Connectors</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.syncing }}</div><div class="kpi-label">Syncing</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.disabled }}</div><div class="kpi-label">Disabled</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.eventsToday }}</div><div class="kpi-label">Events Today</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgLatencyMs }}</div><div class="kpi-label">Avg Latency (ms)</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.errorRate }}</div><div class="kpi-label">Error Rate %</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Connector Type</h3>
              @for (item of data()!.breakdownA; track item.connectorType) {
...
export class IntegrationsDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/integrations/dashboard')
