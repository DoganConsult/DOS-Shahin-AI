...
export class ConnectorHealthComponent implements OnInit {
  ...
  loading = false; connectors: Record<string, any>[] = []; syncLogs: Record<string, any>[] = [];
  testingAll = false; showLogDialog = false; selectedConnector: Record<string, any> | null = null;
  filterStatus = '';
  statusOptions = [
  ...
  filteredConnectors(): Record<string, any>[] {
  ...
  showLogs(c: Record<string, any>) {
    this.selectedConnector = c;
    this.apiclientSvc.get(`/connectors/${c.connector_id || c.id}/logs`).subscribe({
      next: (d: Record<string, any>) => { this.syncLogs = Array.isArray(d) ? d : d.logs || []; this.showLogDialog = true; },
      error: () => { this.syncLogs = []; this.showLogDialog = true; }
    });
  }

  statusSeverity(s: string): 'success' | 'warning' | 'danger' | 'info' {
    const m: Record<string, any> = { healthy: 'success', degraded: 'warning', error: 'danger', offline: 'danger' };
    return m[s] || 'info';
  }
