...
// empty marketplace (NO FRONTEND INVENTION).
...
@Component({
    ...
    imports: [CommonModule, FormsModule, InputTextModule, TagModule, ButtonModule, DialogModule, PasswordModule, DropdownModule, ToastModule, TooltipModule, ConfirmDialogModule],
  ...
  `,
    ...
    .cat-count { background: var(--surface-100, var(--surface-ice)); padding: 1px 6px; border-radius: var(--radius-md); font-size: var(--font-size-nano); }
    ...
    .catalog-card:hover { border-color: var(--primary-300, #93c5fd); box-shadow: 0 2px 8px rgba(var(--color-black-rgb), .06); }
    ...
    .health-stat { font-size: var(--font-size-xs); color: var(--text-muted); display: flex; align-items: center; gap: 4px; }
    .card-produces { display: flex; align-items: center; gap: 4px; margin-bottom: 8px; flex-wrap: wrap; }
    .produces-label { font-size: var(--font-size-nano); color: var(--text-muted); font-weight: 600; }
    .produce-chip { font-size: var(--font-size-nano); padding: 1px 6px; border-radius: var(--radius); background: var(--primary-50, #eff6ff); color: var(--primary, #2563eb); }
    .card-tags { display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 12px; }
    .tag-chip { font-size: var(--font-size-nano); padding: 2px 8px; border-radius: var(--radius-md); background: var(--surface-100, var(--surface-ice)); color: var(--text-muted); }
...
export class IntegrationMarketplaceComponent implements OnInit {
  ...
  private loadAll(): void {
    forkJoin({
      ...
      next: ({ connectors: c, configs: cfg, registry }) => {
        ...
        if ((registry as any)?.connectors?.length) {
          this.catalog = ((registry as any).connectors as any[]).map(mapRegistryToCatalog);
          this.registryLoaded = true;
        } else {
          this.catalog = [];
        }

        const connArr = Array.isArray(c) ? c : ((c as any)?.connectors ?? []);
        const cfgArr = Array.isArray(cfg) ? cfg : ((cfg as any)?.configs ?? (cfg as any)?.data ?? []);
  ...
  testConnection(): void {
    ...
    this.operationsSvc.testConnector({ sourceSystemType: this.installEntry.connectorType!, credentials: { ...this.installCredentials, platform: this.installPlatform.value }, platform: this.installPlatform.value } as any).subscribe({
      next: (r) => { this.installTestResult = r as any; this.installTesting = false; },
  ...
  createConnector(): void {
    ...
    this.operationsSvc.createConnector({ name: `${this.installEntry.name} Connector`, sourceSystemType: this.installEntry.connectorType!, authMethod: this.installPlatform.authMethod, credentials: { ...this.installCredentials, platform: this.installPlatform.value }, schedule: sched } as any).subscribe({
  ...
  syncNow(entry: CatalogEntry): void {
    ...
    this.operationsSvc.runConnector(c.connector_id).subscribe({
      next: (r) => { this.syncingId.set(null); this.msg.add({ severity: 'success', summary: 'Synced', detail: `${(r as any).recordsCollected ?? 0} records` }); this.loadAll(); },
  ...
  removeConnector(entry: CatalogEntry): void {
    ...
    this.confirmationService.confirm({
      message: `Remove ${entry.name} connector?`,
