...
interface AgrcEngineDashboardData {
...
@Component({
  ...
  selector: 'app-agrc-engine-module-dashboard',
  standalone: true,
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('agrcEngine.dashboard') }}</h2>
      @if (loading()) {
...
export class AgrcEngineModuleDashboardComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  private http = inject(HttpClient);
  i18n = inject(I18nService);

  loading = signal(true);
  data = signal<AgrcEngineDashboardData | null>(null);
  kpiEntries = signal<[string, number][]>([]);

  ngOnInit(): void {
    this.http.get<{ data: AgrcEngineDashboardData }>(`/api/agrc-engine/dashboard`)
