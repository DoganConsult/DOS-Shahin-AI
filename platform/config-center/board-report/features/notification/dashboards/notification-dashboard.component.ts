...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('notification.dashboard') }}</h2>

      @if (loading()) {
        <p>{{ i18n.translate('common.loading') }}...</p>
      } @else if (data()) {
        <div class="kpi-grid">
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.totalSent }}</div><div class="kpi-label">Total Sent</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.delivered }}</div><div class="kpi-label">Delivered</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.failed }}</div><div class="kpi-label">Failed</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.readRate }}</div><div class="kpi-label">Read Rate %</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.channelsActive }}</div><div class="kpi-label">Channels Active</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgDeliveryMs }}</div><div class="kpi-label">Avg Delivery (ms)</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.suppressedToday }}</div><div class="kpi-label">Suppressed Today</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Channel</h3>
              @for (item of data()!.breakdownA; track item.channel) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Event Type</h3>
              @for (item of data()!.breakdownB; track item.eventType) {
                <div class="breakdown-row">
...
export class NotificationDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/notification/dashboard')
