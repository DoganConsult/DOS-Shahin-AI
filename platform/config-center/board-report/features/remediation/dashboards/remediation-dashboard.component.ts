...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('remediation.dashboard') }}</h2>
      ...
      @if (loading()) {
        ...
        <div class="kpi-grid">
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--orange-500)">{{ data()!.kpis.blocked }}</div><div class="kpi-label">Blocked</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.verified }}</div><div class="kpi-label">Verified</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.closed }}</div><div class="kpi-label">Closed</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgClosureDays }}</div><div class="kpi-label">Avg Closure (d)</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.escalated }}</div><div class="kpi-label">Escalated</div></div>
        </div>
        ...
        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              ...
              @for (item of data()!.breakdownA; track item.source) {
...
export class RemediationDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/remediation/dashboard')
