...
export class ReportScenarioComponent implements OnInit {
  ...
  loading = false; scenarios: Record<string, unknown>[] = []; schedules: Record<string, unknown>[] = [];
  ...
  ngOnInit() {
    ...
    this.operationsSvc.getReportScenarios().subscribe({
      next: (d: Record<string, unknown>) => { this.scenarios = Array.isArray(d) ? d : d.scenarios || []; this.loading = false; this.cdr.markForCheck(); },
  ...
  generate(s: Record<string, unknown>) {
