...
beforeAll(async () => {
  const mod = await import('./portals.module');
  manifest = mod.PORTALS_MANIFEST ?? mod.default?.manifest;
});

describe('Portals Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('portals'); });
