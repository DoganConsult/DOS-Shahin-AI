...
import {
  ...
  getSuspendedPortals,
  getDraftPages,
} from './portals-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getSuspendedPortals should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getSuspendedPortals('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getSuspendedPortals should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getSuspendedPortals('t1');
      expect(result).toEqual([]);
    });

    it('getDraftPages should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getDraftPages('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getDraftPages should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getDraftPages('t1');
      expect(result).toEqual([]);
    });
