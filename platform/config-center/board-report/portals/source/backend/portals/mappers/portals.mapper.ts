import type { Portal, PortalCreateInput } from '../types/portals.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): Portal {
  return {
    portal_id: row.portal_id ?? row.id,
...
export function toEntityList(rows: GenericRow[]): Portal[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): PortalCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: Portal): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: Portal[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
