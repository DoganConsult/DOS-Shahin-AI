...
export class ReportGeneratorComponent implements OnInit {
  ...
  generate() {
    const key = this.selectedTemplate?.key || this.selectedTemplate?.name;
    try {
      const params = JSON.parse(this.generateParams);
      this.generateLoading = true;
      this.operationsSvc.generateReport(key, params).subscribe({
        next: (d) => {
          this.generatedReports.unshift({ templateKey: key, generatedAt: new Date().toISOString(), data: d });
        ...
        error: (e) => {
          this.generateLoading = false;
          this.cdr.markForCheck();
          this.msg.add({ severity: 'error', summary: this.i18n.translate('common.error'), detail: e.error?.error || e.message });
        }
      });
    } catch {
  ...
  viewReport(r: GrcRecord) { this.viewingReport = r.data; this.showReportView = true; }
}
