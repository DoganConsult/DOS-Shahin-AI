...
export async function createEscalation(input: CreateEscalationInput): Promise<WorkflowEscalation> {
  ...
  try {
    ...
    logger.info('[WorkflowEscalation] Escalation created', {
      escalationId,
    ...
    const escalation = await getEscalation(escalationId, input.tenantId);
    if (!escalation) throw new Error('Failed to retrieve created escalation');
    return escalation;
  } catch (err) {
