...
export async function createTemplate(input: CreateTemplateInput): Promise<WorkflowTemplate> {
  ...
  const parametersSchema = input.parametersSchema ? JSON.stringify(input.parametersSchema) : '{}';

  try {
    // Check for duplicate template_code within tenant scope
    const existing = await safeQuery(
      `SELECT template_id FROM dos.workflow_templates
       WHERE template_code = $1 AND (tenant_id = $2 OR ($2::uuid IS NULL AND tenant_id IS NULL))`,
      [input.templateCode, input.tenantId || null],
    );

    if (existing.rows.length > 0) {
      throw new Error(`Template with code '${input.templateCode}' already exists`);
    }

    await safeQuery(
      `INSERT INTO dos.workflow_templates
         (template_id, tenant_id, template_code, name, description, category,
          definition, parameters_schema, is_active, version, created_by, created_at, updated_at)
    ...
    await _createVersionRecord(templateId, 1, input.definition, input.parametersSchema || {}, 'Initial version', input.createdBy);

    logger.info('[WorkflowTemplate] Template created', {
      templateId,
    ...
    logger.error('[WorkflowTemplate] Failed to create template', { templateCode: input.templateCode, error: toErrorMessage(err) });
    throw err;
  }
}
...
export async function importTemplate(
  ...
  const existing = await getTemplateByCode(serialized.templateCode, tenantId);

  if (existing) {
    // Update existing template with imported definition
    const updated = await updateTemplate(existing.template_id, {
    ...
    if (!updated) throw new Error(`Failed to update existing template ${serialized.templateCode} during import`);
    logger.info('[WorkflowTemplate] Template updated via import', { templateCode: serialized.templateCode, tenantId });
    return updated;
  }
