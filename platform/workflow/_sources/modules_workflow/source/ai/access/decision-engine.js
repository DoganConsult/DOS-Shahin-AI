...
async function evaluateAccess(ctx) {
    ...
    if (userRoles.length > 1) {
       ...
       LIMIT 1`, [userRoles]);
        ...
        if (!s11passed)
            return denyAndLog(schema, ctx, 11, 'sod_passes', `SoD conflict blocks action: ${sodCheck.rows[0].role_code_a} vs ${sodCheck.rows[0].role_code_b}`, steps);
    }
    else {
    ...
    if (ctx.lifecycleFromState && ctx.lifecycleToState && ctx.entityType) {
       ...
       LIMIT 1`, [moduleCode, ctx.lifecycleFromState, ctx.lifecycleToState, ctx.permissionCode]);
        ...
        if (!s12passed)
            return denyAndLog(schema, ctx, 12, 'lifecycle_transition_allowed', 'Lifecycle transition not permitted', steps);
    }
    else {
    ...
    await logDecision(schema, ctx, { allowed: true, failedStep: null, failedCheck: null, reason: 'All 14 checks passed', steps, matchedRole: ctx.role }).catch((0, resilience_1.catchHandler)(resilience_1.EC.DB_CLEANUP));
    ...
    observability_1.logger.info(`[DAuth] ALLOW perm=${ctx.permissionCode} user=${ctx.userId} tenant=${ctx.tenantId} (${pipelineDurationMs}ms)`, {
        dauth: true,
        decision: 'allow',
        userId: ctx.userId,
...
// ── Helpers ──
function deny(step, check, reason, steps) {
...
async function denyAndLog(schema, ctx, step, check, reason, steps) {
    const decision = deny(step, check, reason, steps);
    const pipelineDurationMs = Date.now() - ctx._pipelineStartMs || 0;
    observability_1.logger.warn(`[DAuth] DENY step=${step} check=${check} perm=${ctx.permissionCode} user=${ctx.userId} tenant=${ctx.tenantId} reason=${reason}`, {
        dauth: true,
        decision: 'deny',
        failedStep: step,
        failedCheck: check,
...
function extractSodResult(steps) {
...
async function logDecision(schema, ctx, decision) {
    ...
    observability_1.logger.info(`DAuth ${decision.allowed ? 'ALLOW' : 'DENY'}: ${ctx.permissionCode}`, {
