...
import { ProvisioningStepRunnerService } from '../../modules/onboarding/services/provisioning/provisioning-step-runner.service';
import { ProvisioningJobRepo } from '../../modules/onboarding/repositories/provisioning-job.repo';
import { ProvisioningStepRepo } from '../../modules/onboarding/repositories/provisioning-step.repo';
import { mapAnswersToNormalizedProfile } from '../../modules/onboarding/mappers/normalized-profile.mapper';
import { resolveRegulatoryProfile } from '../../modules/compliance/services/regulatory/regulatory-resolution.service';
...
export async function completeProvisioningJob(
  ...
  const { OnboardingSessionRepo } = await import('../../modules/onboarding/repositories/onboarding-session.repo.js');
...
export async function seedOpenFGATuples(ctx: ProvisioningContext): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const { openfgaConnected } = await import('@dos/platform-core');
    if (!openfgaConnected()) {
    ...
    const { CANONICAL_MODULES } = await import('../../config/canonical-modules.js');
    const moduleCodes = Array.from(CANONICAL_MODULES);
    await seedModuleTuples(ctx.workspaceId || ctx.tenantId, moduleCodes);
    return { step: 'seed_openfga_tuples', tuplesSeeded: true, moduleCount: moduleCodes.length };
  } catch (err: unknown) {
