...
import { createNotification as _createNotification } from '../../modules/notification/services/notification.service';
...
export async function aiGenerateAuditReport(tenantId: string, auditId: string): Promise<Record<string, unknown>> {
  ...
  const [audit, findings, evidence] = await Promise.all([
    swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(`SELECT * FROM "${schema}".audits WHERE audit_id = $1`, [auditId]), { tenantId: tenantId, operation: 'query audits' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(`SELECT * FROM "${schema}".audit_findings WHERE audit_id = $1`, [auditId]), { tenantId: tenantId, operation: 'query audits' }),
    swallowDefault(EC.FALLBACK_QUERY, emptyResult(), safeQuery(`SELECT * FROM "${schema}".audit_evidence WHERE audit_id = $1`, [auditId]), { tenantId: tenantId, operation: 'query audits' }),
  ]);
