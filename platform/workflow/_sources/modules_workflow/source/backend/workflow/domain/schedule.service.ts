...
export async function registerScheduledJob(input: RegisterJobInput): Promise<ScheduledJob> {
  ...
  await query(
    `INSERT INTO dos.workflow_scheduled_jobs
       (job_id, tenant_id, name, cron_expression, job_type, payload, is_active, created_by,
        trigger_count, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, true, $7, 0, NOW(), NOW())`,
    [jobId, input.tenantId, input.name, input.cronExpression, input.jobType, payload, input.createdBy || null],
  );
