...
export async function getApprovalDetail(
  ...
  const schema = tenantSchema(tenantId);
  const result = await safeQuery(
    `SELECT * FROM "${schema}".approval_requests WHERE approval_id = $1 LIMIT 1`,
    [approvalId],
  );
  ...
  return mapApprovalRow(row as GenericRow);
}
...
export async function submitDecision(
  ...
  const schema = tenantSchema(tenantId);
  if (decision.decision === 'delegated' && !decision.delegateTo) {
    throw new Error('delegateTo is required when decision is delegated');
  }

  if (decision.decision === 'delegated') {
    await safeQuery(
      `UPDATE "${schema}".approval_requests
       SET current_approver_id = $1, status = 'pending', updated_at = NOW()
       WHERE approval_id = $2`,
      [decision.delegateTo, approvalId],
    );
    return;
  }
  ...
  await safeQuery(
    `UPDATE "${schema}".approval_requests
     SET status = $1, decision_comment = $2, decided_at = NOW(), updated_at = NOW()
     WHERE approval_id = $3`,
    [decision.decision, decision.reason ?? null, approvalId],
  );
}
