...
import type { WorkflowInstance, WorkflowInstanceCreateInput } from '@dos/types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): WorkflowInstance {
  return {
...
export function toEntityList(rows: GenericRow[]): WorkflowInstance[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): WorkflowInstanceCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as WorkflowInstanceCreateInput;
}

export function toApiResponse(entity: WorkflowInstance): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: WorkflowInstance[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
