  ...
  try {
    await safeQuery(
      `INSERT INTO dos.workflow_tasks
         (task_id, tenant_id, instance_id, task_type, title, description, status, due_at, context, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending', $7, $8, NOW(), NOW())`,
      [taskId, input.tenantId, input.instanceId, input.taskType, input.title, input.description || null, input.dueAt || null, context],
    );
