...
import { WorkItemsApiService, type WorkItemDto } from '@app/core/services/features/workflow/services/work-items-api.service';
        ...
        <table aria-label="Work Items Inbox table" class="w-full text-sm">
          <thead>
            <tr class="border-b border-[var(--border)] bg-[var(--bg-2)]">
              ...
              <th class="text-left p-3 font-medium">Instance</th>
          ...
          <tbody>
            @for (item of items(); track item.id) {
              <tr class="border-b border-[var(--border)] hover:bg-[var(--bg-2)]">
                <td class="p-3">{{ item.title || item.nodeId }}</td>
                <td class="p-3"><span class="rounded-full px-2 py-0.5 text-xs" [class]="statusClass(item.status)">{{ item.status }}</span></td>
                <td class="p-3">{{ item.dueAtUtc ? (item.dueAtUtc | appDate:'short') : '—' }}</td>
                <td class="p-3">{{ item.instanceId }}</td>
...
export class WorkItemsInboxPageComponent implements OnInit {
  ...
  private load(): void {
    this.api.list({ workspaceId: this.workspaceId() || undefined }).subscribe({
  ...
  statusClass(status: string): string {
    ...
    if (status === 'Open') return 'bg-[var(--info)]/20 text-[var(--info)]';
