// Shahin GRC — Workflow Hub  |  GRC Lifecycle: IMPROVE
...
const TABS: HubTab[] = [
  { key: 'workflows',  labelEn: 'Workflows',          labelAr: 'سير العمل',       icon: 'pi-sitemap' },
  { key: 'templates',  labelEn: 'Workflow Templates', labelAr: 'قوالب سير العمل', icon: 'pi-copy' },
  { key: 'autonomous', labelEn: 'AI Autonomous',      labelAr: 'الذكاء الاصطناعي المستقل', icon: 'pi-bolt' },
  { key: 'chains',     labelEn: 'Chains',             labelAr: 'السلاسل',          icon: 'pi-link' },
  { key: 'automation', labelEn: 'Automation Rules',   labelAr: 'قواعد الأتمتة',     icon: 'pi-cog' },
  { key: 'lifecycle',  labelEn: 'Lifecycle',          labelAr: 'دورة الحياة',       icon: 'pi-sync' },
  { key: 'metrics',    labelEn: 'Metrics',            labelAr: 'المقاييس',           icon: 'pi-chart-bar' },
];
...
@Component({
    ...
    imports: [CommonModule, RouterModule, WorkflowsComponent, WorkflowTemplatesComponent, WorkflowChainsTabComponent, AutomationRulesTabComponent, LifecycleTabComponent, WorkflowMetricsTabComponent, AgentBadgeComponent, HubConnectionsStripComponent, RaciPanelComponent],
    styles: [HUB_STYLES],
    template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#e0e7ff"><i class="pi pi-sitemap hub-icon" style="color:#4338ca"></i></div>
          <div>
            <div class="hub-phase" style="background:rgba(var(--module-accent-indigo-rgb), .12);color:#4338ca">{{ i18n.currentLang()==='ar' ? 'التحسين' : 'Improve' }}</div>
            <h1>{{ i18n.currentLang()==='ar' ? 'مركز سير العمل' : 'Workflow Hub' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'سير العمل والقوالب — أتمتة عمليات GRC.' : 'Workflows, templates — automate your GRC processes.' }}</p>
          </div>
        </div>
      ...
      <app-hub-connections-strip [hubKey]="'workflow'" />
      <div class="hub-tab-content">
        @if (activeTab()==='workflows') { <app-workflows [initialTab]="wfTab()" /> }
        @if (activeTab()==='templates') { <app-workflow-templates /> }
        @if (activeTab()==='autonomous') { <app-autonomous-workflow [embedded]="true" /> }
        @if (activeTab()==='chains') { <app-workflow-chains-tab /> }
        @if (activeTab()==='automation') { <app-automation-rules-tab /> }
        @if (activeTab()==='lifecycle') { <app-lifecycle-tab /> }
        @if (activeTab()==='metrics') { <app-workflow-metrics-tab /> }
      </div>
    </div>`
})
export class WorkflowHubComponent implements OnInit {
  i18n = inject(I18nService);
  private route = inject(ActivatedRoute);
  private router = inject(Router);
  tabs = TABS;
  readonly agentId = 'A08';
  activeTab = signal<string>('workflows');
  wfTab = signal<string | null>(null);

  ngOnInit(): void {
    this.route.queryParamMap.subscribe(p => {
      const tab = p.get('tab');
      if (tab && TABS.some(t => t.key === tab)) this.activeTab.set(tab);
      const wf = p.get('wfTab');
      this.wfTab.set(wf ?? null);
    });
  }

  openDesigner(): void {
    this.router.navigate(['/workflow-hub'], {
      queryParams: { tab: 'workflows', wfTab: 'designer' },
      replaceUrl: true,
    });
  }
}
