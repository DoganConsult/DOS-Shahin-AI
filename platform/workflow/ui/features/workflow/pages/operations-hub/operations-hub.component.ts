// Shahin GRC — OperationsHub  |  GRC Lifecycle: Operate
...
import { CalendarViewComponent } from '@app/shared/calendar/calendar-view.component';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';

interface HubTab { key: string; labelEn: string; labelAr: string; icon: string; }
const TABS: HubTab[] = [
  { key: 'timeline',  labelEn: 'Timeline',      labelAr: 'الجدول الزمني',    icon: 'pi-calendar' },
  { key: 'messaging', labelEn: 'Messaging',     labelAr: 'المراسلة',          icon: 'pi-comments' },
  { key: 'actions',   labelEn: 'Action Items',  labelAr: 'عناصر الإجراءات',  icon: 'pi-check-square' },
  { key: 'activity',  labelEn: 'Activity Feed', labelAr: 'تغذية النشاط',     icon: 'pi-history' },
  { key: 'calendar',  labelEn: 'Calendar',      labelAr: 'التقويم',          icon: 'pi-calendar-plus' },
];
...
const HUB_STYLES = `
  ...
  .hub-context{margin:6px 0 0;font-size: var(--font-size-sm);color:var(--text-muted,var(--text-muted));line-height:1.45;max-width:720px}
...
@Component({
    ...
    imports: [CommonModule, RouterModule, TimelineComponent, AgentBadgeComponent, HubConnectionsStripComponent, HubHelpPanelComponent, CalendarViewComponent],
    styles: [HUB_STYLES],
    template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap" style="background:#f3e8ff"><i class="pi pi-cog hub-icon" style="color:#7e22ce"></i></div>
          <div>
            ...
            <h1>{{ i18n.translate('operationsHub.title') }}</h1>
            <p class="hub-subtitle">{{ i18n.translate('operationsHub.subtitle') }}</p>
            <p class="hub-context" *ngIf="showContext()">{{ i18n.translate('operationsHub.panelDescription') }} {{ i18n.translate('operationsHub.pastEventsOrder') }} {{ i18n.translate('operationsHub.inWorkspace') }}</p>
          </div>
        </div>
      ...
      <app-hub-connections-strip [hubKey]="'operations'" />
      <app-hub-help-panel [hubRoute]="'/operations-hub'" />
      <div class="hub-tab-content">
        @if (activeTab()==='timeline')  { <app-timeline /> }
        @if (activeTab()==='messaging') { <app-messaging /> }
        @if (activeTab()==='actions')   { <app-action-items /> }
        @if (activeTab()==='activity')  { <app-activity-feed /> }
        @if (activeTab()==='calendar')  { <app-calendar-view [events]="[]" [locale]="i18n.currentLang()" [direction]="i18n.direction()" /> }
      </div>
    </div>`
})
export class OperationsHubComponent implements OnInit {
  ...
  activeTab = signal<string>('timeline');
  showContext = () => true;
  ngOnInit(): void {
