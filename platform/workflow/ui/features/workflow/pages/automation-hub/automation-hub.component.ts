// Shahin GRC — AutomationHub  |  GRC Lifecycle: Improve
...
const TABS: HubTab[] = [
  { key: 'automation', labelEn: 'Automation', labelAr: 'الأتمتة', icon: 'pi-cog' },
];
...
@Component({
  ...
  selector: 'app-automation-hub', standalone: true,
  imports: [CommonModule, RouterModule, AutomationComponent, AgentBadgeComponent, HubConnectionsStripComponent],
  styles: [HUB_STYLES],
  template: `
    <div class="grc-hub" [dir]="i18n.direction()">
      <header class="hub-header">
        <div class="hub-title-row">
          <div class="hub-icon-wrap hub-icon-wrap--improve"><i class="pi pi-bolt hub-icon hub-icon--improve"></i></div>
          <div>
            <div class="hub-phase hub-phase--improve">{{ i18n.currentLang()==='ar' ? 'التحسين' : 'Improve' }}</div>
            <h1>{{ i18n.currentLang()==='ar' ? 'مركز الأتمتة' : 'Automation Hub' }}</h1>
            <p class="hub-subtitle">{{ i18n.currentLang()==='ar' ? 'الأتمتة وسير العمل المستقل — تنفيذ GRC بالذكاء الاصطناعي.' : 'Automation and autonomous workflows — AI-driven GRC execution.' }}</p>
          </div>
        </div>
      ...
      <app-hub-connections-strip [hubKey]="'automation'" />
      <div class="hub-tab-content">
        @if (activeTab()==='automation') { <app-automation /> }
      </div>
    </div>`
})
export class AutomationHubComponent implements OnInit {
  private destroyRef = inject(DestroyRef);
  i18n = inject(I18nService);
  private route = inject(ActivatedRoute);
  tabs = TABS;
  readonly agentId = 'A01';
  activeTab = signal<string>('automation');
  ngOnInit(): void {
