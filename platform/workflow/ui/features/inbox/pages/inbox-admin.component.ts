...
@Component({
  ...
  selector: 'app-inbox-admin',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="admin-shell" [dir]="i18n.direction()">
      <h2>Inbox — Administration</h2>
...
export class InboxAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Array<{ key: string; value: string }> }>(`/api/inbox/admin/settings`).pipe(
    ...
    this.http.get<{ data: { overallHealth?: string } }>(`/api/inbox/admin/diagnostics`).pipe(
      takeUntilDestroyed(this.destroyRef),
