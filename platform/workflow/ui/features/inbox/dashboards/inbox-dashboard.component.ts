...
@Component({
  ...
  template: `
    <div class="dashboard-page" [dir]="i18n.direction()">
      <h2 class="page-title">{{ i18n.translate('inbox.dashboard') }}</h2>
      ...
      @if (loading()) {
        ...
        <div class="kpi-grid">
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--green-500)">{{ data()!.kpis.unread }}</div><div class="kpi-label">Unread</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.actionRequired }}</div><div class="kpi-label">Action Required</div></div>
          ...
          <div class="kpi-card"><div class="kpi-value" style="color:var(--yellow-500)">{{ data()!.kpis.delegated }}</div><div class="kpi-label">Delegated</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.completedToday }}</div><div class="kpi-label">Completed Today</div></div>
          <div class="kpi-card"><div class="kpi-value">{{ data()!.kpis.avgResponseHours }}</div><div class="kpi-label">Avg Response (hrs)</div></div>
          <div class="kpi-card"><div class="kpi-value" style="color:var(--red-500)">{{ data()!.kpis.escalated }}</div><div class="kpi-label">Escalated</div></div>
        </div>

        <div class="grid-2">
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Item Type</h3>
              @for (item of data()!.breakdownA; track item.itemType) {
          ...
          <div class="breakdown-section">
            <div class="breakdown-card">
              <h3 class="breakdown-title">By Priority</h3>
              @for (item of data()!.breakdownB; track item.priority) {
                <div class="breakdown-row">
...
export class InboxDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<{ data: Record<string, unknown> }>('/api/inbox/dashboard')
