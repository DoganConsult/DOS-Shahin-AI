 ...
 * in foundation_schema_migrations. Idempotent.
 *
 * Usage:
...
async function main() {
  ...
  // Two paths:
  //  (a) Host has installed `pg` and DATABASE_URL/PG* env vars are set → run directly.
  //  (b) Otherwise, allow caller to provide a Client implementation via stdin
  //      JSON (DSN) but fail-closed if neither is available.
  let pg;
  try {
    pg = await import('pg');
  } catch {
  ...
  const { rows: applied } = await client.query('SELECT filename FROM foundation_schema_migrations');
