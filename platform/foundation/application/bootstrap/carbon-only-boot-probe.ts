...
export async function runCarbonOnlyBootProbe(): Promise<CarbonOnlyBootProbeReport> {
  const started = Date.now();

  // (1) Catalog vendor invariant.
  const catalogRow = await query<{ total: number; non_ibm: number }>(
  ...
  const blockedRow = await query<{ count: number }>(
  ...
  const report: CarbonOnlyBootProbeReport = {
    ok:
      catalog.non_ibm === 0 &&
  ...
  if (catalog.non_ibm > 0) {
    throw new Error(
      `[carbon-only/boot-probe] REFUSING TO BOOT: ${catalog.non_ibm} non-IBM rows in dos.ui_carbon_components`,
    );
  }
  if (reg.pointing_to_blocked > 0) {
    throw new Error(
      `[carbon-only/boot-probe] REFUSING TO BOOT: ${reg.pointing_to_blocked} runtime registry rows point at non-IBM catalog rows`,
    );
  }
  if (blockedAllowed > 0) {
    throw new Error(
      `[carbon-only/boot-probe] REFUSING TO BOOT: ${blockedAllowed} blocked-react-only/catalog-only/missing-upstream rows have dynamic_ui_allowed=true`,
    );
  }

  if (reg.unapproved > 0) {
    logger.warn('carbon-only.boot-probe.unapproved-registry-rows', {
      count: reg.unapproved,
      note:
        'unapproved rows are filtered by Layer 2 resolver; consider deleting or approving them',
    });
  }

  logger.info('carbon-only.boot-probe.ok', report);
  return report;
}
