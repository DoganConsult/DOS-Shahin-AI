...
import {
  ...
  getPendingChanges,
  getSuspendedOrgs,
} from './foundation-lifecycle.service';
...
describe('transitionStatus', () => {
  ...
  describe('domain queries', () => {
    it('getPendingChanges should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getPendingChanges('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getPendingChanges should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getPendingChanges('t1');
      expect(result).toEqual([]);
    });

    it('getSuspendedOrgs should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getSuspendedOrgs('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getSuspendedOrgs should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getSuspendedOrgs('t1');
      expect(result).toEqual([]);
    });
