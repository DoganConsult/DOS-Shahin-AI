...
exports.auditTrailRouter.use((0, middleware_port_1.auditMiddleware)('audit_trail'));
...
exports.auditTrailRouter.get('/', (0, auth_adapter_1.requirePermission)('audit_trail.read'), (0, middleware_port_1.asyncHandler)(async (req, res) => {
    const limit = Math.min(Math.max(Number(req.query.limit) || 20, 1), 200);
    const offset = Math.max(Number(req.query.offset) || 0, 0);
    const { where, params } = buildAuditWhere(req.tenantId, req.query);
    const countRes = await (0, database_port_1.query)(`SELECT COUNT(*)::int AS count FROM dos.audit_trail WHERE ${where}`, params);
    const listParams = [...params, limit, offset];
    const result = await (0, database_port_1.query)(`SELECT entry_id, actor_id, action, entity_type, entity_id, module, payload, created_at
...
exports.auditTrailRouter.get('/export', (0, auth_adapter_1.requirePermission)('audit_trail.read'), (0, middleware_port_1.asyncHandler)(async (req, res) => {
    const { where, params } = buildAuditWhere(req.tenantId, req.query);
    const result = await (0, database_port_1.query)(`SELECT entry_id, actor_id, action, entity_type, entity_id, module, payload, created_at
         FROM dos.audit_trail
        WHERE ${where}
