...
exports.getOrgScope = getOrgScope;
/**
...
async function getOrgScope(tenantId, userId) {
    return track('foundation.org_scope.resolve', async () => (0, database_port_1.withTenantClient)(tenantId, async (c) => {
        // 1) Primary position + all positions held by the user
        const positions = await c.query(`SELECT p.position_id, p.title_en, p.title_ar, p.code, p.level, p.bu_id, pa.is_primary
          ...
          ORDER BY pa.is_primary DESC, pa.assigned_at DESC`, [tenantId, userId]);
        const allPositionIds = positions.rows.map((r) => r.position_id);
        ...
        const buRows = primary.bu_id
            ? (await c.query(`WITH RECURSIVE bu_chain AS (
             ...
             SELECT bu_id, name_en, code, organization_id, depth FROM bu_chain ORDER BY depth ASC`, [primary.bu_id, tenantId])).rows
            : [];
        // 3) Org ancestor chain — start from the BU's organization, then walk parent_id
        const rootOrgId = buRows.find((r) => r.organization_id)?.organization_id;
        const orgRows = rootOrgId
            ? (await c.query(`WITH RECURSIVE org_chain AS (
             ...
             SELECT organization_id, name_en, code, org_type, depth FROM org_chain ORDER BY depth ASC`, [rootOrgId, tenantId])).rows
            : [];
        return {
            userId, tenantId,
            primaryPosition: {
