...
exports.registerFoundationSyncJob = registerFoundationSyncJob;
 ...
 */
async function syncOrgHierarchyForTenant(tenantId) {
    ...
    try {
        ...
        const countRes = await (0, database_port_1.safeQuery)(`SELECT COUNT(*) AS total
       ...
       WHERE deleted_at IS NULL`);
        ...
        const orphanRes = await (0, database_port_1.safeQuery)(`UPDATE ${schema}.departments d
       ...
       RETURNING d.id`);
        ...
        const hasAncestorPath = await (0, database_port_1.safeQuery)(`SELECT 1
       FROM information_schema.columns
       WHERE table_schema = $1
         AND table_name   = 'teams'
         AND column_name  = 'ancestor_path'`, [schema]);
        if ((hasAncestorPath.rowCount ?? 0) > 0) {
            // Refresh ancestor_path as JSON array of ancestor ids (shallow — 4 levels max)
            const refreshRes = await (0, database_port_1.safeQuery)(`UPDATE ${schema}.teams t
         SET ancestor_path =
               COALESCE(
        ...
        result.durationMs = Date.now() - start;
        logger_port_1.logger.error('[FoundationSyncJob] Tenant sync failed', { tenantId, error: result.error });
        return result;
    }
}
/**
 ...
 */
async function resolveTenantSchema(tenantId) {
    const res = await (0, database_port_1.safeQuery)(`SELECT schema_name FROM public.tenants WHERE id = $1 LIMIT 1`, [tenantId]);
    return res.rows[0]?.schema_name ?? `tenant_${tenantId}`;
}
// ── Job handler ─────────────────────────────────────────────────────────────
async function runFoundationSync() {
    logger_port_1.logger.info('[FoundationSyncJob] Starting org hierarchy sync across all tenants');
    let tenants;
    try {
        tenants = await (0, platform_port_1.getProvisionedTenants)();
    }
    catch (err) {
        logger_port_1.logger.error('[FoundationSyncJob] Failed to load provisioned tenants', {
            error: err instanceof Error ? err.message : String(err),
        });
        return;
    }
    if (tenants.length === 0) {
        logger_port_1.logger.info('[FoundationSyncJob] No provisioned tenants — skipping');
        return;
    }
    const results = [];
    for (const tenant of tenants) {
        ...
        results.push(result);
        if (!result.error) {
            logger_port_1.logger.info('[FoundationSyncJob] Tenant synced', {
                tenantId: tid,
    ...
    logger_port_1.logger.info('[FoundationSyncJob] Sync complete', {
 ...
 */
function registerFoundationSyncJob() {
    (0, platform_port_1.registerJob)('foundation.org_hierarchy_sync', '0 */6 * * *', // Every 6 hours
    runFoundationSync).then(() => {
        logger_port_1.logger.info('[FoundationSyncJob] Registered: foundation.org_hierarchy_sync (every 6h)');
    }).catch((err) => {
        logger_port_1.logger.error('[FoundationSyncJob] Failed to register job', {
            error: err instanceof Error ? err.message : String(err),
        });
    });
}
//# sourceMappingURL=foundation-sync.job.js.map