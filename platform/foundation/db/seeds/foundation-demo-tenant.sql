...
--
-- Seeds tenants in any non-terminal lifecycle state so freshly-registered
-- users see non-empty Foundation lists during onboarding (orgs, BUs,
-- positions, locations, committees). Terminal states (suspended, archived,
-- deleted) and admin-imposed pending are excluded so we don't reanimate
-- tenants the operator has frozen. Extend by adding to the SEED_TENANTS CTE.
...
BEGIN
  FOR t IN
    -- Tenants registered through onboarding live in public.tenants; tenants
    -- provisioned by tenant-service live in dos.tenants. Union both so every
    -- live tenant gets the demo seed.
    SELECT tenant_id::text AS tenant_id, tenant_code, COALESCE(org_name, tenant_code) AS org_name
      FROM public.tenants
     WHERE status IN (
       'registered','email_pending','verified','onboarding','provisioning','onboarding_ready','active'
     )
    UNION
    SELECT dt.tenant_id::text AS tenant_id, dt.tenant_code, COALESCE(dt.tenant_name, dt.tenant_code) AS org_name
      FROM dos.tenants dt
     WHERE dt.status = 'active'
       AND NOT EXISTS (SELECT 1 FROM public.tenants pt WHERE pt.tenant_id::text = dt.tenant_id::text)
  LOOP
