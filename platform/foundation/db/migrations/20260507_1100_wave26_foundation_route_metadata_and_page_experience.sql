-- =====================================================================
-- Wave 26 — foundation route classification + per-route page experience
--
-- Root cause (visual probe at shahin-ai.com after Wave 25 auth bridge):
--   1. SPA boot calls GET /api/ui-os/route-metadata?route=/foundation/*
--      via the public mount and receives 401 ROUTE_METADATA_UNAUTHENTICATED
--      because every /foundation/* row has is_public=false AND
--      metadata_public=false. Render-mode classification is NOT tenant
--      data; flipping metadata_public=true keeps the actual surface
--      (workspace-runtime, template-binding, page-experience) gated while
--      letting the FE resolve renderMode without auth.
--   2. Every /foundation/* page renders the SAME "Evidence basis / Why
--      it matters" copy because dos.dynamic_ui_routes for every route
--      has NULL title_key, subtitle_key, page_type, layout. Seed per-
--      route values so each page resolves its own canonical copy.
--
-- Doctrine fix (DB only):
--   - UPDATE dos.dynamic_ui_route_metadata SET metadata_public=true
--     WHERE route LIKE '/foundation/%'
--   - UPDATE dos.dynamic_ui_routes SET title_key/subtitle_key/page_type/
--     layout per route from a canonical lookup.
--
...
-- ---------------------------------------------------------------------
-- Part 1 — flip metadata_public=true on /foundation/* (kills 401 storm)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Part 2 — seed per-route page experience contract
-- ---------------------------------------------------------------------
...
UPDATE dos.dynamic_ui_routes r
   SET title_key    = COALESCE(NULLIF(r.title_key,''), ps.title_key),
       subtitle_key = COALESCE(NULLIF(r.subtitle_key,''), ps.subtitle_key),
       page_type    = COALESCE(NULLIF(r.page_type,''), ps.page_type),
       layout       = COALESCE(NULLIF(r.layout,''), ps.layout)
  FROM page_specs ps
...
BEGIN
  SELECT count(*) INTO n
   ...
   WHERE route LIKE '/foundation/%' AND metadata_public = FALSE;
  IF n > 0 THEN
    RAISE EXCEPTION 'wave26: % /foundation/* rows still have metadata_public=false', n;
  END IF;

  SELECT count(*) INTO m
    FROM dos.dynamic_ui_routes
   WHERE path_pattern LIKE '/foundation/%' AND tenant_id IS NULL
     AND (title_key IS NULL OR title_key = '');
  IF m > 0 THEN
    RAISE NOTICE 'wave26 note: % /foundation/* dynamic_ui_routes still missing title_key (no spec row)', m;
  END IF;
  RAISE NOTICE 'wave26 proof: foundation route classification + page experience seeded';
END$$;
