...
vi.mock('../ports/lifecycle.port', () => ({
  ...
  EntityStateMachine: class { constructor(_cfg: any) {} canTransition() { return false; } getAvailableTransitions() { return []; } },
}));

describe('Foundation Lifecycle Registration', () => {
  beforeAll(async () => {
  ...
  it('uses correct module code', () => {
    expect(registrations[0][0]).toBe('foundation');
  });
