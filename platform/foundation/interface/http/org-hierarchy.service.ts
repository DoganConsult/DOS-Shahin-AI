...
export async function getOrgTree(tenantId: string): Promise<OrgTreeNode[]> {
  return track('foundation.hierarchy.tree', async () =>
    withTenantClient(tenantId, async (c) => {
      const r = await c.query(
        `WITH RECURSIVE org_tree AS (
         ...
         SELECT * FROM org_tree ORDER BY depth, name_en`,
        [tenantId],
      );
      return r.rows as OrgTreeNode[];
    }),
  );
}
 ...
 */
export async function getManagerChain(tenantId: string, userId: string): Promise<ManagerChainEntry[]> {
  return track('foundation.hierarchy.manager_chain', async () =>
    withTenantClient(tenantId, async (c) => {
      const r = await c.query(
      ...
      return r.rows as ManagerChainEntry[];
    }),
  );
}

export async function getSubtree(tenantId: string, rootId: string): Promise<OrgTreeNode[]> {
  return track('foundation.hierarchy.subtree', async () =>
    withTenantClient(tenantId, async (c) => {
      const r = await c.query(
        `WITH RECURSIVE subtree AS (
         ...
         SELECT * FROM subtree ORDER BY depth, name_en`,
        [rootId, tenantId],
      );
      return r.rows as OrgTreeNode[];
    }),
