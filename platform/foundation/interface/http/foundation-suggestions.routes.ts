...
export const foundationSuggestionsRouter = Router();
foundationSuggestionsRouter.use(authenticate);
foundationSuggestionsRouter.use(requireTenantId);

foundationSuggestionsRouter.get('/suggestions',
  requirePermission('foundation.read'),
  asyncHandler(async (req: Request, res: Response) => {
    const tenantId = req.tenantId!;
    const schema = tenantSchema(tenantId);
    const suggestions: Array<{ code: string; severity: 'info'|'low'|'medium'|'high'; titleEn: string; titleAr: string; route: string; count: number }> = [];

    // Orphaned departments (no parent BU)
    try {
      const r = await query(
        `SELECT COUNT(*)::int AS n FROM "${schema}".departments WHERE deleted_at IS NULL AND business_unit_id IS NULL`,
      );
      const n = r.rows[0]?.n ?? 0;
      if (n > 0) suggestions.push({
      ...
      const r = await query(
        `SELECT COUNT(*)::int AS n FROM dos.positions p
          WHERE p.tenant_id = $1 AND p.deleted_at IS NULL
            AND NOT EXISTS (SELECT 1 FROM dos.position_assignments a WHERE a.position_id = p.position_id AND a.deleted_at IS NULL)`,
        [tenantId],
      );
      const n = r.rows[0]?.n ?? 0;
      if (n > 0) suggestions.push({
    ...
    try {
      const r = await query(
        `SELECT COUNT(*)::int AS n FROM "${schema}".users u
          WHERE u.deleted_at IS NULL
            AND NOT EXISTS (SELECT 1 FROM platform_dauth.user_role_assignments a WHERE a.user_id = u.user_id AND a.tenant_id = $1)`,
        [tenantId],
      );
      const n = r.rows[0]?.n ?? 0;
      if (n > 0) suggestions.push({
