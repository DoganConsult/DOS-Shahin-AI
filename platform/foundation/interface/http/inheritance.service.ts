  ...
  if (scopeType === 'organization') {
    const r = await client.query(
      `WITH RECURSIVE chain AS (
       ...
       SELECT id, name, level FROM chain ORDER BY level ASC`,
      [scopeId, tenantId],
    );
    return r.rows.map((row) => ({
      id: row.id as string, name: row.name as string, level: Number(row.level),
    }));
  }
...
export async function getInheritance(
  ...
  scopeType: InheritanceScopeType;
  scopeId: string;
  chain: Array<{ id: string; name: string; level: number }>;
  inheritedItems: InheritedItem[];
}> {
  return track('foundation.inheritance.resolve', async () =>
    withTenantClient(tenantId, async (c) => {
      ...
      const inheritedItems: InheritedItem[] = [];
