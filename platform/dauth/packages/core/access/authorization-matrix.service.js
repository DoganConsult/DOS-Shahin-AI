 ...
 */
async function can(tenantId, userId, action, opts) {
    ...
    try {
        ...
        const decision = {
            allowed: false,
            reason: `Authorization error: ${err.message ?? 'unknown'} — deny by default`,
            delegated: false,
        };
        await logDecision(ts, decisionId, tenantId, userId, action, opts, decision, Date.now() - startMs).catch((0, resilience_1.catchHandler)(resilience_1.EC.EVENT_BUS));
        return decision;
    }
}
// ── Direct permission resolution ───────────────────────────────────
async function resolveDirectPermissions(ts, userId, action) {
    const result = await (0, db_1.safeQuery)(`SELECT DISTINCT p.code AS permission_code, r.code AS role_code
...
async function checkDecisionAuthority(ts, userId, action, opts) {
    if (!opts?.entityType)
        return null;
    const result = await (0, db_1.safeQuery)(`SELECT da.id, da.authority_level, da.decision_type
     ...
     WHERE ura.user_id = $1
       ...
       AND da.is_active = true`, [userId, opts.entityType, action]);
    if (result.rows.length === 0)
        return null;
    const authority = result.rows[0];
    // Scope check: if scopeType is provided, verify authority applies in that scope
    if (opts.scopeType && opts.scopeId) {
        const scopeCheck = await (0, db_1.safeQuery)(`SELECT 1 FROM ${ts}.authority_scope_bindings asb
       WHERE asb.authority_id = $1
         AND asb.scope_type = $2
         AND (asb.scope_id = $3 OR asb.scope_id = '*')`, [authority.id, opts.scopeType, opts.scopeId]);
...
async function resolveDelegatedPermissions(ts, userId, action) {
    const result = await (0, db_1.safeQuery)(`SELECT DISTINCT p.code AS permission_code, r.code AS role_code, dc.id AS delegation_id
...
// ── Audit log ──────────────────────────────────────────────────────
async function logDecision(ts, decisionId, tenantId, userId, action, opts, decision, durationMs) {
...
//# sourceMappingURL=authorization-matrix.service.js.map