...
// ScopeBinding is canonically defined in scope-resolution.contract.
...
// ── Phase B-2 enrichment fields ─────────────────────────────────────────
// These extensions back the unified `/api/foundation/access-snapshot`
// endpoint where Foundation composes its hierarchy data with DAuth's
// permission/role data. Optional on `FullAccessSnapshot` so that
// /api/auth/access (auth-service) can keep returning the legacy shape
// during the rollout window without contract drift.
...
export interface FullAccessSnapshot {
  ...
  // ── Phase B-2: Foundation-composed enrichment (all optional) ──
