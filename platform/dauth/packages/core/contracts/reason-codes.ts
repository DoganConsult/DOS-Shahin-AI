...
export const DAUTH_REASON_CODES = {
...
export type DauthReasonCode =
  (typeof DAUTH_REASON_CODES)[keyof typeof DAUTH_REASON_CODES];

/**
 * Map a 14-step pipeline failure (step number + check name) to a reason code.
 * Called from `access/decision-engine.ts#denyAndLog`.
 */
export function reasonCodeForStep(
  step: number,
  check: string,
): DauthReasonCode {
  switch (step) {
    case 1: return DAUTH_REASON_CODES.DENY_NOT_AUTHENTICATED;
    case 2: return DAUTH_REASON_CODES.DENY_SESSION_INVALID;
    case 3: return DAUTH_REASON_CODES.DENY_TENANT_MEMBERSHIP_MISSING;
    case 4: return DAUTH_REASON_CODES.DENY_TENANT_INACTIVE;
    case 5: return DAUTH_REASON_CODES.DENY_PRODUCT_NOT_ENTITLED;
    case 6: return DAUTH_REASON_CODES.DENY_MODULE_DISABLED;
    case 7: return DAUTH_REASON_CODES.DENY_ACCESS_PROFILE_BLOCKED;
    case 8: return DAUTH_REASON_CODES.DENY_PERMISSION_MISSING;
    case 9: return DAUTH_REASON_CODES.DENY_SCOPE_MISMATCH;
    case 10: return DAUTH_REASON_CODES.DENY_AUTHORITY_INSUFFICIENT;
    case 11: return check.includes('self_approval')
      ? DAUTH_REASON_CODES.DENY_SOD_SELF_APPROVAL
      : DAUTH_REASON_CODES.DENY_SOD_CONFLICT;
    case 12: return DAUTH_REASON_CODES.DENY_LIFECYCLE_TRANSITION;
    case 13: return DAUTH_REASON_CODES.DENY_OWNERSHIP_MISSING;
    default: return DAUTH_REASON_CODES.INTERNAL_UNKNOWN;
  }
}

/** True if the code represents an allow (all checks passed or bypass). */
export function isAllowCode(code: DauthReasonCode): boolean {
  return code.startsWith('DAUTH_ALLOW_');
}

/** True if the code represents a deny (hard refusal). */
export function isDenyCode(code: DauthReasonCode): boolean {
  return code.startsWith('DAUTH_DENY_');
}

/** True if the code represents an abstain (shadow mode, no policy matched). */
export function isAbstainCode(code: DauthReasonCode): boolean {
