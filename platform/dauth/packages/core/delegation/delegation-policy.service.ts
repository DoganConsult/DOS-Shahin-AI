...
export async function evaluateDelegation(
  ...
  try {
    ...
    const cfg = getFirstRow(shadowResult) as {
      enabled: boolean;
      consent_granted: boolean;
      autonomy_level: string;
      max_actions_per_day: number;
      actions_today: number;
      actions_today_reset?: Date | string;
    };
...
export async function getDelegationRules(
  ...
  try {
    ...
    return result.rows as unknown as DelegationRule[];
  } catch { return []; }
}
...
export async function upsertDelegationRule(
  ...
  try {
    ...
    return (getFirstRow(result) as { rule_id?: string } | undefined)?.rule_id || null;
  } catch (err: unknown) {
