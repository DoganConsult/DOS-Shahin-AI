...
import type { RebacTupleWrite } from '../ports/rebac.port';
...
interface TupleMapper {
  eventType: string;
  subscriberId: string;
  map: (payload: EventPayload) => RebacTupleWrite[];
}

const MAPPERS: TupleMapper[] = [
  {
  ...
  },
  {
    ...
    map: (p) => {
      ...
      const from = str(p.fromUserId);
      if (!grantId) return [];
      const out: RebacTupleWrite[] = [];
      if (from) out.push({ user: `user:${from}`, relation: 'from', object: `delegation:${grantId}`, op: 'delete' });
      if (to) out.push({ user: `user:${to}`, relation: 'to', object: `delegation:${grantId}`, op: 'delete' });
      return out;
    },
  },
  {
  ...
  },
  {
    ...
    map: (p) => {
      ...
      const out: RebacTupleWrite[] = [];
      if (oldMgr) out.push({ user: `position:${oldMgr}`, relation: 'manager', object: `position:${positionId}`, op: 'delete' });
      if (newMgr) out.push({ user: `position:${newMgr}`, relation: 'manager', object: `position:${positionId}`, op: 'write' });
      return out;
    },
  },
  {
...
export function registerOpenFgaTupleSync(): void {
  const active = DAUTH_CONFIG.openfga.shadow || DAUTH_CONFIG.openfga.enforce;
  for (const mapper of MAPPERS) {
    subscribe({
      ...
      handler: async (event) => {
        if (!active) return;
        try {
          const tuples = mapper.map((event.payload ?? {}) as EventPayload);
          ...
          logger.warn('[DAuth:OpenFGA] tuple-sync failed', {
            eventType: mapper.eventType,
  ...
  logger.info('[DAuth:OpenFGA] tuple-sync subscribers registered', {
