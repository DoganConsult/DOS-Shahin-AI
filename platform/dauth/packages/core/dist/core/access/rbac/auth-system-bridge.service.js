...
exports.getUnifiedUserRoles = getUnifiedUserRoles;
exports.getUnifiedUserPermissions = getUnifiedUserPermissions;
exports.getAuthSystemDiagnostics = getAuthSystemDiagnostics;
/**
...
const logger = (0, db_1.getDbLogger)();
/**
 * Get all roles for a user, merging enterprise (163) and legacy (030) systems.
 * Enterprise roles take precedence.
 */
async function getUnifiedUserRoles(tenantId, userId) {
    const schema = (0, db_1.tenantSchema)(tenantId);
    const roles = [];
    const seenCodes = new Set();
    // 1. Enterprise model (Migration 163) — PRIMARY
    try {
        const { rows: enterpriseRows } = await (0, db_1.safeQuery)(`SELECT fr.role_code, fr.name_en, fr.is_system, fr.is_active
       ...
       WHERE eura.user_id = $1 AND fr.is_active = TRUE
         AND (eura.valid_until IS NULL OR eura.valid_until > NOW())`, [userId]);
    ...
    try {
        const { rows: legacyRows } = await (0, db_1.safeQuery)(`SELECT r.role_code, r.name_en, r.is_system, r.is_active
 ...
 */
async function getUnifiedUserPermissions(tenantId, userId) {
    const schema = (0, db_1.tenantSchema)(tenantId);
    ...
    try {
        const { rows } = await (0, db_1.safeQuery)(`SELECT DISTINCT rfp.permission_code
       FROM "${schema}".user_role_assignments ura
 ...
 */
async function getAuthSystemDiagnostics(tenantId) {
    const schema = (0, db_1.tenantSchema)(tenantId);
    ...
    let legacyPerms = 0;
    try {
        const { rows: er } = await (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS cnt FROM "${schema}".functional_roles WHERE is_active = TRUE`);
        enterpriseRoles = er[0]?.cnt ?? 0;
    }
    catch { /* table may not exist */ }
    try {
        const { rows: lr } = await (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS cnt FROM "${schema}".roles WHERE is_active = TRUE`);
        legacyRoles = lr[0]?.cnt ?? 0;
    }
    catch { /* table may not exist */ }
    try {
        const { rows: ep } = await (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS cnt FROM "${schema}".permissions WHERE is_active = TRUE`);
        enterprisePerms = ep[0]?.cnt ?? 0;
    }
    catch { /* table may not exist */ }
    try {
        const { rows: lp } = await (0, db_1.safeQuery)(`SELECT COUNT(DISTINCT permission_code)::int AS cnt FROM "${schema}".role_function_permissions`);
        legacyPerms = lp[0]?.cnt ?? 0;
    }
    catch { /* table may not exist */ }
...
//# sourceMappingURL=auth-system-bridge.service.js.map