...
const approval_matrix_service_1 = require("../authority/approval-matrix.service");
const self_approval_guard_1 = require("./self-approval.guard");
/**
 ...
 */
async function evaluateLifecycleTransition(tenantId, userId, opts) {
    ...
    const transition = await (0, db_1.safeQuery)(`SELECT required_permission_code, authority_gate, sod_check, required_functional_roles
     FROM "${schema}".module_lifecycle_transitions
     WHERE module_code = $1 AND from_status = $2 AND to_status = $3 LIMIT 1`, [opts.moduleCode, opts.fromState, opts.toState]);
    ...
    const permCheck = await (0, db_1.safeQuery)(`SELECT 1 FROM "${schema}".role_permissions rp
     JOIN "${schema}".functional_roles fr ON fr.id = rp.functional_role_id
     JOIN "${schema}".permissions p ON p.id = rp.permission_id
     WHERE fr.code = ANY($1) AND p.code = $2 AND fr.is_active = TRUE LIMIT 1`, [opts.userRoles, requiredPerm]);
    ...
    if (rule.authority_gate) {
       ...
       WHERE ura.user_id = $1 AND ura.is_active = TRUE ORDER BY al.rank DESC LIMIT 1`, [userId]);
        const requiredAuth = await (0, db_1.safeQuery)(`SELECT rank FROM "${schema}".authority_levels WHERE level_code = $1 LIMIT 1`, [rule.authority_gate]);
    ...
    if (rule.sod_check) {
        ...
        const selfApprovalCheck = await (0, self_approval_guard_1.checkSelfApproval)(tenantId, userId, opts.entityType, opts.entityId, opts.toState);
    ...
    // 5. SoD check
    const sodResult = await (0, sod_engine_1.evaluateSod)(tenantId, opts.userRoles, { moduleCode: opts.moduleCode });
    ...
    if (checks.approvalRequired) {
        ...
        const approvalCheck = await (0, approval_matrix_service_1.checkActorAuthority)(tenantId, userId, transitionAction, opts.moduleCode, opts.entityValue);
...
//# sourceMappingURL=lifecycle-auth.service.js.map