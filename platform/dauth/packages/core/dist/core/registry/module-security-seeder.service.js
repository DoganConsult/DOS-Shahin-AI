...
exports.seedAllModuleSecurity = seedAllModuleSecurity;
...
const module_security_seeder_registry_1 = require("./module-security-seeder.registry");
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const ARCHETYPE_PERMISSION_ACTIONS = {
 ...
 */
async function seedAllModuleSecurity(tenantId, pool) {
    const schema = (0, db_1.tenantSchema)(tenantId);
    const result = { modules: {}, errors: [] };
    const startTime = Date.now();
    const moduleCodes = (0, module_security_seeder_registry_1.getAllModuleCodes)();
    observability_1.logger.info(`[module-security-seeder] Starting security seed for tenant ${tenantId} (${moduleCodes.length} modules)`);
    for (const moduleCode of moduleCodes) {
        const entry = module_security_seeder_registry_1.MODULE_SECURITY_REGISTRY.get(moduleCode);
        if (!entry)
            continue;
        ...
        try {
            ...
            observability_1.logger.info(`[module-security-seeder]   ${moduleCode}: ` +
            ...
            await client.query('ROLLBACK').catch((0, resilience_1.catchHandler)(resilience_1.EC.EVENT_BUS, {}));
            const msg = `Module "${moduleCode}" failed: ${(err instanceof Error ? err.message : String(err))}`;
            observability_1.logger.error(`[module-security-seeder]   ERROR ${msg}`);
    ...
    observability_1.logger.info(`[module-security-seeder] Completed in ${elapsed}ms. ` +
...
// ---------------------------------------------------------------------------
async function seedSingleModule(tenantId, schema, entry, client) {
...
// ---------------------------------------------------------------------------
async function seedRoleDefinitions(tenantId, schema, moduleCode, roles, permissions, client) {
    if (!roles.length)
        return 0;
    const sql = `
...
// ---------------------------------------------------------------------------
async function seedPermissionDefinitions(tenantId, schema, moduleCode, permissions, client) {
...
// ---------------------------------------------------------------------------
async function seedActionDefinitions(tenantId, schema, moduleCode, actions, client) {
    if (!actions.length)
        return 0;
    const sql = `
...
// ---------------------------------------------------------------------------
async function seedRolePermissionMap(tenantId, schema, moduleCode, roles, permissions, client) {
...
// ---------------------------------------------------------------------------
async function seedRoleActionMap(tenantId, schema, moduleCode, roles, actions, client) {
  ...
  `;
    ...
    for (const role of roles) {
        ...
        const grantedActions = actions.filter((a) => {
            if (!a.requiredPermissions.length)
...
// ---------------------------------------------------------------------------
async function seedApprovalMatrix(tenantId, schema, moduleCode, matrix, client) {
    if (!matrix.length)
        return 0;
    const sql = `
...
// ---------------------------------------------------------------------------
async function seedApprovalMatrixRules(tenantId, schema, moduleCode, matrix, client) {
    if (!matrix.length)
        return 0;
  ...
  `);
    ...
    for (const rule of matrix) {
        ...
        await client.query(`INSERT INTO "${schema}".approval_matrix_rules
         (action_code, module_code, required_authority_code, min_approvals, is_active, created_at, updated_at)
       VALUES ($1, $2, $3, $4, TRUE, NOW(), NOW())
       ON CONFLICT (rule_id) DO NOTHING`, [actionCode, moduleCode, rule.requiredRole || 'module_admin', rule.minApprovers ?? 1]);
        count++;
    }
    if (count > 0) {
        observability_1.logger.info('[SecuritySeeder] Approval matrix rules seeded', { moduleCode, count });
    }
    return count;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function seedModuleSodActionRules(tenantId, schema, moduleCode, rules, client) {
    if (!rules.length)
        return 0;
    // Ensure table exists in tenant schema (idempotent)
    await client.query(`
  ...
  `);
    ...
    await client.query(`DELETE FROM "${schema}".module_sod_rules WHERE module_code = $1`, [moduleCode]);
    let count = 0;
    for (const rule of rules) {
        ...
        for (let i = 0; i < actions.length; i++) {
            for (let j = i + 1; j < actions.length; j++) {
                await client.query(`INSERT INTO "${schema}".module_sod_rules
    ...
    if (count > 0) {
        observability_1.logger.info('[SecuritySeeder] Module SoD action rules seeded', { moduleCode, count });
    }
    return count;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function seedSoDRules(tenantId, schema, moduleCode, rules, client) {
    if (!rules.length)
        return 0;
    ...
    await client.query(`DELETE FROM "${schema}".sod_conflict_matrix
     WHERE scope = $1`, [`module:${moduleCode}`]);
    const sql = `
...
function mapSoDSeverity(severity) {
    switch (severity) {
...
async function seedDefaultFieldRbac(tenantId, schema, moduleCode, client) {
...
//# sourceMappingURL=module-security-seeder.service.js.map