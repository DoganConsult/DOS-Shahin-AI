...
exports.invalidateKeyCache = invalidateKeyCache;
exports.getActiveSigningKey = getActiveSigningKey;
...
async function loadKeysFromDb() {
    try {
        ...
        const { rows } = await safeQuery(`SELECT kid, secret, algorithm, status, created_at, retired_at, expires_at
...
async function rotateSigningKey() {
    const { safeQuery } = require('@dos/db');
    const newKid = (0, crypto_1.randomUUID)();
    const newSecret = (0, crypto_1.randomBytes)(64).toString('base64url');
    // Retire the current active key
    await safeQuery(`UPDATE jwt_signing_keys SET status = 'retired', retired_at = NOW() WHERE status = 'active'`);
    // Insert the new active key
    await safeQuery(`INSERT INTO jwt_signing_keys (kid, secret, algorithm, status, created_at)
     VALUES ($1, $2, 'HS256', 'active', NOW())`, [newKid, newSecret]);
    invalidateKeyCache();
    return newKid;
}
/**
...
async function revokeKey(kid) {
    const { safeQuery } = require('@dos/db');
    await safeQuery(`UPDATE jwt_signing_keys SET status = 'revoked', retired_at = COALESCE(retired_at, NOW()) WHERE kid = $1`, [kid]);
    invalidateKeyCache();
}
