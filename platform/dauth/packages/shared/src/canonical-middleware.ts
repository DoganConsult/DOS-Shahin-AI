...
import { verifyGatewayOrigin, GATEWAY_ORIGIN_HEADER, createInMemoryReplayRegistry } from './gateway-origin';
...
const __WAVE25_REPLAY = createInMemoryReplayRegistry();
...
function enrichDecodedRoles(decoded: Record<string, unknown>, req: Request): void {
  const merged = new Set<string>();
  // 1) Existing decoded.roles (if any).
  const existing = decoded.roles as unknown;
  if (Array.isArray(existing)) {
    for (const r of existing) if (typeof r === 'string' && r) merged.add(r);
  }
  ...
  if (realmAccess && Array.isArray(realmAccess.roles)) {
    for (const r of realmAccess.roles) if (typeof r === 'string' && r) merged.add(r);
  }
  // 3) Keycloak resource_access.<client>.roles
  const resourceAccess = decoded.resource_access as Record<string, { roles?: unknown }> | undefined;
  if (resourceAccess && typeof resourceAccess === 'object') {
    for (const entry of Object.values(resourceAccess)) {
      if (entry && Array.isArray(entry.roles)) {
        for (const r of entry.roles) if (typeof r === 'string' && r) merged.add(r);
      }
    }
  }
  ...
  //    Keycloak JWT).
  const gwToken = req.headers[GATEWAY_ORIGIN_HEADER] as string | undefined;
  const secret = process.env.GATEWAY_ORIGIN_HMAC_SECRET;
  if (gwToken && secret && secret.length >= 32) {
    try {
      const result = verifyGatewayOrigin(gwToken, { secret, replayRegistry: __WAVE25_REPLAY });
      if (result.ok && Array.isArray(result.principal.roles)) {
        for (const r of result.principal.roles) if (typeof r === 'string' && r) merged.add(r);
      }
    } catch {
  ...
  if (legacyTrust !== 'false' && legacyTrust !== '0' && legacyTrust !== 'no') {
    ...
    if (typeof raw === 'string' && raw) {
      for (const r of raw.split(',').map((s) => s.trim()).filter(Boolean)) merged.add(r);
    }
  }
  if (merged.size > 0) decoded.roles = Array.from(merged);
}
