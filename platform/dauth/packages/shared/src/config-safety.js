...
exports.verifyDivergenceClean = verifyDivergenceClean;
...
function modeOf(prefix) {
    if (readBool(`${prefix}_ENFORCE`))
        return 'enforce';
    if (readBool(`${prefix}_SHADOW`))
        return 'shadow';
    return 'off';
}
...
function assertDauthConfigSafe(opts = {}) {
    const log = {
    ...
    if (kcMode === 'enforce') {
        ...
        if (jwtAlg !== 'RS256' && !mixedAck) {
            violations.push({
                ...
                severity: 'warn',
                detail: 'DAUTH_KEYCLOAK_ENFORCE=true but DAUTH_JWT_ALGORITHM is not RS256. Native HS256 tokens remain accepted alongside Keycloak RS256 tokens. Set DAUTH_JWT_MIXED_ALG_ACK=true to acknowledge, or migrate native issuance to RS256.',
            });
        }
        ...
        if (!!productId !== !!productSecret) {
            violations.push({
                ...
                severity: 'block',
                detail: 'KEYCLOAK_PRODUCT_OIDC_CLIENT_ID and KEYCLOAK_PRODUCT_OIDC_CLIENT_SECRET must be set together, or neither. Partial config silently falls back to the generic client.',
            });
        }
        ...
        if (!!adminId !== !!adminSecret) {
            violations.push({
                ...
                severity: 'block',
                detail: 'KEYCLOAK_ADMIN_OIDC_CLIENT_ID and KEYCLOAK_ADMIN_OIDC_CLIENT_SECRET must be set together, or neither. Partial config silently falls back to the generic client.',
            });
        }
    ...
    for (const v of violations) {
        if (v.severity === 'block')
            log.error(`[DAuth:config] ${v.code} — ${v.detail}`);
        else
            log.warn(`[DAuth:config] ${v.code} — ${v.detail}`);
    }
    if (!ok && opts.mode === 'throw') {
        throw new Error(`[DAuth] config-safety check failed: ${blockers.map((b) => b.code).join(', ')}`);
    }
    return {
        ok,
        violations,
 ...
 */
async function verifyDivergenceClean(queryFn, opts = { engine: 'keycloak' }) {
    ...
    try {
        ...
        const { rows } = await queryFn(`SELECT engine, SUM(diverged_count)::int AS diverged
       FROM dos.dauth_divergence_report
       WHERE engine = $1 AND window_start >= $2
       GROUP BY engine`, [opts.engine, since]);
...
//# sourceMappingURL=config-safety.js.map