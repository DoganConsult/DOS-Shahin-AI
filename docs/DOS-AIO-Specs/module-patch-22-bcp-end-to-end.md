...
### 0.3 Patch purpose
This patch defines the full canonical target for the **BCP module** end to end.

It tells an agent exactly how to:
- inspect the bcp module across backend, frontend, contracts, events, workflows, data, diagnostics, admin/runtime, and handover layers
- compare the current implementation against the canonical bcp target
- know what belongs to BCP, what belongs to DOS, what belongs to DAuth, what belongs to Workflow, and what belongs to adjacent modules
...
- Runtime role: business continuity planning, scenario readiness, exercise tracking, recovery planning, and continuity oversight
- Primary dependency domains: DOS shell/events/notifications, DAuth control spine, workflow, risk, asset, policy, reporting, analytics, AI
...
## 2.1 What BCP owns directly
- BCP plans and plan structures
- scenario and exercise state
- recovery strategy state from the BCP perspective
- continuity dashboards, diagnostics, and admin/runtime controls

## 2.2 What BCP consumes from DOS
- shell runtime
- notifications
- event backbone
- org and tenant context

## 2.3 What BCP consumes from DAuth
- scoped access
...
## 2.4 What BCP consumes from adjacent modules
- risk, asset, policy, workflow, reporting, analytics, and AI via explicit contracts

## 2.5 What BCP must never implement
- own auth truth
- run hidden workflow approvals
- duplicate risk truth
...
- BCP plan tables
- exercise tables
- recovery strategy tables
- readiness tables
- BCP audit and history tables

DOS owns platform truth.  
DAuth owns auth/control truth.  
BCP owns bcp-domain runtime truth.
...
- plan approval flows
- exercise review flows
- escalation for failed readiness or overdue exercises
...
No protected bcp transition, approval, publish, release, closure, override, or sign-off path may bypass workflow + DAuth.
...
- exercise summary support
- gap and readiness narratives
...
- BCP policy visibility
- exercise cadence visibility
...
- plan completeness diagnostics
- exercise pipeline diagnostics
...
- Centralize bcp runtime truth inside the `bcp` module if it is fragmented across unrelated modules.
...
Pass only if:
- bcp-domain runtime truth is centralized in `bcp`
...
Use this patch to compare the current bcp implementation against the full canonical bcp target, classify every bcp-layer gap, build only the missing bcp artifacts, validate against pass/fail rules, and update the as-built ledger.
