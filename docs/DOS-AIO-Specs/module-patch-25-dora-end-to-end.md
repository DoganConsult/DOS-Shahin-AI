...
### 0.3 Patch purpose
This patch defines the full canonical target for the **DORA module** end to end.

It tells an agent exactly how to:
- inspect the dora module across backend, frontend, contracts, events, workflows, data, diagnostics, admin/runtime, and handover layers
- compare the current implementation against the canonical dora target
- know what belongs to DORA, what belongs to DOS, what belongs to DAuth, what belongs to Workflow, and what belongs to adjacent modules
...
- Runtime role: DORA-specific regulatory resilience management, control posture, obligations, and evidence alignment
- Primary dependency domains: DOS shell/events, DAuth control spine, compliance, risk, controls, evidence, reporting, analytics, AI, workflow
...
## 2.1 What DORA owns directly
- DORA-specific obligations and mappings
- DORA resilience oversight surfaces
- DORA dashboards, diagnostics, and admin/runtime controls

## 2.2 What DORA consumes from DOS
- shell runtime
- event backbone
- observability
- tenant context

## 2.3 What DORA consumes from DAuth
- scoped access
- protected approval authority
- delegation
- SoD
- lifecycle authorization

## 2.4 What DORA consumes from adjacent modules
- compliance, controls, risk, evidence, workflow, reporting, analytics, AI

## 2.5 What DORA must never implement
- duplicate generic compliance truth when shared compliance contracts exist
- own auth truth
...
- DoraObligationService
- DoraResilienceService
- DoraMappingService
- DoraDashboardService
- DoraDiagnosticsService
- DoraAdminService
...
```text
frontend/src/app/features/dora/
  pages/\n  components/\n  services/\n  dashboards/\n  diagnostics/\n  admin/\n  contracts/\n  testing/\n  obligations/\n  resilience/\n
  index.ts
```
...
- DORA obligation tables
- DORA mapping tables
- DORA resilience state tables
- DORA audit and history tables

DOS owns platform truth.  
DAuth owns auth/control truth.  
DORA owns dora-domain runtime truth.
...
- escalation for failed resilience obligations
...
No protected dora transition, approval, publish, release, closure, override, or sign-off path may bypass workflow + DAuth.
...
- Centralize dora runtime truth inside the `dora` module if it is fragmented across unrelated modules.
...
Pass only if:
- dora-domain runtime truth is centralized in `dora`
...
Use this patch to compare the current dora implementation against the full canonical dora target, classify every dora-layer gap, build only the missing dora artifacts, validate against pass/fail rules, and update the as-built ledger.
