...
### 0.3 Patch purpose
This patch defines the full canonical target for the **Asset module** end to end.

It tells an agent exactly how to:
- inspect the asset module across backend, frontend, contracts, events, workflows, data, diagnostics, admin/runtime, and handover layers
- compare the current implementation against the canonical asset target
- know what belongs to Asset, what belongs to DOS, what belongs to DAuth, what belongs to Workflow, and what belongs to adjacent modules
...
- Runtime role: asset inventory, classification, ownership, lifecycle visibility, and asset governance
- Primary dependency domains: DOS foundation/shell/events, DAuth control spine, workflow, controls, risk, evidence, reporting, analytics, AI
...
## 2.1 What Asset owns directly
- asset master records
- asset classification and ownership from the asset perspective
- asset lifecycle state and governance surfaces
- asset dashboards, diagnostics, and admin/runtime controls

## 2.2 What Asset consumes from DOS
- foundation and tenant context
- shell runtime
- event backbone
- observability

## 2.3 What Asset consumes from DAuth
- scoped access
- protected asset change authority
- delegation
- SoD
- lifecycle authorization

## 2.4 What Asset consumes from adjacent modules
- controls, risk, evidence, and workflow through explicit contracts
- AI for classification and summarization

## 2.5 What Asset must never implement
- own auth truth
- bypass workflow + DAuth for protected lifecycle transitions
...
```text
frontend/src/app/features/asset/
  pages/\n  components/\n  services/\n  dashboards/\n  diagnostics/\n  admin/\n  contracts/\n  testing/\n  inventory/\n  classification/\n  lifecycle/\n
  index.ts
```
...
- classification and ownership views
- lifecycle views
...
- asset audit and history tables

DOS owns platform truth.  
DAuth owns auth/control truth.  
Asset owns asset-domain runtime truth.
...
- asset CRUD
- classification and ownership actions
- lifecycle actions
- dashboards and summaries
...
- asset approval and review flows
- escalation for unmanaged or critical assets
...
No protected asset transition, approval, publish, release, closure, override, or sign-off path may bypass workflow + DAuth.
...
- ownership recommendation support
- summary generation

### 8.2 Restricted AI behavior
- autonomous protected lifecycle change
...
- classification policy visibility
- lifecycle rule visibility
...
- inventory integrity diagnostics
- owner gap diagnostics
- lifecycle blockage diagnostics
...
- Centralize asset runtime truth inside the `asset` module if it is fragmented across unrelated modules.
...
Pass only if:
- asset-domain runtime truth is centralized in `asset`
...
**Module Patch 22 — BCP Module End-to-End**

---

## 17. One-Line Use Instruction

Use this patch to compare the current asset implementation against the full canonical asset target, classify every asset-layer gap, build only the missing asset artifacts, validate against pass/fail rules, and update the as-built ledger.
