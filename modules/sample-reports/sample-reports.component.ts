...
export class SampleReportsComponent implements OnInit {
  ...
  loading = false; error = ''; reports: Record<string, unknown>[] = [];
  constructor(public i18n: I18nService, private apiclientSvc: ApiClientService) {}
  ngOnInit() {
    this.loading = true;
    this.apiclientSvc.get('/public/sample-reports').subscribe({
      next: (d: Record<string, unknown>) => { this.reports = Array.isArray(d) ? d : d.reports || []; this.loading = false; this.cdr.markForCheck(); },
      error: () => { this.error = 'Failed to load data'; this.loading = false; this.cdr.markForCheck(); }
    });
  }
  preview(r: Record<string, unknown>) { /* open preview */ }
