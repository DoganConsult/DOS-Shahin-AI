 ...
 */
function computePriority(daysUntilDue: number): ReminderPriority {
...
export async function generateReminders(
  ...
  try {
    ...
    const { rows: tasks } = await safeQuery(
      `SELECT t.task_id, t.title, t.description, t.due_date, t.assigned_to, t.status
    ...
    logger.warn(`[SmartReminder] Failed to fetch process tasks: ${String(err)}`);
  }

  // ── Source 2: Compliance Assessments with deadlines ────────────
  try {
    const { rows: assessments } = await safeQuery(
      `SELECT a.assessment_id, a.title, a.due_date, a.assigned_to, a.status
    ...
    for (const a of assessments) {
      ...
      if (userId && a.assigned_to && a.assigned_to !== userId) continue;
      reminders.push({
    ...
    logger.warn(`[SmartReminder] Failed to fetch assessments: ${String(err)}`);
  }

  // ── Source 3: Evidence expiry ──────────────────────────────────
  try {
    const { rows: evidence } = await safeQuery(
      `SELECT e.evidence_id, e.title, e.expiry_date, e.uploaded_by
    ...
    for (const e of evidence) {
      const daysUntilDue = Math.ceil(
        (new Date(e.expiry_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24),
      );
      if (userId && e.uploaded_by && e.uploaded_by !== userId) continue;
      reminders.push({
    ...
    logger.warn(`[SmartReminder] Failed to fetch evidence: ${String(err)}`);
  }

  // ── Source 4: Policy review cycles ────────────────────────────
  try {
    const { rows: policies } = await safeQuery(
      `SELECT p.policy_id, p.title, p.next_review_date, p.owner_user_id
    ...
    for (const p of policies) {
      const daysUntilDue = Math.ceil(
        (new Date(p.next_review_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24),
      );
      if (userId && p.owner_user_id && p.owner_user_id !== userId) continue;
      reminders.push({
    ...
    logger.warn(`[SmartReminder] Failed to fetch policy reviews: ${String(err)}`);
  }
...
export async function dismissReminder(
  ...
  try {
    await safeQuery(
      `INSERT INTO "${schema}".smart_reminder_dismissals (user_id, reminder_key, dismissed_at)
       VALUES ($1, $2, NOW())
       ON CONFLICT (user_id, reminder_key) DO UPDATE SET dismissed_at = NOW()`,
      [userId, reminderId],
    );
    return { dismissed: true };
  } catch (err: unknown) {
    logger.warn(`[SmartReminder] Failed to dismiss reminder: ${String(err)}`);
...
export async function getUpcomingDeadlines(
  tenantId: string,
  days: number = 14,
): Promise<{ deadlines: SmartReminder[]; summary: ReminderSummary }> {
  const allReminders = await generateReminders(tenantId);
  const deadlines = allReminders.filter(
    (r) => r.daysUntilDue <= days,
  );
