...
// ============================================
// Shahin-Ai — Evidence & Workflow Analytics Service
// ============================================

import { safeQuery, tenantSchema } from '../../ports/database.port';
import type { AnalyticsContext, AdvancedAnalyticsResult, DrillThroughPath, PredictiveInsight, RealTimeMetric } from '../advanced/advanced-analytics.types';
import { getLatestCycleResult as _getLatestCycleResult } from '../advanced/advanced-analytics.helpers';
import type { GenericRow } from '@dos/types';

export async function getAdvancedEvidenceAnalytics(ctx: AnalyticsContext): Promise<AdvancedAnalyticsResult> {
  const startTime = Date.now();
  const schema = tenantSchema(ctx.tenantId);
  try {
    const evidenceResult = await safeQuery(
      `SELECT e.evidence_id, e.title, e.status, e.submitted_at as collected_at, e.expiry_date,
    ...
    const coverageResult = await safeQuery(
      `SELECT COALESCE(c.framework_id, (c.frameworks[1])) as framework_code,
    ...
    const drillThrough: DrillThroughPath[] = coverageResult.rows.map((cov: GenericRow) => ({
    ...
    return {
      ...
      data: { evidence: evidenceResult.rows, coverage: coverageResult.rows, totalEvidence: evidenceResult.rows.length, expiredCount: evidenceResult.rows.filter((e: GenericRow) => e.validity_status === 'expired').length, expiringSoonCount: evidenceResult.rows.filter((e: GenericRow) => e.validity_status === 'expiring_soon').length, coverageRate: coverageResult.rows.length > 0 ? coverageResult.rows.reduce((sum: number, c: GenericRow) => sum + (c.controls_with_evidence / Math.max(1, c.total_controls)), 0) / coverageResult.rows.length : 0 },
    ...
    logger.error('[AdvancedAnalytics] Evidence analytics error:', err);
...
export async function getAdvancedWorkflowAnalytics(ctx: AnalyticsContext): Promise<AdvancedAnalyticsResult> {
  const startTime = Date.now();
  const schema = tenantSchema(ctx.tenantId);
  try {
    const workflowResult = await safeQuery(
      `SELECT w.execution_id, w.workflow_id, w.status, w.started_at, w.completed_at, w.step_log,
    ...
    const slaByRoleResult = await safeQuery(
      `SELECT COALESCE(a.current_approver_id, 'unassigned') as approver_role,
    ...
    const drillThrough: DrillThroughPath[] = slaByRoleResult.rows.map((sla: GenericRow) => ({
    ...
    return {
      ...
      data: { workflows: workflowResult.rows, slaByRole: slaByRoleResult.rows, totalPending: workflowResult.rows.length, totalBreached: slaByRoleResult.rows.reduce((sum: number, s: GenericRow) => sum + (Number(s.breached) || 0), 0), totalAtRisk: slaByRoleResult.rows.reduce((sum: number, s: GenericRow) => sum + (Number(s.at_risk) || 0), 0) },
      drillThrough, predictiveInsights, realTimeMetrics,
    ...
    logger.error('[AdvancedAnalytics] Workflow analytics error:', err);
...
async function getEvidenceRealTimeMetrics(ctx: AnalyticsContext): Promise<RealTimeMetric[]> {
  ...
  } catch (err: unknown) { logger.error('[AdvancedAnalytics] Evidence real-time metrics error:', err); }
  return metrics;
}

async function getWorkflowRealTimeMetrics(ctx: AnalyticsContext): Promise<RealTimeMetric[]> {
  const metrics: RealTimeMetric[] = [];
  try {
    const schema = tenantSchema(ctx.tenantId);
    const pr = await safeQuery(`SELECT COUNT(*)::int as count FROM "${schema}".approval_requests WHERE approval_status = 'pending'`);
    ...
    const br = await safeQuery(`SELECT COUNT(*)::int as count FROM "${schema}".approval_requests WHERE approval_status = 'pending' AND sla_deadline < NOW()`);
  ...
  } catch (err: unknown) { logger.error('[AdvancedAnalytics] Workflow real-time metrics error:', err); }
  return metrics;
}
