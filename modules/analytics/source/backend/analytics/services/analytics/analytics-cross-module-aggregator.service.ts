...
const grcHealthGauge = new prom.Gauge({
...
export interface GRCHealthDashboard {
  tenantId: string;
  riskPosture: { score: number; criticalCount: number; openCount: number };
  compliancePosture: { score: number; frameworkCount: number; gapCount: number };
  auditPosture: { closureRate: number; openFindings: number; criticalFindings: number };
  vendorPosture: { avgScore: number; criticalVendors: number; totalVendors: number };
  incidentPosture: { openCount: number; avgResolutionHours: number; criticalOpen: number };
  remediationPosture: { completionRate: number; overdueCount: number; inProgressCount: number };
  trainingPosture: { coverageRate: number; pendingCount: number };
  compositeScore: number;
  assessedAt: string;
}

export async function computeGRCHealthDashboard(tenantId: string): Promise<GRCHealthDashboard> {
  const schema = tenantSchema(tenantId);

  const [riskRes, compRes, auditRes, vendorRes, incidentRes, remRes, trainRes] = await Promise.all([
    safeQuery(
      `SELECT ROUND(AVG(residual_score)::numeric, 1) AS avg_score,
    ...
    safeQuery(
      `SELECT COUNT(DISTINCT cf.framework_id)::int AS fw_count,
    ...
    safeQuery(
      `SELECT COUNT(*)::int AS total,
       ...
       FROM "${schema}".vendors WHERE deleted_at IS NULL AND status = 'active'`,
    ).catch(() => ({ rows: [{}] })),
    ...
    safeQuery(
      `SELECT COUNT(*) FILTER (WHERE status = 'open')::int AS open_count,
       ...
       FROM "${schema}".incidents WHERE deleted_at IS NULL AND created_at > NOW() - INTERVAL '90 days'`,
    ).catch(() => ({ rows: [{}] })),
  ...
  const c = getFirstRow(compRes) ?? {};
  const a = getFirstRow(auditRes) ?? {};
  const v = getFirstRow(vendorRes) ?? {};
  const i = getFirstRow(incidentRes) ?? {};
  const rem = getFirstRow(remRes) ?? {};
  const t = getFirstRow(trainRes) ?? {};
  ...
  const compositeScore = Math.round(
    riskScore * 0.2 + compScore * 0.2 + auditClosureRate * 0.15 +
  ...
  try {
    await safeQuery(
      `INSERT INTO "${schema}".grc_health_snapshots
      ...
      [riskScore, compScore, auditClosureRate, vendorScore, Math.max(0, 100 - (i.open_count ?? 0) * 5), remCompletionRate, trainCoverage, compositeScore],
    );
...
export function registerAnalyticsCrossModuleSubscribers(): void {
  ...
  for (const trigger of refreshTriggers) {
    eventBus.subscribe(trigger as any, `analytics-agg:${trigger}`, async (event: PlatformEvent) => {
  ...
  logger.info(`[AnalyticsAggregator] ${refreshTriggers.length} cross-module dashboard subscribers registered`);
}
