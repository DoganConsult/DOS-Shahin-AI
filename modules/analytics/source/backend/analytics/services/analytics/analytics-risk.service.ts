...
// ============================================
// Shahin-Ai — Risk Analytics Service
...
import type {
  ...
  PredictiveInsight,
  RealTimeMetric,
} from '../advanced/advanced-analytics.types';
import { getLatestCycleResult } from '../advanced/advanced-analytics.helpers';
import type { GenericRow } from '@dos/types';

/**
 * Get comprehensive risk analytics with drill-through
 */
export async function getAdvancedRiskAnalytics(
  ...
  try {
    ...
    const heatmapResult = await safeQuery(
      `SELECT
        r.risk_id,
        r.title,
        r.likelihood,
    ...
    // Drill-through paths
    const drillThrough: DrillThroughPath[] = rows.flatMap((row, li) =>
      columns.map((col, im) => {
        const cellKey = `${li}-${im}`;
        ...
        return {
          ...
          payload: { likelihood: li + 1, impact: im + 1, riskIds: risks.map((r: GenericRow) => r.riskId) },
          route: `/risk?likelihood=${li + 1}&impact=${im + 1}`,
          children: risks.slice(0, 10).map((risk: GenericRow) => ({
            level: 2,
    ...
    return {
      ...
      data: {
        ...
        criticalRisks: heatmapResult.rows.filter((r: GenericRow) => r.risk_score >= 20).length,
        highRisks: heatmapResult.rows.filter((r: GenericRow) => r.risk_score >= 12 && r.risk_score < 20).length,
      },
      drillThrough,
      predictiveInsights,
      realTimeMetrics,
    ...
    logger.error('[AdvancedAnalytics] Risk analytics error:', err);
    return {
      widgetId: 'advanced-risk-heatmap',
...
async function computeRiskPredictiveInsights(
  ctx: AnalyticsContext,
  risks: unknown[]
  ...
  try {
    ...
    const criticalRisks = risks.filter((r: GenericRow) => r.risk_score >= 20);
    ...
    logger.error('[AdvancedAnalytics] Predictive insights error:', err);
  }
...
async function getRiskRealTimeMetrics(ctx: AnalyticsContext): Promise<RealTimeMetric[]> {
  const metrics: RealTimeMetric[] = [];

  try {
    // Get latest AGRC-OS cycle result (from orchestrator)
    const cycleResult = await getLatestCycleResult(ctx.tenantId);
    if (cycleResult) {
      const recomputedRiskCount = Number(cycleResult.risksRecomputed ?? 0);
      metrics.push({
        name: 'Risks Evaluated (Last Cycle)',
        nameAr: 'المخاطر المُقيّمة (الدورة الأخيرة)',

        value: recomputedRiskCount,
        unit: 'risks',
        trend: recomputedRiskCount > 0 ? 'up' : 'stable',
        changePercent: 0,
    ...
    const schema = tenantSchema(ctx.tenantId);
    const activeRisksResult = await safeQuery(
      `SELECT COUNT(*)::int as count FROM "${schema}".risks WHERE status NOT IN ('closed', 'deleted')`
    );
    const activeCount = activeRisksResult.rows[0]?.count || 0;

    // Compare with previous snapshot (if available) - use event_count as proxy
    const prevSnapshot = await safeQuery(
      `SELECT event_count FROM "${schema}".agrc_metrics_snapshots
    ...
    logger.error('[AdvancedAnalytics] Risk real-time metrics error:', err);
  }
