...
export async function forecastComplianceScore(tenantId: string, daysAhead: number = 90): Promise<{
  currentScore: number;
  ...
  const dataPoints = res.rows.map((r: GenericRow) => ({ date: String(r.date), score: Number(r.score) }));
...
export async function estimateRemediationTime(tenantId: string, severity: string): Promise<{
  ...
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), query(
    `SELECT EXTRACT(DAY FROM (completed_at - created_at))::int as days
  ...
  const days = res.rows.map((r: GenericRow) => Number(r.days)).filter(d => d > 0);
  if (days.length === 0) return { estimatedDays: 14, p50Days: 14, p90Days: 30, sampleSize: 0 };
...
export async function predictRiskEscalation(tenantId: string): Promise<Array<{
  riskId: string;
  title: string;
  currentScore: number;
  predictedScore: number;
  ...
  const schema = tenantSchema(tenantId);
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), query(
    `SELECT r.risk_id, r.title, r.likelihood * r.impact as current_score,
     ...
     GROUP BY r.risk_id, r.title, r.likelihood, r.impact`,
    [],
  ...
  return res.rows.map((r: GenericRow) => {
    ...
    if (history.length >= 3) {
      ...
      const increasing = recentTrend.every((v: number, i: number) =>
        i === 0 || v >= recentTrend[i - 1]);
      if (increasing) escalationProb = 0.7;
    }
    return {
      riskId: r.risk_id,
      title: r.title,
...
export async function forecastBCPReadiness(tenantId: string, daysAhead: number = 90): Promise<{
  ...
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), query(
    `SELECT TO_CHAR(DATE_TRUNC('month', ex.scheduled_date), 'YYYY-MM') AS date,
...
export async function predictRecoveryGap(tenantId: string): Promise<Array<{
  strategyId: string;
  title: string;
  ...
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), query(
    `SELECT rs.strategy_id, rs.title, rs.target_rto_hours, rs.target_rpo_hours,
     ...
     ORDER BY rs.strategy_id, ex.scheduled_date DESC`,
    [],
  ...
  for (const row of res.rows) {
    ...
    const list = grouped.get((key as any)) || [];
    if (list.length < 3) list.push(row);
    grouped.set((key as any), list);
  }
  ...
  return Array.from(grouped.entries()).map(([strategyId, exercises]) => {
    ...
    let trend: 'widening' | 'closing' | 'stable' = 'stable';
    if (exercises.length >= 2) {
      const prevGap = Math.max(0, Number(exercises[1].rto_actual_hours) - Number(exercises[1].target_rto_hours));
      if (rtoGap > prevGap + 0.5) trend = 'widening';
      else if (rtoGap < prevGap - 0.5) trend = 'closing';
    }
...
export async function estimateNextIncidentImpact(tenantId: string): Promise<{
  ...
  const [criticalProcesses, strategies, readiness] = await Promise.allSettled([
    ...
    query(
      `SELECT MAX(target_rto_hours)::numeric AS max_target_rto,
              MAX(COALESCE(
    ...
    query(
      `SELECT COUNT(*)::int AS unprotected FROM "${schema}".bia_assessments ba
  ...
  const critRows: any = getFirstRow((criticalProcesses as any).value) || { total: 0, critical_count: 0 };
  const stratRows: any = getFirstRow((strategies as any).value) || { max_target_rto: 24, max_actual_rto: 24 };

  const readinessRow = (getFirstRow((readiness as any).value) ?? {}) as Record<string, unknown>;
  const unprotectedCount = Number(readinessRow.unprotected ?? 0);

  const { getBCPReadinessScore } = await import('../../../bcp/services/bcp.service.js');
...
export async function getAnalyticsDashboard(tenantId: string): Promise<{
  ...
  const remediationEstimates: Record<string, unknown> = {};
