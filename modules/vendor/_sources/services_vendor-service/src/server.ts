...
import { routes, vendorRiskExtRouter } from './routes/index';
...
const SERVICE_CODE = 'vendor-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    routes: [
      ...
      { path: '/api/vendor-service', router: routes },
      // Phase 7 deeper (2026-04-30): top-level FE prefix lifted from
      // /api/vendor-service/vendor/vendor-risk-ext — FE callers
      // (grc-risk.service.ts, vendor-risk-dashboard.component.ts) hit
      // /api/vendor-risk directly.
      { path: '/api/vendor-risk', router: vendorRiskExtRouter },
    ],
    modules: await loadModuleRegistrations('vendor'),
    healthChecks: {
      database: async () => {
        try {
    ...
    onReady: () => {
      invokeModuleLifecycleHooks(SERVICE_CODE, 'vendor').catch(() => {});
    },
  });
