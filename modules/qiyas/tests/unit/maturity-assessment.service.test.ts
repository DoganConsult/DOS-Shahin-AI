...
import * as svc from '../../domain/maturity-assessment.service';
...
describe('maturity-assessment.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { title: 'test-title', framework: 'test-framework' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { title: 'x', framework: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
