...
import { emitQiyasEvent, emitQiyasStatusChange, emitAssessmentStarted, emitAssessmentCompleted, emitScoreCalculated, emitGapIdentified } from './qiyas-event.service';
...
describe('Qiyas Event Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('emitQiyasEvent', () => {
  ...
  describe('emitQiyasStatusChange', () => {
    it('should emit status_changed event', () => {
      emitQiyasStatusChange('t1', 'assessment' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
  ...
  describe('helper functions', () => {
    it('emitAssessmentStarted should not throw', () => {
      expect(() => emitAssessmentStarted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });
    ...
    it('emitScoreCalculated should not throw', () => {
      expect(() => emitScoreCalculated('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });
