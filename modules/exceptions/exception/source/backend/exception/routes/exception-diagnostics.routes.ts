import { Request, Response, Router } from 'express';
import { z } from "zod";

/**
 * Exception Diagnostics Routes
 * @owner exception
 * @module exception
 * @since 2026-03-31
 */

import { authenticate, requirePermission } from '../ports/auth.port';
import { auditMiddleware, asyncHandler, moduleStack } from '../ports/middleware.port';
import { ExceptionDiagnosticsService } from '../diagnostics/exception-diagnostics.service';
import { validate } from "../ports/middleware.port";
const router = Router();
router.use(moduleStack('exception'));
router.use(auditMiddleware('exception'));

const diagnostics = new ExceptionDiagnosticsService();

router.get('/diagnostics', authenticate, requirePermission('exception.record.read'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
  const result = await diagnostics.runDiagnostics(req.tenantId!);
  res.json({ success: true, data: result });
}));

export default router;
