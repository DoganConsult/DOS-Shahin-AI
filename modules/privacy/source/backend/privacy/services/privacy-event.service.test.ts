import { describe, it, expect, vi, beforeEach } from 'vitest';

vi.mock('../../platform/services/event/event-bus.service', () => ({
  eventBus: { publish: vi.fn() },
}));

import { emitPrivacyEvent, emitPrivacyStatusChange, emitDsrSubmitted, emitDsrCompleted, emitBreachDetected, emitBreachNotified } from './privacy-event.service';

vi.mock('../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
import { eventBus } from '../ports/events.port';


vi.mock('../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
describe('Privacy Event Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('emitPrivacyEvent', () => {
    it('should publish event to event bus', () => {
      emitPrivacyEvent({
        tenantId: 't1',
        entityType: 'dsr' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
    });

    it('should include correlationId in payload', () => {
      emitPrivacyEvent({
        tenantId: 't1',
        entityType: 'dsr' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
        correlationId: 'corr-123',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBe('corr-123');
    });

    it('should auto-generate correlationId if not provided', () => {
      emitPrivacyEvent({
        tenantId: 't1',
        entityType: 'dsr' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBeDefined();
    });

    it('should include timestamp and eventVersion', () => {
      emitPrivacyEvent({
        tenantId: 't1',
        entityType: 'dsr' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.timestamp).toBeDefined();
      expect(call.payload.eventVersion).toBe(1);
    });

    it('should not throw on publish failure', () => {
      (eventBus.publish as any).mockImplementationOnce(() => { throw new Error('fail'); });
      expect(() => emitPrivacyEvent({
        tenantId: 't1',
        entityType: 'dsr' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      })).not.toThrow();
    });
  });

  describe('emitPrivacyStatusChange', () => {
    it('should emit status_changed event', () => {
      emitPrivacyStatusChange('t1', 'dsr' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.action).toBe('status_changed');
    });
  });

  describe('helper functions', () => {
    it('emitDsrSubmitted should not throw', () => {
      expect(() => emitDsrSubmitted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitDsrCompleted should not throw', () => {
      expect(() => emitDsrCompleted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitBreachDetected should not throw', () => {
      expect(() => emitBreachDetected('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitBreachNotified should not throw', () => {
      expect(() => emitBreachNotified('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

  });
});
