...
export async function publish(
  ...
  if (!PRIVACY_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${PRIVACY_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: PRIVACY_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${PRIVACY_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(PRIVACY_EVENT_CONTRACT.published);
}
