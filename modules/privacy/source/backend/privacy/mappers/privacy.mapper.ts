import type { DataSubjectRequest, DsrCreateInput } from '@dos/types/privacy';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): DataSubjectRequest {

  return {
    dsr_id: row.dsr_id ?? row.id,
...
export function toEntityList(rows: GenericRow[]): DataSubjectRequest[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): DsrCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
...
export function toApiResponse(entity: DataSubjectRequest): Record<string, unknown> {

  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: DataSubjectRequest[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
