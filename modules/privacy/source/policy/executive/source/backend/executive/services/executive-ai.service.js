...
async function fetchExecutiveContext(tenantId) {
    const schema = (0, db_1.tenantSchema)(tenantId);
    try {
        const [kpi, risks, incidents, compliance] = await Promise.all([
            (0, db_1.safeQuery)(`SELECT compliance_score, risk_score FROM "${schema}".kpi_snapshots
           ORDER BY snapshot_date DESC LIMIT 1`),
            (0, db_1.safeQuery)(`SELECT COUNT(*) FILTER (WHERE risk_score >= 12 AND status='open')::int AS high_open,
                COUNT(*)::int AS total
         FROM "${schema}".risks WHERE deleted_at IS NULL`),
            (0, db_1.safeQuery)(`SELECT COUNT(*) FILTER (WHERE severity IN ('critical','high') AND status='open')::int AS critical_open
         FROM "${schema}".incidents WHERE deleted_at IS NULL`),
            (0, db_1.safeQuery)(`SELECT AVG(compliance_score)::numeric(5,2) AS avg_score
...
async function getAiRecommendations(tenantId, _context = {}) {
...
//# sourceMappingURL=executive-ai.service.js.map