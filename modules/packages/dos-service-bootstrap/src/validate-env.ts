...
const PLATFORM_ENV_RULES: EnvRule[] = [
  // Database
  ...
  { name: 'REDIS_URL', required: true, secret: true, description: 'Redis URL (must embed auth, e.g. redis://:<pw>@host:port/db)',
    validator: (v) => {
      try {
        ...
        if (process.env.REDIS_ALLOW_NOAUTH === 'true') return true;
 ...
 */
export function validateCriticalEnv(serviceCode: string, extraRules: EnvRule[] = []): EnvValidationResult {
