...
exports.tenantUserCacheKey = tenantUserCacheKey;
...
function tenantCacheKey(tenantId, key) {
    (0, tenant_1.assertTenantId)(tenantId);
    if (!key)
        throw new Error('tenantCacheKey: key is required');
    return `t:${tenantId}:${key}`;
}
function tenantUserCacheKey(tenantId, userId, key) {
    (0, tenant_1.assertTenantId)(tenantId);
    if (!userId)
        throw new Error('tenantUserCacheKey: userId is required');
    if (!key)
        throw new Error('tenantUserCacheKey: key is required');
