...
export interface CapabilitiesClientOptions {
  brokerBaseUrl?: string;     // default: http://127.0.0.1:4150 or env CAPABILITY_BROKER_URL
  gatewayBaseUrl?: string;    // default: http://127.0.0.1:4000 or env GATEWAY_URL
...
export class CapabilitiesClient {
  private cache = new Map<string, { value: CapabilityRecord; expiresAt: number }>();
  ...
  async resolve(capability: string): Promise<CapabilityRecord> {
    const now = Date.now();
    ...
    if (!r.ok) throw new Error(`broker_error_${r.status}: ${capability}`);
    const value = (await r.json()) as CapabilityRecord;
    if (!value.is_active) {
      this.cache.delete(capability);
  ...
  invalidate(capability?: string) {
    if (capability) this.cache.delete(capability);
    else this.cache.clear();
  }

  async invoke<T = unknown>(capability: string, opts: InvokeOptions = {}): Promise<T> {
    const cap = await this.resolve(capability);
    if (!cap.http_route) throw new Error(`capability_no_http_route: ${capability}`);
...
export async function enrollTenantModule(opts: EnrollOptions): Promise<EnrollResult> {
...
let _default: CapabilitiesClient | null = null;
export const capabilities = {
  client(): CapabilitiesClient {
    if (!_default) _default = new CapabilitiesClient();
    return _default;
  },
  ...
  invoke<T = unknown>(capability: string, opts?: InvokeOptions): Promise<T> {
    return this.client().invoke<T>(capability, opts);
  },
  invalidate(capability?: string) { this.client().invalidate(capability); },
  configure(opts: CapabilitiesClientOptions) { _default = new CapabilitiesClient(opts); },
};
