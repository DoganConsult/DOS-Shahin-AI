 ...
 */
export function registerNavigationIntegration(moduleCode, opts) {
 ...
 */
export async function registerModuleNavItems(tenantId, moduleCode, items) {
    const schema = tenantSchema(tenantId);
    let registered = 0;
    let skipped = 0;
    for (const item of items) {
        try {
            const result = await safeQuery(`INSERT INTO "${schema}".module_nav_registration
    ...
    for (const item of items) {
        await safeQuery(`INSERT INTO "${schema}".navigation_registry
       ...
       ON CONFLICT (nav_key) DO NOTHING`, [item.navKey, item.parentNavKey ?? null, item.labelEn, item.labelAr ?? '',
            ...
            item.sortOrder ?? 0, item.permissionCode ?? null]).catch(catchHandler(EC.EVENT_BUS));
    }
 ...
 */
export async function trackNavigationAccess(tenantId, userId, navKey, route, moduleCode, sessionId) {
    const schema = tenantSchema(tenantId);
    // Insert usage log
    await safeQuery(`INSERT INTO "${schema}".navigation_usage_log
      (tenant_id, user_id, nav_key, route, module_code, session_id)
     VALUES ($1, $2, $3, $4, $5, $6)`, [tenantId, userId, navKey, route, moduleCode, sessionId ?? null]).catch(catchHandler(EC.EVENT_BUS));
    // Bump frequency_score on navigation_registry
    await safeQuery(`UPDATE "${schema}".navigation_registry
     SET frequency_score = frequency_score + 1, last_accessed_at = now()
     WHERE nav_key = $1`, [navKey]).catch(catchHandler(EC.EVENT_BUS));
}
/**
 * Get navigation usage stats for a tenant (top items, trending, etc.)
 */
export async function getNavigationUsageStats(tenantId, opts) {
    const schema = tenantSchema(tenantId);
    const days = opts?.days ?? 30;
    const limit = opts?.limit ?? 20;
    const { rows } = await safeQuery(`SELECT nav_key, route, module_code,
