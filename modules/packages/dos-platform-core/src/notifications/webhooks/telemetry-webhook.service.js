...
export async function processWebhook(tenantId, payload) {
    ...
    await safeQuery(`INSERT INTO public.telemetry_webhook_events (event_id, tenant_id, source_name, event_type, severity, payload) VALUES ($1, $2, $3, $4, $5, $6::jsonb)`, [eventId, tenantId, sourceName, eventType, severity, JSON.stringify(payload)]);
...
export async function authenticateWebhook(apiKey, rawBody, signature) {
    const result = await safeQuery(`SELECT key_id, label, scopes, expires_at FROM public.api_keys WHERE hashed_key = $1 LIMIT 1`, [hashKey(apiKey)]);
...
export async function createWebhookApiKey(tenantId, keyName, sourceName, enableHmac = false) {
    const keyId = randomUUID();
    const apiKey = randomBytes(24).toString('hex');
    const createdAt = new Date().toISOString();
    const scopes = { kind: 'telemetry_webhook', tenantId, sourceName, enableHmac };
    await safeQuery(`INSERT INTO public.api_keys (key_id, hashed_key, label, scopes, created_at) VALUES ($1, $2, $3, $4::jsonb, $5)`, [keyId, hashKey(apiKey), keyName, JSON.stringify(scopes), createdAt]);
    return { keyId, apiKey, keyName, sourceName, enableHmac, createdAt };
}
export async function listWebhookApiKeys(tenantId) {
...
export async function revokeWebhookApiKey(tenantId, keyId) {
