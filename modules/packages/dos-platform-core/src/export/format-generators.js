...
export function humanizeHeader(field) {
    return field
        .replace(/_/g, ' ')
...
// ── XML ──────────────────────────────────────────────────────────────────
export function toXML(data, moduleCode) {
...
// ── XLSX ─────────────────────────────────────────────────────────────────
export function toXLSX(data, columns, sheetName = 'Export') {
...
export function toPDF(data, title, columns, watermark) {
    const headers = columns && columns.length > 0 ? columns : (data.length > 0 ? Object.keys(data[0]) : []);
    const headerLabels = headers.map(h => humanizeHeader(h));
    const lines = [];
    lines.push(title);
