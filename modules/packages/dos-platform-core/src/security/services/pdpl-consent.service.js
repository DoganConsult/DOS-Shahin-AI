...
async function appendConsentLog(entry) {
    ...
    await safeQuery(`INSERT INTO public.pdpl_consent_log
...
export async function getConsentStatus(tenantId, userId, consentType = DEFAULT_CONSENT_TYPE) {
    const result = await safeQuery(`SELECT user_id, consent_type, granted, granted_at, expires_at
...
export async function grantConsent(tenantId, userId, purpose, actorId) {
    const grantedAt = new Date().toISOString();
    await safeQuery(`INSERT INTO public.pdpl_consents (user_id, consent_type, granted, granted_at, expires_at)
...
export async function revokeConsent(tenantId, userId, actorId) {
    const grantedAt = new Date().toISOString();
    await safeQuery(`INSERT INTO public.pdpl_consents (user_id, consent_type, granted, granted_at, expires_at)
...
export async function rightToForget(tenantId, userId, actorId) {
...
export async function getConsentLog(tenantId, userId) {
    await ensureConsentLogTable();
    try {
        ...
        const result = await safeQuery(`SELECT tenant_id, user_id, consent_type, granted, action, actor_id, purpose, occurred_at, expires_at
