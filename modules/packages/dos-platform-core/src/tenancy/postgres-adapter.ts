...
import { setTenancyHandler, type PlatformTenancy, type ProvisionedTenant } from './tenancy';
 ...
 */
const ELIGIBLE_STATUSES = ['active', 'onboarding'] as const;

export class PostgresTenancyAdapter implements PlatformTenancy {
  async getProvisionedTenants(): Promise<ProvisionedTenant[]> {
    try {
      const placeholders = ELIGIBLE_STATUSES.map((_, i) => `$${i + 1}`).join(',');
      const result = await safeQuery<TenantRow>(
        `SELECT tenant_id, settings, status, plan, schema_name
           FROM public.tenants
 ...
 */
export function registerTenancyAdapter(): void {
