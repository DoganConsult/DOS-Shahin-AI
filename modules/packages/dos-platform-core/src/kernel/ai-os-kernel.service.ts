...
import { emptyResult, safeQuery, tenantSchema, query as dbQuery, toErrorMessage } from '@dos/db';
import { logger } from '../observability';
import type { KernelLogEvent } from './ai-os-kernel.types';
import { swallowDefault, EC } from '../resilience/resilience';
...
export async function getKernelStatus(tenantId: string): Promise<KernelStatus> {
  ...
  const proc = (processRes.rows[0] || {}) as R;
  const agent = (agentRes.rows[0] || {}) as R;
  const circuit = (circuitRes.rows[0] || {}) as R;
  const lat = (latencyRes.rows[0] || {}) as R;
  const ipc = (ipcRes.rows[0] || {}) as R;
...
export async function getSchedulerTable(tenantId: string): Promise<SchedulerEntry[]> {
  ...
  try {
    ...
    return jobRes.rows.map((r: Record<string, unknown>, i: number) => {
...
export async function rebootAgent(
  ...
  try {
    ...
    try {
      ...
      const reset = (globalThis as { __resetAgentCircuit?: (t: string, a: string) => void }).__resetAgentCircuit;
      if (reset) {
...
async function logKernelEvent(tenantId: string, eventType: string, entityId: string, payload: Record<string, unknown>): Promise<void> {
  try {
    const s = tenantSchema(tenantId);
    await safeQuery(
...
export async function getAgentDetail(tenantId: string, agentId: string): Promise<AgentDetail | null> {
  ...
  try {
    ...
    const cfg = configRes.rows[0] as Record<string, unknown> | undefined;
    if (!cfg) return null;

    const cb = (circuitRes.rows[0] || {}) as Record<string, unknown>;
    const runs = (runsRes.rows[0] || {}) as Record<string, unknown>;
    const mem = memRes.rows[0] as Record<string, unknown> | undefined;
...
export async function listKernelSnapshots(tenantId: string, limit = 20): Promise<Array<{ snapshotId: string; createdAt: string; data: Record<string, unknown> }>> {
  const schema = tenantSchema(tenantId);
  try {
    const result = await safeQuery(
