import type { LifecycleDefinition, ModuleRegistrationContract, ProductManifest, ProvisioningContract, TenantContract, WorkspaceContract } from '@dos/contracts';
import type { EventRegistration, PlatformEvent, NotificationPayload, NotificationRecord, SearchQuery, SearchResponse, SearchEntityType, ScheduledJob, JobRun } from '@dos/types';
...
export interface DosStoragePort {
    put(tenantId: string, key: string, data: Buffer | Uint8Array | string, metadata?: Record<string, string>): Promise<{
        key: string;
        size: number;
    }>;
    ...
    list(tenantId: string, prefix?: string): Promise<Array<{
        key: string;
        size: number;
        updatedAt: string;
    }>>;
...
export interface DosSearchPort {
    ...
    indexDocument(tenantId: string, entityType: SearchEntityType, entityId: string, document: Record<string, unknown>): Promise<void>;
