...
function authHeaders(ctx?: OpenClawContext): Record<string, string> {
...
function mapGatewayToolRow(row: Record<string, unknown>): Record<string, unknown> {
  const name = (row.tool_name ?? row.toolName ?? row.name) as string | undefined;
  return {
    name: name ?? 'unknown-tool',
    description: (row.description as string | undefined) ?? '',
    inputSchema: (row.input_schema ?? row.inputSchema) as Record<string, unknown> | undefined,
    serverId: row.server_id ?? row.serverId,
    toolId: row.tool_id ?? row.toolId,
  };
}

function normalizeToolsPayload(body: unknown): Array<Record<string, unknown>> {
  if (Array.isArray(body)) {
    return body.map((t) => (typeof t === 'object' && t ? (t as Record<string, unknown>) : { name: 'tool' }));
  }
  if (body && typeof body === 'object') {
    ...
    if (Array.isArray(tools)) {
      return tools.map((t) =>
        typeof t === 'object' && t ? (t as Record<string, unknown>) : { name: 'tool' },
      );
    }
  }
  return [];
...
function normalizeResourcesPayload(body: unknown): Array<Record<string, unknown>> {
  if (Array.isArray(body)) {
    return body.map((r) => (typeof r === 'object' && r ? (r as Record<string, unknown>) : { uri: '', name: '' }));
  }
  if (body && typeof body === 'object') {
    const o = body as Record<string, unknown>;
    ...
    if (Array.isArray(res)) {
      return res.map((r) =>
        typeof r === 'object' && r ? (r as Record<string, unknown>) : { uri: '', name: '' },
      );
    }
  }
  return [];
...
export async function bridgeIsOpenClawAvailable(): Promise<boolean> {
...
export async function bridgeGetOpenClawServiceConfig(): Promise<Record<string, unknown>> {
...
export async function bridgeListOpenClawTools(ctx?: OpenClawContext): Promise<Array<Record<string, unknown>>> {
  const httpBase = envStr('OPENCLAW_HTTP_BASE_URL');
  if (httpBase) {
    ...
    return raw.map((t) => ({
      ...
      inputSchema: (t.inputSchema ?? t.input_schema) as Record<string, unknown> | undefined,
    }));
  }
  ...
  if (gw && ctx?.tenantId && ctx?.authorization) {
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        inputSchema: m.inputSchema as Record<string, unknown> | undefined,
      };
    });
  }
...
export async function bridgeListOpenClawResources(
  ...
  return raw.map((r) => ({
    ...
    content: (r.content ?? r.payload) as Record<string, unknown> | undefined,
  }));
}

export async function bridgeExecuteOpenClawTool(
  tool: string,
  ...
  if (!httpBase) {
    throw new Error(
      'OpenClaw tool execution requires OPENCLAW_HTTP_BASE_URL pointing to an HTTP executor; MCP gateway lists tools only.',
    );
  }
  ...
  if (body && typeof body === 'object' && 'result' in (body as object)) {
    return { ...(body as Record<string, unknown>), executionTime: elapsed };
  }
  return { result: body as Record<string, unknown>, executionTime: elapsed };
}
