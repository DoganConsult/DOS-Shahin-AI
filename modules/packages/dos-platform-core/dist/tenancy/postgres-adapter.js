...
exports.PostgresTenancyAdapter = void 0;
exports.registerTenancyAdapter = registerTenancyAdapter;
...
class PostgresTenancyAdapter {
    async getProvisionedTenants() {
        try {
            const placeholders = ELIGIBLE_STATUSES.map((_, i) => `$${i + 1}`).join(',');
            const result = await (0, db_1.safeQuery)(`SELECT tenant_id, settings, status, plan, schema_name
           FROM public.tenants
          ...
          ORDER BY created_at ASC`, [...ELIGIBLE_STATUSES]);
            ...
            observability_1.logger.error('[tenancy-adapter] getProvisionedTenants failed', {
...
exports.PostgresTenancyAdapter = PostgresTenancyAdapter;
let _registered = false;
/**
 * Register the canonical Postgres tenancy adapter with @dos/platform-core.
 * Idempotent — safe to call from multiple entry points.
 */
function registerTenancyAdapter() {
    if (_registered)
        return;
    (0, tenancy_1.setTenancyHandler)(new PostgresTenancyAdapter());
    _registered = true;
    observability_1.logger.info('[tenancy-adapter] Registered PostgresTenancyAdapter with @dos/platform-core/tenancy');
}
