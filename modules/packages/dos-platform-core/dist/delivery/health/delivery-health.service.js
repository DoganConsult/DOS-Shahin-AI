...
exports.getDeliveryHealthSnapshot = getDeliveryHealthSnapshot;
exports.getDeploymentSuccessRateByEnvironment = getDeploymentSuccessRateByEnvironment;
exports.getMigrationHealthByRelease = getMigrationHealthByRelease;
const db_1 = require("@dos/db");
async function getDeliveryHealthSnapshot(tenantId) {
    const now = new Date().toISOString();
    const releaseResult = await (0, db_1.safeQuery)(`SELECT
       COUNT(*) FILTER (WHERE status NOT IN ('released','rolled_back','cancelled')) AS active_releases
     FROM public.dos_releases`, []);
    const migrationResult = await (0, db_1.safeQuery)(`SELECT
       COUNT(*) FILTER (WHERE status IN ('pending','running')) AS pending_migrations,
       COUNT(*) FILTER (WHERE status = 'failed') AS failed_migrations
     FROM public.dos_migrations${tenantId ? ' WHERE tenant_id = $1' : ''}`, tenantId ? [tenantId] : []);
    let deploymentResult;
    if (tenantId) {
        const schema = `tenant_${tenantId.replace(/-/g, '_')}`;
        deploymentResult = await (0, db_1.safeQuery)(`SELECT
         COUNT(*) AS total,
         COUNT(*) FILTER (WHERE status = 'completed') AS successful,
         COUNT(*) FILTER (WHERE status = 'failed') AS failed,
         COUNT(*) FILTER (WHERE status = 'rolled_back') AS rolled_back
       FROM dos_deployments`, []).catch(() => ({ rows: [{ total: 0, successful: 0, failed: 0, rolled_back: 0 }] }));
    }
    else {
...
async function getDeploymentSuccessRateByEnvironment(tenantId, environment) {
    const schema = `tenant_${tenantId.replace(/-/g, '_')}`;
    const result = await (0, db_1.safeQuery)(`SELECT
       COUNT(*) AS total,
       COUNT(*) FILTER (WHERE status = 'completed') AS successful
     FROM dos_deployments WHERE environment = $1`, [environment]).catch(() => ({ rows: [{ total: 0, successful: 0 }] }));
    const total = Number(result.rows[0]?.total ?? 0);
    const successful = Number(result.rows[0]?.successful ?? 0);
    return total > 0 ? Math.round((successful / total) * 100) : 100;
}
async function getMigrationHealthByRelease(releaseId) {
    const result = await (0, db_1.safeQuery)(`SELECT
       COUNT(*) AS total,
...
//# sourceMappingURL=delivery-health.service.js.map