...
exports.resolveWorkspaceProfile = resolveWorkspaceProfile;
exports.getEffectiveFeatures = getEffectiveFeatures;
exports.getWorkspaceModules = getWorkspaceModules;
exports.getWorkspaceTheme = getWorkspaceTheme;
exports.updateWorkspaceProfile = updateWorkspaceProfile;
...
function cacheKey(tenantId, workspaceId) {
    return `${tenantId}:${workspaceId}`;
}
function getCachedProfile(tenantId, workspaceId) {
...
function setCachedProfile(tenantId, workspaceId, profile) {
 ...
 */
async function resolveWorkspaceProfile(tenantId, workspaceId) {
    const cached = getCachedProfile(tenantId, workspaceId);
    if (cached)
        return cached;
    try {
        // Load base workspace profile record
        const { rows } = await (0, db_1.safeQuery)(`SELECT workspace_id, tenant_id, display_name, description, locale, timezone,
       ...
       LIMIT 1`, [workspaceId, tenantId]);
        ...
        if (!rows[0]) {
            const { rows: wsRows } = await (0, db_1.safeQuery)(`SELECT workspace_id, tenant_id, name, settings, created_at
         FROM workspaces
         WHERE workspace_id = $1 AND tenant_id = $2 LIMIT 1`, [workspaceId, tenantId]);
            if (!wsRows[0])
                return null;
            const ws = wsRows[0];
        ...
        logger_1.logger.error(`[WorkspaceProfileRuntime] Failed to resolve profile for workspace ${workspaceId}: ${err.message}`);
        return null;
    }
}
/**
 ...
 */
async function getEffectiveFeatures(tenantId, workspaceId) {
    const features = {};
    try {
        // Tenant-level feature flags (baseline)
        const schema = (0, db_1.tenantSchema)(tenantId);
        const { rows: tenantFlags } = await (0, db_1.safeQuery)(`SELECT feature_key, enabled FROM "${schema}".feature_flags
    ...
    try {
        ...
        const { rows: wsFlags } = await (0, db_1.safeQuery)(`SELECT feature_key, enabled FROM workspace_feature_overrides
       WHERE workspace_id = $1 AND tenant_id = $2
       ORDER BY feature_key`, [workspaceId, tenantId]);
 ...
 */
async function getWorkspaceModules(tenantId, workspaceId) {
    try {
        // First try workspace-specific module assignments
        const { rows: wsModules } = await (0, db_1.safeQuery)(`SELECT module_code FROM workspace_modules
       WHERE workspace_id = $1 AND tenant_id = $2 AND is_enabled = TRUE
       ORDER BY module_code`, [workspaceId, tenantId]);
        if (wsModules.length > 0) {
            return wsModules.map((r) => r.module_code);
        }
        // Fall back to tenant-level module entitlements
        const { rows: tenantModules } = await (0, db_1.safeQuery)(`SELECT module_code FROM public.module_operating_states
       WHERE tenant_id = $1 AND is_active = TRUE
       ORDER BY module_code`, [tenantId]);
        return tenantModules.map((r) => r.module_code);
    }
    catch (err) {
        logger_1.logger.error(`[WorkspaceProfileRuntime] Failed to get modules for workspace ${workspaceId}: ${err.message}`);
        return [];
    }
}
/**
 ...
 */
async function getWorkspaceTheme(tenantId, workspaceId) {
    const defaultTheme = {
        primaryColor: null,
        logoUrl: null,
        faviconUrl: null,
    ...
    try {
        ...
        const { rows: wsThemeRows } = await (0, db_1.safeQuery)(`SELECT primary_color, logo_url, favicon_url, brand_name, custom_css
       ...
       LIMIT 1`, [workspaceId, tenantId]);
        ...
        const { rows: tenantThemeRows } = await (0, db_1.safeQuery)(`SELECT primary_color, logo_url, favicon_url, brand_name, custom_css
       ...
       LIMIT 1`, [tenantId]);
        ...
        logger_1.logger.error(`[WorkspaceProfileRuntime] Failed to get theme for workspace ${workspaceId}: ${err.message}`);
        return defaultTheme;
    }
}
/**
 ...
 */
async function updateWorkspaceProfile(tenantId, workspaceId, profile) {
    const sets = [];
    ...
    try {
        ...
        await (0, db_1.safeQuery)(`INSERT INTO workspace_profiles (workspace_id, tenant_id, display_name, updated_at)
       VALUES ($1, $2, '', NOW())
       ON CONFLICT (workspace_id, tenant_id) DO NOTHING`, [workspaceId, tenantId]);
        await (0, db_1.safeQuery)(`UPDATE workspace_profiles SET ${sets.join(', ')}
       WHERE workspace_id = $1 AND tenant_id = $2`, params);
        logger_1.logger.info(`[WorkspaceProfileRuntime] Updated profile for workspace ${workspaceId}`);
        // Invalidate cache after update
        invalidateProfileCache(tenantId, workspaceId);
        return resolveWorkspaceProfile(tenantId, workspaceId);
    }
    catch (err) {
        logger_1.logger.error(`[WorkspaceProfileRuntime] Failed to update profile for workspace ${workspaceId}: ${err.message}`);
        throw err;
    }
}
/**
 ...
 */
function invalidateProfileCache(tenantId, workspaceId) {
    const key = cacheKey(tenantId, workspaceId);
    const existed = profileCache.delete(key);
    if (existed) {
        logger_1.logger.info(`[WorkspaceProfileRuntime] Cache invalidated for workspace ${workspaceId}`);
    }
}
/**
 ...
 */
async function getWorkspaceCapabilities(tenantId, workspaceId) {
    const capabilitySet = new Set();
    try {
        // Product-level capabilities from tenant entitlement
        const { rows: productCaps } = await (0, db_1.safeQuery)(`SELECT capability_code FROM product_capabilities
       WHERE tenant_id = $1 AND is_enabled = TRUE
       ORDER BY capability_code`, [tenantId]);
    ...
    try {
        ...
        if (modules.length > 0) {
            ...
            const { rows: modCaps } = await (0, db_1.safeQuery)(`SELECT DISTINCT capability_code FROM module_capabilities
         WHERE module_code IN (${placeholders}) AND is_enabled = TRUE`, modules);
    ...
    try {
        ...
        const { rows: wsOverrides } = await (0, db_1.safeQuery)(`SELECT capability_code, is_enabled FROM workspace_capability_overrides
       WHERE workspace_id = $1 AND tenant_id = $2`, [workspaceId, tenantId]);
...
//# sourceMappingURL=workspace-profile-runtime.service.js.map