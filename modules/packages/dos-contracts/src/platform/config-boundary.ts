...
export const VALID_SERVER_ROLES = ['web', 'web+jobs', 'jobs-only'] as const;
export type ServerRole = typeof VALID_SERVER_ROLES[number];

export const VALID_TOPOLOGIES = ['single-node', 'multi-node'] as const;
...
export function getServerRole(): ServerRole {
  ...
  if (!VALID_SERVER_ROLES.includes(raw as ServerRole)) {
    throw new Error(`Invalid SERVER_ROLE '${raw}' — must be one of: ${VALID_SERVER_ROLES.join(', ')}`);
  }
  return raw as ServerRole;
}

export function getProductionTopology(): ProductionTopology {
  const raw = process.env.PRODUCTION_TOPOLOGY || 'single-node';
  if (!VALID_TOPOLOGIES.includes(raw as ProductionTopology)) {
    throw new Error(`Invalid PRODUCTION_TOPOLOGY '${raw}' — must be one of: ${VALID_TOPOLOGIES.join(', ')}`);
  }
  return raw as ProductionTopology;
}

export function isSchedulerEnabled(): boolean {
  if (process.env.DISABLE_SCHEDULERS === 'true') return false;
  const role = getServerRole();
  return role === 'web+jobs' || role === 'jobs-only';
}

export function isMigrationOwner(): boolean {
  const role = getServerRole();
  return role === 'web+jobs' || role === 'jobs-only';
}
