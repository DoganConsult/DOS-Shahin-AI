    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                const rows = await (0, module_sdk_1.safeRows)(`SELECT org_name, org_type, legal_form, listing_status, org_size, employee_count,
           ...
           FROM tenants WHERE tenant_id = $1`, [tenantId]);
                ...
                const missing = [];
                if (!t.org_type)
                    missing.push('org_type');
                if (!t.primary_sector_id)
                    missing.push('primary_sector_id');
                if (!t.employee_count)
                    missing.push('employee_count');
                if (!t.data_classification_level)
                    missing.push('data_classification_level');
                if (t.has_ciso === null)
                    missing.push('has_ciso');
                if (t.has_dpo === null)
                    missing.push('has_dpo');
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                ...
                const frameworks = await (0, module_sdk_1.safeRows)(`SELECT framework_id, name, status, controls_total, controls_compliant FROM "${schema}".frameworks ORDER BY name`);
                const controlCount = await (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS n FROM "${schema}".ucf_controls`);
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                ...
                const [risks, controls, policies, evidence, incidents, tasks, recentActivity] = await Promise.all([
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status != 'closed')::int AS open FROM "${schema}".risks`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total FROM "${schema}".ucf_controls`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'draft')::int AS draft FROM "${schema}".policies`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE expires_at < NOW())::int AS expired FROM "${schema}".evidence`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'open')::int AS open FROM "${schema}".incidents`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'open')::int AS open FROM "${schema}".tasks`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS n FROM "${schema}".activity_feed WHERE created_at > NOW() - INTERVAL '24 hours'`),
                ]);
                return {
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const task = await (0, workflows_1.createProcessTask)(tenantId, {
                    title: `[A01] ${input.title}`,
...
//# sourceMappingURL=a01-onboarding-tools.js.map