    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const programs = await (0, module_sdk_1.safeRows)(`SELECT program_id, title, category, status, created_at
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const rows = await (0, module_sdk_1.safeRows)(`INSERT INTO "${schema}".training_assignments (program_id, user_id, status, due_date, assigned_at)
           VALUES ($1, $2, 'assigned', $3, NOW()) RETURNING assignment_id`, [input.programId, input.userId, input.dueDate || null]);
                if (rows.length === 0)
                    return { assigned: false, reason: 'Insert failed' };
                await (0, observability_1.recordAudit)({ tenantId, userId: 'agent-A12', module: 'training', action: 'create', entityType: 'training_assignment', entityId: String(rows[0].assignment_id), afterState: { programId: input.programId, userId: input.userId, dueDate: input.dueDate } });
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const stats = await (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total,
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const rows = await (0, module_sdk_1.safeRows)(`INSERT INTO "${schema}".training_programs (title, description, category, status, created_at)
           VALUES ($1, $2, 'awareness_campaign', 'draft', NOW()) RETURNING program_id`, [`[A12] ${input.title}`, input.description || '']);
                if (rows.length === 0)
                    return { created: false, reason: 'Insert failed' };
                await (0, observability_1.recordAudit)({ tenantId, userId: 'agent-A12', module: 'training', action: 'create', entityType: 'training_program', entityId: String(rows[0].program_id), afterState: { title: input.title, category: 'awareness_campaign' } });
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const gaps = await (0, module_sdk_1.safeRows)(`SELECT u.user_id, u.email, u.full_name,
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                ...
                const [roles, programs] = await Promise.all([
                    (0, module_sdk_1.safeRows)(`SELECT DISTINCT role FROM users WHERE tenant_id = $1 AND status = 'active'`, [tenantId]),
                    (0, module_sdk_1.safeRows)(`SELECT program_id, title, category FROM "${schema}".training_programs WHERE status = 'published' AND deleted_at IS NULL`),
                ]);
                return { activeRoles: roles.map((r) => r.role), availablePrograms: programs, recommendation: 'Assign mandatory compliance training to all roles with access to sensitive data' };
            },
        },
    ];
}
//# sourceMappingURL=a12-training-tools.js.map