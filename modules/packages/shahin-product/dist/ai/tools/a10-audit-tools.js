    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const findings = await (0, module_sdk_1.safeRows)(`SELECT finding_id, title, severity, status, source_type, source_id, created_at
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                ...
                const [controls, evidence, policies, findings] = await Promise.all([
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE test_status = 'passed')::int AS passed FROM "${schema}".ucf_controls`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'active' AND (expires_at IS NULL OR expires_at > NOW()))::int AS current FROM "${schema}".evidence`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'published')::int AS published FROM "${schema}".policies`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS open_count FROM "${schema}".findings WHERE status = 'open'`),
                ]);
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                ...
                const [fw, risks, gaps, remediations] = await Promise.all([
                    (0, module_sdk_1.safeRows)(`SELECT name, controls_total, controls_compliant FROM "${schema}".frameworks ORDER BY name`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE risk_score > 15)::int AS high, AVG(risk_score)::numeric(5,1) AS avg_score FROM "${schema}".risks WHERE status != 'closed'`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS open FROM "${schema}".compliance_gaps WHERE status = 'open'`),
                    (0, module_sdk_1.safeRows)(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'completed')::int AS done FROM "${schema}".remediation_tasks`),
                ]);
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const rows = await (0, module_sdk_1.safeRows)(`INSERT INTO "${schema}".findings (title, description, severity, status, source_type, created_at)
           VALUES ($1, $2, $3, 'open', $4, NOW()) RETURNING finding_id`, [`[A10] ${input.title}`, input.description, input.severity, input.sourceType || 'agent_A10']);
                ...
                await (0, observability_1.recordAudit)({ tenantId, userId: 'agent-A10', module: 'audit', action: 'create', entityType: 'finding', entityId: String(rows[0].finding_id), afterState: { title: input.title, severity: input.severity, sourceType: input.sourceType || 'agent_A10' } });
