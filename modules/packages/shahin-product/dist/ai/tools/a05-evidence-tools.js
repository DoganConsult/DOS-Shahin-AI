    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const expired = await (0, module_sdk_1.safeRows)(`SELECT evidence_id, title, linked_entity_type, linked_entity_id, expires_at FROM "${schema}".evidence WHERE expires_at < NOW() AND status != 'archived' ORDER BY expires_at LIMIT 30`);
                const noEvidence = await (0, module_sdk_1.safeRows)(`SELECT c.control_id, c.code, c.title FROM "${schema}".ucf_controls c
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const stats = await (0, module_sdk_1.safeRows)(`SELECT
             COUNT(*)::int AS total,
...
//# sourceMappingURL=a05-evidence-tools.js.map