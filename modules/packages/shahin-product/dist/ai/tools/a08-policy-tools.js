    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const policies = await (0, module_sdk_1.safeRows)(`SELECT policy_id, title, status, review_date, version, owner, created_at, updated_at
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const frameworks = await (0, module_sdk_1.safeRows)(`SELECT framework_id, name FROM "${schema}".frameworks`);
                const policies = await (0, module_sdk_1.safeRows)(`SELECT policy_id, title, tags FROM "${schema}".policies WHERE status IN ('published', 'draft')`);
