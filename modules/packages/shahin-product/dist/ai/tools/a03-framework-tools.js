    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const frameworks = await (0, module_sdk_1.safeRows)(`SELECT framework_id, name, status, controls_total, controls_compliant, controls_partial, controls_not_met FROM "${schema}".frameworks ORDER BY name`);
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const unmapped = await (0, module_sdk_1.safeRows)(`SELECT control_id, code, title FROM "${schema}".ucf_controls WHERE mapped_frameworks IS NULL OR mapped_frameworks = '[]'::jsonb LIMIT 50`);
                const lowCoverage = await (0, module_sdk_1.safeRows)(`SELECT framework_id, name, controls_total, controls_compliant FROM "${schema}".frameworks WHERE controls_total > 0 AND (controls_compliant::float / controls_total::float) < 0.5`);
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = (0, db_1.tenantSchema)(tenantId);
                const mappings = await (0, module_sdk_1.safeRows)(`SELECT source_control_id, target_requirement_id, relationship, confidence
...
//# sourceMappingURL=a03-framework-tools.js.map