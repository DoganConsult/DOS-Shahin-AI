...
exports.buildA02Tools = buildA02Tools;
    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const users = await (0, module_sdk_1.safeRows)(`SELECT user_id, email, display_name AS name, role, platform_role,
        ...
        },
        {
            ...
            input_schema: { type: 'object', properties: {}, required: [] },
            handler: async (tenantId) => {
                const roles = await (0, module_sdk_1.safeRows)(`SELECT role, COUNT(*)::int AS cnt, COUNT(*) FILTER (WHERE is_active)::int AS active
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                ...
                await events_1.eventBus.publish({ eventType: 'agent.action_proposed', tenantId, sourceService: 'agent-A02', severity: input.priority === 'critical' ? 'critical' : 'warning', payload: { agentId: 'A02', title: input.title } });
                return { flagged: true };
            },
        },
        {
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const users = await (0, module_sdk_1.safeRows)(`SELECT user_id, email, name, role, is_active, mfa_enabled, last_login_at FROM users WHERE tenant_id = $1 ORDER BY name`, [tenantId]);
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                ...
                const roles = await (0, module_sdk_1.safeRows)(`SELECT ura.role_id, r.role_code, r.name_en FROM "${schema}".user_role_assignments ura JOIN "${schema}".roles r ON r.role_id = ura.role_id WHERE ura.user_id = $1 AND ura.active = TRUE`, [input.userId]);
        ...
        },
        {
            ...
            input_schema: { type: 'object', properties: {}, required: [] },
            handler: async (tenantId) => {
                const config = await (0, module_sdk_1.safeRows)(`SELECT key, value FROM tenant_preferences WHERE tenant_id = $1 AND key LIKE 'sso_%'`, [tenantId]);
        ...
        },
        {
            ...
            input_schema: { type: 'object', properties: {}, required: [] },
            handler: async (tenantId) => {
                const users = await (0, module_sdk_1.safeRows)(`SELECT user_id, email, role, mfa_enabled FROM users WHERE tenant_id = $1 AND is_active = TRUE`, [tenantId]);
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                ...
                let roleAssignments = [];
                try {
                    roleAssignments = await (0, module_sdk_1.safeRows)(`SELECT ura.user_id, COUNT(*)::int AS role_count FROM "${schema}".user_role_assignments ura WHERE ura.active = TRUE GROUP BY ura.user_id`, []);
                }
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                const schema = `tenant_${tenantId}`;
                if (input.userId) {
                    const roles = await (0, resilience_1.swallowEmpty)(resilience_1.EC.FALLBACK_QUERY, (0, module_sdk_1.safeRows)(`SELECT ura.role_id, r.role_code, r.name_en, ura.assigned_at FROM "${schema}".user_role_assignments ura JOIN "${schema}".roles r ON r.role_id = ura.role_id WHERE ura.user_id = $1 AND ura.active = TRUE`, [input.userId]), { operation: 'query user_role_assignments' });
                    const user = await (0, module_sdk_1.safeRows)(`SELECT user_id, email, name, role FROM users WHERE tenant_id = $1 AND user_id = $2`, [tenantId, input.userId]);
                ...
                const allAssignments = await (0, resilience_1.swallowEmpty)(resilience_1.EC.FALLBACK_QUERY, (0, module_sdk_1.safeRows)(`SELECT ura.user_id, r.role_code, r.name_en FROM "${schema}".user_role_assignments ura JOIN "${schema}".roles r ON r.role_id = ura.role_id WHERE ura.active = TRUE ORDER BY ura.user_id`, []), { operation: 'query user_role_assignments' });
