    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const noEvidence = await safeRows(`SELECT c.control_id, c.code, c.title FROM "${schema}".ucf_controls c
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = tenantSchema(tenantId);
                const stats = await safeRows(`SELECT
             COUNT(*)::int AS total,
