    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const gaps = await safeRows(`SELECT gap_id, framework_id, control_id, severity, status, description, created_at
           FROM "${schema}".compliance_gaps WHERE status = 'open'
           ORDER BY CASE severity WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END
           LIMIT 50`);
                const overdue = await safeRows(`SELECT COUNT(*)::int AS n FROM "${schema}".remediation_tasks WHERE status = 'open' AND due_date < NOW()`);
                const compScore = await safeRows(`SELECT AVG(score)::int AS avg_score FROM "${schema}".compliance_assessments WHERE created_at > NOW() - INTERVAL '30 days'`);
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                ...
                const gaps = await safeRows(`SELECT g.gap_id, g.framework_id, g.control_id, g.severity, g.description,
