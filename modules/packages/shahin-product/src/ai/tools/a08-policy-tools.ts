...
export function buildA08Tools(): AgentToolDefinition[] {
  return [
    {
      ...
      handler: async (tenantId) => {
        ...
        const policies = await safeRows(
          `SELECT policy_id, title, status, review_date, version, owner, created_at, updated_at
           FROM "${schema}".policies ORDER BY
             CASE WHEN review_date < NOW() THEN 0 ELSE 1 END,
             review_date NULLS LAST LIMIT 50`
        );
        const overdue = policies.filter((p: GenericRow) => p.review_date && new Date(p.review_date) < new Date() && p.status === 'published');
        const pendingApproval = policies.filter((p: GenericRow) => p.status === 'pending_approval');
        const expiringSoon = policies.filter((p: GenericRow) => p.review_date && new Date(p.review_date) > new Date() && new Date(p.review_date) < new Date(Date.now() + 30 * 86400000));
    ...
    },
    {
      ...
      handler: async (tenantId) => {
        ...
        const existingTypes = policies.map((p: GenericRow) => p.title?.toLowerCase() || '');
        const missing = requiredPolicyTypes.filter(rp => !existingTypes.some((e: string) => e.includes(rp.toLowerCase())));
