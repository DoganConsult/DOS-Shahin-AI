    ...
    return [
        {
            ...
            handler: async (tenantId) => {
                ...
                const risks = await safeRows(`SELECT risk_id, title, category, risk_score, likelihood, impact, status, owner, treatment_strategy, created_at, updated_at
        ...
        },
        {
            ...
            handler: async (tenantId, input) => {
                ...
                const rows = await safeRows(`UPDATE "${schema}".risks SET risk_score = $1, likelihood = $2, impact = $3, ai_assessment = $4, updated_at = NOW() WHERE risk_id = $5 RETURNING risk_id`, [score, input.likelihood, input.impact, input.rationale, input.riskId]);
                if (rows.length === 0)
        ...
        },
        {
            ...
            handler: async (tenantId) => {
                const schema = tenantSchema(tenantId);
                const breaches = await safeRows(`SELECT r.risk_id, r.title, r.category, r.risk_score, a.max_acceptable_score
           FROM "${schema}".risks r
