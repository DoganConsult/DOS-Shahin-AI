...
import { enqueueHandoff, getHandoffBatch } from '@dos/platform-core/agents/handoff/handoff-queue.service';
...
export function createHandoff(
  ...
  import('../../../openclaw/a2a/agent-message-queue').then(({ sendAgentMessage }) =>
 ...
 */
export function _evaluateCooperationRules(tenantId: string, discovery: AgentDiscovery, ctx: SharedAgentContext): void {
  ...
  for (const rule of COOPERATION_RULES) {
    ...
    import('../../../modules/platform/services/agrc/agrc-os-integration.service').then(({ persistHandoff }) =>
