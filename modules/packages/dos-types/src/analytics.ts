...
export interface KpiTrend {
  ...
  tenantId: string;
  period: string;
  datapoints: Array<{ timestamp: string; value: number; status: KpiStatus }>;
...
export interface AnalyticsUsageSummary {
  ...
  topModules: Array<{ moduleCode: string; usageCount: number }>;
...
export const ANALYTICS_STATUSES: readonly AnalyticsStatus[] = ['draft', 'active', 'published', 'deprecated', 'archived'] as const;

export type AnalyticsSource = 'manual' | 'import' | 'api' | 'workflow' | 'ai_agent' | 'system';

export const ANALYTICS_SOURCES: readonly AnalyticsSource[] = ['manual', 'import', 'api', 'workflow', 'ai_agent', 'system'] as const;
