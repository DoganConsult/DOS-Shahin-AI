...
async function getAiRecommendations(tenantId, _context = {}) {
    ...
    try {
        const [untreated, overdue, expired, findings] = await Promise.all([
            (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS n FROM "${schema}".risks
         WHERE (treatment_status IS NULL OR treatment_status='none')
           AND status='open' AND risk_score >= 8`),
            (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS n FROM "${schema}".controls
         WHERE next_test_date < NOW() AND status='implemented'`),
            (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS n FROM "${schema}".evidence
         WHERE expiry_date < NOW()`),
            (0, db_1.safeQuery)(`SELECT COUNT(*)::int AS n FROM "${schema}".findings
...
//# sourceMappingURL=grc-query-ai.service.js.map