...
export async function publish(
  ...
  if (!GOVERNANCE_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${GOVERNANCE_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: GOVERNANCE_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${GOVERNANCE_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(GOVERNANCE_EVENT_CONTRACT.published);
}
