import type { GovernanceCharter, GovernanceCharterCreateInput } from '@dos/types/governance';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): GovernanceCharter {
  return {
    charter_id: row.charter_id ?? row.id,
    tenant_id: row.tenant_id,
    ...row,
    created_at: row.created_at?.toISOString?.() ?? row.created_at,
    updated_at: row.updated_at?.toISOString?.() ?? row.updated_at,
    created_by: row.created_by ?? 'system',
  } as GovernanceCharter;
}

export function toEntityList(rows: GenericRow[]): GovernanceCharter[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): GovernanceCharterCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as GovernanceCharterCreateInput;
}

export function toApiResponse(entity: GovernanceCharter): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: GovernanceCharter[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
