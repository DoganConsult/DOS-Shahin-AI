    ...
    if (scoreHistory.length > 0) {
      ...
      if ((latestScore as any).totalScore < ENGAGEMENT_SCORE_THRESHOLD) {
        const result: GateValidationResult = {
          allowed: false,
          gateType: 'vendor',
          reason: `Vendor engagement score (${latestScore.totalScore}) is below threshold (${ENGAGEMENT_SCORE_THRESHOLD}) — onboarding and new contracts blocked`,
        ...
        await logGateDecision(schema, {
          ...
          details: {
            riskScore,
            engagementScore: latestScore.totalScore,
            engagementThreshold: ENGAGEMENT_SCORE_THRESHOLD,
            scoreBreakdown: {
              responseTimeScore: latestScore.responseTimeScore,
              completionRateScore: latestScore.completionRateScore,
              evidenceTimelinessScore: latestScore.evidenceTimelinessScore,
              remediationRateScore: latestScore.remediationRateScore,
            },
          },
        });
  ...
  if (gate.ai_analysis_enabled && gate.ai_prompt_template) {
    try {
      ...
      aiAnalysis = await claudeJSON({
        systemPrompt: `You are an AI-first GRC enforcement gate analyst. Provide structured JSON with:
    ...
    case 'ai_assessment': {
      ...
      try {
        ...
        const assessment = await claudeJSON({
          systemPrompt: config.system_prompt || 'Assess if this action should be allowed. Respond with JSON: {passed: boolean, reason: string, confidence: number}',
          userMessage: `Context: ${JSON.stringify(params)}`,
          maxTokens: 512,
          temperature: 0.1,
        });
        return { passed: assessment.passed !== false, message: assessment.reason };
      } catch {
