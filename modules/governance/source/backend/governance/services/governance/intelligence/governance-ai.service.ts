export const GOVERNANCE_AI_CONFIG = {
  moduleCode: 'governance' as const,
  enabled: true,

  allowedActions: [
    'governance.record.draft',
    'governance.record.recommend',
    'governance.record.summarize',
    'governance.record.classify',
    'governance.record.score',
    'governance.record.analyze',
    'governance.record.generate_report',
  ],

  blockedActions: [
    'governance.record.delete',
    'governance.record.approve',
    'governance.record.reject',
    'governance.record.override',
    'governance.record.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['governance.record.approve', 'governance.record.reject', 'governance.record.delete'],
    requiresHumanReview: ['governance.record.classify', 'governance.record.score'],
    autoExecutable: ['governance.record.draft', 'governance.record.summarize', 'governance.record.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'governance_system_prompt_v1',
  },
...
export function isGovernanceAiActionAllowed(action: string): boolean {
  return (GOVERNANCE_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isGovernanceAiActionBlocked(action: string): boolean {
  return (GOVERNANCE_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresGovernanceHumanApproval(action: string): boolean {
  return (GOVERNANCE_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".governance_items WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Governance entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Governance"} item. Status: \${r.status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".governance_items ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".governance_items ${where} GROUP BY status`, params);
