...
@Component({
  ...
  imports: [
    ...
    InputTextModule, InputTextarea, DropdownModule, CheckboxModule, TooltipModule,
    ...
    <app-page-shell
      ...
      <p-toolbar styleClass="mb-3">
        <ng-template pTemplate="start">
          ...
          <p-button *ngIf="showUnassignedBanner()"
            ...
            icon="pi pi-filter" styleClass="ms-2" [outlined]="true"
        ...
        <ng-template pTemplate="end">
          <div *ngIf="selected.length > 0" class="flex align-items-center gap-2 me-3">
            ...
            <p-dropdown [options]="teamOptions()" optionLabel="label" optionValue="value"
              [(ngModel)]="bulkTeamId"
              [placeholder]="i18n.translate('controls.assignToTeam')"
              styleClass="bulk-team-dd" [filter]="true" />
            <p-button [label]="i18n.translate('controls.assign')"
      ...
      <p-table aria-label="Data table" [value]="displayedControls()" [paginator]="true" [rows]="20"
        ...
        <ng-template pTemplate="header">
          <tr>
            <th style="width:3rem"><p-table [paginator]="true" [rows]="20" [rowsPerPageOptions]="[10,20,50]" aria-label="Data table"HeaderCheckbox /></th>
            <th pSortableColumn="title">{{ i18n.translate('nav.controls') }}</th>
            <th>{{ i18n.translate('common.status') }}</th>
            <th>{{ i18n.translate('controls.ownerTeam') }}</th>
            <th>{{ i18n.translate('nav.frameworks') }}</th>
            <th pSortableColumn="evidenceCount">{{ i18n.translate('controls.evidence') }}</th>
            <th pSortableColumn="riskCount">{{ i18n.translate('controls.risks') }}</th>
            <th style="width:100px">{{ i18n.translate('common.actions') }}</th>
          </tr>
        </ng-template>
        <ng-template pTemplate="body" let-c>
          <tr>
            <td><p-table [paginator]="true" [rows]="20" [rowsPerPageOptions]="[10,20,50]" aria-label="C table"Checkbox [value]="c" /></td>
            <td>
              <strong>{{ c.title }}</strong>
              <p-tag *ngIf="c.automatable" [value]="i18n.translate('controls.auto')" severity="success" icon="pi pi-bolt" styleClass="ms-2" />
            </td>
            <td><app-status-badge [status]="c.status" /></td>
            <td>
              <div class="owner-cell">
                <span *ngIf="c.ownerTeamName" class="team-badge">
                  <i class="pi pi-users"></i> {{ c.ownerTeamName }}
                </span>
                ...
                <p-dropdown *ngIf="quickAssignId === c.control_id || quickAssignId === c.controlId"
            ...
            <td><p-tag *ngFor="let fw of c.frameworks?.slice(0,2)" [value]="fw" severity="info" styleClass="me-1" /></td>
            <td>
              <p-tag [value]="(c.evidenceCount || 0) + ''" [severity]="c.evidenceCount > 0 ? 'success' : 'warning'" icon="pi pi-folder-open" />
            </td>
            <td>
              <p-tag [value]="(c.riskCount || 0) + ''" [severity]="c.riskCount > 0 ? 'danger' : 'secondary'" icon="pi pi-exclamation-triangle" />
            </td>
            <td>
              <button aria-label="User" class="icon-btn" (click)="toggleQuickAssign(c)"
                [pTooltip]="i18n.translate('controls.assignTeam')">
                <i class="pi pi-users"></i>
              </button>
              <button aria-label="Edit" class="icon-btn" (click)="openEdit(c)" pTooltip="Edit"><i class="pi pi-pencil"></i></button>
              <button aria-label="Delete" class="icon-btn danger" (click)="confirmDel(c)" pTooltip="Delete"><i class="pi pi-trash"></i></button>
            </td>
          </tr>
        </ng-template>
      ...
      <!-- Create/Edit Dialog -->
      <p-dialog [header]="editMode ? 'Edit Control' : 'New Control'" [(visible)]="showDialog" [modal]="true" [style]="{width:'560px'}">
        <div class="dialog-form">
          ...
          <div class="field">
            ...
            <p-dropdown [(ngModel)]="form.owner_team_id" [options]="teamOptions()" optionLabel="label" optionValue="value"
          ...
          <div class="field"><p-checkbox [(ngModel)]="form.automatable" [binary]="true" inputId="automatable" /><label for="automatable" class="ml-2">Automatable</label></div>
        </div>
        <ng-template pTemplate="footer">
          <p-button [label]="i18n.translate('common.cancel')" severity="secondary" [text]="true" (onClick)="showDialog=false" />
          <p-button [label]="i18n.translate('common.save')" icon="pi pi-check" (onClick)="save()" [disabled]="!form.title" />
        </ng-template>
      </p-dialog>
...
export class ControlsComponent implements OnInit {
  ...
  delTarget: Record<string, unknown> | null = null;
  ...
  form: Record<string, unknown> = { title: '', description: '', status: 'not_started', owner: '', owner_team_id: null, frameworksStr: '', automatable: false };
  ...
  teamOptions = computed(() =>
    this.teams().map((t: Record<string, unknown>) => ({ label: t.name || t.team_name, value: t.team_id || t.teamId }))
  );
  ...
  teamDistribution: Record<string, unknown>[] = [];
  ...
  load(): void {
    ...
    this.apiclientSvc.get(`/controls?offset=${this.currentOffset}&limit=20`).subscribe({
      next: (res: Record<string, unknown>) => {
        ...
        this.allControls.set(list.map((c: Record<string, unknown>) => ({
          ...c,
      ...
      error: () => {
        this.complianceSvc.getControls().subscribe(c => { this.allControls.set(c as unknown[]); this.loaded = true; });
        this._loadInFlight = false;
        this._initialLoadDone = true;
      }
    });
  ...
  loadTeamDistribution(): void {
    this.apiclientSvc.get('/controls/team-distribution').subscribe({
      next: (r: Record<string, unknown>) => { this.teamDistribution = r.distribution || []; },
      error: (e: unknown) => devError("[API]", e),
    });
  }
  ...
  openEdit(c: Record<string, unknown>): void {
  ...
  save(): void {
    ...
    const data: Record<string, unknown> = { ...this.form, frameworks: this.form.frameworksStr.split(',').map((s: string) => s.trim()).filter(Boolean) };
  ...
  confirmDel(c: Record<string, unknown>): void { this.delTarget = c; this.showDelDialog = true; }
  ...
  toggleQuickAssign(c: Record<string, unknown>): void {
  ...
  applyQuickAssign(c: Record<string, unknown>): void {
    const id = c.control_id || c.controlId;
