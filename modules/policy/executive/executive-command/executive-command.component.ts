...
import { AppNumberPipe } from '../../../shared/pipes';
...
export class ExecutiveCommandComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    ...
    // Days since incident
    this.http.get<unknown>(`${this.api}/incidents`, { params: { limit: '1' } }).pipe(catchError(() => of(null)), takeUntilDestroyed(this.destroyRef)).subscribe(d => {
    ...
    // Top risks
    this.http.get<unknown>(`${this.api}/risks`, { params: { limit: '5', sort: 'score', order: 'desc' } }).pipe(catchError(() => of({ risks: [] })), takeUntilDestroyed(this.destroyRef)).subscribe(d => {
      this.topRisks.set(d?.risks || []);
      this.buildRiskBubbles();
    });

    // Evidence freshness
    this.http.get<unknown>(`${this.api}/evidence`).pipe(catchError(() => of({ evidence: [] })), takeUntilDestroyed(this.destroyRef)).subscribe(d => {
  ...
  buildPostureGauge(): void {
    ...
    this.postureGaugeOpts.set({
      series: [{
        ...
        axisLine: { lineStyle: { width: 18, color: [[1, 'rgba(0,0,0,0.05)']] } },
        axisTick: { show: false },
  ...
  buildTrendSparklines(): void {
    ...
    this.trendSparklineOpts.set({
      ...
      series: [
        { name: 'Compliance', type: 'line', data: t.map(d => d.complianceScore || 0), smooth: true, lineStyle: { width: 2 }, symbol: 'none', itemStyle: { color: '#22c55e' }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(34,197,94,0.15)' }, { offset: 1, color: 'rgba(34,197,94,0)' }] } } },
  ...
  buildRiskBubbles(): void {
    ...
    this.riskBubbleOpts.set({
      ...
      series: [{
        type: 'scatter',
        symbolSize: (d: Record<string, unknown>) => Math.max(16, (d[2] || 1) * 4),
        data: risks.map((r: Record<string, unknown>) => [r.impact || r.impact_score || 3, r.likelihood || r.likelihood_score || 3, r.score || r.risk_score || 10, r.title || r.name || 'Risk']),
  ...
  buildEvidenceHeatmap(evidence: Record<string, unknown>[]): void {
    ...
    this.evidenceHeatmapOpts.set({
      ...
      series: [{ type: 'heatmap', coordinateSystem: 'calendar', data } as unknown]
    });
  }
  ...
  getMaturityLabel(): string {
    ...
    return `Level ${m.level} — ${labels[m.level] || 'Unknown'}`;
  }
}
