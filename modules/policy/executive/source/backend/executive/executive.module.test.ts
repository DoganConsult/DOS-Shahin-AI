...
describe('executive module manifest', () => {
  it('exports a valid manifest', async () => {
    const mod = await import('./executive.module');
    const manifest = mod.EXECUTIVE_MANIFEST ?? mod.default;
    expect(manifest).toBeTruthy();
    expect(manifest.code).toBe('executive');
