import type { RemediationTask, RemediationTaskCreateInput } from '../types/remediation.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): RemediationTask {
  return {
...
export function toEntityList(rows: GenericRow[]): RemediationTask[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): RemediationTaskCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as RemediationTaskCreateInput;
}

export function toApiResponse(entity: RemediationTask): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: RemediationTask[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
