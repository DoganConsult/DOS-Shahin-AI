...
beforeAll(async () => {
  const mod = await import('./training.module');
  manifest = mod.TRAINING_MANIFEST ?? mod.default?.manifest;
});

describe('Training Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('training'); });
