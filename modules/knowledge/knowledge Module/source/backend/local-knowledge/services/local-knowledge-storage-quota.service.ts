...
export async function getStorageQuota(tenantId: string): Promise<StorageQuota> {
  ...
  try {
    const res = await LocalKnowledgeAutoRepo.query166(tenantSchema(tenantId), [tenantId]);
    ...
    if (res.rows.length > 0) {
      ...
      const insertRes = await LocalKnowledgeAutoRepo.query165(tenantSchema(tenantId), [tenantId, defaultQuota]);
...
export async function calculateStorageUsage(tenantId: string): Promise<number> {
  ...
  try {
    ...
    const res = await LocalKnowledgeAutoRepo.query164(tenantSchema(tenantId), [tenantId]);
...
export async function updateStorageUsage(tenantId: string): Promise<QuotaCheckResult> {
  ...
  try {
    ...
    // Update quota record
    await LocalKnowledgeAutoRepo.query163(tenantSchema(tenantId), [usedBytes, alertThreshold80, alertThreshold90, alertThreshold100, tenantId]);
...
export async function setStorageQuota(
  ...
  try {
    await LocalKnowledgeAutoRepo.query162(tenantSchema(tenantId), [tenantId, quotaBytes]);
