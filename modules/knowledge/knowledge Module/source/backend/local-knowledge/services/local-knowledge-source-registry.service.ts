...
export async function listSources(
  ...
  // Total count
  const countResult = await LocalKnowledgeAutoRepo.query161(tenantSchema(tenantId), params);
  const total = countResult.rows[0]?.total ?? 0;

  // Data with document count via LEFT JOIN on ingestion_log -> documents
  const dataResult = await LocalKnowledgeAutoRepo.query160(tenantSchema(tenantId), [...params, limit, offset]);
...
export async function updateSource(
  ...
  try {
    const result = await LocalKnowledgeAutoRepo.query158(tenantSchema(tenantId), params);
...
export async function deleteSource(tenantId: string, sourceId: string): Promise<void> {
  ...
  try {
    await LocalKnowledgeAutoRepo.query157(tenantSchema(tenantId), [sourceId, tenantId]);
...
export async function getSyncHistory(
  ...
  const result = await LocalKnowledgeAutoRepo.query147(tenantSchema(tenantId), [tenantId, sourceId, Math.min(limit, 200)]);
...
export async function getSource(tenantId: string, sourceId: string): Promise<KnowledgeSource | null> {
  const _schema = tenantSchema(tenantId);
  const { rows } = await LocalKnowledgeAutoRepo.query146(tenantSchema(tenantId), [sourceId, tenantId]);
...
export async function testSourceConnection(
  ...
  try {
    const _schema = tenantSchema(tenantId);
    await LocalKnowledgeAutoRepo.query145(tenantSchema(tenantId), [sourceId]);
