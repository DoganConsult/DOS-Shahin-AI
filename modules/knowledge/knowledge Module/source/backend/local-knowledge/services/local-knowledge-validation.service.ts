...
export async function validateDocument(
  ...
  try {
    const { rows } = await LocalKnowledgeAutoRepo.query172(tenantSchema(tenantId), [documentId, tenantId]);
  ...
  if (doc.checksum) {
    try {
      const { rows: dupes } = await LocalKnowledgeAutoRepo.query171(tenantSchema(tenantId), [tenantId, doc.checksum, documentId]);
      if (dupes[0]?.cnt > 0) {
...
export async function validateChunks(
  ...
  try {
    const { rows } = await LocalKnowledgeAutoRepo.query170(tenantSchema(tenantId), [tenantId, documentId, MAX_CHUNK_SIZE, MIN_CHUNK_SIZE]);
...
export async function validateSearchIndex(
  ...
  try {
    const { rows } = await LocalKnowledgeAutoRepo.query169(tenantSchema(tenantId), [tenantId]);
    ...
    try {
      const { rows: noChunkRows } = await LocalKnowledgeAutoRepo.query168(tenantSchema(tenantId), [tenantId]);
    ...
    try {
      const { rows: danglingRows } = await LocalKnowledgeAutoRepo.query167(tenantSchema(tenantId), [tenantId]);
