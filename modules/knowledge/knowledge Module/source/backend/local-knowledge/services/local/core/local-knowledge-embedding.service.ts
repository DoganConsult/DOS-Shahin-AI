...
async function generateEmbeddingsBatch(
  ...
  for (let i = 0; i < texts.length; i += batchSize) {
    ...
    try {
      ...
      const { gatewayEmbeddings } = await import('../../../../ai/services/gateway/ai-gateway.service.js');
...
export async function storeEmbeddings(
  ...
  try {
    ...
    await safeQuery(
      ...
      [embeddingLiteral, chunkId, tenantId],
    );
    ...
    await safeQuery(
      ...
      [tenantId, chunkId, embeddingLiteral],
  ...
  try {
    ...
    if (hasNonZero) {
      ...
      const res = await safeQuery(
        `SELECT c.chunk_id, c.document_id, c.chunk_text, c.chunk_index, c.metadata,
