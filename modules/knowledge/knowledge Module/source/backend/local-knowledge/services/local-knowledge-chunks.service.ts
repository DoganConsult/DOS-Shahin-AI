...
import { emptyResult, safeQuery as _safeQuery, tenantSchema } from '../../action/ports/database.port';
import { logger } from '../../action/ports/logger.port';

import { catchHandler, swallowDefault, EC } from '@dos/platform-core';
...
export async function getChunks(
  ...
  try {
    const res = await LocalKnowledgeAutoRepo.query43(tenantSchema(tenantId), [documentId, tenantId]);
...
export async function searchChunks(
  ...
  try {
    ...
    const res = await LocalKnowledgeAutoRepo.query42(tenantSchema(tenantId), params);
...
export async function createChunks(
  ...
  if (!resolvedIngestionId) {
    try {
      const docRes = await LocalKnowledgeAutoRepo.query41(tenantSchema(tenantId), [documentId, tenantId]);
  ...
  for (const chunk of chunks) {
    try {
      ...
      const res = await LocalKnowledgeAutoRepo.query40(tenantSchema(tenantId), [
                tenantId,
                documentId,
                resolvedIngestionId,
                chunk.chunkIndex,
...
export async function deleteChunks(
  ...
  try {
    ...
    const chunkRes = await LocalKnowledgeAutoRepo.query39(tenantSchema(tenantId), [documentId, tenantId]);
    ...
    if (deletedCount > 0) {
      ...
      await LocalKnowledgeAutoRepo.query38(tenantSchema(tenantId), [tenantId, chunkIds]).catch(catchHandler(EC.FALLBACK_QUERY, {
...
export async function getChunkContext(
  ...
  try {
    ...
    const chunkRes = await LocalKnowledgeAutoRepo.query37(tenantSchema(tenantId), [chunkId, tenantId]);
    ...
    const windowRes = await LocalKnowledgeAutoRepo.query36(tenantSchema(tenantId), [documentId, tenantId, minIndex, maxIndex]);
...
async function fallbackILikeSearch(
  ...
  const res = await swallowDefault(EC.FALLBACK_QUERY, emptyResult(), LocalKnowledgeAutoRepo.query35(tenantSchema(tenantId), params), { tenantId: tenantId, operation: 'query local_knowledge_chunks' }); // similarity() requires pg_trgm; degrade gracefully
...
async function attachSurroundingContext(
  ...
  for (const [documentId, indices] of contextNeeded) {
    ...
    try {
      const res = await LocalKnowledgeAutoRepo.query34(tenantSchema(tenantId), [documentId, tenantId, indexArray]);
