...
beforeAll(async () => {
  const mod = await import('./incident.module');
  manifest = mod.INCIDENT_MANIFEST ?? mod.default?.manifest;
});

describe('Incident Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('incident'); });
