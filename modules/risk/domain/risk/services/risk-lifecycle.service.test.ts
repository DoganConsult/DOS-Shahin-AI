...
vi.mock('../ports/database.port', () => ({
  ...
  describe('transitionStatus', () => {
    it('should transition from draft to in_review', async () => {
      ...
      const result = await transitionStatus('t1', 'e1', 'submitted', 'u1');
    ...
    it('should throw 404 for missing entity', async () => {
      ...
      await expect(transitionStatus('t1', 'missing', 'submitted', 'u1')).rejects.toThrow();
    });
  ...
  describe('bulkTransitionStatus', () => {
    it('should process multiple entities', async () => {
      (safeQuery as any)
        .mockResolvedValueOnce({ rows: [{ status: 'draft' }] })
        .mockResolvedValue({ rows: [] });
      const result = await bulkTransitionStatus('t1', ['e1'], 'submitted', 'u1');
  ...
  describe('getStatusHistory', () => {
    it('should return history array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ fromStatus: 'draft', toStatus: 'submitted', changedBy: 'u1', changedAt: '2024-01-01' }] });
  ...
  describe('domain queries', () => {
    it('getHighRisks should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getHighRisks('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getHighRisks should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getHighRisks('t1');
      expect(result).toEqual([]);
    });

    it('getOverdueAssessments should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverdueAssessments('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverdueAssessments should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverdueAssessments('t1');
      expect(result).toEqual([]);
    });
