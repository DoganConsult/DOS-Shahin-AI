...
export class RiskMetricsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiclientSvc.get('/risk-metrics/kpis').subscribe({
  ...
  loadTrends() {
    ...
    this.apiclientSvc.get(`/risk-metrics/trends?startDate=${this.startDate.toISOString()}&endDate=${this.endDate.toISOString()}`).subscribe({
      next: (d: any) => { this.trends = d || []; }
    });
  }
}
