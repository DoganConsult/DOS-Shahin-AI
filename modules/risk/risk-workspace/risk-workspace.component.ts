...
import type {
  ...
  AppetiteTrendDto,
} from '@risk-module/ui/features/risk';
...
@Component({
  ...
  imports: [
    ...
    GrcFormFieldComponent, EntityDetailDrawerComponent, AiPanelComponent,
...
export class RiskWorkspaceComponent implements OnInit {
  ...
  catSeverity(category: string): 'danger' | 'warning' | 'success' | 'info' {
    const map: Record<string, unknown> = { cyber: 'danger', compliance: 'info', operational: 'warning' };
  ...
  kriTagSeverity(status: string): 'danger' | 'warning' | 'success' | 'info' {
    const map: Record<string, unknown> = { breach: 'danger', warning: 'warning', normal: 'success' };
