...
router.get('/widgets/executive/summary', authenticate, async (req: any, res: Response) => {
  try {
...
router.get('/widgets/executive/top-breached-kris', authenticate, async (req: any, res: Response) => {
  try {
    const tenantId = resolveTenantId(req);
...
router.get('/widgets/executive/policy-review-debt', authenticate, async (req: any, res: Response) => {
  try {
    const tenantId = resolveTenantId(req);
    const limit = Number(req.query?.limit ?? 10);
    if (!tenantId) {
...
router.get('/widgets/executive/engine-trend', authenticate, async (req: any, res: Response) => {
  try {
    const tenantId = resolveTenantId(req);
