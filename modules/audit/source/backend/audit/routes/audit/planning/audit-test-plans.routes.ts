...
import {
  ...
  updateTestResult,
  getControlCoverage,
} from '../../../services/audit/execution/audit-test-plans.service.js';
...
import { createTestPlanBody, updateTestResultBody, idParam } from "../../../schemas/audit.schemas.js";
...
router.post("/", authenticate, requirePermission("audit.record.manage"), validate({ body: createTestPlanBody }), async (req: Request, res: Response) => {
  ...
  setAuditData(req, { action: "create", entityType: "test_plan", entityId: result.id, afterState: result });
...
router.put("/:id/result", authenticate, requirePermission("audit.record.manage"), validate({ params: idParam, body: updateTestResultBody }), async (req: Request, res: Response) => {
  ...
  setAuditData(req, { action: "update", entityType: "test_plan", entityId: id, afterState: result });
