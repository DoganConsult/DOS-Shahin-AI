import type { AuditEngagement, AuditEngagementCreateInput } from '../types/audit.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): AuditEngagement {
  return {
...
export function toEntityList(rows: GenericRow[]): AuditEngagement[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): AuditEngagementCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as AuditEngagementCreateInput;
}

export function toApiResponse(entity: AuditEngagement): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: AuditEngagement[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
