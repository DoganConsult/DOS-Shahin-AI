...
beforeAll(async () => {
  const mod = await import('./inbox.module');
  manifest = mod.INBOX_MANIFEST ?? mod.default?.manifest;
});

describe('Inbox Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('inbox'); });
