import type { InboxMessage, InboxMessageCreateInput } from '../types/inbox.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): InboxMessage {
  return {
...
export function toEntityList(rows: GenericRow[]): InboxMessage[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): InboxMessageCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as InboxMessageCreateInput;
}

export function toApiResponse(entity: InboxMessage): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: InboxMessage[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
