...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getUnreadMessages should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getUnreadMessages('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getUnreadMessages should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getUnreadMessages('t1');
      expect(result).toEqual([]);
    });

    it('getOverdueActions should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverdueActions('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverdueActions should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverdueActions('t1');
      expect(result).toEqual([]);
    });
