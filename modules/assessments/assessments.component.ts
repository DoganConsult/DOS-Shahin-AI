...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, AppDatePipe, AppNumberPipe, FormsModule, PageShellComponent, EmptyStateComponent, StatusBadgeComponent, TableModule, TagModule, ButtonModule, DialogModule, InputTextModule, DropdownModule, ToastModule, TooltipModule, ConfirmDialogModule],
  ...
  template: `
    <app-page-shell icon="clipboard-check" [title]="i18n.translate('assessments.title')"
      ...
      <div class="toolbar" *ngIf="!error">
        <p-button [label]="i18n.translate('assessments.newAssessment')" icon="pi pi-plus" (onClick)="showCreate = true" />
      </div>
      <app-empty-state
      ...
      <p-table [attr.aria-label]="i18n.translate('assessments.ariaAssessmentsTable')" [value]="assessments" [paginator]="true" [rows]="15" styleClass="p-datatable-sm p-datatable-striped" *ngIf="!error && assessments.length > 0">
        ...
        <ng-template pTemplate="body" let-a>
          <tr>
            ...
            <td class="actions">
              <p-button icon="pi pi-list" class="p-button-sm p-button-text" [pTooltip]="i18n.translate('assessments.viewItems')" (onClick)="viewItems(a)" />
              <p-button icon="pi pi-chart-bar" class="p-button-sm p-button-text p-button-info" [pTooltip]="i18n.translate('assessments.score')" (onClick)="scoreAssessment(a)" />
              <p-button icon="pi pi-trash" class="p-button-sm p-button-text p-button-danger" [pTooltip]="i18n.translate('common.delete')" (onClick)="deleteAssessment(a)" />
            </td>
          </tr>
        </ng-template>
      </p-table>
      ...
      <!-- Items Dialog -->
      <p-dialog [header]="i18n.translate('assessments.assessmentItems')" [(visible)]="showItems" [modal]="true" [style]="{width:'700px'}">
        <p-table [paginator]="true" [rows]="20" [rowsPerPageOptions]="[10,20,50]" [attr.aria-label]="i18n.translate('assessments.ariaItemsTable')" [value]="items" styleClass="p-datatable-sm">
          ...
          <ng-template pTemplate="body" let-item>
            <tr>
              ...
              <td><p-tag [value]="item.status" [severity]="itemSeverity(item.status)" /></td>
              <td>
                <p-dropdown [options]="statusOptions" [(ngModel)]="item.newStatus" placeholder="Change..." appendTo="body" [style]="{width:'160px'}" />
                <p-button icon="pi pi-check" class="p-button-sm p-button-text" (onClick)="updateItemStatus(item)" *ngIf="item.newStatus" />
              </td>
            </tr>
          </ng-template>
        </p-table>
      ...
      <!-- Create Dialog -->
      <p-dialog [header]="i18n.translate('assessments.newAssessment')" [(visible)]="showCreate" [modal]="true" [style]="{width:'450px'}">
        ...
        <ng-template pTemplate="footer">
          <p-button [label]="i18n.translate('common.cancel')" class="p-button-text" (onClick)="showCreate = false" />
          <p-button [label]="i18n.translate('assessments.create')" icon="pi pi-check" (onClick)="createAssessment()" />
        </ng-template>
      </p-dialog>
