...
import {
  ...
  getStaleInsights,
  getDraftConfigs,
} from './proactive-leadership-lifecycle.service';
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getStaleInsights should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getStaleInsights('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getStaleInsights should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getStaleInsights('t1');
      expect(result).toEqual([]);
    });

    it('getDraftConfigs should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getDraftConfigs('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getDraftConfigs should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getDraftConfigs('t1');
      expect(result).toEqual([]);
    });
