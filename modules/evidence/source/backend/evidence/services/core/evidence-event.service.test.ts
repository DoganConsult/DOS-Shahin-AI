import { describe, it, expect, vi, beforeEach } from 'vitest';

vi.mock('../../../platform/services/event/event-bus.service', () => ({
  eventBus: { publish: vi.fn() },
}));

import { emitEvidenceEvent, emitEvidenceStatusChange, emitEvidenceCollected, emitEvidenceSubmitted, emitEvidenceAccepted, emitEvidenceRejected } from './evidence-event.service';

vi.mock('../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
import { eventBus } from '../../ports/events.port';


vi.mock('../../ports/events.port', () => ({
  eventBus: { publish: vi.fn().mockReturnValue(true), subscribe: vi.fn() },
  emitEvent: vi.fn().mockReturnValue(true),
  publishEvent: vi.fn().mockReturnValue(true)
}));
describe('Evidence Event Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('emitEvidenceEvent', () => {
    it('should publish event to event bus', () => {
      emitEvidenceEvent({
        tenantId: 't1',
        entityType: 'evidence' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
    });

    it('should include correlationId in payload', () => {
      emitEvidenceEvent({
        tenantId: 't1',
        entityType: 'evidence' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
        correlationId: 'corr-123',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBe('corr-123');
    });

    it('should auto-generate correlationId if not provided', () => {
      emitEvidenceEvent({
        tenantId: 't1',
        entityType: 'evidence' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.correlationId).toBeDefined();
    });

    it('should include timestamp and eventVersion', () => {
      emitEvidenceEvent({
        tenantId: 't1',
        entityType: 'evidence' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      });
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.timestamp).toBeDefined();
      expect(call.payload.eventVersion).toBe(1);
    });

    it('should not throw on publish failure', () => {
      (eventBus.publish as any).mockImplementationOnce(() => { throw new Error('fail'); });
      expect(() => emitEvidenceEvent({
        tenantId: 't1',
        entityType: 'evidence' as any,
        entityId: 'e1',
        action: 'created' as any,
        triggeredBy: 'u1',
      })).not.toThrow();
    });
  });

  describe('emitEvidenceStatusChange', () => {
    it('should emit status_changed event', () => {
      emitEvidenceStatusChange('t1', 'evidence' as any, 'e1', 'draft', 'active', 'u1');
      expect(eventBus.publish).toHaveBeenCalledTimes(1);
      const call = (eventBus.publish as any).mock.calls[0][0];
      expect(call.payload.action).toBe('status_changed');
    });
  });

  describe('helper functions', () => {
    it('emitEvidenceCollected should not throw', () => {
      expect(() => emitEvidenceCollected('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitEvidenceSubmitted should not throw', () => {
      expect(() => emitEvidenceSubmitted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitEvidenceAccepted should not throw', () => {
      expect(() => emitEvidenceAccepted('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

    it('emitEvidenceRejected should not throw', () => {
      expect(() => emitEvidenceRejected('t1', 'id1', 'test' as any, 'u1')).not.toThrow();
    });

  });
});
