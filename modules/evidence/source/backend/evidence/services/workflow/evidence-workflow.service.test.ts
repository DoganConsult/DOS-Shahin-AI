...
vi.mock('../core/evidence-event.service', () => ({
  emitEvidenceEvent: vi.fn(),
}));
...
import {
  ...
  onClosure,
  onFailure,
} from './evidence-workflow.service';
...
describe('Evidence Workflow Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('validateTransition', () => {
    it('should accept valid transitions', () => {
      expect(validateTransition('requested', 'collecting')).toBe(true);
    });

    it('should reject invalid transitions', () => {
      expect(validateTransition('requested', 'archived')).toBe(false);
    });

    it('should reject from unknown states', () => {
      expect(validateTransition('nonexistent', 'collecting')).toBe(false);
    });
  });

  describe('getAvailableTransitions', () => {
    it('should return transitions for a state', () => {
      const r = getAvailableTransitions('requested');
      expect(Array.isArray(r)).toBe(true);
      expect(r).toContain('collecting');
    });
  ...
  describe('executeTransition', () => {
    it('should execute valid transitions', async () => {
      const r = await executeTransition('t1', 'e1', 'requested', 'collecting', 'u1');
      expect(r.success).toBe(true);
    });

    it('should reject invalid transitions', async () => {
      const r = await executeTransition('t1', 'e1', 'requested', 'archived', 'u1');
  ...
  describe('handleApprovalOutcome', () => {
    it('should handle approved outcome', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ status: 'requested' }] });
