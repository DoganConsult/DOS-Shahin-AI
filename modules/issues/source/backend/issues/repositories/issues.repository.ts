...
export class IssueRepository {
  ...
  async findById(id: string): Promise<GenericRow | null> {
    const result = await safeQuery(
      `SELECT * FROM "${this.schema}".issues WHERE issue_id = $1 AND deleted_at IS NULL`, [id]);
    return getFirstRow(result);
  }

  async findAll(filters: { status?: string; category?: string; severity?: string; assigned_to?: string; source_module?: string; search?: string; page?: number; pageSize?: number; sortBy?: string; sortDir?: string } = {}): Promise<{ rows: GenericRow[]; total: number }> {
    ...
    const countResult = await safeQuery(`SELECT COUNT(*)::int AS total FROM "${this.schema}".issues ${where}`, params);
    const total = getFirstRow(countResult)?.total ?? 0;
    const dataResult = await safeQuery(
      `SELECT * FROM "${this.schema}".issues ${where} ORDER BY ${sortCol} ${sortDir} LIMIT $${idx++} OFFSET $${idx++}`,
  ...
  async create(data: Record<string, unknown>): Promise<GenericRow | null> {
    ...
    for (const [k, v] of Object.entries(data)) { if (k !== 'issue_id') { cols.push(k); vals.push(v); } }
    const placeholders = vals.map((_, i) => `$${i + 1}`).join(',');
    const result = await safeQuery(
      `INSERT INTO "${this.schema}".issues (${cols.join(',')}) VALUES (${placeholders}) RETURNING *`, vals);
    return getFirstRow(result);
  }
  ...
  async update(id: string, data: Record<string, unknown>): Promise<GenericRow | null> {
    ...
    params.push(id);
    const result = await safeQuery(
      `UPDATE "${this.schema}".issues SET ${sets.join(', ')} WHERE issue_id = $${idx} AND deleted_at IS NULL RETURNING *`, params);
    return getFirstRow(result);
  }

  async softDelete(id: string, deletedBy?: string): Promise<boolean> {
    const result = await safeQuery(
      `UPDATE "${this.schema}".issues SET deleted_at = NOW(), deleted_by = $2, updated_at = NOW() WHERE issue_id = $1 AND deleted_at IS NULL RETURNING issue_id`,
  ...
  async count(filters: { status?: string } = {}): Promise<number> {
    ...
    if (filters.status) { conditions.push(`status = $${idx++}`); params.push(filters.status); }
    const result = await safeQuery(
      `SELECT COUNT(*)::int AS total FROM "${this.schema}".issues WHERE ${conditions.join(' AND ')}`, params);
