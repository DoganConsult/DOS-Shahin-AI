...
vi.mock('./issues-event.service', () => ({
  emitIssuesEvent: vi.fn(),
}));
...
vi.mock('../ports/database.port', () => ({
  ...
  describe('validateTransition', () => {
    it('should accept valid transitions', () => {
      expect(validateTransition('open', 'triaged')).toBe(true);
    });

    it('should reject invalid transitions', () => {
      expect(validateTransition('open', 'archived')).toBe(false);
    });

    it('should reject from unknown states', () => {
      expect(validateTransition('nonexistent', 'triaged')).toBe(false);
    });
  });

  describe('getAvailableTransitions', () => {
    it('should return transitions for a state', () => {
      const r = getAvailableTransitions('open');
      expect(Array.isArray(r)).toBe(true);
      expect(r).toContain('triaged');
    });
  ...
  describe('executeTransition', () => {
    it('should execute valid transitions', async () => {
      const r = await executeTransition('t1', 'e1', 'open', 'triaged', 'u1');
      expect(r.success).toBe(true);
    });

    it('should reject invalid transitions', async () => {
      const r = await executeTransition('t1', 'e1', 'open', 'archived', 'u1');
