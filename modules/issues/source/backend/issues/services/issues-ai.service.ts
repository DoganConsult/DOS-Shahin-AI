export const ISSUES_AI_CONFIG = {
  moduleCode: 'issues' as const,
  enabled: true,

  allowedActions: [
    'issues.record.draft',
    'issues.record.recommend',
    'issues.record.summarize',
    'issues.record.classify',
    'issues.record.score',
    'issues.record.analyze',
    'issues.record.generate_report',
  ],

  blockedActions: [
    'issues.record.delete',
    'issues.record.approve',
    'issues.record.reject',
    'issues.record.override',
    'issues.record.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['issues.record.approve', 'issues.record.reject', 'issues.record.delete'],
    requiresHumanReview: ['issues.record.classify', 'issues.record.score'],
    autoExecutable: ['issues.record.draft', 'issues.record.summarize', 'issues.record.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'issues_system_prompt_v1',
  },
...
export function isIssuesAiActionAllowed(action: string): boolean {
  return (ISSUES_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isIssuesAiActionBlocked(action: string): boolean {
  return (ISSUES_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresIssuesHumanApproval(action: string): boolean {
  return (ISSUES_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".issues WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'Issues entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "Issue"} (\${r.issue_type ?? "finding"}). Status: \${r.status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".issues ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".issues ${where} GROUP BY status`, params);
