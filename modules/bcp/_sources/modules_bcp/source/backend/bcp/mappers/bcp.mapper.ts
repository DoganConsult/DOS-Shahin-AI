import type { BusinessContinuityPlan, BusinessContinuityPlanCreateInput } from '../types/bcp.types';
import type { GenericRow } from '@dos/types';

export function toEntity(row: GenericRow): BusinessContinuityPlan {
  return {
  ...
  } as BusinessContinuityPlan;
}

export function toEntityList(rows: GenericRow[]): BusinessContinuityPlan[] {
  return rows.map(toEntity);
}

export function toCreateInput(dto: Record<string, unknown>, tenantId: string, userId: string): BusinessContinuityPlanCreateInput {
  return {
    ...dto,
    tenant_id: tenantId,
    created_by: userId,
  } as BusinessContinuityPlanCreateInput;
}

export function toApiResponse(entity: BusinessContinuityPlan): Record<string, unknown> {
  const { deleted_at: _deleted_at, ...rest } = entity;
  return rest;
}

export function toApiListResponse(entities: BusinessContinuityPlan[], total: number): { data: Record<string, unknown>[]; total: number } {
  return {
