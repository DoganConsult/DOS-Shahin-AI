export const BCP_AI_CONFIG = {
  moduleCode: 'bcp' as const,
  enabled: true,
  ...
  blockedActions: [
    ...
    'bcp.plan.reject',
    'bcp.plan.override',
    'bcp.plan.bulk_delete',
  ],

  humanInLoopBoundaries: {
    requiresHumanApproval: ['bcp.plan.approve', 'bcp.plan.reject', 'bcp.plan.delete'],
    requiresHumanReview: ['bcp.plan.classify', 'bcp.plan.score'],
    autoExecutable: ['bcp.plan.draft', 'bcp.plan.summarize', 'bcp.plan.analyze'],
  },
  ...
  promptContracts: {
    ...
    temperature: 0.3,
    systemPromptTemplate: 'bcp_system_prompt_v1',
  },
...
export function isBcpAiActionAllowed(action: string): boolean {
  return (BCP_AI_CONFIG.allowedActions as readonly string[]).includes(action);
}

export function isBcpAiActionBlocked(action: string): boolean {
  return (BCP_AI_CONFIG.blockedActions as readonly string[]).includes(action);
}

export function requiresBcpHumanApproval(action: string): boolean {
  return (BCP_AI_CONFIG.humanInLoopBoundaries.requiresHumanApproval as readonly string[]).includes(action);
}
...
export async function summarize(tenantId: string, entityId: string): Promise<{ summary: string; keyPoints: string[]; confidence: number }> {
  ...
  const result = await safeQuery(
    `SELECT * FROM "${schema}".bcp_plans WHERE id = $1`,
    [entityId]
  );
  const r = result.rows[0];
  if (!r) return { summary: 'BCP entity not found.', keyPoints: [], confidence: 0 };
  return {
    summary: `\${r.title ?? "BCP Plan"}. Status: \${r.status ?? "N/A"}.`,
    keyPoints: [
      `Status: ${r.status ?? 'N/A'}`,
...
export async function generateReport(tenantId: string, filters?: { status?: string; dateFrom?: string; dateTo?: string }): Promise<{ totalCount: number; byStatus: Record<string, number>; insights: string[]; generatedAt: string }> {
  ...
  const countResult = await safeQuery(`SELECT COUNT(*) as total FROM "${schema}".bcp_plans ${where}`, params);
  const statusResult = await safeQuery(`SELECT status, COUNT(*) as cnt FROM "${schema}".bcp_plans ${where} GROUP BY status`, params);
