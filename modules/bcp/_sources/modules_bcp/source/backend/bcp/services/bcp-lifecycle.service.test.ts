...
vi.mock('../ports/database.port', () => ({
  ...
  describe('domain queries', () => {
    it('getOverduePlans should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getOverduePlans('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getOverduePlans should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getOverduePlans('t1');
      expect(result).toEqual([]);
    });

    it('getDraftPlans should return array', async () => {
      (safeQuery as any).mockResolvedValueOnce({ rows: [{ id: '1', status: 'test', updatedAt: '2024-01-01' }] });
      const result = await getDraftPlans('t1');
      expect(Array.isArray(result)).toBe(true);
    });

    it('getDraftPlans should return empty on error', async () => {
      (safeQuery as any).mockRejectedValueOnce(new Error('db'));
      const result = await getDraftPlans('t1');
      expect(result).toEqual([]);
    });
