...
beforeAll(async () => {
  const mod = await import('./controls.module');
  manifest = mod.CONTROLS_MANIFEST ?? mod.default?.manifest;
});

describe('Controls Module Manifest', () => {
  it('has correct module code', () => { expect(manifest?.code).toBe('controls'); });
