...
const router = Router();
router.use(moduleStack('ksa-regulatory'));
router.use(auditMiddleware('ksa_regulatory'));

router.get(
  '/settings',
  authenticate,
  requirePermission('ksa_regulatory.manage'), validate({ query: z.record(z.unknown()) }), asyncHandler(async (req: Request, res: Response) => {
