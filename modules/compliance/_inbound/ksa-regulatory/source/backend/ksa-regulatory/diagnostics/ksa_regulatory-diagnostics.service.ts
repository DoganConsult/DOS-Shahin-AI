...
export async function runDiagnostics(tenantId: string): Promise<DiagnosticsResult> {
  ...
  const ownedTables = ['ksa_regulatory_readiness_snapshots'];
  ...
  checks.push({
    ...
    detail: `${tableRows.length}/${ownedTables.length} core ksa-regulatory tables found`,
  });

  const { rows: staleRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".ksa_regulatory_readiness_snapshots WHERE updated_at < NOW() - INTERVAL '90 days'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
  ...
  const { rows: auditRows } = await safeQuery(
    `SELECT COUNT(*) AS cnt FROM "${schema}".audit_trail WHERE module = 'ksa-regulatory' AND created_at > NOW() - INTERVAL '24 hours'`,
    [],
  ).catch(() => ({ rows: [{ cnt: 0 }] }));
