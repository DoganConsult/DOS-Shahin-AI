...
vi.mock('../domain/compliance.service', () => ({
...
import router from '../routes/compliance.routes';
import * as domainSvc from '../../domain/compliance.service';
...
describe('compliance routes', () => {
  ...
  describe('GET /api/compliance/:id', () => {
    // Route handler rejects non-UUID ids with 404 so that sibling sub-routes
    // like /compliance/frameworks don't collide with this catch-all.
    const UUID = '11111111-1111-4111-8111-111111111111';

    it('returns single record', async () => {
      const res = await request(app).get(`/api/compliance/${UUID}`);
      expect(res.status).toBe(200);
    });
  ...
  describe('DELETE /api/compliance/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/compliance/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
