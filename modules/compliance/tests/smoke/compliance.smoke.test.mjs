/**
 * Compliance module standalone smoke test using node:test.
 * Run: node --test tests/smoke/compliance.smoke.test.mjs
 */
...
test('bootstrap exports registerCompliance + lifecycle', () => {
  assert.equal(typeof mod.registerCompliance, 'function');
  assert.equal(typeof mod.bindCompliancePorts, 'function');
...
test('index barrel exports runMigrations + contract version', () => {
  ...
  assert.equal(idx.COMPLIANCE_CONTRACT_VERSION, 'v1');
});
...
test('registerCompliance returns module info', () => {
  const app = express();
  const result = mod.registerCompliance({ app });
  assert.equal(result.moduleCode, 'compliance');
  assert.equal(result.routeBase, '/api/compliance');
...
test('runMigrations applies migrations against an in-memory client', async () => {
  ...
  const tmp = fs.mkdtempSync(path.join(require('node:os').tmpdir(), 'cmpl-mig-'));
  ...
  const client = {
    async query(sql, params) {
      if (/SELECT filename FROM compliance_schema_migrations/i.test(sql)) {
        return { rows: [...fakeApplied].map((f) => ({ filename: f })), rowCount: fakeApplied.size };
      }
      if (/INSERT INTO compliance_schema_migrations/i.test(sql)) {
...
test('bindCompliancePorts accepts host bindings without throwing', () => {
  mod.bindCompliancePorts({
