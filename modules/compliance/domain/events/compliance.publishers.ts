...
export async function publish(
  ...
  if (!COMPLIANCE_EVENT_CONTRACT.published[eventName]) {
    logger.warn(`[${COMPLIANCE_EVENT_CONTRACT.moduleCode}] attempted to publish undeclared event: ${eventName}`);
    return;
  }
  const fullPayload: ModuleEventPayload = { ...payload, moduleCode: COMPLIANCE_EVENT_CONTRACT.moduleCode };
  if (_publisher) {
    await _publisher(fullPayload);
  }
  logger.debug(`[${COMPLIANCE_EVENT_CONTRACT.moduleCode}] published ${eventName}`);
}

export function getPublishedEventNames(): string[] {
  return Object.keys(COMPLIANCE_EVENT_CONTRACT.published);
}
