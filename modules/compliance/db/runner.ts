/**
 * Compliance DB migration runner — accepts any client conforming to DbClient.
 * Mirrors Foundation runner: usable from a host service that already has a
 * Postgres client wired (no `pg` dep required at module level).
 */
...
export async function runMigrations(
  ...
  const applied = await client.query<MigrationRecord>(
    'SELECT filename FROM compliance_schema_migrations',
  );
