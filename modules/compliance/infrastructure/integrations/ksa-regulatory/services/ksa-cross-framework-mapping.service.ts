  ...
  try {
    ...
    if (mappingDetails.length > 0 && mappingDetails.length <= 20) {
      try {
        ...
        const validated = Array.isArray(aiValidation?.validatedMappings) ? aiValidation.validatedMappings : [];
        const explanationMap = new Map<string, string>();
        for (const v of validated) {
          if (typeof v?.sourceControlCode !== 'string') continue;
          if (typeof v?.explanation !== 'string') continue;
          if (!v.explanation.trim()) continue;
        ...
        for (const m of mappingDetails) {
          ...
          if (explanation) m.mappingRationale = explanation;
        }
      } catch {
  ...
  for (let i = 0; i < frameworks.length; i++) {
    for (let j = i + 1; j < frameworks.length; j++) {
      ...
      try {
        ...
        const aiMappingsList = Array.isArray(aiMappings?.mappings) ? aiMappings.mappings : [];
        if (aiMappingsList.length > 0) {
          const aMap = new Map(controlsA.map(c => [c.code, c]));
          const bMap = new Map(controlsB.map(c => [c.code, c]));

          for (const m of aiMappingsList) {
            if (!m || typeof m.sourceCode !== 'string' || typeof m.targetCode !== 'string') continue;
            ...
            allMappings.push({
              ...
              mappingRationale: typeof m.rationale === 'string' ? m.rationale : '',
              commonDomain: typeof m.domain === 'string' && m.domain ? m.domain : null,
              commonThemes: toStringArray(m.themes),
            });
          }
        }
      } catch (aiErr) {
...
async function findEquivalencesWithAi(
  ...
  try {
    ...
    const candidateMap = new Map<string, GenericRow>(
      othersResult.rows.map((r: GenericRow) => [r.control_id, r]),
    );

    const equivalences: ControlEquivalence[] = [];
    const aiEquivalences = Array.isArray(aiResult.equivalences) ? aiResult.equivalences : [];
    for (const eq of aiEquivalences) {
      if (!eq || typeof eq.controlId !== 'string') continue;
    ...
    return {
      equivalences,
      complianceImpact: typeof aiResult.complianceImpact === 'string' && aiResult.complianceImpact
        ? aiResult.complianceImpact
        : 'See equivalences for cross-framework impact.',
    };
  } catch {
    return {
      equivalences: [],
