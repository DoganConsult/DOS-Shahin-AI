...
import { ButtonModule, DialogModule, DropdownModule, InputModule, PlaceholderModule, TableModule, TagModule, TilesModule, TooltipModule } from 'carbon-components-angular';
...
@Component({
    ...
    imports: [CommonModule, AppDatePipe, FormsModule, PageShellComponent, TableModule, TagModule, ButtonModule, TilesModule, TooltipModule, DropdownModule, InputModule, DialogModule],
    template: `
    <app-page-shell
      icon="sitemap"
      ...
      <div class="refresh-bar" *ngIf="lastUpdated">
        ...
        <button aria-label="Refresh" class="refresh-btn" (click)="loadData()" [disabled]="loading"><i class="" [class.pi-spin]="loading"></i></button>
      </div>
      ...
      <div class="efficiency-banner">
        ...
        <div class="eff-arrow"><i class=""></i></div>
        <div class="eff-stat">
      ...
      <div class="view-toggle">
        <button class="toggle-btn" [class.active]="viewMode === 'matrix'" (click)="viewMode = 'matrix'">
          <i class=""></i> {{ i18n.translate('frameworkMapping.matrixView') }}
        </button>
        <button class="toggle-btn" [class.active]="viewMode === 'visual'" (click)="viewMode = 'visual'">
          <i class=""></i> {{ i18n.translate('frameworkMapping.visualMap') }}
        </button>
      </div>
      ...
      <div *ngIf="viewMode === 'matrix'">
        <div class="filter-bar">
          ...
          <cds-dropdown [options]="frameworkOptions" [(ngModel)]="selectedFramework"
                      placeholder="All Frameworks" [showClear]="true" (onChange)="filterMappings()" />
        </div>

        <table cdsTable aria-label="Filtered Mappings table" [value]="filteredMappings" [paginator]="true" [rows]="15" [rowsPerPageOptions]="[10,15,25]"
                 styleClass="p-datatable-striped p-datatable-sm" [sortField]="'coverageCount'" [sortOrder]="-1">
          <ng-template pTemplate="header">
            <tr>
              <th >{{ i18n.translate('frameworkMapping.code') }}</th>
              ...
              <th  style="text-align: center">{{ i18n.translate('frameworkMapping.coverage') }}</th>
            </tr>
          </ng-template>
          <ng-template pTemplate="body" let-row>
            <tr>
              ...
              <td *ngFor="let fw of frameworkColumns" style="text-align: center">
                <i *ngIf="row.mappedFrameworks.includes(fw)" class="" style="color: var(--success); font-size: var(--font-size-md);"></i>
          ...
          <div class="vis-col">
            ...
            <div *ngFor="let m of mappingsWithConnections">
              <div *ngIf="highlightedControl === m.controlId || !highlightedControl" class="vis-targets">
                <cds-tag *ngFor="let fw of m.mappedFrameworks" [value]="fw" [severity]="fw === 'ECC' ? 'info' : 'success'" styleClass="vis-tag" />
      ...
      <div class="recommendations">
        <h3><i class=""></i> {{ i18n.translate('frameworkMapping.top10Coverage') }}</h3>
        <div class="rec-grid">
          <div *ngFor="let m of topCoverage" class="rec-card">
            ...
            <div class="rec-fws">
              <cds-tag *ngFor="let fw of m.mappedFrameworks" [value]="fw" severity="info" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="export-bar">
        <button class="export-btn pdf" (click)="exportMapping('pdf')"><i class=""></i> PDF</button>
        <button class="export-btn excel" (click)="exportMapping('excel')"><i class=""></i> Excel</button>
        <button class="export-btn html" (click)="exportMapping('html')"><i class=""></i> Interactive HTML</button>
      </div>
    </app-page-shell>
  `,
    styles: [`
...
export class FrameworkMappingComponent implements OnInit, OnDestroy {
  ...
  private applyData(data: Record<string, unknown>): void {
    const d = data as any;
    this.allMappings = d.mappings || [];
    this.frameworkColumns = d.frameworkColumns || [];
    this.uniqueControls = d.uniqueControls || 0;
    this.totalSatisfied = d.totalSatisfied || 0;
    this.efficiencyRatio = String(d.efficiencyRatio || '1.0');
