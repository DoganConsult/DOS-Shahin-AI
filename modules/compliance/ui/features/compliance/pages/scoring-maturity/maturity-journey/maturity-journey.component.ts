...
@Component({
    ...
    <div class="mat-root" [dir]="i18n.direction()">
      <header class="mat-header">
        ...
        <a routerLink="/agrc-os" class="mat-nav"><i class=""></i> AGRC-OS</a>
      </header>
      ...
      <div class="mat-grid">
        <section class="mat-card">
          <h2 class="mat-card-title"><i class=""></i> {{ i18n.translate('maturityJourney.maturityOverTime') }}</h2>
          <div class="mat-chart-wrap">
            <app-echart [options]="journeyOpts()" ariaLabel="Maturity journey timeline" />
          </div>
        </section>
        <section class="mat-card">
          <h2 class="mat-card-title"><i class=""></i> {{ i18n.translate('maturityJourney.capabilityRadar') }}</h2>
          <div class="mat-chart-wrap">
...
export class MaturityJourneyComponent implements OnInit {
  ...
  loadAll(): void {
    this.http.get<any>(`${this.api}/analytics/maturity`).pipe(catchError(() => of(null)), takeUntilDestroyed(this.destroyRef)).subscribe(m => {
      if (m) {
    ...
    this.http.get<any>(`${this.api}/analytics/predictions`).pipe(catchError(() => of(null)), takeUntilDestroyed(this.destroyRef)).subscribe(p => {
  ...
  buildJourney(): void {
    ...
    const aggs = t.map((d: Record<string, any>) => {
  ...
  buildRadar(criteria: Record<string, any>): void {
    if (!criteria) return;
    this.radarOpts.set({
      radar: {
        indicator: [
