/**
 * NcaAssessmentTableComponent — Dumb presentational component
 * Renders the NCA ECC assessment controls within a subdomain,
 * with status pills, notes, and evidence tags.
 * Parent: NCAAssessmentComponent
 */
...
/** Shared interfaces for NCA ECC assessment domain structures */
...
@Component({
  selector: 'app-nca-assessment-table',
  standalone: true,
        ...
        <div *ngFor="let ctrl of sd.controls" class="control-card"
          ...
          <div class="ctrl-notes" *ngIf="getItemStatus(ctrl.id) !== 'not_implemented'">
            <textarea pInputTextarea [rows]="1"
                      [placeholder]="i18n.translate('ncaAssessment.notesPlaceholder')"
                      [attr.aria-label]="i18n.translate('ncaAssessment.notesPlaceholder')"
...
export class NcaAssessmentTableComponent {
  i18n = inject(I18nService);

  /** The domain whose subdomains/controls to render */
  @Input() domain!: ECCDomain;
  ...
  getSubdomainAssessed(sd: ECCSubdomain): number {
    return sd.controls.filter(c => this.getItemStatus(c.id) !== 'not_implemented').length;
  }

  getSubdomainProgress(sd: ECCSubdomain): number {
