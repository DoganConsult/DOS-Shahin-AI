...
import { ButtonModule, DialogModule, DropdownModule, InputModule, NotificationModule, TableModule, TabsModule, TagModule, TilesModule, TooltipModule, UIShellModule } from 'carbon-components-angular';
...
@Component({
    ...
    imports: [
        ...
        TabsModule, TableModule, TilesModule, ButtonModule,
        DialogModule, InputModule, InputModule,
        DropdownModule, UIShellModule, TooltipModule, NotificationModule, TagModule, AppDatePipe,
    ],
    providers: [],
    template: `
    <app-page-shell
      ...
      <cds-tabs *ngIf="!loadError">
        <cds-tab [header]="isAr() ? 'قواعد السلوك' : 'Code of Conduct'">
          <section cdsToolbar styleClass="mb-3">
            <ng-template pTemplate="start">
              <button cdsButton [label]="isAr() ? 'إضافة قاعدة' : 'Add Code'" icon="" (onClick)="openCreateCode()" />
            </ng-template>
            <ng-template pTemplate="end">
              <app-export-button module="ethics-code-of-conduct" [data]="codeOfConductItems" />
            </ng-template>
          </section>
          <table cdsTable aria-label="Code Of Conduct Items table" [value]="codeOfConductItems" [paginator]="true" [rows]="10" styleClass="p-datatable-striped p-datatable-sm" *ngIf="codeOfConductItems.length > 0">
          ...
          <div class="empty-state" *ngIf="codeOfConductItems.length === 0 && !loading"><i class=" empty-icon"></i><p>{{ isAr() ? 'لا توجد قواعد سلوك' : 'No code of conduct items' }}</p></div>
        </cds-tab>

        <cds-tab [header]="isAr() ? 'الإفصاحات' : 'Disclosures'">
          <section cdsToolbar styleClass="mb-3">
            <ng-template pTemplate="start">
              <button cdsButton [label]="isAr() ? 'إضافة إفصاح' : 'Add Disclosure'" icon="" (onClick)="openCreateDisclosure()" />
              <span class="ml-3" *ngIf="disclosures.length > 0">
                ...
                <cds-dropdown [options]="statusFilterOptions" [(ngModel)]="disclosureStatusFilter" optionLabel="label" optionValue="value" [placeholder]="isAr() ? 'الكل' : 'All'" (onChange)="cdr.markForCheck()" [style]="{'min-width':'140px'}" />
              </span>
            </ng-template>
          </section>
          <table cdsTable aria-label="Disclosures table" [value]="disclosuresFiltered" [paginator]="true" [rows]="10" styleClass="p-datatable-striped p-datatable-sm" *ngIf="disclosuresFiltered.length > 0">
            <ng-template pTemplate="header">
              <tr>
            ...
            <ng-template pTemplate="body" let-item>
              <tr>
                <td><cds-tag [value]="item.report_type || item.disclosure_type || item.category" severity="info" /></td>
                <td>{{ item.title || item.discloser_name || '—' }}</td>
                <td>{{ (item.created_at || item.submitted_date) | appDate:'medium' }}</td>
                <td><app-status-badge [status]="item.status" /></td>
                <td>
                  <cds-dropdown [options]="statusOptions" [(ngModel)]="item.status" optionLabel="label" optionValue="value" [placeholder]="isAr() ? 'الحالة' : 'Status'" (onChange)="updateReportStatus(item, $event.value)" [style]="{'min-width':'120px'}" />
                </td>
              </tr>
            </ng-template>
          </table>
          <div class="empty-state" *ngIf="disclosuresFiltered.length === 0 && !loading"><i class=" empty-icon"></i><p>{{ isAr() ? 'لا توجد إفصاحات' : 'No disclosures' }}</p></div>
        </cds-tab>

        <cds-tab [header]="isAr() ? 'تقارير المخالفات' : 'Whistleblower Reports'">
          <section cdsToolbar styleClass="mb-3" *ngIf="whistleblowerReports.length > 0">
            <ng-template pTemplate="start">
              <label [attr.aria-label]="isAr() ? 'تصفية حسب الحالة' : 'Filter by status'" class="mr-2">{{ isAr() ? 'الحالة:' : 'Status:' }}</label>
              <cds-dropdown [options]="statusFilterOptions" [(ngModel)]="whistleblowerStatusFilter" optionLabel="label" optionValue="value" [placeholder]="isAr() ? 'الكل' : 'All'" (onChange)="cdr.markForCheck()" [style]="{'min-width':'140px'}" />
            </ng-template>
          </section>
          <table cdsTable aria-label="Whistleblower Reports table" [value]="whistleblowerFiltered" [paginator]="true" [rows]="10" styleClass="p-datatable-striped p-datatable-sm" *ngIf="whistleblowerFiltered.length > 0">
            <ng-template pTemplate="header">
              <tr>
            ...
            <ng-template pTemplate="body" let-item>
              <tr>
                ...
                <td>{{ item.category || '—' }}</td>
                <td><cds-tag [value]="item.severity" [severity]="item.severity === 'critical' || item.severity === 'high' ? 'danger' : 'info'" /></td>
          ...
          <div class="empty-state" *ngIf="whistleblowerFiltered.length === 0 && !loading"><i class=" empty-icon"></i><p>{{ isAr() ? 'لا توجد تقارير' : 'No reports' }}</p></div>
        </cds-tab>

        <cds-tab [header]="isAr() ? 'تضارب المصالح' : 'Conflict of Interest'">
          <table cdsTable aria-label="Conflict Of Interest table" [value]="conflictOfInterest" [paginator]="true" [rows]="10" styleClass="p-datatable-striped p-datatable-sm" *ngIf="conflictOfInterest.length > 0">
            <ng-template pTemplate="header">
              <tr>
            ...
            <ng-template pTemplate="body" let-item>
              <tr>
                ...
                <td><cds-tag [value]="item.report_type || item.category || item.conflict_type" severity="warning" /></td>
          ...
          <div class="empty-state" *ngIf="conflictOfInterest.length === 0 && !loading"><i class=" empty-icon"></i><p>{{ isAr() ? 'لا يوجد تضارب مصالح' : 'No conflicts' }}</p></div>
        </cds-tab>

        <cds-tab [header]="isAr() ? 'روابط المنصة' : 'Cross-Module Links'">
    ...
    <cds-modal [header]="isAr() ? 'إضافة قاعدة سلوك' : 'Add Code of Conduct'" [(visible)]="showCodeDialog" [modal]="true" [style]="{width:'520px'}">
      ...
      <ng-template pTemplate="footer">
        <button cdsButton label="Cancel" icon="" styleClass="" (onClick)="showCodeDialog=false" />
        <button cdsButton label="Save" icon="" (onClick)="saveCode()" [disabled]="!codeForm.title_en || !codeForm.owner" />
      </ng-template>
    </cds-modal>

    <cds-modal [header]="isAr() ? 'إضافة إفصاح' : 'Add Disclosure'" [(visible)]="showDisclosureDialog" [modal]="true" [style]="{width:'520px'}">
      <div class="dialog-form">
        <div class="field"><label>Disclosure Type</label>
          <cds-dropdown [options]="disclosureTypes" [(ngModel)]="disclosureForm.disclosure_type" optionLabel="label" optionValue="value" [style]="{width:'100%'}" />
        </div>
      ...
      <ng-template pTemplate="footer">
        <button cdsButton label="Cancel" icon="" styleClass="" (onClick)="showDisclosureDialog=false" />
        <button cdsButton label="Save" icon="" (onClick)="saveDisclosure()" [disabled]="!disclosureForm.discloser_name" />
      </ng-template>
    </cds-modal>
  `,
    styles: [`
