...
import { UiOsPreferenceManager } from '../managers/ui-os-preference.manager.js';
import { PreferencePatchSchema, LocalePatchSchema, ThemePatchSchema, DensityPatchSchema, } from '../schemas/preferences.schemas.js';
...
export function createPreferencesRouter(pool) {
    const router = Router();
    ...
    router.put('/preferences', async (req, res) => {
        ...
        try {
            const prefs = await manager.upsert(ctx.tenantId, ctx.userId, parsed.data);
            res.json(prefs);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : 'unknown_error';
    ...
    router.put('/preferences/locale', async (req, res) => {
        ...
        try {
            const prefs = await manager.setLocale(ctx.tenantId, ctx.userId, parsed.data.locale, parsed.data.direction);
            res.json(prefs);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : 'unknown_error';
    ...
    router.put('/preferences/theme', async (req, res) => {
        ...
        try {
            const prefs = await manager.setTheme(ctx.tenantId, ctx.userId, parsed.data.appearance, parsed.data.accent_color ?? null);
            res.json(prefs);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : 'unknown_error';
    ...
    router.put('/preferences/density', async (req, res) => {
        ...
        try {
            const prefs = await manager.setDensity(ctx.tenantId, ctx.userId, parsed.data.density);
            res.json(prefs);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : 'unknown_error';
