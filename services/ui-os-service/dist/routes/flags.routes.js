...
import { FeatureFlagSchema, FlagAssignmentSchema, ExperimentSchema, VariantSchema, ExperimentAssignmentSchema, RolloutRuleSchema, KillSwitchSchema, KillSwitchTriggerSchema, } from '../schemas/flags.schemas.js';
...
export function createFlagsRouter(pool) {
    const router = Router();
    ...
    router.get('/feature-flags', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ flags: await m.listFlags(c.tenantId) });
        }
        catch (e) {
            fail(res, 'flags_list_failed', e);
        }
    });
    router.get('/feature-flags/:flagId', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.flagId)) {
    ...
    router.put('/feature-flags', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertFlag(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'flag_upsert_failed', e);
        }
    });
    ...
    router.get('/feature-flags/:flagId/assignments', fgaViewer, async (req, res) => {
        ...
        try {
            res.json({ assignments: await m.listFlagAssignments(c.tenantId, req.params.flagId) });
        }
        catch (e) {
            fail(res, 'flag_assignments_list_failed', e);
        }
    });
    router.put('/feature-flags/:flagId/assignments', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertFlagAssignment(c.tenantId, req.params.flagId, p.data));
        }
        catch (e) {
            fail(res, 'flag_assignment_upsert_failed', e);
        }
    });
    router.delete('/feature-flag-assignments/:assignmentId', fgaAdmin, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'flag_assignment_delete_failed', e);
        }
    });
    // Experiments
    router.get('/experiments', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ experiments: await m.listExperiments(c.tenantId, String(req.query.activeOnly ?? '') === 'true') });
        }
        catch (e) {
            fail(res, 'experiments_list_failed', e);
        }
    });
    router.get('/experiments/:experimentId', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.experimentId)) {
            res.status(400).json({ error: 'invalid_experiment_id' });
            return;
        }
        try {
            ...
            fail(res, 'experiment_get_failed', e);
        }
    });
    router.put('/experiments', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertExperiment(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'experiment_upsert_failed', e);
        }
    });
    router.delete('/experiments/:experimentId', fgaAdmin, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'experiment_delete_failed', e);
        }
    });
    // Variants
    router.get('/experiments/:experimentId/variants', fgaViewer, async (req, res) => {
        ...
        try {
            res.json({ variants: await m.listVariants(c.tenantId, req.params.experimentId) });
        }
        catch (e) {
            fail(res, 'variants_list_failed', e);
        }
    });
    router.put('/experiments/:experimentId/variants', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertVariant(c.tenantId, req.params.experimentId, p.data));
        }
        catch (e) {
            fail(res, 'variant_upsert_failed', e);
        }
    });
    router.delete('/experiment-variants/:variantId', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.variantId)) {
        ...
        try {
            ...
            fail(res, 'variant_delete_failed', e);
        }
    });
    ...
    router.get('/experiments/:experimentId/assignments', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json({ assignments: await m.listExperimentAssignments(c.tenantId, req.params.experimentId, parseInt(String(req.query.limit ?? 200), 10)) });
        }
        catch (e) {
            fail(res, 'experiment_assignments_list_failed', e);
        }
    });
    router.get('/experiments/:experimentId/assignments/me', fgaViewer, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'experiment_assignment_get_failed', e);
        }
    });
    router.put('/experiments/:experimentId/assignments', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertExperimentAssignment(c.tenantId, req.params.experimentId, p.data.user_id, p.data.variant_id));
        }
        catch (e) {
            fail(res, 'experiment_assignment_upsert_failed', e);
        }
    });
    // Rollout rules
    router.get('/rollout-rules', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ rules: await m.listRolloutRules(c.tenantId, {
                    flagId: req.query.flagId ?? null,
                    experimentId: req.query.experimentId ?? null,
                }) });
        }
        catch (e) {
            fail(res, 'rollout_rules_list_failed', e);
        }
    });
    router.post('/rollout-rules', fgaAdmin, async (req, res) => {
        ...
        try {
            res.status(201).json(await m.createRolloutRule(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'rollout_rule_create_failed', e);
        }
    });
    router.delete('/rollout-rules/:ruleId', fgaAdmin, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'rollout_rule_delete_failed', e);
        }
    });
    // Kill switches
    router.get('/kill-switches', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ switches: await m.listKillSwitches(c.tenantId, String(req.query.activeOnly ?? '') === 'true') });
        }
        catch (e) {
            fail(res, 'kill_switches_list_failed', e);
        }
    });
    router.put('/kill-switches', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertKillSwitch(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'kill_switch_upsert_failed', e);
        }
    });
    router.post('/kill-switches/:switchCode/trigger', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.triggerKillSwitch(c.tenantId, req.params.switchCode, c.userId, p.data.reason ?? null));
        }
        catch (e) {
            fail(res, 'kill_switch_trigger_failed', e);
        }
    });
    router.post('/kill-switches/:switchCode/resolve', fgaAdmin, async (req, res) => {
        ...
        try {
            ...
            if (!r) {
                res.status(404).json({ error: 'switch_not_active' });
                return;
            }
            res.json(r);
        }
        catch (e) {
            fail(res, 'kill_switch_resolve_failed', e);
        }
    });
    return router;
}
