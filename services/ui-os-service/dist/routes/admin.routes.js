...
import { UiOsAdminManager } from '../managers/ui-os-admin.manager.js';
...
export function createAdminRouter(pool) {
    ...
    router.get('/admin/drafts', async (req, res) => {
        ...
        if (!ctx)
            return;
        try {
            res.json({ drafts: await admin.listDrafts(ctx.tenantId, req.query.targetType ?? null) });
        }
        catch (e) {
            res.status(500).json({ error: 'drafts_list_failed', message: e.message });
        }
    });
    router.post('/admin/drafts', async (req, res) => {
        ...
        try {
            res.status(201).json(await admin.createDraft(ctx.tenantId, ctx.userId, parsed.data));
        }
        catch (e) {
            const msg = e.message;
            if (msg.includes('duplicate key')) {
                res.status(409).json({ error: 'draft_exists' });
                return;
            }
    ...
    router.put('/admin/drafts/:draftId', async (req, res) => {
        ...
        try {
            const r = await admin.updateDraft(ctx.tenantId, req.params.draftId, parsed.data);
            if (!r) {
            ...
            res.status(500).json({ error: 'draft_update_failed', message: e.message });
        }
    });
    router.post('/admin/drafts/:draftId/validate', async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ error: 'draft_validate_failed', message: e.message });
        }
    });
    router.post('/admin/drafts/:draftId/submit', async (req, res) => {
        ...
        try {
            ...
            if (!r) {
                res.status(404).json({ error: 'draft_not_submittable' });
                return;
            }
            res.json(r);
        }
        catch (e) {
            res.status(500).json({ error: 'draft_submit_failed', message: e.message });
        }
    });
    router.post('/admin/publish/:draftId', async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ error: 'draft_publish_failed', message: e.message });
        }
    });
