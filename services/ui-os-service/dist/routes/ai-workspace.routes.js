...
import { AiContextPanelSchema, AiSuggestionSchema, AiSuggestionFeedbackSchema, AiActionDraftSchema, AiActionDraftStateSchema, AiMemorySchema, AiPromptTemplateSchema, AiPromptTemplateToolSchema, AiToolSurfaceBindingSchema, } from '../schemas/ai-workspace.schemas.js';
...
export function createAiWorkspaceRouter(pool) {
    const router = Router();
    ...
    router.get('/ai/panels', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ panels: await m.listPanels(c.tenantId) });
        }
        catch (e) {
    ...
    router.put('/ai/panels', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertPanel(c.tenantId, p.data));
        }
        catch (e) {
    ...
    router.delete('/ai/panels/:panelId', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.panelId)) {
            res.status(400).json({ error: 'invalid_panel_id' });
            return;
        }
        try {
            ...
            fail(res, 'panel_delete_failed', e);
        }
    });
    // Suggestions
    router.post('/ai/suggestions', fgaEditor, async (req, res) => {
        ...
        try {
            res.status(201).json(await m.recordSuggestion(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'suggestion_record_failed', e);
        }
    });
    router.get('/ai/suggestions', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ suggestions: await m.listSuggestions(c.tenantId, c.userId, parseInt(String(req.query.limit ?? 50), 10)) });
        }
        catch (e) {
            fail(res, 'suggestions_list_failed', e);
        }
    });
    // Suggestion feedback
    router.put('/ai/suggestions/:suggestionId/feedback', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.suggestionId)) {
            res.status(400).json({ error: 'invalid_suggestion_id' });
            return;
        }
        ...
        try {
            res.json(await m.recordFeedback(c.tenantId, req.params.suggestionId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'feedback_record_failed', e);
        }
    });
    router.get('/ai/suggestions/:suggestionId/feedback', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json({ feedback: await m.listFeedback(c.tenantId, req.params.suggestionId) });
        }
        catch (e) {
            fail(res, 'feedback_list_failed', e);
        }
    });
    // Action drafts
    router.get('/ai/action-drafts', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ drafts: await m.listDrafts(c.tenantId, c.userId, {
                    state: req.query.state ?? null,
                    limit: parseInt(String(req.query.limit ?? 100), 10),
                }) });
        }
        catch (e) {
            fail(res, 'action_drafts_list_failed', e);
        }
    });
    router.get('/ai/action-drafts/:draftId', fgaViewer, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'action_draft_get_failed', e);
        }
    });
    router.post('/ai/action-drafts', fgaEditor, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'action_draft_create_failed', e);
        }
    });
    router.post('/ai/action-drafts/:draftId/state', fgaEditor, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'action_draft_state_failed', e);
        }
    });
    router.delete('/ai/action-drafts/:draftId', fgaEditor, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'action_draft_delete_failed', e);
        }
    });
    // Workspace memory
    router.get('/ai/memory', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ memory: await m.listMemory(c.tenantId, c.userId) });
        }
        catch (e) {
            fail(res, 'memory_list_failed', e);
        }
    });
    ...
    router.put('/ai/memory', fgaViewer, async (req, res) => {
        ...
        try {
            res.json(await m.upsertMemory(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'memory_upsert_failed', e);
        }
    });
    router.delete('/ai/memory/:memoryKey', fgaViewer, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'memory_delete_failed', e);
        }
    });
    // Prompt templates
    router.get('/ai/prompt-templates', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ templates: await m.listPromptTemplates(c.tenantId, String(req.query.activeOnly ?? '') === 'true') });
        }
        catch (e) {
            fail(res, 'templates_list_failed', e);
        }
    });
    router.get('/ai/prompt-templates/:templateId', fgaViewer, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'template_get_failed', e);
        }
    });
    router.put('/ai/prompt-templates', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertPromptTemplate(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'template_upsert_failed', e);
        }
    });
    router.delete('/ai/prompt-templates/:templateId', fgaAdmin, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'template_delete_failed', e);
        }
    });
    ...
    router.get('/ai/prompt-templates/:templateId/tools', fgaViewer, async (req, res) => {
        ...
        try {
            res.json({ tools: await m.listTemplateTools(c.tenantId, req.params.templateId) });
        }
        catch (e) {
            fail(res, 'template_tools_list_failed', e);
        }
    });
    router.put('/ai/prompt-templates/:templateId/tools', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertTemplateTool(c.tenantId, req.params.templateId, p.data));
        }
        catch (e) {
            fail(res, 'template_tool_upsert_failed', e);
        }
    });
    router.delete('/ai/prompt-template-tools/:toolBindingId', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.toolBindingId)) {
            res.status(400).json({ error: 'invalid_tool_binding_id' });
            return;
        }
        try {
            const ok = await m.deleteTemplateTool(c.tenantId, req.params.toolBindingId);
            if (!ok) {
                res.status(404).json({ error: 'tool_binding_not_found' });
                return;
            }
            res.status(204).send();
        }
        catch (e) {
            fail(res, 'template_tool_delete_failed', e);
        }
    });
    ...
    router.get('/ai/tool-surface-bindings', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ bindings: await m.listToolSurfaceBindings(c.tenantId, req.query.surfaceKey ?? null) });
        }
        catch (e) {
            fail(res, 'tool_surface_bindings_list_failed', e);
        }
    });
    router.put('/ai/tool-surface-bindings', fgaAdmin, async (req, res) => {
        ...
        try {
            res.json(await m.upsertToolSurfaceBinding(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'tool_surface_binding_upsert_failed', e);
        }
    });
    router.delete('/ai/tool-surface-bindings/:bindingId', fgaAdmin, async (req, res) => {
        ...
        try {
            ...
            fail(res, 'tool_surface_binding_delete_failed', e);
        }
    });
    return router;
}
//# sourceMappingURL=ai-workspace.routes.js.map