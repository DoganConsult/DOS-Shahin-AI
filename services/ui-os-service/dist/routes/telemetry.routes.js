...
import { PageViewSchema, ClickSchema, CommandSchema, RenderPerfSchema, ErrorSchema, WidgetUsageSchema, SearchEventSchema, FunnelStepSchema, RetentionSchema, } from '../schemas/telemetry.schemas.js';
...
export function createTelemetryRouter(pool) {
    const router = Router();
    ...
    router.post('/telemetry/page-view', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordPageView(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'page_view_record_failed', e);
        }
    });
    router.get('/telemetry/page-view/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.pageViewStats(c.tenantId, {
                    routeKey: req.query.routeKey ?? null,
                    sinceHours: parseInt(String(req.query.sinceHours ?? 24), 10),
                }) });
        }
        catch (e) {
            fail(res, 'page_view_stats_failed', e);
        }
    });
    // ── Click ───────────────────────────────────────────────────
    router.post('/telemetry/click', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordClick(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'click_record_failed', e);
        }
    });
    router.get('/telemetry/click/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.clickStats(c.tenantId, {
                    targetKind: req.query.targetKind ?? null,
                    sinceHours: parseInt(String(req.query.sinceHours ?? 24), 10),
                }) });
        }
        catch (e) {
            fail(res, 'click_stats_failed', e);
        }
    });
    // ── Command ─────────────────────────────────────────────────
    router.post('/telemetry/command', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordCommand(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'command_record_failed', e);
        }
    });
    router.get('/telemetry/command/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.commandStats(c.tenantId, parseInt(String(req.query.sinceHours ?? 24), 10)) });
        }
        catch (e) {
            fail(res, 'command_stats_failed', e);
        }
    });
    // ── Render perf ─────────────────────────────────────────────
    router.post('/telemetry/render-perf', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordRenderPerf(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'render_perf_record_failed', e);
        }
    });
    router.get('/telemetry/render-perf/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.renderPerfStats(c.tenantId, {
                    routeKey: req.query.routeKey ?? null,
                    sinceHours: parseInt(String(req.query.sinceHours ?? 24), 10),
                }) });
        }
        catch (e) {
            fail(res, 'render_perf_stats_failed', e);
        }
    });
    // ── Error ───────────────────────────────────────────────────
    router.post('/telemetry/error', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordError(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'error_record_failed', e);
        }
    });
    router.get('/telemetry/error/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.errorStats(c.tenantId, {
                    errorKind: req.query.errorKind ?? null,
                    sinceHours: parseInt(String(req.query.sinceHours ?? 24), 10),
                }) });
        }
        catch (e) {
            fail(res, 'error_stats_failed', e);
        }
    });
    // ── Widget usage ────────────────────────────────────────────
    router.post('/telemetry/widget-usage', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordWidgetUsage(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'widget_usage_record_failed', e);
        }
    });
    router.get('/telemetry/widget-usage/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.widgetUsageStats(c.tenantId, parseInt(String(req.query.sinceHours ?? 24), 10)) });
        }
        catch (e) {
            fail(res, 'widget_usage_stats_failed', e);
        }
    });
    // ── Search events ───────────────────────────────────────────
    router.post('/telemetry/search', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordSearchEvent(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'search_event_record_failed', e);
        }
    });
    router.get('/telemetry/search/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ stats: await m.searchStats(c.tenantId, parseInt(String(req.query.sinceHours ?? 24), 10)) });
        }
        catch (e) {
            fail(res, 'search_stats_failed', e);
        }
    });
    // ── Funnel events ───────────────────────────────────────────
    router.post('/telemetry/funnel', fgaViewer, async (req, res) => {
        ...
        try {
            res.status(202).json(await m.recordFunnelStep(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'funnel_record_failed', e);
        }
    });
    router.get('/telemetry/funnel/:funnelCode/stats', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ steps: await m.funnelStats(c.tenantId, req.params.funnelCode, parseInt(String(req.query.sinceHours ?? 168), 10)) });
        }
        catch (e) {
            fail(res, 'funnel_stats_failed', e);
        }
    });
    // ── Retention snapshots ─────────────────────────────────────
    router.post('/telemetry/retention', fgaAdmin, async (req, res) => {
        ...
        try {
            res.status(201).json(await m.recordRetention(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'retention_record_failed', e);
        }
    });
    router.get('/telemetry/retention', fgaAdmin, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ matrix: await m.retentionMatrix(c.tenantId, req.query.cohortCode ?? null) });
        }
        catch (e) {
            fail(res, 'retention_matrix_failed', e);
        }
    });
    return router;
}
