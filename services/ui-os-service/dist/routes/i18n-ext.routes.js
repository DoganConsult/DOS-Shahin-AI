...
import { NamespaceSchema, VersionSchema, SetCurrentVersionSchema, OverrideSchema, LocalePreferencesSchema, RtlValidationSchema, } from '../schemas/i18n-ext.schemas.js';
...
export function createI18nExtRouter(pool) {
    const router = Router();
    ...
    router.get('/i18n/namespaces', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ namespaces: await m.listNamespaces(c.tenantId) });
        }
        catch (e) {
            fail(res, 'namespaces_list_failed', e);
        }
    });
    router.put('/i18n/namespaces', fgaEditor, async (req, res) => {
        ...
        try {
            res.json(await m.upsertNamespace(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'namespace_upsert_failed', e);
        }
    });
    router.delete('/i18n/namespaces/:namespaceId', fgaEditor, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.namespaceId)) {
            res.status(400).json({ error: 'invalid_namespace_id' });
            return;
        }
        try {
            ...
            fail(res, 'namespace_delete_failed', e);
        }
    });
    // Versions
    router.get('/i18n/namespaces/:namespaceId/versions', fgaViewer, async (req, res) => {
        ...
        try {
            res.json({ versions: await m.listVersions(c.tenantId, req.params.namespaceId, req.query.locale ?? null) });
        }
        catch (e) {
            fail(res, 'versions_list_failed', e);
        }
    });
    router.put('/i18n/namespaces/:namespaceId/versions', fgaEditor, async (req, res) => {
        ...
        try {
            res.json(await m.upsertVersion(c.tenantId, req.params.namespaceId, p.data));
        }
        catch (e) {
            fail(res, 'version_upsert_failed', e);
        }
    });
    ...
    router.get('/i18n/namespaces/:namespaceId/overrides', fgaViewer, async (req, res) => {
        ...
        try {
            res.json({ overrides: await m.listOverrides(c.tenantId, req.params.namespaceId, req.query.locale ?? null) });
        }
        catch (e) {
            fail(res, 'overrides_list_failed', e);
        }
    });
    router.put('/i18n/namespaces/:namespaceId/overrides', fgaEditor, async (req, res) => {
        ...
        try {
            res.json(await m.upsertOverride(c.tenantId, c.userId, req.params.namespaceId, p.data));
        }
        catch (e) {
            fail(res, 'override_upsert_failed', e);
        }
    });
    router.delete('/i18n/overrides/:overrideId', fgaEditor, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.overrideId)) {
            res.status(400).json({ error: 'invalid_override_id' });
            return;
        }
        try {
            ...
            fail(res, 'override_delete_failed', e);
        }
    });
    ...
    router.get('/i18n/locale-preferences', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json(await m.getLocalePreferences(c.tenantId, c.userId) ?? {});
        }
        catch (e) {
            fail(res, 'locale_prefs_get_failed', e);
        }
    });
    router.put('/i18n/locale-preferences', fgaViewer, async (req, res) => {
        ...
        try {
            res.json(await m.upsertLocalePreferences(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'locale_prefs_put_failed', e);
        }
    });
    ...
    router.get('/i18n/rtl-validations', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ results: await m.listRtlValidations(c.tenantId, {
            ...
            fail(res, 'rtl_validations_list_failed', e);
        }
    });
    router.post('/i18n/rtl-validations', fgaEditor, async (req, res) => {
        ...
        try {
            res.status(201).json(await m.recordRtlValidation(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'rtl_validation_record_failed', e);
        }
    });
    return router;
}
//# sourceMappingURL=i18n-ext.routes.js.map