...
function isWorkspaceRoute(row) {
    ...
    if (!pathPattern || NON_WORKSPACE_PATHS.has(pathPattern))
        return false;
    if (pathPattern.startsWith('/admin/'))
        return false;
    if (moduleCode === 'marketing')
        return false;
    if (componentKey.startsWith('auth.'))
        return false;
    if (!row.permission_key)
        return false;
    return true;
}
export function createDynamicUiContractRouter(pool) {
    const router = Router();
    ...
    router.get('/workspace/nav', async (req, res) => {
        try {
            ...
            const [legacyR, newR] = await Promise.all([
                pool.query(`SELECT r.module_code, r.path_pattern, r.component_key,
            ...
            ORDER BY r.module_code, r.sort_order, r.path_pattern`, [tenantId]),
                pool.query(`SELECT i.module_code,
            ...
            const seen = new Set();
            const rows = [];
            const allRows = [...legacyR.rows, ...newR.rows];
    ...
    router.get('/route-catalog', async (_req, res) => {
        try {
            const { rows } = await pool.query(`SELECT id::text, module_code, path_pattern, component_key,
          ...
          ORDER BY module_code, sort_order, path_pattern`);
            ...
            res.status(500).json({ error: 'route_catalog_failed', message: e.message });
        }
    });
    router.get('/contract/:moduleCode', async (req, res) => {
        ...
        try {
            const [moduleRow, routes, theme, kpis, actions, resources, i18n, widgets, pageHeaders, gridColumns, paletteActions, searchScopes, aiTips] = await Promise.all([
                pool.query(`SELECT module_code, platform_key, product_key, display_name,
                  default_route, registry_status, canonical_source
             FROM dos.dynamic_ui_modules
            WHERE module_code = $1`, [moduleCode]),
                pool.query(`SELECT id::text, path_pattern, component_key, permission_key,
            ...
            ORDER BY sort_order, path_pattern`, [moduleCode]),
                pool.query(`SELECT token_key, token_value, scope, route
             FROM dos.dynamic_ui_theme_tokens
            WHERE tenant_id IS NULL AND module_code = $1 AND is_active = TRUE
            ORDER BY scope, token_key`, [moduleCode]),
                pool.query(`SELECT route, kpi_key, label_key, unit, data_resource,
                  permission, scope, format, trend_enabled, sort_order
             FROM dos.dynamic_ui_kpis
            WHERE tenant_id IS NULL AND module_code = $1 AND is_active = TRUE
            ORDER BY scope, sort_order, kpi_key`, [moduleCode]),
                pool.query(`SELECT route, action_id, position, label_key, icon,
                  permission, risk_level, requires_approval,
                  evidence_required, handler_key, sort_order
             FROM dos.dynamic_ui_actions
            WHERE tenant_id IS NULL AND module_code = $1 AND is_active = TRUE
            ORDER BY route, position, sort_order, action_id`, [moduleCode]),
                pool.query(`SELECT resource_key, resource_type, url_or_query, permission,
                  cache_ttl_sec, realtime_topic, pagination, shape_ref
             FROM dos.dynamic_ui_data_resources
            WHERE tenant_id IS NULL AND module_code = $1 AND is_active = TRUE
            ORDER BY resource_key`, [moduleCode]),
                pool.query(`SELECT key_path, en, ar
             FROM dos.dynamic_ui_i18n_keys
            WHERE module_code = $1
            ORDER BY key_path`, [moduleCode]),
                pool.query(`SELECT route, widget_key, zone, permission, sort_order,
                  is_signature, config
             FROM dos.dynamic_ui_widgets
            WHERE module_code = $1 AND tenant_id IS NULL AND is_active = TRUE
            ORDER BY route, sort_order, widget_key`, [moduleCode]),
                pool.query(`SELECT route, variant, eyebrow_key, title_key, subtitle_key,
            ...
            ORDER BY route`, [moduleCode]),
                pool.query(`SELECT route, column_key, label_i18n_key, data_path, data_type,
                  formatter_key, width_px, align, sortable, filterable,
                  hidden_default, sort_order
             FROM dos.dynamic_ui_grid_columns
            WHERE module_code = $1 AND tenant_id = '' AND is_active = TRUE
            ORDER BY route, sort_order, column_key`, [moduleCode]),
                pool.query(`SELECT action_code, label_i18n_key, shortcut, route, intent_code, sort_order
             FROM dos.dynamic_ui_command_palette_actions
            WHERE module_code = $1 AND registry_status = 'active'
            ORDER BY sort_order, action_code`, [moduleCode]),
                pool.query(`SELECT scope_code, label_i18n_key, resource_key, result_route, sort_order
             FROM dos.dynamic_ui_search_scopes
            WHERE module_code = $1 AND registry_status = 'active'
            ORDER BY sort_order, scope_code`, [moduleCode]),
                pool.query(`SELECT route, tip_code, severity, title_key, body_key,
                  cta_label_key, cta_route, cta_action_id, audience_role,
                  source, sort_order
             FROM dos.dynamic_ui_ai_tips
            WHERE module_code = $1 AND tenant_id = '' AND is_active = TRUE
            ORDER BY route, sort_order, tip_code`, [moduleCode]),
            ]);
            ...
            // the §10 drift test.
            const driftRows = routes.rows.filter(r => !r.page_type || !r.layout || !r.kpi_scope || !r.title_key);
            ...
            res.status(500).json({ error: 'contract_query_failed', message: e.message });
        }
    });
    ...
    router.get('/page-experience', async (req, res) => {
        ...
        try {
            const [routeRows, actions, kpis] = await Promise.all([
                pool.query(`SELECT id::text, module_code, path_pattern, component_key,
            ...
            ORDER BY position, sort_order, action_id`, [route]),
                pool.query(`SELECT route, kpi_key, label_key, unit, data_resource,
                  permission, scope, format, trend_enabled, sort_order
             FROM dos.dynamic_ui_kpis
            WHERE tenant_id IS NULL AND route = $1 AND is_active = TRUE
            ORDER BY sort_order, kpi_key`, [route]),
            ]);
            ...
            for (const a of actions.rows) {
                if (!a.permission || userPerms.has(a.permission))
                    visibleActions.push(a);
                else
                    hiddenActions.push(a);
            }
            const visibleKpis = [];
            for (const k of kpis.rows) {
                if (!k.permission || userPerms.has(k.permission))
                    visibleKpis.push(k);
            }
            res.json({
                route: r.path_pattern,
            ...
            res.status(500).json({ error: 'page_experience_failed', message: e.message });
        }
    });
    return router;
}
//# sourceMappingURL=dynamic-ui-contract.routes.js.map