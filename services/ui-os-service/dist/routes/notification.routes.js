...
import { NotificationCreateSchema, NotificationPreferenceSchema, InboxViewSchema, InboxRuleSchema, InboxSnoozeSchema, InboxAssignmentSchema, } from '../schemas/notification.schemas.js';
...
export function createNotificationRouter(pool) {
    ...
    router.get('/notifications/inbox', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ items: await m.listInbox(c.tenantId, c.userId, {
                    unreadOnly: req.query.unread === 'true' ? true : null,
                    category: req.query.category ?? null,
                    limit: parseInt(String(req.query.limit ?? 100), 10),
                }) });
        }
        catch (e) {
            fail(res, 'inbox_list_failed', e);
        }
    });
    router.get('/notifications/unread-count', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ count: await m.unreadCount(c.tenantId, c.userId) });
        }
        catch (e) {
            fail(res, 'unread_count_failed', e);
        }
    });
    router.post('/notifications', fgaEditor, async (req, res) => {
        ...
        try {
            res.status(201).json(await m.createNotification(c.tenantId, p.data));
        }
        catch (e) {
            fail(res, 'notification_create_failed', e);
        }
    });
    router.post('/notifications/:notificationId/read', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.notificationId)) {
            res.status(400).json({ error: 'invalid_notification_id' });
            return;
        }
        try {
            res.json(await m.markRead(c.tenantId, c.userId, req.params.notificationId));
        }
        catch (e) {
            fail(res, 'mark_read_failed', e);
        }
    });
    router.post('/notifications/:notificationId/dismiss', fgaViewer, async (req, res) => {
        ...
        try {
            res.json(await m.markDismissed(c.tenantId, c.userId, req.params.notificationId));
        }
        catch (e) {
            fail(res, 'mark_dismissed_failed', e);
        }
    });
    ...
    router.get('/notifications/preferences', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ preferences: await m.listPreferences(c.tenantId, c.userId) });
        }
        catch (e) {
            fail(res, 'prefs_list_failed', e);
        }
    });
    router.put('/notifications/preferences', fgaViewer, async (req, res) => {
        ...
        try {
            res.json(await m.upsertPreference(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'pref_upsert_failed', e);
        }
    });
    // Inbox views
    router.get('/inbox/views', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ views: await m.listViews(c.tenantId, c.userId) });
        }
        catch (e) {
    ...
    router.put('/inbox/views', fgaViewer, async (req, res) => {
        ...
        try {
            res.json(await m.upsertView(c.tenantId, c.userId, p.data));
        }
        catch (e) {
    ...
    router.get('/inbox/rules', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        const scope = req.query.scope === 'tenant' ? null : c.userId;
        try {
            res.json({ rules: await m.listRules(c.tenantId, scope) });
        }
        catch (e) {
    ...
    router.put('/inbox/rules', fgaEditor, async (req, res) => {
        ...
        try {
            res.json(await m.upsertRule(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'rule_upsert_failed', e);
        }
    });
    ...
    router.get('/inbox/snoozes', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ snoozes: await m.listSnoozes(c.tenantId, c.userId) });
        }
        catch (e) {
            fail(res, 'snoozes_list_failed', e);
        }
    });
    router.put('/inbox/snoozes', fgaViewer, async (req, res) => {
        ...
        try {
            res.json(await m.upsertSnooze(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'snooze_upsert_failed', e);
        }
    });
    router.delete('/inbox/snoozes/:snoozeId', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        if (!UUID.test(req.params.snoozeId)) {
            res.status(400).json({ error: 'invalid_snooze_id' });
            return;
        }
        try {
            const ok = await m.deleteSnooze(c.tenantId, c.userId, req.params.snoozeId);
            if (!ok) {
                res.status(404).json({ error: 'snooze_not_found' });
                return;
            }
            res.status(204).send();
        }
        catch (e) {
            fail(res, 'snooze_delete_failed', e);
        }
    });
    // Inbox assignments
    router.get('/inbox/assignments', fgaViewer, async (req, res) => {
        ...
        if (!c)
            return;
        try {
            res.json({ assignments: await m.listAssignments(c.tenantId, c.userId) });
        }
        catch (e) {
    ...
    router.post('/inbox/assignments', fgaEditor, async (req, res) => {
        ...
        try {
            res.status(201).json(await m.createAssignment(c.tenantId, c.userId, p.data));
        }
        catch (e) {
            fail(res, 'assignment_create_failed', e);
        }
    });
