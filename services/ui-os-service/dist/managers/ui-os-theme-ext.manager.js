...
export class UiOsThemeExtManager {
    ...
    async listProfiles(tenantId) {
        const { rows } = await this.pool.query(`SELECT id::text, profile_code, parent_profile_id::text AS parent_profile_id,
    ...
    async upsertProfile(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_theme_profiles
       ...
       RETURNING id::text, profile_code, is_default, is_active`, [tenantId, body.profile_code, body.parent_profile_id ?? null,
    ...
    async deleteProfile(tenantId, profileId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_theme_profiles WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, profileId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Theme tokens ────────────────────────────────────────────
    async listTokens(tenantId, profileId) {
        const { rows } = await this.pool.query(`SELECT id::text, token_key, token_kind::text AS token_kind, token_value, is_active
         FROM dos.ui_theme_tokens
        WHERE tenant_id=$1 AND theme_profile_id=$2::uuid
        ORDER BY token_kind, token_key`, [tenantId, profileId]);
        return rows;
    }
    async upsertToken(tenantId, profileId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_theme_tokens
       ...
       RETURNING id::text, token_key, token_kind::text AS token_kind`, [tenantId, profileId, body.token_key, body.token_kind, body.token_value, body.is_active ?? null]);
        return rows[0];
    }
    async deleteToken(tenantId, tokenId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_theme_tokens WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, tokenId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Theme assignments ───────────────────────────────────────
    async listAssignments(tenantId, targetKind) {
        const { rows } = await this.pool.query(`SELECT a.id::text, a.target_kind::text AS target_kind, a.target_id,
        ...
        ORDER BY a.target_kind, a.target_id`, [tenantId, targetKind ?? null]);
        return rows;
    }
    async upsertAssignment(tenantId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_theme_assignments
       ...
       ON CONFLICT (tenant_id, target_kind, target_id) DO UPDATE
         SET theme_profile_id=EXCLUDED.theme_profile_id,
             is_active=EXCLUDED.is_active, updated_at=NOW()
       RETURNING id::text, target_kind::text AS target_kind, target_id`, [tenantId, body.target_kind, body.target_id, body.theme_profile_id, body.is_active ?? null]);
        return rows[0];
    }
    async deleteAssignment(tenantId, assignmentId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_theme_assignments WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, assignmentId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Brand assets ────────────────────────────────────────────
    async listAssets(tenantId, kind) {
        const { rows } = await this.pool.query(`SELECT id::text, asset_kind::text AS asset_kind, variant, url, mime_type,
    ...
    async upsertAsset(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_brand_assets
       ...
       ON CONFLICT (tenant_id, asset_kind, variant) DO UPDATE
         SET url=EXCLUDED.url, mime_type=EXCLUDED.mime_type,
             ...
             is_active=EXCLUDED.is_active,
             updated_by=EXCLUDED.updated_by, updated_at=NOW()
       RETURNING id::text, asset_kind::text AS asset_kind, variant, url`, [tenantId, body.asset_kind, body.variant ?? null, body.url, body.mime_type ?? null,
            body.width_px ?? null, body.height_px ?? null, body.size_bytes ?? null,
            body.checksum ?? null, body.is_active ?? null, userId]);
        return rows[0];
    }
    async deleteAsset(tenantId, assetId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_brand_assets WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, assetId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Login branding ──────────────────────────────────────────
    async getLoginBranding(tenantId) {
        const { rows } = await this.pool.query(`SELECT id::text, hero_asset_id::text AS hero_asset_id,
    ...
    async upsertLoginBranding(tenantId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_login_branding
       ...
       RETURNING id::text, is_active`, [tenantId, body.hero_asset_id ?? null, body.logo_asset_id ?? null,
    ...
    // ── Email branding ──────────────────────────────────────────
    async getEmailBranding(tenantId) {
        const { rows } = await this.pool.query(`SELECT id::text, header_html, footer_html, accent_color,
              logo_asset_id::text AS logo_asset_id, config, is_active
         FROM dos.ui_email_branding WHERE tenant_id=$1 LIMIT 1`, [tenantId]);
        return rows[0] ?? null;
    }
    async upsertEmailBranding(tenantId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_email_branding
       ...
       ON CONFLICT (tenant_id) DO UPDATE
         SET header_html=EXCLUDED.header_html, footer_html=EXCLUDED.footer_html,
             ...
             config=EXCLUDED.config, is_active=EXCLUDED.is_active, updated_at=NOW()
       RETURNING id::text, is_active`, [tenantId, body.header_html ?? null, body.footer_html ?? null,
            body.accent_color ?? null, body.logo_asset_id ?? null,
            body.config !== undefined ? JSON.stringify(body.config) : null, body.is_active ?? null]);
        return rows[0];
    }
    // ── Report branding ─────────────────────────────────────────
    async getReportBranding(tenantId) {
        const { rows } = await this.pool.query(`SELECT id::text, cover_template_html, watermark_text,
    ...
    async upsertReportBranding(tenantId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_report_branding
       ...
       ON CONFLICT (tenant_id) DO UPDATE
         SET cover_template_html=EXCLUDED.cover_template_html,
             ...
             config=EXCLUDED.config, is_active=EXCLUDED.is_active, updated_at=NOW()
       RETURNING id::text, is_active`, [tenantId, body.cover_template_html ?? null, body.watermark_text ?? null,
            body.watermark_asset_id ?? null, body.legal_footer ?? null,
            body.config !== undefined ? JSON.stringify(body.config) : null, body.is_active ?? null]);
        return rows[0];
    }
    // ── Print templates ─────────────────────────────────────────
    async listPrintTemplates(tenantId) {
        const { rows } = await this.pool.query(`SELECT id::text, template_key, engine::text AS engine, default_locale, is_active
         FROM dos.ui_print_templates WHERE tenant_id=$1
        ORDER BY template_key, default_locale`, [tenantId]);
        return rows;
    }
    async getPrintTemplate(tenantId, templateId) {
        const { rows } = await this.pool.query(`SELECT id::text, template_key, engine::text AS engine, template_body,
              default_locale, is_active
         FROM dos.ui_print_templates WHERE tenant_id=$1 AND id=$2::uuid LIMIT 1`, [tenantId, templateId]);
        return rows[0] ?? null;
    }
    async upsertPrintTemplate(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_print_templates
       ...
       ON CONFLICT (tenant_id, template_key, default_locale) DO UPDATE
         SET engine=EXCLUDED.engine, template_body=EXCLUDED.template_body,
             is_active=EXCLUDED.is_active,
             updated_by=EXCLUDED.updated_by, updated_at=NOW()
       RETURNING id::text, template_key, engine::text AS engine, default_locale`, [tenantId, body.template_key, body.engine ?? null, body.template_body,
            body.default_locale ?? null, body.is_active ?? null, userId]);
        return rows[0];
    }
    async deletePrintTemplate(tenantId, templateId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_print_templates WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, templateId]);
        return (r.rowCount ?? 0) > 0;
    }
}
//# sourceMappingURL=ui-os-theme-ext.manager.js.map