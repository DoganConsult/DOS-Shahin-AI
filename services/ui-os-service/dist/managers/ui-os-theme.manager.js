export class UiOsThemeManager {
    pool;
    constructor(pool) {
        this.pool = pool;
    }
    async get(tenantId, moduleCode) {
        const { rows } = await this.pool.query(`SELECT id::text AS id, module_code, token_key, token_value, scope, route, is_active
         FROM dos.dynamic_ui_theme_tokens
        WHERE is_active = TRUE
          AND (tenant_id IS NULL OR tenant_id = $1)
          AND ($2::text IS NULL OR module_code = $2 OR module_code IS NULL)
        ORDER BY scope, module_code NULLS FIRST, token_key`, [tenantId, moduleCode ?? null]);
        return { tokens: rows };
    }
    async upsert(tenantId, patch) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            for (const t of patch.tokens) {
                await client.query(`INSERT INTO dos.dynamic_ui_theme_tokens
             (tenant_id, module_code, token_key, token_value, scope, route, is_active)
           VALUES ($1,$2,$3,$4,$5,$6,TRUE)
           ON CONFLICT DO NOTHING`, [tenantId, t.module_code ?? null, t.token_key, t.token_value, t.scope ?? 'tenant', t.route ?? null]);
                await client.query(`UPDATE dos.dynamic_ui_theme_tokens
              SET token_value = $4, is_active = TRUE
            WHERE tenant_id = $1
              AND COALESCE(module_code,'') = COALESCE($2,'')
              AND token_key = $3`, [tenantId, t.module_code ?? null, t.token_key, t.token_value]);
            }
            await client.query('COMMIT');
        }
        catch (e) {
            await client.query('ROLLBACK');
            throw e;
        }
        finally {
            client.release();
        }
        return this.get(tenantId);
    }
    async preview(tenantId, patch) {
        const base = await this.get(tenantId);
        const map = new Map();
        for (const t of base.tokens)
            map.set(`${t.module_code ?? ''}:${t.token_key}`, t);
        for (const t of patch.tokens) {
            const k = `${t.module_code ?? ''}:${t.token_key}`;
            const existing = map.get(k);
            map.set(k, {
                id: existing?.id ?? 'preview',
                module_code: t.module_code ?? null,
                token_key: t.token_key,
                token_value: t.token_value,
                scope: t.scope ?? existing?.scope ?? 'tenant',
                route: t.route ?? existing?.route ?? null,
                is_active: true,
            });
        }
        return { tokens: Array.from(map.values()), preview: patch.tokens.map((t) => ({
                id: 'preview', module_code: t.module_code ?? null, token_key: t.token_key,
                token_value: t.token_value, scope: t.scope ?? 'tenant', route: t.route ?? null, is_active: true,
            })) };
    }
    async publish(tenantId, patch) {
        return this.upsert(tenantId, patch);
    }
}
//# sourceMappingURL=ui-os-theme.manager.js.map