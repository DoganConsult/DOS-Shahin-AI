...
export class UiOsGridStateManager {
    ...
    async get(tenantId, userId, gridKey) {
        const { rows } = await this.pool.query(`SELECT grid_key, module_code, route_key,
        ...
        LIMIT 1`, [tenantId, userId, gridKey]);
        if (rows.length === 0)
            return { ...EMPTY, grid_key: gridKey };
        return rows[0];
    }
    async upsert(tenantId, userId, gridKey, patch) {
        ...
        await this.pool.query(`INSERT INTO dos.ui_data_grid_states
    ...
    async reset(tenantId, userId, gridKey) {
        await this.pool.query(`DELETE FROM dos.ui_data_grid_states
        WHERE tenant_id = $1 AND user_id = $2 AND grid_key = $3`, [tenantId, userId, gridKey]);
        return { ...EMPTY, grid_key: gridKey };
    }
    async listViews(tenantId, userId, gridKey) {
        const { rows } = await this.pool.query(`SELECT id::text AS id, view_key, name, description,
        ...
        WHERE tenant_id = $1
          ...
          AND view_key LIKE $3 || ':%'
        ORDER BY is_default DESC, name`, [tenantId, userId, gridKey]);
        return rows;
    }
    async createView(tenantId, userId, gridKey, body) {
...
//# sourceMappingURL=ui-os-grid-state.manager.js.map