...
export class UiOsFormManager {
    ...
    async listDefinitions(tenantId, filter) {
        const { rows } = await this.pool.query(`SELECT id::text, tenant_id, form_key, version, module_code, product_code,
        ...
        ORDER BY form_key, version DESC`, [tenantId, filter?.moduleCode ?? null, filter?.productCode ?? null]);
        return rows;
    }
    async getDefinition(tenantId, formKey, version) {
        const { rows } = await this.pool.query(`SELECT id::text, tenant_id, form_key, version, module_code, product_code,
              entity_kind, submit_workflow_code, title_key, description_key, is_active
         FROM dos.ui_form_definitions
        WHERE tenant_id=$1 AND form_key=$2
          AND ($3::int IS NULL OR version=$3)
        ORDER BY version DESC LIMIT 1`, [tenantId, formKey, version ?? null]);
        return rows[0] ?? null;
    }
    async createDefinition(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_definitions
       ...
       RETURNING id::text, form_key, version, is_active`, [tenantId, body.form_key, body.version ?? null, body.module_code ?? null,
    ...
    async updateDefinition(tenantId, formId, userId, patch) {
        const { rows } = await this.pool.query(`UPDATE dos.ui_form_definitions
        ...
        RETURNING id::text, form_key, version, is_active`, [tenantId, formId, patch.module_code ?? null, patch.product_code ?? null,
    ...
    async deleteDefinition(tenantId, formId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_definitions WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, formId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Sections ────────────────────────────────────────────────
    async listSections(tenantId, formId) {
        const { rows } = await this.pool.query(`SELECT id::text, section_key, display_order, title_key, description_key,
              is_collapsible, default_collapsed, is_active
         FROM dos.ui_form_sections
        WHERE tenant_id=$1 AND form_definition_id=$2::uuid
        ORDER BY display_order, section_key`, [tenantId, formId]);
        return rows;
    }
    async upsertSection(tenantId, formId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_sections
       ...
       ON CONFLICT (form_definition_id, section_key) DO UPDATE
         SET display_order=EXCLUDED.display_order,
             ...
             updated_at=NOW()
       RETURNING id::text, section_key, display_order`, [tenantId, formId, body.section_key, body.display_order ?? null,
    ...
    async deleteSection(tenantId, sectionId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_sections WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, sectionId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Fields ──────────────────────────────────────────────────
    async listFields(tenantId, sectionId) {
        const { rows } = await this.pool.query(`SELECT id::text, field_key, field_kind::text AS field_kind, display_order,
        ...
        ORDER BY display_order, field_key`, [tenantId, sectionId]);
        return rows;
    }
    async upsertField(tenantId, sectionId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_fields
       ...
       ON CONFLICT (form_section_id, field_key) DO UPDATE
         SET field_kind=EXCLUDED.field_kind,
             ...
             updated_at=NOW()
       RETURNING id::text, field_key, field_kind::text AS field_kind`, [tenantId, sectionId, body.field_key, body.field_kind, body.display_order ?? null,
    ...
    async deleteField(tenantId, fieldId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_fields WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, fieldId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Field rules ─────────────────────────────────────────────
    async listFieldRules(tenantId, fieldId) {
        const { rows } = await this.pool.query(`SELECT id::text, rule_kind::text AS rule_kind, expression, message_key, priority, is_active
         FROM dos.ui_form_field_rules
        WHERE tenant_id=$1 AND form_field_id=$2::uuid
        ORDER BY priority, rule_kind`, [tenantId, fieldId]);
        return rows;
    }
    async createFieldRule(tenantId, userId, fieldId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_field_rules
       ...
       RETURNING id::text, rule_kind::text AS rule_kind, priority`, [tenantId, fieldId, body.rule_kind,
            body.expression !== undefined ? JSON.stringify(body.expression) : null,
            body.message_key ?? null, body.priority ?? null, userId]);
        return rows[0];
    }
    async deleteFieldRule(tenantId, ruleId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_field_rules WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, ruleId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Validation rules ────────────────────────────────────────
    async listValidationRules(tenantId, fieldId) {
        const { rows } = await this.pool.query(`SELECT id::text, validator_kind::text AS validator_kind, params, message_key, is_active
         FROM dos.ui_form_validation_rules
        WHERE tenant_id=$1 AND form_field_id=$2::uuid
        ORDER BY validator_kind`, [tenantId, fieldId]);
        return rows;
    }
    async upsertValidationRule(tenantId, fieldId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_validation_rules
       ...
       RETURNING id::text, validator_kind::text AS validator_kind`, [tenantId, fieldId, body.validator_kind,
            body.params !== undefined ? JSON.stringify(body.params) : null,
            body.message_key ?? null]);
        return rows[0];
    }
    async deleteValidationRule(tenantId, ruleId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_validation_rules WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, ruleId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Default values ──────────────────────────────────────────
    async getDefaultValue(tenantId, fieldId) {
        const { rows } = await this.pool.query(`SELECT id::text, default_kind::text AS default_kind, value, expression, is_active
         FROM dos.ui_form_default_values
        WHERE tenant_id=$1 AND form_field_id=$2::uuid LIMIT 1`, [tenantId, fieldId]);
        return rows[0] ?? null;
    }
    async upsertDefaultValue(tenantId, fieldId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_default_values
       ...
       ON CONFLICT (form_field_id) DO UPDATE
         SET default_kind=EXCLUDED.default_kind,
             ...
             updated_at=NOW()
       RETURNING id::text, default_kind::text AS default_kind`, [tenantId, fieldId, body.default_kind,
            body.value !== undefined ? JSON.stringify(body.value) : null,
            body.expression !== undefined ? JSON.stringify(body.expression) : null]);
        return rows[0];
    }
    // ── Submissions ─────────────────────────────────────────────
    async listSubmissions(tenantId, filter) {
        const { rows } = await this.pool.query(`SELECT id::text, form_definition_id::text AS form_definition_id,
        ...
        ORDER BY created_at DESC LIMIT 500`, [tenantId, filter.formId ?? null, filter.submitterUserId ?? null,
    ...
    async getSubmission(tenantId, submissionId) {
        const { rows } = await this.pool.query(`SELECT id::text, form_definition_id::text AS form_definition_id,
         ...
         FROM dos.ui_form_submissions
        WHERE tenant_id=$1 AND id=$2::uuid LIMIT 1`, [tenantId, submissionId]);
        return rows[0] ?? null;
    }
    async createSubmission(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_submissions
       ...
       RETURNING id::text, status::text AS status, submitted_at`, [tenantId, body.form_definition_id, userId, body.entity_kind ?? null,
            body.entity_id ?? null,
    ...
    async transitionSubmission(tenantId, submissionId, status) {
            ...
            : status === 'approved' ? 'approved_at'
                : status === 'rejected' ? 'rejected_at'
                    : status === 'withdrawn' ? 'withdrawn_at'
                        : null;
        const { rows } = await this.pool.query(`UPDATE dos.ui_form_submissions
          SET status=$3::dos.ui_form_submission_status_t,
              ${stamp ? `${stamp}=NOW(),` : ''}
              updated_at=NOW()
        WHERE tenant_id=$1 AND id=$2::uuid
        RETURNING id::text, status::text AS status, submitted_at, approved_at, rejected_at, withdrawn_at`, [tenantId, submissionId, status]);
        return rows[0] ?? null;
    }
    // ── Drafts ──────────────────────────────────────────────────
    async getDraft(tenantId, userId, formId, entityId) {
        const { rows } = await this.pool.query(`SELECT id::text, form_definition_id::text AS form_definition_id,
        ...
        LIMIT 1`, [tenantId, userId, formId, entityId]);
        return rows[0] ?? null;
    }
    async upsertDraft(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_drafts
       ...
       ON CONFLICT (tenant_id, form_definition_id, user_id, entity_id) DO UPDATE
         SET payload=EXCLUDED.payload,
             ...
             updated_at=NOW()
       RETURNING id::text, last_autosave_at`, [tenantId, body.form_definition_id, userId, body.entity_kind ?? null,
            body.entity_id ?? null,
            body.payload !== undefined ? JSON.stringify(body.payload) : null]);
        return rows[0];
    }
    async deleteDraft(tenantId, draftId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_drafts WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, draftId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Attachments ─────────────────────────────────────────────
    async listAttachments(tenantId, owner) {
        const { rows } = await this.pool.query(`SELECT id::text, form_submission_id::text AS form_submission_id,
        ...
        ORDER BY created_at DESC LIMIT 200`, [tenantId, owner.submissionId ?? null, owner.draftId ?? null]);
        return rows;
    }
    async createAttachment(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_attachments
       ...
       RETURNING id::text, file_name, size_bytes`, [tenantId, body.form_submission_id ?? null, body.form_draft_id ?? null,
    ...
    async deleteAttachment(tenantId, attachmentId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_form_attachments WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, attachmentId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── Approval links ──────────────────────────────────────────
    async listApprovalLinks(tenantId, submissionId) {
        const { rows } = await this.pool.query(`SELECT id::text, workflow_run_id::text AS workflow_run_id,
        ...
        ORDER BY created_at`, [tenantId, submissionId]);
        return rows;
    }
    async upsertApprovalLink(tenantId, submissionId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_form_approval_links
       ...
       RETURNING id::text, approval_step_code, decision::text AS decision`, [tenantId, submissionId, body.workflow_run_id ?? null,
            body.approval_step_code, body.decision ?? null]);
        return rows[0];
    }
    async decideApproval(tenantId, linkId, userId, decision, comments) {
        const { rows } = await this.pool.query(`UPDATE dos.ui_form_approval_links
          SET decision=$3::dos.ui_form_decision_t,
              decided_by=$4, decided_at=NOW(),
              comments=COALESCE($5, comments),
              updated_at=NOW()
        WHERE tenant_id=$1 AND id=$2::uuid
        RETURNING id::text, decision::text AS decision, decided_by, decided_at`, [tenantId, linkId, decision, userId, comments ?? null]);
        return rows[0] ?? null;
    }
}
//# sourceMappingURL=ui-os-form.manager.js.map