...
export class UiOsGridExtManager {
    ...
    // ── column catalog ──────────────────────────────────────────
    async listColumns(tenantId, gridKey) {
        const { rows } = await this.pool.query(`SELECT id::text, tenant_id, grid_key, column_key,
        ...
        ORDER BY display_order, column_key`, [tenantId, gridKey]);
        return rows;
    }
    async upsertColumn(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_column_catalog
       ...
       ON CONFLICT (tenant_id, grid_key, column_key) DO UPDATE
         SET data_type=EXCLUDED.data_type,
             ...
             updated_at=NOW()
       RETURNING id::text, column_key, data_type::text AS data_type, display_order, is_active`, [tenantId, body.grid_key, body.column_key, body.data_type, body.cell_renderer_key ?? null,
    ...
    async deleteColumn(tenantId, columnId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_data_grid_column_catalog WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, columnId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── column permissions ─────────────────────────────────────
    async listColumnPermissions(tenantId, columnId) {
        const { rows } = await this.pool.query(`SELECT id::text, permission_code, effect::text AS effect, capability::text AS capability,
              role_code, user_id, is_active
         FROM dos.ui_data_grid_column_permissions
        WHERE tenant_id=$1 AND column_catalog_id=$2::uuid
        ORDER BY permission_code`, [tenantId, columnId]);
        return rows;
    }
    async upsertColumnPermission(tenantId, userId, columnId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_column_permissions
       ...
       RETURNING id::text, permission_code, effect::text AS effect, capability::text AS capability, role_code, user_id`, [tenantId, columnId, body.permission_code, body.effect, body.capability ?? null,
            body.role_code ?? null, body.user_id ?? null, userId]);
        return rows[0];
    }
    async deleteColumnPermission(tenantId, permissionId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_data_grid_column_permissions WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, permissionId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── grid-scoped saved views ────────────────────────────────
    async listSavedViews(tenantId, gridKey, userId) {
        const { rows } = await this.pool.query(`SELECT id::text, grid_key, view_key, user_id, name_key, view_config,
              is_shared, is_default, is_active
         FROM dos.ui_data_grid_saved_views
        WHERE tenant_id=$1 AND grid_key=$2
          AND (user_id IS NULL OR user_id=$3 OR is_shared=TRUE)
        ORDER BY is_default DESC, view_key`, [tenantId, gridKey, userId ?? null]);
        return rows;
    }
    async upsertSavedView(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_saved_views
       ...
       RETURNING id::text, view_key, is_shared, is_default`, [tenantId, body.grid_key, body.view_key, body.user_id ?? null, body.name_key ?? null,
            body.view_config !== undefined ? JSON.stringify(body.view_config) : null,
            body.is_shared ?? null, body.is_default ?? null, userId]);
        return rows[0];
    }
    async deleteSavedView(tenantId, viewId) {
        const r = await this.pool.query(`DELETE FROM dos.ui_data_grid_saved_views WHERE tenant_id=$1 AND id=$2::uuid`, [tenantId, viewId]);
        return (r.rowCount ?? 0) > 0;
    }
    // ── exports ────────────────────────────────────────────────
    async listExports(tenantId, userId) {
        const { rows } = await this.pool.query(`SELECT id::text, grid_key, view_id::text AS view_id, format::text AS format,
        ...
        ORDER BY created_at DESC LIMIT 200`, [tenantId, userId]);
        return rows;
    }
    async createExport(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_exports
       ...
       RETURNING id::text, grid_key, format::text AS format, status::text AS status, created_at`, [tenantId, userId, body.grid_key, body.view_id ?? null, body.format,
            body.filters !== undefined ? JSON.stringify(body.filters) : null]);
        return rows[0];
    }
    // ── bulk jobs ──────────────────────────────────────────────
    async listBulkJobs(tenantId, userId) {
        const { rows } = await this.pool.query(`SELECT id::text, grid_key, action_kind, status::text AS status,
    ...
    async createBulkJob(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_bulk_jobs
        (tenant_id, user_id, grid_key, action_kind, targets, payload)
       VALUES ($1,$2,$3,$4,COALESCE($5::jsonb,'[]'::jsonb),COALESCE($6::jsonb,'{}'::jsonb))
       RETURNING id::text, grid_key, action_kind, status::text AS status, progress, created_at`, [tenantId, userId, body.grid_key, body.action_kind,
    ...
    async getInlineEditSession(tenantId, sessionKey) {
        const { rows } = await this.pool.query(`SELECT id::text, session_key, grid_key, row_pk, draft_payload, expires_at, is_active
         FROM dos.ui_data_grid_inline_edit_sessions
        WHERE tenant_id=$1 AND session_key=$2 LIMIT 1`, [tenantId, sessionKey]);
        return rows[0] ?? null;
    }
    async upsertInlineEditSession(tenantId, userId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_inline_edit_sessions
       ...
       RETURNING id::text, session_key, grid_key, expires_at, is_active`, [tenantId, userId, body.session_key, body.grid_key,
    ...
    async listValidationErrors(tenantId, sessionId) {
        const { rows } = await this.pool.query(`SELECT id::text, column_key, error_code, message_key, details, created_at
         FROM dos.ui_data_grid_validation_errors
        WHERE tenant_id=$1 AND inline_edit_session_id=$2::uuid
        ORDER BY column_key, error_code`, [tenantId, sessionId]);
        return rows;
    }
    async upsertValidationError(tenantId, sessionId, body) {
        const { rows } = await this.pool.query(`INSERT INTO dos.ui_data_grid_validation_errors
       ...
       RETURNING id::text, column_key, error_code`, [tenantId, sessionId, body.column_key, body.error_code, body.message_key ?? null,
            body.details !== undefined ? JSON.stringify(body.details) : null]);
        return rows[0];
    }
}
//# sourceMappingURL=ui-os-grid-ext.manager.js.map