...
export class UiOsManagerStudioManager {
  ...
  // ── Projects ────────────────────────────────────────────────
  async listProjects(tenantId: string, activeOnly = false) {
    const { rows } = await this.pool.query(
      `SELECT id::text, project_key, name, description, owner_user_id, is_active,
  ...
  async upsertProject(tenantId: string, ownerUserId: string, body: any) {
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_projects
       ...
       RETURNING id::text, project_key, name, is_active`,
      [tenantId, body.project_key, body.name, body.description ?? null,
       ownerUserId, body.is_active ?? null]);
    return rows[0];
  }
  async deleteProject(tenantId: string, projectId: string) {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_manager_projects WHERE tenant_id=$1 AND id=$2::uuid`,
      [tenantId, projectId]);
    return (r.rowCount ?? 0) > 0;
  }

  // ── Drafts ──────────────────────────────────────────────────
  async listDrafts(tenantId: string, opts: { projectId?: string | null; targetKind?: string | null; targetId?: string | null; state?: string | null }) {
    const { rows } = await this.pool.query(
      `SELECT id::text, project_id::text AS project_id,
      ...
      [tenantId, opts.projectId ?? null, opts.targetKind ?? null,
  ...
  async createDraft(tenantId: string, authorId: string, body: any) {
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_drafts
       ...
       RETURNING id::text, state::text AS state, created_at`,
      [tenantId, body.project_id, body.target_kind, body.target_id,
       body.payload !== undefined ? JSON.stringify(body.payload) : null,
       body.state ?? null, authorId]);
    return rows[0];
  }
  async updateDraft(tenantId: string, draftId: string, body: any) {
    const { rows } = await this.pool.query(
      `UPDATE dos.ui_manager_drafts
  ...
  async deleteDraft(tenantId: string, draftId: string) {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_manager_drafts WHERE tenant_id=$1 AND id=$2::uuid`,
      [tenantId, draftId]);
    return (r.rowCount ?? 0) > 0;
  }

  // ── Locks ───────────────────────────────────────────────────
  async listActiveLocks(tenantId: string, opts: { targetKind?: string | null; targetId?: string | null }) {
    const { rows } = await this.pool.query(
  ...
  async acquireLock(tenantId: string, userId: string, body: any) {
    const ttlMin = Math.min(Math.max(body.ttl_minutes ?? 15, 1), 240);
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_locks
       ...
       RETURNING id::text, locked_at, expires_at`,
      [tenantId, body.target_kind, body.target_id, userId, ttlMin]);
    return rows[0];
  }
  async releaseLock(tenantId: string, lockId: string, userId: string) {
    const { rows } = await this.pool.query(
      `UPDATE dos.ui_manager_locks
          SET released_at=NOW()
        WHERE tenant_id=$1 AND id=$2::uuid AND locked_by_user_id=$3 AND released_at IS NULL
       RETURNING id::text, released_at`,
      [tenantId, lockId, userId]);
    return rows[0] ?? null;
  }

  // ── Review comments ─────────────────────────────────────────
  async listComments(tenantId: string, draftId: string) {
    const { rows } = await this.pool.query(
      `SELECT id::text, author_id, body, resolved, resolved_by, resolved_at, created_at
         FROM dos.ui_manager_review_comments
        WHERE tenant_id=$1 AND draft_id=$2::uuid
        ORDER BY created_at DESC`, [tenantId, draftId]);
    return rows;
  }
  async addComment(tenantId: string, draftId: string, authorId: string, body: any) {
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_review_comments
        (tenant_id, draft_id, author_id, body)
       VALUES ($1,$2::uuid,$3,$4)
       RETURNING id::text, created_at`,
      [tenantId, draftId, authorId, body.body]);
    return rows[0];
  }
  async resolveComment(tenantId: string, commentId: string, userId: string) {
    const { rows } = await this.pool.query(
      `UPDATE dos.ui_manager_review_comments
          SET resolved=TRUE, resolved_by=$3, resolved_at=NOW(), updated_at=NOW()
        WHERE tenant_id=$1 AND id=$2::uuid AND resolved=FALSE
       RETURNING id::text, resolved, resolved_at`,
      [tenantId, commentId, userId]);
    return rows[0] ?? null;
  }
  async deleteComment(tenantId: string, commentId: string) {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_manager_review_comments WHERE tenant_id=$1 AND id=$2::uuid`,
      [tenantId, commentId]);
    return (r.rowCount ?? 0) > 0;
  }

  // ── Validation runs ─────────────────────────────────────────
  async recordValidationRun(tenantId: string, body: any) {
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_validation_runs
        (tenant_id, project_id, kind, passed, findings, validator_version)
       VALUES ($1,$2::uuid,$3,$4,COALESCE($5::jsonb,'[]'::jsonb),$6)
       RETURNING id::text, passed, ran_at`,
      [tenantId, body.project_id, body.kind, body.passed,
       body.findings !== undefined ? JSON.stringify(body.findings) : null,
       body.validator_version ?? null]);
    return rows[0];
  }
  async listValidationRuns(tenantId: string, projectId: string, limit = 100) {
    const { rows } = await this.pool.query(
      `SELECT id::text, kind, passed, findings, validator_version, ran_at
         FROM dos.ui_manager_validation_runs
        WHERE tenant_id=$1 AND project_id=$2::uuid
        ORDER BY ran_at DESC LIMIT $3`,
      [tenantId, projectId, Math.min(limit, 500)]);
    return rows;
  }

  // ── Preview sessions ────────────────────────────────────────
  async createPreviewSession(tenantId: string, userId: string, body: any) {
    const ttlMin = Math.min(Math.max(body.ttl_minutes ?? 60, 1), 1440);
    const token = randomBytes(32).toString('hex').slice(0, 64);
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_preview_sessions
      ...
      [tenantId, body.project_id, token, userId, ttlMin]);
    return rows[0];
  }
  async listPreviewSessions(tenantId: string, projectId: string) {
    const { rows } = await this.pool.query(
      `SELECT id::text, token, created_by_user_id, expires_at, revoked_at, created_at
         FROM dos.ui_manager_preview_sessions
        WHERE tenant_id=$1 AND project_id=$2::uuid
        ORDER BY created_at DESC`, [tenantId, projectId]);
    return rows;
  }
  async revokePreviewSession(tenantId: string, sessionId: string) {
    const { rows } = await this.pool.query(
      `UPDATE dos.ui_manager_preview_sessions
          SET revoked_at=NOW()
  ...
  // ── Import jobs ─────────────────────────────────────────────
  async listImportJobs(tenantId: string, opts: { projectId?: string | null; status?: string | null; limit?: number }) {
    const { rows } = await this.pool.query(
      `SELECT id::text, project_id::text AS project_id, kind, source_url,
      ...
      [tenantId, opts.projectId ?? null, opts.status ?? null, Math.min(opts.limit ?? 100, 500)]);
    return rows;
  }
  async createImportJob(tenantId: string, createdBy: string, body: any) {
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_import_jobs
        (tenant_id, project_id, kind, source_url, created_by)
       VALUES ($1,$2::uuid,$3,$4,$5)
       RETURNING id::text, status::text AS status, created_at`,
      [tenantId, body.project_id ?? null, body.kind, body.source_url ?? null, createdBy]);
    return rows[0];
  }
  async updateImportJob(tenantId: string, jobId: string, body: any) {
    const { rows } = await this.pool.query(
      `UPDATE dos.ui_manager_import_jobs
          SET status=COALESCE($3::dos.ui_job_status_t, status),
              ...
              updated_at=NOW()
        WHERE tenant_id=$1 AND id=$2::uuid
       RETURNING id::text, status::text AS status, progress`,
      [tenantId, jobId, body.status ?? null, body.progress ?? null,
  ...
  // ── Export jobs ─────────────────────────────────────────────
  async listExportJobs(tenantId: string, opts: { projectId?: string | null; status?: string | null; limit?: number }) {
    const { rows } = await this.pool.query(
      `SELECT id::text, project_id::text AS project_id, kind, target_format,
  ...
  async createExportJob(tenantId: string, createdBy: string, body: any) {
    const { rows } = await this.pool.query(
      `INSERT INTO dos.ui_manager_export_jobs
        (tenant_id, project_id, kind, target_format, created_by)
       VALUES ($1,$2::uuid,$3,$4,$5)
       RETURNING id::text, status::text AS status, created_at`,
      [tenantId, body.project_id ?? null, body.kind, body.target_format, createdBy]);
    return rows[0];
  }
  async updateExportJob(tenantId: string, jobId: string, body: any) {
    const { rows } = await this.pool.query(
      `UPDATE dos.ui_manager_export_jobs
          SET status=COALESCE($3::dos.ui_job_status_t, status),
              progress=COALESCE($4, progress),
