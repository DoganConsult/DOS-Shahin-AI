...
/**
 * Wave 11a-§5 Widgets — extended manager covering the 9 §5 tables created
 * by migrations 20260502_0100/0101/0102:
 *
 *   ui_widget_instances
 ...
 *
 * The pre-existing UiOsWidgetManager continues to own the
 * dashboards-grid path (dos.ui_dashboard_widgets); this manager owns
 * the per-tenant catalog/binding/personalization graph.
 *
 * Every SELECT/UPDATE/DELETE scopes by tenant_id (enforced by the
 * ui-os-tenant-isolation-guard CI gate). Identity headers always come
 * from the gateway-origin verified principal — never req.body.
 */
...
export interface WidgetInstancePermission {
  id: string;
...
export interface WidgetInstanceRoleGrant {
  id: string;
  role_code: string;
  granted_at: string;
}
...
export class UiOsWidgetExtManager {
  ...
  // ── ui_widget_instances ─────────────────────────────────────────
  async listInstances(tenantId: string, filter?: { dashboardId?: string | null; pageLayoutId?: string | null }): Promise<WidgetInstance[]> {
    const { rows } = await this.pool.query<WidgetInstance>(
      `SELECT id::text AS id, tenant_id, instance_key,
      ...
      [tenantId, filter?.dashboardId ?? null, filter?.pageLayoutId ?? null],
    );
    return rows;
  }

  async getInstance(tenantId: string, instanceKey: string): Promise<WidgetInstance | null> {
    const { rows } = await this.pool.query<WidgetInstance>(
      `SELECT id::text AS id, tenant_id, instance_key,
              widget_catalog_id::text AS widget_catalog_id,
              dashboard_id::text AS dashboard_id,
              ...
              required_permission, is_active
         FROM dos.ui_widget_instances
        WHERE tenant_id = $1 AND instance_key = $2 LIMIT 1`,
      [tenantId, instanceKey],
    );
    return rows[0] ?? null;
  }

  async createInstance(tenantId: string, userId: string, body: WidgetInstanceCreate): Promise<WidgetInstance> {
    const { rows } = await this.pool.query<WidgetInstance>(
      `INSERT INTO dos.ui_widget_instances
  ...
  async updateInstance(tenantId: string, instanceKey: string, userId: string, patch: WidgetInstancePatch): Promise<WidgetInstance | null> {
    const { rows } = await this.pool.query<WidgetInstance>(
      `UPDATE dos.ui_widget_instances
  ...
  async deleteInstance(tenantId: string, instanceKey: string): Promise<boolean> {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_widget_instances
        WHERE tenant_id = $1 AND instance_key = $2`,
      [tenantId, instanceKey],
    );
  ...
  // ── ui_widget_instance_permissions ──────────────────────────────
  async listPermissions(tenantId: string, instanceId: string): Promise<WidgetInstancePermission[]> {
    const { rows } = await this.pool.query<WidgetInstancePermission>(
      `SELECT id::text AS id, permission_code, effect, role_code, user_id, notes, is_active
  ...
  async upsertPermission(tenantId: string, userId: string, instanceId: string, body: {
    permission_code: string;
    effect: 'allow' | 'deny';
    role_code?: string | null;
    user_id?: string | null;
    notes?: string | null;
  }): Promise<WidgetInstancePermission> {
    const { rows } = await this.pool.query<WidgetInstancePermission>(
      `INSERT INTO dos.ui_widget_instance_permissions
       ...
       DO UPDATE SET effect = EXCLUDED.effect,
                     ...
                     updated_at = NOW()
       RETURNING id::text AS id, permission_code, effect, role_code, user_id, notes, is_active`,
      [tenantId, instanceId, body.permission_code, body.effect,
  ...
  async deletePermission(tenantId: string, permissionId: string): Promise<boolean> {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_widget_instance_permissions
  ...
  // ── ui_widget_instance_role_grants ──────────────────────────────
  async listRoleGrants(tenantId: string, instanceId: string): Promise<WidgetInstanceRoleGrant[]> {
    const { rows } = await this.pool.query<WidgetInstanceRoleGrant>(
      `SELECT id::text AS id, role_code, granted_at::text AS granted_at
  ...
  async grantRole(tenantId: string, userId: string, instanceId: string, roleCode: string): Promise<WidgetInstanceRoleGrant> {
    const { rows } = await this.pool.query<WidgetInstanceRoleGrant>(
      `INSERT INTO dos.ui_widget_instance_role_grants
       ...
       RETURNING id::text AS id, role_code, granted_at::text AS granted_at`,
      [tenantId, instanceId, roleCode, userId],
    );
    return rows[0];
  }

  async revokeRole(tenantId: string, instanceId: string, roleCode: string): Promise<boolean> {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_widget_instance_role_grants
        WHERE tenant_id = $1 AND widget_instance_id = $2::uuid AND role_code = $3`,
      [tenantId, instanceId, roleCode],
    );
    return (r.rowCount ?? 0) > 0;
  }

  // ── ui_widget_data_bindings ─────────────────────────────────────
  async getBinding(tenantId: string, instanceId: string): Promise<WidgetDataBinding | null> {
    const { rows } = await this.pool.query<WidgetDataBinding>(
      `SELECT id::text AS id, widget_instance_id::text AS widget_instance_id,
        ...
        WHERE tenant_id = $1 AND widget_instance_id = $2::uuid LIMIT 1`,
      [tenantId, instanceId],
    );
    return rows[0] ?? null;
  }

  async upsertBinding(tenantId: string, userId: string, instanceId: string, body: {
    binding_kind: string;
    endpoint_url?: string | null;
    http_method?: string;
    request_template?: Record<string, unknown>;
    ...
    cache_ttl_seconds?: number;
    auth_secret_ref?: string | null;
  }): Promise<WidgetDataBinding> {
    const { rows } = await this.pool.query<WidgetDataBinding>(
      `INSERT INTO dos.ui_widget_data_bindings
       ...
       RETURNING id::text AS id, widget_instance_id::text AS widget_instance_id,
                 ...
                 request_template, response_mapping,
                 cache_ttl_seconds, auth_secret_ref, is_active`,
      [tenantId, instanceId, body.binding_kind, body.endpoint_url ?? null,
  ...
  async getRefreshPolicy(tenantId: string, instanceId: string): Promise<WidgetRefreshPolicy | null> {
    const { rows } = await this.pool.query<WidgetRefreshPolicy>(
      `SELECT p.id::text AS id, p.widget_instance_id::text AS widget_instance_id,
              p.interval_seconds,
              COALESCE(
        ...
        WHERE p.tenant_id = $1 AND p.widget_instance_id = $2::uuid LIMIT 1`,
      [tenantId, instanceId],
    );
    return rows[0] ?? null;
  }

  async upsertRefreshPolicy(tenantId: string, userId: string, instanceId: string, body: {
    interval_seconds?: number;
    refresh_on_event_codes?: string[];
    pause_when_hidden?: boolean;
    refresh_on_focus?: boolean;
  }): Promise<WidgetRefreshPolicy> {
    const client = await this.pool.connect();
    let policyId: string;
    try {
      await client.query('BEGIN');
      const ins = await client.query<{ id: string }>(
        `INSERT INTO dos.ui_widget_refresh_policies
        ...
        [tenantId, instanceId, body.interval_seconds ?? null,
      ...
      if (body.refresh_on_event_codes !== undefined) {
        await client.query(
          `DELETE FROM dos.ui_widget_refresh_policy_events WHERE policy_id = $1::uuid`,
          [policyId],
        );
        if (body.refresh_on_event_codes.length > 0) {
          await client.query(
            ...
            [policyId, body.refresh_on_event_codes],
          );
        }
      }
    ...
    if (!result) throw new Error('refresh_policy_upsert_inconsistency');
    return result;
  }

  // ── ui_widget_error_states ──────────────────────────────────────
  async listErrorStates(tenantId: string, instanceId: string): Promise<WidgetErrorState[]> {
    const { rows } = await this.pool.query<WidgetErrorState>(
      `SELECT id::text AS id, widget_instance_id::text AS widget_instance_id,
  ...
  async upsertErrorState(tenantId: string, userId: string, instanceId: string, body: {
    error_code: string;
    fallback_component_key?: string | null;
    message_key?: string | null;
    retry_strategy?: string;
    retry_max_attempts?: number;
  }): Promise<WidgetErrorState> {
    const { rows } = await this.pool.query<WidgetErrorState>(
      `INSERT INTO dos.ui_widget_error_states
       ...
       RETURNING id::text AS id, widget_instance_id::text AS widget_instance_id,
                 ...
                 retry_strategy::text AS retry_strategy,
                 retry_max_attempts, is_active`,
      [tenantId, instanceId, body.error_code, body.fallback_component_key ?? null,
  ...
  // ── ui_widget_visibility_rules ──────────────────────────────────
  async listVisibilityRules(tenantId: string, instanceId: string): Promise<WidgetVisibilityRule[]> {
    const { rows } = await this.pool.query<WidgetVisibilityRule>(
      `SELECT id::text AS id, widget_instance_id::text AS widget_instance_id,
  ...
  async createVisibilityRule(tenantId: string, userId: string, instanceId: string, body: {
    rule_kind: string;
    rule_payload?: Record<string, unknown>;
    effect?: 'show' | 'hide';
    priority?: number;
  }): Promise<WidgetVisibilityRule> {
    const { rows } = await this.pool.query<WidgetVisibilityRule>(
      `INSERT INTO dos.ui_widget_visibility_rules
       ...
       RETURNING id::text AS id, widget_instance_id::text AS widget_instance_id,
                 rule_kind::text AS rule_kind,
                 rule_payload, effect::text AS effect, priority, is_active`,
      [tenantId, instanceId, body.rule_kind,
  ...
  async deleteVisibilityRule(tenantId: string, ruleId: string): Promise<boolean> {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_widget_visibility_rules
        WHERE tenant_id = $1 AND id = $2::uuid`,
      [tenantId, ruleId],
    );
    return (r.rowCount ?? 0) > 0;
  ...
  // ── ui_widget_personalization ───────────────────────────────────
  async getPersonalization(tenantId: string, instanceId: string, userId: string): Promise<WidgetPersonalization | null> {
    const { rows } = await this.pool.query<WidgetPersonalization>(
      `SELECT id::text AS id, widget_instance_id::text AS widget_instance_id,
              user_id, personalization, is_collapsed, is_pinned,
              display_order, is_active
         FROM dos.ui_widget_personalization
        WHERE tenant_id = $1 AND widget_instance_id = $2::uuid AND user_id = $3 LIMIT 1`,
      [tenantId, instanceId, userId],
    );
    return rows[0] ?? null;
  }

  async upsertPersonalization(tenantId: string, userId: string, instanceId: string, body: {
    personalization?: Record<string, unknown>;
    is_collapsed?: boolean;
    is_pinned?: boolean;
    display_order?: number | null;
    ...
    const { rows } = await this.pool.query<WidgetPersonalization>(
      `INSERT INTO dos.ui_widget_personalization
  ...
  // ── ui_widget_catalog_categories ────────────────────────────────
  async listCategories(tenantId: string): Promise<WidgetCatalogCategory[]> {
    const { rows } = await this.pool.query<WidgetCatalogCategory>(
  ...
  async upsertCategory(tenantId: string, userId: string, body: {
    category_code: string;
    parent_category_id?: string | null;
    display_name_key?: string | null;
    description_key?: string | null;
    display_order?: number;
    icon_key?: string | null;
  }): Promise<WidgetCatalogCategory> {
    const { rows } = await this.pool.query<WidgetCatalogCategory>(
      `INSERT INTO dos.ui_widget_catalog_categories
       ...
       RETURNING id::text AS id, category_code,
                 ...
                 display_name_key, description_key, display_order,
                 icon_key, is_active`,
      [tenantId, body.category_code, body.parent_category_id ?? null,
  ...
  async deleteCategory(tenantId: string, categoryCode: string): Promise<boolean> {
    const r = await this.pool.query(
      `DELETE FROM dos.ui_widget_catalog_categories
