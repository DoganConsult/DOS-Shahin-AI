...
import {
  WidgetExtInstanceCreateSchema,
  WidgetExtInstancePatchSchema,
  WidgetExtPermissionSchema,
  WidgetExtRoleGrantSchema,
  WidgetExtBindingSchema,
  WidgetExtRefreshPolicySchema,
  WidgetExtErrorStateSchema,
  WidgetExtVisibilityRuleSchema,
  WidgetExtPersonalizationSchema,
  WidgetExtCategorySchema,
} from '../schemas/widget-ext.schemas.js';
...
function fail(res: Response, code: string, err: unknown, status = 500) {
  const msg = (err as Error).message;
  if (msg.includes('duplicate key')) { res.status(409).json({ error: 'conflict' }); return; }
  res.status(status).json({ error: code, message: msg });
}

export function createWidgetExtRouter(pool: DbPool): Router {
  const router = Router();
  ...
  router.get('/widgets/instances-ext', fgaViewer, async (req, res) => {
    ...
    try {
      const items = await m.listInstances(ctx.tenantId, {
        dashboardId: (req.query.dashboardId as string | undefined) ?? null,
        pageLayoutId: (req.query.pageLayoutId as string | undefined) ?? null,
      });
      res.json({ instances: items });
    } catch (e) { fail(res, 'instances_list_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceKey', fgaViewer, async (req, res) => {
    ...
    try {
      ...
      if (!inst) { res.status(404).json({ error: 'instance_not_found' }); return; }
      res.json(inst);
    } catch (e) { fail(res, 'instance_get_failed', e); }
  });
  ...
  router.post('/widgets/instances-ext', fgaEditor, async (req, res) => {
    ...
    try { res.status(201).json(await m.createInstance(ctx.tenantId, ctx.userId, parsed.data as unknown as Parameters<typeof m.createInstance>[2])); }
    catch (e) { fail(res, 'instance_create_failed', e); }
  });
  ...
  router.put('/widgets/instances-ext/:instanceKey', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      if (!inst) { res.status(404).json({ error: 'instance_not_found' }); return; }
      res.json(inst);
    } catch (e) { fail(res, 'instance_update_failed', e); }
  });
  ...
  router.delete('/widgets/instances-ext/:instanceKey', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'instance_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'instance_delete_failed', e); }
  });

  // ── Permissions ─────────────────────────────────────────────
  router.get('/widgets/instances-ext/:instanceId/permissions', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json({ permissions: await m.listPermissions(ctx.tenantId, req.params.instanceId) }); }
    catch (e) { fail(res, 'permissions_list_failed', e); }
  });
  ...
  router.post('/widgets/instances-ext/:instanceId/permissions', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.status(201).json(await m.upsertPermission(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data as unknown as Parameters<typeof m.upsertPermission>[3])); }
    catch (e) { fail(res, 'permission_upsert_failed', e); }
  });

  router.delete('/widgets/instances-ext/:instanceId/permissions/:permissionId', fgaEditor, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.permissionId)) { res.status(400).json({ error: 'invalid_permission_id' }); return; }
    try {
      const ok = await m.deletePermission(ctx.tenantId, req.params.permissionId);
      if (!ok) { res.status(404).json({ error: 'permission_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'permission_delete_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceId/roles', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json({ roles: await m.listRoleGrants(ctx.tenantId, req.params.instanceId) }); }
    catch (e) { fail(res, 'roles_list_failed', e); }
  });
  ...
  router.post('/widgets/instances-ext/:instanceId/roles', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.status(201).json(await m.grantRole(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data.role_code)); }
    catch (e) { fail(res, 'role_grant_failed', e); }
  });
  ...
  router.delete('/widgets/instances-ext/:instanceId/roles/:roleCode', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'role_grant_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'role_revoke_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceId/binding', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json(await m.getBinding(ctx.tenantId, req.params.instanceId) ?? null); }
    catch (e) { fail(res, 'binding_get_failed', e); }
  });
  ...
  router.put('/widgets/instances-ext/:instanceId/binding', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.json(await m.upsertBinding(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data as unknown as Parameters<typeof m.upsertBinding>[3])); }
    catch (e) { fail(res, 'binding_upsert_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceId/refresh', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json(await m.getRefreshPolicy(ctx.tenantId, req.params.instanceId) ?? null); }
    catch (e) { fail(res, 'refresh_get_failed', e); }
  });
  ...
  router.put('/widgets/instances-ext/:instanceId/refresh', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.json(await m.upsertRefreshPolicy(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data)); }
    catch (e) { fail(res, 'refresh_upsert_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceId/errors', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json({ errors: await m.listErrorStates(ctx.tenantId, req.params.instanceId) }); }
    catch (e) { fail(res, 'errors_list_failed', e); }
  });
  ...
  router.put('/widgets/instances-ext/:instanceId/errors', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.json(await m.upsertErrorState(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data as unknown as Parameters<typeof m.upsertErrorState>[3])); }
    catch (e) { fail(res, 'error_state_upsert_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceId/visibility', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json({ rules: await m.listVisibilityRules(ctx.tenantId, req.params.instanceId) }); }
    catch (e) { fail(res, 'visibility_list_failed', e); }
  });
  ...
  router.post('/widgets/instances-ext/:instanceId/visibility', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.status(201).json(await m.createVisibilityRule(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data as unknown as Parameters<typeof m.createVisibilityRule>[3])); }
    catch (e) { fail(res, 'visibility_create_failed', e); }
  });
  ...
  router.delete('/widgets/instances-ext/:instanceId/visibility/:ruleId', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      res.status(204).send();
    } catch (e) { fail(res, 'visibility_delete_failed', e); }
  });
  ...
  router.get('/widgets/instances-ext/:instanceId/personalization', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    if (!UUID.test(req.params.instanceId)) { res.status(400).json({ error: 'invalid_instance_id' }); return; }
    try { res.json(await m.getPersonalization(ctx.tenantId, req.params.instanceId, ctx.userId) ?? null); }
    catch (e) { fail(res, 'personalization_get_failed', e); }
  });
  ...
  router.put('/widgets/instances-ext/:instanceId/personalization', fgaViewer, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.json(await m.upsertPersonalization(ctx.tenantId, ctx.userId, req.params.instanceId, parsed.data)); }
    catch (e) { fail(res, 'personalization_upsert_failed', e); }
  });

  // ── Catalog categories ──────────────────────────────────────
  router.get('/widgets/categories', fgaViewer, async (req, res) => {
    const ctx = context(req, res); if (!ctx) return;
    try { res.json({ categories: await m.listCategories(ctx.tenantId) }); }
    catch (e) { fail(res, 'categories_list_failed', e); }
  });
  ...
  router.put('/widgets/categories', fgaEditor, async (req, res) => {
    ...
    if (!parsed.success) { res.status(400).json({ error: 'invalid_request', details: parsed.error.flatten() }); return; }
    try { res.json(await m.upsertCategory(ctx.tenantId, ctx.userId, parsed.data as unknown as Parameters<typeof m.upsertCategory>[2])); }
    catch (e) { fail(res, 'category_upsert_failed', e); }
  });
  ...
  router.delete('/widgets/categories/:categoryCode', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'category_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'category_delete_failed', e); }
  });
