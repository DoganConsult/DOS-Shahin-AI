...
import {
  ManagerProjectSchema, ManagerDraftCreateSchema, ManagerDraftUpdateSchema,
  ManagerLockAcquireSchema, ManagerCommentSchema, ManagerValidationRunSchema,
  ManagerPreviewSessionSchema, ManagerImportJobCreateSchema,
  ManagerExportJobCreateSchema, ManagerJobUpdateSchema,
} from '../schemas/manager-studio.schemas.js';
...
export function createManagerStudioRouter(pool: DbPool): Router {
  const router = Router();
  ...
  router.get('/manager/projects', fgaViewer, async (req, res) => {
    ...
    try { res.json({ projects: await m.listProjects(c.tenantId,
      String(req.query.activeOnly ?? '') === 'true') }); } catch (e) { fail(res, 'projects_list_failed', e); }
  });
  router.get('/manager/projects/:projectId', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.projectId)) { res.status(400).json({ error: 'invalid_project_id' }); return; }
    try {
      const p = await m.getProject(c.tenantId, req.params.projectId);
      if (!p) { res.status(404).json({ error: 'project_not_found' }); return; }
      res.json(p);
    } catch (e) { fail(res, 'project_get_failed', e); }
  });
  router.put('/manager/projects', fgaAdmin, async (req, res) => {
    ...
    try { res.json(await m.upsertProject(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'project_upsert_failed', e); }
  });
  router.delete('/manager/projects/:projectId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'project_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'project_delete_failed', e); }
  });

  // Drafts
  router.get('/manager/drafts', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ drafts: await m.listDrafts(c.tenantId, {
      projectId: (req.query.projectId as string | undefined) ?? null,
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
      state: (req.query.state as string | undefined) ?? null,
    }) }); } catch (e) { fail(res, 'drafts_list_failed', e); }
  });
  router.get('/manager/drafts/:draftId', fgaViewer, async (req, res) => {
    ...
    try {
      ...
      res.json(d);
    } catch (e) { fail(res, 'draft_get_failed', e); }
  });
  router.post('/manager/drafts', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ManagerDraftCreateSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.createDraft(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'draft_create_failed', e); }
  });
  router.patch('/manager/drafts/:draftId', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      res.json(r);
    } catch (e) { fail(res, 'draft_update_failed', e); }
  });
  ...
  router.get('/manager/locks', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ locks: await m.listActiveLocks(c.tenantId, {
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
    }) }); } catch (e) { fail(res, 'locks_list_failed', e); }
  });
  router.post('/manager/locks', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ManagerLockAcquireSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.acquireLock(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'lock_acquire_failed', e); }
  });
  router.post('/manager/locks/:lockId/release', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.lockId)) { res.status(400).json({ error: 'invalid_lock_id' }); return; }
    try {
      const r = await m.releaseLock(c.tenantId, req.params.lockId, c.userId);
      if (!r) { res.status(404).json({ error: 'lock_not_found_or_not_owner' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'lock_release_failed', e); }
  });
  ...
  router.get('/manager/drafts/:draftId/comments', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.draftId)) { res.status(400).json({ error: 'invalid_draft_id' }); return; }
    try { res.json({ comments: await m.listComments(c.tenantId, req.params.draftId) }); } catch (e) { fail(res, 'comments_list_failed', e); }
  });
  router.post('/manager/drafts/:draftId/comments', fgaEditor, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.addComment(c.tenantId, req.params.draftId, c.userId, p.data)); } catch (e) { fail(res, 'comment_add_failed', e); }
  });
  router.post('/manager/comments/:commentId/resolve', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.commentId)) { res.status(400).json({ error: 'invalid_comment_id' }); return; }
    try {
      const r = await m.resolveComment(c.tenantId, req.params.commentId, c.userId);
      if (!r) { res.status(404).json({ error: 'comment_not_found_or_already_resolved' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'comment_resolve_failed', e); }
  });
  router.delete('/manager/comments/:commentId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'comment_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'comment_delete_failed', e); }
  });
  ...
  router.post('/manager/validation-runs', fgaEditor, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.recordValidationRun(c.tenantId, p.data)); } catch (e) { fail(res, 'validation_run_record_failed', e); }
  });
  router.get('/manager/projects/:projectId/validation-runs', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.projectId)) { res.status(400).json({ error: 'invalid_project_id' }); return; }
    try { res.json({ runs: await m.listValidationRuns(c.tenantId, req.params.projectId,
      parseInt(String(req.query.limit ?? 100), 10)) }); } catch (e) { fail(res, 'validation_runs_list_failed', e); }
  });
  ...
  router.post('/manager/preview-sessions', fgaEditor, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.createPreviewSession(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'preview_session_create_failed', e); }
  });
  router.get('/manager/projects/:projectId/preview-sessions', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.projectId)) { res.status(400).json({ error: 'invalid_project_id' }); return; }
    try { res.json({ sessions: await m.listPreviewSessions(c.tenantId, req.params.projectId) }); } catch (e) { fail(res, 'preview_sessions_list_failed', e); }
  });
  router.post('/manager/preview-sessions/:sessionId/revoke', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.sessionId)) { res.status(400).json({ error: 'invalid_session_id' }); return; }
    try {
      const r = await m.revokePreviewSession(c.tenantId, req.params.sessionId);
      if (!r) { res.status(404).json({ error: 'session_not_found_or_already_revoked' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'preview_session_revoke_failed', e); }
  });

  // Import jobs
  router.get('/manager/import-jobs', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ jobs: await m.listImportJobs(c.tenantId, {
      projectId: (req.query.projectId as string | undefined) ?? null,
      status: (req.query.status as string | undefined) ?? null,
      limit: parseInt(String(req.query.limit ?? 100), 10),
    }) }); } catch (e) { fail(res, 'import_jobs_list_failed', e); }
  });
  router.post('/manager/import-jobs', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ManagerImportJobCreateSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.createImportJob(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'import_job_create_failed', e); }
  });
  router.patch('/manager/import-jobs/:jobId', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.jobId)) { res.status(400).json({ error: 'invalid_job_id' }); return; }
    const p = ManagerJobUpdateSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try {
      const r = await m.updateImportJob(c.tenantId, req.params.jobId, p.data);
      if (!r) { res.status(404).json({ error: 'job_not_found' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'import_job_update_failed', e); }
  });

  // Export jobs
  router.get('/manager/export-jobs', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ jobs: await m.listExportJobs(c.tenantId, {
      projectId: (req.query.projectId as string | undefined) ?? null,
      status: (req.query.status as string | undefined) ?? null,
      limit: parseInt(String(req.query.limit ?? 100), 10),
    }) }); } catch (e) { fail(res, 'export_jobs_list_failed', e); }
  });
  router.post('/manager/export-jobs', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ManagerExportJobCreateSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.createExportJob(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'export_job_create_failed', e); }
  });
  router.patch('/manager/export-jobs/:jobId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!r) { res.status(404).json({ error: 'job_not_found' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'export_job_update_failed', e); }
  });
