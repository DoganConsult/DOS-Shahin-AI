...
import {
  ChangeLogSchema, PublishRequestSchema, PublishRequestResolveSchema,
  PublishApprovalSchema, PublishedVersionSchema, DraftSchema,
  RollbackPointSchema, SchemaValidationSchema, ContractDriftSchema,
  AdminActivitySchema,
} from '../schemas/governance.schemas.js';
...
export function createGovernanceRouter(pool: DbPool): Router {
  const router = Router();
  ...
  router.post('/change-log', fgaEditor, async (req, res) => {
    ...
    try { res.status(201).json(await m.recordChange(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'change_log_record_failed', e); }
  });
  router.get('/change-log', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ entries: await m.listChangeLog(c.tenantId, {
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
      actorId: (req.query.actorId as string | undefined) ?? null,
      limit: parseInt(String(req.query.limit ?? 200), 10),
    }) }); } catch (e) { fail(res, 'change_log_list_failed', e); }
  });

  // Publish requests
  router.get('/publish-requests', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ requests: await m.listPublishRequests(c.tenantId, {
      status: (req.query.status as string | undefined) ?? null,
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
    }) }); } catch (e) { fail(res, 'publish_requests_list_failed', e); }
  });
  router.post('/publish-requests', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = PublishRequestSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.createPublishRequest(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'publish_request_create_failed', e); }
  });
  router.post('/publish-requests/:requestId/resolve', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.requestId)) { res.status(400).json({ error: 'invalid_request_id' }); return; }
    ...
    try {
      ...
      if (!r) { res.status(404).json({ error: 'request_not_found' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'publish_request_resolve_failed', e); }
  });
  ...
  router.get('/publish-requests/:requestId/approvals', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.requestId)) { res.status(400).json({ error: 'invalid_request_id' }); return; }
    try { res.json({ approvals: await m.listApprovals(c.tenantId, req.params.requestId) }); } catch (e) { fail(res, 'approvals_list_failed', e); }
  });
  router.put('/publish-requests/:requestId/approvals', fgaAdmin, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertApproval(c.tenantId, req.params.requestId, c.userId, p.data)); } catch (e) { fail(res, 'approval_upsert_failed', e); }
  });

  // Published versions
  router.get('/published-versions', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ versions: await m.listPublishedVersions(c.tenantId, {
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
      currentOnly: String(req.query.currentOnly ?? '') === 'true',
    }) }); } catch (e) { fail(res, 'published_versions_list_failed', e); }
  });
  router.get('/published-versions/:versionId', fgaViewer, async (req, res) => {
    ...
    try {
      ...
      if (!v) { res.status(404).json({ error: 'version_not_found' }); return; }
      res.json(v);
    } catch (e) { fail(res, 'published_version_get_failed', e); }
  });
  router.post('/published-versions', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = PublishedVersionSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.publishVersion(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'publish_version_failed', e); }
  });
  ...
  router.get('/rollback-points', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ points: await m.listRollbackPoints(c.tenantId, {
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
    }) }); } catch (e) { fail(res, 'rollback_points_list_failed', e); }
  });
  router.get('/rollback-points/:pointId', fgaViewer, async (req, res) => {
    ...
    try {
      ...
      res.json(r);
    } catch (e) { fail(res, 'rollback_point_get_failed', e); }
  });
  router.post('/rollback-points', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = RollbackPointSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.createRollbackPoint(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'rollback_point_create_failed', e); }
  });
  ...
  router.post('/schema-validations', fgaEditor, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.recordSchemaValidation(c.tenantId, p.data)); } catch (e) { fail(res, 'schema_validation_record_failed', e); }
  });
  router.get('/schema-validations', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ results: await m.listSchemaValidations(c.tenantId, {
    ...
    }) }); } catch (e) { fail(res, 'schema_validations_list_failed', e); }
  });
  ...
  router.post('/contract-drift', fgaEditor, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.recordDrift(c.tenantId, p.data)); } catch (e) { fail(res, 'drift_record_failed', e); }
  });
  router.get('/contract-drift', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ results: await m.listDrift(c.tenantId, {
      openOnly: String(req.query.openOnly ?? '') === 'true',
      limit: parseInt(String(req.query.limit ?? 100), 10),
    }) }); } catch (e) { fail(res, 'drift_list_failed', e); }
  });
  router.post('/contract-drift/:driftId/resolve', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    if (!UUID.test(req.params.driftId)) { res.status(400).json({ error: 'invalid_drift_id' }); return; }
    try {
      const r = await m.resolveDrift(c.tenantId, req.params.driftId);
      if (!r) { res.status(404).json({ error: 'drift_not_found' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'drift_resolve_failed', e); }
  });
  ...
  router.post('/admin-activity', fgaAdmin, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.recordAdminActivity(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'admin_activity_record_failed', e); }
  });
  router.get('/admin-activity', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ entries: await m.listAdminActivity(c.tenantId, {
      actorId: (req.query.actorId as string | undefined) ?? null,
      actionCode: (req.query.actionCode as string | undefined) ?? null,
      limit: parseInt(String(req.query.limit ?? 200), 10),
    }) }); } catch (e) { fail(res, 'admin_activity_list_failed', e); }
  });
