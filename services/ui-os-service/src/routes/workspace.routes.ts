...
import { UiOsWorkspaceStateManager, type WorkspacePatch } from '../managers/ui-os-workspace-state.manager.js';
...
function workspaceKey(req: Request): string {
  const wk = (req.query.workspaceKey ?? req.header('x-dos-workspace-key')) as string | undefined;
  return wk ?? 'default';
}

export function createWorkspaceRouter(pool: DbPool): Router {
  const router = Router();
  ...
  router.get('/workspace-state', async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: 'workspace_get_failed', message: (err as Error).message });
    }
  });
  ...
  router.put('/workspace-state', async (req, res) => {
    ...
    try {
      const wk = (parsed.data.workspace_key as string | undefined) ?? workspaceKey(req);
      const state = await manager.upsert(ctx.tenantId, ctx.userId, wk, parsed.data as WorkspacePatch);
      res.json(state);
    } catch (err) {
      res.status(500).json({ error: 'workspace_put_failed', message: (err as Error).message });
    }
  });
  ...
  router.post('/workspace-state/reset', async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: 'workspace_reset_failed', message: (err as Error).message });
    }
  });
  ...
  router.post('/workspace-state/snapshot', async (req, res) => {
    ...
    try {
      const wk = (parsed.data.workspace_key as string | undefined) ?? workspaceKey(req);
      const snap = await manager.snapshot(ctx.tenantId, ctx.userId, wk, parsed.data.snapshot_key as string);
      res.status(201).json(snap);
    } catch (err) {
      res.status(500).json({ error: 'workspace_snapshot_failed', message: (err as Error).message });
    }
  });
