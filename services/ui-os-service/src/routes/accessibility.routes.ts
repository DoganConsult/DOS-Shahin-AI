...
import {
  AccessibilitySchema, ReducedMotionSchema, ContrastSchema, FontScaleSchema,
  DevicePreferenceSchema, DeviceSessionSchema, ViewportProfileSchema,
} from '../schemas/accessibility.schemas.js';
...
export function createAccessibilityRouter(pool: DbPool): Router {
  const router = Router();
  ...
  router.get('/accessibility/preferences', fgaViewer, async (req, res) => {
    ...
    try { res.json(await m.getAccessibility(c.tenantId, c.userId) ?? {}); } catch (e) { fail(res, 'a11y_get_failed', e); }
  });
  router.put('/accessibility/preferences', fgaViewer, async (req, res) => {
    ...
    try { res.json(await m.upsertAccessibility(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'a11y_put_failed', e); }
  });

  // reduced motion
  router.get('/accessibility/reduced-motion', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json(await m.getReducedMotion(c.tenantId, c.userId) ?? {}); } catch (e) { fail(res, 'reduced_motion_get_failed', e); }
  });
  router.put('/accessibility/reduced-motion', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ReducedMotionSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertReducedMotion(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'reduced_motion_put_failed', e); }
  });

  // contrast
  router.get('/accessibility/contrast', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json(await m.getContrast(c.tenantId, c.userId) ?? {}); } catch (e) { fail(res, 'contrast_get_failed', e); }
  });
  router.put('/accessibility/contrast', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ContrastSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertContrast(c.tenantId, c.userId, p.data.contrast_mode)); } catch (e) { fail(res, 'contrast_put_failed', e); }
  });

  // font scale
  router.get('/accessibility/font-scale', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json(await m.getFontScale(c.tenantId, c.userId) ?? {}); } catch (e) { fail(res, 'font_scale_get_failed', e); }
  });
  router.put('/accessibility/font-scale', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = FontScaleSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertFontScale(c.tenantId, c.userId, p.data.scale_percent)); } catch (e) { fail(res, 'font_scale_put_failed', e); }
  });

  // device preferences
  router.get('/devices/preferences', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ preferences: await m.listDevicePreferences(c.tenantId, c.userId) }); } catch (e) { fail(res, 'device_prefs_list_failed', e); }
  });
  router.put('/devices/preferences', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = DevicePreferenceSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertDevicePreference(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'device_pref_upsert_failed', e); }
  });

  // device sessions
  router.get('/devices/sessions', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ sessions: await m.listDeviceSessions(c.tenantId, c.userId) }); } catch (e) { fail(res, 'device_sessions_list_failed', e); }
  });
  router.post('/devices/sessions/touch', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = DeviceSessionSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.touchDeviceSession(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'device_session_touch_failed', e); }
  });
  router.delete('/devices/sessions/:sessionId', fgaViewer, async (req, res) => {
    ...
    try {
      ...
      res.status(204).send();
    } catch (e) { fail(res, 'device_session_revoke_failed', e); }
  });

  // viewport profiles
  router.get('/devices/viewport-profiles', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ profiles: await m.listViewportProfiles(c.tenantId) }); } catch (e) { fail(res, 'viewport_profiles_list_failed', e); }
  });
  router.put('/devices/viewport-profiles', fgaEditor, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = ViewportProfileSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertViewportProfile(c.tenantId, p.data)); } catch (e) { fail(res, 'viewport_profile_upsert_failed', e); }
  });
  router.delete('/devices/viewport-profiles/:profileId', fgaEditor, async (req, res) => {
    ...
    try {
      ...
      res.status(204).send();
    } catch (e) { fail(res, 'viewport_profile_delete_failed', e); }
  });
