...
import {
  VisibilityRuleSchema, VisibilityRuleUpdateSchema, PermissionBindingSchema,
  PolicyEvaluationSchema, DeniedRenderSchema,
  RoleLayoutAssignmentSchema, RoleDashboardAssignmentSchema, RoleNavigationAssignmentSchema,
...
export function createPermissionRouter(pool: DbPool): Router {
  const router = Router();
  ...
  router.post('/visibility-rules', fgaAdmin, async (req, res) => {
    ...
    try { res.status(201).json(await m.createVisibilityRule(c.tenantId, p.data)); } catch (e) { fail(res, 'visibility_rule_create_failed', e); }
  });
  router.put('/visibility-rules/:ruleId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!r) { res.status(404).json({ error: 'rule_not_found' }); return; }
      res.json(r);
    } catch (e) { fail(res, 'visibility_rule_update_failed', e); }
  });
  router.delete('/visibility-rules/:ruleId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      res.status(204).send();
    } catch (e) { fail(res, 'visibility_rule_delete_failed', e); }
  });

  // Permission bindings
  router.get('/permission-bindings', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ bindings: await m.listPermissionBindings(c.tenantId, {
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
    }) }); } catch (e) { fail(res, 'permission_bindings_list_failed', e); }
  });
  router.put('/permission-bindings', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = PermissionBindingSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertPermissionBinding(c.tenantId, p.data)); } catch (e) { fail(res, 'permission_binding_upsert_failed', e); }
  });
  router.delete('/permission-bindings/:bindingId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      res.status(204).send();
    } catch (e) { fail(res, 'permission_binding_delete_failed', e); }
  });
  ...
  router.post('/policy-evaluations', fgaViewer, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.logPolicyEvaluation(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'policy_eval_log_failed', e); }
  });
  router.get('/policy-evaluations', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ events: await m.listPolicyEvaluations(c.tenantId, {
      userId: (req.query.userId as string | undefined) ?? null,
      targetKind: (req.query.targetKind as string | undefined) ?? null,
      targetId: (req.query.targetId as string | undefined) ?? null,
      limit: parseInt(String(req.query.limit ?? 200), 10),
    }) }); } catch (e) { fail(res, 'policy_eval_list_failed', e); }
  });
  ...
  router.post('/denied-renders', fgaViewer, async (req, res) => {
    ...
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.status(201).json(await m.logDeniedRender(c.tenantId, c.userId, p.data)); } catch (e) { fail(res, 'denied_render_log_failed', e); }
  });
  router.get('/denied-renders', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ events: await m.listDeniedRenders(c.tenantId, {
      userId: (req.query.userId as string | undefined) ?? null,
      limit: parseInt(String(req.query.limit ?? 200), 10),
    }) }); } catch (e) { fail(res, 'denied_render_list_failed', e); }
  });

  // Role → layout
  router.get('/role-assignments/layouts', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ assignments: await m.listRoleLayoutAssignments(c.tenantId, (req.query.roleCode as string | undefined) ?? null) }); } catch (e) { fail(res, 'role_layout_list_failed', e); }
  });
  router.put('/role-assignments/layouts', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = RoleLayoutAssignmentSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertRoleLayoutAssignment(c.tenantId, p.data)); } catch (e) { fail(res, 'role_layout_upsert_failed', e); }
  });
  router.delete('/role-assignments/layouts/:assignmentId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      res.status(204).send();
    } catch (e) { fail(res, 'role_layout_delete_failed', e); }
  });

  // Role → dashboard
  router.get('/role-assignments/dashboards', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ assignments: await m.listRoleDashboardAssignments(c.tenantId, (req.query.roleCode as string | undefined) ?? null) }); } catch (e) { fail(res, 'role_dashboard_list_failed', e); }
  });
  router.put('/role-assignments/dashboards', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = RoleDashboardAssignmentSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertRoleDashboardAssignment(c.tenantId, p.data)); } catch (e) { fail(res, 'role_dashboard_upsert_failed', e); }
  });
  router.delete('/role-assignments/dashboards/:assignmentId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'assignment_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'role_dashboard_delete_failed', e); }
  });

  // Role → navigation
  router.get('/role-assignments/navigation', fgaViewer, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    try { res.json({ assignments: await m.listRoleNavigationAssignments(c.tenantId, (req.query.roleCode as string | undefined) ?? null) }); } catch (e) { fail(res, 'role_nav_list_failed', e); }
  });
  router.put('/role-assignments/navigation', fgaAdmin, async (req, res) => {
    const c = ctx(req, res); if (!c) return;
    const p = RoleNavigationAssignmentSchema.safeParse(req.body ?? {});
    if (!p.success) { res.status(400).json({ error: 'invalid_request', details: p.error.flatten() }); return; }
    try { res.json(await m.upsertRoleNavigationAssignment(c.tenantId, p.data)); } catch (e) { fail(res, 'role_nav_upsert_failed', e); }
  });
  router.delete('/role-assignments/navigation/:assignmentId', fgaAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!ok) { res.status(404).json({ error: 'assignment_not_found' }); return; }
      res.status(204).send();
    } catch (e) { fail(res, 'role_nav_delete_failed', e); }
  });
