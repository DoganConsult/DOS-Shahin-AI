...
export function createPlatformAdminRouter(pool: DbPool): Router {
  const router = Router();

  router.get('/platform-admin/services', async (_req, res) => {
    try {
      const { rows } = await pool.query(
        `SELECT s.service_code, s.display_name, s.module_code, s.port,
      ...
      res.status(500).json({ error: 'services_query_failed', message: (e as Error).message });
    }
  });
  ...
  router.get('/platform-admin/modules', async (_req, res) => {
    try {
      const { rows } = await pool.query(
        `SELECT m.module_code, m.platform_key, m.product_key, m.display_name,
      ...
      res.status(500).json({ error: 'modules_query_failed', message: (e as Error).message });
    }
  });
  ...
  router.get('/platform-admin/navigation', async (_req, res) => {
    try {
      const { rows } = await pool.query(
        `SELECT id::text, module_code, label, route, sort_order,
      ...
      res.status(500).json({ error: 'navigation_query_failed', message: (e as Error).message });
    }
  });
  ...
  router.get('/platform-admin/module-roles', async (_req, res) => {
    try {
      const { rows } = await pool.query(
        `SELECT module_code, role_code, granted_by, granted_at
           FROM dos.platform_admin_module_roles
          ORDER BY module_code, role_code`,
      );
      res.json({ moduleRoles: rows, count: rows.length });
    } catch (e) {
      res.status(500).json({ error: 'module_roles_query_failed', message: (e as Error).message });
    }
  });
