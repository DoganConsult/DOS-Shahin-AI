...
describe('verifyGatewayOrigin', () => {
  ...
  it('rejects MISSING when token is empty', () => {
    ...
    if (!r.ok) expect((r as Extract<typeof r, { ok: false }>).reason).toBe('MISSING');
  });

  it('rejects MALFORMED when there is no dot separator', () => {
    const r = verifyGatewayOrigin('no-dot-here', { secret: SECRET });
    expect(r.ok).toBe(false);
    if (!r.ok) expect((r as Extract<typeof r, { ok: false }>).reason).toBe('MALFORMED');
  });
  ...
  it('rejects BAD_HMAC when signature does not verify', () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect((r as Extract<typeof r, { ok: false }>).reason).toBe('BAD_HMAC');
  });
  ...
  it('rejects EXPIRED tokens', () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect((r as Extract<typeof r, { ok: false }>).reason).toBe('EXPIRED');
  });

  it('rejects WRONG_SOURCE', () => {
    const r = verifyGatewayOrigin(sign(basePayload({ src: 'attacker' })), { secret: SECRET });
    expect(r.ok).toBe(false);
    if (!r.ok) expect((r as Extract<typeof r, { ok: false }>).reason).toBe('WRONG_SOURCE');
  });

  it('rejects WRONG_VERSION', () => {
    const r = verifyGatewayOrigin(sign(basePayload({ v: 99 })), { secret: SECRET });
    expect(r.ok).toBe(false);
    if (!r.ok) expect((r as Extract<typeof r, { ok: false }>).reason).toBe('WRONG_VERSION');
  });
  ...
  it('rejects REPLAYED jti within ttl', () => {
    ...
    if (!b.ok) expect((b as Extract<typeof b, { ok: false }>).reason).toBe('REPLAYED');
  });
});
