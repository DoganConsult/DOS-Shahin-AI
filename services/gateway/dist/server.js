...
(0, decision_ledger_1.initDecisionLedger)(process.env.DATABASE_URL);
...
const tenantPool = process.env.DATABASE_URL
    ? new pg_1.Pool({ connectionString: process.env.DATABASE_URL, max: Number(process.env.DB_POOL_MAX || 5) })
    : null;
if (!tenantPool) {
...
async function resolveTenantId(sub, email) {
    if (!tenantPool || !sub)
        return null;
    const key = sub;
    const now = Date.now();
    ...
    let tenantId = null;
    try {
        const r = await tenantPool.query(`SELECT tenant_id FROM dos.tenant_memberships
        ...
        if (r.rows.length > 0) {
            ...
            const r2 = await tenantPool.query(`SELECT u.tenant_id FROM dos.users u
          WHERE u.user_id = $1 OR u.email = $2
          ORDER BY (u.user_id = $1) DESC LIMIT 1`, [sub, email]);
        ...
        console.warn('[gateway] tenant resolve failed', e.message);
    }
...
const jwks = (0, jose_1.createRemoteJWKSet)(new URL(KC_JWKS_URL), {
...
function authGuard(req, res, next) {
    if (PUBLIC_AUTH_PATHS.has(req.path))
        return next();
    const cookieToken = (req.cookies && req.cookies['dos_access_token']);
...
function forwardOriginalUrl(req, res, next) {
...
function injectIdentityHeaders(req, res, next) {
    ...
    if (sub && email) {
        try {
            const token = (0, gateway_origin_1.signGatewayOrigin)({ sub, email, tenantId, roles, ttlSeconds: GATEWAY_ORIGIN_TTL_SECONDS }, GATEWAY_ORIGIN_HMAC_SECRET);
            req.headers[gateway_origin_1.GATEWAY_ORIGIN_HEADER] = token;
        }
        catch (err) {
            console.error('[gateway] gateway-origin sign failed', err.message);
...
app.use('/api/tenants/home', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use('/api/tenants', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
}));
...
app.use('/api/trials', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
    pathRewrite: (p) => `/trials${p.startsWith('/') ? p : `/${p}`}`,
}));
app.use('/api/subscription', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
if (INTEGRATIONS_SERVICE_URL) {
    app.use('/api/integrations', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    ...
    app.use('/api/connector-health', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: INTEGRATIONS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
...
if (AI_ENGINE_SERVICE_URL) {
    app.use('/api/ai-engine', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/ai-enhanced', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/ai-os', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/nudges', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    ...
    app.use('/api/copilot/public-chat', (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
        pathRewrite: () => '/api/copilot/public-chat',
    }));
    app.use('/api/copilot/public-agents', (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
        pathRewrite: () => '/api/copilot/public-agents',
    }));
    app.use('/api/copilot', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/agrc-os', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/security/quantum', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    ...
    app.use('/api/runtime-health', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    ...
    ], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
...
if (AI_GOVERNANCE_SERVICE_URL) {
    app.use('/api/ai-governance', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
if (AI_GATEWAY_SERVICE_URL) {
    ...
    app.use('/api/ai', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_GATEWAY_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/ai-gateway', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_GATEWAY_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
...
if (UI_OS_SERVICE_URL) {
    app.use('/api/ui-os', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: UI_OS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
...
if (UI_OS_SERVICE_URL) {
    app.use('/api/dynamic-ui', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: UI_OS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    ...
    app.use('/api/dynamic-ui', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
if (LANGFUSE_URL) {
    ...
    const langfuseProxy = (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: LANGFUSE_URL,
        changeOrigin: true,
        xfwd: true,
        ws: true,
...
app.use('/admin/pm2', (0, http_proxy_middleware_1.createProxyMiddleware)({
...
if (AI_ENGINE_SERVICE_URL) {
    app.use('/api/dnoc/ai', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
        pathRewrite: { '^/api/dnoc/ai': '/api/dnoc-ai' },
    }));
    app.use('/api/dsoc/ai', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    ...
    app.use('/api/ai-hr', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    ...
    app.use('/api/sales/copilot-leads', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: AI_ENGINE_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
...
app.use('/api/dsoc/port', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: DSOC_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
    pathRewrite: (p) => `/api/dsoc/port${p.startsWith('/') ? p : `/${p}`}`,
}));
app.use('/api/dnoc/port', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: DNOC_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
    pathRewrite: (p) => `/api/dnoc/port${p.startsWith('/') ? p : `/${p}`}`,
}));
app.use('/api/dos', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use('/api/dnoc', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: DNOC_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
    pathRewrite: (p) => `/api/dnoc${p.startsWith('/') ? p : `/${p}`}`,
}));
app.use('/api/dsoc', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: DSOC_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
const _adminZoneAgentRaw = (0, admin_zone_mtls_1.adminZoneMtlsAgent)();
const _adminUpstreamsAllHttps = ((process.env.ADMIN_CONSOLE_BFF_URL || 'http://127.0.0.1:4013').startsWith('https:') &&
...
console.log(`[gateway] admin-zone mTLS: ${(0, admin_zone_mtls_1.adminZoneMtlsStatus)()}${_adminZoneAgentRaw && !_adminUpstreamsAllHttps ? ' (agent staged, awaiting https:// upstreams)' : ''}`);
...
app.use('/api/admin/workflow', (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use('/api/admin', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: ADMIN_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
app.get('/api/session/bootstrap', authGuard, async (req, res) => {
    ...
    try {
        const headers = {};
        if (u.sub)
            headers['x-user-sub'] = String(u.sub);
        if (u.email)
        ...
        console.warn('[gateway] bootstrap tenant enrich failed', e.message);
    }
...
app.use('/api/platform/bootstrap', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
    pathRewrite: () => '/bootstrap',
}));
app.use('/api/entitlements', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
const myPermissionsProxyError = (req, res) => {
    if (req.method !== 'GET' || res.headersSent) {
        if (!res.headersSent)
            res.status(502).json({ error: 'TENANT_UPSTREAM_UNAVAILABLE' });
        return;
    }
    return res.json({ tenantId: null, roles: [], permissions: [], modules: [] });
};
app.use('/api/access/my-permissions', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
// centralized here.
app.use('/api/module-config', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
}));
// Foundation Settings page — read-only tenant configuration aggregator.
app.use('/api/tenant-config', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
if (PRIVACY_SERVICE_URL) {
    app.use('/api/privacy-ops', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    ...
    app.use('/api/privacy', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: PRIVACY_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    ...
    app.use('/api/dpia', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: PRIVACY_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
...
const tenantHomeProxyError = (req, res) => {
    if (req.method !== 'GET' || res.headersSent) {
        if (!res.headersSent)
            res.status(502).json({ error: 'TENANT_UPSTREAM_UNAVAILABLE' });
        return;
...
app.use('/api/tenant-home', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
    pathRewrite: (p) => `/tenant-home${p.startsWith('/') ? p : `/${p}`}`,
...
if (USER_SERVICE_URL) {
    app.use('/api/health/foundation', (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: USER_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
...
app.use('/api/config/products-modules', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
// hit it directly.
app.use('/api/config-center', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
}));
...
app.use('/api/platform-config', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
}));
...
app.use('/api/navigation', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
}));
...
if (DASHBOARD_WIDGETS_SERVICE_URL) {
    app.use('/api/shell', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: DASHBOARD_WIDGETS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/dashboard', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: DASHBOARD_WIDGETS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/dashboard-editor', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: DASHBOARD_WIDGETS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/widgets', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: DASHBOARD_WIDGETS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
...
if (ANALYTICS_SERVICE_URL) {
    ...
    app.use('/api/kpi', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: ANALYTICS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
        pathRewrite: (path) => `/api/analytics/analytics/kpi-detail${path}`,
    }));
    app.use('/api/analytics', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: ANALYTICS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
    app.use('/api/grc-query', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: ANALYTICS_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
    }));
...
app.use('/api/module-kickstart-status', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
if (NOTIFICATION_SERVICE_URL) {
    ...
    const notificationsProxyError = (req, res) => {
        if (req.method !== 'GET' || res.headersSent) {
            if (!res.headersSent)
                res.status(502).json({ error: 'NOTIFICATION_UPSTREAM_UNAVAILABLE' });
            return;
        }
    ...
    app.use('/api/notifications', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: NOTIFICATION_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
        onError: notificationsProxyError,
    }));
    app.use('/api/notification', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
        target: NOTIFICATION_SERVICE_URL,
        changeOrigin: true,
        xfwd: true,
        onError: notificationsProxyError,
...
app.use('/api/workspaces', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TENANT_SERVICE_URL,
    changeOrigin: true,
    xfwd: true,
...
app.use('/api/mcp', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: MCP_GATEWAY_SERVICE_URL, changeOrigin: true, xfwd: true, pathRewrite: { '^/api/mcp': '' },
}));
app.use('/api/agrc-os-service', authGuard, injectIdentityHeaders, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/audit', '/api/activity-feed'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: AUDIT_SERVICE_URL, changeOrigin: true, xfwd: true,
}));
app.use('/api/onboarding', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use('/api/executive', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/records', '/api/record'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/remediation', '/api/action', '/api/remediation-action'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: REMEDIATION_ACTION_SERVICE_URL, changeOrigin: true, xfwd: true,
}));
// portals-service mounts /api/portals AND /api/portal.
app.use(['/api/portals', '/api/portal'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: PORTALS_SERVICE_URL, changeOrigin: true, xfwd: true,
}));
app.use('/api/training', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
    target: TRAINING_SERVICE_URL, changeOrigin: true, xfwd: true,
}));
app.use('/api/dora', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use('/api/reporting', authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/bcp', '/api/bcp-service'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/vendor', '/api/vendors', '/api/vendor-service', '/api/vendor-risk'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/platform-core', '/api/mobile'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
app.use(['/api/platform-product', '/api/product', '/api/knowledge', '/api/operating-cockpit', '/api/platform-stats'], authGuard, injectIdentityHeaders, forwardOriginalUrl, (0, http_proxy_middleware_1.createProxyMiddleware)({
...
httpServer.on('upgrade', async (req, socket, head) => {
    ...
    const writeLedger = (allowed, reasonCode, userId, tenantId, detail) => {
        void (0, decision_ledger_1.writeDecision)({
    ...
    try {
        ...
        const { payload } = await (0, jose_1.jwtVerify)(token, jwks, { issuer: KC_ISSUER, audience: KC_AUDIENCE });
        const identity = {
        ...
        writeLedger(false, 'INVALID_TOKEN', 'anonymous', '', { err: err.message });
        try {
            socket.write('HTTP/1.1 401 Unauthorized\r\n\r\n');
        }
        catch { /* ignore */ }
