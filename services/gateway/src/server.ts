...
async function resolveDbRoles(sub: string, tenantId: string): Promise<string[]> {
  if (!tenantPool || !sub || !tenantId) return [];
  ...
  let roles: string[] = [];
  try {
    const r = await tenantPool.query<{ role_code: string }>(
    ...
    console.warn('[gateway] db-role resolve failed', (e as Error).message);
  }
...
async function injectIdentityHeaders(req: Request, res: Response, next: NextFunction) {
  ...
  const dbRoles: string[] = sub && tenantId ? await resolveDbRoles(sub, tenantId) : [];
...
if (UI_OS_SERVICE_URL) {
  ...
  if (typeof allowlistTimer.unref === 'function') allowlistTimer.unref();
