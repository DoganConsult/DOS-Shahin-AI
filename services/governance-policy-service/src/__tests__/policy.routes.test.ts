...
vi.mock('../events/publisher', () => ({
  ...
  publishPolicyCreated: vi.fn().mockResolvedValue(undefined),
  publishPolicyPublished: vi.fn().mockResolvedValue(undefined),
  publishPolicyReviewed: vi.fn().mockResolvedValue(undefined),
  publishPolicyRetired: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock('../adapters/auth.adapter', () => ({
  ...
  requireDauth: vi.fn(() => (_req: any, _res: any, next: any) => next()),
  requireOwnershipOf: vi.fn(() => (_req: any, _res: any, next: any) => next()),
  requireAnyPermission: vi.fn(() => (_req: any, _res: any, next: any) => next()),
  requireSuperAdmin: vi.fn((_req: any, _res: any, next: any) => next()),
}));

import router from '../routes/policy.routes';
import * as domainSvc from '../domain/policy.service';
...
describe('policy routes', () => {
  ...
  describe('GET /api/policy/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/policy/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/policy/missing');
  ...
  describe('DELETE /api/policy/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/policy/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
