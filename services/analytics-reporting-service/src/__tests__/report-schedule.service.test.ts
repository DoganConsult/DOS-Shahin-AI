...
import * as svc from '../domain/report-schedule.service';
...
describe('report-schedule.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { report_type: 'test-report_type', title: 'test-title', frequency: 'test-frequency' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { report_type: 'x', title: 'x', frequency: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
