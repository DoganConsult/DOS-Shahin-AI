...
vi.mock('../domain/report-schedule.service', () => ({
...
import router from '../routes/report-schedule.routes';
import * as domainSvc from '../domain/report-schedule.service';
...
function createApp() {
  ...
  app.use('/api/report-schedule', router);
  return app;
}
...
describe('report-schedule routes', () => {
  ...
  describe('GET /api/report-schedule/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/report-schedule/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/report-schedule/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/report-schedule/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/report-schedule/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/report-schedule/', () => {
  ...
  describe('PUT /api/report-schedule/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/report-schedule/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/report-schedule/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/report-schedule/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
