...
vi.mock('../domain/finding.service', () => ({
...
import router from '../routes/finding.routes';
import * as domainSvc from '../domain/finding.service';
...
describe('finding routes', () => {
  ...
  describe('GET /api/finding/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/finding/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/finding/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/finding/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/finding/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/finding/', () => {
  ...
  describe('DELETE /api/finding/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/finding/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
