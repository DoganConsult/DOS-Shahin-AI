...
import * as svc from '../domain/mobile-session.service';
...
describe('mobile-session.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { user_id: 'test-user_id', device_type: 'test-device_type' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { user_id: 'x', device_type: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
