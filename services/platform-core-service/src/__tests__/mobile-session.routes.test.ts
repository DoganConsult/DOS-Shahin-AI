...
vi.mock('../domain/mobile-session.service', () => ({
...
import router from '../routes/mobile-session.routes';
import * as domainSvc from '../domain/mobile-session.service';
...
function createApp() {
  ...
  app.use('/api/mobile-session', router);
  return app;
}
...
describe('mobile-session routes', () => {
  ...
  describe('GET /api/mobile-session/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/mobile-session/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/mobile-session/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/mobile-session/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/mobile-session/missing');
  ...
  describe('PUT /api/mobile-session/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/mobile-session/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/mobile-session/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/mobile-session/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
