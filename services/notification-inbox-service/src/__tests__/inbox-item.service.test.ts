...
import * as svc from '../domain/inbox-item.service';

const mockQuery = safeQuery as ReturnType<typeof vi.fn>;
const TENANT = 'tenant-001';

describe('inbox-item.service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { user_id: 'test-user_id', title: 'test-title', type: 'test-type', entity_type: 'test-entity_type' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { user_id: 'x', title: 'x', type: 'x', entity_type: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
