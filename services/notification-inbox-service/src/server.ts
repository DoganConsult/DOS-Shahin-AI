...
app.use(createHealthRouter('notification_inbox', {
  db: dbHealthCheck,
...
const SERVICE_CODE = 'notification-inbox-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    port: config.port,
    routes: [
      { path: '/api/inbox', router: routes },
    ],
    // Wave 2D: Module lifecycle registration
    modules: await loadModuleRegistrations('notification'),
    healthChecks: {
      database: async () => {
        try {
    ...
    onReady: () => {
      ...
      invokeModuleLifecycleHooks(SERVICE_CODE, 'notification').catch(() => {});
    },
  });
