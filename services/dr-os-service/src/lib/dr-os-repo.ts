...
export async function getRecord(recordKey: string, version?: number) {
  const r = version
    ? await masterQuery(`SELECT * FROM dos.dr_record WHERE record_key=$1 AND version=$2`, [recordKey, version])
    : await masterQuery(`SELECT * FROM dos.dr_record WHERE record_key=$1 ORDER BY version DESC LIMIT 1`, [recordKey]);
...
export async function createRecord(input: { record_key: string; title: string; kind: string; trust_zone: 'public'|'tenant'|'admin'; config: unknown; created_by: string; }) {
  ...
  const v = await masterQuery(
    `SELECT COALESCE(MAX(version),0)::int + 1 AS next FROM dos.dr_record WHERE record_key=$1`,
    [input.record_key],
  );
  const next = (v.rows[0] as { next: number }).next;
  const r = await masterQuery(
    `INSERT INTO dos.dr_record (record_key, version, title, kind, trust_zone, status, config, created_by)
...
export async function publishRecord(recordKey: string, version: number) {
  await actor();
  const r = await masterQuery(
    `UPDATE dos.dr_record SET status='published', published_at=now()
...
export async function listEvents(recordKey?: string, limit = 100) {
  const r = await masterQuery(
    `SELECT e.id, e.record_id, COALESCE(e.record_key, d.record_key) AS record_key,
       ...
       FROM dos.dr_event e
       LEFT JOIN dos.dr_record d ON d.id = e.record_id
...
export async function emitEvent(input: { record_key: string; kind: string; payload?: Record<string, unknown>; emitted_by: string; }) {
  ...
  const r = await masterQuery(
    `INSERT INTO dos.dr_event (record_id, record_key, kind, payload, emitted_by)
...
// ── Phase 3 / L30 — dr domain logic: deterministic record evaluator.
...
export async function evaluateRecord(recordKey: string, ctx: RecordEvalCtx): Promise<RecordEvalResult> {
  ...
  await masterQuery(
    ...
    [rec ? (rec as { id: string }).id : null, recordKey, 'dr_evaluated',
     JSON.stringify({ ctx, decision, reason, version: row.version }), 'dr-os-service'],
  );
