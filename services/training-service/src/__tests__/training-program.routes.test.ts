...
vi.mock('../events/publisher', () => ({
  ...
  publishTrainingAssigned: vi.fn().mockResolvedValue(undefined),
  publishTrainingCompleted: vi.fn().mockResolvedValue(undefined),
  publishTrainingOverdue: vi.fn().mockResolvedValue(undefined),
  publishTrainingCertificateIssued: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock('../domain/training-program.service', () => ({
...
import router from '../routes/training-program.routes';
import * as domainSvc from '../domain/training-program.service';
...
describe('training-program routes', () => {
  ...
  describe('GET /api/training-program/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/training-program/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/training-program/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/training-program/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/training-program/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/training-program/', () => {
  ...
  describe('PUT /api/training-program/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/training-program/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/training-program/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/training-program/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
