...
import * as svc from '../domain/training-program.service';

const mockQuery = safeQuery as ReturnType<typeof vi.fn>;
const TENANT = 'tenant-001';

describe('training-program.service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { title: 'test-title', category: 'test-category' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { title: 'x', category: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
