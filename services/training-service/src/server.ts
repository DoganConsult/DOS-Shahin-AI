...
app.use(createHealthRouter('training', {
  db: dbHealthCheck,
...
const SERVICE_CODE = 'training-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    port: config.port,
    routes: [
      { path: '/api/training', router: routes },
    ],
    modules: await loadModuleRegistrations('training'),
    healthChecks: {
      database: async () => {
        try {
    ...
    onReady: () => {
      invokeModuleLifecycleHooks(SERVICE_CODE, 'training').catch(() => {});
    },
  });
