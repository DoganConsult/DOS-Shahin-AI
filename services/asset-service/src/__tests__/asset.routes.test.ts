...
import router from '../routes/asset.routes';
import * as domainSvc from '../domain/asset.service';
...
describe('asset routes', () => {
  ...
  describe('GET /api/asset/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/asset/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/asset/missing');
  ...
  describe('DELETE /api/asset/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/asset/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
