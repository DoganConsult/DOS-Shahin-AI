...
vi.mock('../events/publisher', () => ({
  ...
  publishAssetCreated: vi.fn().mockResolvedValue(undefined),
  publishAssetUpdated: vi.fn().mockResolvedValue(undefined),
  publishAssetDecommissioned: vi.fn().mockResolvedValue(undefined),
  publishAssetVulnerabilityDetected: vi.fn().mockResolvedValue(undefined),
}));

import { registerConsumers } from '../events/consumer';

describe('asset-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["risk.created","incident.created","vendor.assessment.completed"];
