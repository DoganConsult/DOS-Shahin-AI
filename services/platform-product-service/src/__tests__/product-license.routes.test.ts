...
vi.mock('../events/publisher', () => ({
  ...
  publishProductLicenseActivated: vi.fn().mockResolvedValue(undefined),
  publishProductLicenseUpgraded: vi.fn().mockResolvedValue(undefined),
  publishProductLicenseExpiring: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock('../domain/product-license.service', () => ({
...
import router from '../routes/product-license.routes';
import * as domainSvc from '../domain/product-license.service';
...
function createApp() {
  ...
  app.use('/api/product-license', router);
  return app;
}

describe('product-license routes', () => {
  ...
  describe('GET /api/product-license/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/product-license/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/product-license/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/product-license/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/product-license/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/product-license/', () => {
    it('creates new record', async () => {
      const res = await request(app)
        .post('/api/product-license/')
        .send({ title: 'New Item', category: 'test', type: 'test', severity: 'medium', name: 'test', framework_name: 'test', control_ref: 'CR-1' });
      expect([200, 201, 400]).toContain(res.status);
    });
  });

  describe('PUT /api/product-license/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/product-license/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/product-license/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/product-license/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
