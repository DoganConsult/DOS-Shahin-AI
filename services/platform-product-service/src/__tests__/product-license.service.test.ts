...
import * as svc from '../domain/product-license.service';

const mockQuery = safeQuery as ReturnType<typeof vi.fn>;
const TENANT = 'tenant-001';

describe('product-license.service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { product_code: 'test-product_code', plan: 'test-plan' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { product_code: 'x', plan: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
