...
describe('platform-product-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["tenant.created","tenant.updated"];
  ...
  it('handles tenant.updated without throwing', async () => {
    const handler = handlers.get('tenant.updated');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
