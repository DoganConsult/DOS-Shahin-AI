...
vi.mock('../events/publisher', () => ({
  ...
  publishAgrcTaskCreated: vi.fn(),
  publishAgrcTaskCompleted: vi.fn(),
  publishAgrcAgentInvoked: vi.fn(),
}));
...
vi.mock('../domain/agrc-task.service', () => ({
  list: vi.fn().mockResolvedValue({ data: [], total: 0, page: 1, pageSize: 25 }),
  getById: vi.fn().mockResolvedValue({ id: 'r1', task_id: 'r1', tenant_id: 't1', title: 'Test' }),
  create: vi.fn().mockResolvedValue({ id: 'new1', task_id: 'new1', tenant_id: 't1', entryId: 'new1' }),
  update: vi.fn().mockResolvedValue({ id: 'r1', task_id: 'r1', tenant_id: 't1', title: 'Updated' }),
...
import router from '../routes/agrc-task.routes';
import * as domainSvc from '../domain/agrc-task.service';
...
function createApp() {
  ...
  app.use('/api/agrc-task', router);
  return app;
}

describe('agrc-task routes', () => {
  ...
  describe('GET /api/agrc-task/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/agrc-task/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/agrc-task/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/agrc-task/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/agrc-task/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/agrc-task/', () => {
  ...
  describe('PUT /api/agrc-task/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/agrc-task/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/agrc-task/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/agrc-task/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
