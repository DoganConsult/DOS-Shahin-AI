...
vi.mock('../events/publisher', () => ({
  ...
  publishAgrcTaskCreated: vi.fn().mockResolvedValue(undefined),
  publishAgrcTaskCompleted: vi.fn().mockResolvedValue(undefined),
  publishAgrcAgentInvoked: vi.fn().mockResolvedValue(undefined),
}));

import { safeQuery } from '@dos/db';
import * as svc from '../domain/agrc-task.service';

const mockQuery = safeQuery as ReturnType<typeof vi.fn>;
const TENANT = 'tenant-001';

describe('agrc-task.service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
