 ...
 */
async function resolveAllowedModulesFromDb(client, productCode) {
    const r = await client.query(`SELECT module_code
       FROM dos.dynamic_ui_modules
      WHERE product_key = $1
        AND default_tenant_enrollment_status = 'active'
      ORDER BY module_code`, [productCode]);
    const modules = r.rows.map(row => row.module_code);
    if (!modules.includes('foundation'))
        modules.unshift('foundation');
    return modules;
}
async function resolveTrialConfig(client, productCode) {
