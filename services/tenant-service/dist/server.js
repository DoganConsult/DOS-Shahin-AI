...
const anti_abuse_1 = require("./middleware/anti-abuse");
const trial_bundle_1 = require("./domain/trial-bundle");
const openfga_1 = require("./openfga");
const trial_lifecycle_1 = require("./domain/trial-lifecycle");
...
let _prom = null;
let _promRegister = null;
let _failedGauge = null;
let _byStatusGauge = null;
try {
    _prom = require('prom-client');
...
async function _refreshTenantMigrationsLedger() {
    if (!_failedGauge || !_byStatusGauge)
        return;
    try {
        const failed = await pool.query(`SELECT tenant_id, count(*)::int AS n FROM dos.tenant_migrations WHERE status='failed' GROUP BY tenant_id`);
        _failedGauge.reset();
        for (const r of failed.rows)
            _failedGauge.set({ tenant_id: String(r.tenant_id) }, Number(r.n));
        const byStatus = await pool.query(`SELECT status, count(*)::int AS n FROM dos.tenant_migrations GROUP BY status`);
        _byStatusGauge.reset();
        for (const r of byStatus.rows)
            _byStatusGauge.set({ status: String(r.status) }, Number(r.n));
    }
    catch (err) {
        console.warn('[tenant-service] tenant_migrations refresh failed', err.message);
    }
}
...
app.get('/metrics', async (_req, res) => {
    if (!_promRegister) {
        res.status(503).type('text/plain').send('# prom-client unavailable\n');
        return;
    }
    try {
...
app.use((0, gateway_origin_1.requireGatewayOrigin)());
// ── Caller identity ─────────────────────────────────────────────────────────
//
...
// not authority.
function callerName(req) { return req.headers['x-user-name'] || undefined; }
function requireCaller(req, res) {
    const p = req.principal;
    if (!p?.sub) {
        res.status(401).json({ error: 'NO_CALLER' });
        return null;
    }
    if (!p.email) {
        res.status(401).json({ error: 'NO_EMAIL_CLAIM' });
        return null;
    }
...
function suggestTenantCode(orgName, email) {
...
async function findUserBySub(sub) {
    const { rows } = await pool.query(`SELECT u.user_id, u.email, u.display_name, u.tenant_id, u.status, u.sso_provider,
...
async function findUsersByEmail(email) {
    const { rows } = await pool.query(`SELECT user_id, email, tenant_id, sso_provider
       FROM dos.users
      WHERE lower(email) = lower($1) AND deleted_at IS NULL`, [email]);
    return rows;
}
async function findActiveMembership(client, sub) {
    const { rows } = await client.query(`SELECT m.tenant_id, m.role_code, m.is_tenant_owner,
            t.tenant_name, t.tenant_code, t.status AS tenant_status
...
app.post('/register', (0, anti_abuse_1.antiAbuse)({ pool }), async (req, res) => {
    ...
    const orgName = (req.body?.orgName?.trim()
        || req.headers['x-user-company']?.trim()
        || undefined);
    const orgNameAr = (req.body?.orgNameAr?.trim()
        || req.headers['x-user-company-ar']?.trim()
        || undefined);
    ...
    try {
        ...
        const existingUser = await client.query(`SELECT user_id, tenant_id FROM dos.users WHERE user_id = $1 AND deleted_at IS NULL`, [c.sub]);
        ...
        await client.query(`INSERT INTO dos.tenants (tenant_id, tenant_code, tenant_name, schema_name, status)
         VALUES ($1, $2, $3, $4, 'active')
         ON CONFLICT (tenant_id) DO NOTHING`, [tenantId, tenantCode, tenantName, schemaName]);
        // 3. Upsert user row pinned to this tenant.
        await client.query(`INSERT INTO dos.users (user_id, email, display_name, full_name, tenant_id, status, email_verified, sso_provider)
         VALUES ($1, $2, $3, $3, $4, 'active', true, 'keycloak')
         ON CONFLICT (user_id) DO UPDATE
           SET email = EXCLUDED.email,
               tenant_id = COALESCE(dos.users.tenant_id, EXCLUDED.tenant_id)`, [c.sub, c.email, c.name || c.email, tenantId]);
        ...
        if (orgNameAr) {
            try {
                ...
                await client.query(`INSERT INTO dos.ui_tenant_branding (tenant_id, brand_name, is_active)
             VALUES ($1, $2, TRUE)
             ON CONFLICT (tenant_id) DO UPDATE SET brand_name = EXCLUDED.brand_name`, [tenantId, orgNameAr]);
                await client.query('RELEASE SAVEPOINT brand_ar');
            }
            catch (e) {
                await client.query('ROLLBACK TO SAVEPOINT brand_ar').catch(() => { });
                console.warn('[tenant-service] orgNameAr branding skipped:', e.message);
            }
        }
        ...
        //    owner is appointed.
        await client.query(`INSERT INTO dos.tenant_memberships (user_id, tenant_id, role_code, status, membership_type, is_tenant_owner)
         VALUES ($1, $2, $3, 'active', 'internal', false)
         ON CONFLICT DO NOTHING`, [c.sub, tenantId, DEFAULT_REGISTRATION_ROLE]);
        ...
        await client.query(`INSERT INTO dos.user_role_assignments
       ...
       ON CONFLICT DO NOTHING`, [
        ...
        for (const productKey of defaultProducts) {
            await client.query(`INSERT INTO dos.tenant_product_activation (tenant_id, product_key, status)
           VALUES ($1, $2, 'active')
           ON CONFLICT DO NOTHING`, [tenantId, productKey]);
        }
        ...
        const bundle = await (0, trial_bundle_1.createTrialBundle)(client, {
            tenantId,
        ...
        let fgaWrite = null;
        try {
            fgaWrite = await (0, openfga_1.writeTuples)((0, openfga_1.buildRegistrationTuples)({
            ...
            if (fgaWrite.outcome === 'unavailable' || fgaWrite.outcome === 'error') {
                ...
                if ((0, openfga_1.isOpenFgaEnforced)()) {
            ...
            console.warn('[tenant-service] /register OpenFGA seed threw', e.message);
            if ((0, openfga_1.isOpenFgaEnforced)()) {
                return res.status(503).json({
        ...
        return res.status(201).json({
            ...
            authz: {
                provider: 'openfga',
                configured: (0, openfga_1.isOpenFgaConfigured)(),
                enforce: (0, openfga_1.isOpenFgaEnforced)(),
...
app.get('/trials/current', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    const client = await pool.connect();
    try {
        ...
        const productCode = req.query.productCode
            ...
            || 'shahin-ai';
        const summary = await (0, trial_bundle_1.getTrialSummary)(client, tenantId, productCode);
...
app.post('/trials/convert', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    const productCode = req.body?.productCode
        || process.env.TRIAL_PRODUCT_CODE
        || 'shahin-ai';
    const planCode = req.body?.planCode || 'paid';
    ...
    try {
        ...
        try {
            ...
            const trialQ = await client.query(`SELECT trial_id, status FROM dos.tenant_trials
          ...
          ORDER BY created_at DESC LIMIT 1`, [tenantId, productCode]);
            ...
            await client.query(`UPDATE dos.tenant_trials
            SET status='trial_converted', converted_at=now(), updated_at=now()
          WHERE trial_id=$1`, [trial.trial_id]);
            await client.query(`UPDATE dos.tenant_subscriptions
         ...
         VALUES ($1,$2,$3,$4,'active','manually_approved',$5,now(),NULL,'manual',$6)`, [subId, tenantId, productCode, planCode, trial.trial_id,
          ...
          WHERE trial_id=$2 AND source='trial' AND entitlement_status='active'`, [subId, trial.trial_id]);
         ...
         VALUES ($1,$2,$3,$4,'trial_converted',$5,'trial_converted',$6,$7)`, [tenantId, trial.trial_id, c.sub, productCode, trial.status, reason,
...
app.post('/trials/cancel', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    const productCode = req.body?.productCode
    ...
    try {
        ...
        try {
          ...
          WHERE trial_id=$1 AND source='trial' AND entitlement_status='active'`, [trial.trial_id]);
            await client.query(`UPDATE dos.tenant_product_entitlements
            SET entitlement_status='cancelled', updated_at=now()
          WHERE trial_id=$1 AND source='trial' AND entitlement_status='active'`, [trial.trial_id]);
            await client.query(`INSERT INTO dos.trial_audit_log
           (tenant_id, trial_id, user_id, product_code, action,
            old_status, new_status, reason, metadata_json)
         VALUES ($1,$2,$3,$4,'trial_cancelled',$5,'trial_cancelled',$6,'{}'::jsonb)`, [tenantId, trial.trial_id, c.sub, productCode, trial.status, reason]);
            await client.query('COMMIT');
...
app.get('/subscription/current', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    const client = await pool.connect();
    try {
        ...
        const productCode = req.query.productCode
            || process.env.TRIAL_PRODUCT_CODE
            || 'shahin-ai';
        const { rows } = await client.query(`SELECT subscription_id, tenant_id, product_code, plan_code, status,
              billing_status, trial_id, current_period_start, current_period_end,
...
app.get('/me', async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            if (candidates.length === 1) {
                ...
                if (existing.user_id === c.sub) {
                    ...
                    await client.query(`INSERT INTO dos.users (user_id, email, display_name, full_name, tenant_id, status, email_verified, sso_provider, sso_protocol)
               VALUES ($1, $2, $3, $3, $4, 'active', true, 'keycloak', 'oidc')
               ON CONFLICT (user_id) DO NOTHING`, [c.sub, c.email, c.name || c.email, existing.tenant_id]);
                    // Move legacy memberships to sub-keyed user_id (idempotent).
                    await client.query(`UPDATE dos.tenant_memberships SET user_id = $1
        ...
        return res.json({
            ...
            memberships: memberships.map((m) => ({ tenantId: m.tenant_id, name: m.tenant_name, code: m.tenant_code, roleCode: m.role_code, isOwner: m.is_tenant_owner })),
            recovered,
        });
    }
    catch (err) {
...
app.patch('/me', async (req, res) => {
    ...
    if (!c)
        return;
    const newName = req.body?.tenantName?.trim();
    const newNameAr = req.body?.tenantNameAr?.trim();
    ...
    try {
        ...
        await client.query('BEGIN');
        if (newName) {
            await client.query(`UPDATE dos.tenants SET tenant_name = $1 WHERE tenant_id = $2`, [newName, primary.tenant_id]);
        }
        if (newNameAr) {
            try {
                ...
                await client.query(`INSERT INTO dos.ui_tenant_branding (tenant_id, brand_name, is_active)
             VALUES ($1, $2, TRUE)
             ON CONFLICT (tenant_id) DO UPDATE SET brand_name = EXCLUDED.brand_name`, [primary.tenant_id, newNameAr]);
                await client.query('RELEASE SAVEPOINT brand_ar_patch');
            }
            catch (e) {
                await client.query('ROLLBACK TO SAVEPOINT brand_ar_patch').catch(() => { });
                console.warn('[tenant-service] PATCH /me orgNameAr persist skipped:', e.message);
            }
        }
        await client.query('COMMIT');
        return res.json({
...
app.get('/bootstrap', async (req, res) => {
    ...
    try {
        ...
        if (!user)
            return res.status(409).json({ blocked: 'NO_USER' });
        const { rows: memberships } = await pool.query(`SELECT m.tenant_id, t.tenant_name, t.tenant_code, t.status AS tenant_status, m.role_code, m.is_tenant_owner
         FROM dos.tenant_memberships m
         JOIN dos.tenants t ON t.tenant_id = m.tenant_id
        WHERE m.user_id = $1 AND m.status = 'active'
        ORDER BY m.is_tenant_owner DESC, m.created_at ASC LIMIT 1`, [c.sub]);
        if (memberships.length === 0)
            return res.status(409).json({ blocked: 'NO_MEMBERSHIP' });
        ...
        const { rows: settings } = await pool.query(`SELECT settings FROM dos.tenants WHERE tenant_id = $1 LIMIT 1`, [m.tenant_id]);
        ...
        const tenantLandingRoute = typeof tenantSettings.tenantLandingRoute === 'string' && tenantSettings.tenantLandingRoute.trim()
            ? tenantSettings.tenantLandingRoute.trim()
            : null;
        // Maintenance flag (feature flag of platform scope).
        const { rows: flags } = await pool.query(`SELECT enabled FROM dos.feature_flags WHERE flag_code = 'platform.maintenance' AND is_active = true LIMIT 1`);
...
app.get('/entitlements', async (req, res) => {
    ...
    if (!c)
        return;
    try {
        const { rows: mems } = await pool.query(`SELECT tenant_id FROM dos.tenant_memberships
        WHERE user_id = $1 AND status = 'active'
        ORDER BY is_tenant_owner DESC, created_at ASC LIMIT 1`, [c.sub]);
        if (mems.length === 0)
            return res.status(409).json({ blocked: 'NO_MEMBERSHIP' });
        const tenantId = mems[0].tenant_id;
        // Tenant-activated products → modules.
        const { rows: modules } = await pool.query(`SELECT m.module_code, m.product_key, m.display_name, m.status
        ...
        const { rows: settings } = await pool.query(`SELECT settings FROM dos.tenants WHERE tenant_id = $1 LIMIT 1`, [tenantId]);
        ...
        const { rows: flagRows } = await pool.query(`SELECT flag_code, enabled FROM dos.feature_flags WHERE is_active = true`);
        const features = {};
        for (const f of flagRows)
            features[f.flag_code] = !!f.enabled;
        res.json({
            tenantId,
...
app.get('/tenant-config', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        if (mems.length === 0)
            return res.status(409).json({ blocked: 'NO_MEMBERSHIP' });
        const tenantId = mems[0].tenant_id;
        const { rows: tenantRows } = await pool.query(`SELECT tenant_id, tenant_name AS name, status, settings, created_at FROM dos.tenants WHERE tenant_id = $1 LIMIT 1`, [tenantId]);
        const t = tenantRows[0] ?? {};
        const settings = t.settings ?? {};
        const { rows: flagRows } = await pool.query(`SELECT flag_code, enabled FROM dos.feature_flags WHERE is_active = true ORDER BY flag_code`);
        const featureFlags = {};
        for (const f of flagRows)
            featureFlags[f.flag_code] = !!f.enabled;
        const { rows: modRows } = await pool.query(`SELECT m.module_code FROM dos.tenant_product_activation tpa
         JOIN dos.module_registry m ON m.product_key = tpa.product_key
        WHERE tpa.tenant_id = $1 AND tpa.status = 'active' AND m.status = 'active'
        ORDER BY m.module_code`, [tenantId]);
        res.json({
...
app.get('/permissions', async (req, res) => {
    ...
    try {
        ...
        const { rows: idRows } = await pool.query(`SELECT user_id FROM public.iam_identities
        WHERE provider='keycloak' AND external_subject=$1 AND status='active' LIMIT 1`, [c.sub]);
        const canonicalUserId = idRows[0]?.user_id || c.sub;
        const { rows: mems } = await pool.query(`SELECT tenant_id, role_code, is_tenant_owner FROM dos.tenant_memberships
        WHERE user_id = ANY($1) AND status = 'active'
        ORDER BY is_tenant_owner DESC, created_at ASC LIMIT 1`, [[c.sub, canonicalUserId]]);
        if (mems.length === 0)
            return res.status(409).json({ blocked: 'NO_MEMBERSHIP' });
        ...
        const { rows: assignments } = await pool.query(`SELECT role_code FROM platform_dauth.user_role_assignments
        WHERE user_id = ANY($2) AND is_active = true
          AND (tenant_id = $1 OR tenant_id = 'platform' OR tenant_id IS NULL)
          AND (expires_at IS NULL OR expires_at > NOW())
          AND revoked_at IS NULL`, [tenantId, [c.sub, canonicalUserId]]);
        const roleSet = new Set([membershipRole, ...assignments.map(a => a.role_code)].filter(Boolean));
        const roles = Array.from(roleSet);
        // Resolve permission set from functional_roles.permissions (text[]).
        const permSet = new Set();
        if (roles.length > 0) {
            const { rows: roleRows } = await pool.query(`SELECT role_code, permissions FROM platform_dauth.functional_roles WHERE role_code = ANY($1)`, [roles]);
        ...
        const { rows: modules } = await pool.query(`SELECT module_code FROM (
...
app.get('/workspaces', async (req, res) => {
    ...
    if (!c)
        return;
    try {
        const { rows } = await pool.query(`SELECT m.tenant_id, m.role_code, m.is_tenant_owner, m.created_at AS joined_at,
              t.tenant_name, t.tenant_code, t.status AS tenant_status
         FROM dos.tenant_memberships m
         JOIN dos.tenants t ON t.tenant_id = m.tenant_id
        WHERE m.user_id = $1 AND m.status = 'active'
        ORDER BY m.is_tenant_owner DESC, m.created_at ASC`, [c.sub]);
...
app.get('/tenant-home/overview', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        const { rows: mems } = await pool.query(`SELECT m.tenant_id, m.role_code, m.is_tenant_owner,
        ...
        const { rows: moduleRows } = await pool.query(`SELECT m.module_code, m.product_key, m.display_name
         FROM dos.tenant_product_activation tpa
         JOIN dos.module_registry m ON m.product_key = tpa.product_key
        WHERE tpa.tenant_id = $1 AND tpa.status = 'active' AND m.status = 'active'
        ORDER BY m.module_code`, [tenantId]);
        ...
        const { rows: memberCountRows } = await pool.query(`SELECT count(*)::int AS count FROM dos.tenant_memberships
        ...
        const [vacancies, policiesTotal, policiesPublished, ownershipGaps] = await Promise.all([
          ...
          WHERE p.tenant_id = $1
            ...
            safeCount(`SELECT count(*)::int AS count FROM dos.governance_policies
          WHERE tenant_id = $1::uuid AND deleted_at IS NULL`, [tenantId]),
            safeCount(`SELECT count(*)::int AS count FROM dos.governance_policies
          WHERE tenant_id = $1::uuid AND deleted_at IS NULL
            AND status IN ('published','active','approved','effective')`, [tenantId]),
         ...
         )::int AS count`, [tenantId]),
        ]);
        const complianceRate = policiesTotal > 0
            ? Math.round((policiesPublished / policiesTotal) * 100)
            : 0;
        const usersCount = await safeCount(`SELECT count(*)::int AS count FROM dos.tenant_memberships
        WHERE tenant_id = $1 AND status = 'active'`, [tenantId]);
        ...
        const teamsCount = await safeCount(`SELECT count(*)::int AS count FROM dos.tenant_memberships WHERE 1=0`, []);
        return res.json({
            summary: {
                tenantId,
                workspaceId,
...
app.get('/tenant-home/activity', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
...
app.get('/config-center/products-modules', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        if (mems.length === 0)
            return res.status(409).json({ blocked: 'NO_MEMBERSHIP' });
        const tenantId = mems[0].tenant_id;
        const { rows: activatedProducts } = await pool.query(`SELECT product_key FROM dos.tenant_product_activation
        WHERE tenant_id = $1 AND status = 'active'`, [tenantId]);
        const productKeys = activatedProducts.map(r => r.product_key);
        const { rows: modules } = await pool.query(`SELECT module_code, product_key, display_name
         FROM dos.module_registry
        WHERE status = 'active'
          AND ($1::text[] = '{}'::text[] OR product_key = ANY($1::text[]))`, [productKeys]);
        const modulesByProduct = {};
        const internalKeyToBusinessLabel = {};
        const visibleModules = [];
        for (const m of modules) {
            const pk = (m.product_key ?? 'platform');
            (modulesByProduct[pk] ||= []).push(m.module_code);
            visibleModules.push(m.module_code);
            if (m.display_name)
                internalKeyToBusinessLabel[m.module_code] = m.display_name;
        }
        return res.json({
...
app.get('/navigation/tree', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
...
app.get('/config-center/gateway/shell-override', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        if (mems.length === 0)
            return res.status(409).json({ blocked: 'NO_MEMBERSHIP' });
        const tenantId = mems[0].tenant_id;
        const { rows } = await pool.query(`SELECT settings FROM dos.tenants WHERE tenant_id = $1 LIMIT 1`, [tenantId]);
        const settings = rows[0]?.settings ?? {};
        const shellOverrides = settings.shellOverrides ?? {};
...
app.get('/module-kickstart-status', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        const { rows: modules } = await pool.query(`SELECT m.module_code, tpa.status AS activation_status, tpa.created_at AS kicked_at
         FROM dos.tenant_product_activation tpa
         JOIN dos.module_registry m ON m.product_key = tpa.product_key
...
async function callerRoles(sub, tenantId) {
    const { rows } = await pool.query(`SELECT DISTINCT role_code FROM platform_dauth.user_role_assignments
      WHERE user_id=$1 AND is_active=TRUE
        AND (tenant_id=$2 OR tenant_id='platform' OR tenant_id IS NULL)
        AND (expires_at IS NULL OR expires_at > NOW())
        AND revoked_at IS NULL`, [sub, tenantId]);
...
function isTenantAdmin(roles) {
    return roles.has('tenant_admin') || roles.has('tenant_owner');
}
async function callerTenantId(sub) {
    const { rows } = await pool.query(`SELECT tenant_id FROM dos.tenant_memberships
      WHERE user_id=$1 AND status='active'
      ORDER BY is_tenant_owner DESC, created_at ASC LIMIT 1`, [sub]);
    return rows[0]?.tenant_id ?? null;
}
// GET /config-center/settings?category=&module=&lang=
app.get('/config-center/settings', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        if (moduleCode) {
            ...
            filter += ` AND d.module_code = $${params.length}`;
        }
        ...
        const { rows } = await pool.query(`WITH defs AS (
       ...
       SELECT d.config_key AS key, d.label, d.description, d.module_code AS module,
        ...
        ORDER BY d.category NULLS LAST, d.config_key`, params);
        const items = rows.map(r => ({
            key: r.key,
            label: r.label,
...
app.get('/config-center/settings/:key', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        const { rows } = await pool.query(`SELECT d.id, d.config_key, d.label, d.description, d.allowed_scopes,
         ...
         FROM dos.config_definitions d WHERE d.config_key=$3 LIMIT 1`, [tenantId, c.sub, key]);
        if (rows.length === 0)
...
async function writeConfigValue(definitionId, configKey, scopeType, scopeId, value, updatedBy, source) {
    // Deactivate existing active row, then insert a fresh active row.
    await pool.query(`UPDATE dos.config_values SET is_active=FALSE
      WHERE definition_id=$1 AND scope_type=$2 AND scope_id=$3 AND is_active=TRUE`, [definitionId, scopeType, scopeId]);
    await pool.query(`INSERT INTO dos.config_values
       (config_key, scope_type, scope_id, value, definition_id, is_active, source, updated_by, updated_at)
     VALUES ($1, $2, $3, $4::jsonb, $5, TRUE, $6, $7, NOW())`, [configKey, scopeType, scopeId, JSON.stringify(value ?? null), definitionId, source, updatedBy]);
    await pool.query(`INSERT INTO platform_dsoc.audit_log
     ...
     ON CONFLICT DO NOTHING`, [scopeType === 'tenant' ? scopeId : 'platform', updatedBy, 'config_value', `${configKey}@${scopeType}:${scopeId}`,
        JSON.stringify({ scopeType, scopeId, source })]).catch(() => { });
}
...
app.put('/config-center/settings/:key', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        const { rows: defs } = await pool.query(`SELECT id, config_key, allowed_scopes, is_overridable, is_lockable
         FROM dos.config_definitions WHERE config_key=$1 LIMIT 1`, [key]);
        if (defs.length === 0)
            return res.status(404).json({ error: 'UNKNOWN_KEY' });
        const def = defs[0];
        ...
        if (requestedScope === 'tenant') {
            ...
            const { rows: locks } = await pool.query(`SELECT 1 FROM dos.config_locks WHERE definition_id=$1 AND scope_type='tenant' AND scope_id=$2`, [def.id, tenantId]);
            if (locks.length > 0)
                return res.status(423).json({ error: 'CONFIG_LOCKED' });
            ...
            if (def.is_overridable === false) {
                ...
                if (!isPlatformAdmin(roles2))
                    return res.status(403).json({ error: 'NOT_OVERRIDABLE' });
            }
...
app.put('/platform-config/settings/:key', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        const key = req.params.key;
        const { rows: defs } = await pool.query(`SELECT id, config_key FROM dos.config_definitions WHERE config_key=$1 LIMIT 1`, [key]);
        if (defs.length === 0)
            return res.status(404).json({ error: 'UNKNOWN_KEY' });
...
app.get('/platform-config/settings', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        const { rows } = await pool.query(`SELECT d.config_key AS key, d.label, d.description, d.module_code AS module,
              d.category, d.data_type, d.enum_values, d.allowed_scopes,
...
app.get('/config-center/permissions-catalog', async (req, res) => {
    const c = requireCaller(req, res);
    if (!c)
        return;
    try {
        ...
        if (moduleCode) {
            params.push(moduleCode);
            where = `WHERE module_code = $${params.length}`;
        }
        const { rows } = await pool.query(`SELECT permission_code AS code, module_code AS module, resource_type, action_type,
              label_en, label_ar, description_en, description_ar
         FROM platform_dauth.permissions ${where}
         ORDER BY module_code NULLS LAST, permission_code`, params);
...
app.all('/module-config/:moduleCode/*', async (req, res) => {
...
app.listen(PORT, '127.0.0.1', () => {
    ...
    if (process.env.TRIAL_LIFECYCLE_DISABLED !== 'true') {
        (0, trial_lifecycle_1.startTrialLifecycleSweeper)(pool);
...
app.post('/trials/sweep', async (req, res) => {
    try {
        const stats = await (0, trial_lifecycle_1.runTrialLifecycleSweep)(pool);
