...
export interface TrialSweepStats {
  scanned:          number;
  expiringMarked:   number;
  graceMarked:      number;
  suspendedMarked:  number;
  notificationsTried: number;
  auditRows:        number;
  errors:           number;
}
...
async function appendAudit(
  client: PoolClient,
  row: { tenantId: string; trialId: string; productCode: string; userId: string | null;
         action: string; oldStatus: string; newStatus: string; reason?: string;
  ...
  await client.query(
    ...
    [row.tenantId, row.trialId, row.userId, row.productCode, row.action,
...
async function tryNotify(
  tenantId: string,
  userId: string | null,
  type: 'trial_expiring' | 'trial_grace' | 'trial_suspended' | 'trial_activated',
...
export async function runTrialLifecycleSweep(
  ...
  logger: { info: (m: string, meta?: unknown) => void;
           ...
           error: (m: string, meta?: unknown) => void } = console as any,
): Promise<TrialSweepStats> {
  const stats: TrialSweepStats = {
  ...
  try {
    ...
    try {
      ...
      const expQ = await client.query<TrialRow>(
        `SELECT trial_id, tenant_id, product_code, status, starts_at, ends_at,
      ...
      for (const row of expQ.rows) {
        ...
        try {
          await client.query(
            `UPDATE dos.tenant_trials SET status='trial_expiring', updated_at=now()
          ...
          stats.errors += 1;
          logger.warn(`[trial-lifecycle] expiring transition failed for ${row.trial_id}: ${(err as Error).message}`);
        }
      }

      // 2. trial_expiring → trial_grace (now > endsAt)
      const grQ = await client.query<TrialRow>(
        `SELECT trial_id, tenant_id, product_code, status, starts_at, ends_at,
                grace_ends_at, created_by_user_id
           FROM dos.tenant_trials
      ...
      for (const row of grQ.rows) {
        ...
        try {
          await client.query(
            `UPDATE dos.tenant_trials SET status='trial_grace', updated_at=now()
              WHERE trial_id=$1 AND status='trial_expiring'`, [row.trial_id]);
          await client.query(
            `UPDATE dos.tenant_subscriptions SET status='grace', updated_at=now()
          ...
          await client.query('ROLLBACK');
          stats.errors += 1;
          logger.warn(`[trial-lifecycle] grace transition failed for ${row.trial_id}: ${(err as Error).message}`);
        }
      }

      // 3. trial_grace → trial_suspended (now > graceEndsAt) +
      //    deactivate trial entitlements (NOT platform_dna rows).
      const suQ = await client.query<TrialRow>(
        `SELECT trial_id, tenant_id, product_code, status, starts_at, ends_at,
                grace_ends_at, created_by_user_id
           FROM dos.tenant_trials
      ...
      for (const row of suQ.rows) {
        ...
        try {
          await client.query(
            `UPDATE dos.tenant_trials SET status='trial_suspended', suspended_at=now(), updated_at=now()
              WHERE trial_id=$1 AND status='trial_grace'`, [row.trial_id]);
          await client.query(
            `UPDATE dos.tenant_subscriptions SET status='suspended', updated_at=now()
          ...
          await client.query('ROLLBACK');
          stats.errors += 1;
          logger.warn(`[trial-lifecycle] suspend transition failed for ${row.trial_id}: ${(err as Error).message}`);
        }
      }

      logger.info('[trial-lifecycle] sweep complete', stats as unknown as Record<string, unknown>);
      return stats;
    } finally {
 ...
 * boot isn't blocked.
 */
export function startTrialLifecycleSweeper(pool: Pool): NodeJS.Timer {
  const interval = Math.max(15_000,
    Number(process.env.TRIAL_LIFECYCLE_INTERVAL_MS) || DEFAULT_INTERVAL_MS);
  const t = setTimeout(() => {
    void runTrialLifecycleSweep(pool).catch(err =>
      console.warn('[trial-lifecycle] initial sweep failed:', err?.message));
    setInterval(() => {
      void runTrialLifecycleSweep(pool).catch(err =>
        console.warn('[trial-lifecycle] sweep failed:', err?.message));
    }, interval).unref();
  }, 5_000);
  (t as unknown as { unref?: () => void }).unref?.();
  return t as unknown as NodeJS.Timer;
}
