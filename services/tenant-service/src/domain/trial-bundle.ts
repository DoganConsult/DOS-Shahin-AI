...
export interface TrialBundleInput {
  tenantId:          string;
  productCode:       string;
  ownerUserId:       string;
  signupDomain?:     string | null;
  source?:           string;            // default 'self_registration'
  planCode?:         string;
}

export interface TrialBundleOutput {
  trialId:        string;
  subscriptionId: string;
  productEntitlementId: string;
  moduleEntitlementIds: string[];
...
function readCsv(env: string | undefined, fallback: string[]): string[] {
  if (!env || env.trim() === '') return fallback;
  return env.split(',').map((s) => s.trim()).filter(Boolean);
}

/**
 * Read trial defaults from env. tenant-service receives these from Config OS
 * resolver (Phase B) at boot via runtime-config; until that wire is complete
 * we accept env directly so /register can already create real trial rows.
 *
 * Keys mirror config-schemas.ts TrialConfigSchema:
 *   TRIAL_DEFAULT_DAYS         — platform.trial.defaultDays
 *   TRIAL_GRACE_DAYS           — platform.trial.graceDays
 *   TRIAL_ALLOWED_MODULES      — product.shahin.trial.allowedModules (csv)
 *   TRIAL_MAX_USERS            — product.shahin.trial.maxUsers
 *   TRIAL_AI_CREDITS           — product.shahin.trial.aiCredits
 */
export interface ResolvedTrialConfig {
  defaultDays:    number;
  graceDays:      number;
  allowedModules: string[];
  maxUsers?:      number;
 ...
 */
export async function createTrialBundle(
  client: PoolClient,
  ...
  const existing = await client.query<{
    ...
    `SELECT trial_id, starts_at, ends_at, grace_ends_at
  ...
  if (existing.rows.length > 0) {
    ...
    const sub = await client.query<{ subscription_id: string }>(
      `SELECT subscription_id FROM dos.tenant_subscriptions
        WHERE tenant_id=$1 AND product_code=$2
          AND status IN ('trialing','active','past_due','grace')
        LIMIT 1`, [input.tenantId, input.productCode]);
    const prodEnt = await client.query<{ entitlement_id: string }>(
      `SELECT entitlement_id FROM dos.tenant_product_entitlements
        WHERE tenant_id=$1 AND product_code=$2 AND entitlement_status='active'
        LIMIT 1`, [input.tenantId, input.productCode]);
    const modEnts = await client.query<{ entitlement_id: string; module_code: string }>(
      `SELECT entitlement_id, module_code FROM dos.tenant_module_entitlements
        WHERE tenant_id=$1 AND product_code=$2 AND entitlement_status='active'`,
        [input.tenantId, input.productCode]);
  ...
  const moduleEntitlementIds: string[] = [];
  ...
  for (const moduleCode of cfg.allowedModules) {
    ...
    if (!isDna) limits[moduleCode] = trialLimits;
  }
...
export async function getTrialSummary(
  ...
  const trialQ = await client.query(
    `SELECT trial_id, tenant_id, product_code, plan_code, status,
            starts_at, ends_at, grace_ends_at, created_by_user_id,
            signup_domain, verification_status, source, metadata,
  ...
  const modsQ = await client.query(
    `SELECT module_code, limits_json, source
  ...
  const expiredQ = await client.query<{ module_code: string }>(
    `SELECT module_code FROM dos.tenant_module_entitlements
      WHERE tenant_id=$1 AND product_code=$2
        AND source='trial'
        AND entitlement_status IN ('expired','suspended','cancelled')`,
    [tenantId, productCode]);
