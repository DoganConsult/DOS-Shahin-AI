...
const DEFAULT_DISPOSABLE = new Set<string>([
...
const PUBLIC_FREE_DOMAINS = new Set<string>([
...
interface SlidingWindow { times: number[]; }
const ipWindow:     Map<string, SlidingWindow> = new Map();
const emailWindow:  Map<string, SlidingWindow> = new Map();
const domainWindow: Map<string, SlidingWindow> = new Map();

function pushAndCount(map: Map<string, SlidingWindow>, key: string, windowMs: number): number {
  const now = Date.now();
  let w = map.get(key);
  if (!w) { w = { times: [] }; map.set(key, w); }
  // Drop expired
...
function pruneAll(): void {
  ...
  for (const map of [ipWindow, emailWindow, domainWindow]) {
    for (const [k, w] of map.entries()) {
      while (w.times.length && w.times[0] < cutoff) w.times.shift();
      if (w.times.length === 0) map.delete(k);
    }
  }
}
 ...
 */
export function antiAbuse(deps: AntiAbuseDeps) {
  ...
  return async function antiAbuseMiddleware(req: Request, res: Response, next: NextFunction) {
    ...
    if (oneTrialPerDomain && !isPublicFree(domain)) {
      try {
        const { rows } = await deps.pool.query<{ trial_id: string; created_by_user_id: string | null }>(
          `SELECT trial_id, created_by_user_id FROM dos.tenant_trials
        ...
        console.warn(`[anti-abuse] domain-uniqueness check failed: ${(err as Error).message}`);
      }
    }
