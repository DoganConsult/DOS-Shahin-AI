...
export interface FgaTuple {
  user: string;     // e.g. 'user:<sub>' or 'tenant:<tenantId>' or 'platform:<id>'
  relation: string; // e.g. 'admin', 'member', 'owner', 'ui_admin', 'platform', 'tenant'
  object: string;   // e.g. 'tenant:<tenantId>' or 'ui_os_tenant:<tenantId>'
}

export type FgaWriteOutcome = 'ok' | 'partial' | 'unavailable' | 'disabled' | 'error';

export interface FgaWriteResult {
  outcome: FgaWriteOutcome;
  written: number;
  skipped: number;
  failed: number;
  message?: string;
 ...
 */
export async function writeTuples(tuples: FgaTuple[]): Promise<FgaWriteResult> {
  ...
  if (failed === 0) return { outcome: 'ok', written, skipped, failed };
 ...
 */
export function buildRegistrationTuples(args: {
  tenantId: string;
  userId: string;
  platformId?: string;
}): FgaTuple[] {
...
export function isOpenFgaEnforced(): boolean {
  return ENFORCE;
}
