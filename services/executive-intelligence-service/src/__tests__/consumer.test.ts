...
vi.mock('../events/publisher', () => ({
  ...
  publishExecutiveBriefingCreated: vi.fn().mockResolvedValue(undefined),
  publishExecutiveBriefingDistributed: vi.fn().mockResolvedValue(undefined),
  publishExecutiveAlertTriggered: vi.fn().mockResolvedValue(undefined),
}));

import { registerConsumers } from '../events/consumer';

describe('executive-intelligence-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["risk.updated","compliance.assessed","incident.created","analytics.kpi.threshold.breached"];
  ...
  it('handles analytics.kpi.threshold.breached without throwing', async () => {
    const handler = handlers.get('analytics.kpi.threshold.breached');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
      eventType: 'analytics.kpi.threshold.breached',
      tenantId: 't1',
      userId: 'u1',
      payload: { title: 'Test', category: 'auto', severity: 'medium', type: 'test' },
      timestamp: new Date().toISOString(),
