...
vi.mock('../domain/briefing.service', () => ({
...
import * as domainSvc from '../domain/briefing.service';
...
describe('briefing routes', () => {
  ...
  describe('GET /api/briefing/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/briefing/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/briefing/missing');
  ...
  describe('DELETE /api/briefing/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/briefing/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
