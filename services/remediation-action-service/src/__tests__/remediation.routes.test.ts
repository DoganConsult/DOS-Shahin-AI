...
vi.mock('../domain/remediation.service', () => ({
...
import router from '../routes/remediation.routes';
import * as domainSvc from '../domain/remediation.service';
...
describe('remediation routes', () => {
  ...
  describe('GET /api/remediation/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/remediation/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/remediation/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/remediation/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/remediation/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/remediation/', () => {
  ...
  describe('PUT /api/remediation/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/remediation/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/remediation/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/remediation/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
