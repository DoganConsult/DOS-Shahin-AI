...
vi.mock('../domain/action-item.service', () => ({
...
import router from '../routes/action-item.routes';
import * as domainSvc from '../domain/action-item.service';
...
function createApp() {
  ...
  app.use('/api/action-item', router);
  return app;
}
...
describe('action-item routes', () => {
  ...
  describe('GET /api/action-item/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/action-item/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/action-item/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/action-item/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/action-item/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/action-item/', () => {
  ...
  describe('PUT /api/action-item/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/action-item/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/action-item/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/action-item/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
