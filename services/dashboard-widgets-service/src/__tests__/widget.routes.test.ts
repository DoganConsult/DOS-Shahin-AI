...
import router from '../routes/widget.routes';
import * as domainSvc from '../domain/widget.service';
...
describe('widget routes', () => {
  ...
  describe('GET /api/widget/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/widget/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/widget/missing');
  ...
  describe('DELETE /api/widget/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/widget/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
