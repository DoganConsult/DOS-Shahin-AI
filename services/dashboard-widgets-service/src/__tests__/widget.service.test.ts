...
import * as svc from '../domain/widget.service';
...
describe('widget.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { title: 'test-title', type: 'test-type', data_source: 'test-data_source' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { title: 'x', type: 'x', data_source: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
