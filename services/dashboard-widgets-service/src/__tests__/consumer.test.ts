...
vi.mock('../events/publisher', () => ({
  ...
  publishWidgetCreated: vi.fn().mockResolvedValue(undefined),
  publishWidgetUpdated: vi.fn().mockResolvedValue(undefined),
  publishWidgetDataRefreshed: vi.fn().mockResolvedValue(undefined),
}));

import { registerConsumers } from '../events/consumer';

describe('dashboard-widgets-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["analytics.dashboard.created","risk.updated","compliance.assessed"];
    for (const event of expectedEvents) {
      expect(handlers.has(event), `Missing handler for ${event}`).toBe(true);
    }
  });

  it('handles analytics.dashboard.created without throwing', async () => {
    const handler = handlers.get('analytics.dashboard.created');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
      eventType: 'analytics.dashboard.created',
      tenantId: 't1',
      userId: 'u1',
