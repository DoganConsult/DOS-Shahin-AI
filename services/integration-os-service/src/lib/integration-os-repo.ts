...
// ── Phase 3 / L30 — integration domain logic: deterministic record evaluator.
...
export async function evaluateRecord(recordKey: string, ctx: RecordEvalCtx): Promise<RecordEvalResult> {
  ...
  await masterQuery(
    ...
    [rec ? (rec as { id: string }).id : null, recordKey, 'integration_evaluated',
     JSON.stringify({ ctx, decision, reason, version: row.version }), 'integration-os-service'],
  );
