...
vi.mock('../domain/integration.service', () => ({
...
import router from '../routes/integration.routes';
import * as domainSvc from '../domain/integration.service';
...
describe('integration routes', () => {
  ...
  describe('GET /api/integration/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/integration/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/integration/missing');
  ...
  describe('DELETE /api/integration/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/integration/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
