...
vi.mock('../events/publisher', () => ({
  ...
  publishIntegrationConnected: vi.fn().mockResolvedValue(undefined),
  publishIntegrationSyncCompleted: vi.fn().mockResolvedValue(undefined),
  publishIntegrationSyncFailed: vi.fn().mockResolvedValue(undefined),
  publishIntegrationDisconnected: vi.fn().mockResolvedValue(undefined),
}));

import { registerConsumers } from '../events/consumer';

describe('integrations-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["tenant.created","asset.created","vendor.created"];
    for (const event of expectedEvents) {
      expect(handlers.has(event), `Missing handler for ${event}`).toBe(true);
    }
  });

  it('handles tenant.created without throwing', async () => {
    const handler = handlers.get('tenant.created');
  ...
  it('handles asset.created without throwing', async () => {
    const handler = handlers.get('asset.created');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
  ...
  it('handles vendor.created without throwing', async () => {
    const handler = handlers.get('vendor.created');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
