...
import * as svc from '../domain/integration.service';
...
describe('integration.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { name: 'test-name', type: 'test-type', provider: 'test-provider' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { name: 'x', type: 'x', provider: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
