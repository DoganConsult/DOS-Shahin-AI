...
function createRedisBackend() {
    ...
    const getClient = async () => {
        ...
        try {
            ...
            initFailed = true;
            module_sdk_1.logger.warn('[user-service.rate-limiter] Redis backend unavailable, falling back to memory', {
                err: err?.message,
            });
        }
        return redis;
    ...
    return {
        ...
        check: async (key, max, windowMs) => {
            ...
            try {
                ...
                module_sdk_1.logger.warn('[user-service.rate-limiter] Redis check failed, using memory this call', {
                    err: err?.message,
                });
                return memoryBackend.check(key, max, windowMs);
            }
