...
vi.mock('../events/publisher', () => ({
  ...
  publishPrivacyAssessmentCreated: vi.fn().mockResolvedValue(undefined),
  publishPrivacyDpiaCompleted: vi.fn().mockResolvedValue(undefined),
  publishPrivacyBreachDetected: vi.fn().mockResolvedValue(undefined),
  publishPrivacyConsentUpdated: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock('../domain/privacy-assessment.service', () => ({
...
import router from '../routes/privacy-assessment.routes';
import * as domainSvc from '../domain/privacy-assessment.service';
...
function createApp() {
  ...
  app.use('/api/privacy-assessment', router);
  return app;
}

describe('privacy-assessment routes', () => {
  ...
  describe('GET /api/privacy-assessment/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/privacy-assessment/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/privacy-assessment/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/privacy-assessment/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/privacy-assessment/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/privacy-assessment/', () => {
  ...
  describe('PUT /api/privacy-assessment/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/privacy-assessment/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/privacy-assessment/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/privacy-assessment/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
