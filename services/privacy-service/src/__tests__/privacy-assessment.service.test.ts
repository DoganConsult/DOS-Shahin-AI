...
import * as svc from '../domain/privacy-assessment.service';

const mockQuery = safeQuery as ReturnType<typeof vi.fn>;
const TENANT = 'tenant-001';

describe('privacy-assessment.service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { title: 'test-title', type: 'test-type' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { title: 'x', type: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
