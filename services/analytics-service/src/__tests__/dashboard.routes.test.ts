...
vi.mock('../events/publisher', () => ({
  ...
  publishAnalyticsDashboardCreated: vi.fn().mockResolvedValue(undefined),
  publishAnalyticsKpiThresholdBreached: vi.fn().mockResolvedValue(undefined),
}));
...
import * as domainSvc from '../domain/dashboard.service';
...
describe('dashboard routes', () => {
  ...
  describe('GET /api/dashboard/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/dashboard/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/dashboard/missing');
  ...
  describe('DELETE /api/dashboard/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/dashboard/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
