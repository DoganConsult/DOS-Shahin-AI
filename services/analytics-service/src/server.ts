...
import { routes, grcQueryRoutes } from './routes/index';
...
const SERVICE_CODE = 'analytics-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    port: config.port,
    routes: [
      { path: '/api/analytics', router: routes },
      // W5 — dedicated /api/grc-query mount for <app-ai-insight-panel>.
      { path: '/api/grc-query', router: grcQueryRoutes },
    ],
    // Wave 2D: Module lifecycle registration
    modules: await loadModuleRegistrations('analytics'),
    healthChecks: {
      database: async () => {
        try {
      ...
      clickhouse: async () => clickHouseHealthCheck(),
    },
    onReady: () => {
      // Wave 2D: invoke module onBoot hooks
      invokeModuleLifecycleHooks(SERVICE_CODE, 'analytics').catch(() => {});
      // ClickHouse bootstrap — ensures DDL, starts flush timer, begins health probe loop.
      bootstrapClickHouse().catch((err) =>
        console.error(`[${SERVICE_CODE}] ClickHouse bootstrap failed:`, err),
      );
    },
  });
