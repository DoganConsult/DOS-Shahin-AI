...
import * as svc from '../domain/incident.service';
...
describe('incident.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { title: 'test-title', severity: 'test-severity' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { title: 'x', severity: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
