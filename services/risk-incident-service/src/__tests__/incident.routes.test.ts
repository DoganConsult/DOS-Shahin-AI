...
vi.mock('../domain/incident.service', () => ({
...
import * as domainSvc from '../domain/incident.service';
...
describe('incident routes', () => {
  ...
  describe('GET /api/incident/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/incident/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/incident/missing');
  ...
  describe('DELETE /api/incident/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/incident/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
