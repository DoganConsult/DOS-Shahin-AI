...
import router from '../routes/risk.routes';
import * as domainSvc from '../domain/risk.service';
...
describe('risk routes', () => {
  ...
  describe('GET /api/risk/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/risk/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/risk/missing');
  ...
  describe('DELETE /api/risk/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/risk/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
