...
vi.mock('../events/publisher', () => ({
  ...
  publishPortalCreated: vi.fn().mockResolvedValue(undefined),
  publishPortalAccessGranted: vi.fn().mockResolvedValue(undefined),
  publishPortalSubmissionReceived: vi.fn().mockResolvedValue(undefined),
}));

import { registerConsumers } from '../events/consumer';

describe('portals-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["vendor.created","vendor.assessment.completed","compliance.assessed"];
  ...
  it('handles vendor.assessment.completed without throwing', async () => {
    const handler = handlers.get('vendor.assessment.completed');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
      eventType: 'vendor.assessment.completed',
      tenantId: 't1',
      userId: 'u1',
      payload: { title: 'Test', category: 'auto', severity: 'medium', type: 'test' },
      timestamp: new Date().toISOString(),
