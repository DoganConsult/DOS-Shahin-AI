...
import router from '../routes/portal.routes';
import * as domainSvc from '../domain/portal.service';
...
describe('portal routes', () => {
  ...
  describe('GET /api/portal/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/portal/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/portal/missing');
  ...
  describe('DELETE /api/portal/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/portal/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
