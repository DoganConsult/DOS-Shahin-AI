...
import * as svc from '../domain/portal.service';
...
describe('portal.service', () => {
  ...
  describe('create', () => {
    it('inserts and returns new record', async () => {
      ...
      const result = await svc.create(TENANT, { name: 'test-name', type: 'test-type' } as any);
    ...
    it('propagates database errors on insert', async () => {
      ...
      await expect(svc.create(TENANT, { name: 'x', type: 'x' } as any)).rejects.toThrow('unique violation');
    });
  });
