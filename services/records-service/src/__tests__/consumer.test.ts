...
vi.mock('../events/publisher', () => ({
  ...
  publishRecordCreated: vi.fn().mockResolvedValue(undefined),
  publishRecordArchived: vi.fn().mockResolvedValue(undefined),
  publishRecordRetentionExpiring: vi.fn().mockResolvedValue(undefined),
}));

import { registerConsumers } from '../events/consumer';

describe('records-service event consumers', () => {
  ...
  it('registers all expected event subscriptions', () => {
    const expectedEvents = ["evidence.submitted","policy.published","audit.finding.created"];
    for (const event of expectedEvents) {
      expect(handlers.has(event), `Missing handler for ${event}`).toBe(true);
    }
  });

  it('handles evidence.submitted without throwing', async () => {
    const handler = handlers.get('evidence.submitted');
  ...
  it('handles policy.published without throwing', async () => {
    const handler = handlers.get('policy.published');
    expect(handler).toBeDefined();
    await expect(handler!({
      eventId: 'evt-1',
