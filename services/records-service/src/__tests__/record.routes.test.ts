...
import router from '../routes/record.routes';
import * as domainSvc from '../domain/record.service';
...
describe('record routes', () => {
  ...
  describe('GET /api/record/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/record/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/record/missing');
  ...
  describe('DELETE /api/record/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/record/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
