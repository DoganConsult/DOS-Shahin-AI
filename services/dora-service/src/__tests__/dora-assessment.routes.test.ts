...
vi.mock('../events/publisher', () => ({
  ...
  publishDoraAssessmentCreated: vi.fn().mockResolvedValue(undefined),
  publishDoraAssessmentCompleted: vi.fn().mockResolvedValue(undefined),
  publishDoraIctRiskIdentified: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock('../domain/dora-assessment.service', () => ({
...
import router from '../routes/dora-assessment.routes';
import * as domainSvc from '../domain/dora-assessment.service';
...
function createApp() {
  ...
  app.use('/api/dora-assessment', router);
  return app;
}

describe('dora-assessment routes', () => {
  ...
  describe('GET /api/dora-assessment/', () => {
    it('returns list', async () => {
      const res = await request(app).get('/api/dora-assessment/');
      expect(res.status).toBe(200);
      expect(res.body).toBeDefined();
    });
  });

  describe('GET /api/dora-assessment/:id', () => {
    it('returns single record', async () => {
      const res = await request(app).get('/api/dora-assessment/r1');
      expect(res.status).toBe(200);
    });

    it('returns 404 for missing record', async () => {
      (domainSvc.getById as any).mockResolvedValueOnce(null);
      const res = await request(app).get('/api/dora-assessment/missing');
      expect(res.status).toBe(404);
    });
  });

  describe('POST /api/dora-assessment/', () => {
  ...
  describe('PUT /api/dora-assessment/:id', () => {
    it('updates existing record', async () => {
      const res = await request(app)
        .put('/api/dora-assessment/r1')
        .send({ title: 'Updated' });
      expect([200, 204, 400]).toContain(res.status);
    });
  });

  describe('DELETE /api/dora-assessment/:id', () => {
    it('deletes existing record', async () => {
      const res = await request(app).delete('/api/dora-assessment/r1');
      expect([200, 204, 400]).toContain(res.status);
    });
  });
