...
import doraExtensionsRouter from './routes/dora-extensions.routes';
...
app.use(createHealthRouter('dora', {
  db: dbHealthCheck,
...
const SERVICE_CODE = 'dora-service';
...
async function main() {
  ...
  const { start } = await createServiceServer({
    ...
    port: config.port,
    routes: [
      // Phase-12E wire-closure: extensions router mounted before the main
      // aggregator so /backups and /test-results resolve to the new
      // dedicated handlers.
      { path: '/api/dora', router: doraExtensionsRouter },
      { path: '/api/dora', router: routes },
    ],
    modules: await loadModuleRegistrations('dora'),
    healthChecks: {
      database: async () => {
        try {
    ...
    onReady: () => {
      invokeModuleLifecycleHooks(SERVICE_CODE, 'dora').catch(() => {});
    },
  });
