...
/** DOS Master Phase 2 — admin-console-bff → release-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const releaseOsProxyRouter = Router();

const TARGET = process.env.DOS_RELEASE_OS_SERVICE_URL || 'http://127.0.0.1:4029';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/release-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'release-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

releaseOsProxyRouter.use(requireAdmin);
releaseOsProxyRouter.all(/.*/, forward);
