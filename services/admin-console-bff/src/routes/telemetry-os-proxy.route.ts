...
/** DOS Master Phase 2 — admin-console-bff → telemetry-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const telemetryOsProxyRouter = Router();

const TARGET = process.env.DOS_TELEMETRY_OS_SERVICE_URL || 'http://127.0.0.1:4051';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/telemetry-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'telemetry-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

telemetryOsProxyRouter.use(requireAdmin);
telemetryOsProxyRouter.all(/.*/, forward);
