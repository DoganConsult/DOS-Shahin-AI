...
/** DOS Master Phase 2 — admin-console-bff → billing-os-service proxy.
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/billing-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'billing-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

billingOsProxyRouter.use(requireAdmin);
billingOsProxyRouter.all(/.*/, forward);
