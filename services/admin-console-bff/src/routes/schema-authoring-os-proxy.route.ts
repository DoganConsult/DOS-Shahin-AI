...
/** DOS Master Phase 2 — admin-console-bff → schema-authoring-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const schemaAuthoringOsProxyRouter = Router();

const TARGET = process.env.DOS_SCHEMA_AUTHORING_OS_SERVICE_URL || 'http://127.0.0.1:4027';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/schema-authoring-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'schema-authoring-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

schemaAuthoringOsProxyRouter.use(requireAdmin);
schemaAuthoringOsProxyRouter.all(/.*/, forward);
