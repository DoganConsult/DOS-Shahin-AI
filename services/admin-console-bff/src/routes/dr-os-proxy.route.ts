...
/** DOS Master Phase 2 — admin-console-bff → dr-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const drOsProxyRouter = Router();

const TARGET = process.env.DOS_DR_OS_SERVICE_URL || 'http://127.0.0.1:4053';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/dr-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'dr-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

drOsProxyRouter.use(requireAdmin);
drOsProxyRouter.all(/.*/, forward);
