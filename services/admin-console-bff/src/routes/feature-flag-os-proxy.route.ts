...
/** DOS Master Phase 2 — admin-console-bff → feature-flag-os-service proxy.
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/feature-flag-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'feature-flag-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

featureFlagOsProxyRouter.use(requireAdmin);
featureFlagOsProxyRouter.all(/.*/, forward);
