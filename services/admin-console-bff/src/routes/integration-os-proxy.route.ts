...
/** DOS Master Phase 2 — admin-console-bff → integration-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const integrationOsProxyRouter = Router();

const TARGET = process.env.DOS_INTEGRATION_OS_SERVICE_URL || 'http://127.0.0.1:4021';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/integration-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'integration-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

integrationOsProxyRouter.use(requireAdmin);
integrationOsProxyRouter.all(/.*/, forward);
