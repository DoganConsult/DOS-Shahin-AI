...
/** DOS Master Phase 2 — admin-console-bff → ai-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const aiOsProxyRouter = Router();

const TARGET = process.env.DOS_AI_OS_SERVICE_URL || 'http://127.0.0.1:4019';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/ai-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'ai-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

aiOsProxyRouter.use(requireAdmin);
aiOsProxyRouter.all(/.*/, forward);
