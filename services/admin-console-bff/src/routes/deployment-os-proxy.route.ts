...
/** DOS Master Phase 2 — admin-console-bff → deployment-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const deploymentOsProxyRouter = Router();

const TARGET = process.env.DOS_DEPLOYMENT_OS_SERVICE_URL || 'http://127.0.0.1:4028';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/deployment-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'deployment-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

deploymentOsProxyRouter.use(requireAdmin);
deploymentOsProxyRouter.all(/.*/, forward);
