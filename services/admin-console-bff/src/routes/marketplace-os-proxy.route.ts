...
/** DOS Master Phase 2 — admin-console-bff → marketplace-os-service proxy.
 *  Doctrine: Article 4 (admin BFF mediates every admin-FE call). */
export const marketplaceOsProxyRouter = Router();

const TARGET = process.env.DOS_MARKETPLACE_OS_SERVICE_URL || 'http://127.0.0.1:4052';
...
function forward(req: Request, res: Response): void {
  const url = new URL(`${TARGET}/api/admin/marketplace-os${req.url}`);
  ...
  upstream.on('error', (e) => res.status(502).json({ error: 'marketplace-os_upstream_error', detail: String(e.message) }));
  if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') upstream.write(JSON.stringify(req.body ?? {}));
  upstream.end();
}

marketplaceOsProxyRouter.use(requireAdmin);
marketplaceOsProxyRouter.all(/.*/, forward);
